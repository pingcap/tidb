...
func TestAddIndexIngestUniqueKey(t *testing.T) {
	...
	tk.MustContainErrMsg("alter table t add unique index idx(b);", "found index conflict records in table t, index name is 't.idx', unique key is '[1]'")
	...
	tk.MustExec("split table t by ('m');")
	tk.MustContainErrMsg("alter table t add unique index idx(b, c);", "found index conflict records in table t, index name is 't.idx'")
}
...
func TestAddUniqueIndexDuplicatedError(t *testing.T) {
	...
	tk.MustContainErrMsg("ALTER TABLE `b1cce552` ADD unique INDEX `65290727` (`4c74082f`, `d9337060`, `8069da7b`);", "found index conflict records in table")
}
