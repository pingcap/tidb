...
func (s *mockGCSSuite) TestImportFromSelectChecksum() {
	s.prepareAndUseDB("from_select")
	s.tk.MustExec("create table src(id int)")
	s.tk.MustExec("insert into src values (1), (2), (3)")
	// A table without a primary key uses the hidden, non-clustered row ID.
	...
	s.tk.MustQuery("select count(*), sum(id) from dst").Check(testkit.Rows("3 6"))
}
