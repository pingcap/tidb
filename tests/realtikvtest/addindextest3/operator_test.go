...
func TestBackfillOperators(t *testing.T) {
	...
	// Test IndexIngestOperator.
	{
		...
		require.NoError(t, err)
		defer bd.Close()
		bcCtx, err := ingest.NewBackendCtxBuilder(ctx, dom.GetPDClient(), realJob).Build(cfg, bd)
		require.NoError(t, err)
		defer bcCtx.Close()
...
func TestBackfillOperatorPipelineException(t *testing.T) {
	...
	require.NoError(t, err)
	defer bd.Close()
	bcCtx, err := ingest.NewBackendCtxBuilder(context.Background(), dom.GetPDClient(), realJob).Build(cfg, bd)
	require.NoError(t, err)
	defer bcCtx.Close()
	mockEngine := ingest.NewMockEngineInfo(nil)
