...
func (s *mockGCSSuite) TestShowJob() {
	...
	testfailpoint.EnableCall(s.T(), "github.com/pingcap/tidb/pkg/dxf/framework/taskexecutor/syncAfterSubtaskFinish",
		func() {
			...
			if newVal == 1 {
				...
				s.NoError(failpoint.Disable("github.com/pingcap/tidb/pkg/dxf/framework/taskexecutor/syncAfterSubtaskFinish"))
			}
		},
	)
	s.server.CreateObject(fakestorage.Object{
	...
	tkUser1 := testkit.NewTestKit(s.T(), s.store)
	s.NoError(tkUser1.Session().Auth(&auth.UserIdentity{Username: "test_show_job1", Hostname: "localhost"}, nil, nil, nil))
	tkUser1.MustExec("USE test_show_job")
	tkUser1.MustQuery(fmt.Sprintf(`import into t4 FROM 'gs://test-show-job/t.csv?endpoint=%s' with group_key='%s'`, gcsEndpoint, groupKey))
	jobID4 := importer.TestLastImportJobID.Load()
	tkUser2 := testkit.NewTestKit(s.T(), s.store)
	s.NoError(tkUser2.Session().Auth(&auth.UserIdentity{Username: "test_show_job2", Hostname: "localhost"}, nil, nil, nil))
	tkUser2.MustExec("USE test_show_job")
	tkUser2.MustQuery(fmt.Sprintf(`import into t5 FROM 'gs://test-show-job/t.csv?endpoint=%s' with group_key='%s'`, gcsEndpoint, groupKey))
	jobID5 := importer.TestLastImportJobID.Load()

	rawList := tkUser2.MustQuery(fmt.Sprintf(`show raw import jobs where group_key = '%s'`, groupKey)).Rows()
	s.Len(rawList, 1)
	s.Equal(strconv.FormatInt(jobID5, 10), rawList[0][0])
...
func (s *mockGCSSuite) TestCancelJob() {
	...
	testfailpoint.EnableCall(s.T(), "github.com/pingcap/tidb/pkg/dxf/importinto/syncAfterJobStarted",
		func() {
			close(syncCh)
		},
	)
	...
	s.NoError(failpoint.Disable("github.com/pingcap/tidb/pkg/dxf/importinto/syncAfterJobStarted"))
	wg := sync.WaitGroup{}
	wg.Add(1)
	tk2 := testkit.NewTestKit(s.T(), s.store)
	testfailpoint.EnableCall(s.T(), "github.com/pingcap/tidb/pkg/dxf/importinto/syncBeforePostProcess",
		func(jobID int64) {
			go func() {
				defer wg.Done()
	...
	s.NoError(failpoint.Disable("github.com/pingcap/tidb/pkg/dxf/importinto/syncBeforePostProcess"))
...
func (s *mockGCSSuite) TestJobFailWhenDispatchSubtask() {
	...
	testfailpoint.Enable(s.T(), "github.com/pingcap/tidb/pkg/dxf/importinto/failWhenDispatchPostProcessSubtask", "return(true)")
	testfailpoint.Enable(s.T(), "github.com/pingcap/tidb/pkg/executor/importer/setLastImportJobID", `return(true)`)
	s.NoError(s.tk.Session().Auth(&auth.UserIdentity{Username: "root", Hostname: "localhost"}, nil, nil, nil))
