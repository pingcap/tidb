...
func TestBasicTxnState(t *testing.T) {
	...
	require.Equal(t, tk.Session().GetSessionVars().ConnectionID, info.Process.ConnectionID)
	require.Equal(t, "", info.Process.Username)
	require.Equal(t, "test", info.Process.CurrentDB)
	require.Equal(t, startTS, info.StartTS)
