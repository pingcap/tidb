...
func insertWorker(ctx *suiteContext, tk *testkit.TestKit) error {
	...
	if !isSkippedError(err, ctx) {
		log.Info("workload insert failed", zap.String("category", "add index test"), zap.String("sql", insStr), zap.Error(err))
		return err
	}
	if rs != nil {
...
func updateWorker(ctx *suiteContext, tk *testkit.TestKit) error {
	...
	if !isSkippedError(err, ctx) {
		log.Info("workload update failed", zap.String("category", "add index test"), zap.String("sql", upStr), zap.Error(err))
		return err
	}
	if rs != nil {
		if err := rs.Close(); err != nil {
...
func deleteWorker(ctx *suiteContext, wl *workload, tk *testkit.TestKit) error {
	...
	if !isSkippedError(err, ctx) {
		log.Info("workload delete failed", zap.String("category", "add index test"), zap.String("sql", delStr), zap.Error(err))
		return err
	}
	if rs != nil {
		if err := rs.Close(); err != nil {
