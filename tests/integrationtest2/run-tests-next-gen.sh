...
TIKV_WORKER_DATA_DIR="./data/tikv_worker_data"
...
# Next-Gen Variables
NEXT_GEN=$NEXT_GEN
CONFIG_DIR="$(realpath "$(dirname "${BASH_SOURCE[0]}")/next-gen-configs")"
...
TIKV_WORKER_LOG_FILE="./logs/tikv_worker.log"
...
function start_pd_server() {
    client_port=${1:-2379}
    peer_port=${2:-2380}
    data_dir=${3:-$PD_DATA_DIR}
    log_dir=${4:-$PD_LOG_FILE}
    config_file=${5:-$CONFIG_DIR/pd.toml}
	...
	$PD_BIN --name="pd" --data-dir="$data_dir" --log-file="$log_dir" \
          ...
          --config="$config_file" &
	sleep 5  # Wait for PD to start
}

# TiKV Server Configuration
function start_tikv_server() {
    pd_client_port=${1:-2379}
    tikv_port=${2:-20160}
    tikv_status_port=${3:-20180}
    data_dir=${4:-$TIKV_DATA_DIR}
    log_file=${5:-$TIKV_LOG_FILE}
    config_file=${6:-$CONFIG_DIR/tikv.toml}
...
# TiKV Worker
function start_tikv_worker() {
    pd_client_port=${1:-2379}
    tikv_worker_port=${2:-19000}
    data_dir=${4:-$TIKV_WORKER_DATA_DIR}
    log_file=${5:-$TIKV_WORKER_LOG_FILE}
    config_file=${6:-$CONFIG_DIR/tikv-worker.toml}

	echo "Starting TiKV worker..."
    mkdir -p $data_dir

	$TIKV_BIN --pd-endpoints="http://127.0.0.1:$pd_client_port" \
        --addr="127.0.0.1:$tikv_worker_port" \
		--data-dir="$data_dir" \
		--log-file="$log_file" \
		--config="$config_file" &
    sleep 5  # Wait for tikv-worker to connect to PD
}

# TiFlash Server Configuration
function start_tiflash_server() {
...
function start_tidb_server()
{
    ...
    keyspace_name=${5:-SYSTEM}
    config_file="config.toml"
    start_options="-P $tidb_port -status $tidb_status_port -config $config_file -store tikv -path ${pd_client_addr} -keyspace-name $keyspace_name"
    if [ -n "$NEXT_GEN" ] && [ "$NEXT_GEN" != "0" ] && [ "$NEXT_GEN" != "false" ]; then
        start_options="$start_options "
    fi

    echo "start tidb-server, log file: $log_file"
    $tidb_server $start_options > $log_file 2>&1 &
    SERVER_PID=$!
...
function start_minio() {
    ...
    "$MINIO_BIN_PATH" server "$data_base_dir" --address ":$MINIO_PORT" > "$MINIO_LOG_FILE" 2>&1 &
    MINIO_PID=$!
...
function start_tidb_cluster()
{
    ...
    mkdir -pv "${MINIO_DATA_DIR}/next-gen-test"
    start_minio "${MINIO_DATA_DIR}"
	...
	start_tidb_server "127.0.0.1:$pd_client_port" $tidb_port $tidb_status_port $TIDB_LOG_FILE SYSTEM
