...
function run_mysql_tester()
{
    tests=("$@")
    if [ $record -eq 1 ]; then
        $mysql_tester -port "$UPSTREAM_PORT" -downstream "root:@tcp(127.0.0.1:$DOWNSTREAM_PORT)/test" --check-error=true --record $tests
    else
        echo "run integration test cases: $tests"
        $mysql_tester -port "$UPSTREAM_PORT" -downstream "root:@tcp(127.0.0.1:$DOWNSTREAM_PORT)/test" --check-error=true $tests
    fi
}
...
if [ -z "$tests" ]; then
    # Safely handle filenames by using find -print0 and xargs -0 to avoid splitting on spaces
    non_ticdc_cases=($(find t -type f -name '*.test' ! -path "t/ticdc/*" -print0 | xargs -0 -n1 basename | sed 's/\.test$//'))
    ticdc_cases=($(find t/ticdc -type f -name '*.test' -print0 | xargs -0 -n1 basename | sed 's/\.test$//'))
else
    ticdc_cases=($(echo "$tests" | grep '^t/ticdc' | sed 's/\.test$//'))
    non_ticdc_cases=($(echo "$tests" | grep -v '^t/ticdc' | sed 's/\.test$//'))
fi
