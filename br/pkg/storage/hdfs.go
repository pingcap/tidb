...
func getHdfsBin() (string, error) {
	...
	if !ok {
		return "", newWrappedError(berrors.ErrEnvNotSpecified, "please specify environment variable HADOOP_HOME")
	}
...
func (s *HDFSStorage) WriteFile(_ context.Context, name string, data []byte) error {
	...
	if err != nil {
		return newWrappedError(err, string(out))
	}
	return nil
}
...
func (*HDFSStorage) ReadFile(_ context.Context, _ string) ([]byte, error) {
	return nil, newWrappedError(berrors.ErrUnsupportedOperation, "currently HDFS backend only support rawkv backup")
}
...
func (s *HDFSStorage) FileExists(_ context.Context, name string) (bool, error) {
	...
	if err != nil {
		return false, newWrappedError(err, string(out))
	}
...
func (*HDFSStorage) DeleteFile(_ context.Context, _ string) error {
	return newWrappedError(berrors.ErrUnsupportedOperation, "currently HDFS backend only support rawkv backup")
}
