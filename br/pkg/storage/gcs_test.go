...
func TestMultiWriter(t *testing.T) {
	...
	require.NoError(t, err)
	w, err := stg.Create(ctx, "testgcs", nil)
	require.NoError(t, err)
	...
	for i := 0; i < 10*1024; i++ {
		_, err := w.Write(ctx, []byte("01234567890123456789012345678901234567890123456789"))
		require.NoError(t, err)
	}
	err = w.Close(ctx)
	require.NoError(t, err)
	rd, err := stg.Open(ctx, "testgcs", nil)
	require.NoError(t, err)
	buf := make([]byte, 10)
	...
	startOffset := int64(101)
	rd, err = stg.Open(ctx, "testgcs", &ReaderOption{StartOffset: &startOffset})
	require.NoError(t, err)
	buf = make([]byte, 10)
	_, err = io.ReadFull(rd, buf)
	require.NoError(t, err)
	require.Equal(t, []byte("1234567890"), buf)

	startOffset = int64(100101)
	rd, err = stg.Open(ctx, "testgcs", &ReaderOption{StartOffset: &startOffset})
	require.NoError(t, err)
	buf = make([]byte, 10)
	_, err = io.ReadFull(rd, buf)
