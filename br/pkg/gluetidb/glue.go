...
func (gs *tidbSession) RefreshMeta(
	...
	d := domain.GetDomain(gs.se).DDLExecutor()
	gs.se.SetValue(sessionctx.QueryString,
		fmt.Sprintf("REFRESH META INVOLVED_DB_ID=%d INVOLVED_TABLE_ID=%d INVOLVED_DB_NAME=%s INVOLVED_TABLE_NAME=%s", args.InvolvedDBID, args.InvolvedTableID, args.InvolvedDBName, args.InvolvedTableName))
