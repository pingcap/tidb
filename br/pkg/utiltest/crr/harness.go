...
type TestHarness struct {
	upstreamStorage   storage.ExternalStorage
	downstreamStorage storage.ExternalStorage
	tc                *TestContext
...
func newLocalTestHarness(
	...
	upstreamStorage, err := storage.NewLocalStorage(upstreamDir)
	if err != nil {
		return nil, fmt.Errorf("create local upstream storage at %s: %w", upstreamDir, err)
	}
	downstreamStorage, err := storage.NewLocalStorage(downstreamDir)
	if err != nil {
	...
	advancer := streamhelper.NewCheckpointAdvancer(pd)
...
func assertReadableFile(ctx context.Context, storage storage.ExternalStorage, name string) error {
