...
func FetchVersion(ctx context.Context, db utils.QueryExecutor) (string, error) {
	...
	if err == nil && tidbReleaseVersionWithCommitIdRegex.FindString(versionInfo) != "" {
...
var (
	...
	// `select tidb_version()` result contains commit id as release version instead of tidb version
	tidbReleaseVersionWithCommitIdRegex = regexp.MustCompile(`Release Version:\s*v\d+\.\d+\.\d+([0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?`)
)
