...
// RunBackupGCP starts a backup task to backup volume via gcp volume snapshot.
func RunBackupGCP(c context.Context, g glue.Glue, cfg *BackupConfig) error {
	cfg.Adjust()
	...
	defer func() {
		if finished {
			summary.Log("GCP backup success", zap.Int64("size", totalSize), zap.Uint64("resolved_ts", resolvedTs), zap.Uint64("backup_start_ts", backupStartTs))
		} else {
			summary.Log("GCP backup failed, please check the log for details.")
		}
	}()
	...
	backupInfo := &config.VolSnapBackupMeta{}
...
func saveMetaFile(c context.Context, backupInfo *config.VolSnapBackupMeta, externalStorage storage.ExternalStorage) error {
