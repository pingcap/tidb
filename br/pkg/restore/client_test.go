...
func TestCreateTables(t *testing.T) {
	m := mc
	g := gluetidb.New()
	client := restore.NewRestoreClient(m.PDClient, m.PDHTTPCli, nil, defaultKeepaliveCfg, false, 1)
...
func TestNeedCheckTargetClusterFresh(t *testing.T) {
	...
	g := gluetidb.New()
	client := restore.NewRestoreClient(cluster.PDClient, cluster.PDHTTPCli, nil, defaultKeepaliveCfg, false, 1)
...
func TestGetTSWithRetry(t *testing.T) {
	t.Run("PD leader is healthy:", func(t *testing.T) {
		...
		client := restore.NewRestoreClient(pDClient, nil, nil, defaultKeepaliveCfg, false, 1)
...
func TestPreCheckTableTiFlashReplicas(t *testing.T) {
	...
	client := restore.NewRestoreClient(fakePDClient{
		stores: mockStores,
	}, nil, nil, defaultKeepaliveCfg, false, 1)
	err := client.Init(g, m.Storage)
	require.NoError(t, err)
