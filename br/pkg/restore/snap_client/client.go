...
func (rc *SnapClient) setMergeOptionForTables(ctx context.Context, createdTables []*restoreutils.CreatedTable) error {
	...
	for _, ct := range createdTables {
		...
		if len(table.PartitionMergeOptionAllowed) > 0 &&
			...
			// Create a map from old partition name to new partition definition for ID lookup
			oldPartitionMap := make(map[string]*model.PartitionDefinition)
			for i := range table.Info.Partition.Definitions {
				def := &table.Info.Partition.Definitions[i]
				oldPartitionMap[def.Name.O] = def
			}
	...
	for i := 0; i < totalRules; i += utils.LabelRuleBatchSize {
		end := i + utils.LabelRuleBatchSize
		if end > totalRules {
			end = totalRules
		}
		batch := rulesToSet[i:end]
		...
		for _, rule := range batch {
			err := infosync.PutLabelRule(ctx, rule)
			if err != nil {
				return errors.Annotatef(err, "failed to put label rule %s for merge_option to PD", rule.ID)
			}
		}
	}
