...
func (importer *SnapFileImporter) getKeyRangeForFiles(
	...
	for _, files := range filesGroup {
		for _, f := range files.Files {
			start, end, err = getRangeFn(f, files.RewriteRule)
			if err != nil {
...
func (importer *SnapFileImporter) ImportSSTFiles(
	...
	var (
		hintFilesCount  int    = 0
		filesTotalKvs   uint64 = 0
		filesTotalBytes uint64 = 0
	)
	for _, files := range filesGroup {
		hintFilesCount += len(files.Files)
		for _, f := range files.Files {
			filesTotalKvs += f.TotalKvs
			filesTotalBytes += f.TotalBytes
		}
	}
	...
	summary.CollectSuccessUnit(summary.TotalKV, 1, filesTotalKvs)
	summary.CollectSuccessUnit(summary.TotalBytes, 1, filesTotalBytes)
	return nil
}
...
func (importer *SnapFileImporter) downloadSST(
	...
	for _, files := range filesGroup {
		for _, file := range files.Files {
			req, sstMeta, skip, err := importer.buildDownloadRequest(file, files.RewriteRule, regionInfo, cipher)
			if err != nil {
