...
func (rc *SnapClient) replaceTemporaryTableToSystable(ctx context.Context, ti *model.TableInfo, db *database) error {
	...
	if restoreutils.IsStatsTable(dbName, tableName) {
		return nil
	}

	if restoreutils.IsPlanReplayerTables(dbName, tableName) {
		return nil
	}

	if restoreutils.IsUnrecoverableTable(dbName, tableName) {
		return nil
	}
...
func CheckSysTableCompatibility(dom *domain.Domain, tables []*metautil.Table) error {
	...
	for _, table := range tables {
		...
		if ok && decodedSysDBName.L == mysql.SystemDB && restoreutils.IsSysPrivilegeTable(table.Info.Name.L) {
