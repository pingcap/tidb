...
func TestCheckSysTableCompatibility(t *testing.T) {
	...
	mockedDBTI.Columns = append(dbTI.Columns, &model.ColumnInfo{Name: pmodel.NewCIStr("new-name")})
	_, err = snapclient.CheckSysTableCompatibility(cluster.Domain, []*metautil.Table{{
		DB:   tmpSysDB,
		Info: mockedDBTI,
	}}, false)
...
func TestCheckPrivilegeTableRowsCollateCompatibility(t *testing.T) {
	...
	downstreamDBTable, err := restore.GetTableSchema(cluster.Domain, pmodel.NewCIStr("mysql"), pmodel.NewCIStr("db"))
	require.NoError(t, err)
	downstreamTablesTable, err := restore.GetTableSchema(cluster.Domain, pmodel.NewCIStr("mysql"), pmodel.NewCIStr("tables_priv"))
	require.NoError(t, err)
	downstreamColumnsTable, err := restore.GetTableSchema(cluster.Domain, pmodel.NewCIStr("mysql"), pmodel.NewCIStr("columns_priv"))
	require.NoError(t, err)
	// case 1: privilege db
	mse.MustExecute(CreateDBSQL)
	backupTable, err := restore.GetTableSchema(cluster.Domain, pmodel.NewCIStr("__TiDB_BR_Temporary_mysql"), pmodel.NewCIStr("db"))
	require.NoError(t, err)
	...
	backupTable, err = restore.GetTableSchema(cluster.Domain, pmodel.NewCIStr("__TiDB_BR_Temporary_mysql"), pmodel.NewCIStr("tables_priv"))
	require.NoError(t, err)
	...
	backupTable, err = restore.GetTableSchema(cluster.Domain, pmodel.NewCIStr("__TiDB_BR_Temporary_mysql"), pmodel.NewCIStr("columns_priv"))
	require.NoError(t, err)
...
>>>>>>> 57bbce668b8 (session: add upgrade policy for analyze distsql concurrency (#65424))
}
