...
func TestAddIngestRecorder(t *testing.T) {
	dbInfo := &model.DBInfo{Name: model.NewCIStr(SchemaName)}
	...
	builder, err := infoschema.NewBuilder(nil, nil, infoschema.NewData()).InitWithDBInfos(allSchemas, nil, nil, 0)
	require.NoError(t, err)
	is := builder.Build(math.MaxInt64)
