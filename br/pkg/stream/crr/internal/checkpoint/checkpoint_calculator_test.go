...
type recordingUpstreamStorage struct {
	inner interface {
		WalkDir(ctx context.Context, opt *storage.WalkOption, fn func(path string, size int64) error) error
		ReadFile(ctx context.Context, name string) ([]byte, error)
		URI() string
	}
	uri      string
	walkOpts []storage.WalkOption
}
...
func (s *recordingUpstreamStorage) WalkDir(
	...
	if opt == nil {
		s.walkOpts = append(s.walkOpts, storage.WalkOption{})
	} else {
