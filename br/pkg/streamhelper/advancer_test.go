...
func TestTickWritesGlobalCheckpointToStorage(t *testing.T) {
	...
	restoreFactory := streamhelper.SetGlobalCheckpointStorageFactoryForTest(
		func(ctx context.Context, backend *backup.StorageBackend, options *storage.ExternalStorageOptions) (storage.ExternalStorage, error) {
			createCount.Inc()
			storage, err := storage.New(ctx, backend, options)
			if err != nil {
				return nil, err
			}
			return writeRecordStorage{
				ExternalStorage: &countingStorage{ExternalStorage: storage, closeCount: closeCount},
...
func TestTickTimesOutGlobalCheckpointStorageWrite(t *testing.T) {
	...
	adv := streamhelper.NewCheckpointAdvancer(env)
	adv.UpdateConfigWith(func(cfg *config.Config) {
