...
func TestTickWritesGlobalCheckpointToStorage(t *testing.T) {
	...
	restoreFactory := streamhelper.SetGlobalCheckpointStorageFactoryForTest(
		func(ctx context.Context, backend *backup.StorageBackend, opts *storage.ExternalStorageOptions) (storage.ExternalStorage, error) {
			createCount.Inc()
			storage, err := storage.New(ctx, backend, opts)
			if err != nil {
				return nil, err
			}
