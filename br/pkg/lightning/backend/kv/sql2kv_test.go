...
func TestEncode(t *testing.T) {
	...
	// Strict mode with meta column
	strictMode, err = lkv.NewTableKVEncoder(tbl, &lkv.SessionOptions{
		SQLMode:   mysql.ModeStrictAllTables,
		Timestamp: 1234567890,
		SysVars: map[string]string{
			"tidb_write_by_ticdc": "1",
		},
	}, nil, logger)
	require.NoError(t, err)
	pairs, err = strictMode.Encode(logger, rows, 1, []int{0, 1}, "1.csv", 1234)
	...
	require.Equal(t, pairs, lkv.MakeRowFromKvPairs([]common.KvPair{
		{
			Key:   []uint8{0x74, 0x80, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x5f, 0x72, 0x80, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1},
			Val:   []uint8{0x8, 0x2, 0x8, 0x2, 0x8, 0x0, 0x9, 0x1},
			RowID: 2,
		},
	}))
	...
	require.Equal(t, pairs, lkv.MakeRowFromKvPairs([]common.KvPair{
		{
			Key:   []uint8{0x74, 0x80, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x5f, 0x72, 0x80, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1},
			Val:   []uint8{0x8, 0x2, 0x8, 0xfe, 0x1, 0x8, 0x0, 0x9, 0x1},
			RowID: 1,
		},
	}))
}
	...
	require.Equal(t, pairs, lkv.MakeRowFromKvPairs([]common.KvPair{
		{
			...
			Val: []uint8{
				0x80,     // version
				0x0,      // flag = 0 = not big
				0x2, 0x0, // number of not null columns = 2
				0x0, 0x0, // number of null columns = 0
				0x0, 0x1, // column IDs = [0, 1]
				0x1, 0x0, // not null offsets = [1]
				0x2, 0x0, // not null offsets = [2]
				0x1, 0x7f, // column version = 127 (10000000 clamped to TINYINT)
			},
			RowID: 1,
		},
	}))
