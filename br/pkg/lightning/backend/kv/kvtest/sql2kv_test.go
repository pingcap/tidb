...
func TestEncodeDoubleAutoIncrement(t *testing.T) {
	...
	require.Equal(t, tbl.Allocators(lkv.GetSession4test(encoder)).Get(autoid.AutoIncrementType).Base(), int64(70))
}
...
func SetUpTest(b *testing.B) *benchSQL2KVSuite {
	...
	require.NoError(b, err)
	encoder, err := lkv.NewTableKVEncoder(tbl, &lkv.SessionOptions{SysVars: map[string]string{"tidb_row_format_version": "2"}})
	require.NoError(b, err)
	logger := log.Logger{Logger: zap.NewNop()}
