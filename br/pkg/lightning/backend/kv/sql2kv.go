...
func (kvcodec *tableKVEncoder) Encode(row []types.Datum, rowID int64, columnPermutation []int, offset int64) (encode.Row, error) {
	...
	for i, col := range cols {
		...
		if err != nil {
			return nil, logKVConvertFailed(kvcodec.logger, row, j, col.ToInfo(), err)
		}
	...
	if common.TableHasAutoRowID(meta) {
		...
		if err != nil {
			return nil, logKVConvertFailed(kvcodec.logger, row, j, ExtraHandleColumnInfo, err)
		}
		record = append(record, value)
	...
	if len(kvcodec.genCols) > 0 {
		if errCol, err := evaluateGeneratedColumns(kvcodec.se, record, cols, kvcodec.genCols); err != nil {
			return nil, logEvalGenExprFailed(kvcodec.logger, row, errCol, err)
		}
	}
