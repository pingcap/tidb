...
func (local *local) WriteToTiKV(
	...
	flushKVs := func() error {
		for i := range clients {
			if err := local.writeLimiter.WaitN(ctx, allPeers[i].StoreId, int(size)); err != nil {
				return errors.Trace(err)
			}
...
func (local *local) writeAndIngestByRanges(ctx context.Context, engine *Engine, ranges []Range, regionSplitSize int64, regionSplitKeys int64, pausePdSchedulerScope string) error {
	...
	log.FromContext(ctx).Info("the ranges Length write to tikv", zap.Int("Length", len(ranges)), zap.String("pausePdSchedulerScope", pausePdSchedulerScope))
...
func (local *local) ImportEngine(ctx context.Context, engineUUID uuid.UUID, regionSplitSize, regionSplitKeys, minRegionNum int64, pausePdSchedulerScope string) error {
	...
	log.FromContext(ctx).Info("start import engine", zap.Stringer("uuid", engineUUID),
		zap.Int("ranges", len(ranges)), zap.Int64("count", lfLength), zap.Int64("size", lfTotalSize), zap.String("pausePdSchedulerScope", pausePdSchedulerScope))
