...
type AbstractBackend interface {
	...
	ImportEngine(ctx context.Context, engineUUID uuid.UUID, regionSplitSize, regionSplitKeys, minRegionNum int64, pausePdSchedulerScope string) error
...
// this method.
func (be Backend) UnsafeImportAndReset(ctx context.Context, engineUUID uuid.UUID, regionSplitSize, regionSplitKeys, minRegionNum int64, pausePdSchedulerScope string) error {
	...
	if err := closedEngine.Import(ctx, regionSplitSize, regionSplitKeys, minRegionNum, pausePdSchedulerScope); err != nil {
		return err
	}
...
func (engine *ClosedEngine) Import(ctx context.Context, regionSplitSize, regionSplitKeys, minRegionNum int64, pausePdSchedulerScope string) error {
	var err error

	for i := 0; i < importMaxRetryTimes; i++ {
		task := engine.logger.With(zap.Int("retryCnt", i)).Begin(zap.InfoLevel, "import")
		err = engine.backend.ImportEngine(ctx, engine.uuid, regionSplitSize, regionSplitKeys, minRegionNum, pausePdSchedulerScope)
