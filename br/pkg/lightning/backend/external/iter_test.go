...
func TestMergeKVIter(t *testing.T) {
	...
	for i, filename := range filenames {
		...
		for _, kv := range data[i] {
			err = kvStore.AddData(getEncodedData([]byte(kv[0]), []byte(kv[1])))
			require.NoError(t, err)
		}
	...
	iter, err := NewMergeKVIter(ctx, filenames, []uint64{0, 0, 0}, trackStore, 5, 1000)
	require.NoError(t, err)
...
func TestAllEmpty(t *testing.T) {
	...
	trackStore := &trackOpenMemStorage{MemStorage: memStore}
	iter, err := NewMergeKVIter(ctx, []string{filenames[0]}, []uint64{0}, trackStore, 5, 1000)
	require.NoError(t, err)
	require.EqualValues(t, 0, trackStore.opened.Load())
	require.False(t, iter.Next())
	require.NoError(t, iter.Error())
	...
	iter, err = NewMergeKVIter(ctx, filenames, []uint64{0, 0}, trackStore, 5, 1000)
	require.NoError(t, err)
	require.EqualValues(t, 0, trackStore.opened.Load())
	require.False(t, iter.Next())
	require.NoError(t, iter.Close())
...
func testMergeIterSwitchMode(t *testing.T, f func([]byte, int) []byte) {
	...
	iter, err := NewMergeKVIter(context.Background(), dataNames, offsets, st, 2048, 1000)
	require.NoError(t, err)
...
func TestHotspot(t *testing.T) {
	...
	for i, filename := range filenames {
		...
		for _, k := range keys[i] {
			err = kvStore.AddData(getEncodedData([]byte(k), value))
			require.NoError(t, err)
		}
		err = writer.Close(ctx)
		require.NoError(t, err)
	...
	iter, err := NewMergeKVIter(ctx, filenames, make([]uint64, len(filenames)), store, 26, 1000)
	require.NoError(t, err)
...
func TestMemoryUsageWhenHotspotChange(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		for j := 0; j < checkHotspotPeriod; j++ {
			...
			err = kvStore.AddData(getEncodedData([]byte(key), []byte(val)))
			require.NoError(t, err)
			cur++
		}
		for j := 0; j <= 12; j++ {
			key := fmt.Sprintf("key999%06d", cur+j)
			err = kvStore.AddData(getEncodedData([]byte(key), largeChunk))
			require.NoError(t, err)
		}
		err = writer.Close(ctx)
		require.NoError(t, err)
	...
	iter, err := NewMergeKVIter(ctx, filenames, make([]uint64, len(filenames)), store, 1024, 1000)
	require.NoError(t, err)
	i := 0
	for cur > 0 {
		cur--
