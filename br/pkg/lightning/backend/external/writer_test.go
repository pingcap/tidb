...
func TestWriterDuplicateDetect(t *testing.T) {
	...
	writer := NewWriterBuilder().
		...
		SetOnCloseFunc(func(summary *WriterSummary) {
			startKey = summary.Min
			endKey = summary.Max.Next()
		}).
		Build(memStore, "/test", "0")
	kvCount := 20
	for i := 0; i < kvCount; i++ {
...
func TestWriterMultiFileStat(t *testing.T) {
	...
	allStatFiles := make([]string, 9)
	for i := range allDataFiles {
		allDataFiles[i] = fmt.Sprintf("/test/0/%d", i)
		allStatFiles[i] = fmt.Sprintf("/test/0_stat/%d", i)
	}
	...
	require.NoError(t, err)
	require.Equal(t, 1, len(summary.MultipleFilesStats))
	expected = MultipleFilesStat{
		MinKey: []byte("key01"),
		MaxKey: []byte("key24"),
		Filenames: [][2]string{
