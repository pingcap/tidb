...
func TestAddKeyValueMaintainRangeProperty(t *testing.T) {
	...
	err = kvStore.addEncodedData(nil, getEncodedData(k1, v1))
	require.NoError(t, err)
	...
	err = kvStore.addEncodedData(nil, getEncodedData(k2, v2))
	require.NoError(t, err)
	...
	err = kvStore.addEncodedData(nil, getEncodedData(k3, v3))
	require.NoError(t, err)
	require.Len(t, rc.props, 1)
...
func TestKVReadWrite(t *testing.T) {
	...
	for i := 0; i < kvCnt; i++ {
		...
		rand.Read(values[i])
		err = kvStore.addEncodedData(nil, getEncodedData(keys[i], values[i]))
		require.NoError(t, err)
	}
	err = writer.Close(ctx)
	require.NoError(t, err)
