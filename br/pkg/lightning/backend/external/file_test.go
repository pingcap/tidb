...
func TestAddKeyValueMaintainRangeProperty(t *testing.T) {
	...
	err = kvStore.AddData(getEncodedData(k1, v1))
	require.NoError(t, err)
	...
	err = kvStore.AddData(getEncodedData(k2, v2))
	require.NoError(t, err)
	...
	err = kvStore.AddData(getEncodedData(k3, v3))
	require.NoError(t, err)
	require.Len(t, rc.props, 1)
...
func TestKVReadWrite(t *testing.T) {
	...
	for i := 0; i < kvCnt; i++ {
		...
		rand.Read(values[i])
		err = kvStore.AddData(getEncodedData(keys[i], values[i]))
		require.NoError(t, err)
	}
	err = writer.Close(ctx)
	require.NoError(t, err)
