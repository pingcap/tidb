...
func TestOnefileWriterBasic(t *testing.T) {
	...
	kvReader, err := newKVReader(ctx, "/test/0/one-file", memStore, 0, bufSize, 0)
	require.NoError(t, err)
	for i := 0; i < kvCnt; i++ {
...
func checkOneFileWriterStatWithDistance(t *testing.T, kvCnt int, keysDistance uint64, memSizeLimit uint64, prefix string) {
	...
	bufSize := rand.Intn(100) + 1
	kvReader, err := newKVReader(ctx, "/"+prefix+"/0/one-file", memStore, 0, bufSize, 0)
	require.NoError(t, err)
	for i := 0; i < kvCnt; i++ {
		key, value, err := kvReader.nextKV()
		require.NoError(t, err)
...
func TestMergeOverlappingFilesInternal(t *testing.T) {
	...
	kvReader, err := newKVReader(ctx, "/test2/mergeID/one-file", memStore, 0, 100, 0)
	require.NoError(t, err)
	for i := 0; i < kvCount; i++ {
		key, value, err := kvReader.nextKV()
		require.NoError(t, err)
...
func TestOnefileWriterManyRows(t *testing.T) {
	...
	bufSize := rand.Intn(100) + 1
	kvReader, err := newKVReader(ctx, "/test2/mergeID/one-file", memStore, 0, bufSize, 0)
	require.NoError(t, err)
	for i := 0; i < kvCnt; i++ {
		key, value, err := kvReader.nextKV()
		require.NoError(t, err)
