...
func (s *mdLoaderSetup) pruneViewPlaceholders(ctx context.Context) {
	...
	for _, dbMeta := range s.loader.dbs {
		...
		for _, tableMeta := range dbMeta.Tables {
			key := filter.Table{Schema: tableMeta.DB, Name: tableMeta.Name}
			if _, ok := s.viewIndexMap[key]; ok {
				prunedCount++
	...
	if prunedCount > 0 {
		log.FromContext(ctx).Info("pruned view placeholder tables",
	...
	for _, dbMeta := range s.loader.dbs {
		for i, tableMeta := range dbMeta.Tables {
			s.tableIndexMap[filter.Table{Schema: tableMeta.DB, Name: tableMeta.Name}] = i
		}
	}
}
