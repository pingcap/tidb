...
// ZapRanges make zap fields for logging Range slice.
func ZapProgressRanges(ranges []ProgressRange) zapcore.Field {
	return zap.Object("progress ranges", progressRangesMarshaler(ranges))
}

type progressRangesMarshaler []ProgressRange

func (rs progressRangesMarshaler) MarshalLogArray(encoder zapcore.ArrayEncoder) error {
	for _, r := range rs {
		encoder.AppendString(r.Origin.String())
	}
	return nil
}

func (rs progressRangesMarshaler) MarshalLogObject(encoder zapcore.ObjectEncoder) error {
	total := len(rs)
	encoder.AddInt("total", total)
	elements := make([]string, 0, total)
	for _, r := range rs {
		elements = append(elements, r.Origin.String())
	}
	_ = encoder.AddArray("ranges", logutil.AbbreviatedArrayMarshaler(elements))
	...
	totalFile := 0
	for _, r := range rs {
		for _, f := range r.Origin.Files {
			totalKV += f.GetTotalKvs()
			totalBytes += f.GetTotalBytes()
			totalSize += f.GetSize_()
		}
		totalFile += len(r.Origin.Files)
	}
