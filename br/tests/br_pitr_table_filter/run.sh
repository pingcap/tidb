...
test_index_filter() {
    ...
    # run admin checksum after restore
    echo "Running admin checksum after restore for btree_index_table..."
    run_sql "ADMIN CHECKSUM TABLE $DB.btree_index_table" || {
        echo "Admin checksum failed for btree_index_table after restore"
        exit 1
    }
    ...
    # run admin checksum after restore for all tables
    echo "Running admin checksum after restore for all tables..."
    run_sql "ADMIN CHECKSUM TABLE $DB.btree_index_table" || {
        echo "Admin checksum failed for btree_index_table after restore"
        exit 1
    }
    run_sql "ADMIN CHECKSUM TABLE $DB.hash_index_table" || {
        echo "Admin checksum failed for hash_index_table after restore"
        exit 1
    }
    run_sql "ADMIN CHECKSUM TABLE $DB.multi_index_table" || {
        echo "Admin checksum failed for multi_index_table after restore"
        exit 1
    }
