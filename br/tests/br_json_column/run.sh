...
run_sql "CREATE TABLE $DB.t( \
  ...
  j json, \
  j1 char(20) as (j ->> '\$.a'), \
  index idx((json_type(j)))
)"

run_sql "INSERT INTO $DB.t(j) VALUES ('{\"a\": \"b\"}');"
run_sql "INSERT INTO $DB.t(j) VALUES ('{\"a\": \"c\"}');"
...
run_curl https://$TIDB_STATUS_ADDR/ddl/history | grep -E '/\*from\(br\)\*/CREATE TABLE.*json_type.*'
