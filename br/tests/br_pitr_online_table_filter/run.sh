...
# Run the table filter tests in online mode
# Pass all necessary environment variables to the table filter script
INTEGRATION_TEST=1 \
TEST_DIR="$TEST_DIR" \
TEST_NAME="$TEST_NAME" \
PD_ADDR="$PD_ADDR" \
...
TIKV_ADDR="$TIKV_ADDR" \
BR_LOG_TO_TERM=1 \
bash "$CUR/../br_pitr_table_filter/run.sh" --online
