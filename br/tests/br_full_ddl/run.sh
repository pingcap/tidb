...
run_br restore full -s "local://$TEST_DIR/$DB" --pd $PD_ADDR --log-file $LOG --fast-load-sys-tables=false --use-checkpoint=false --check-requirements=false  || { cat $LOG; exit 1; }
export GO_FAILPOINTS=""
