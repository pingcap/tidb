...
run_lightning -d "tests/$TEST_NAME/data_sql" --check-only sample=2,1000 > $out_file_name 2>&1 || true
...
echo ">>> check-only=normal & check-requirements=true"
run_lightning -d "tests/$TEST_NAME/data_sql" --check-requirements=true --check-only normal > $out_file_name
check_contains "tidb lightning exit successfully"
run_sql "$check_db_sql"
check_contains "s: 0"
...
run_lightning -d "tests/$TEST_NAME/data_sql" --check-only sample=1,2 > $out_file_name
...
run_lightning -d "tests/$TEST_NAME/data_csv" --check-only sample=1,2 > $out_file_name
check_contains "All checks have been passed"
# check using gbk charset
run_lightning --config "tests/$TEST_NAME/config_gbk.toml" -d "tests/$TEST_NAME/data_csv" --check-only sample=1,1 > $out_file_name
check_contains "All checks have been passed"
run_lightning --config "tests/$TEST_NAME/config_gbk.toml" -d "tests/$TEST_NAME/data_csv" --check-only sample=1,2 > $out_file_name
check_contains "Total sample of 2 rows of data checked, 1 errors found."
check_contains "Some checks failed, please check the log for more information."
check_contains "Log file location: /tmp/backup_restore_test/lightning.log"
...
run_lightning --config "tests/$TEST_NAME/config_gbk.toml" -d "tests/$TEST_NAME/data_multiple_csv" --check-only sample > $out_file_name
check_contains "Total sample of 3 rows of data checked"
run_lightning --config "tests/$TEST_NAME/config_gbk.toml" -d "tests/$TEST_NAME/data_multiple_csv" --check-only sample=1,1000 > $out_file_name
