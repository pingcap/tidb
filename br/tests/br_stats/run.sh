...
run_sql "CREATE TABLE ${DB}1.br_stats_partition (id INT NOT NULL, store_id INT NOT NULL, store_n INT NOT NULL, UNIQUE KEY i1(id) GLOBAL, KEY i2(store_id), KEY i3(store_n)) PARTITION BY RANGE (store_id) (PARTITION p0 VALUES LESS THAN (6), PARTITION p1 VALUES LESS THAN MAXVALUE);"
for j in $(seq 9); do
    run_sql "INSERT INTO ${DB}1.br_stats_partition values ($j, $j, $j);"
done
