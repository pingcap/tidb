...
func (ts *tidbTestTopSQLSuite) TestTopSQLCPUProfile(c *C) {
	...
	cases1 := []struct {
		...
		{sql: "select * from t use index(idx) where a<10000;", planRegexp: ".*IndexLookUp.*"},
	...
	cases2 := []struct {
		...
		{prepare: "replace into t1 (b) values (?);", args: []interface{}{1}, planRegexp: ""},
		{prepare: "select * from t1 use index(idx) where a<?;", args: []interface{}{10000}, planRegexp: ".*IndexLookUp.*"},
	...
	cases3 := []struct {
		...
		{prepare: "select * from t2 use index(idx) where a<?;", args: []interface{}{10000}, planRegexp: ".*IndexLookUp.*"},
