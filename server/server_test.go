...
func (cli *testServerClient) runFailedTestMultiStatements(c *C) {
	cli.runTestsOnNewDB(c, nil, "FailedMultiStatements", func(dbt *DBTest) {
		...
		c.Assert(err.Error(), Equals, "Error 1105: client has multi-statement capability disabled. Run 'SET GLOBAL tidb_allow_multi_statement=1' after you understand the security risk")

		dbt.mustExec("SET tidb_allow_multi_statement=1;")

		// Server can not run identical test to runTestMultiStatements
