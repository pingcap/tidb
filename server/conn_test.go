...
type ConnSerialTestSuite struct {
	dom   *domain.Domain
	store kv.Storage
}

var _ = SerialSuites(&ConnSerialTestSuite{})

func (ts *ConnSerialTestSuite) SetUpSuite(c *C) {
	testleak.BeforeTest()
	var err error
	ts.store, err = mockstore.NewMockStore()
	c.Assert(err, IsNil)
...
func (ts *ConnSerialTestSuite) TearDownSuite(c *C) {
	ts.dom.Close()
	ts.store.Close()
	testleak.AfterTest(c)()
}

func (ts *ConnSerialTestSuite) TestHandleQueryCleanUp(c *C) {
	store, err := mockstore.NewMockStore()
	c.Assert(err, IsNil)
	defer store.Close()
	dom, err := session.BootstrapSession(store)
	...
	_, err = se.Execute(context.Background(), "create table test.t(id int)")
	c.Assert(err, IsNil)
	var outBuffer bytes.Buffer
	tidbdrv := NewTiDBDriver(ts.store)
	cfg := newTestConfig()
	...
	testcases := []struct {
		...
		{
			sql: "update test.t set id = 2 where id = 1",
		},
		{
	...
	for _, testcase := range testcases {
		err := cc.handleQuery(context.Background(), testcase.sql)
		c.Assert(err, IsNil)
