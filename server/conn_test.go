...
func (ts *ConnTestSuite) TestTiFlashFallback(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/pingcap/tidb/store/tikv/locate/tikvStoreSendReqResult", "return(\"requestTiFlashError\")"), IsNil)
	testFallbackWork(c, tk, cc, "select sum(a) from t")
	c.Assert(failpoint.Disable("github.com/pingcap/tidb/store/tikv/locate/tikvStoreSendReqResult"), IsNil)
