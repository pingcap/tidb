...
func (s *testUtilSuite) TestDumpBinaryTime(c *C) {
	...
	t, err := types.ParseTimestamp(sc, "0000-00-00 00:00:00.000000")
	c.Assert(err, IsNil)
	...
	t, err = types.ParseDatetime(sc, "0000-00-00 00:00:00.000000")
	c.Assert(err, IsNil)
	d = dumpBinaryDateTime(nil, t)
	c.Assert(d, DeepEquals, []byte{0})
	t, err = types.ParseDatetime(sc, "1993-07-13 01:01:01.000000")
	c.Assert(err, IsNil)
	d = dumpBinaryDateTime(nil, t)
	// 201 & 7 composed to uint16 1993 (litter-endian)
	c.Assert(d, DeepEquals, []byte{7, 201, 7, 7, 13, 1, 1, 1})

	t, err = types.ParseDate(sc, "0000-00-00")
	c.Assert(err, IsNil)
	d = dumpBinaryDateTime(nil, t)
	c.Assert(d, DeepEquals, []byte{0})
	t, err = types.ParseDate(sc, "1992-06-01")
	c.Assert(err, IsNil)
	d = dumpBinaryDateTime(nil, t)
