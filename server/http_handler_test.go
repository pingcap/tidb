...
func (ts *HTTPHandlerTestSerialSuite) TestPostSettings(c *C) {
	...
	form.Set("tidb_enable_mutation_checker", "1")
	...
	c.Assert(err, IsNil)
	c.Assert(val, Equals, variable.On)
	val, err = variable.GetGlobalSystemVar(se.GetSessionVars(), variable.TiDBEnableMutationChecker)
	c.Assert(err, IsNil)
	c.Assert(val, Equals, variable.On)
	...
	form.Set("tidb_enable_mutation_checker", "0")
	resp, err = ts.formStatus("/settings", form)
	c.Assert(err, IsNil)
	c.Assert(resp.StatusCode, Equals, http.StatusOK)
