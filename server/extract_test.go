...
func TestExtractServerHanlder(t *testing.T) {
	setupStmtSummary()
	...
	require.NoError(t, err)
	extractServerHandler := &extractTaskServeHandler{
	...
	router.Handle("/extract_task/dump", extractServerHandler)

	u, err := url.Parse(fmt.Sprintf("/extract_task/dump?begin=%s&end=%s&type=plan",
		startTime.Format(types.DateFormat), endTime.Format(types.DateFormat)))
	require.NoError(t, err)
	resp0, err := client.fetchStatus(u.RawQuery)
	require.NoError(t, err)
	defer func() {
		require.NoError(t, resp0.Body.Close())
	}()
	require.Equal(t, resp0.StatusCode, int(http.StatusOK))
}
