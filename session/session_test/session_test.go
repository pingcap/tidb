...
func TestSetVarHint(t *testing.T) {
	...
	require.NoError(t, tk.Session().GetSessionVars().SetSystemVar("tikv_client_read_timeout", "0"))
	...
	tk.MustExec("set @@tikv_client_read_timeout = 5")
	tk.MustQuery("SELECT /*+ set_var(tikv_client_read_timeout=1) */ @@tikv_client_read_timeout;").Check(testkit.Rows("1"))
	...
	tk.MustQuery("SELECT /*+ set_var(tikv_client_read_timeout=xyz) */ @@tikv_client_read_timeout;").Check(testkit.Rows("5"))
	require.Len(t, tk.Session().GetSessionVars().StmtCtx.GetWarnings(), 1)
	require.Equal(t, "[variable:1232]Incorrect argument type to variable 'tikv_client_read_timeout'", tk.Session().GetSessionVars().StmtCtx.GetWarnings()[0].Err.Error())
