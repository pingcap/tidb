...
func (s *testSessionSuite) TestAutoIncrementID(c *C) {
	...
	// TiDB nature : _tidb_rowid will also consume the autoID when auto_increment column is not primary key.
	// Behaviour like MySQL using the MaxUint64 and MaxInt64 as autoID upper limit will cause insert fail if
	// the values is 9223372036854775806. Because autoID will want alloc 9223372036854775807.
