...
func (s *testSessionSuite) TestSetPDClientDynmaicOption(c *C) {
	...
	if *withTiKV {
		...
		err = tk.ExecToErr("set global tidb_tso_client_batch_max_wait_time = -0.1;")
		c.Assert(err, NotNil)
		c.Assert(err, ErrorMatches, ".*invalid max TSO batch wait interval.*")
		err = tk.ExecToErr("set global tidb_tso_client_batch_max_wait_time = -0.01;")
		c.Assert(err, NotNil)
		c.Assert(err, ErrorMatches, ".*invalid max TSO batch wait interval.*")
		err = tk.ExecToErr("set global tidb_tso_client_batch_max_wait_time = 10.01;")
		c.Assert(err, NotNil)
		c.Assert(err, ErrorMatches, ".*invalid max TSO batch wait interval.*")
		err = tk.ExecToErr("set global tidb_tso_client_batch_max_wait_time = 10.1;")
		c.Assert(err, NotNil)
		c.Assert(err, ErrorMatches, ".*invalid max TSO batch wait interval.*")
		...
		tk.MustQuery("show warnings").Check(testutil.RowsWithSep("|", "Warning|1292|Truncated incorrect tidb_tso_client_batch_max_wait_time value: '-0.01'"))
		tk.MustExec("set global tidb_tso_client_batch_max_wait_time = 10.01;")
		tk.MustQuery("show warnings").Check(testutil.RowsWithSep("|", "Warning|1292|Truncated incorrect tidb_tso_client_batch_max_wait_time value: '10.01'"))
