...
func (s *testSessionSuite3) TestUnique(c *C) {
	...
	c.Assert(err.Error(), Equals, "previous statement: INSERT INTO test.test (id,val) VALUES (1,1): [kv:1062]Duplicate entry '1' for key 'PRIMARY'")

	_, err = tk1.Exec("commit")
	c.Assert(err, NotNil)
	c.Assert(terror.ErrorEqual(err, kv.ErrKeyExists), IsTrue, Commentf("err %v", err))
	c.Assert(err.Error(), Equals, "previous statement: INSERT INTO test.test (id,val) VALUES (2,2): [kv:1062]Duplicate entry '2' for key 'val'")
