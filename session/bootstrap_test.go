...
func (s *testBootstrapSuite) TestUpgradeVersion66(c *C) {
	...
	err = txn.Commit(context.Background())
	c.Assert(err, IsNil)
	mustExecSQL(c, seV3, "update mysql.tidb set variable_value='65' where variable_name='tidb_server_version'")
	mustExecSQL(c, seV3, "set @@global.tidb_track_aggregate_memory_usage = 0")
	mustExecSQL(c, seV3, "commit")
	unsetStoreBootstrapped(store.UUID())
	ver, err := getBootstrapVersion(seV3)
	c.Assert(err, IsNil)
	...
	domV56, err := BootstrapSession(store)
	c.Assert(err, IsNil)
	defer domV56.Close()
	seV56 := newSession(c, store, s.dbName)
	ver, err = getBootstrapVersion(seV56)
	c.Assert(err, IsNil)
	c.Assert(ver, Equals, int64(currentBootstrapVersion))
	r := mustExecSQL(c, seV56, `select @@global.tidb_track_aggregate_memory_usage, @@session.tidb_track_aggregate_memory_usage`)
	req := r.NewChunk()
	c.Assert(r.Next(ctx, req), IsNil)
	c.Assert(req.NumRows(), Equals, 1)
	row := req.GetRow(0)
