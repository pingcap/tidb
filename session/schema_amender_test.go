...
func prepareTestData(se *session, mutations *tikv.PlainMutations, oldTblInfo table.Table, newTblInfo table.Table,
	...
	for _, op := range expectedAmendOps {
		...
		genIndexKV := func(inputRow []types.Datum) ([]byte, []byte) {
			...
			c.Assert(err, IsNil)
			idxVal, err = tablecodec.GenIndexValuePortal(se.sessionVars.StmtCtx, newTblInfo.Meta(), info.indexInfoAtCommit.Meta(), false, info.indexInfoAtCommit.Meta().Unique, false, indexDatums, kvHandle, nil)
			c.Assert(err, IsNil)
