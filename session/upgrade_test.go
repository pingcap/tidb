...
func TestUpgradeVersion75(t *testing.T) {
	...
	tk.MustExec("update mysql.tidb set variable_value='74' where variable_name='tidb_server_version'")
	tk.MustExec("commit")
	tk.MustExec("ALTER TABLE mysql.user DROP PRIMARY KEY")
	tk.MustExec("ALTER TABLE mysql.user MODIFY COLUMN Host CHAR(64)")
	tk.MustExec("ALTER TABLE mysql.user ADD PRIMARY KEY(Host, User)")
	session.UnsetStoreBootstrapped(store.UUID())
	ver, err := session.GetBootstrapVersion(seV74)
	require.NoError(t, err)
	require.Equal(t, int64(74), ver)
	r, err := tk.Exec(`desc mysql.user`)
	require.NoError(t, err)
	req := r.NewChunk(nil)
	row := req.GetRow(0)
	...
	ver, err = session.GetBootstrapVersion(tk2.Session())
	require.NoError(t, err)
	require.Equal(t, session.CurrentBootstrapVersion, ver)
	r, err = tk2.Exec(`desc mysql.user`)
	require.NoError(t, err)
	req = r.NewChunk(nil)
	row = req.GetRow(0)
	require.NoError(t, r.Next(ctx, req))
