...
func TestFailStatementCommitInRetry(t *testing.T) {
	...
	s.setup(func(err error) { require.NoError(t, err) })
	defer s.teardown(func(err error) { require.NoError(t, err) })
...
func TestGetTSFailDirtyStateInretry(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("tikvclient/mockGetTSErrorInRetry", `1*return(true)->return(false)`))
...
func TestKillFlagInBackoff(t *testing.T) {
	...
	defer func() { failpoint.Disable("tikvclient/tikvStoreSendReqResult") }()
...
func TestClusterTableSendError(t *testing.T) {
	...
	require.Regexp(t, ".*TiDB server timeout, address is.*", tk.Session().GetSessionVars().StmtCtx.GetWarnings()[0].Err)
}
