...
- [x] **infosync/tiflash_manager.go decomposition** - 2026-02-21 - Extracted `tiflash_manager_mock.go` (~635 lines) with MockTiFlash struct and all mock methods. Reduced tiflash_manager.go from 937 to 333 lines (64% reduction).
- [x] **unistore/tikv/mvcc.go decomposition** - 2026-02-21 - Extracted `mvcc_scan_resolve.go` (~411 lines) with Cleanup, scanPessimisticLocks, ScanLock, ResolveLock, SafePoint, GC functions. Reduced mvcc.go from 930 to 549 lines (41% reduction).
- [x] **session/session_bootstrap.go decomposition** - 2026-02-21 - Extracted `session_bootstrap_create.go` (~389 lines) with createSession*, CreateSession*, loadCollationParameter, DatabaseBasicInfo, TableBasicInfo, DDLJobTables, bootstrap version helpers. Reduced session_bootstrap.go from 955 to 601 lines (37% reduction).
- [x] **physicalop/base_physical_agg.go decomposition** - 2026-02-21 - Extracted `base_physical_agg_build.go` (427 lines) with AggInfo, BuildFinalModeAggregation, RemoveUnnecessaryFirstRow, CheckAggCanPushCop, ExplainInfo, ResolveIndices, ExhaustPhysicalPlans4LogicalAggregation, checkCanPushDownToMPP. Reduced base_physical_agg.go from 979 to 582 lines (41% reduction).
- [x] **lightning/mydump/loader.go decomposition** - 2026-02-21 - Extracted `loader_files.go` (427 lines) with FileHandler, FileIterator, allFileIterator, constructFileInfo, shouldSkip, route, insertDB, insertTable, insertView, GetDatabases, GetStore, GetAllFiles, calculateFileBytes, EstimateRealSizeForFile, SampleFileCompressRatio. Reduced loader.go from 978 to 580 lines (41% reduction).
- [x] **expression/builtin_cast.go decomposition** - 2026-02-21 - Extracted `builtin_cast_json_vector.go` (361 lines) with castJSONAsArrayFunctionSig, ConvertJSON2Tp, castAsJSONFunctionClass, castAsVectorFloat32FunctionClass, all VectorFloat32 cast signatures. Reduced builtin_cast.go from 973 to 637 lines (35% reduction).
- [x] **dxf/framework/storage/task_table.go decomposition** - 2026-02-21 - Extracted `task_table_ops.go` (385 lines) with SwitchTaskStep, updateTaskStateStep, insertSubtasks, SwitchTaskStepInBatch, splitSubtasks, serializeErr, GetSubtasksWithHistory, GetAllTasks, ActiveTaskSummary, GetActiveTaskCountsByKeyspace, GetAllSubtasks, AdjustTaskOverflowConcurrency, UpdateSubtaskCheckpoint, GetSubtaskCheckpoint, UpdateTaskExtraParams. Reduced task_table.go from 940 to 589 lines (37% reduction).
- [x] **physicalop/physical_table_scan.go decomposition** - 2026-02-21 - Extracted `physical_table_scan_ops.go` (410 lines) with haveCorCol, IsFullScan, AppendExtraHandleCol, BuildPushedDownSelection, GetPlanCostVer1/2, GetScanRowSize, ResolveIndices, ToPB, partitionTableScanToPBForFlash, BuildIndexMergeTableScan. Reduced physical_table_scan.go from 932 to 557 lines (40% reduction).
- [x] **util/chunk/column.go decomposition** - 2026-02-21 - Extracted `column_ops.go` (242 lines) with GetBytes, GetEnum, GetSet, GetTime, GetDuration, getNameValue, GetRaw, GetRawLength, SetRaw, reconstruct, CopyReconstruct, MergeNulls, DestroyDataForTest, ContainsVeryLargeElement. Reduced column.go from 949 to 732 lines (23% reduction).
- [x] **planner/core/expression_rewriter_leave.go decomposition** - 2026-02-21 - Extracted `expression_rewriter_leave_ops.go` (391 lines) with inToExpression, deriveCollationForIn, castCollationForIn, caseToExpression, patternLikeOrIlikeToExpression, regexpToScalarFunc, rowToScalarFunc, wrapExpWithCast, betweenToExpression. Reduced expression_rewriter_leave.go from 925 to 562 lines (39% reduction).
- [x] **planner/core/plan_cache_utils.go decomposition** - 2026-02-21 - Extracted `plan_cache_value.go` (455 lines) with PlanCacheValue, RuntimeInfo, MemoryUsage, planCacheHasherPool, NewPlanCacheValue, planCacheStmtProcessor, PointGetExecutorCache, PlanCacheStmt, GetPreparedStmt, checkTypesCompatibility4PC, isSafePointGetPath4PlanCache, parseParamTypes. Reduced plan_cache_utils.go from 919 to 501 lines (45% reduction).
- [x] **util/ranger/ranger.go decomposition** - 2026-02-21 - Extracted `ranger_utils.go` (380 lines) with sortRange, UnionRanges, hasPrefix, cutPrefixForPoints, CutDatumByPrefixLen, ReachPrefixLen, newFieldType, points2EqOrInCond, RangesToString, RangeSingleColToString. Reduced ranger.go from 911 to 561 lines (38% reduction).
