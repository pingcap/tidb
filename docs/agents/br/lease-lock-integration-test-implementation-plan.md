...
- Decision: stale reclaim case 采用同一路径的两阶段验证。
  Rationale: 先证明未过期同一 physical lock 会阻塞真实 truncate，再改成 stale 并证明真实命令删除它，避免手写路径未被扫描导致假通过。
  Date/Author: 2026-06-01 / 独立审查后确认。
...
- success case 断言至少出现两个 marker：一次 acquire 或 proof，一次 renewal。
- 不改变返回的 PD time。
...
- 阻塞直到 `writeCtx.Done()`。
- 返回 `errRenewWriteTimeout`，让 renewal loop 进入 lease lost。
...
- 集成测试至少断言出现两个 attempt marker，并检查日志包含 transient retry/backoff 信息。
...
- [ ] 添加 helper：
  ...
  - `run_br_bg_with_failpoints <failpoints> <log_file> <args...>`：后台运行 BR、记录 pid、stdout/stderr 和完整 failpoint 字符串。
  - `wait_pid_with_timeout <pid> <timeout_seconds> <expected_exit>`：超时后打印日志并 kill pid；expected_exit 支持 `success` 和 `failure`。
  - `cleanup_background_pids`：trap 中清理仍在运行的 BR 进程，并重置 `GO_FAILPOINTS=""`。
...
- [ ] 断言 `<pd-clock-dir>` 下至少有两个 `now.*` marker，证明 acquire/proof 和 renewal 都经过 PD-backed clock。
...
- [ ] 再运行一个会获取 migration write lock 的真实路径，例如 `br operator unsafe-migrate-to` 或带 `--clean-up-compactions` 的 truncate 路径，确认残留 read lock 不会阻塞 migration write。
...
- [ ] error 模式断言日志包含 transient retry/backoff 信息，最后包含 `lease lost` 或 context cancellation。
- [ ] 断言 after marker 不存在。
...
- [ ] 保存 exact physical lock path 和 txn id。
- [ ] 读取并断言 `ExpireAt` 推进。
- [ ] 断言 `<pd-clock-dir>` 下至少有两个 `now.*` marker。
- [ ] 创建 `<release>` 放行 truncate。
- [ ] 等待命令 exit code 为 0。
- [ ] 断言 `<after>` 存在。
- [ ] 断言 truncate safepoint 文件被推进到本 case 的 `truncate_ts`，或 fixture 中明确的目标 metadata/data file 状态发生预期变化。
- [ ] 断言 BR 日志不包含 `lease lost`、`onLeaseLost`、`context canceled`。
...
- [ ] error 模式断言日志包含 transient retry/backoff 信息，最后包含 `lease lost` 或 context cancellation。
- [ ] 断言 after marker 不存在。
- [ ] 断言 truncate safepoint 未推进，且关键待删 metadata 或 data file 仍存在。
- [ ] 对 `block` 和 `error` 各运行一次。
...
- [ ] 第一阶段把同一个 exact path 写成未过期 lock，例如 `expire_at` 设置为当前 PD time 之后。运行真实 `br log truncate -s "local://<log_storage>" --until "<truncate_ts>" -y`，断言命令失败或在短 timeout 内无法 acquire，证明该 path 会阻塞 truncate lock family。
- [ ] 第二阶段把同一个 exact path 改成 stale lock，满足 `ExpireAt + staleReclaimGrace < current PD lease time`。运行真实 `br log truncate -s "local://<log_storage>" --until "<truncate_ts>" -y`，同时设置 `lease-lock-test-constants` 缩短 `stale-reclaim-grace`。
- [ ] 断言命令 exit code 为 0。
- [ ] 断言同一个 exact stale lock file 被删除；不能只用“后续不阻塞”替代删除断言。
- [ ] 断言日志包含 stale cleanup/reclaim 相关信息；如果日志文本不稳定，以命令成功和 exact stale file 消失为主断言。
...
- migration success case 中 exact `v1/LOCK.READ.<32hex>` 的 `ExpireAt` 推进，PD clock marker 出现至少两次，BR restore 成功且数据校验通过，当前 holder lock 被删除，后续 migration write 路径不被阻塞，日志无 lost/cancel。
- migration block/error lost case 中 block fault 或至少两个 error attempt marker 出现，BR restore 失败退出，日志显示 timeout/retry/lost/cancel，after marker 不存在，restore 业务副作用没有发生。
- truncate success case 中 exact `truncating.lock.<32hex>` 的 `ExpireAt` 推进，PD clock marker 出现至少两次，真实 `br log truncate` 成功并推进 safepoint 或目标 metadata/data 状态，当前 holder lock 被删除，后续 truncate 不被阻塞，日志无 lost/cancel。
...
- stale reclaim case 中同一个 exact `truncating.lock.<32hex>` 先以未过期状态阻塞真实 truncate，再以 stale 状态被真实 truncate 删除并允许命令成功。
