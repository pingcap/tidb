...
- 2026-06-01 WIP 验证记录：`make build_for_br_integration_test` 通过，且 failpoint transform 后未发现 `_curpkg_`、`failpoint.Eval`、`failpoint.Call` 残留。随后在当前本地环境尝试运行 `TEST_NAME=br_lease_lock br/tests/run.sh`，但缺少 `pd-server`，服务启动停在 `https://127.0.0.1:2379/pd/api/v1/version` 等待；已终止该测试进程树。
- 2026-06-04 状态更新：用户反馈此前已在可用环境中跑通过 `br_lease_lock` 集成测试。该集成测试相对独立，当前阶段不阻塞后续 `pkg/objstore` 并发 / 竞态测试设计；后续做最终 Ready 验证或 PR 前仍应重新运行 `make build_for_br_integration_test` 和 `TEST_NAME=br_lease_lock br/tests/run.sh` 并记录具体输出。
