...
## Partition column modifiability check paths

When the modified/renamed column is part of partitioning expressions or partitioning columns, DDL calls:

- `pkg/ddl/modify_column.go:checkPartitionColumnModifiable`

This check has two paths:

- Conservative path (default): `conservativeCheckPartitionColumnModifiable`
  - This is the production default behavior.
  - It only allows metadata-only updates that do not change partition-routing semantics (for example default/comment updates).
  - It rejects rename and type-related changes that may require partition reorganization, including type/length/decimal, signedness, nullability, charset/collation, and enum element changes.
- Graceful path (failpoint-controlled): `gracefulCheckPartitionColumnModifiable`
  - This path is enabled only when failpoint `useGracefulPartitionColumnModifiableCheck` is turned on.
  - It keeps broader compatibility checks (including partition-definition revalidation) and may allow changes that the conservative path rejects.
  - Use this path for testing, not as the normal default behavior.
...
- `github.com/pingcap/tidb/pkg/ddl/useGracefulPartitionColumnModifiableCheck` (switch partition-column check from conservative default to graceful path for testing)
