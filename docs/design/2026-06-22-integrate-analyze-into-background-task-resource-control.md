...
Therefore, this document introduces a new internal task type, `StatsMaintenance`, for stats maintenance work other than `ANALYZE`. Under this proposal, the existing `stats` task type continues to represent `ANALYZE` requests, including both automatic and manually triggered `ANALYZE`.
...
We can introduce a new task type, `StatsMaintenance`, for internal stats maintenance work other than `ANALYZE` stats collection.

This keeps the behavior backward compatible. In most cases, users who enable this feature to limit stats resource usage are trying to control `ANALYZE` requests. Therefore, instead of introducing a new task type dedicated to `ANALYZE`, we can keep using the existing `stats` task type for `ANALYZE` and move other stats maintenance work to the new `StatsMaintenance` task type. **`StatsMaintenance` should not be allowed to be throttled as background work, because these tasks are critical to user query optimization.** To enforce this, Resource Control should not expose `StatsMaintenance` as a configurable `TASK_TYPES` value.
...
Based on the TiDB code example above, we need to update `StatsCtx` to use the new `StatsMaintenance` type.
