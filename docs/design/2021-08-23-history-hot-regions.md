...
- Discussion PR: [#27487](https://github.com/pingcap/tidb/pull/27487)
- Tracking Issue: [#25281](https://github.com/pingcap/tidb/issues/25281)
...
TiDB has a memory table `TIDB_HOT_REGIONS` that provides information about hotspot regions. But it can only dispaly the current hotspot information. This leads to the fact that when DBAs want to query historical hotspot details, they have no way to find the corresponding hotspot information.

According to the [documentation](https://docs.pingcap.com/tidb/stable/information-schema-tidb-hot-regions) for the current `TIDB_HOT_REGIONS` table, it can only provide information about current hotspot regions calculated by PD via the heartbeat from tikv. It is inconvenient to obtain hotspot regions of past time, and locate which store the region is. For ease of use, we can store extended hotspot region information in PD. Here are some common use cases:
...
1. TiDB sends an HTTP request to PD to obtain current hotspot regions.
...
3. After TiDB catches the response, it fetches the `START_KEY` and `END_KEY` of the hot region from region cache or PD by `REGION_ID` to decode the corresponding schema information like：`DB_NAME`, `TABLE_NAME`, `TABLE_ID`, `INDEX_NAME`, `INDEX_ID`.

4. TiDB returns the hotspot region rows to the upper caller.
...
1. New table header
  ...
  | Field       | Type        | Null | Key  | Default | Extra |
  +-------------+-------------+------+------+---------+-------+
  | UPDATE_TIME | timestamp(6)| YES  |      | NULL    |       | // new (compared to TIDB_HOT_REGIONS)
  | DB_NAME     | varchar(64) | YES  |      | NULL    |       |
  | TABLE_NAME  | varchar(64) | YES  |      | NULL    |       |
  | TABLE_ID    | bigint(21)  | YES  |      | NULL    |       |
  | INDEX_NAME  | varchar(64) | YES  |      | NULL    |       |
...
1. Timing write：
   ...
   The leader of PD will encrypt `START_KEY` and `END_KEY` of hotspot region to protect user information security before periodically writing to `LevelDB`. The write interval can be configured. Here are the full write fields:
...
3. Pull interface
   ...
   PD queries the data in `LevleDB` with filters pushed down, decodes the data and responses to TiDB.

4. New options

   There are two config options need to be add in PD’s `config.go`:

   - `HotRegionsWriteInterval`: time interval for pd to record hotspot region information, default: 10 minutes.
   - `HotRegionsReservedDays`: maximum hold day for history hotspot region, default: 7 days. 0 means close.

5. GC

    `HistoryHotRegion` wraps hot region info, it is the storage format of `hotRegionStorage`. The unserialized size of mock `HistoryHotRegion` (random value, 20 bytes `StartKey`, 20bytes `Endkey`, without `EncryptionMeta`) is 144 Bytes, after serialization, the size is 353 bytes.
...
2. Add `HotRegionsHistoryTableExtractor` to push down some predicates to PD in order to reduce network IO.
...
3. Add `hotRegionsHistoryRetriver` to fetch hotspot regions from all PD servers by HTTP request(GET method), then supplement fields like `DB_NAME`, `TABLE_NAME`, `TABLE_ID`, `INDEX_NAME`, `INDEX_ID` according to the `START_KEY` and `END_KEY` of the hotspot region, and merge the results.
