...
Measured on a table with 8,000 partitions and 30M rows ([accuracy report](https://github.com/user-attachments/files/26221115/sample-based-accuracy.md)), building histograms directly from sample data instead of merging produced 3–5× more uniform buckets (coefficient of variation 0.000–0.048 vs 0.062–0.158) and 1,000× more accurate histogram value range boundaries (the histogram's upper bound overshoots the actual maximum value by only 7 vs 7,999 for the merge-based path — lower is better). The sample-based path also computes column correlation (the merge path always returns 0) and avoids the O(P × (T + B)) memory spike that caused a second full ANALYZE to crash with OOM in testing.
...
**Bernoulli weights**: Currently Bernoulli samples have `Weight = 0` in the `tipb.RowSample` proto. We propose that TiKV populate `RowSample.Weight` for Bernoulli samples, derived from the same random value used for the include/exclude decision. The weight should be uniformly distributed so that it can be used for deterministic threshold-based pruning — the same samples always survive pruning because the decision reuses the original value, not a new random draw. TiDB already copies the weight from the proto response (`pbSample.Weight`), so no TiDB-side deserialization changes are needed.

**TiKV version compatibility**: If TiDB receives Bernoulli samples with `Weight = 0` from an old TiKV (pre-weight-fix), TiDB can assign a random weight on the TiDB side. The TiKV-side value is preferred because it is deterministic across rebuilds, but a TiDB-side replacement is statistically valid.
