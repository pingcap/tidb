...
PointGet and BatchPointGet reuse the range-scan storage and transport units instead of publishing point-specific CPU work. For the aggregate typed detail tuple `(T, P, B) = (TotalKeys, ProcessedKeys, ProcessedKeysSize)`, publish `scan_bytes = T * B / P`, exactly the existing `readBillingDemoRangeScanInput` proxy. `(P, B) = (0, 0)` publishes zero scan bytes even when `T > 0`; every other non-positive or non-finite tuple fails closed. `TotalKeys`, `ProcessedKeys`, and `ProcessedKeysSize` remain zero-weight diagnostics. Point lookup emits no `cpu_work`.

Point `net_bytes` is TiKV response payload, not complete protobuf response size. A successful Get contributes `len(value)`; a miss contributes zero. BatchGet and BufferBatchGet contribute the sum of `len(key) + len(value)` for successful pairs, while response-level and pair-level errors contribute zero. The client-go typed parser accumulates this value at exactly the same recognized-response boundary as scan-detail coverage and exposes:

    GetPointResponsePayloadAndCoverage() (payloadBytes uint64, payloadRecords uint64, completedResponses uint64, valid bool)

TiDB publishes `net_bytes = payloadBytes` and the zero-weight `payload_records` diagnostic. It consumes the value only when `valid` is true, the payload completed-response count agrees with scan-detail coverage, and `payloadRecords == completedResponses`. With zero completed responses, all payload and detail counters and values must be zero. Lock and key-error responses count as recognized zero-payload records; a later successful retry adds only its actual payload. Transport errors and region errors occur before the recognized body and add neither coverage nor payload. Complete protobuf response size, ExecDetails, error text, NotFound, CommitTs, and framing are never formula inputs.

Point lookup coverage keeps one narrow local-short-circuit refinement. A constructed PointGet or BatchPointGet executor must still expose its plan-local typed `SnapshotRuntimeStats`; completed remote responses retain both typed coverage checks above. However, partition pruning can replace the executor with `TableDualExec` after the physical PointGet plan has been flattened. When that plan ID has completed byte-bearing `BasicRuntimeStats` and exactly zero actual rows, absence of the snapshot group is authoritative evidence that the local replacement performed no point storage or payload work; that plan contributes zero to all point work and diagnostics. A missing byte sentinel, nonzero actual rows, or any invalid/incomplete typed detail or payload remains fail-closed. Basic stats are used only as completion evidence and never as unit data.

Locking SELECT is supported without introducing request work. `SelectLock` is a non-billable TiDB wrapper, and the `Lock` flag on PointGet/BatchPointGet does not change the storage or payload formula. Pessimistic-lock, lock-wait, retry, and related RPC counts are intentionally ignored because v6 has no request unit or coefficient. If a locking point read is satisfied by the lock cache and its typed snapshot reports zero completed point responses, it publishes zero point work under the existing coverage rule. If point storage responses are present, scan detail and successful response payload are consumed normally; lock-error responses contribute zero payload and do not suppress a later successful retry. Lock counters and lock-response protobuf envelopes are never substituted for scan detail, payload bytes, reader network bytes, or committed write work.

For the demo-stage dependency, TiDB temporarily replaces `github.com/tikv/client-go/v2` with the reviewed fork backport `github.com/winoros/client-go/v2` commit `3d7111ec1927b93239b0dd6b331bffb9497611fd` on branch `agent/point-response-payload-backport`. This extends the earlier scan-detail backport with typed point payload accumulation, Clone/Merge, checked arithmetic, synchronous/asynchronous BatchGet, BufferBatchGet, multi-region, key-error/lock-retry, epoch-not-match, and concurrent-access coverage. The fork remains acceptable for this pre-test demo; a production release must move the API to the official module.

Scalar-subquery plan attribution is statement scoped. `SessionVars.MapScalarSubQ` is cleared at `ResetContextOfStmt`, before every compilation or retry selects a plan, and the existing logical-plan build reset remains as a defensive internal boundary. A scalar subquery evaluated while building the current statement registers after that reset and is still flattened and billed. A later ordinary fast plan, multi-statement `PointPlanKey`, or non-prepared plan-cache hit cannot inherit the prior statement's scalar tree or use a colliding reallocated plan ID to read current runtime stats. This is an attribution fix only and does not change a unit, formula, model version, or weight version.
...
- [x] (2026-08-11 17:31+08:00) Added the client-go typed payload regression before implementation. The focused package run failed to compile because `GetPointResponsePayloadAndCoverage` and the point-response aggregation boundary did not exist, preserving the requested pre-fix evidence.
- [x] (2026-08-11 17:39+08:00) Implemented typed Get/BatchGet/BufferBatchGet payload aggregation on fork branch `agent/point-response-payload-backport`, including zero/error records, retry behavior, Clone/Merge, checked arithmetic, and concurrent access. Focused package, race, and UniStore integration coverage passed; signed commit `3d7111ec1927` was pushed to `winoros/client-go`.
- [x] (2026-08-11 18:05+08:00) Added TiDB regressions that require Scan-equivalent Point `scan_bytes`, typed payload `net_bytes`, scan/payload coverage failures, local zero, locking neutrality, checked multi-plan aggregation, absence of Point CPU, and consistent EXPLAIN, General Log, Prometheus, and statement-summary output. The focused planner case failed before the production change by retaining direct processed bytes and Point CPU and by lacking payload output.
- [x] (2026-08-11 18:08+08:00) Wired client-go pseudo-version `v2.0.0-20260811093833-3d7111ec1927`, implemented the v6 formula/output and immutable weight-version change, ran `go mod tidy`, and regenerated `DEPS.bzl` plus planner test dependencies with `make bazel_prepare`.
- [x] (2026-08-11 18:14+08:00) Completed cleanup-safe real-TiKV coverage on isolated PD port 12379 for PointGet hit/miss, BatchPointGet full/partial/all-miss, unique-index two-response hit and one-response miss, and pessimistic `FOR UPDATE`. The first run corrected the test's byte-column parser; the second proved the lock cache produces a valid zero-point-response shape; the final run passed with failpoint refcount zero, the PD endpoint closed, and the isolated TIUP_HOME removed.
- [x] (2026-08-11 18:20+08:00) Completed the final Ready profile and self-review from the final code state. Planner formula/rendering, executor output/lifecycle, statement-summary, tagged metrics, client-go package/race/integration, and real-TiKV tests passed; every failpoint wrapper restored refcount zero. `make bazel_prepare`, final-state `make lint`, `gofmt -d`, `git diff --check`, dependency resolution, generated metadata review, payload-size API search, and cleanup checks passed. Self-review added the missing `diagnostic_only=true` classification for `payload_records` before rerunning planner and lint. Commit and push remain the handoff actions.

- [x] (2026-08-06 11:21+08:00) Added the ordinary scalar-subquery-to-PointGet regression before changing production code. The failpoint-enabled executor run failed at the new isolation assertion because the following fast plan retained one prior `MapScalarSubQ` entry; the wrapper restored its shared refcount to zero.
- [x] (2026-08-06 11:26+08:00) Moved scalar-subquery cleanup to `ResetContextOfStmt`, added the equivalent boundary before each optimistic transaction history rebuild, and extended the regression across ordinary PointGet, a warmed non-prepared BatchPointGet cache hit, current-statement scalar flattening, empty scalar trees on both following fast plans, General Log point units, transaction multi-statement `prefetchPointPlanKeys`/`PointPlanKey`, and forced optimistic replay. Focused executor, server, and session tests pass with failpoint refcount restored to zero; Ready validation remains pending.
- [x] (2026-08-06 11:45+08:00) Completed the Ready profile from the final scalar-isolation diff. The focused executor, server, and session suites passed under the failpoint wrapper with every refcount restored to zero; `make bazel_prepare` regenerated the new server-test dependencies, `make lint`, `gofmt`, and `git diff --check` passed. Self-review confirmed ordinary `TryFastPlan`, prebuilt `PointPlanKey`, non-prepared cache hits, prepared cache lookup through `OptimizeExecStmt`'s logical-build boundary, statement retry, optimistic transaction replay, and plan-ID reuse all begin with statement-owned scalar state.
- [x] (2026-08-05 18:10+08:00) Added exact-zero Limit regressions before changing production code. With root runtime stats intentionally absent and every dominated child detail empty, root `PhysicalLimit{Count:0}` failed `unknown_input/missing_runtime_rows`, while embedded IndexLookUp/IndexMerge limits failed `unknown_input/missing_scan_width_evidence`; the failpoint wrapper restored its shared refcount to zero.
- [x] (2026-08-05 18:20+08:00) Added one structural exact-zero execution-mask rule for root PhysicalLimit and embedded IndexLookUp/IndexMerge `PushedLimit`. It requires exact Count zero, complete flat-subtree closure, no execution evidence anywhere below the owning node, exclusive global plan-ID ownership, and no contradictory root producer/nonzero evidence. Explicit Limit emits `cpu_work=0` from `physical_plan`; embedded readers suppress only their zero producer and do not manufacture scan, point, or network units. Frontend work and independent real readers remain visible.
- [x] (2026-08-05 18:30+08:00) Extended the focused planner regression with nonzero count/output, descendant evidence, alias, first-child-gap, nested-overlap, pushdown IndexLookUp, nonzero offset, and mixed-real-reader cases. The exact-zero target passes under the failpoint wrapper with refcount restored to zero.
- [x] (2026-08-05 18:40+08:00) Addressed review findings for exact-zero composition. Root unary and Join formulas now read an accepted explicit Limit or suppressed reader direct child as exact zero even when that child has no Basic stats, while retaining ordinary runtime rows for every active sibling. Static-zero-only input uses `physical_plan` provenance; a Join with any ordinary input keeps `runtime_child_act_rows`, and ordering retains its existing ordering-work source. Nested valid zero intervals deterministically retain only the outermost proof; non-containing overlaps remain conflicts. Added parent Projection/Selection, active-side MergeJoin, nested Limit/Limit, and Limit/IndexLookUp regressions without adding a new test target.
- [x] (2026-08-05 18:43+08:00) Completed implementation review convergence: the first reviewer found the parent-row and nested-candidate defects above, and a fresh second reviewer approved the corrected diff with no P0/P1. The full focused planner test, EXPLAIN RU executor regression, `make lint`, formatting, diff, Bazel gate, and failpoint-refcount checks pass from the final code state.
- [x] (2026-08-05 17:20+08:00) Added the IndexMerge zero-handle regression to the existing active v6 planner test before changing production code. Union with two empty partials, intersection with two nonempty but disjoint partials, and intersection with one empty partial all failed atomically with `unknown_input/missing_scan_width_evidence` at the unconstructed final table scan; the failpoint wrapper restored its shared refcount to zero.
- [x] (2026-08-05 17:35+08:00) Extended the immutable execution mask with exact `PhysicalIndexMergeReader` partial/table child mapping, received-response summary coverage on every partial root, completed byte-bearing zero output at the IndexMerge root, whole-table-subtree no-evidence proof, and global plan-ID ownership. Only the final table subtree is omitted; partial scan units and one statement reader `net_bytes` remain visible, IndexMerge root/handle merging stays non-weight-bearing, and lock activity adds no unit. Focused planner and MockStore union-empty/locking regressions pass with failpoint refcount zero.
- [x] (2026-08-05 17:55+08:00) Addressed implementation review round 2's parent-closure finding. Candidate discovery now applies the existing exact preorder-closure validator to the entire IndexMerge interval before inspecting its children, so an unowned node between the parent and first partial cannot be ignored. A first-child-gap regression rejects the mask; the focused planner target passes with failpoint refcount restored to zero.
...
- Observation: the byte metric already used by Scan is not a serialized coprocessor response size, and PointGet cannot safely reuse client-go's generic response size.
  Evidence: TiKV records cop response bytes from `resp.data.len()`, while point storage records Get `len(value)` and BatchGet/BufferBatchGet successful `len(key)+len(value)`. `tikvrpc.Response.GetSize()` includes ExecDetails, errors, metadata, and protobuf framing, so it is positive even for some zero-payload responses and materially overstates small results.

- Observation: client-go's typed point parser is the only existing boundary that sees exact point payload and can share the established completed-response coverage semantics.
  Evidence: Get reads `cmdGetResp.GetValue()` after region errors have been handled, and `collectBatchGetResponseData` has typed successful/error pairs for synchronous, asynchronous, and BufferBatchGet paths. Recording there excludes epoch-not-match and transport failures, admits zero-byte misses, and naturally counts lock-error plus successful retry as two completed responses.

- Observation: a pessimistic `SELECT ... FOR UPDATE` can complete its PointGet from the lock cache without a recognized Get/BatchGet response.
  Evidence: the real-TiKV regression returned complete zero-valued point diagnostics with `completed_responses=0`, `scan_bytes=0`, and `net_bytes=0` after the pessimistic-lock phase supplied the row. The lock RPC is intentionally outside the point-response contract, so treating it as a fabricated Get response would violate the request-retirement and payload-only rules.

- Observation: resetting scalar-subquery state only in `buildLogicalPlan` is not a statement lifecycle boundary.
  Evidence: ordinary `TryFastPlan`, a prebuilt multi-statement `PointPlanKey`, and a non-prepared plan-cache hit can all return before `buildLogicalPlan`. `FlattenPhysicalPlan` then unconditionally traverses the session-level `MapScalarSubQ`; because `TryFastPlan` reallocates plan IDs from zero, a stale tree can either fail on missing current runtime rows or alias current stats.

- Observation: a literal `LIMIT 0` normally becomes `LogicalTableDual`, but parameterized and pushed shapes can retain the zero bound as either a root `PhysicalLimit` or `Physical{IndexLookUp,IndexMerge}Reader.PushedLimit`.
  Evidence: `logical_plan_builder.go::buildLimit` replaces only `offset+count==0` immediately, while `task.go::{sinkIntoIndexLookUp,sinkIntoIndexMerge}` embeds the original Count and removes the upper Limit when the reader shape permits it. Both reader executors can construct lazy result objects without calling their `Next` path, leaving every cop summary and ScanDetail absent.

- Observation: IndexMerge constructs every partial worker independently, but constructs a final table reader only for a dispatched merged-handle task.
  Evidence: `pkg/executor/index_merge_reader.go::startWorkers` starts each partial worker before the process worker; `indexMergeTableScanWorker` calls `buildFinalTableReader` only after it receives a task containing handles. Union partials may all be empty, while intersection partials may each be nonempty yet produce no common handle, so requiring every partial row count to be zero would reject valid short circuits.

- Observation: matching expected/observed cop task counts on an IndexMerge partial root prove summary coverage only for responses TiDB received, not that all possible partial ranges ran to exhaustion.
  Evidence: pushed limits and cancellation may stop additional work after the received responses are accounted for. The skip conclusion therefore comes from the completed byte-bearing zero-row IndexMerge root plus absence of every final-table-subtree evidence type; partial equality is a conservative coverage gate for the existing scan units, not a claim of full range completion.
...
- Decision: make PointGet/BatchPointGet publish only Scan-equivalent `scan_bytes` and payload-equivalent `net_bytes`, while retaining typed scan/payload coverage diagnostics and removing Point `cpu_work`.
  Rationale: the range-scan proxy `TotalKeys * ProcessedKeysSize / ProcessedKeys` gives point reads the same MVCC-version extrapolation and existing ScanWeight. Exact successful point response payload uses the existing NetWeight without charging ExecDetails or protobuf overhead. Reusing the current synthetic `point_lookup@statement` plan-ID owner keeps the two typed aggregates together and avoids conflating Point responses with statement-scope coprocessor transport.
  Date/Author: 2026-08-11 / Codex.

- Decision: aggregate point response payload in the client-go typed parser and expose it with payload-record/completed-response coverage plus an invalid flag.
  Rationale: no TiKV protocol addition or backend capability is needed. One locked aggregation boundary makes zero results, errors, retry, Clone/Merge, overflow, and concurrent access explicit. Complete response size was rejected because its fields do not match the Scan payload contract.
  Date/Author: 2026-08-11 / Codex.

- Decision: clear `SessionVars.MapScalarSubQ` at `executor.ResetContextOfStmt`, repeat that boundary before every transaction-history `RebuildPlan`, and retain the existing `buildLogicalPlan` reset.
  Rationale: the statement reset runs before compilation on ordinary execution and statement-level retry, so it covers fast plans, `PointPlanKey`, and plan-cache returns without making the RU collector infer ownership from reusable plan IDs. Optimistic transaction replay rebuilds saved statements directly and therefore needs the same boundary in `session.retry`. Current-statement logical construction occurs afterward and repopulates the map normally. Keeping the logical reset protects direct/internal optimizer builds and existing alternative-round save/restore semantics.
  Date/Author: 2026-08-06 / Codex.

- Decision: treat an exact physical Count-zero Limit as a static zero producer only when its entire dominated flat subtree has no execution evidence and every involved plan ID has one global occurrence.
  Rationale: this recognizes both root `PhysicalLimit` and embedded IndexLookUp/IndexMerge `PushedLimit` without interpreting general missing summaries as zero. Root completion is not required, because a successful statement may leave the zero operator itself unentered; a present nonzero root or SelectResult/Snapshot producer remains contradictory. Root Limit publishes `cpu_work=0` from `physical_plan`. Embedded readers and all descendants publish no reader transport, point, scan, or CPU unit; a direct root parent consumes either accepted marker as exact zero, so its own completed work remains reportable. For nested valid candidates the outermost interval owns the proof and masks the inner candidate; non-containing overlaps, malformed or aliased structures, and execution evidence keep the prior fail-closed path. Independent sibling producers remain unaffected.
  Date/Author: 2026-08-05 / Codex.

- Decision: recognize the final table subtree of `PhysicalIndexMergeReader` as unconstructed when its root completed with byte-bearing zero output, every partial root has positive matching received-response summary coverage, the exact table subtree has no execution evidence, and all proof/masked plan IDs have exclusive global ownership.
  Rationale: union may yield no handles because all partials are empty, and intersection may yield no handles from nonempty disjoint partials. Partial row counts are therefore not a zero predicate. Exact child order/Build-Probe labels and ownership prevent borrowing aggregated evidence; absence of table Basic/cop/ScanDetail/SelectResult/Snapshot evidence prevents hiding a constructed table leg. Only the table subtree is masked, while partial scan units and one statement reader transport remain attributable. The separate exact Count-zero rule handles the no-summary static-zero shape without weakening this runtime proof.
  Date/Author: 2026-08-05 / Codex.
...
The 2026-08-11 pre-test v6 amendment aligns PointGet and BatchPointGet with range Scan without reviving Point CPU or request billing. Point storage work uses the existing Scan width proxy, while exact successful Get/BatchGet/BufferBatchGet response payload supplies Point `net_bytes`; complete protobuf sizes, errors, locks, and envelopes never enter the formula. The compatible client-go fork exposes scan and payload coverage under one recognized-response boundary with checked arithmetic and concurrent Clone/Merge behavior. TiDB retains six raw point diagnostics, uses a new immutable uncalibrated weight version, and keeps local/lock-cache zero paths reportable. Formula and four-output regressions plus the cleanup-safe real-TiKV hit/miss/partial/unique/locking matrix are recorded in `Progress`; promotion from the demo fork to the official module remains release work.

The 2026-08-06 attribution fix makes scalar-subquery plan trees genuinely statement scoped. Normal execution clears `MapScalarSubQ` before compilation, while optimistic transaction replay repeats the boundary before each saved statement rebuild; logical construction then repopulates only the current statement. Ordinary PointGet, cached BatchPointGet, and prebuilt multi-statement PointGet regressions prove that stale trees and colliding plan IDs cannot suppress or misattribute preview RU, while an executing scalar-subquery statement still flattens its own tree. No preview formula, unit, model version, or weight version changed.
...
### IndexMergeReader final-table short circuit

A standalone `PhysicalIndexMergeReader` can likewise complete without constructing its final table reader, but its proof differs from IndexLookUp. A union may have every partial path return no handles; an intersection may have nonempty partial paths whose handles are disjoint, or one empty partial beside a nonempty partial. Therefore no partial output-row value is required to be zero.

The statement-wide execution mask may omit only the exact final TablePlan subtree when all of the following hold:

- the IndexMerge root has byte-bearing Basic runtime stats with exactly zero output rows;
- `PartialPlansRaw` is nonempty, TablePlan is present, and the original partial/table IDs map in exact order to structurally valid flat Build children followed by one Probe child;
- every partial cop root has positive observed execution summaries and equal expected/observed response-summary coverage, after which the existing estimator independently validates each partial's rows and ScanDetail;
- every node in the final table subtree has no Basic byte record, expected or observed cop task, ScanDetail attachment, SelectResult group, or Snapshot group; and
- the IndexMerge root, every partial proof root, and every final-table node each own exactly one plan occurrence across Main, CTE, and scalar-subquery trees.

The completed zero-row root together with the whole-table-subtree absence proves that the final table leg was not constructed. Matching partial coverage applies only to responses TiDB received; it does not assert that every possible partial range ran to exhaustion. Only the final table subtree is masked. Every executed partial retains its existing `cpu_work`/`scan_bytes` construction, and statement reader transport continues to emit `net_bytes` once with `operator_kind=index_merge`. The IndexMerge root and handle union/intersection remain non-weight-bearing, and `SelectLock` or lock RPC activity adds no unit. Root missing/nonzero, partial coverage mismatch, table evidence, malformed child structure, or a plan-ID alias keeps the original fail-closed path. Exact Count-zero plans with no positive partial summary coverage use the separate static-zero rule below.

### Exact Count-zero Limit short circuit

An exact `Count == 0` can survive physical planning as a root `PhysicalLimit`, or be embedded in `PhysicalIndexLookUpReader.PushedLimit` or `PhysicalIndexMergeReader.PushedLimit`. The absence of every child execution detail is authoritative only for these immutable physical bounds; it is not a general missing-summary fallback.

The execution mask accepts the static-zero occurrence only when the complete owning interval passes preorder parent closure, the original child plan IDs and Build/Probe roles match the flattened direct children, every dominated descendant lacks Basic byte evidence, expected or observed cop tasks, ScanDetail, SelectResult, and Snapshot evidence, and the owner plus every descendant have exclusive global plan-ID ownership. Root Basic completion is optional. If byte-bearing root stats exist they must report zero rows, and a root SelectResult or Snapshot producer is contradictory. Nonzero offsets are accepted under the same evidence rule; if the executor actually reads offset rows, that descendant evidence disables the shortcut.

A root PhysicalLimit stays visible and publishes exactly `cpu_work=0` with `input_source=physical_plan`; it must not claim a missing `runtime_child_act_rows` source. An embedded reader remains a non-billable wrapper, while that reader and its whole cop/index/table child subtree are excluded from reader transport and point lookup construction. When either accepted zero producer is a direct child of a root unary operator or Join, the parent formula consumes exact zero input for that child without requiring a child Basic record; every unmasked Join sibling still uses its own runtime rows. A unary CPU unit derived solely from static zero uses `input_source=physical_plan`. A Join uses that source only when every included child is statically zero; if any included child supplies ordinary rows, its aggregate CPU source remains `runtime_child_act_rows`. Sort and TopN retain their existing ordering-work provenance. No zero `net_bytes`, point, scan, or descendant CPU unit is synthesized, and missing statement transport metrics cannot fail a statement whose only producer is this exact-zero reader. Other real reader or point producers remain active and use their normal evidence and statement transport. The successful execution result still independently appends available frontend work.

Valid nested exact-zero candidates do not invalidate one another. The outermost complete interval is retained deterministically and masks every inner candidate and descendant, so only the outer explicit Limit publishes zero CPU and no inner reader transport is synthesized. A positive count, descendant evidence, nonzero/producer root contradiction, malformed closure or child identity, plan-ID alias, or a non-containing interval overlap retains the existing fail-closed behavior.
...
| point `scan_bytes` | typed plan-local `SnapshotRuntimeStats.{TotalKeys,ProcessedKeys,ProcessedKeysSize}` | coverage-complete checked sum, then the range-Scan proxy `TotalKeys*ProcessedKeysSize/ProcessedKeys`; paired processed zero yields zero | no TiDB field; fork client-go scan-detail API |
| point `net_bytes` | typed plan-local successful response payload | Get value bytes; BatchGet/BufferBatchGet successful key+value bytes; coverage must match completed recognized responses | **yes, compatible client-go fork API** |
| point diagnostics | typed `TotalKeys`, `ProcessedKeys`, `ProcessedKeysSize`, detail records, payload records, completed responses | checked sum; present-zero, missing detail, and missing payload remain distinguishable | no TiDB field beyond the fork APIs |
...
A multi-reader or IndexMerge case proves that statement `net_bytes` appears once while every scan retains its own `scan_bytes`. IndexMerge additionally covers union-all-empty, intersection-nonempty-disjoint, intersection-one-empty, ordinary table-hit, locking union-empty, missing/nonzero root evidence, partial summary mismatch, whole-table-subtree evidence, malformed child mapping, and cross-tree plan-ID alias rejection; only the proven-unconstructed final table subtree disappears. PointGet and BatchPointGet cases prove the range-Scan `TotalKeys*ProcessedKeysSize/ProcessedKeys` proxy, exact successful response payload bytes, six raw diagnostics, absence of point CPU work, present-zero acceptance, scan-detail and payload coverage mismatch rejection, completed-zero local replacement, locking acceptance without lock work, multi-plan checked accumulation, and independent coexistence with reader network transport. DML point plans use the same plan-local typed source. A UnionScan case proves `cpu_work` equals direct-child rows. IndexJoin emits no Join-local transport term. Sort/TopN cases retain their materialization, saturation, offset, overflow, and fast-path coverage. Reader gates distinguish missing evidence from attributable zero bytes. ROLLBACK remains explicitly unsupported.
...
Keep all v6 model types private to `pkg/planner/core` except the already established narrow `execdetails.HashTableRuntimeStats` and client-go runtime-coverage interfaces. Do not export the preview weight container, concrete executor-private Join stats, or add public session/global variables.

The implementation uses existing TiDB packages plus the demo fork backport of client-go described in the current v6 amendment. Both typed accessors are consumed through narrow private TiDB interfaces; no TiKV protocol field is added. `go.mod`, `go.sum`, and generated Bazel dependency metadata must move atomically to the reviewed pseudo-version.
...
At milestone completion, the key internal interfaces should have these conceptual signatures:
    ...
    type pointResponsePayloadStats interface {
        GetPointResponsePayloadAndCoverage() (payloadBytes uint64, payloadRecords uint64, completedResponses uint64, valid bool)
    }
...
Revision note (2026-08-11): before external v6 testing, PointGet and BatchPointGet were aligned with range Scan. Point `cpu_work=TotalKeys` and direct `scan_bytes=ProcessedKeysSize` were replaced by the existing `scan_bytes=TotalKeys*ProcessedKeysSize/ProcessedKeys` proxy plus exact successful response payload `net_bytes`. A typed client-go fork API supplies payload and response coverage without using complete protobuf response size. The model remains v6 but moves to immutable `v6-point-payload-work-uncalibrated`; Point keeps raw scan diagnostics and adds `payload_records`.
...
Revision note (2026-08-05): standalone IndexMerge now shares the execution mask's final-table omission without borrowing IndexLookUp's zero-partial predicate. A completed byte-bearing zero-row IndexMerge root, exact partial/table structure, positive matching received-response summary coverage for every partial root, no evidence throughout the final TablePlan subtree, and exclusive plan-ID ownership allow only that final subtree to be omitted. Nonempty disjoint intersection partials remain billable, reader `net_bytes` remains statement-scoped and emitted once, IndexMerge merging and locks add no unit, and every ambiguous case remains fail-closed. This changes no formula, semantic unit, weight, version, executor detail, or production resource control.

Revision note (2026-08-06): scalar-subquery physical trees are now isolated at the true statement boundary instead of relying only on `buildLogicalPlan`. `ResetContextOfStmt` clears prior registrations before fast/cache plan selection, and transaction replay clears them before each direct `RebuildPlan`; current logical construction still registers and flattens its own scalar subqueries. This prevents stale-tree fail-closed results and plan-ID aliasing without changing preview RU v6 semantics or versions.
