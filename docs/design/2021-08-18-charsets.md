...
Currently, TiDB only supports ascii, binary, latin1, utf-8, and utf8mb4 character sets. This proposal aims to add a framework for character sets to TiDB to facilitate the support of other characters. This design takes the gbk character set as an example.

# Motivation or Background

Character: Various words and symbols are collectively referred to as characters. A character can be a Chinese character, an English letter, an Arabic numeral, a punctuation mark, a graphic symbol or a control symbol, etc.
...
- Supports all code points defined by the gbk standard. Supports all code points defined by the gbk standard.
- Support conversion with other character sets.
  - Support data automatically converted according to the corresponding character set.
...
After receiving the gbk character set request, this solution will convert it to the utf-8 character set, and then it will be the utf-8 character set when calculating or storing in the TiDB runtime layer and the storage layer, and finally return the result of the conversion to the gbk character set.

### Encoder/Decoder

- Encapsulate read and write functions, using the Transform functions of gbkDecoder and gbkEncoder in x/text (fixed version).
...
- Ranger module
  ...
  - Functions such as BuildFromPatternLike and checkLikeFunc need to consider charset.
...
Add two collations, gbk_chinese_ci and gbk_bin. In addition, considering performance, we can add utf8mb4 collation(gbk_utf8mb4_bin).
...
- Upgrade compatibility:
  ...
  - Upgrade from version 4.0 or higher
    - There may be compatibility issues when performing gbk-related operations during the rolling upgrade.
...
- DM
  ...
  - When DDL is synchronized, TiDB-parser will be used to parse the DDL statement. Currently, empty charset and collation are used. Use the charset in binlog to pass in the Parser function (it is expected to be available in the [header of the DDL event](https://dev.mysql.com/doc/internals/en/query-event.html#q-charset-database-code)):
    - If it supports (ascii, binary, latin1, utf-8, utf8mb4, gbk), handle it directly.
...
- TiDB-binlog
  - Drainer needs to convert to gbk encoding when outputting data downstream.
...
- Covers the supported built-in string functions with gbk character set, including tests for conversion of different character sets.
