...
TiDB have no collation with the default config, all the collation will be ignored. After tidb 4.0, the collation framework is supported and uses a config to control it. After a new charset is added into tidb, the related collation should be also added.
...
We should implement at least `gb18030_bin` and `gb18030_chinese_ci` collation.
