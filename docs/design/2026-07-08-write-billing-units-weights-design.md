...
- `pkg/executor/load_data.go`
  ...
  - `parserData2TableData` 做文件行解析、列映射、user variable、`SET` 表达式、cast/default/generated column。
  - `commitWorker.checkAndInsertOneBatch` 复用 insert / duplicate-check 写入路径。
...
| `load_data_decode` | LOAD DATA 文件行解析、列映射、user vars、SET 表达式进入 row prepare 前的 TiDB 本地处理 | `LoadDataWorker.load`、`readOneBatchRows`、`parserData2TableData` |
...
| `batch_events` | batch commit / worker task 数 | 是 |
