...
- Decision: MVP carries the point's coordinates as additional hidden generated **index key columns** (`INDEX (tidb_spatial_key(p), ST_X(p), ST_Y(p))`), so the bbox/MBR pre-filter runs as an index filter on those columns (pushed to the coprocessor, before the lookback); targets non-partitioned tables only, with the global index for partitioned tables a follow-on.
  Rationale: realizes the Sunny Bains review's MBR pre-lookback filter at the cheapest point (a point's bbox is just its coordinates) via index key columns rather than the value. This sidesteps what was an open wrinkle: index columns are part of the standard key encoding, so no index-value-generation extension was needed and the value stays empty (`research.md` open question 6, now resolved for the bbox; in-value is reserved for the optional full-EWKB covering index).
...
Fourth, add `s2Coverer` for SRID 4326 using `github.com/golang/geo/s2`, behind the same `CellCoverer` interface, with spherical-cap covering for distance queries and correct antimeridian/pole handling. No code above the coverer interface changes, with one 4326-specific exception in the planner hook: the pushed bbox pre-filter must handle wraparound (an antimeridian-crossing cap has a wrapping longitude interval, so split the filter into two boxes or drop the longitude constraint; a wrong reject is a wrong result), and the axis convention must be pinned (MySQL `ST_X` on SRID 4326 returns latitude; the covering rewrite must reattach the SRID).
