...
- Decision: Adopt two refinements from the Sunny Bains review (2026-06-25): store the geometry bounding box in the index value for a cheap pre-lookback filter (optionally the full EWKB for a covering index), and make spatial indexes on partitioned tables global (Hilbert-ordered across all partitions, `partition_id` in the value) rather than per-partition local.
  Rationale: the bbox-in-value recovers MBR pruning inside the flat MVI without a tree; a global index avoids fanning every spatial query out to all partitions (local indexes fragment Hilbert space). `partition_id` here is the `PARTITION BY` physical partition id, not the PK (which is already in the index key); this reuses TiDB's existing global-index machinery. Open questions for this dimension are tracked in `research.md` -> "Index value contents and table partitioning". 
