...
After the keyspace is created on PD, it can be configured on multiple TiDB instances, forming a TiDB cluster for that keyspace. However, a TiDB instance can serve only one keyspace at a time.

### Client-go

In TiDB, client-go utilizes codec V2 to add a keyspace prefix when the keyspace is set. As a result, the current TiKV client can only read and write data from that keyspace.
...
Client-go also supports API V2 and Keyspace. This will make all the codec logic transparent to upper-level users. We currently maintain these implementations in tikv/client-go/api-v2. The new version of client-go unifies all the codec logic by introducing a new interface called `Codec`:
```go
...
This interface will encode the key-related request right before it is sent to the TiKV/TiFlash/PD server. It will decode the response immediately after it is received. This design aims to be neat enough to make encoding and decoding transparent to upper-level users.
