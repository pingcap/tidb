...
If "keyspace-name" is set when create a changefeed to TiCDC server, the TiCDC server will subscribe the table data of the specified keyspace for the changefeed. When the TiCDC server creates a changefeed with a specified keyspace, it will encode the table range with the keyspace prefix to subscribe the table data for the changefeed, and the TiCDC server will decode the keyspace prefix from the key that received from TiKV to get the table data.
