...
TiDB Cluster deployment does not contain additional log collection components. The logs generated by TiDB/TiKV/PD are saved on their respective nodes. In log retrieval, the following problems are mainly encountered:

- The logs are distributed on each node and need to be logged in to each node to search using keywords.
- The log files will be rotated every day, so you need to search multiple log files on a single node.
...
Monitoring information is mainly defined internally by each component monitoring metrics, currently TiDB/TiKV/PD will provide the `/metrics` HTTP API, and then through the deployed Prometheus component timing (default configuration 15s) pull the monitoring metrics of each node of the cluster. And the Grafana component is deployed to pull the monitoring data from Prometheus for visualization.
...
How the PromQL expression query results are converted to the data in the system table depends mainly on the Schema of the query result. The expression `sum(rate(pd_client_cmd_handle_cmds_duration_seconds_count{type!="tso"}[1m]offset 0)) by (type) ` For example, the result of the query is:
