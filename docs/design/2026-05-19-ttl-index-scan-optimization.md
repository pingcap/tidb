...
- Before creating a TTL job, the TTL manager compares the current TiDB's complete server version identity (the reported version string and Git hash) with every real TiDB instance registered in server info. The current process is always the comparison baseline, even if its etcd entry is temporarily absent; assumed cross-keyspace server-info records are not workers and are ignored. Only identical builds may create index scan tasks, because an old worker cannot interpret index boundaries as PK boundaries during a rolling upgrade. A known build mismatch blocks the current job submission; the timer retries it and can create the job after all servers converge. Server-info lookup failures or an empty result are logged and fall back to the old PK scan path (`scan_index_id` remains `NULL`) instead of preventing the TTL job from running. Successful and fallback checks are cached for 10 seconds to coalesce bursts of job creation; a detected mismatch is cached for one minute to avoid frequent etcd reads during a rolling upgrade.
