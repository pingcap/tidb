...
the processing of hash table lookup results by various join types and the generation of the final join result.
...
right semi/outer join. This field is optional, if all the column from build side is not null and used_flag is not used this field is not needed.
...
mode which is faster and easier to support complex other condition.
