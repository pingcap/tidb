...
**S2 edges are not MySQL's edges.** S2 shapes live on a sphere, so a covering computed
from a stored polygon treats its edges as great circles, while MySQL's are Andoyer
geodesics on the ellipsoid. The two diverge by metres to kilometres, growing with edge
...
- **Curved edges (Phase 2, non-point geometries).** A stored geometry's bbox must not come
  ...
  its endpoints. Measured on MySQL 9.7.2 with
  `POLYGON((50 -60, 50 60, 60 60, 60 -60, 50 -60))` (SRID 4326, so latitude first), whose
  vertex latitudes are only 50 and 60: at longitude 0 the polygon actually spans latitude
  67.2665 to 73.9089, so `ST_Within` is false at latitude 55 and true at 70 and 73.8. A
  MySQL `SPATIAL INDEX` scan returns exactly the rows a full scan does, so MySQL's own
  R-tree prefilter follows the curve too (`bg::strategy::envelope::geographic<andoyer>`,
  `sql/gis/mbr_utils.cc`); a vertex bbox would have dropped both true matches. The bbox
  columns must therefore be generated by a geodesic bounder (`s2.RectBounder` /
  `Polygon.RectBound` plus `ExpandForSubregions`, already available in
  `github.com/golang/geo`) **and padded**: S2 bounds great-circle edges, which on that
...
- **Antimeridian.** A query cap crossing the antimeridian has a wrapping longitude
  ...
  (Phase 2) keeps the wrap, encoded as `minX > maxX`, rather than widening to a full-width
  longitude range: that is what MySQL does (for a polygon spanning longitude 170 to -170,
  `MBRContains` is true at longitude 175 and 180 and false at 0 and 90, and the index
  returns the correct rows), and it is the GeoParquet/Iceberg convention that Apache
  Sedona also follows. The cost is a second branch in the pushed-down filter for wrapping
  rows; the gain is that one antimeridian-crossing geometry no longer defeats longitude
  pruning for every query. Both variants are measured in
  `spatial-index/research.md` -> "SRID 4326: spherical S2 cells".

The axis convention must be pinned too: MySQL's axis order for SRID 4326 makes `ST_X`
return *latitude*, and the point covering rewrite must reattach the SRID (a bare
`ST_Point(x, y)` is SRID 0).
...
2. **Phase 2, generic geometry**: multi-cell covering via MVI and
   ...
   **array-valued** hidden generated column `tidb_spatial_keys(col)` returns the covering
   ...
   index-filter machinery (no new operator), as for the point index. This is also the
   phase where curved geographic edges first matter: on SRID 4326 both the covering and
   the bbox columns must be geodesic and padded (see Query path and Coverer and SRID
   schemes).
...
- Geographic edge curvature (Phase 2, SRID 4326): a polygon whose interior reaches outside
  its vertices' lat/long box returns identical rows with and without the index, matching
  MySQL. `POLYGON((50 -60, 50 60, 60 60, 60 -60, 50 -60))` is the worked case: at
  longitude 0 it spans latitude 67.2665 to 73.9089 although its vertices only reach 60, so
  both the stored bbox and the covering must admit the bulge. An antimeridian-crossing
  stored geometry is the paired case.
...
| Type model | Distinct `geometry` (planar) and `geography` (geodesic, meters) types, chosen per column; `geometry` on SRID 4326 does planar degree math (a footgun) | One geometry type; the SRID's SRS decides. `SRID 0` is planar Cartesian; **`SRID 4326` is the geography equivalent**: `ST_Distance`/`ST_Length` return ellipsoidal meters automatically, while `ST_Area` is not in the base design's v1 at all and lands with the function tail. The delta is ergonomic (one type + SRS vs two types), not a missing capability, and there is no degrees-on-4326 footgun |
