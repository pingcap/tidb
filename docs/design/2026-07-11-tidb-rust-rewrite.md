...
| Planner | Narrow source-owned pseudo-cardinality/equality/less/between/range and exponential-backoff leaves, a typed physical-plan metadata tree, bounded projection of an ordered static signed-BIGINT table descriptor, source-shaped physical Selection for the six signed-BIGINT comparisons plus flattened `AND`, and clustered-primary-key access/residual detachment into normalized signed ranges; projection columns remain a stable prefix and predicate-only scan columns are addressed through explicit offsets | Dynamic infoschema/catalog resolution, typed predicates beyond the bounded comparison/`AND` subset, NULL/coercion/unsigned semantics, `OR`/functions/arithmetic, index/common-handle/partition ranges, joins, aggregation, ordering/limit, the full logical/physical rule pipeline, session/statistics-aware `pseudoSelectivity`, cost model, hints, bindings, SQL normalization/digest, and plan-digest ring |
...
### Campaign 13 replica, cancellation, and optimistic-lock state

Campaign `2026-07-read-path-13` keeps one `RegionCache`, channel pool, unary
core, and DistSQL retry/publication authority while directly transiting three
pinned client-go gaps. DistSQL now derives a request-owned replica policy and
seed, reaches legal nonleader voters without promoting them as leaders, and
falls back through source-shaped `NotLeader` and `DataIsNotReady` transitions.
Every bound transport request owns a registered cancellation child: executor
cancellation fans out to active siblings, response close remains local, and
Cop, retry, CheckTxnStatus, ResolveLock, and TTL wait share one absolute
deadline. Initial PD discovery still precedes query binding and retains its
separate PD timeout; it is not claimed by this campaign.

The bounded optimistic resolver decodes a locked response, routes primary
status through the same cache/client, uses fresh TSO for remaining TTL, fails
closed on unsupported status, resolves exactly the locked secondary for a
committed or rolled-back primary, and retries only the unconsumed Cop task.
Pessimistic, async-commit, and TxnNotFound recovery remain explicit gaps.

The final live replica proof selected nonleader peer 25/store 2 while
preserving cached leader peer 15. The final v8.5.7 lock proof split primary and
...
### Campaign 14 adaptive routing and unary forwarding state

Campaign `2026-07-read-path-14` directly transits the connected synchronous
client-go routing ring without adding another cache, selector, channel pool,
or retry loop. The pinned `metapb.Store.labels = 4` field now reaches the sole
RegionCache authority through PD. Canonical StoreState retains generation,
labels, client/TiKV slow health, decaying ServerIsBusy load, and preferred
proxy identity. Request-local selection consumes immutable snapshots using the
exact ordered five-bit score (`NotSlow`, `LabelMatches`, `PreferLeader`,
`NormalPeer`, `NotAttempted`), strict busy comparison, and no-idle leader
fallback without invalidating the region.

Each unary route preserves its logical target and optional physical proxy.
The retained tonic client connects to the physical address, attaches exactly
one nonempty `tikv-forwarded-host` for the logical leader, attributes failure
to the physical proxy, publishes only generation-matching success, reuses or
rotates the proxy, and clears preference after foreground direct recovery.
Cancellation precedence, stale/replica/locality network metrics, and the
existing response-owned publication order remain on the same DistSQL path.

All 57 exact pinned Go obligations and 107 focused Rust checks pass. A TiUP
v8.5.6 one-PD/three-TiKV proof forwarded two usable Cop responses from logical
leader `127.0.0.1:49160` through proxy `127.0.0.1:49161`, exercised the exact
500/800/150 ms busy sequence, returned to a usable direct leader response, and
proved cleanup. Ready lint and the frozen 12-job workspace gate passed and
issued `integration_receipt 4`; all four claims are released as `partial`,
membership is archived, and the queue is empty. Async/batch/stream forwarding,
background health/fetch feedback, TLS, TiFlash/flashback/bucket routing, and
production cache TTL/concurrency remain explicit gaps.

### Campaign 15 bucket, batch-location, and async scheduling state

Campaign `2026-07-read-path-15` directly transits the dependency foundations
for the next runtime step. The exact pinned PD and kvproto bucket fields now
cross generated protobufs, the retained PD worker, and the sole RegionCache
loader without cache-side synthesis. Request finishing clears bucket metadata
whenever the caller did not request it. RegionCache now owns strict per-access
TTL/reload state, ordered overlap replacement, exact observed-epoch bucket
inheritance, monotonic bucket error publication, clamping, bounded batch loads,
leader filtering, caller-owned gap retry, and live cached/fresh merging.

In parallel, one completion authority preserves client-go's full-body
`sync.Once`, reverse injection, queue drain, cancellation, and concurrent-driver
rejection. One BatchCommands scheduler authority preserves Go heap semantics,
shared request progress, terminal observations, grouping, concurrency limits,
absolute send timing, stream-wide response IDs, and exact JSON batch policy.
This is intentionally the scheduler/completion foundation, not a fabricated
tonic-unary implementation of `SendRequestAsync`.

All 11 exact production sources and 53 pinned original Go obligations were
run successfully. The frozen 12-job Rust integration gate passed workspace
clippy/tests, 49 Python gate tests, static ledgers, and parser/plan checks, then
issued `integration_receipt 4`. All four slices are conservatively `partial`,
all claims are released, membership is archived, and the queue is empty. The
evidence ledger keeps seven exact residual behaviors open: leader-filtering
scan shapes, stale send contexts across down/witness peer-vector changes,
unary follower/router leader redirection, in-place background store health and
label refresh, the full cache-validity API matrix, and non-publishing direct-PD
region-ID lookup. BatchCommands wire/connection lifecycle, async RegionRequest,
cold previous-region lookup, cache background concurrency/GC, and coalesced
...
### Campaign 17 connected cache and BatchCommands state

Campaign `2026-07-read-path-17` connects three Campaign 16 production
boundaries without adding another PD client, cache, selector, retry owner,
completion table, channel pool, or Tokio runtime. The retained PD worker now
serves exact API-v1 `GetPrevRegion` end-key queries through its existing
endpoint/leader failover and region projection. `SharedReadRuntime` now gives
foreground reads and the sole coalesced, joined maintenance worker one
synchronized RegionCache authority, and production failure handling validates
the exact selection-time peer-vector observation before mutating cache, store,
proxy, or retry state. The retained tonic worker now owns route-scoped
BatchCommands streams, pending-before-send publication, response-ID
demultiplexing, forwarded-route isolation, generation-safe cancellation and
failure retirement, bounded demand-budgeted reconnect, and close drainage.

The campaign records 14 exact pinned Go production sources and 65 exact
original Go obligations. Every local pinned Go command passed, and the full
12-job Rust integration gate and Ready lint passed with
`integration_receipt 3`. All three slices remain `PARTIAL`: Campaign 17 has no
live PD/TiKV proof. PD router service, TLS, and cancellation-aware discovery
and failover backoff remain open. Region loader I/O still runs under the coarse
cache mutex, with no concurrent live foreground/maintenance proof.
BatchCommands reconnect is bounded and demand-budgeted, not the Go
prolonged-outage recovery loop, and the concrete async RegionRequest adapter is
deferred until it can reuse this stream plus the canonical selector/backoff
state.

### Campaign 18 connected BatchCommands-first read path

Campaign `2026-07-read-path-18` closes the next connected read-path boundary
without adding a second cache, selector, retry loop, completion authority,
channel pool, or Tokio runtime. Region and store metadata loads now select a
complete immutable plan under the canonical cache lock, perform PD I/O after
releasing that lock, and publish only after full optimistic revalidation.
The concrete Coprocessor adapter carries the final context and forwarding host
through the retained BatchCommands scheduler, route-scoped in-flight table,
and sole once-only completion. DistSQL attempts BatchCommands first, but owns
all selection, cancellation precedence, feedback, backoff, and synchronous
fallback exactly once. Local opening-slot pressure is `AdmissionBusy`: it
rolls back the exact unsent selector attempt and provisional network metrics
without poisoning topology, liveness, backoff, or replica-attempt budget.

The first live run exposed a grpc-go/tonic semantic mismatch: tonic awaited
response headers before returning a send-capable duplex stream, while live
TiKV may await the first inbound BatchCommands request before publishing those
headers. Stream opening is now generation-owned, returns a send-capable route
immediately, admits one bounded opening packet, and receives headers/responses
concurrently. The regression proves the first request reaches the server
before response headers are released.

All 50 exact pinned original Go test obligations passed, and the 12 exact
production-source anchors were promoted to honest `PARTIAL` successors. The
full 12-job Rust gate separately issued `integration_receipt 4`, and repository
`make -j12 lint` passed. The hardened
three-PD/three-TiKV proof split adjacent regions, kept direct and forwarded
streams on one physical proxy, disabled PD leader scheduling, froze and killed
an exact nonleader voter, observed one forwarded route-generation failure
without retiring the direct stream, restarted that follower at the same
address, proved it ready directly without warming the proxy, and then used the
measured client's first post-restart forwarded attempt successfully on route
generation 3 after generation 1 failed on physical channel version 1. The
leader remained store 2 throughout, and cleanup removed every owned process,
endpoint, registry row, phase directory, and tag directory.

All four claims are released as `PARTIAL` and membership is archived. TLS, PD
router streaming, cancellation-aware PD backoff, complete BatchCommands
...
### Campaign 20 stored-column read path

Campaign `2026-07-read-path-20` widens that same deployable path from one
clustered key to an ordered static list of signed `BIGINT` columns. The startup
surface uses one repeatable atomic descriptor,
`--column name:id:clustered-pk|stored-not-null`, validates unique names and
IDs, and requires exactly one clustered key. Planner resolution now preserves
source name, output alias, projection order, TiKV column ID, and key/stored
flags. Executor lowering emits scan columns and DAG output offsets in that same
order, decodes every returned datum, and publishes exact MySQL metadata.

The live proof created `campaign20.rows(id BIGINT PRIMARY KEY CLUSTERED,
balance BIGINT NOT NULL)` and seeded `(-7,913)`, `(0,-2048)`, and `(42,77)`.
A stock MySQL client queried the Rust node with
`SELECT balance AS amount, id FROM campaign20.rows` and received exactly those
stored/key pairs with header `amount, id`. The stored values cannot be inferred
from the record keys. Evidence recorded table ID 114, PD cluster
7664074606058950579, snapshot TSO 467777994228760577, region 16 located and
dispatched, one BatchCommands attempt, no unary fallback, zero active
connections, and complete tag-owned cleanup.

This closes the key-only synthetic-value risk; it does not widen the admitted
type or SQL families. The node remains serial, loopback/root-empty-password
only, static-catalog, signed-BIGINT-only, and direct-projection-only. Dynamic
catalog discovery, predicates, joins, aggregation, ordering/limit, concurrent
connections, full authentication/TLS, writes, and the remaining original Go
obligations remain open.

### Campaign 21 concurrent authenticated SQL node

Campaign `2026-07-read-path-21` replaces Campaign 20's serial, empty-password
process with one bounded concurrent server authority. Startup requires a
mode-0600 native-password user file; the handshake uses an OS-random salt and
the real `mysql_native_password` verifier, including AuthSwitch and a dummy
unknown-user verification path. A fixed worker pool owns exactly the configured
connection capacity. Each worker creates a thread-confined read session while
all sessions share one process-owned PD client, maintained RegionCache, and
cloneable TiKV transport authority. Connection deadlines, signal-driven drain,
query cancellation, and terminal lifecycle accounting are explicit.

Transport ownership was widened without serializing foreground requests. Unary
fallback runs on the transport runtime rather than blocking its command loop;
cold BatchCommands stream opening accepts the client-go-sized 128-packet
publication window; and stale StoreNotMatch feedback invalidates only the exact
observed region route rather than closing a replacement channel by address.
Deterministic regressions cover transport head-of-line blocking, cross-session
route generations, eight concurrent client requests, blocked-query
cancellation, early disconnect, connection admission, and post-spawn accept
setup errors draining every worker before return.

The cleanup-safe live proof authenticated eight overlapping stock MySQL clients
and executed `SELECT balance AS amount, id FROM campaign20.rows` through real PD
and TiKV. Every client returned exactly `(913,-7)`, `(-2048,0)`, and `(77,42)`;
the server observed `max_query_active=8`, eight distinct snapshot TSOs, eight
BatchCommands transport records, zero unary fallbacks, and final lifecycle
`accepted=9 completed=9 active=0` after a separate ninth early-disconnect
client. Cleanup stopped all tag-owned processes and removed cluster data and
the authentication file.

The frozen shared gate passed strict all-target workspace Clippy, the complete
workspace and differential test matrix, 49 governance tests, parser/plan and
static ledger checks, then issued one immutable shared receipt covering six
claims. All six claim entries were consumed, claims returned to zero,
membership is archived, generated status is current, and repository
`make -j12 lint` passed.

This is the first bounded concurrent authenticated deployable read-only node,
not full TiDB parity. It remains loopback-only, plaintext, static-catalog,
signed-BIGINT-only, and direct-projection-only. TLS, grants, prepared
statements, general SQL, writes/transactions, topology-churn-complete exact
channel invalidation, and fallible propagation of maintenance/transport worker
shutdown failures remain explicit next boundaries.

### Campaign 22 topology-churn and fallible process lifecycle

Campaign `2026-07-read-path-22` closes those topology and lifecycle boundaries
without widening the bounded SQL surface. Immutable dispatch evidence keeps
physical address and ChannelPool version distinct from BatchCommands stream
generation. A current direct `StoreNotMatch` retires only the exact physical
channel version observed by that request; a stale response cannot retire its
replacement, and forwarded recovery preserves the healthy shared proxy.
RegionCache restores a failed same-address store only after an out-of-lock
TiKV health probe reports `Reachable` for the exact unchanged store, address,
epoch, resolve state, and observed liveness generation.

The process now owns explicit fallible shutdown in dependency order:
connections, RegionCache maintenance, TiKV transport, then PD. Partial worker
construction, worker death, drain, close acknowledgement, and join failures
remain observable through the executable result instead of disappearing in
`Drop` or behind the first error.

The live receipt kept one Rust PID and one authenticated stock MySQL connection
through leader stores `1 -> 5 -> 2 -> 5`, returned exactly
`[(913,-7),(-2048,0),(77,42)]`, and routed through physical addresses
`127.0.0.1:63161 -> 127.0.0.1:63160 -> 127.0.0.1:63162 -> 127.0.0.1:63160`.
Restarted store B retained ChannelPool version `1` while its BatchCommands
stream generation advanced `1 -> 5`. A real prewrite-blocked query published
on B before SIGTERM; shutdown completed
...
all tag-owned runtime state.

The proof requires an explicit failpoint-enabled TiDB v8.5.6 compatibility
fixture because a current-tree TiDB may issue RPCs absent from the pinned
v8.5.6 PD/TiKV topology. That Go binary is test-only: it creates the fixture
and establishes the deterministic prewrite barrier; every asserted read goes
...
members consumed the receipt, and claims returned to zero.

### Campaign 24 clustered-primary-key range detachment

Campaign `2026-07-read-path-24` implements the next bounded optimization
without creating a key-only execution path. A source-shaped signed-BIGINT
ranger detaches clustered-primary-key access conditions for all six
comparisons, reversed operands, flattened conjunctions, MIN/MAX boundaries,
contradictions, and split `!=` ranges. Planner-owned logical ranges become
exact half-open TiDB record-key ranges in DistSQL. Residual predicates remain
owned by TiKV `[TableScan, Selection]`, and contradictions return locally
before PD TSO or transport publication.

The live topology lifecycle now has one shared owner for Campaigns 22, 23, and
24. Scenario runners retain only SQL data and plan/result assertions. A static
test prevents topology, failpoint, shutdown, or cleanup orchestration from
being copied back into scenarios. Cleanup force-containment is based on
current exact tag ownership plus zero descendants, with injected positive and
unowned-process regressions for early scenario failure.

The passing Campaign 24 proof kept Rust PID `40428`, MySQL connection `1`, and
read session `1` across leader stores `1 -> 4 -> 5 -> 4` at
`127.0.0.1:63160 -> 127.0.0.1:63162 -> 127.0.0.1:63161 -> 127.0.0.1:63162`.
It covered `eq,ne,lt,le,gt,ge,reversed,bounded,min,max,contradiction,residual`;
the contradiction published `snapshot_ts=null` and zero transports, while the
residual plan remained `[TableScan, Selection]`. Restarted B kept channel
version `1` and advanced stream generation `1 -> 5`. Query `19` blocked before
SIGTERM; ordered shutdown finished in `125 ms`, exited zero, balanced
`accepted=1 completed=1 failed=0 active=0`, and passed tag-owned cleanup.
Implementation and live acceptance are complete; the shared Ready gate,
repository lint, receipt consumption, and release are still pending.
...
have not started. Dynamic catalog/infoschema, general planning and expressions,
the full Datum/type domain, TLS, grants, prepared statements, writes and
transaction protocols, DDL, statistics integration, bootstrap and cluster
services, plan/result/transaction zero-diff gates, shadow traffic, performance
gates, and Jepsen coverage remain open.
...
Campaign 24's implementation and live milestone is complete. Its remaining
integration step is one shared Ready gate, repository lint, six receipt
consumptions, claim release, and generated-status refresh. Do not rerun the
live topology as a substitute for that frozen batch gate.
