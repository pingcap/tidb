...
| Transaction/storage | Portable key, handle, version, iterator, and request-tag contracts; a bounded plaintext PD gRPC client; API-v1 region loading; one RegionCache topology authority; lazy address-keyed TiKV Coprocessor RPC; and a PD-only-input live TiUP proof | Endpoint discovery/refresh/failover, TLS/forwarding/router service, retries/backoff, locks/MVCC, production cache concurrency/TTL, 2PC, async commit, 1PC, full table/DAG lowering, and COM_QUERY integration |
