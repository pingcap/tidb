...
* Local transaction: only allow to read & write the local data which located in the same zone with local TSO.
...
Since the transaction process is managed on the TiDB side, before initiating a transaction, the tidb-server need to know the topology information for itself so that we can know the PD with the same location to request. Like PD, TiDB needs to introduce a field such as ***dc-location*** in the label configuration. This label is string type, the default value is empty, and used to set which DC the TiDB node belongs to, so that it can distinguish local/global transaction needs when performing TSO services.

As the label configuration could be modified by other purpose, we will also provide a switch to enable/disable the Local/Global transaction ability in the configuration. It will be named as enable-local-transaction, bool type and the default value is false.
