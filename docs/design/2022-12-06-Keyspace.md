...
If "keyspace-name" is set when executing the Lightning command, it will encode key to add keyspace before appends rows to the SST file, and decode key before records conflicts data.  
