...
In contrast, there is only one gcworker for GC in the whole cluster. Each keyspace has its own gcworker leader that can separately advance its own GC safepoint, safepoint information stored in PD, It is also isolated by keyspace prefix, and only cleans the data below its keyspace range, without affecting each other.
