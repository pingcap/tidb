...
### Table `(CLUSTER_)TRX_SQL`

| Field      | Type       | Comment |
|------------|------------|---------|
| `TRX_ID`    | `unsigned bigint`   | ID of the transaction |
| `DIGEST`|`text`| Digest of the SQL |
| `INDEX`|`unsigned bigint`| Index of the statement in this transaction|

* Life span of rows:
  * Create on executing an SQL in a transaction.
* Collecting, storing and querying:
  * Collecting and stored just like part of `(CLUSTER_)TIDB_TRX`. But querying as a standalone table for convenience.
* Permission:
  * Same as `(CLUSTER_)TIDB_TRX`.
...
* Since lock waiting on TiKV may timeout and retry, it's possible that in a single query to `DATA_LOCK_WAIT` table doesn't show all (logical) lock waiting.
