...
This proposal introduces a `tidb_max_rows_read` session variable and corresponding `SET_VAR` hint that limits the number of rows examined (scanned by the storage engine, not returned to the client) during `SELECT` query execution. The limit is pushed down to TiKV's coprocessor layer for efficient early termination. When the limit is exceeded, the query is terminated with an error. The feature is inspired by MariaDB's [`LIMIT ROWS EXAMINED`](https://mariadb.com/docs/server/ha-and-performance/optimization-and-tuning/query-optimizations/limit-rows-examined) and similar row-examination limiting features present in other MySQL-compatible databases.
...
3. **Per-query safety nets**: Using `SET_VAR` hints, developers can annotate individual queries with specific row-scan budgets directly in application code, without DBA involvement.

4. **Query tuning and observability**: The companion `tidb_rows_examined` status variable lets developers measure how many rows each query (and each session) actually examines, enabling data-driven query optimization.
...
```
SQL: SELECT /*+ SET_VAR(tidb_max_rows_read=100) */ * FROM t

TiDB Layer:
  SessionVars.MaxRowsRead = 100 (via SET_VAR or SET SESSION)
      ↓
  DistSQLContext.MaxRowsRead = 100
      ↓
  kv.Request.MaxRowsRead = 100
      ↓
  copIterator: tracks cumulative rowsScanned (atomic counter)
      ↓
  copIteratorTaskSender: per task, sets
    task.maxRowsRead = remaining / concurrency  (proactive split)
      ↓
  handleTaskOnce(): sets copReq.MaxRowsRead = task.maxRowsRead

TiKV Layer:
  Coprocessor reads Request.max_rows_read
      ↓
...
TiDB Layer (response):
  copIterator.Next(): accumulates scanned rows from ScanDetail.ProcessedKeys
      ↓
  When cumulative >= max_rows_read → return error to client
```
...
SET SESSION tidb_max_rows_read = 1000;

-- Global default (applies to all new sessions)
SET GLOBAL tidb_max_rows_read = 1000;

-- Per-query hint (overrides session variable for this query only)
SELECT /*+ SET_VAR(tidb_max_rows_read=500) */ * FROM orders WHERE status = 'pending';

-- Disable for one query when session has a limit
SELECT /*+ SET_VAR(tidb_max_rows_read=0) */ * FROM orders;
```
...
```
ERROR 8270 (HY000): Query execution was interrupted due to reaching tidb_max_rows_read limit
```
...
#### Monitoring Rows Examined

```sql
-- Check cumulative rows examined in this session
SHOW STATUS LIKE 'tidb_rows_examined';
...
1. Parser extracts `SET_VAR(tidb_max_rows_read=N)` from the SQL hint comment.
2. `ParseStmtHints()` validates via `setVarHintChecker` (checks the variable exists and is hint-updatable).
3. Planner applies `SetSystemVar("tidb_max_rows_read", "N")` which calls the `SetSession` hook.
4. After the statement completes, the old value is restored from `SetVarHintRestore`.

### Plan Cache Compatibility

Queries using `SET_VAR(tidb_max_rows_read=N)` are **fully compatible** with both the prepared and non-prepared plan caches. No special handling is needed because
`tidb_max_rows_read` does not influence the query plan — it is an execution-time limit applied in the coprocessor dispatch layer (`copIterator`).

#### Trade-off

Different `SET_VAR(tidb_max_rows_read=N)` values produce separate cache entries with identical plans. For example, a query executed with `N=100` and then `N=200`
...
| Property   | Value                                                              |
|------------|--------------------------------------------------------------------|
| Name       | `tidb_rows_examined`                                               |
| Scope      | Session                                                            |
| Shows      | Cumulative rows examined across all SELECT queries in this session |
| Reset      | `FLUSH STATUS`                                                     |
| Initial    | `0`                                                                |

This counter provides developers with a tool for identifying expensive queries within a session, validating that query optimization reduced rows scanned, and monitoring application-level scan patterns — functionality with no equivalent in the runaway queries framework.

### Protobuf Changes

Add a `max_rows_read` field to `coprocessor.Request` in the **kvproto** repository:

```protobuf
message Request {
    // ... existing fields ...
    uint64 max_rows_read = N; // Max rows to scan for this coprocessor task (0 = unlimited)
}
```
...
**No new response fields are needed.** TiDB tracks scanned rows using the existing `ScanDetail.ProcessedKeys` returned in every coprocessor response. This field is already populated by TiKV.

### TiDB Coprocessor Dispatch with Concurrency-Aware Splitting

#### Problem

`tidb_distsql_scan_concurrency` controls how many coprocessor tasks execute in parallel. Without splitting, each concurrent task receives the full remaining limit. If `limit=100` and `concurrency=15`, all 15 tasks could scan up to 100 rows each = **1,500 total rows examined** — a 15x overshoot.
...
The overshoot is bounded: at most `concurrency` tasks each scan `remaining/concurrency` rows, so the total overshoot is at most `remaining` extra rows beyond the limit. In practice it is much less because:
...
```
SessionVars.MaxRowsRead
    → DistSQLContext.MaxRowsRead      (pkg/distsql/context/context.go)
    → kv.Request.MaxRowsRead          (pkg/distsql/request_builder.go:SetFromSessionVars)
    → copIterator.maxRowsRead         (pkg/store/copr/coprocessor.go)
    → copTask.maxRowsRead             (per-task split)
    → coprocessor.Request.MaxRowsRead (per-task proto field)
```

### TiKV Coprocessor Enforcement

TiKV reads `max_rows_read` from the `coprocessor::Request` and enforces it within the `BatchExecutorsRunner` — the component that drives the scan loop and collects results. This follows the same pattern as paging enforcement where the runner checks accumulated rows against a threshold after each `next_batch()` call.

#### Implementation approach

The enforcement point is in `BatchExecutorsRunner::handle_request()` (runner.rs:733-843), which already has a loop that accumulates `record_all` and checks `paging_size`. The `max_rows_read` check is added to the same loop:

```rust
// In handle_request(), existing paging stop condition at line 790:
if drained.stop()
    || self.paging_size.is_some_and(|p| record_all >= p as usize)
    || self.max_rows_read.is_some_and(|m| record_all >= m as usize)  // NEW
{
...
The TiKV-side enforcement leverages the existing `scanned_rows_per_range: Vec<usize>` counter in `RangesScanner` (scanner.rs:29-50) for precise tracking. The `BatchExecutorsRunner` already tracks `record_all` which counts rows returned by the outermost executor per batch iteration. For `max_rows_read`, we use the cumulative scanned rows from the scan executor, which accounts for rows examined before filtering.
...
The error is raised **at the TiDB layer** (in `copIterator.Next()`) when the cumulative scanned rows across all coprocessor responses exceed the limit. TiKV does not return an error — it returns a partial result, and TiDB makes the final determination. This is important for correctness in the distributed case where TiDB has the global view across all tasks.
...
| **Plan cache** | Fully compatible. Queries with `SET_VAR(tidb_max_rows_read=N)` are cached normally. Different `N` values produce separate cache entries (because the hint text is part of the SQL text used in the cache key). On cache hit, the `StmtHints.SetVars` stored in the cached plan are applied to session variables before execution, so the correct `MaxRowsRead` value is always in effect. Since `tidb_max_rows_read` does not affect the query plan (it is purely an execution-time limit), the cached plan is always valid regardless of the limit value. |
...
| **Index lookup** | `IndexLookUpExecutor` issues both index scan and table scan coprocessor requests. Both contribute to the cumulative row count.                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **DML** | Not affected. The limit only applies to SELECT queries. `GetMaxRowsRead()` returns 0 when not in a SELECT statement. The `SET_VAR` hint on DML is silently ignored.                                                                                                                                                                                                                                                                                                                                                                                        |
| **Subqueries** | The limit applies to the entire statement. Rows scanned by subqueries contribute to the cumulative count. This matches MariaDB's behavior where "there can be only one `LIMIT ROWS EXAMINED` clause for the whole SELECT statement."                                                                                                                                                                                                                                                                                                                       |
| **JOINs** | Rows from all tables participating in the JOIN are counted together.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Aggregation** | The limit counts scanned rows, not result rows. `SELECT COUNT(*) FROM t` with `max_rows_read=10` will error if the table has more than 10 rows, even though the result is a single row.                                                                                                                                                                                                                                                                                                                                                                    |
| **Upgrade** | New proto field `max_rows_read` defaults to `0` (unlimited). Old TiKV versions ignore unknown protobuf fields. TiDB-side enforcement still works via `ProcessedKeys` tracking.                                                                                                                                                                                                                                                                                                                                                                             |
...
- Validate `GetMaxRowsRead()` accessor: returns 0 when not in a SELECT statement.
...
- `BatchExecutorsRunner` with `max_rows_read` set — verify partial result returned when limit reached.
- `BatchExecutorsRunner` with `max_rows_read = 0` — verify no limit applied.

### Scenario Tests

**Integration tests in `tests/integrationtest/t/executor/max_rows_read.test`:**

| # | Scenario | Expected |
|---|----------|----------|
| 1 | Basic enforcement: table with 100 rows, `SET tidb_max_rows_read = 10`, `SELECT *` | Error 8270 |
| 2 | `SET_VAR` hint: `SELECT /*+ SET_VAR(tidb_max_rows_read=5) */ * FROM t` | Error 8270 |
| 3 | Precedence: Session=100, hint=5 | Hint wins, error at ~5 rows |
| 4 | Disable per-query: Session=10, `SET_VAR(tidb_max_rows_read=0)` | No limit, full result |
| 5 | JOINs: `SELECT * FROM t1 JOIN t2` | Rows from both tables counted |
| 6 | Aggregation: `SELECT COUNT(*) FROM t` | Counts scanned rows, not result rows |
| 7 | Status variable: `SHOW STATUS LIKE 'tidb_rows_examined'` | Returns cumulative count |
| 8 | `FLUSH STATUS` | Resets `tidb_rows_examined` to 0 |
| 9 | DML not affected: `INSERT ... SELECT`, `UPDATE`, `DELETE` | No limit applied |
| 10 | Global scope: `SET GLOBAL tidb_max_rows_read = 100` | Applies to new sessions |
| 11 | Under limit: table with 10 rows, `SET tidb_max_rows_read = 100`, `SELECT *` | Full result, no error |
| 12 | Exact limit: table with 10 rows, `SET tidb_max_rows_read = 10` | Borderline — may or may not error depending on overshoot |
| 13 | Plan cache hit: same SET_VAR hint, second execution | `@@last_plan_from_cache = 1`, limit still enforced |
| 14 | Plan cache with session variable (no hint) | Cached plan reused; session-level `MaxRowsRead` applies at execution time |
...
- **Plan cache with SET_VAR**: Execute `SELECT /*+ SET_VAR(tidb_max_rows_read=5) */ * FROM t` twice. Second execution should get a plan cache hit. The limit should
  still be enforced on the cached path (error 8270 if table has >5 rows).
- **Plan cache without hint**: Execute `SET SESSION tidb_max_rows_read = 5; SELECT * FROM t` twice. Both executions should use the same cached plan, and the limit
...
- **Early termination benefit**: Measure latency reduction when the feature aborts a full table scan early (e.g., 1M row table with `max_rows_read=1000`). This validates that TiKV pushdown provides tangible savings vs. TiDB-only enforcement.
...
- **Row counting accuracy**: `ScanDetail.ProcessedKeys` counts storage engine keys processed, which closely approximates rows examined but may differ for index lookups or MVCC version traversal. For index lookups, `ProcessedKeys` may count both index keys and table keys separately, so the effective count may be higher than the logical row count. This is a conservative approximation (may trigger the limit earlier than expected) which is acceptable for a safety-bound feature.
...
TiDB already provides a `PROCESSED_KEYS` threshold as part of the [resource control runaway queries](https://docs.pingcap.com/tidb/stable/tidb-resource-control-runaway-queries/) feature (available since v7.2.0). Both features count coprocessor-level keys/rows scanned. This section explains why `tidb_max_rows_read` is still needed as a complementary mechanism.

#### Feature Comparison

| Aspect | PROCESSED_KEYS (Runaway Queries) | tidb_max_rows_read |
|--------|----------------------------------|-------------------|
...
| Per-query override | Not possible | `SET_VAR(tidb_max_rows_read=N)` per query |
...
| Monitoring | `information_schema.tidb_runaway_queries` | `SHOW STATUS LIKE 'tidb_rows_examined'` |
...
`tidb_max_rows_read` supports `SET_VAR` hints, allowing per-query override:

```sql
SET SESSION tidb_max_rows_read = 1000;  -- default safety net
-- This specific report query needs more headroom:
SELECT /*+ SET_VAR(tidb_max_rows_read=100000) */ * FROM analytics WHERE date > '2024-01-01';
...
`tidb_max_rows_read` works independently with zero prerequisites. A user sets a session variable and gets immediate protection.
...
`tidb_max_rows_read` pushes the limit down to TiKV via the `coprocessor.Request.max_rows_read` proto field. TiKV can stop scanning early and return a partial result, **saving TiKV CPU, disk I/O, and network bandwidth**. For a query that would scan 10 million rows on a table split across 100 regions, the pushdown avoids transmitting millions of unnecessary rows. This is the primary technical advantage.

**5. Cumulative session counter.**

`tidb_rows_examined` provides visibility into how many rows a session has examined across all queries — a debugging and tuning tool with no equivalent in the runaway queries framework. Developers use this for:

- Identifying which queries in a transaction are expensive.
- Validating that query optimization reduced rows scanned.
...
- The `tidb_max_rows_read` check happens in `copIterator.Next()`, also after each coprocessor response.

Both checks are independent and coexist naturally. If both `PROCESSED_KEYS` and `tidb_max_rows_read` are set, whichever threshold is reached first triggers its respective action. They serve different governance models operating at different organizational levels.
...
| Aspect | MariaDB `LIMIT ROWS EXAMINED` | TiDB `tidb_max_rows_read` |
|--------|-------------------------------|---------------------------|
