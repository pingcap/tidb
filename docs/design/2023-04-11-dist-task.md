...
TiDB is a distributed database that provides high scalability, available and performance for database service. Starting from version v7.1,
TiDB introduce a unified distributed parallel execution framework to gradually support all backend tasks, such as DDL, data import, TTL, auto-analyze and Backup|Restore, in a distributed manner. This framework offers three significant benefits to TiDB users:
...
## Goal and non-goal
In this section, we define the goals and non-goals for the first version of the framework. Future versions will be described in the "Future Work" section.
### Goal
The first version of the framework aims to:
...
### Non-goal
The first version of the framework does not aim to:

1. Support query execution. For query distributed execution support, please see the TiFlash documentation.
2. Provide sophisticated high availability (HA) capability, which will be improved in future releases.
3. Support task runtime status, such as task execution progress, which will be provided in later releases.
4. Support TTL, auto-analyze, and BR, which will be supported in later releases.
...
## Usage
The phase I of the unified distributed parallel execution framework is currently planned as an experimental feature, which will be released in version 7.1. This feature is designed to support the execution of `Create index` or `Add index` tasks in a distributed manner.
