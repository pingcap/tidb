...
**truncate lock**:
BR log truncate 流程外层持有的 `truncating.lock.<generation>` 物理 lock 实例。它不包含 truncate 流程内部可能再次获取的 migration write lock。
_Avoid_: migration write lock, all locks used during truncate
...
Developer: "truncate lock 测试要不要把 truncate 流程内部的 migration write lock 也算进去？"

Domain expert: "不要。这里的 truncate lock 专指外层 `truncating.lock.<generation>`；内部 migration write lock 归 migration lock 测试覆盖。"
