...
### Tests
- `pkg/executor/adapter_internal_test.go` - executor: Tests private adapter runtime accounting, including v6 General Log point storage-work/raw diagnostics, network-only reader transport, mutation `cpu_work`, committed `write_keys`/`write_bytes`, DML/source/side provenance, serialization, and deterministic ordering.
- `pkg/executor/adapter_test.go` - executor: Tests statement adapter behavior, including RU v2 finalization and self-describing preview RU General Log completion after lazy SELECT close, MockStore PointGet/BatchPointGet incomplete-detail fail-closed output, DML/early errors, compile-error suppression, cursor-detach gating, repeated finish calls, and EXPLAIN ANALYZE DML commit errors.
...
- `pkg/executor/explain_test.go` - executor: Tests EXPLAIN ANALYZE formats, including production-default uncalibrated v6 units and absent totals, early scalar-subquery child CPU units, non-billable Apply traversal with descendant units, serial/parallel Apply zero-outer omission of the unexecuted inner, reused and recursive CTE producer-only accounting with non-billable consumers, network-only reader transport, IndexMerge union-empty final-table omission with one transport publisher and lock-neutral `FOR UPDATE`, typed PointGet/BatchPointGet Scan-equivalent `scan_bytes` and successful response-payload `net_bytes` plus raw diagnostics and no point CPU unit, strict MockStore `incomplete_point_scan_detail` fail-closed behavior, six pushed TiKV operator classes, raw DML mutation diagnostics with gated mutation `cpu_work`, autocommit write-key/byte output, explicit pessimistic DML omission and COMMIT ownership, statement-summary/Prometheus output versions, retired request-unit absence, restricted/feature-off isolation, and legacy v3 behavior retained as explicitly superseded coverage.
