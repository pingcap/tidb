...
- `pkg/planner/core/common_plans_test.go` - planner/core: Tests common plans plus preview RU gating, weights, Sort/TopN algorithmic `order_work`, TiDB Join/Agg and guarded TiKV Agg diagnostic output shadows, TiKV component/child-row/scan-width input estimation and width barriers, DML mutation/write and explicit-COMMIT payload construction, pipelined payload suppression, and ancillary-work partial diagnostics.
