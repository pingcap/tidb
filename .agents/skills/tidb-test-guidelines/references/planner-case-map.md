...
- `pkg/planner/core/common_plans_test.go` - planner/core: Tests common plans plus preview RU gating, the eight private v6 semantic weights/model binding and overflow rejection, scalar-subquery, Apply, and CTE non-billable wrappers plus MaxOneRow row-limit classification, CTE producer seed/recursive-part execution-evidence masking, expression-slot counts for Agg/Join/Window families, Sort CPU work plus TopN input saturation/zero-count/checked bounds, UnionScan direct-child CPU work, occurrence-scoped IndexJoin skipped-inner and IndexLookUp/IndexMerge final-table proof/masking with exact-subtree preorder closure, union/intersection IndexMerge zero-handle shapes, exact Count-zero PhysicalLimit and embedded IndexLookUp/IndexMerge whole-subtree masking with pushdown/offset/evidence/outermost-nesting/parent-unary/active-Join-sibling/mixed-reader coverage and static-zero provenance, cross-tree plan-ID ownership, and transport isolation, network-only reader transport, typed PointGet/BatchPointGet Scan-equivalent `TotalKeys*ProcessedKeysSize/ProcessedKeys` work, successful response payload bytes, and raw coverage diagnostics with present-zero/missing/negative/overflow/nil-promoted/DML/locking cases, real distsql ScanDetail ownership/provenance and fail-closed coverage, HashJoin state inputs, calibrated mutation normalization into dimension-qualified `cpu_work`, and v6 autocommit/explicit/pessimistic/COMMIT/pipelined write-key/byte ownership and fail-closed pairs.
