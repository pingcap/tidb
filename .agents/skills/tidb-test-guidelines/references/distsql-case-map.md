...
- `pkg/distsql/select_result_test.go` - Tests coprocessor runtime-stat updates, including exact plan-ID row summaries, last-plan scan-detail ownership/provenance counts, multi-response aggregation, independent expected-task coverage, and close-time stats-only handling that cannot replay stale summaries.
