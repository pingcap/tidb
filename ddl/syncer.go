...
func PutKVToEtcd(ctx context.Context, etcdCli *clientv3.Client, retryCnt int, key, val string,
	...
	for i := 0; i < retryCnt; i++ {
		...
		log.Warn("etcd-cli put kv failed", zap.String("key", key), zap.String("value", val), zap.Error(err), zap.Int("retryCnt", i))
...
func DeleteKeyFromEtcd(key string, etcdCli *clientv3.Client, retryCnt int, timeout time.Duration) error {
	...
	for i := 0; i < retryCnt; i++ {
		...
		log.Warn("etcd-cli delete key failed", zap.String("key", key), zap.Error(err), zap.Int("retryCnt", i))
	}
	return errors.Trace(err)
}
	...
	for {
		if err != nil {
			if failedCnt%intervalCnt == 0 {
				log.Info("syncer get global version failed", zap.Error(err))
			}
			time.Sleep(keyOpRetryInterval)
			failedCnt++
		}
...
func (s *schemaVersionSyncer) OwnerCheckAllVersions(ctx context.Context, latestVer int64) error {
	...
	for {
		...
		if err != nil {
			log.Info("syncer check all versions failed, continue checking.", zap.Error(err))
			continue
		}
		...
		for _, kv := range resp.Kvs {
			...
			if err != nil {
				log.Info("syncer check all versions, convert value to int failed, continue checking.", zap.String("ddl", string(kv.Key)), zap.String("value", string(kv.Value)), zap.Error(err))
				succ = false
				break
			}
