...
const (
	...
	LitErrUnknownMemType    string = "[ddl-lightning] unknown struct mem required for lightning add index"
	...
	LitErrGetBackendFail    string = "[ddl-lightning] can not get cached backend"
	...
	LitErrRemoteDupCheckErr string = "[ddl-lightning] remote duplicate check error"
	LitErrRemoteDupExistErr string = "[ddl-lightning] remote duplicate index key exist"
	LitErrDiskQuotaLess     string = "[ddl-lightning] specified disk quota is less than 100 GB disable the lightning"
	LitErrIncompatiblePiTR  string = "[ddl-lightning] the storage config log-backup.enable should be false when the lightning backfill is enabled"
	...
	LitWarnBackendNotExist  string = "[ddl-lightning] backend not exist"
	...
	LitWarnExtentWorker     string = "[ddl-lightning] extend worker failed will use worker count number worker to keep doing backfill task"
	LitWarnDiskShortage     string = "[ddl-lightning] local disk storage shortage"
	...
	LitInfoCleanUpEngine    string = "[ddl-lightning] cleanUp one engine for index reorg task"
	...
	LitInfoDelEngine        string = "[ddl-lightning] delete one engine"
	...
	LitInfoEngineDelete     string = "[ddl-lightning] delete one engine from engine manager cache"
	LitInfoUnsafeImport     string = "[ddl-lightning] do a partial import data into the storage"
	LitInfoDiskMaxLimit     string = "[ddl-lightning] local disk storage usage arrive up limited"
	LitInfoDiskQuotaChg     string = "[ddl-lightning] local disk storage quota changed"
)
