...
func TestMemoryRoot(t *testing.T) {
	...
	require.True(t, memRoot.TryConsume(1023))
	require.True(t, memRoot.TryConsume(1024))
	require.False(t, memRoot.TryConsume(1025))

	memRoot.Consume(512)
	require.Equal(t, int64(512), memRoot.CurrentUsage())
	require.True(t, memRoot.TryConsume(512))
	require.False(t, memRoot.TryConsume(513))
	require.Equal(t, int64(1024), memRoot.MaxMemoryQuota())
	...
	require.False(t, memRoot.TryConsume(20)) // 502+20 > 512
	memRoot.Release(502)
	...
	require.False(t, memRoot.TryConsume(1))
	memRoot.ReleaseWithTag("a")
	require.Equal(t, int64(512), memRoot.CurrentUsage())

	memRoot.ReleaseWithTag("a") // Double release.
	require.Equal(t, int64(512), memRoot.CurrentUsage())
	require.True(t, memRoot.TryConsume(10))
