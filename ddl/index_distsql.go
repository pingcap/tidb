...
func (w *addIndexWorker) fetchRowColValsFromSelect(ctx context.Context,
	srcResult distsql.SelectResult, handleRange reorgBackfillTask) ([]*indexRecord, kv.Key, bool, error) {
	...
	for {
		...
		for row := iter.Begin(); row != iter.End(); row = iter.Next() {
			...
			if len(w.idxRecords) >= w.batchCnt {
				key := tablecodec.EncodeRecordKey(w.table.RecordPrefix(), handle).Next()
				return w.idxRecords, key, false, nil
			}
			rsData := tables.TryGetHandleRestoredDataWrapper(w.table, hdDt, nil, w.index.Meta())
			w.idxRecords = append(w.idxRecords, &indexRecord{handle: handle, key: nil, vals: idxDt, rsData: rsData, skip: false})
			current = handle
		}
	}
	...
	if current != nil {
		key := tablecodec.EncodeRecordKey(w.table.RecordPrefix(), current).Next()
		return w.idxRecords, key, taskDone, nil
	}
	return w.idxRecords, handleRange.endKey, taskDone, nil
}
