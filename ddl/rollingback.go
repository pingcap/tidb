...
func rollingbackModifyColumn(w *worker, d *ddlCtx, t *meta.Meta, job *model.Job) (ver int64, err error) {
	...
	if !needChangeColumnData(oldCol, modifyInfo.newCol) {
		...
		if job.SchemaState == model.StateNone {
			...
			if modifyInfo.modifyColumnTp == mysql.TypeNull && tblInfo.Columns[oldCol.Offset].GetFlag()|mysql.PreventNullInsertFlag != 0 {
				job.State = model.JobStateRollingback
				return ver, dbterror.ErrCancelledDDLJob
			}
...
func rollingbackDropTableOrView(d *ddlCtx, t *meta.Meta, job *model.Job) error {
...
func convertJob2RollbackJob(w *worker, d *ddlCtx, t *meta.Meta, job *model.Job) (ver int64, err error) {
	...
	case model.ActionDropTable, model.ActionDropView, model.ActionDropSequence:
		err = rollingbackDropTableOrView(d, t, job)
