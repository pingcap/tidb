...
func (s *testStateChangeSuite) TestParallelRenameModifyColumn(c *C) {
	sql1 := "alter table t rename to t_add;"
	sql2 := "alter table t change c d int;"
...
func (s *testStateChangeSuite) TestParallelRenameDropTable(c *C) {
	sql1 := "alter table t rename to t_add;"
	sql2 := "drop table t;"
	defer s.se.Execute(context.Background(), "drop table t_add;")
	f := func(c *C, err1, err2 error) {
		c.Assert(err1, IsNil)
		c.Assert(err2.Error(), Equals, "[schema:1051]Unknown table 'test_db_state.t'")
	}
	s.testControlParallelExecSQL(c, sql1, sql2, f)
}
...
func (s *testStateChangeSuite) TestParallelRenameDropColumn(c *C) {
	sql1 := "alter table t rename to t_add;"
	sql2 := "alter table t drop c;"
	defer s.se.Execute(context.Background(), "drop table t_add;")
	f := func(c *C, err1, err2 error) {
		c.Assert(err1, IsNil)
		c.Assert(err2.Error(), Equals, "[schema:1146]Table 'test_db_state.t' doesn't exist")
...
func (s *testStateChangeSuite) TestParallelRenameCreateIndex(c *C) {
	sql1 := "alter table t rename to t_add;"
	sql2 := "alter table t add index index_b(b);"
	defer s.se.Execute(context.Background(), "drop table t_add;")
	f := func(c *C, err1, err2 error) {
		c.Assert(err1, IsNil)
		c.Assert(err2.Error(), Equals, "[schema:1146]Table 'test_db_state.t' doesn't exist")
...
func (s *testStateChangeSuite) TestParallelRenameAlterColumn(c *C) {
	sql1 := "alter table t rename to t_add;"
	sql2 := "alter table t alter column a set default 0;"
	defer s.se.Execute(context.Background(), "drop table t_add;")
	f := func(c *C, err1, err2 error) {
		c.Assert(err1, IsNil)
		c.Assert(err2.Error(), Equals, "[schema:1146]Table 'test_db_state.t' doesn't exist")
...
func (s *testStateChangeSuite) TestParallelRenameRebaseAutoID(c *C) {
	sql1 := "alter table t rename to t_add;"
	sql2 := "alter table t auto_increment = 5;"
	defer s.se.Execute(context.Background(), "drop table t_add;")
	f := func(c *C, err1, err2 error) {
		c.Assert(err1, IsNil)
		c.Assert(err2.Error(), Equals, "[schema:1146]Table 'test_db_state.t' doesn't exist")
...
func (s *testStateChangeSuite) TestParallelRenameTableModifyTableComment(c *C) {
	sql1 := "alter table t rename to t_add;"
	sql2 := "alter table t comment 'table_t';"
	defer s.se.Execute(context.Background(), "drop table t_add;")
	f := func(c *C, err1, err2 error) {
		c.Assert(err1, IsNil)
		c.Assert(err2.Error(), Equals, "[schema:1146]Table 'test_db_state.t' doesn't exist")
