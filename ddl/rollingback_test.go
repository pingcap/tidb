...
func (s *testRollingBackSuite) TestCancelAddIndexJobError(c *C) {
	...
	c.Assert(err.Error(), Equals, "[ddl:-1]job being converted to rollback job errors and error count beyond the limitation 3, cancelled")
	...
	c.Assert(job.Error.Error(), Equals, "[ddl:-1]job being converted to rollback job errors and error count beyond the limitation 3, cancelled")
