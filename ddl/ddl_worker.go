...
// countForErrorOrPanic records the error count for ddl job.
// 1: err not nil comes from normal error case.
// 2: err nil comes from abnormal panic case.
func (w *worker) countForErrorOrPanic(err error, job *model.Job) error {
	if err == nil {
...
func (w *worker) runDDLJob(d *ddlCtx, t *meta.Meta, job *model.Job) (ver int64, err error) {
	...
	if err != nil {
		err = w.countForErrorOrPanic(err, job)
	}
	return
}
