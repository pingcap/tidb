...
func (d *ddl) addBatchDDLJobs2Table(tasks []*limitJobTask) error {
	...
	job, err := getJobsBySQL(session.NewSession(sess), JobTable, fmt.Sprintf("type = %d", model.ActionFlashbackCluster))
	if err != nil {
		return errors.Trace(err)
	}
	if len(job) != 0 {
	...
	if err == nil {
		...
		err = insertDDLJobs2Table(session.NewSession(sess), true, jobTasks...)
	}
	return errors.Trace(err)
}
