...
func (w *jobContext) setDDLLabelForTopSQL(job *model.Job) {
	...
	if job.Query != w.cacheSQL || w.cacheDigest == nil {
		...
		w.ddlJobCtx = topsql.AttachSQLInfo(context.Background(), w.cacheNormalizedSQL, w.cacheDigest, "", nil, false, false)
	} else {
		topsql.AttachSQLInfo(w.ddlJobCtx, w.cacheNormalizedSQL, w.cacheDigest, "", nil, false, false)
	}
}
