...
func (d *ddl) getJob(sess *session.Session, tp jobType, filter func(*model.Job) (bool, error)) (*model.Job, error) {
	not := "not"
...
func (d *ddl) getGeneralJob(sess *session.Session) (*model.Job, error) {
...
func (d *ddl) checkJobIsRunnable(sess *session.Session, sql string) (bool, error) {
	rows, err := sess.Execute(context.Background(), sql, "check_runnable")
	return len(rows) == 0, err
}

func (d *ddl) getReorgJob(sess *session.Session) (*model.Job, error) {
...
func (d *ddl) loadDDLJobAndRun(sess *session.Session, pool *workerPool, getJob func(*session.Session) (*model.Job, error)) {
	wk, err := pool.get()
	if err != nil || wk == nil {
...
func (d *ddl) markJobProcessing(sess *session.Session, job *model.Job) error {
...
func insertDDLJobs2Table(sess *session.Session, updateRawArgs bool, jobs ...*model.Job) error {
...
func updateDDLJob2Table(sctx *session.Session, job *model.Job, updateRawArgs bool) error {
	b, err := job.Encode(updateRawArgs)
	if err != nil {
		return err
	}
...
func getDDLReorgHandle(sess *session.Session, job *model.Job) (element *meta.Element, startKey, endKey kv.Key, physicalTableID int64, err error) {
...
func updateDDLReorgHandle(sess *session.Session, jobID int64, startKey kv.Key, endKey kv.Key, physicalTableID int64, element *meta.Element) error {
...
func removeDDLReorgHandle(s *session.Session, job *model.Job, elements []*meta.Element) error {
	if len(elements) == 0 {
		return nil
	}
...
func removeReorgElement(s *session.Session, job *model.Job) error {
	sql := fmt.Sprintf("delete from mysql.tidb_ddl_reorg where job_id = %d", job.ID)
	return s.RunInTxn(func(se *session.Session) error {
		_, err := se.Execute(context.Background(), sql, "remove_handle")
		return err
...
func cleanDDLReorgHandles(s *session.Session, job *model.Job) error {
...
func getJobsBySQL(sess *session.Session, tbl, condition string) ([]*model.Job, error) {
...
func AddBackfillHistoryJob(sess *session.Session, backfillJobs []*BackfillJob) error {
...
func AddBackfillJobs(s *session.Session, backfillJobs []*BackfillJob) error {
...
func GetBackfillJobsForOneEle(s *session.Session, batch int, excludedJobIDs []int64, lease time.Duration) ([]*BackfillJob, error) {
	...
	err = s.RunInTxn(func(se *session.Session) error {
...
func GetAndMarkBackfillJobsForOneEle(s *session.Session, batch int, jobID int64, uuid string, lease time.Duration) ([]*BackfillJob, error) {
	var validLen int
	var bJobs []*BackfillJob
	err := s.RunInTxn(func(se *session.Session) error {
		currTime, err := GetOracleTimeWithStartTS(se)
...
func GetInterruptedBackfillJobsForOneEle(sess *session.Session, jobID, eleID int64, eleKey []byte) ([]*BackfillJob, error) {
...
func GetBackfillJobCount(sess *session.Session, tblName, condition string, label string) (int, error) {
...
func getUnsyncedInstanceIDs(sess *session.Session, jobID int64, label string) ([]string, error) {
...
func GetBackfillJobs(sess *session.Session, tblName, condition string, label string) ([]*BackfillJob, error) {
...
func RemoveBackfillJob(sess *session.Session, isOneEle bool, backfillJob *BackfillJob) error {
...
func updateBackfillJob(sess *session.Session, tableName string, backfillJob *BackfillJob, label string) error {
