...
func initDDLReorgHandle(s *res.Session, jobID int64, startKey kv.Key, endKey kv.Key, physicalTableID int64, element *meta.Element) error {
...
func removeDDLReorgHandle(s *res.Session, job *model.Job, elements []*meta.Element) error {
	if len(elements) == 0 {
		return nil
	}
...
func removeReorgElement(s *res.Session, job *model.Job) error {
	sql := fmt.Sprintf("delete from mysql.tidb_ddl_reorg where job_id = %d", job.ID)
	return res.RunInTxn(s, func(se *res.Session) error {
		_, err := se.Execute(context.Background(), sql, "remove_handle")
		return err
...
func cleanDDLReorgHandles(s *res.Session, job *model.Job) error {
...
func AddBackfillJobs(s *res.Session, backfillJobs []*BackfillJob) error {
...
func GetBackfillJobsForOneEle(s *res.Session, batch int, excludedJobIDs []int64, lease time.Duration) ([]*BackfillJob, error) {
...
func GetAndMarkBackfillJobsForOneEle(s *res.Session, batch int, jobID int64, uuid string, lease time.Duration) ([]*BackfillJob, error) {
	var validLen int
	var bJobs []*BackfillJob
	err := res.RunInTxn(s, func(se *res.Session) error {
		currTime, err := GetOracleTimeWithStartTS(se)
...
func getUnsyncedInstanceIDs(sess *res.Session, jobID int64, label string) ([]string, error) {
