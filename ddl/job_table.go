...
func (d *ddl) getJob(sess *res.Session, tp jobType, filter func(*model.Job) (bool, error)) (*model.Job, error) {
	not := "not"
...
func (d *ddl) getGeneralJob(sess *res.Session) (*model.Job, error) {
...
func (d *ddl) checkJobIsRunnable(sess *res.Session, sql string) (bool, error) {
	rows, err := sess.Execute(context.Background(), sql, "check_runnable")
	return len(rows) == 0, err
}

func (d *ddl) getReorgJob(sess *res.Session) (*model.Job, error) {
...
func (d *ddl) loadDDLJobAndRun(sess *res.Session, pool *workerPool, getJob func(*res.Session) (*model.Job, error)) {
	wk, err := pool.get()
	if err != nil || wk == nil {
...
func (d *ddl) markJobProcessing(sess *res.Session, job *model.Job) error {
...
func insertDDLJobs2Table(sess *res.Session, updateRawArgs bool, jobs ...*model.Job) error {
...
func updateDDLJob2Table(sctx *res.Session, job *model.Job, updateRawArgs bool) error {
	b, err := job.Encode(updateRawArgs)
	if err != nil {
		return err
	}
...
func getDDLReorgHandle(sess *res.Session, job *model.Job) (element *meta.Element, startKey, endKey kv.Key, physicalTableID int64, err error) {
...
func updateDDLReorgStartHandle(sess *res.Session, job *model.Job, element *meta.Element, startKey kv.Key) error {
...
func updateDDLReorgHandle(sess *res.Session, jobID int64, startKey kv.Key, endKey kv.Key, physicalTableID int64, element *meta.Element) error {
...
func initDDLReorgHandle(sess *res.Session, jobID int64, startKey kv.Key, endKey kv.Key, physicalTableID int64, element *meta.Element) error {
...
func removeDDLReorgHandle(sess *res.Session, job *model.Job, elements []*meta.Element) error {
	if len(elements) == 0 {
		return nil
	}
...
func removeReorgElement(sess *res.Session, job *model.Job) error {
	sql := fmt.Sprintf("delete from mysql.tidb_ddl_reorg where job_id = %d", job.ID)
	_, err := sess.Execute(context.Background(), sql, "remove_handle")
	return err
}
...
func getJobsBySQL(sess *res.Session, tbl, condition string) ([]*model.Job, error) {
...
func AddBackfillJobs(sess *res.Session, backfillJobs []*BackfillJob) error {
...
func AddBackfillHistoryJob(sess *res.Session, backfillJobs []*BackfillJob) error {
	return addBackfillJobs(sess, BackfillHistoryTable, backfillJobs)
}

// addBackfillJobs adds the backfill jobs to the tidb_ddl_backfill table.
func addBackfillJobs(sess *res.Session, tableName string, backfillJobs []*BackfillJob) error {
...
func GetBackfillJobsForOneEle(sess *res.Session, batch int, excludedJobIDs []int64, lease time.Duration) ([]*BackfillJob, error) {
	...
	s := res.NewSession(sess)
	err = res.RunInTxn(s, func(se *res.Session) error {
...
func GetAndMarkBackfillJobsForOneEle(sess *res.Session, batch int, jobID int64, uuid string, lease time.Duration) ([]*BackfillJob, error) {
	var validLen int
	var bJobs []*BackfillJob
	s := res.NewSession(sess)
	err := res.RunInTxn(s, func(se *res.Session) error {
...
func GetInterruptedBackfillJobsForOneEle(sess *res.Session, jobID, eleID int64, eleKey []byte) ([]*BackfillJob, error) {
...
func GetBackfillJobCount(sess *res.Session, tblName, condition string, label string) (int, error) {
...
func GetBackfillJobs(sess *res.Session, tblName, condition string, label string) ([]*BackfillJob, error) {
...
func RemoveBackfillJob(sess *res.Session, isOneEle bool, backfillJob *BackfillJob) error {
...
func updateBackfillJob(sess *res.Session, tableName string, backfillJob *BackfillJob, label string) error {
