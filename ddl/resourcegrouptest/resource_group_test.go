...
func TestResourceGroupBasic(t *testing.T) {
	...
	tk.MustExec("alter resource group y RU_PER_SEC=4000 PRIORITY=10 BURSTABLE")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'y'").Check(testkit.Rows("y 4000 10 YES"))
	tk.MustQuery("show create resource group y").Check(testkit.Rows("y CREATE RESOURCE GROUP `y` RU_PER_SEC=4000 PRIORITY=10 BURSTABLE"))
