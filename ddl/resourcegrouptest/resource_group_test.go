...
func TestResourceGroupBasic(t *testing.T) {
	...
	tk.MustExec("alter resource group x RU_PER_SEC=2000 BURSTABLE QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC='15s' ACTION COOLDOWN WATCH SIMILAR DURATION '10m')")
	g = testResourceGroupNameFromIS(t, tk.Session(), "x")
	...
	tk.MustQuery("select * from information_schema.resource_groups where name = 'x'").Check(testkit.Rows("x 2000 MEDIUM YES EXEC_ELAPSED_IN_SEC=15s, ACTION=COOLDOWN, WATCH=SIMILAR[10m0s]"))
	...
	g = testResourceGroupNameFromIS(t, tk.Session(), "y")
	checkFunc(g)
	tk.MustQuery("select * from information_schema.resource_groups where name = 'y'").Check(testkit.Rows("y 5000 MEDIUM YES EXEC_ELAPSED_IN_SEC=15s, ACTION=KILL"))
	...
	tk.MustContainErrMsg("create resource group x ru_per_sec=1000 burstable QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC='15s' action kill action cooldown)", "Dupliated options specified")
	tk.MustContainErrMsg("create resource group x ru_per_sec=1000 QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC='15s') QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC='15s')", "Dupliated options specified")
	tk.MustContainErrMsg("create resource group x ru_per_sec=1000 QUERY_LIMIT=(action kill)", "invalid exec elapsed time")
	tk.MustGetErrCode("create resource group x ru_per_sec=1000 QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC='15s' action kil)", mysql.ErrParse)
	tk.MustContainErrMsg("create resource group x ru_per_sec=1000 QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC='15s')", "unknown resource group runaway action")
	tk.MustGetErrCode("create resource group x ru_per_sec=1000 EXEC_ELAPSED_IN_SEC='15s' action kill", mysql.ErrParse)
	tk.MustContainErrMsg("create resource group x ru_per_sec=1000 QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC='15d' action kill)", "unknown unit \"d\"")
	...
	tk.MustExec("alter resource group x RU_PER_SEC=2000 BURSTABLE QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC='15s' action kill)")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'x'").Check(testkit.Rows("x 2000 LOW YES EXEC_ELAPSED_IN_SEC=15s, ACTION=KILL"))
	tk.MustQuery("show create resource group x").Check(testkit.Rows("x CREATE RESOURCE GROUP `x` RU_PER_SEC=2000, PRIORITY=LOW, BURSTABLE, QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC=\"15s\" ACTION=KILL)"))
	tk.MustExec("CREATE RESOURCE GROUP `x_new` RU_PER_SEC=2000 PRIORITY=LOW BURSTABLE=true QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC=\"15s\" ACTION=KILL)")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'x_new'").Check(testkit.Rows("x_new 2000 LOW YES EXEC_ELAPSED_IN_SEC=15s, ACTION=KILL"))
	tk.MustExec("alter resource group x BURSTABLE=false RU_PER_SEC=3000")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'x'").Check(testkit.Rows("x 3000 LOW NO EXEC_ELAPSED_IN_SEC=15s, ACTION=KILL"))
	tk.MustQuery("show create resource group x").Check(testkit.Rows("x CREATE RESOURCE GROUP `x` RU_PER_SEC=3000, PRIORITY=LOW, QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC=\"15s\" ACTION=KILL)"))

	tk.MustExec("create resource group y BURSTABLE RU_PER_SEC=2000 QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC='1s' action COOLDOWN WATCH EXACT duration '1h')")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'y'").Check(testkit.Rows("y 2000 MEDIUM YES EXEC_ELAPSED_IN_SEC=1s, ACTION=COOLDOWN, WATCH=EXACT[1h0m0s]"))
	tk.MustQuery("show create resource group y").Check(testkit.Rows("y CREATE RESOURCE GROUP `y` RU_PER_SEC=2000, PRIORITY=MEDIUM, BURSTABLE, QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC=\"1s\" ACTION=COOLDOWN WATCH=EXACT DURATION=\"1h0m0s\")"))
	tk.MustExec("CREATE RESOURCE GROUP `y_new` RU_PER_SEC=2000 PRIORITY=MEDIUM QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC=\"1s\" ACTION=COOLDOWN WATCH EXACT DURATION=\"1h0m0s\")")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'y_new'").Check(testkit.Rows("y_new 2000 MEDIUM NO EXEC_ELAPSED_IN_SEC=1s, ACTION=COOLDOWN, WATCH=EXACT[1h0m0s]"))
	tk.MustExec("alter resource group y_new RU_PER_SEC=3000")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'y_new'").Check(testkit.Rows("y_new 3000 MEDIUM NO EXEC_ELAPSED_IN_SEC=1s, ACTION=COOLDOWN, WATCH=EXACT[1h0m0s]"))

	tk.MustExec("alter resource group y RU_PER_SEC=4000")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'y'").Check(testkit.Rows("y 4000 MEDIUM YES EXEC_ELAPSED_IN_SEC=1s, ACTION=COOLDOWN, WATCH=EXACT[1h0m0s]"))
	tk.MustQuery("show create resource group y").Check(testkit.Rows("y CREATE RESOURCE GROUP `y` RU_PER_SEC=4000, PRIORITY=MEDIUM, BURSTABLE, QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC=\"1s\" ACTION=COOLDOWN WATCH=EXACT DURATION=\"1h0m0s\")"))

	tk.MustExec("alter resource group y RU_PER_SEC=4000 PRIORITY=HIGH BURSTABLE")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'y'").Check(testkit.Rows("y 4000 HIGH YES EXEC_ELAPSED_IN_SEC=1s, ACTION=COOLDOWN, WATCH=EXACT[1h0m0s]"))
	tk.MustQuery("show create resource group y").Check(testkit.Rows("y CREATE RESOURCE GROUP `y` RU_PER_SEC=4000, PRIORITY=HIGH, BURSTABLE, QUERY_LIMIT=(EXEC_ELAPSED_IN_SEC=\"1s\" ACTION=COOLDOWN WATCH=EXACT DURATION=\"1h0m0s\")"))
