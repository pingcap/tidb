		...
		t.Run(fmt.Sprintf("isCommandHandle(%v)", test.isCommonHandle), func(t *testing.T) {
			...
			for i := 0; i < 1000; i++ {
				...
				if err == nil {
					...
					m = meta.NewMeta(txn)
					info, err1 := getReorgInfo(&workerCtx{}, d.ddlCtx, m, job, mockTbl, nil)
			...
			err = kv.RunInNewTxn(context.Background(), d.store, false, func(ctx context.Context, txn kv.Transaction) error {
				m := meta.NewMeta(txn)
				var err1 error
				_, err1 = getReorgInfo(&workerCtx{}, d.ddlCtx, m, job, mockTbl, []*meta.Element{element})
			...
			err = kv.RunInNewTxn(context.Background(), d.store, false, func(ctx context.Context, txn kv.Transaction) error {
				m := meta.NewMeta(txn)
				info1, err1 := getReorgInfo(&workerCtx{}, d.ddlCtx, m, job, mockTbl, []*meta.Element{element})
				require.NoError(t, err1)
