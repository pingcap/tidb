...
func (s *testIndexChangeSuite) TestIndexChange(c *C) {
	...
	_, err = originTable.AddRecord(ctx, types.MakeDatums(1, 1), false, true)
	c.Assert(err, IsNil)
	_, err = originTable.AddRecord(ctx, types.MakeDatums(2, 2), false, true)
	c.Assert(err, IsNil)
	_, err = originTable.AddRecord(ctx, types.MakeDatums(3, 3), false, true)
	c.Assert(err, IsNil)
...
func (s *testIndexChangeSuite) checkAddWriteOnly(d *ddl, ctx sessionctx.Context, delOnlyTbl, writeOnlyTbl table.Table) error {
	...
	_, err = delOnlyTbl.AddRecord(ctx, types.MakeDatums(4, 4), false, true)
	if err != nil {
		return errors.Trace(err)
	}
	...
	_, err = writeOnlyTbl.AddRecord(ctx, types.MakeDatums(5, 5), false, true)
	if err != nil {
		return errors.Trace(err)
	}
...
func (s *testIndexChangeSuite) checkAddPublic(d *ddl, ctx sessionctx.Context, writeTbl, publicTbl table.Table) error {
	...
	_, err = writeTbl.AddRecord(ctx, types.MakeDatums(6, 6), false, true)
	if err != nil {
		return errors.Trace(err)
	}
	...
	_, err = publicTbl.AddRecord(ctx, types.MakeDatums(7, 7), false, true)
	if err != nil {
		return errors.Trace(err)
	}
...
func (s *testIndexChangeSuite) checkDropWriteOnly(d *ddl, ctx sessionctx.Context, publicTbl, writeTbl table.Table) error {
	...
	_, err = writeTbl.AddRecord(ctx, types.MakeDatums(8, 8), false, true)
	if err != nil {
		return errors.Trace(err)
	}
...
func (s *testIndexChangeSuite) checkDropDeleteOnly(d *ddl, ctx sessionctx.Context, writeTbl, delTbl table.Table) error {
	...
	_, err = writeTbl.AddRecord(ctx, types.MakeDatums(9, 9), false, true)
	if err != nil {
		return errors.Trace(err)
	}
	...
	_, err = delTbl.AddRecord(ctx, types.MakeDatums(10, 10), false, true)
	if err != nil {
		return errors.Trace(err)
	}
