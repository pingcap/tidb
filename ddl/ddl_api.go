...
func (d *ddl) CreateIndex(ctx sessionctx.Context, ti ast.Ident, keyType ast.IndexKeyType, indexName model.CIStr,
	...
	finnalColumns := make([]*model.ColumnInfo, len(tblInfo.Columns), len(tblInfo.Columns) + len(hiddenCols))
	copy(finnalColumns, tblInfo.Columns)
	finnalColumns = append(finnalColumns, hiddenCols...)
	// Check before the job is put to the queue.
	// This check is redundant, but useful. If DDL check fail before the job is put
	// to job queue, the fail path logic is super fast.
	// After DDL job is put to the queue, and if the check fail, TiDB will run the DDL cancel logic.
	// The recover step causes DDL wait a few seconds, makes the unit test painfully slow.
	// For same reason, decide whether index is global here.
	indexColumns, err := buildIndexColumns(finnalColumns, indexPartSpecifications)
	if err != nil {
		return errors.Trace(err)
	}
	...
	if !unique && tblInfo.IsCommonHandle {
		...
		idxLen, err = indexColumnsLen(finnalColumns, indexColumns)
		if err != nil {
			return err
		}
		if pkLen+idxLen > config.GetGlobalConfig().MaxIndexLength {
