...
func (d *ddl) CreatePrimaryKey(ctx sessionctx.Context, ti ast.Ident, indexName model.CIStr,
	idxColNames []*ast.IndexColNameWithExpr, indexOption *ast.IndexOption) error {
...
func (d *ddl) CreateIndex(ctx sessionctx.Context, ti ast.Ident, keyType ast.IndexKeyType, indexName model.CIStr,
	idxColNames []*ast.IndexColNameWithExpr, indexOption *ast.IndexOption, ifNotExists bool) error {
...
func buildFKInfo(fkName model.CIStr, keys []*ast.IndexColNameWithExpr, refer *ast.ReferenceDef, cols []*table.Column, tbInfo *model.TableInfo) (*model.FKInfo, error) {
	if len(keys) != len(refer.IndexColNameWithExprs) {
	...
	fkInfo.RefCols = make([]model.CIStr, len(refer.IndexColNameWithExprs))
	for i, key := range refer.IndexColNameWithExprs {
...
func (d *ddl) CreateForeignKey(ctx sessionctx.Context, ti ast.Ident, fkName model.CIStr, keys []*ast.IndexColNameWithExpr, refer *ast.ReferenceDef) error {
	is := d.infoHandle.Get()
