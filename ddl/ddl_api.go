...
func (d *ddl) getModifiableColumnJob(ctx sessionctx.Context, ident ast.Ident, originalColName model.CIStr,
	...
	if !mysql.HasNotNullFlag(col.Flag) && mysql.HasNotNullFlag(newCol.Flag) {
		var isDataTruncated bool
		for _, column := range t.Cols() {
			if !mysql.HasNotNullFlag(column.Flag) {
				isDataTruncated = true
				break
			}
		}
		if err = checkForNullValue(ctx, isDataTruncated, ident.Schema, ident.Name, newCol.Name, col.ColumnInfo); err != nil {
			return nil, errors.Trace(err)
		}
