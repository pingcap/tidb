...
func (s *stateChangeSuite) TestShowCreateTable() {
	...
	callback.OnJobUpdatedExported = func(job *model.Job) {
		...
		if job.SchemaState != model.StatePublic {
			...
			tbl2 := testutil.GetTableByName(s.T(), tkInternal, "test", "t2")
...
func (s *stateChangeSuite) TestParallelUpdateTableReplica() {
	...
	t1 := testutil.GetTableByName(s.T(), s.tk, "test_db_state", "t1")
...
func (s *stateChangeSuite) TestModifyColumnTypeArgs() {
	...
	tbl := testutil.GetTableByName(s.T(), tk, "test", "t_modify_column_args")
