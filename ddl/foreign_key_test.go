...
func createForeignKeySuite(t *testing.T) (s *foreignKeySuite, clean func()) {
	s = new(foreignKeySuite)
	s.store = testCreateStore(t, "test_foreign")
	clean = func() {
		err := s.store.Close()
...
func (s *foreignKeySuite) testCreateForeignKey(t *testing.T, tblInfo *model.TableInfo, fkName string, keys []string, refTable string, refKeys []string, onDelete ast.ReferOptionType, onUpdate ast.ReferOptionType) *model.Job {
...
func TestForeignKey(t *testing.T) {
	s, clean := createForeignKeySuite(t)
	defer clean()
	...
	s.d = d
	s.dbInfo = testSchemaInfo(t, d, "test_foreign")
	ctx := testNewContext(d)
	s.ctx = ctx
	testCreateSchema(t, ctx, d, s.dbInfo)
	...
	job := s.testCreateForeignKey(t, tblInfo, "c1_fk", []string{"c1"}, "t2", []string{"c1"}, ast.ReferOptionCascade, ast.ReferOptionSetNull)
	...
	job = testDropForeignKey(t, ctx, d, s.dbInfo, tblInfo, "c1_fk")
