...
func TestModifyColumnWithForeignKey(t *testing.T) {
	...
	err := tk.ExecToErr("alter table t1 modify column b varchar(5);")
	require.Error(t, err)
	require.Equal(t, "[ddl:1833]Cannot change column 'b': used in a foreign key constraint 'fk' of table 'test.t2'", err.Error())
	err = tk.ExecToErr("alter table t1 modify column b bigint;")
	require.Error(t, err)
	require.Equal(t, "[ddl:3780]Referencing column 'a' and referenced column 'b' in foreign key constraint 'fk' are incompatible.", err.Error())
	tk.MustExec("alter table t1 modify column b varchar(20);")
	err = tk.ExecToErr("alter table t1 modify column b varchar(10);")
	require.Error(t, err)
	require.Equal(t, "[ddl:1833]Cannot change column 'b': used in a foreign key constraint 'fk' of table 'test.t2'", err.Error())
	tk.MustExec("alter table t2 modify column a varchar(20);")
	tk.MustExec("alter table t2 modify column a varchar(21);")
	err = tk.ExecToErr("alter table t2 modify column a varchar(5);")
	require.Error(t, err)
	require.Equal(t, "[ddl:1832]Cannot change column 'a': used in a foreign key constraint 'fk'", err.Error())
	err = tk.ExecToErr("alter table t2 modify column a bigint;")
	require.Error(t, err)
	require.Equal(t, "[ddl:3780]Referencing column 'a' and referenced column 'b' in foreign key constraint 'fk' are incompatible.", err.Error())
	...
	err = tk.ExecToErr("alter table t1 modify column b decimal(10, 6);")
	require.Error(t, err)
	require.Equal(t, "[ddl:1833]Cannot change column 'b': used in a foreign key constraint 'fk' of table 'test.t2'", err.Error())
	err = tk.ExecToErr("alter table t1 modify column b decimal(10, 3);")
	require.Error(t, err)
	require.Equal(t, "[ddl:1833]Cannot change column 'b': used in a foreign key constraint 'fk' of table 'test.t2'", err.Error())
	err = tk.ExecToErr("alter table t1 modify column b decimal(5, 2);")
	require.Error(t, err)
	require.Equal(t, "[ddl:1833]Cannot change column 'b': used in a foreign key constraint 'fk' of table 'test.t2'", err.Error())
	err = tk.ExecToErr("alter table t1 modify column b decimal(20, 10);")
	require.Error(t, err)
	require.Equal(t, "[ddl:1833]Cannot change column 'b': used in a foreign key constraint 'fk' of table 'test.t2'", err.Error())
	err = tk.ExecToErr("alter table t2 modify column a decimal(30, 15);")
	require.Error(t, err)
	require.Equal(t, "[ddl:1832]Cannot change column 'a': used in a foreign key constraint 'fk'", err.Error())
