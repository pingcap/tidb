...
func (dr *delRange) addDelRangeJob(ctx context.Context, job *model.Job) error {
	...
	if err != nil {
		log.Error("add job into delete-range table failed", zap.String("category", "ddl"), zap.Int64("jobID", job.ID), zap.String("jobType", job.Type.String()), zap.Error(err))
		return errors.Trace(err)
	}
	if !dr.storeSupport {
		dr.emulatorCh <- struct{}{}
	}
	log.Info("add job into delete-range table", zap.String("category", "ddl"), zap.Int64("jobID", job.ID), zap.String("jobType", job.Type.String()))
	return nil
}
...
func (dr *delRange) clear() {
	log.Info("closing delRange", zap.String("category", "ddl"))
	close(dr.quitCh)
	dr.wait.Wait()
}
...
func (dr *delRange) startEmulator() {
	defer dr.wait.Done()
	log.Info("start delRange emulator", zap.String("category", "ddl"))
	for {
		select {
		case <-dr.emulatorCh:
		case <-dr.quitCh:
...
func (dr *delRange) doDelRangeWork() error {
	sctx, err := dr.sessPool.Get()
	if err != nil {
		log.Error("delRange emulator get session failed", zap.String("category", "ddl"), zap.Error(err))
		return errors.Trace(err)
	}
	defer dr.sessPool.Put(sctx)
	...
	if err != nil {
		log.Error("delRange emulator load tasks failed", zap.String("category", "ddl"), zap.Error(err))
		return errors.Trace(err)
	}

	for _, r := range ranges {
		if err := dr.doTask(sctx, r); err != nil {
			log.Error("delRange emulator do task failed", zap.String("category", "ddl"), zap.Error(err))
			return errors.Trace(err)
		}
	}
	return nil
	...
	for {
		...
		if finish {
			if err := util.CompleteDeleteRange(sctx, r, true); err != nil {
				log.Error("delRange emulator complete task failed", zap.String("category", "ddl"), zap.Error(err))
				return errors.Trace(err)
			}
			startKey, endKey := r.Range()
			log.Info("delRange emulator complete task", zap.String("category", "ddl"),
		...
		if err := util.UpdateDeleteRange(sctx, r, newStartKey, oldStartKey); err != nil {
			log.Error("delRange emulator update task failed", zap.String("category", "ddl"), zap.Error(err))
		}
...
func doBatchDeleteIndiceRange(ctx context.Context, s sqlexec.SQLExecutor, jobID, tableID int64, indexIDs []int64, ts uint64, ea *elementIDAlloc) error {
	log.Info("batch insert into delete-range indices", zap.String("category", "ddl"), zap.Int64("jobID", jobID), zap.Int64("tableID", tableID), zap.Int64s("indexIDs", indexIDs))
...
func doInsert(ctx context.Context, s sqlexec.SQLExecutor, jobID, elementID int64, startKey, endKey kv.Key, ts uint64, comment string) error {
	log.Info("insert into delete-range table", zap.String("category", "ddl"), zap.Int64("jobID", jobID), zap.Int64("elementID", elementID), zap.String("comment", comment))
	startKeyEncoded := hex.EncodeToString(startKey)
	endKeyEncoded := hex.EncodeToString(endKey)
...
func doBatchInsert(ctx context.Context, s sqlexec.SQLExecutor, jobID int64, tableIDs []int64, ts uint64, ea *elementIDAlloc) error {
	log.Info("batch insert into delete-range table", zap.String("category", "ddl"), zap.Int64("jobID", jobID), zap.Int64s("tableIDs", tableIDs))
	var buf strings.Builder
	buf.WriteString(insertDeleteRangeSQLPrefix)
