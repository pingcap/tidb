...
func (s *testDDLSuite) TestCancelJob(c *C) {
	...
	_, err = originTable.AddRecord(ctx, row, false, true)
	c.Assert(err, IsNil)
...
func (s *testDDLSuite) TestParallelDDL(c *C) {
	...
	*/
	...
	_, err = tbl1.AddRecord(ctx, types.MakeDatums(1, 1), false, true)
	c.Assert(err, IsNil)
	_, err = tbl1.AddRecord(ctx, types.MakeDatums(2, 2), false, true)
	c.Assert(err, IsNil)
	...
	_, err = tbl2.AddRecord(ctx, types.MakeDatums(1, 1, 1), false, true)
	c.Assert(err, IsNil)
	_, err = tbl2.AddRecord(ctx, types.MakeDatums(2, 2, 2), false, true)
	c.Assert(err, IsNil)
	_, err = tbl2.AddRecord(ctx, types.MakeDatums(3, 3, 3), false, true)
	c.Assert(err, IsNil)
	...
	_, err = tbl3.AddRecord(ctx, types.MakeDatums(11, 22, 33, 44), false, true)
	c.Assert(err, IsNil)
