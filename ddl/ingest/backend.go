...
func (bc *BackendContext) Flush(indexID int64) error {
	...
	if bc.diskRoot.CurrentUsage() >= uint64(importThreshold*float64(bc.diskRoot.MaxQuota())) {
		...
		err = bc.backend.UnsafeImportAndReset(bc.ctx, ei.uuid, int64(config.SplitRegionSize)*int64(config.MaxSplitRegionSizeRatio), int64(config.SplitRegionKeys), int64(config.MinRegionNum), config.PauseRegionByTable)
		if err != nil {
