...
func NewCheckpointManager(ctx context.Context, flushCtrl FlushController,
	...
	log.Info("create checkpoint manager", zap.String("category", "ddl-ingest"),
...
func (s *CheckpointManager) Close() {
	...
	s.updaterWg.Wait()
	log.Info("close checkpoint manager", zap.String("category", "ddl-ingest"),
...
func (s *CheckpointManager) Sync() {
	...
	if err != nil {
		log.Warn("flush local engine failed", zap.String("category", "ddl-ingest"), zap.Error(err))
	}
	s.mu.Lock()
	s.progressLocalSyncMinKey()
	s.mu.Unlock()
...
func (s *CheckpointManager) Reset(newPhysicalID int64, start, end kv.Key) {
	s.mu.Lock()
	defer s.mu.Unlock()
	log.Info("reset checkpoint manager", zap.String("category", "ddl-ingest"),
...
func (s *CheckpointManager) resumeCheckpoint() error {
	...
	return ddlSess.RunInTxn(func(se *sess.Session) error {
		...
		if cp := reorgMeta.Checkpoint; cp != nil {
			...
			log.Info("resume checkpoint", zap.String("category", "ddl-ingest"),
		...
		log.Info("checkpoint is empty", zap.String("category", "ddl-ingest"),
			zap.Int64("job ID", s.jobID), zap.Int64("index ID", s.indexID))
		return nil
	})
}
...
func (s *CheckpointManager) updateCheckpoint() error {
	...
	log.Info("update checkpoint", zap.String("category", "ddl-ingest"),
		zap.Int64("job ID", s.jobID), zap.Int64("index ID", s.indexID),
...
func (s *CheckpointManager) updateCheckpointLoop() {
	...
	for {
		...
		case wg := <-s.updaterCh:
			...
			if err != nil {
				log.Error("update checkpoint failed", zap.String("category", "ddl-ingest"), zap.Error(err))
			}
			wg.Done()
		case <-ticker.C:
			s.mu.Lock()
