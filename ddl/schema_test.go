...
func testSchemaInfo(t *testing.T, d *ddl, name string) *model.DBInfo {
	dbInfo := &model.DBInfo{
...
func isDDLJobDone(t *testing.T, trans *meta.Meta) bool {
	job, err := trans.GetDDLJobByIdx(0)
	require.NoError(t, err)
	if job == nil {
		return true
	}
...
func testCheckSchemaState(t *testing.T, d *ddl, dbInfo *model.DBInfo, state model.SchemaState) {
	isDropped := true

	for {
		err := kv.RunInNewTxn(context.Background(), d.store, false, func(ctx context.Context, txn kv.Transaction) error {
			trans := meta.NewMeta(txn)
			info, err := trans.GetDatabase(dbInfo.ID)
			require.NoError(t, err)

			if state == model.StateNone {
				isDropped = isDDLJobDone(t, trans)
				if !isDropped {
					return nil
				}
				require.Nil(t, info)
...
func TestSchema(t *testing.T) {
	...
	dbInfo := testSchemaInfo(t, d, "test_schema")
	...
	tblInfo1 := testTableInfo(t, d, "t", 3)
	...
	tblInfo2 := testTableInfo(t, d, "t1", 3)
	...
	dbInfo1 := testSchemaInfo(t, d, "test1")
...
func TestSchemaWaitJob(t *testing.T) {
	...
	dbInfo := testSchemaInfo(t, d2, "test_schema")
