...
func checkCancelState(txn kv.Transaction, job *model.Job, test *testCancelJob, sess sessionctx.Context) error {
	var checkErr error
	...
	if test.cancelState == job.SchemaState && !addIndexFirstReorg && !job.IsRollingback() {
		...
		if variable.AllowConcurrencyDDL.Load() {
			errs, err = CancelConcurrencyJobs(sess, test.jobIDs)
		} else {
...
func (s *testDDLSerialSuiteToVerify) TestCancelJob() {
	...
	hookCancelFunc := func(job *model.Job) {
		...
		mu.Lock()
		checkErr = checkCancelState(txn, job, test, ctx)
		mu.Unlock()
		if checkErr != nil {
			return
		}
...
func doDDLJobErrWithSchemaState(ctx sessionctx.Context, ddl *ddl, t *testing.T, schemaID, tableID int64, tp model.ActionType,
	...
	testCheckJobCancelled(t, ddl.store, job, state)

	return job
}

func doDDLJobSuccess(ctx sessionctx.Context, ddl DDL, t *testing.T, schemaID, tableID int64, tp model.ActionType,
	args []interface{}) {
	job := &model.Job{
		SchemaID:   schemaID,
		TableID:    tableID,
