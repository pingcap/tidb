...
func (w *worker) doModifyColumn(
	...
	if oldCol.ID != newCol.ID {
		job.State = model.JobStateRollingback
		return ver, errKeyColumnDoesNotExits.GenWithStack("column %s id %d does not exist, column id may be updated by other DDL ran in parallel", oldCol.Name, newCol.ID)
	}
	// Column from null to not null.
