...
func (w *worker) doModifyColumnTypeWithData(
	...
	case model.StateNone:
		...
		job.SchemaState = model.StateDeleteOnly
		getBackfillProgressByTableID(tblInfo.ID, metrics.LblModifyColumn, job.SchemaName, tblInfo.Name.String()).Set(0)
...
func newUpdateColumnWorker(sessCtx sessionctx.Context, id int, t table.PhysicalTable, decodeColMap map[int64]decoder.Column, reorgInfo *reorgInfo, jc *JobContext) *updateColumnWorker {
	...
	return &updateColumnWorker{
		...
		oldColInfo:     oldCol,
		newColInfo:     newCol,
		metricCounter:  getBackfillTotalByTableID(reorgInfo.PhysicalTableID, "update_col_rate", reorgInfo.SchemaName, t.Meta().Name.String()),
