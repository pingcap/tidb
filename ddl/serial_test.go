...
func (s *testSerialSuite) TestCheckEnumLength(c *C) {
	...
	// disable enum/set length limit
	tk.MustExec("set @@global.tidb_enable_enum_length_limit=off")
	tk1 := testkit.NewTestKitWithInit(c, s.store)
	tk1.MustExec("drop table if exists t1,t2")
	_, err := tk1.Exec("create table t1 (a enum(''))")
	c.Assert(err, IsNil)
	tk1.MustExec("insert into t1 values(1)")
	tk1.MustQuery("select a from t1").Check(testkit.Rows(""))
	_, err = tk1.Exec("create table t2 (a set(''))")
	c.Assert(err, IsNil)
	// enable enum/set length limit
	tk.MustExec("set @@global.tidb_enable_enum_length_limit=on")
	tk2 := testkit.NewTestKitWithInit(c, s.store)
	tk2.MustExec("drop table if exists t1")
	tk2.MustGetErrCode("create table t1 (a enum(''))", errno.ErrTooLongValueForType)
	tk2.MustGetErrCode("create table t (a set(''))", errno.ErrTooLongValueForType)
	tk.MustExec("drop table if exists t1,t2")
	tk1.MustExec("drop table if exists t1,t2")
	tk2.MustExec("drop table if exists t1")
}
