...
func (s *testSerialSuite) TestCheckEnumLength(c *C) {
	...
	tk.MustGetErrCode("create table t1 (a enum(''))", errno.ErrTooLongValueInType)
	_, err := tk.Exec("create table t2 (a enum('a'))")
	c.Assert(err, IsNil)
	tk.MustGetErrCode("alter table t2 modify a enum('aaaa')", errno.ErrUnsupportedDDLOperation)
	tk.MustExec("create table t3 (id int primary key)")
	tk.MustGetErrCode("alter table t3 add a enum('')", errno.ErrTooLongValueInType)
	config.UpdateGlobal(func(conf *config.Config) {
		conf.EnableEnumLengthLimit = false
	})
	_, err = tk.Exec("create table t4 (a enum(''))")
	c.Assert(err, IsNil)
	tk.MustExec("insert into t4 values(1)")
	tk.MustQuery("select a from t4").Check(testkit.Rows(""))
	tk.MustExec("drop table if exists t1,t2,t3,t4")
}
