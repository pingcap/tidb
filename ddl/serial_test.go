...
func (s *testSerialSuite) TestCheckEnumLength(c *C) {
	...
	_, err := tk.Exec("create table t3 (a enum('a'))")
	c.Assert(err, IsNil)
	tk.MustGetErrCode("alter table t3 modify a enum('aaaa')", errno.ErrUnsupportedDDLOperation)
	tk.MustExec("create table t4 (id int primary key)")
	tk.MustGetErrCode("alter table t4 add a enum('')", errno.ErrTooLongValueInType)
	config.UpdateGlobal(func(conf *config.Config) {
	...
	tk.MustQuery("select a from t1").Check(testkit.Rows(""))
	tk.MustExec("drop table if exists t1,t2,t3,t4")
}
