...
func (s *testDBSuite) TestCheckTooBigFieldLength(c *C) {
	...
	s.tk.MustExec("create table tr_04 (a varchar(20000) charset utf8 ) default charset utf8;")
...
func (s *testDBSuite) TestCheckConvertToCharacter(c *C) {
	...
	rs, err := s.tk.Exec("alter table t charset utf8 collate utf8_bin;")
	c.Assert(err, NotNil)
	rs, err = s.tk.Exec("alter table t charset utf8mb4 collate utf8mb4_bin;")
	c.Assert(err, NotNil)
	rs, err = s.tk.Exec("alter table t charset latin collate latin1_bin;")
	c.Assert(err, NotNil)
	if rs != nil {
		rs.Close()
	}
