...
func (s *testDBSuite1) TestModifyColumnTime(c *C) {
	...
	tests := []struct {
		...
		{"datetime", `"9999-01-02 23:59:59"`, "tiemstamp", "", errno.ErrTruncatedWrongValue},
	...
	for _, t := range tests {
		...
		if t.err != 0 {
			...
			c.Assert(err, ErrorMatches, fmt.Sprintf("Error %d", t.err), Commentf("%+v", t))
			continue
		}
		c.Assert(err, IsNil, Commentf("%+v", t))

		tk.MustQuery("select a from t_mc").Check(testkit.Rows(t.expect))
	}
}
