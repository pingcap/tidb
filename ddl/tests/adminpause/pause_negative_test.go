	...
	hook.OnJobRunBeforeExported = func(job *model.Job) {
		...
		if job.Type == model.ActionAddIndex && job.State == model.JobStateRunning &&
			...
			require.NoError(t, failpoint.Enable(
				"github.com/pingcap/tidb/ddl/mockFailedCommandOnConcurencyDDL", `return(true)`))
			defer func() {
				require.NoError(t, failpoint.Disable(
					"github.com/pingcap/tidb/ddl/mockFailedCommandOnConcurencyDDL"))
			}()
...
func TestPauseFailedOnCommit(t *testing.T) {
	...
	hook.OnJobRunBeforeExported = func(job *model.Job) {
		...
		if job.Type == model.ActionAddIndex && job.State == model.JobStateRunning &&
			job.SchemaState == model.StateWriteReorganization {
			require.NoError(t, failpoint.Enable(
				"github.com/pingcap/tidb/ddl/mockCommitFailedOnDDLCommand", `return(true)`))
			defer func() {
