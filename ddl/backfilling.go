...
func (w *backfillWorker) handleBackfillTask(d *ddlCtx, task *reorgBackfillTask, bf backfiller) *backfillResult {
	...
	for {
		...
		if num := result.scanCount - lastLogCount; num >= 90000 {
			...
			log.Info("backfill worker back fill index", zap.String("category", "ddl"), zap.Stringer("worker", w),
	...
	log.Info("backfill worker finish task", zap.String("category", "ddl"),
...
func (w *backfillWorker) run(d *ddlCtx, bf backfiller, job *model.Job) {
	log.Info("backfill worker start", zap.String("category", "ddl"), zap.Stringer("worker", w))
	var curTaskID int
	defer w.wg.Done()
	...
	for {
		if util.HasCancelled(w.ctx) {
			log.Info("backfill worker exit on context done", zap.String("category", "ddl"), zap.Stringer("worker", w))
			return
		}
		task, more := <-w.taskCh
		if !more {
			log.Info("backfill worker exit", zap.String("category", "ddl"), zap.Stringer("worker", w))
			return
		}
		curTaskID = task.id
		d.setDDLLabelForTopSQL(job.ID, job.Query)

		log.Debug("backfill worker got task", zap.String("category", "ddl"), zap.Int("workerID", w.GetCtx().id), zap.String("task", task.String()))
		...
		w.resultCh <- result
		if result.err != nil {
			log.Info("backfill worker exit on error", zap.String("category", "ddl"),
...
func splitTableRanges(t table.PhysicalTable, store kv.Storage, startKey, endKey kv.Key, limit int) ([]kv.KeyRange, error) {
	log.Info("split table range from PD", zap.String("category", "ddl"),
...
func (s *resultConsumer) run(scheduler backfillScheduler, start kv.Key, totalAddedCount *int64) {
	...
	go func() {
		...
		if err != nil {
			log.Warn("backfill worker handle tasks failed", zap.String("category", "ddl"),
			...
			log.Info("backfill workers successfully processed", zap.String("category", "ddl"),
...
func handleOneResult(result *backfillResult, scheduler backfillScheduler, consumer *resultConsumer,
	...
	if result.err != nil {
		log.Warn("backfill worker failed", zap.String("category", "ddl"),
	...
	if taskSeq%(scheduler.currentWorkerSize()*4) == 0 {
		if !consumer.distribute {
			...
			if err != nil {
				log.Warn("backfill worker is not runnable", zap.String("category", "ddl"), zap.Error(err))
				scheduler.drainTasks() // Make it quit early.
				return err
			}
			...
			if err != nil {
				log.Warn("update reorg meta failed", zap.String("category", "ddl"),
		...
		if err != nil {
			log.Warn("cannot adjust backfill worker size", zap.String("category", "ddl"),
				zap.Int64("job ID", reorgInfo.ID), zap.Error(err))
		}
	}
	return nil
...
func getBatchTasks(t table.Table, reorgInfo *reorgInfo, kvRanges []kv.KeyRange,
	...
	for _, keyRange := range kvRanges {
		...
		if err != nil {
			log.Info("get backfill range task, get reverse key failed", zap.String("category", "ddl"), zap.Error(err))
		} else {
			log.Info("get backfill range task, change end key", zap.String("category", "ddl"),
...
func (dc *ddlCtx) writePhysicalTableRecord(sessPool *sess.Pool, t table.PhysicalTable, bfWorkerType backfillerType, reorgInfo *reorgInfo) error {
	...
	for {
		...
		log.Info("start backfill workers to reorg record", zap.String("category", "ddl"),
...
func logSlowOperations(elapsed time.Duration, slowMsg string, threshold uint32) {
	...
	if elapsed >= time.Duration(threshold)*time.Millisecond {
		log.Info("slow operations", zap.String("category", "ddl"), zap.Duration("takeTimes", elapsed), zap.String("msg", slowMsg))
	}
}
