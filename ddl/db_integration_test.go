...
func TestChangingTableCharset(t *testing.T) {
	...
	checkCharset := func(chs, coll string) {
		tbl := testkit.GetTableByName(t, tk.Session(), "test", "t")
...
func TestCaseInsensitiveCharsetAndCollate(t *testing.T) {
	...
	require.True(t, ok)
	tbl := testkit.GetTableByName(t, tk.Session(), "test_charset_collate", "t5")
	tblInfo := tbl.Meta().Clone()
	...
	tblInfo = testkit.GetTableByName(t, tk.Session(), "test_charset_collate", "t5").Meta()
	require.Equal(t, "utf8mb4", tblInfo.Charset)
	require.Equal(t, "utf8mb4", tblInfo.Columns[0].Charset)
...
func TestDropTemporaryTable(t *testing.T) {
	...
	sequenceTable := testkit.GetTableByName(t, tk.Session(), "test", "b_table_local_and_normal")
