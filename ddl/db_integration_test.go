...
func (s *testIntegrationSuite) assertErrorExec(c *C, sql string, expectedWarn *terror.Error) {
	_, err := s.tk.Exec(sql)
	c.Assert(err, NotNil)
	c.Assert(expectedWarn.Equal(err), IsTrue, Commentf("error:%v", err))
}
...
func (s *testIntegrationSuite) assertAlterErrorExec(c *C, sql string) {
	s.assertErrorExec(c, sql, ddl.ErrAlterOperationNotSupported)
}
