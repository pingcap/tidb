...
func (s *testIntegrationSuite6) TestCreateTableTooLarge(c *C) {
	...
	originLimit := config.GetGlobalConfig().TableColumnLimit
	atomic.StoreUint32(&config.GetGlobalConfig().TableColumnLimit, uint32(cnt*4))
	_, err := tk.Exec(sql)
	c.Assert(kv.ErrEntryTooLarge.Equal(err), IsTrue, Commentf("err:%v", err))
	atomic.StoreUint32(&config.GetGlobalConfig().TableColumnLimit, originLimit)
}
...
func (s *testIntegrationSuite6) TestAddColumnTooMany(c *C) {
	tk := testkit.NewTestKit(c, s.store)
	tk.MustExec("use test")
	count := int(atomic.LoadUint32(&config.GetGlobalConfig().TableColumnLimit) - 1)
	var cols []string
	for i := 0; i < count; i++ {
