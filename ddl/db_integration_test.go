...
func TestCaseInsensitiveCharsetAndCollate(t *testing.T) {
	...
	require.True(t, ok)
	tbl := testutil.GetTableByName(t, tk, "test_charset_collate", "t5")
	tblInfo := tbl.Meta().Clone()
	...
	tblInfo = testutil.GetTableByName(t, tk, "test_charset_collate", "t5").Meta()
	require.Equal(t, "utf8mb4", tblInfo.Charset)
	require.Equal(t, "utf8mb4", tblInfo.Columns[0].Charset)
...
func TestDropTemporaryTable(t *testing.T) {
	...
	sequenceTable := testutil.GetTableByName(t, tk, "test", "b_table_local_and_normal")
