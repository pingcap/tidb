...
func TestColumnTypeChangeStateBetweenInteger(t *testing.T) {
	...
	require.NotNil(t, testutil.GetModifyColumn(t, tk, "test", "t", "c2", false))
	...
	hook.OnJobRunBeforeExported = func(job *model.Job) {
		...
		case model.StateNone:
			tbl = testutil.GetTableByName(t, internalTK, "test", "t")
			if tbl == nil {
		...
		case model.StateDeleteOnly, model.StateWriteOnly, model.StateWriteReorganization:
			...
			if tbl == nil {
				...
				checkErr = errors.New("len(cols) is not right")
			} else if testutil.GetModifyColumn(t, internalTK, "test", "t", "c2", true).Flag&mysql.PreventNullInsertFlag == uint(0) {
				checkErr = errors.New("old col's flag is not right")
			} else if testutil.GetModifyColumn(t, internalTK, "test", "t", "_Col$_c2_0", true) == nil {
	...
	require.NotNil(t, tbl)
	require.Equal(t, 2, len(tbl.Cols()))
	col := testutil.GetModifyColumn(t, tk, "test", "t", "c2", false)
...
func TestColumnTypeChangeFromIntegerToOthers(t *testing.T) {
	...
	modifiedColumn := testutil.GetModifyColumn(t, tk, "test", "t", "a", false)
	...
	modifiedColumn = testutil.GetModifyColumn(t, tk, "test", "t", "b", false)
	require.NotNil(t, modifiedColumn)
	...
	modifiedColumn = testutil.GetModifyColumn(t, tk, "test", "t", "c", false)
	require.NotNil(t, modifiedColumn)
	require.Equal(t, mysql.TypeString, modifiedColumn.Tp)
	...
	modifiedColumn = testutil.GetModifyColumn(t, tk, "test", "t", "d", false)
	require.NotNil(t, modifiedColumn)
	require.Equal(t, mysql.TypeVarchar, modifiedColumn.Tp)
	tk.MustQuery("select d from t").Check(testkit.Rows("1111"))

	tk.MustExec("alter table t modify e blob(10)")
	modifiedColumn = testutil.GetModifyColumn(t, tk, "test", "t", "e", false)
	require.NotNil(t, modifiedColumn)
	...
	modifiedColumn = testutil.GetModifyColumn(t, tk, "test", "t", "f", false)
	require.NotNil(t, modifiedColumn)
	require.Equal(t, mysql.TypeTinyBlob, modifiedColumn.Tp)
...
func TestChangingAttributeOfColumnWithFK(t *testing.T) {
	...
	tbl := testutil.GetTableByName(t, tk, "test", "orders")
...
func TestCancelCTCInReorgStateWillCauseGoroutineLeak(t *testing.T) {
	...
	tbl := testutil.GetTableByName(t, tk, "test", "ctc_goroutine_leak")
