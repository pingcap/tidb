...
func (s *testColumnSuite) checkAddColumn(state model.SchemaState, d *ddl, tblInfo *model.TableInfo, handle int64, newCol *table.Column, oldRow []types.Datum, columnValue interface{}) error {
	...
	case model.StateNone:
		err = s.checkNoneColumn(ctx, d, tblInfo, handle, newCol, columnValue)
	case model.StateDeleteOnly:
		err = s.checkDeleteOnlyColumn(ctx, d, tblInfo, handle, newCol, oldRow, columnValue)
	case model.StateWriteOnly:
		err = s.checkWriteOnlyColumn(ctx, d, tblInfo, handle, newCol, oldRow, columnValue)
	case model.StateWriteReorganization, model.StateDeleteReorganization:
		err = s.checkReorganizationColumn(ctx, d, tblInfo, handle, newCol, oldRow, columnValue)
	case model.StatePublic:
		err = s.checkPublicColumn(ctx, d, tblInfo, handle, newCol, oldRow, columnValue)
	}
	return err
}
