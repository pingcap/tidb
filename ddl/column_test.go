...
func (s *testColumnSuite) TestColumn(c *C) {
	...
	num := 10
	for i := 0; i < num; i++ {
		_, err := t.AddRecord(ctx, types.MakeDatums(i, 10*i, 100*i), false, true)
		c.Assert(err, IsNil)
	}
	...
	h, err := t.AddRecord(ctx, types.MakeDatums(11, 12, 13, 14), false, true)
	c.Assert(err, IsNil)
	err = ctx.NewTxn(context.Background())
	c.Assert(err, IsNil)
...
func (s *testColumnSuite) checkDeleteOnlyColumn(ctx sessionctx.Context, d *ddl, tblInfo *model.TableInfo, handle int64, col *table.Column, row []types.Datum, columnValue interface{}) error {
	...
	newHandle, err := t.AddRecord(ctx, newRow, false, true)
	if err != nil {
		return errors.Trace(err)
	}
	err = ctx.NewTxn(context.Background())
...
func (s *testColumnSuite) checkPublicColumn(ctx sessionctx.Context, d *ddl, tblInfo *model.TableInfo, handle int64, newCol *table.Column, oldRow []types.Datum, columnValue interface{}) error {
	...
	handle, err = t.AddRecord(ctx, newRow, false, true)
	if err != nil {
		return errors.Trace(err)
	}
	err = ctx.NewTxn(context.Background())
...
func (s *testColumnSuite) TestAddColumn(c *C) {
	...
	handle, err := t.AddRecord(ctx, oldRow, false, true)
	c.Assert(err, IsNil)
...
func (s *testColumnSuite) TestDropColumn(c *C) {
	...
	_, err = t.AddRecord(ctx, append(row, types.NewDatum(defaultColValue)), false, true)
	c.Assert(err, IsNil)
