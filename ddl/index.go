...
func (d *ddl) backfillBatchTasks(startTime time.Time, startHandle int64, reorgInfo *reorgInfo, totalAddedCount *int64, workers []*addIndexWorker, batchTasks []*reorgIndexTask) error {
	...
	if err != nil {
		...
		metrics.BatchAddIdxHistogram.WithLabelValues("batch_add_idx_fail").Observe(elapsedTime)
	...
	metrics.BatchAddIdxHistogram.WithLabelValues("batch_add_idx_succ").Observe(elapsedTime)
