...
import (
	...
	"github.com/pingcap/tidb/v3/store/mockoracle"
...
func (s *testGCWorkerSuite) TestDoGCForOneRegion(c *C) {
	...
	gofail.Enable("github.com/pingcap/tidb/v3/store/tikv/tikvStoreSendReqResult", `return("timeout")`)
	regionErr, err = taskWorker.doGCForRegion(bo, 20, loc.Region)
	c.Assert(regionErr, IsNil)
	c.Assert(err, NotNil)
	gofail.Disable("github.com/pingcap/tidb/v3/store/tikv/tikvStoreSendReqResult")

	gofail.Enable("github.com/pingcap/tidb/v3/store/tikv/tikvStoreSendReqResult", `return("GCNotLeader")`)
	regionErr, err = taskWorker.doGCForRegion(bo, 20, loc.Region)
	c.Assert(regionErr.GetNotLeader(), NotNil)
	c.Assert(err, IsNil)
	gofail.Disable("github.com/pingcap/tidb/v3/store/tikv/tikvStoreSendReqResult")

	gofail.Enable("github.com/pingcap/tidb/v3/store/tikv/tikvStoreSendReqResult", `return("GCServerIsBusy")`)
	regionErr, err = taskWorker.doGCForRegion(bo, 20, loc.Region)
