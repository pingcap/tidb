	...
	if d.Task.Step != proto.StepInit {
		...
		for _, id := range instanceIDs {
			...
			subTasks = append(subTasks, proto.NewSubtask(
				...
				d.Task.Concurrency, proto.EmptyMeta, 0))
		}
	}
...
func (d *BaseDispatcher) onNextStage() (err error) {
	...
	return d.dispatchSubTask(nextStep, metas, serverNodes)
}
...
func (d *BaseDispatcher) dispatchSubTask(
	...
	for i, meta := range metas {
		...
		subTasks = append(subTasks, proto.NewSubtask(
			subtaskStep, d.Task.ID, d.Task.Type, instanceID, d.Task.Concurrency, meta, i+1))
