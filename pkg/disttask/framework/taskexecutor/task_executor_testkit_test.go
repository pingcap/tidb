...
func runOneTask(ctx context.Context, t *testing.T, mgr *storage.TaskManager, taskKey string, subtaskCnt int) {
	...
	for i := 0; i < subtaskCnt; i++ {
		testutil.CreateSubTask(t, mgr, taskID, proto.StepOne, "test", nil, proto.TaskTypeExample, 11)
	}
	task, err = mgr.GetTaskByID(ctx, taskID)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	for i := 0; i < subtaskCnt; i++ {
		testutil.CreateSubTask(t, mgr, taskID, proto.StepTwo, "test", nil, proto.TaskTypeExample, 11)
	}
	task, err = mgr.GetTaskByID(ctx, taskID)
	require.NoError(t, err)
	require.NoError(t, executor.RunStep(ctx, task, nil))
