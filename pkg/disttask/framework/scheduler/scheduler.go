...
func (s *BaseScheduler) onCancelling() error {
	task := s.GetTask()
	logutil.Logger(s.logCtx).Info("on cancelling state", zap.Stringer("state", task.State), zap.String("step", proto.Step2Str(task.Type, task.Step)))
...
func (s *BaseScheduler) onPausing() error {
	task := s.GetTask()
	logutil.Logger(s.logCtx).Info("on pausing state", zap.Stringer("state", task.State), zap.String("step", proto.Step2Str(task.Type, task.Step)))
...
func (s *BaseScheduler) onPaused() error {
	task := s.GetTask()
	logutil.Logger(s.logCtx).Info("on paused state", zap.Stringer("state", task.State), zap.String("step", proto.Step2Str(task.Type, task.Step)))
...
func (s *BaseScheduler) onResuming() error {
	task := s.GetTask()
	logutil.Logger(s.logCtx).Info("on resuming state", zap.Stringer("state", task.State), zap.String("step", proto.Step2Str(task.Type, task.Step)))
	cntByStates, err := s.taskMgr.GetSubtaskCntGroupByStates(s.ctx, task.ID, task.Step)
	if err != nil {
		logutil.Logger(s.logCtx).Warn("check task failed", zap.Error(err))
		return err
...
func (s *BaseScheduler) onReverting() error {
	task := s.GetTask()
	logutil.Logger(s.logCtx).Debug("on reverting state", zap.Stringer("state", task.State), zap.String("step", proto.Step2Str(task.Type, task.Step)))
	cntByStates, err := s.taskMgr.GetSubtaskCntGroupByStates(s.ctx, task.ID, task.Step)
	if err != nil {
		logutil.Logger(s.logCtx).Warn("check task failed", zap.Error(err))
		return err
...
func (s *BaseScheduler) onPending() error {
	task := s.GetTask()
	logutil.Logger(s.logCtx).Debug("on pending state", zap.Stringer("state", task.State), zap.String("step", proto.Step2Str(task.Type, task.Step)))
...
func (s *BaseScheduler) switch2NextStep() (err error) {
	...
	logutil.Logger(s.logCtx).Info("switch to next step",
...
func (s *BaseScheduler) GetPreviousSubtaskMetas(taskID int64, step proto.Step) ([][]byte, error) {
	...
	if err != nil {
		logutil.Logger(s.logCtx).Warn("get previous succeed subtask failed", zap.String("step", proto.Step2Str(s.GetTask().Type, step)))
		return nil, err
	}
