...
func TestFrameworkSubtaskFinishedCancel(t *testing.T) {
	...
	RegisterTaskMeta(t, ctrl, &m, &testDispatcherExt{})
	distContext := testkit.NewDistExecutionContext(t, 3)
	err := failpoint.Enable("github.com/pingcap/tidb/pkg/disttask/framework/scheduler/MockSubtaskFinishedCancel", "1*return(true)")
	require.NoError(t, err)
	defer func() {
...
func TestFrameworkRunSubtaskCancel(t *testing.T) {
	...
	RegisterTaskMeta(t, ctrl, &m, &testDispatcherExt{})
	distContext := testkit.NewDistExecutionContext(t, 3)
	err := failpoint.Enable("github.com/pingcap/tidb/pkg/disttask/framework/scheduler/MockRunSubtaskCancel", "1*return(true)")
	require.NoError(t, err)
	DispatchTaskAndCheckState("key1", t, &m, proto.TaskStateReverted)
