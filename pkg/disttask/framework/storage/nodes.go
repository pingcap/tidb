...
func (mgr *TaskManager) DeleteDeadNodes(ctx context.Context, nodes []string) error {
	...
	if err := injectfailpoint.DXFRandomError(0.01, errors.New("injected random error DeleteDeadNodes")); err != nil {
		return err
	}
...
func (mgr *TaskManager) GetAllNodes(ctx context.Context) ([]proto.ManagedNode, error) {
	...
	if err := injectfailpoint.DXFRandomError(0.01, errors.New("injected random error GetAllNodes")); err != nil {
		return nodes, err
	}
...
func (mgr *TaskManager) GetUsedSlotsOnNodes(ctx context.Context) (map[string]int, error) {
	if err := injectfailpoint.DXFRandomError(0.01, errors.New("injected random error GetUsedSlotsOnNodes")); err != nil {
		return nil, err
	}
