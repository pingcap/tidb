...
func TestCollectTiCIPreSplitImportKVMetaGroups(t *testing.T) {
	tblInfo := &model.TableInfo{
		...
		Indices: []*model.IndexInfo{
			{ID: 1, Name: pmodel.NewCIStr("normal_idx"), State: model.StatePublic},
			{ID: 2, Name: pmodel.NewCIStr("ft_idx"), State: model.StatePublic, FullTextInfo: &model.FullTextIndexInfo{}},
			{ID: 3, Name: pmodel.NewCIStr("hybrid_idx"), State: model.StatePublic, HybridInfo: &model.HybridIndexInfo{}},
			{ID: 4, Name: pmodel.NewCIStr("empty_ft_idx"), State: model.StatePublic, FullTextInfo: &model.FullTextIndexInfo{}},
		},
	}
	kvMetas := map[string]*external.SortedKVMeta{
		dataKVGroup:    sortedKVMetaForTiCIPreSplitTest("d-a", "d-z", 100, 10, [][2]string{{"data/1", "stat/1"}}),
...
func TestBuildTiCIPreSplitImportShardsRequestForImportInto(t *testing.T) {
	require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/pkg/disttask/importinto/mockBuildTiCIPreSplitReportGroupsForImportInto", `return(true)`))
	t.Cleanup(func() {
		require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/pkg/disttask/importinto/mockBuildTiCIPreSplitReportGroupsForImportInto"))
	})
...
func TestGenerateWriteIngestSpecsTiCIPreSplitBestEffort(t *testing.T) {
	...
	ctx := context.Background()
	sortDir := t.TempDir()
	sortStore, err := storage.NewLocalStorage(sortDir)
	require.NoError(t, err)
	...
	specs, err := generateWriteIngestSpecs(planner.PlanCtx{
		...
		ThreadCnt: 1,
		Store:     taskStore.(kv.StorageWithPD),
	}, &LogicalPlan{JobID: 909, Stmt: "IMPORT INTO test.tb FROM '/tmp/input.csv'", Plan: importer.Plan{
		Path:            filepath.Join(sortDir, "input.csv"),
		InImportInto:    true,
...
func TestGenerateWriteIngestSpecsTiCIPreSplitUsesMergedMeta(t *testing.T) {
	...
	_, err = generateWriteIngestSpecs(planner.PlanCtx{
		...
		ThreadCnt: 1,
		Store:     taskStore.(kv.StorageWithPD),
	}, &LogicalPlan{JobID: 910, Stmt: "IMPORT INTO test.tb FROM '/tmp/input.csv'", Plan: importer.Plan{
		Path:            filepath.Join(sortDir, "input.csv"),
		InImportInto:    true,
		Format:          importer.DataFormatCSV,
		DBName:          "test",
