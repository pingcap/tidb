...
func TestConflictResolutionStepExecutor(t *testing.T) {
	...
	stepExe := importinto.NewConflictResolutionStepExecutor(1, store, taskMeta, logger)
	execute.SetFrameworkInfo(stepExe, &proto.Task{TaskBase: proto.TaskBase{ID: 1}}, nil, nil, nil)
	...
	stMeta := importinto.ConflictResolutionStepMeta{
		Infos: importinto.KVGroupConflictInfos{
	...
	require.NoError(t, stepExe.RunSubtask(ctx, st))
	tk.MustQuery("select * from tc").Sort().Check(testkit.Rows("4 4 4", "5 5 5"))
}
