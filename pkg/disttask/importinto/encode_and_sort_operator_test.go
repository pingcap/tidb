...
func TestEncodeAndSortOperator(t *testing.T) {
	...
	op := newEncodeAndSortOperator(context.Background(), executorForParam, nil, 3, 1, nil)
	op.SetSource(source)
	...
	source = operator.NewSimpleDataChannel(make(chan *importStepMinimalTask))
	op = newEncodeAndSortOperator(context.Background(), executorForParam, nil, 2, 2, nil)
	op.SetSource(source)
