...
func TestPostProcessStepExecutor(t *testing.T) {
	...
	ticiTableInfo.Indices = append(ticiTableInfo.Indices, &metamodel.IndexInfo{
		ID:           101,
	...
	nonTiCITableInfo.Indices = append(nonTiCITableInfo.Indices, &metamodel.IndexInfo{
		ID:    102,
		Name:  model.NewCIStr("idx_normal"),
		State: metamodel.StatePublic,
	})
...
func TestImportStepExecutorCleanupAllLocalEnginesOnRetry(t *testing.T) {
	...
	dom, err := session.GetDomain(store)
	require.NoError(t, err)
	dbInfo, ok := dom.InfoSchema().SchemaByName(model.NewCIStr("test"))
	require.True(t, ok)
	...
	testfailpoint.Enable(t, "github.com/pingcap/tidb/pkg/disttask/importinto/errorWhenSortChunk", `return(true)`)
