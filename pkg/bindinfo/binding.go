...
func MatchSQLBindingForPlanCache(sctx sessionctx.Context, stmtNode ast.StmtNode, info *BindingMatchInfo) (bindingSQL string) {
	binding, matched, _ := matchSQLBindingForPlanCache(sctx, stmtNode, info)
	if matched {
...
func MatchSQLBinding(sctx sessionctx.Context, stmtNode ast.StmtNode) (binding *Binding, matched bool, scope string) {
	return matchSQLBinding(sctx, stmtNode)
}
...
func matchSQLBinding(sctx sessionctx.Context, stmtNode ast.StmtNode) (binding *Binding, matched bool, scope string) {
	...
	if intest.InTest {
		binding, matched, scope = matchSQLBindingInternal(sctx, sessionVars, stmtNode, nil)
	...
	return matchSQLBindingInternal(sctx, sessionVars, stmtNode, nil)
}

func matchSQLBindingForPlanCache(sctx sessionctx.Context, stmtNode ast.StmtNode, info *BindingMatchInfo) (binding *Binding, matched bool, scope string) {
	sessionVars := sctx.GetSessionVars()
	defer func(begin time.Time) {
		sessionVars.DurationOptimizer.BindingMatch = time.Since(begin)
	}(time.Now())
	...
	cache := getMatchSQLBindingCache(sessionVars, stmtNode)
	if intest.InTest {
		binding, matched, scope = matchSQLBindingInternal(sctx, sessionVars, stmtNode, info)
		assertMatchSQLBinding(cache, matched, binding, scope)
		return
	}
	if cache != nil {
	...
	return matchSQLBindingInternal(sctx, sessionVars, stmtNode, info)
}

func matchSQLBindingInternal(sctx sessionctx.Context, sessionVars *variable.SessionVars, stmtNode ast.StmtNode, info *BindingMatchInfo) (binding *Binding, matched bool, scope string) {
