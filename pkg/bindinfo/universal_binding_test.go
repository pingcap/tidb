			...
			for _, idx := range []string{"a", "b", "c", "d", "e"} {
				...
				for _, useDB := range []string{"test", "test1", "test2"} {
					...
					for _, testDB := range []string{"", "test.", "test1.", "test2."} {
						tk.MustExec(`set @@tidb_opt_enable_fuzzing_binding=1`) // enabled
						require.True(t, tk.MustUseIndex(fmt.Sprintf("select * from %vt", testDB), idx))
						tk.MustQuery(`select @@last_plan_from_binding`).Check(testkit.Rows("1"))
						require.True(t, tk.MustUseIndex(fmt.Sprintf("select * from %vt", testDB), idx))
						tk.MustQuery(`show warnings`).Check(testkit.Rows())    // no warning
						tk.MustExec(`set @@tidb_opt_enable_fuzzing_binding=0`) // disabled
...
func TestUniversalBindingPriority(t *testing.T) {
	...
	tk.MustExec(`set @@tidb_opt_enable_fuzzing_binding=0`)
	tk.MustExec(`create global binding using select /*+ use_index(t, b) */ * from t`)
	tk.MustUseIndex(`select * from t`, "b")
	tk.MustQuery(`select @@last_plan_from_binding`).Check(testkit.Rows("1"))
	tk.MustExec(`set @@tidb_opt_enable_fuzzing_binding=1`)
...
func TestUniversalBindingSwitch(t *testing.T) {
	...
	tk2.MustExec(`set @@tidb_opt_enable_fuzzing_binding=1`)
	tk2.MustUseIndex(`select * from test.t`, "b")
	tk2.MustQuery(`select @@last_plan_from_binding`).Check(testkit.Rows("1"))
	tk2.MustExec(`set @@tidb_opt_enable_fuzzing_binding=0`)
	tk2.MustQuery(`select * from test.t`).Check(testkit.Rows())
	tk2.MustQuery(`select @@last_plan_from_binding`).Check(testkit.Rows("0"))

	// the default value is off
	tk3 := testkit.NewTestKit(t, store)
	tk3.MustQuery(`select @@tidb_opt_enable_fuzzing_binding`).Check(testkit.Rows("0"))
	tk3.MustQuery(`show session variables like 'tidb_opt_enable_fuzzing_binding'`).Check(testkit.Rows("tidb_opt_enable_fuzzing_binding OFF"))
	tk3.MustQuery(`show global variables like 'tidb_opt_enable_fuzzing_binding'`).Check(testkit.Rows("tidb_opt_enable_fuzzing_binding OFF"))
}
...
func TestUniversalBindingSetVar(t *testing.T) {
	...
	tk.MustQuery(`select @@last_plan_from_binding`).Check(testkit.Rows("0"))
	tk.MustExec(`select /*+ set_var(tidb_opt_enable_fuzzing_binding=1) */ * from t`)
	tk.MustQuery(`select @@last_plan_from_binding`).Check(testkit.Rows("1"))
	tk.MustExec(`select /*+ set_var(tidb_opt_enable_fuzzing_binding=0) */ * from t`)
	tk.MustQuery(`select @@last_plan_from_binding`).Check(testkit.Rows("0"))
