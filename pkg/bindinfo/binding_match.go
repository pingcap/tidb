...
// MatchBinding returns the matched binding for this StmtNode.
func MatchBinding(sctx sessionctx.Context, stmt ast.StmtNode) (bindRecord *BindRecord, scope string, matched bool) {
	useBinding := sctx.GetSessionVars().UsePlanBaselines
	if !useBinding || stmt == nil {
		return nil, "", false
	}
	...
	stmtNode, normalizedSQL, sqlDigest, err := NormalizeStmtForBinding(stmt, currentDB, false)
	if err != nil || stmtNode == nil {
		return nil, "", false
	}
	var normalizedSQLUni, sqlDigestUni string
	if sctx.GetSessionVars().EnableUniversalBinding {
		_, normalizedSQLUni, sqlDigestUni, err = NormalizeStmtForBinding(stmt, currentDB, true)
		if err != nil {
			return nil, "", false
		}
	}
	...
	if bindRecord := sessionHandle.GetSessionBinding(sqlDigest, normalizedSQL, ""); bindRecord != nil && bindRecord.HasEnabledBinding() {
		return bindRecord, metrics.ScopeSession, true
	}
	if sctx.GetSessionVars().EnableUniversalBinding {
	...
	if bindRecord := globalHandle.GetGlobalBinding(sqlDigest, normalizedSQL, ""); bindRecord != nil && bindRecord.HasEnabledBinding() {
		return bindRecord, metrics.ScopeGlobal, true
	}
	if sctx.GetSessionVars().EnableUniversalBinding {
...
func normalizeStmt(stmtNode ast.StmtNode, specifiedDB string, flag int) (stmt ast.StmtNode, normalizedStmt, sqlDigest string, err error) {
	...
	case *ast.ExplainStmt:
		...
		case *ast.SetOprStmt:
			...
			if idx == -1 {
				return x.Stmt, normalizeExplainSQL, bindingSQLDigest(normalizeExplainSQL), nil
			}
			normalizeSQL := normalizeExplainSQL[idx:]
			return x.Stmt, normalizeSQL, bindingSQLDigest(normalizeSQL), nil
		}
...
func bindingSQLDigest(normalizedSQL string) string {
	_, d := parser.NormalizeDigestForBinding(normalizedSQL)
	return d.String()
}
