...
type state struct {
	...
	indexHint *indexHint    // index hint for single-table queries
...
func generatePlanWithSCtx(sctx sessionctx.Context, defaultSchema, sql, charset, collation string) (plans []*genedPlan, err error) {
	...
	possibleIndexHints := extractSelectIndexHints(sctx, stmt, tableNames)
					...
					Tables: []ast.HintTable{
						{
							...
							TableName: ast.NewCIStr(state.indexHint.table.name),
						},
					},
...
func (e *predicateColumnExtractor) Enter(in ast.Node) (node ast.Node, skipChildren bool) {
	...
	case *ast.ColumnNameExpr:
		...
		if !e.allowAnyTable && n.Name.Table.L != "" && n.Name.Table.L != e.table.name {
			return in, false
		}
		if n.Name.Schema.L != "" && n.Name.Schema.L != e.table.schema {
			return in, false
		}
...
func extractSelectIndexHints(sctx sessionctx.Context, node ast.StmtNode, tableNames []*tableName) []*indexHint {
	...
	hints := make([]*indexHint, 0, len(tblInfo.Indices))
