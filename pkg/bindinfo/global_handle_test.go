...
func TestSetVarFixControlWithBinding(t *testing.T) {
	...
	tk.MustQuery(`explain select * from t where c = 10 and (a = 'xx' or (a = 'kk' and b = 1))`).Check(
		testkit.Rows(
			...
			`├─IndexRangeScan_5(Build) 0.10 cop[tikv] table:t, index:idx_ab(a, b) range:["kk" 1,"kk" 1], keep order:false, stats:pseudo`,
			`└─Selection_7(Probe) 0.00 cop[tikv]  eq(test.t.c, 10)`,
			`  └─TableRowIDScan_6 0.10 cop[tikv] table:t keep order:false, stats:pseudo`))
