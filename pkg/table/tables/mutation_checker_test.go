...
func TestCheckRowInsertionConsistency(t *testing.T) {
	...
	mockValue233, err := tablecodec.EncodeRow(
		sessVars.StmtCtx.TimeZone(), []types.Datum{types.NewIntDatum(233)}, []int64{101}, nil, nil, &rd, nil)
	require.Nil(t, err)
		...
		for _, lc := range locations {
			for _, columnInfos := range columnInfoSets {
				...
				for i, indexInfo := range indexInfos {
					...
					rowValue, err := tablecodec.EncodeRow(sessVars.StmtCtx.TimeZone(), rowToInsert, []int64{1, 2}, nil, nil, &rd, nil)
					require.Nil(t, err)
