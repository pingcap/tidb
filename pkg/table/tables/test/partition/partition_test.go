...
func checkDMLInAllStates(t *testing.T, tk, tk2 *testkit.TestKit, schemaName, alterStr string,
	...
	res = tk.MustQuery(`select * from t order by b`)
	res2 = tk.MustQuery(`select * from t2 order by b`)
	require.Equal(t, res.Rows(), res2.Rows())
	tk.MustQuery(`select count(*) from (select a from t except select a from t2) a`).Check(testkit.Rows("0"))
	//tk.MustExec("select * from t into outfile 't.out'")
	//tk.MustExec("select * from t2 into outfile 't2.out'")
