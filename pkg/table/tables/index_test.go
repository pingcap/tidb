...
func TestMeetPartialCondition(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			tableDefinition: "create table t (a int, b int, c int as (a+b), primary key (a), key testidx(c) where c > 2)",
			row:             []any{1, 2, 3},
			meet:            true,
		},
		{
			tableDefinition: "create table t (a int, b int, c int as (a+b), primary key (a), key testidx(c) where c > 3)",
			row:             []any{1, 2, 3},
			meet:            false,
		},
		// tidb rowid case
...
func TestPartialIndexDMLDuringDDL(t *testing.T) {
	...
	testCases := []testCase{
		...
		// This test will fail even without partial index. Ref https://github.com/pingcap/tidb/issues/63258
		// {
