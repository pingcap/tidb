...
func CheckDataConsistency(
	txn kv.Transaction, ctx table.MutateContext, t *TableCommon,
	...
	if err := checkIndexKeys(
		ctx, t, rowToInsert, rowToRemove, indexMutations, columnMaps.IndexIDToInfo, columnMaps.IndexIDToRowColInfos,
	); err != nil {
		return errors.Trace(err)
	}
	return nil
...
func checkIndexKeys(
	ctx table.MutateContext, t *TableCommon, rowToInsert, rowToRemove []types.Datum,
	...
	for _, m := range indexMutations {
		...
		for i, v := range decodedIndexValues {
			...
			datum, err := tablecodec.DecodeColumnValue(v, fieldType, location(ctx))
			if err != nil {
				return errors.Trace(err)
			}
		...
		if len(value) == 0 || isTmpIdxValAndDeleted {
			err = compareIndexData(typeCtx(ctx), t.Columns, indexData, rowToRemove, indexInfo, t.Meta())
		} else {
			err = compareIndexData(typeCtx(ctx), t.Columns, indexData, rowToInsert, indexInfo, t.Meta())
		}
		if err != nil {
			return errors.Trace(err)
		}
