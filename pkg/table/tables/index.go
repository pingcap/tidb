...
func NewIndex(physicalID int64, tblInfo *model.TableInfo, indexInfo *model.IndexInfo) (table.Index, error) {
	...
	if len(conditionString) > 0 {
		...
		cols, err := ExtractColumnsFromCondition(ectx, indexInfo, tblInfo, true)
		if err != nil {
			return nil, err
		}
		cols = DedupIndexColumns(cols)
		// Get the columns in the partial index condition expression.
		for _, col := range cols {
			index.conditionAffectColumn = append(index.conditionAffectColumn, tblInfo.Columns[col.Offset])
		}
	}
	return index, nil
}
