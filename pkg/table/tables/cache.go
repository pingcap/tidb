...
func (c *cachedTable) RemoveRecordWithGivenInfo(ctx table.MutateContext, h kv.Handle, r []types.Datum, indexPosInRow map[int64][]int, refColOfNonPubColForNonPubIndex int) error {
	txnCtxAddCachedTable(ctx, c.Meta().ID, c)
	return c.TableCommon.RemoveRecordWithGivenInfo(ctx, h, r, indexPosInRow, refColOfNonPubColForNonPubIndex)
}
