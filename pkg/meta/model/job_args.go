...
// Type aliases for compatibility with pkg/meta/model package structure
// These types are defined in pkg/parser/model in the target branch
type (
	TableInfo     = pmodel.TableInfo
	DBInfo        = pmodel.DBInfo
	Job           = pmodel.Job
	PolicyRefInfo = pmodel.PolicyRefInfo
	PartitionInfo = pmodel.PartitionInfo
	ColumnInfo    = pmodel.ColumnInfo
	ActionType = pmodel.ActionType
	JobState   = pmodel.JobState
)

// Job version constants
// In the target branch, Job.Version is int64, not a separate type
const (
	JobVersion1 int64 = 0
	JobVersion2 int64 = 1

	ActionCreateTable        = pmodel.ActionCreateTable
	ActionDropTable          = pmodel.ActionDropTable
	ActionCreateView         = pmodel.ActionCreateView
	ActionCreateSequence     = pmodel.ActionCreateSequence
	ActionAddTablePartition  = pmodel.ActionAddTablePartition
	ActionDropColumn         = pmodel.ActionDropColumn
	ActionModifySchemaCharsetAndCollate = pmodel.ActionModifySchemaCharsetAndCollate

	JobStateRollingback  = pmodel.JobStateRollingback
	JobStateRollbackDone = pmodel.JobStateRollbackDone
)
