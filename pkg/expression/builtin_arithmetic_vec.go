...
func (b *builtinArithmeticMultiplyRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if math.IsInf(x[i], 0) {
			...
			return types.ErrOverflow.GenWithStackByArgs("DOUBLE", fmt.Sprintf("(%s * %s)", b.args[0].StringWithCtx(false), b.args[1].StringWithCtx(false)))
		}
	}
	return nil
}
...
func (b *builtinArithmeticDivideDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if err == types.ErrDivByZero {
			...
			return types.ErrOverflow.GenWithStackByArgs("DECIMAL", fmt.Sprintf("(%s / %s)", b.args[0].StringWithCtx(false), b.args[1].StringWithCtx(false)))
		} else {
			return err
		}
		x[i] = to
	...
	for i := 0; i < n; i++ {
		...
		if err = types.DecimalSub(&x[i], &y[i], &to); err != nil {
			if err == types.ErrOverflow {
				err = types.ErrOverflow.GenWithStackByArgs("DECIMAL", fmt.Sprintf("(%s - %s)", b.args[0].StringWithCtx(false), b.args[1].StringWithCtx(false)))
			}
			return err
		}
		x[i] = to
...
func (b *builtinArithmeticMinusIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < len(lhi64s); i++ {
		...
		if overflow {
			...
			return types.ErrOverflow.GenWithStackByArgs(errType, fmt.Sprintf("(%s - %s)", b.args[0].StringWithCtx(false), b.args[1].StringWithCtx(false)))
		}
...
func (b *builtinArithmeticMultiplyIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if (x[i] != 0 && tmp/x[i] != y[i]) || (tmp == math.MinInt64 && x[i] == -1) {
			...
			result.SetNull(i, true)
			return types.ErrOverflow.GenWithStackByArgs("BIGINT", fmt.Sprintf("(%s * %s)", b.args[0].StringWithCtx(false), b.args[1].StringWithCtx(false)))
		}
...
func (b *builtinArithmeticPlusIntSig) plusUU(result *chunk.Column, lhi64s, rhi64s, resulti64s []int64) error {
	for i := 0; i < len(lhi64s); i++ {
		...
		if uint64(lh) > math.MaxUint64-uint64(rh) {
			...
			return types.ErrOverflow.GenWithStackByArgs("BIGINT UNSIGNED", fmt.Sprintf("(%s + %s)", b.args[0].StringWithCtx(false), b.args[1].StringWithCtx(false)))
		}
