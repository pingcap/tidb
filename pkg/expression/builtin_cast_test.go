...
func TestCastFunctions(t *testing.T) {
	...
	res, err := f.Eval(ctx.GetEvalCtx(), chunk.Row{})
	...
	warnings := ctx.GetEvalCtx().CopyWarnings(nil)
	...
	previousWarnings := ctx.GetEvalCtx().WarningCount()
	f = BuildCastFunction(ctx, &Constant{Value: types.NewDatum("-1"), RetType: types.NewFieldType(mysql.TypeString)}, tp1)
	res, err = f.Eval(ctx.GetEvalCtx(), chunk.Row{})
	require.NoError(t, err)
	require.True(t, res.GetInt64() == -1)
	require.True(t, ctx.GetEvalCtx().WarningCount() == previousWarnings)
...
func TestCastFuncSig(t *testing.T) {
	...
	for i, c := range castToDecCases {
		...
		res, err := evalBuiltinFunc(sig, ctx.GetEvalCtx(), c.row.ToRow())
		require.NoError(t, err)
	...
	sRes, err := evalBuiltinFunc(sig, ctx.GetEvalCtx(), row.ToRow())
	require.NoError(t, err)
	...
	iRes, err := evalBuiltinFunc(sig, ctx.GetEvalCtx(), row.ToRow())
	require.NoError(t, err)
...
func TestCastJSONAsDecimalSig(t *testing.T) {
	...
	for _, tt := range tests {
		...
		res, isNull, err := sig.evalDecimal(ctx.GetEvalCtx(), row.ToRow())
...
func TestWrapWithCastAsTypesClasses(t *testing.T) {
	...
	strRes, isNull, err = strExpr.EvalString(ctx.GetEvalCtx(), chunk.MutRowFromDatums([]types.Datum{types.NewUintDatum(1234)}).ToRow())
	require.NoError(t, err)
	require.Equal(t, false, isNull)
...
func TestWrapWithCastAsTime(t *testing.T) {
	...
	for d, c := range cases {
		...
		res, isNull, err := expr.EvalTime(ctx.GetEvalCtx(), chunk.Row{})
		require.NoError(t, err)
		require.Equal(t, false, isNull)
...
func TestWrapWithCastAsDuration(t *testing.T) {
	...
	for _, c := range cases {
		...
		res, isNull, err := expr.EvalDuration(ctx.GetEvalCtx(), chunk.Row{})
		require.NoError(t, err)
		require.Equal(t, false, isNull)
...
func TestWrapWithCastAsString(t *testing.T) {
	...
	for _, c := range cases {
		...
		res, _, _ := expr.EvalString(ctx.GetEvalCtx(), chunk.Row{})
		if c.warn {
			warns := ctx.GetEvalCtx().CopyWarnings(nil)
...
func TestCastIntAsIntVec(t *testing.T) {
	...
	for row := it.Begin(); row != it.End(); row = it.Next() {
		v, _, err := cast.evalInt(ctx.GetEvalCtx(), row)
		require.NoError(t, err)
...
func TestCastStringAsDecimalSigWithUnsignedFlagInUnion(t *testing.T) {
	...
	for _, c := range cases {
		res, isNull, err := cast.evalDecimal(ctx.GetEvalCtx(), c.row.ToRow())
		require.Equal(t, false, isNull)
		require.NoError(t, err)
...
func TestCastBinaryStringAsJSONSig(t *testing.T) {
	...
	for _, tt := range tests {
		...
		res, isNull, err := sig.evalJSON(ctx.GetEvalCtx(), row.ToRow())
		require.NoError(t, err)
...
func TestCastArrayFunc(t *testing.T) {
	...
	for _, tt := range tbl {
		...
		val, isNull, err := f.EvalJSON(ctx.GetEvalCtx(), chunk.Row{})
		if tt.success {
			require.NoError(t, err, tt.input)
			if tt.expected == nil {
