	...
	for i := 0; i < n; i++ {
		...
		if err != nil {
			if types.ErrOverflow.Equal(err) {
				err = evalVars(ctx).StmtCtx.HandleError(err)
			}
...
func (b *builtinCastDurationAsIntSig) vecEvalInt(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if isYear {
			i64s[i], err = duration.ConvertToYear(evalVars(ctx).StmtCtx.TypeCtx())
		} else {
			var dur types.Duration
			dur, err = duration.RoundFrac(types.DefaultFsp, evalVars(ctx).Location())
			if err != nil {
				return err
			}
...
func (b *builtinCastJSONAsTimeSig) vecEvalTime(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		case types.JSONTypeCodeDuration:
			...
			tm, err = tm.RoundFrac(stmtCtx.TypeCtx(), fsp)
			if err != nil {
				return err
			}
			times[i] = tm
...
func (b *builtinCastDurationAsTimeSig) vecEvalTime(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		duration.Duration = ds[i]
		duration.Fsp = fsp
		tm, err := duration.ConvertToTimeWithTimestamp(evalVars(ctx).StmtCtx.TypeCtx(), b.tp.GetType(), ts)
		if err != nil {
			if err = handleInvalidTimeError(ctx, err); err != nil {
				return err
			}
...
func (b *builtinCastIntAsStringSig) vecEvalString(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		str, err = types.ProduceStrWithSpecifiedTp(str, b.tp, evalVars(ctx).StmtCtx.TypeCtx(), false)
		if err != nil {
			return err
		}
		var d bool
		...
		if !b.inUnion || bufreal[i] >= 0 {
			if err = resdecimal[i].FromFloat64(bufreal[i]); err != nil {
				if types.ErrOverflow.Equal(err) {
					...
					err = evalVars(ctx).StmtCtx.HandleErrorWithAlias(err, err, warnErr)
...
func (b *builtinCastStringAsIntSig) vecEvalInt(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	result.MergeNulls(buf)
	typeCtx := evalVars(ctx).StmtCtx.TypeCtx()
	i64s := result.Int64s()
...
func (b *builtinCastStringAsDurationSig) vecEvalDuration(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, isNull, err := types.ParseDuration(evalVars(ctx).StmtCtx.TypeCtx(), buf.GetString(i), b.tp.GetDecimal())
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = evalVars(ctx).StmtCtx.HandleError(err)
			}
...
func (b *builtinCastJSONAsStringSig) vecEvalString(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		s, err := types.ProduceStrWithSpecifiedTp(buf.GetJSON(i).String(), b.tp, evalVars(ctx).StmtCtx.TypeCtx(), false)
		if err != nil {
			return err
		}
...
func (b *builtinCastRealAsDurationSig) vecEvalDuration(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, _, err := types.ParseDuration(evalVars(ctx).StmtCtx.TypeCtx(), strconv.FormatFloat(f64s[i], 'f', -1, 64), b.tp.GetDecimal())
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = evalVars(ctx).StmtCtx.HandleError(err)
				if err != nil {
...
func (b *builtinCastTimeAsDurationSig) vecEvalDuration(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, t := range arg0s {
		...
		d, err = d.RoundFrac(b.tp.GetDecimal(), evalVars(ctx).Location())
		if err != nil {
			return err
		}
		ds[i] = d.Duration
...
func (b *builtinCastDurationAsDurationSig) vecEvalDuration(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, v := range res {
		...
		dur.Duration = v
		rd, err = dur.RoundFrac(b.tp.GetDecimal(), evalVars(ctx).Location())
		if err != nil {
			return err
		}
		res[i] = rd.Duration
...
func (b *builtinCastTimeAsIntSig) vecEvalInt(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		t, err := times[i].RoundFrac(sc.TypeCtx(), types.DefaultFsp)
		if err != nil {
			return err
		}
...
func (b *builtinCastTimeAsTimeSig) vecEvalTime(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		tm, err := res.RoundFrac(stmt.TypeCtx(), fsp)
		if err != nil {
			return err
		}
		times[i] = tm
...
func (b *builtinCastStringAsTimeSig) vecEvalTime(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if tm.IsZero() && evalVars(ctx).SQLMode.HasNoZeroDateMode() {
...
func (b *builtinCastDecimalAsDurationSig) vecEvalDuration(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, _, err := types.ParseDuration(evalVars(ctx).StmtCtx.TypeCtx(), string(args[i].ToString()), b.tp.GetDecimal())
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = evalVars(ctx).StmtCtx.HandleError(err)
				if err != nil {
