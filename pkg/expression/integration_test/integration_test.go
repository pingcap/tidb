...
func TestAutoEmbeddingVectorSearchRewrite(t *testing.T) {
	...
	checkExecFirstArgErr := func(sql string) {
		t.Helper()
	...
	tk.MustExec(`insert into t_dup set id = 2, text = '[1,2,3]', dist = vec_embed_l2_distance(vec, '[1,2,3]')`)
	tk.MustQuery(`select dist = vec_l2_distance(vec, ` + explicitEmbedding + `) from t_dup where id = 2`).Check(testkit.Rows("1"))
	...
	tk.MustQuery(`select dist = vec_l2_distance(vec, ` + explicitEmbedding + `) from t_dup where id = 1`).Check(testkit.Rows("1"))
	checkRewrite(
		`select vec_embed_l2_distance(vec, '[1,2,3]') from t join t_dup using(vec) order by 1`,
		`select vec_l2_distance(vec, `+explicitEmbedding+`) from t join t_dup using(vec) order by 1`,
	)
	checkRewrite(
		`select vec_embed_l2_distance(t_dup.vec, '[1,2,3]') from t join t_dup using(vec) order by 1`,
		`select vec_l2_distance(t_dup.vec, `+explicitEmbedding+`) from t join t_dup using(vec) order by 1`,
	)
	...
	checkExecFirstArgErr(`update t join normal_vec using(vec) set t.dist = vec_embed_l2_distance(t.vec, '[1,2,3]') where t.id = 1`)
	checkExecFirstArgErr(`delete t from t join normal_vec using(vec) where vec_embed_l2_distance(t.vec, '[1,2,3]') < 1`)
	...
	checkExecFirstArgErr(`update t natural join normal_vec_nat set t.dist = vec_embed_l2_distance(t.vec, '[1,2,3]') where t.id = 1`)
	tk.MustExec(`
	...
	tk.MustExec(`create view v_auto_embed as select vec from t`)
	...
	boundaryCases := []struct {
		...
		{
			name:     "distinct output is derived",
			boundary: "derived",
			sql:      `select vec_embed_l2_distance(vec, '[1,2,3]') from (select distinct vec from t) dt`,
			wantErr:  true,
		},
		{
			name:     "aggregation output is derived",
			boundary: "derived",
			sql:      `select vec from t group by vec having vec_embed_l2_distance(vec, '[1,2,3]') < 1`,
			wantErr:  true,
		},
		{
			name:     "group by derived table output is derived",
			boundary: "derived",
			sql:      `select vec_embed_l2_distance(vec, '[1,2,3]') from (select vec from t group by vec) dt`,
			wantErr:  true,
		},
		{
		...
		},
		{
			name:     "view is opaque",
			boundary: "opaque",
			sql:      `select vec_embed_l2_distance(vec, '[1,2,3]') from v_auto_embed`,
			wantErr:  true,
		},
		{
			name:     "shared CTE is opaque",
			boundary: "opaque",
			sql:      `with c as (select id, vec from t) select vec_embed_l2_distance(c1.vec, '[1,2,3]') from c c1 join c c2 on c1.id = c2.id`,
			wantErr:  true,
		},
		{
			name:     "lateral apply right side is opaque",
			boundary: "opaque",
			sql:      `select vec_embed_l2_distance(v, '[1,2,3]') from t, lateral (select t.vec as v) dt`,
			wantErr:  true,
		},
		{
			name:     "scalar subquery expression is opaque",
			boundary: "opaque",
			sql:      `select (select vec_embed_l2_distance(t.vec, '[1,2,3]')) from t`,
			wantErr:  true,
		},
		{
			name:     "scalar subquery column is opaque",
			boundary: "opaque",
			sql:      `select vec_embed_l2_distance((select u.vec from t u where u.id = t.id), '[1,2,3]') from t`,
			wantErr:  true,
		},
		{
			name:     "correlated semi subquery outer column is opaque",
			boundary: "opaque",
			sql:      `select id from t where exists (select 1 where vec_embed_l2_distance(t.vec, '[1,2,3]') < 1)`,
			wantErr:  true,
		},
		{
	...
	func() {
		restoreOracleHook := plannercore.SetAutoEmbedOracleSkipOpaqueBoundaryInvalidationForTest(true)
		defer restoreOracleHook()
		err := tk.QueryToErr(`select vec_embed_l2_distance(vec, '[1,2,3]') from v_auto_embed`)
		require.ErrorContains(t, err, "auto-embed lineage oracle rejected")
		require.ErrorContains(t, err, "not traceable through transparent logical plan lineage")
	}()

	checkExecFirstArgErr(`insert into t_dup(id, text, dist) select 1, text, 0 from t where id = 1 on duplicate key update dist = vec_embed_l2_distance(values(dist), '[1,2,3]')`)
	...
	checkExecFirstArgErr(`insert into target_norm(id, vec, dist) select 1, src.vec, 0 from t as src where src.id = 1 on duplicate key update dist = vec_embed_l2_distance(values(vec), '[1,2,3]')`)
}
