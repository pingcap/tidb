...
func TestVectorizedWeekInvalidDates(t *testing.T) {
	for _, name := range []string{ast.Weekday, ast.WeekOfYear, ast.Week, ast.YearWeek} {
		for _, withMode := range []bool{false, true} {
			if withMode && name != ast.Week && name != ast.YearWeek {
				continue
			}
			for _, strict := range []bool{false, true} {
				...
				args := []Expression{&Column{RetType: dateType, Index: 0}}
				if withMode {
					args = append(args, &Constant{Value: types.NewIntDatum(3), RetType: types.NewFieldType(mysql.TypeLonglong)})
				}
				f, err := funcs[name].getFunction(ctx, args)
				require.NoError(t, err)
				require.True(t, f.vectorized() && f.isChildrenVectorized())
				for _, date := range []types.Time{
					types.ZeroDate,
					...
					input := chunk.NewChunkWithCapacity([]*types.FieldType{dateType}, 2)
					input.AppendTime(0, date)
					input.AppendNull(0)
					sc.SetWarnings(nil)
					want, isNull, scalarErr := f.evalInt(ctx, input.GetRow(0))
					warnings := sc.WarningCount()
					sc.SetWarnings(nil)
					result := chunk.NewColumn(types.NewFieldType(mysql.TypeLonglong), 2)
					vecErr := vecEvalType(ctx, f, types.ETInt, input, result)
					if scalarErr != nil {
						require.EqualError(t, vecErr, scalarErr.Error(), "%s mode=%v strict=%v date=%s", name, withMode, strict, date)
						continue
					}
					require.NoError(t, vecErr)
					require.Equal(t, isNull, result.IsNull(0), "%s mode=%v strict=%v date=%s", name, withMode, strict, date)
					require.True(t, result.IsNull(1))
					require.Equal(t, warnings, sc.WarningCount())
					if !isNull {
						require.Equal(t, want, result.Int64s()[0])
					}
				}
			}
		}
