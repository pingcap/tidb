...
func (s *propConstSolver) setConds2ConstFalse() {
	if MaybeOverOptimized4PlanCache(s.ctx, s.conditions) {
		s.ctx.GetSessionVars().StmtCtx.SetSkipPlanCache(errors.NewNoStackError("some parameters may be overwritten when constant propagation"))
	}
...
func (s *basePropConstSolver) dealWithPossibleHybridType(col *Column, con *Constant) (*Constant, bool) {
	...
	if col.GetType().GetType() == mysql.TypeEnum {
		...
		if MaybeOverOptimized4PlanCache(s.ctx, []Expression{con}) {
			s.ctx.GetSessionVars().StmtCtx.SetSkipPlanCache(errors.NewNoStackError("Skip plan cache since mutable constant is restored and propagated"))
		}
		switch d.Kind() {
