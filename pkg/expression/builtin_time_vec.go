...
func (b *builtinDateSig) vecEvalTime(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < len(times); i++ {
		...
		if times[i].IsZero() && evalVars(ctx).SQLMode.HasNoZeroDateMode() {
...
func (b *builtinTidbParseTsoSig) vecEvalTime(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if err := r.ConvertTimeZone(time.Local, evalVars(ctx).Location()); err != nil {
			return err
		}
		times[i] = r
	}
...
func (b *builtinStrToDateDatetimeSig) vecEvalTime(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	times := result.Times()
	sc := evalVars(ctx).StmtCtx
	hasNoZeroDateMode := evalVars(ctx).SQLMode.HasNoZeroDateMode()
	fsp := b.tp.GetDecimal()
...
func (b *builtinSecToTimeSig) vecEvalDuration(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		duration, _, err := types.ParseDuration(evalVars(ctx).StmtCtx.TypeCtx(), fmt.Sprintf("%s%02d:%02d:%s", negative, hour, minute, strconv.FormatFloat(secondDemical, 'f', -1, 64)), b.tp.GetDecimal())
		if err != nil {
			return err
		}
...
func (b *builtinUTCTimeWithoutArgSig) vecEvalDuration(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	res, _, err := types.ParseDuration(evalVars(ctx).StmtCtx.TypeCtx(), nowTs.UTC().Format(types.TimeFormat), types.DefaultFsp)
	if err != nil {
		return err
	}
	result.ResizeGoDuration(n, false)
...
func (b *builtinMakeTimeSig) vecEvalDuration(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, err := b.makeTime(evalVars(ctx).StmtCtx.TypeCtx(), hours[i], minutes[i], seconds[i], hourUnsignedFlag)
		if err != nil {
			return err
		}
		durs[i] = dur.Duration
...
func (b *builtinMonthNameSig) vecEvalString(ctx EvalContext, input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		mon := ds[i].Month()
		if (ds[i].IsZero() && evalVars(ctx).SQLMode.HasNoZeroDateMode()) || mon < 0 || mon > len(types.MonthNames) {
			if err := handleInvalidTimeError(ctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, ds[i].String())); err != nil {
				return err
			}
			result.AppendNull()
