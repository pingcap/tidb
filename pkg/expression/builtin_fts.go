...
// scalarFuncSigFTSMatchWord is the FTS_MATCH_WORD signature id in tipb.
const scalarFuncSigFTSMatchWord = tipb.ScalarFuncSig(5201)

type ftsMatchWordFunctionClass struct {
	baseFunctionClass
	expropt.SessionVarsPropReader // added to set `FTSFunctionIsUsed`
}
...
func (c *ftsMatchWordFunctionClass) getFunction(ctx BuildContext, args []Expression) (builtinFunc, error) {
	...
	// only for FTS.
	// ctx.GetSessionVars().StmtCtx.FTSFunctionIsUsed = true
	sctx, err := c.GetSessionVars(ctx.GetEvalCtx())
	if err != nil {
		return nil, err
	}
	sctx.StmtCtx.FTSFunctionIsUsed = true

	sig := &builtinFtsMatchWordSig{bf}
	sig.setPbCode(scalarFuncSigFTSMatchWord)
	return sig, nil
}
