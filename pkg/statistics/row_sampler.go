...
func (s *RowSampleBuilder) Collect() (RowSampleCollector, error) {
	...
	if math.IsNaN(ndvSampleRate) || math.IsInf(ndvSampleRate, 0) || ndvSampleRate < 0 || ndvSampleRate > NDVSampleSkipRate {
		return nil, fmt.Errorf("invalid NDVSampleRate %v, must be in [0, 1]", ndvSampleRate)
	}
