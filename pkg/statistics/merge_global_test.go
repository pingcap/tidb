...
type mergeCase struct {
	...
	extra func(t *testing.T, hist *statistics.Histogram, topN *statistics.TopN)

	// Optional assertion on what the optimizer would estimate from the
	// merged stats. globalNDV is the NDV the merge's caller fills in
	// from the merged FMSketch, which the uniform fallback divides by,
	// so a case asserting estimates has to declare it.
	globalNDV int64
...
// estimator runs the merged stats through the cardinality entry points
// the optimizer uses. Bucket geometry is only a means to an end: what
// matters is the row count a predicate gets, and that is decided by
// TopN lookup, Bucket.Repeat and the uniform fallback together.
...
// equal is the estimate for `col = v`.
func (e estimator) equal(v int64) float64 { return e.rangeCount(v, v) }

// between is the estimate for `col between lo and hi`.
func (e estimator) between(lo, hi int64) float64 { return e.rangeCount(lo, hi) }

func (e estimator) rangeCount(lo, hi int64) float64 {
	ran := &ranger.Range{
		LowVal:    []types.Datum{types.NewIntDatum(lo)},
		HighVal:   []types.Datum{types.NewIntDatum(hi)},
...
// cardinality package expects.
func newEstimator(t *testing.T, tc mergeCase, hist *statistics.Histogram, topN *statistics.TopN) estimator {
	const id = 1
	h := *hist
	h.NDV = tc.globalNDV
	// Loading stats from storage precomputes the scalar bounds the
	// range estimators interpolate over (read.go does this right after
	// building the histogram); the merge output has not been through
	// that yet.
	h.PreCalculateScalar()
	rows := int64(0)
	if h.Len() > 0 {
		rows = h.Buckets[h.Len()-1].Count
	}
	if topN != nil {
	...
	if tc.isIndex {
		coll.SetIdx(id, &statistics.Index{
			Histogram:         h,
			TopN:              topN,
			Info:              &model.IndexInfo{ID: id, Columns: []*model.IndexColumn{{Offset: 0}}},
			StatsLoadedStatus: statistics.NewStatsFullLoadStatus(),
			StatsVer:          statistics.Version2,
		})
	} else {
		coll.SetCol(id, &statistics.Column{
			Histogram:         h,
			TopN:              topN,
			Info:              &model.ColumnInfo{ID: id, FieldType: *newCaseFieldType(tc.colTp, false)},
			StatsLoadedStatus: statistics.NewStatsFullLoadStatus(),
			StatsVer:          statistics.Version2,
		})
	}
	return estimator{t: t, sctx: mock.NewContext(), coll: coll, id: id, isIndex: tc.isIndex}
}
...
func buildInputs(t *testing.T, sc *stmtctx.StatementContext, tc mergeCase) ([]*statistics.TopN, []*statistics.Histogram) {
	tp := newCaseFieldType(tc.colTp, tc.isIndex)
	topNs := make([]*statistics.TopN, len(tc.parts))
	hists := make([]*statistics.Histogram, len(tc.parts))
	for p, ps := range tc.parts {
		tn := statistics.NewTopN(len(ps.topN))
		for _, e := range ps.topN {
			tn.AppendTopN(encodeDatum(t, sc, e.val), e.count)
		}
		topNs[p] = tn

		h := statistics.NewHistogram(1, 0, 0, 0, tp, chunk.InitialCapacity, 0)
		var cum int64
...
func runMergeCase(t *testing.T, tc mergeCase) {
	...
	if tc.estimates != nil {
		tc.estimates(t, newEstimator(t, tc, gHist, gTopN))
	}
}
...
func assertInvariants(t *testing.T, sc *stmtctx.StatementContext, tc mergeCase, hist *statistics.Histogram, topN *statistics.TopN) {
...
func assertTopN(t *testing.T, sc *stmtctx.StatementContext, want []topnSpec, got *statistics.TopN) {
...
func assertBuckets(t *testing.T, want []bucketSpec, hist *statistics.Histogram, isIndex bool) {
...
func bucketMasses(h *statistics.Histogram) []int64 {
