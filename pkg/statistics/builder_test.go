...
func BenchmarkBuildHistAndTopN(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = BuildHistAndTopN(ctx, 256, 500, 0, collector, filedType, true, memoryTracker, false, false)
	}
}
...
func BenchmarkBuildHistAndTopNWithLowNDV(b *testing.B) {
	for _, enable := range []bool{true, false} {
		b.Run(fmt.Sprintf("%v", enable), func(b *testing.B) {
			ctx := mock.NewContext()
			const cnt = 1000_000
			sketch := NewFMSketch(cnt)
			data := make([]*SampleItem, 0, 8)
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				_, _, _ = BuildHistAndTopN(ctx, 256, 500, 0, collector, filedType, true, memoryTracker, true, true)
			}
		})
	}
}
