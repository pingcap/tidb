...
// specific singletons that win heap ties are arbitrary. When the heap
// fills to capacity (more distinct candidates than numTopN slots) the
// merge may drop those singletons -- but only for the default TopN
// size. BuildHistAndTopN (builder.go) disables pruning whenever numTopN
// differs from the active analyze default
// (tidb_analyze_default_num_topn), treating an explicit size as a value
// the user wants honored; the merge mirrors that so a partitioned
// table's global TopN matches an identical non-partitioned table.
//
// Four checks, all of which must preserve total row count across the
...
//  4. The gate follows tidb_analyze_default_num_topn rather than the
...
func TestMergePartTopNAndHistToGlobalSingletonFilter(t *testing.T) {
	...
	rowsPreserved := func(t *testing.T, tc mergeCase, gTopN *TopN, gHist *Histogram) {
	...
	defaultTC := mergeCase{
		...
		numTopN:    DefaultTopNValue,
		expBuckets: 128,
		parts:      []partSpec{{hist: genSingletons(DefaultTopNValue + 50)}},
	}
	dTopNs, dHists := buildInputs(t, sc, defaultTC)
	dTopN, dHist, err := MergePartTopNAndHistToGlobal(
	...
	// 4. The gate follows tidb_analyze_default_num_topn, mirroring
	// BuildHistAndTopN: with the active default raised to 150,
	// numTopN=150 prunes singletons, while numTopN=DefaultTopNValue
	...
	sTopN, sHist, err := MergePartTopNAndHistToGlobal(
	...
	cTopN, cHist, err := MergePartTopNAndHistToGlobal(
	...
	require.Equalf(t, int(constTC.numTopN), len(cTopN.TopN),
		"numTopN=DefaultTopNValue with active default 150 should keep DefaultTopNValue singletons; got %d", len(cTopN.TopN))
