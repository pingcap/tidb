...
func mergeCases() []mergeCase {
	return []mergeCase{
		...
		// ----------------------------------------------------------------
		{
			...
			extra: func(t *testing.T, hist *statistics.Histogram, _ *statistics.TopN) {
		...
		{
			// A global bucket boundary landing exactly on a partition
			// bucket's upper must leave that bucket's Repeat on the left,
			// where the value is the bucket upper. Attributing it to the
			// right bucket (where the same value is the lower) buries the
			// point mass as interior rows: EqualRowCount matches the left
			// bucket's upper and returns its Repeat, so a value occurring
			// 20 times would be estimated at 0.
			name:       "boundary_repeat_stays_with_left_bucket",
			expBuckets: 2,
			parts: []partSpec{
				{hist: []bucketSpec{{lo: di(0), up: di(10), mass: 100, repeat: 20}}},
				{hist: []bucketSpec{{lo: di(10), up: di(20), mass: 100, repeat: 1}}},
			},
			wantBuckets: []bucketSpec{
				{lo: di(0), up: di(10), mass: 100},
				{lo: di(10), up: di(20), mass: 100},
			},
			extra: func(t *testing.T, hist *statistics.Histogram, _ *statistics.TopN) {
				require.Equalf(t, int64(20), hist.Buckets[0].Repeat,
					"Repeat must stay on the bucket whose upper is the boundary value; got %d",
					hist.Buckets[0].Repeat)
				cnt, matched := hist.EqualRowCount(nil, types.NewIntDatum(10), false)
				require.True(t, matched, "boundary value must match a bucket upper")
				require.Equalf(t, float64(20), cnt,
					"boundary value must be estimated from its Repeat; got %v", cnt)
			},
			globalNDV: 50,
			estimates: func(t *testing.T, e estimator) {
				require.Equalf(t, float64(20), e.equal(10),
					"the optimizer must see the boundary value's 20 rows")
				require.GreaterOrEqualf(t, e.between(0, 20), float64(20),
					"a range covering the boundary must include its rows")
			},
		},
		{
			// p0's wide bucket is cut where p1's begins, so the left
			// global bucket ends at 90. No partition bucket has 90 as
			// its upper, so nothing observed a point frequency for it
			// and the merge reports none: Repeat stays 0 rather than
			// being invented from the other partitions' averages,
			// which would scale with the partition count. The
			// estimator treats that zero as "unknown" and falls back
			// to the uniform average, see equalRowCount.
			name:       "cut_boundary_has_no_observed_repeat",
			expBuckets: 2,
			parts: []partSpec{
				{hist: []bucketSpec{{lo: di(1), up: di(100), mass: 100, repeat: 1}}},
				{hist: []bucketSpec{{lo: di(90), up: di(110), mass: 100, repeat: 1}}},
			},
			wantBuckets: []bucketSpec{
				{lo: di(1), up: di(90), mass: 90},
				{lo: di(90), up: di(110), mass: 110},
			},
			extra: func(t *testing.T, hist *statistics.Histogram, _ *statistics.TopN) {
				require.Equalf(t, int64(0), hist.Buckets[0].Repeat,
					"the merge must not invent a frequency for an unobserved boundary; got %d",
					hist.Buckets[0].Repeat)
			},
			// 200 rows over an NDV of 100, so the uniform fallback is 2.
			globalNDV: 100,
			estimates: func(t *testing.T, e estimator) {
				require.Equalf(t, float64(2), e.equal(90),
					"an unobserved boundary must fall back to the uniform average, not report zero")
			},
		},
		{
			// Two dense partitions (50 rows per distinct value) start
			// at 5, a sparse one trails from 1, so the merge leaves a
			// one-row tail bucket ending at 5 while all three
			// partitions cover that value. Estimating a Repeat there
			// from the partitions would give 1 + 50 + 50 = 101 on a
			// bucket holding a single row, and LessRowCount subtracts
			// Repeat from the bucket's cumulative count, so it would
			// return -100. Reporting only observed repeats keeps the
			// histogram self-consistent.
			name:       "repeat_never_exceeds_bucket_mass",
			expBuckets: 2,
			parts: []partSpec{
				{hist: []bucketSpec{{lo: di(1), up: di(10), mass: 2, repeat: 1}}},
				{hist: []bucketSpec{{lo: di(5), up: di(100), mass: 100, repeat: 50}}},
				{hist: []bucketSpec{{lo: di(5), up: di(100), mass: 100, repeat: 50}}},
			},
			wantBuckets: []bucketSpec{
				{lo: di(1), up: di(5), mass: 1},
				{lo: di(5), up: di(100), mass: 201},
			},
			extra: func(t *testing.T, hist *statistics.Histogram, _ *statistics.TopN) {
				var prev int64
				for i := range hist.Len() {
					...
					require.LessOrEqualf(t, hist.Buckets[i].Repeat, mass,
						"bucket %d Repeat %d exceeds its mass %d", i, hist.Buckets[i].Repeat, mass)
				}
				require.GreaterOrEqualf(t, hist.LessRowCount(nil, types.NewIntDatum(5)), float64(0),
					"a Repeat above the bucket mass drives LessRowCount negative")
			},
			globalNDV: 4,
			estimates: func(t *testing.T, e estimator) {
				require.GreaterOrEqualf(t, e.between(1, 5), float64(0),
					"a range over the thin tail bucket must not go negative")
				require.LessOrEqualf(t, e.between(1, 100), float64(202),
					"a full-range estimate must not exceed the merged row count")
			},
		},
		{
			...
			globalNDV: 11,
			estimates: func(t *testing.T, e estimator) {
				require.Equalf(t, float64(1000), e.equal(200),
					"a promoted value must be answered by the global TopN, not the histogram")
			},
		},
		{
		...
		},
		{
			...
			// Index estimates go through the encoded-key path, which
			// the column cases never exercise.
			globalNDV: 10,
			estimates: func(t *testing.T, e estimator) {
				require.Equalf(t, float64(90), e.equal(1),
					"value 1 is the global TopN's largest entry (50 + 40)")
				require.Greaterf(t, e.between(1, 200), float64(0),
					"a range over the merged index must estimate some rows")
			},
		},
		{
...
func TestMergePartTopNAndHistToGlobalErrors(t *testing.T) {
	...
	t.Run("nil_topns_and_hists", func(t *testing.T) {
		_, _, err := statistics.MergePartTopNAndHistToGlobal(sc, &killer, nil, nil, 2, 10, false)
		require.Error(t, err)
	})
...
// singleton filter on global TopN admission, and that it honors the
// same "explicit TopN size" contract as per-table analyze.
//
// A value with accumulated count == 1 carries no more selectivity
// signal than the histogram + NDV fallback already provides, and the
// specific singletons that win heap ties are arbitrary. When there are
// more distinct candidates than numTopN slots the merge may drop those
// singletons, but only for the default TopN size. BuildHistAndTopN
// (builder.go) disables pruning whenever numTopN differs from the active
// analyze default (tidb_analyze_default_num_topn), treating an explicit
// size as a value the user wants honored; the merge mirrors that so a
// partitioned table's global TopN matches an identical non-partitioned
// table.
//
// Five checks, all of which must preserve total row count across the
// global TopN + histogram:
//  1. Explicit (non-default) numTopN, candidate pool over budget:
//     pruning is off, so singletons are retained.
//  2. Default numTopN, candidate pool over budget: the filter fires and
//     no count==1 value survives in the global TopN.
//  3. Pool below capacity (NDV < numTopN): singletons enumerate the
//     complete distinct-value set and are always retained.
//  4. Pool exactly at capacity (NDV == numTopN): nothing was evicted, so
//     this is a complete enumeration too and the singletons survive.
//  5. The gate follows tidb_analyze_default_num_topn rather than the
//     compiled-in constant: with the active default raised, that size
//     prunes while the constant default is treated as explicit.
func TestMergePartTopNAndHistToGlobalSingletonFilter(t *testing.T) {
	sc := stmtctx.NewStmtCtxWithTimeZone(time.UTC)
	killer := sqlkiller.SQLKiller{}

	// genSingletons builds n non-overlapping buckets whose upper value
	// appears exactly once (Repeat=1), i.e. n singleton TopN candidates.
	genSingletons := func(n int) []bucketSpec {
		bs := make([]bucketSpec, n)
		for i := range n {
			lo := int64(i*10 + 1)
			bs[i] = bucketSpec{lo: di(lo), up: di(lo + 5), mass: 4, repeat: 1}
		}
		return bs
	}
	rowsPreserved := func(t *testing.T, tc mergeCase, gTopN *statistics.TopN, gHist *statistics.Histogram) {
	...
	// 1. Explicit numTopN with an over-budget pool: the user asked for a
	// specific size, so singletons are retained (parity with per-table
	// analyze, allowPruning=false).
	explicitTC := mergeCase{
		name:       "explicit_topn_keeps_singletons_over_budget",
		numTopN:    3,
		expBuckets: 5,
		parts:      []partSpec{{hist: genSingletons(5)}},
	}
	topNs, hists := buildInputs(t, sc, explicitTC)
	gTopN, gHist, err := statistics.MergePartTopNAndHistToGlobal(
		sc, &killer, topNs, hists, explicitTC.numTopN, explicitTC.expBuckets, explicitTC.isIndex,
	)
	require.NoError(t, err)
	require.NotNil(t, gHist)
	require.NotNil(t, gTopN, "explicit numTopN must retain singletons")
	require.Equalf(t, int(explicitTC.numTopN), len(gTopN.TopN),
		"explicit numTopN should keep numTopN singletons; got %d", len(gTopN.TopN))
	rowsPreserved(t, explicitTC, gTopN, gHist)

	// 2. Default numTopN with a pool larger than the budget: the filter
	...
	dTopN, dHist, err := statistics.MergePartTopNAndHistToGlobal(
		sc, &killer, dTopNs, dHists, defaultTC.numTopN, defaultTC.expBuckets, defaultTC.isIndex,
	)
	require.NoError(t, err)
	require.NotNil(t, dHist)
	if dTopN != nil {
		for _, m := range dTopN.TopN {
			require.GreaterOrEqualf(t, m.Count, uint64(2),
				"default-topn global TopN must not contain a value with count < 2; got %d", m.Count)
		}
	}
	rowsPreserved(t, defaultTC, dTopN, dHist)

	// 3. Pool below capacity (NDV < numTopN): singletons enumerate the
	// complete distinct-value set and must be retained.
	smallTC := mergeCase{
		name:       "global_topn_keeps_singletons_below_capacity",
		numTopN:    20,
		expBuckets: 4,
		parts:      []partSpec{{hist: genSingletons(4)}},
	}
	smallTopNs, smallHists := buildInputs(t, sc, smallTC)
	smallGTopN, smallGHist, err := statistics.MergePartTopNAndHistToGlobal(
	...
	rowsPreserved(t, smallTC, smallGTopN, smallGHist)

	// 4. Pool exactly at capacity (NDV == numTopN): every candidate fit
	// and nothing was evicted, so this is a complete distinct-value
	// enumeration just like check 3 and the singletons must survive. The
	// heap cannot report this on its own, since it caps at numTopN and a
	// full heap looks identical to an over-budget pool; the gate uses the
	...
	exactGTopN, exactGHist, err := statistics.MergePartTopNAndHistToGlobal(
		sc, &killer, exactTopNs, exactHists, exactTC.numTopN, exactTC.expBuckets, exactTC.isIndex,
	)
	require.NoError(t, err)
