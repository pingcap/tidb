...
func ClearOutdatedHistoryStats(sctx sessionctx.Context) error {
	sql := "select count(1) from mysql.stats_meta_history use index (idx_create_time) where create_time <= NOW() - INTERVAL %? SECOND"
	...
	if count > 0 {
		...
		sql = "delete from mysql.stats_history use index (idx_create_time) where create_time <= NOW() - INTERVAL %? SECOND LIMIT 1000"
		_, err = util.Exec(sctx, sql, variable.HistoricalStatsDuration.Load().Seconds())
