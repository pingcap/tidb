...
func (pq *AnalysisPriorityQueue) ProcessDMLChanges() {
	...
	if err := statsutil.CallWithSCtx(pq.statsHandle.SPool(), func(sctx sessionctx.Context) error {
		...
		statslogutil.StatsErrVerboseLogger().Error("Failed to process DML changes", zap.Error(err))
	}
}
...
func (pq *AnalysisPriorityQueue) RequeueMustRetryJobs() {
	...
	if err := statsutil.CallWithSCtx(pq.statsHandle.SPool(), func(sctx sessionctx.Context) error {
		...
		for tableID := range pq.syncFields.mustRetryJobs {
			...
			if err != nil {
				statslogutil.StatsErrVerboseLogger().Error("Failed to recreate and push job for table", zap.Error(err), zap.Int64("tableID", tableID))
				continue
			}
		}
		return nil
	}, statsutil.FlagWrapTxn); err != nil {
		statslogutil.StatsErrVerboseLogger().Error("Failed to requeue must retry jobs", zap.Error(err))
	}
}
	...
	if err := statsutil.CallWithSCtx(pq.statsHandle.SPool(), func(sctx sessionctx.Context) error {
		...
		for _, job := range jobs {
			...
			if !ok {
				...
				err := pq.syncFields.inner.delete(job)
				if err != nil {
					statslogutil.StatsErrVerboseLogger().Error("Failed to delete job from priority queue",
						zap.Error(err),
						zap.String("job", job.String()),
					)
				}
			...
			if err := pq.syncFields.inner.update(job); err != nil {
				statslogutil.StatsErrVerboseLogger().Error("Failed to add job to priority queue",
					zap.Error(err),
					zap.String("job", job.String()),
				)
			}
		...
		statslogutil.StatsErrVerboseLogger().Error("Failed to refresh last analysis duration", zap.Error(err))
	}
}
