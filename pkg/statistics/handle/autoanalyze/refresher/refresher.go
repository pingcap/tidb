...
func (r *Refresher) AnalyzeHighestPriorityTables(sctx sessionctx.Context) bool {
	...
	if err != nil {
		statslogutil.StatsErrVerboseLogger().Error("Set auto analyze time window failed", zap.Error(err))
		return false
	}
	...
	if !r.jobs.IsInitialized() {
		if err := r.jobs.Initialize(r.ctx); err != nil {
			statslogutil.StatsErrVerboseLogger().Error("Failed to initialize the queue", zap.Error(err))
			return false
		}
		...
		if currentAutoAnalyzeRatio != r.lastSeenAutoAnalyzeRatio || currentPruneMode != r.lastSeenPruneMode {
			...
			err := r.jobs.Rebuild()
			if err != nil {
				statslogutil.StatsErrVerboseLogger().Error("Failed to rebuild the queue", zap.Error(err))
				return false
			}
		}
	}
