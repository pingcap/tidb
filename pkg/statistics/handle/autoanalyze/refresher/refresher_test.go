...
func TestIgnoreNilOrPseudoStatsOfPartitionedTable(t *testing.T) {
	...
	defer r.Close()
	require.False(t, r.AnalyzeHighestPriorityTables())
	require.Equal(t, 0, r.GetQueueLength(), "No jobs are added because table stats are nil")
}
...
func TestIgnoreTinyTable(t *testing.T) {
	...
	defer r.Close()
	require.True(t, r.AnalyzeHighestPriorityTables())
	require.Equal(t, 0, r.GetQueueLength(), "Only t1 is added to the job queue, because t2 is a tiny table(not enough data)")
}
