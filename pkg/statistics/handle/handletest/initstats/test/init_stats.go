...
func RunLiteInitStatsWithTableIDs(t *testing.T) {
...
func RunConcurrentlyInitStatsWithMemoryLimit(t *testing.T) {
...
func RunConcurrentlyInitStatsWithoutMemoryLimit(t *testing.T) {
	restore := config.RestoreFunc()
	defer restore()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Performance.LiteInitStats = false
...
func RunDropTableBeforeConcurrentlyInitStats(t *testing.T) {
	restore := config.RestoreFunc()
	defer restore()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Performance.LiteInitStats = false
...
func RunDropTableBeforeNonLiteInitStats(t *testing.T) {
	restore := config.RestoreFunc()
	defer restore()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Performance.LiteInitStats = false
...
func RunSkipStatsInitWithSkipInitStats(t *testing.T) {
	restore := config.RestoreFunc()
	defer restore()
	config.UpdateGlobal(func(conf *config.Config) {
...
func RunNonLiteInitStatsAndCheckTheLastTableStats(t *testing.T) {
	store, dom := session.CreateStoreAndBootstrap(t)
	defer store.Close()
	se := session.CreateSessionAndSetID(t, store)
	session.MustExec(t, se, "use test")
