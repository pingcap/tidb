...
func TestLiteInitStatsWithTableIDs(t *testing.T) {
	...
	withStatsLease(t, -1, func() {
		...
		require.NoError(t, err)
		dom.InfoSyncer().SetSessionManager(&testkit.MockSessionManager{})
		h := dom.StatsHandle()
...
func TestNonLiteInitStatsAndCheckTheLastTableStats(t *testing.T) {
	...
	withStatsLease(t, -1, func() {
		...
		stats1, ok := h.Get(tbl1.Meta().ID)
		require.True(t, ok)
		idx1 := stats1.GetIdx(tbl1.Meta().Indices[0].ID)
		require.NotNil(t, idx1)
		require.True(t, idx1.IsFullLoad())
		stats2, ok := h.Get(tbl2.Meta().ID)
		require.True(t, ok)
		idx2 := stats2.GetIdx(tbl2.Meta().Indices[0].ID)
		require.NotNil(t, idx2)
		require.True(t, idx2.IsFullLoad())
		stats3, ok := h.Get(tbl3.Meta().ID)
		require.True(t, ok)
		idx3 := stats3.GetIdx(tbl3.Meta().Indices[0].ID)
		require.NotNil(t, idx3)
		require.True(t, idx3.IsFullLoad())
