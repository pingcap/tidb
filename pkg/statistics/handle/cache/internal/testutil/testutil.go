...
func NewMockStatisticsTable(columns int, indices int, withCMS, withTopN, withHist bool) *statistics.Table {
	t := &statistics.Table{HistColl: *statistics.NewHistColl(0, false, 0, 0, columns, indices)}
	t.ColAndIdxExistenceMap = statistics.NewColAndIndexExistenceMap(columns, indices)
	for i := 1; i <= columns; i++ {
		var (
		...
		t.ColAndIdxExistenceMap.InsertCol(int64(i), true)
	}
	for i := 1; i <= indices; i++ {
		var (
			cms  *statistics.CMSketch
		...
		t.ColAndIdxExistenceMap.InsertIndex(int64(i), true)
	}
	return t
}
