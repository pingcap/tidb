...
func testIssues24349(t *testing.T, testKit *testkit.TestKit, store kv.Storage) {
	...
	// column b's 8 histogram rows land in two near-equi-depth global
	// buckets: [1,3] with mass 3 (the b=1 rows plus p1's interior row)
	// and [3,4] with mass 5 (all b=3 rows and the b=4 row); repeat 1 on
	// [3,4] is the single b=4 row at the bucket upper.
	testKit.MustQuery("show stats_buckets where table_name='t'").Sort().Check(testkit.Rows(
		...
		"test t global b 0 0 3 0 1 3 0",
		"test t global b 0 1 8 1 3 4 0",
