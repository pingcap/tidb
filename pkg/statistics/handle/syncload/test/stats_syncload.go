...
func RunConcurrentLoadHist(t *testing.T) {
...
func RunConcurrentLoadHistTimeout(t *testing.T) {
	store, dom := testkit.CreateMockStoreAndDomain(t)
...
func RunConcurrentLoadHistWithPanicAndFail(t *testing.T) {
...
func RunSendLoadRequestsWaitTooLong(t *testing.T) {
	originConfig := config.GetGlobalConfig()
	newConfig := config.NewConfig()
	newConfig.Performance.StatsLoadConcurrency = -1 // no worker to consume channel
...
func RunSyncLoadOnObjectWhichCanNotFoundInStorage(t *testing.T) {
	store, dom := testkit.CreateMockStoreAndDomain(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
