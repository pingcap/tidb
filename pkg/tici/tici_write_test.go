...
func TestTiCIDataWriterGroup_FinishPartitionUploadIndexID(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			indices: []*model.IndexInfo{
				{ID: 2, Name: pmodel.NewCIStr("idx_1"), FullTextInfo: &model.FullTextIndexInfo{}},
				{ID: 3, Name: pmodel.NewCIStr("idx_2"), FullTextInfo: &model.FullTextIndexInfo{}},
			},
			inputIndexID: 3,
			wantIndexID:  3,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tbl := &model.TableInfo{ID: 1, Name: pmodel.NewCIStr("t"), Indices: tt.indices}
			mockClient := new(MockMetaServiceClient)
			ticiMgr := newTestTiCIManagerCtx(mockClient)
			mockClient.
				On("GetImportStoragePrefix", mock.Anything, mock.Anything).
