	...
	tblInfo := &model.TableInfo{
		...
		Partition: &model.PartitionInfo{
			...
			Definitions: []model.PartitionDefinition{
				{ID: 101, Name: pmodel.NewCIStr("p2023")},
				{ID: 102, Name: pmodel.NewCIStr("p2024")},
			},
			AddingDefinitions: []model.PartitionDefinition{
				{ID: 103, Name: pmodel.NewCIStr("p2026")},
			},
			DroppingDefinitions: []model.PartitionDefinition{
				{ID: 104, Name: pmodel.NewCIStr("p_old")},
			},
		},
	}
...
func TestFinishPartitionUpload(t *testing.T) {
	...
	matchRequest := func(req *FinishImportPartitionUploadRequest) bool {
		return matchKeyspace[*FinishImportPartitionUploadRequest](keyspaceID)(req) &&
			req.GetTidbTaskId() == taskID &&
			req.GetIndexId() == indexID &&
			req.GetStorageUri() == "/s3/path" &&
			req.GetKeyRange() != nil &&
	...
	// 1st call – success
	mockClient.
		On("FinishImportPartitionUpload", mock.Anything, mock.MatchedBy(matchRequest)).
