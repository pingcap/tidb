...
func TestResourceGroupBasic(t *testing.T) {
	...
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default UNLIMITED MEDIUM YES <nil> TASK_TYPES='stats'"))
	tk.MustExec("alter resource group `default` PRIORITY=LOW")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default UNLIMITED LOW YES <nil> TASK_TYPES='stats'"))
	tk.MustExec("alter resource group `default` ru_per_sec=1000")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default 1000 LOW YES <nil> TASK_TYPES='stats'"))
...
func TestResourceGroupRunaway(t *testing.T) {
	...
	tk.MustExec("create resource group rg2 BURSTABLE RU_PER_SEC=2000 QUERY_LIMIT=()")
	tk.MustExec("create resource group rg3 BURSTABLE RU_PER_SEC=2000 QUERY_LIMIT=()")
...
func TestResourceGroupRunawayFlood(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/pkg/resourcegroup/runaway/skipFlush", `return(1)`))
	...
	require.ErrorContains(t, err, "Query execution was interrupted, identified as runaway query")
	// resume the flush to ensure an aggregated record can be queried later.
	require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/pkg/resourcegroup/runaway/skipFlush"))
