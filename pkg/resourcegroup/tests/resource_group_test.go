...
func TestResourceGroupBasic(t *testing.T) {
	...
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default UNLIMITED MEDIUM YES(UNLIMITED) <nil> <nil>"))
	tk.MustExec("alter resource group `default` PRIORITY=LOW")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default UNLIMITED LOW YES(UNLIMITED) <nil> <nil>"))
	tk.MustExec("alter resource group `default` ru_per_sec=1000")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default 1000 LOW YES(UNLIMITED) <nil> <nil>"))
	tk.MustExec("alter resource group `default` BURSTABLE=OFF")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default 1000 LOW NO <nil> <nil>"))
	tk.MustExec("alter resource group `default` BURSTABLE=UNLIMITED")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default 1000 LOW YES(UNLIMITED) <nil> <nil>"))
	tk.MustExec("alter resource group `default` BURSTABLE=MODERATED")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default 1000 LOW YES(MODERATED) <nil> <nil>"))
	...
	// tk.MustExec("create resource group y RU_PER_SEC=UNLIMITED BURSTABLE=UNLIMITED")
	...
	tk.MustExec("alter resource group x RU_PER_SEC=2000 BURSTABLE=UNLIMITED QUERY_LIMIT=(EXEC_ELAPSED='15s' ACTION DRYRUN WATCH SIMILAR DURATION '10m0s')")
	g = testResourceGroupNameFromIS(t, tk.Session(), "x")
	...
	tk.MustQuery("select * from information_schema.resource_groups where name = 'x'").Check(testkit.Rows("x UNLIMITED MEDIUM NO EXEC_ELAPSED='15s', ACTION=SWITCH_GROUP(y), WATCH=SIMILAR DURATION='10m0s' <nil>"))
	...
	g = testResourceGroupNameFromIS(t, tk.Session(), "y")
	checkFunc(g)
	tk.MustExec("alter resource group y BURSTABLE=UNLIMITED RU_PER_SEC=5000 QUERY_LIMIT=(EXEC_ELAPSED='15s' ACTION KILL)")
	checkFunc = func(groupInfo *model.ResourceGroupInfo) {
		re.Equal(true, groupInfo.ID != 0)
		re.Equal("y", groupInfo.Name.L)
		re.Equal(groupID.Load(), groupInfo.ID)
		re.Equal(uint64(5000), groupInfo.RURate)
		re.Equal(int64(-1), groupInfo.GetBurstLimitAdjusted())
	...
	g = testResourceGroupNameFromIS(t, tk.Session(), "y")
	checkFunc(g)
	tk.MustExec("alter resource group y RU_PER_SEC=6000 BURSTABLE=OFF")
	checkFunc = func(groupInfo *model.ResourceGroupInfo) {
		re.Equal(true, groupInfo.ID != 0)
		re.Equal("y", groupInfo.Name.L)
		re.Equal(groupID.Load(), groupInfo.ID)
	...
	g = testResourceGroupNameFromIS(t, tk.Session(), "y")
	checkFunc(g)
	tk.MustExec("alter resource group y RU_PER_SEC=5000 BURSTABLE=MODERATED")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'y'").Check(testkit.Rows("y 5000 HIGH YES(MODERATED) EXEC_ELAPSED='15s', ACTION=KILL <nil>"))
	...
	tk.MustContainErrMsg("create resource group x BURSTABLE=UNLIMITED, BURSTABLE=UNLIMITED", "Dupliated options specified")
	tk.MustGetErrCode("create resource group x  ru_per_sec=1000, burstable, burstable", mysql.ErrParse)
	tk.MustContainErrMsg("create resource group x  ru_per_sec=1000, BURSTABLE=UNLIMITED, BURSTABLE=UNLIMITED", "Dupliated options specified")
	tk.MustGetErrCode("create resource group x  burstable, ru_per_sec=1000, burstable", mysql.ErrParse)
	tk.MustContainErrMsg("create resource group x BURSTABLE=UNLIMITED, ru_per_sec=1000, BURSTABLE=UNLIMITED", "Dupliated options specified")
	tk.MustContainErrMsg("create resource group x ru_per_sec=1000 BURSTABLE=UNLIMITED QUERY_LIMIT=(EXEC_ELAPSED='15s' action kill action cooldown)", "Dupliated runaway options specified")
	tk.MustContainErrMsg("create resource group x ru_per_sec=1000 QUERY_LIMIT=(EXEC_ELAPSED='15s') BURSTABLE=UNLIMITED priority=Low, QUERY_LIMIT=(EXEC_ELAPSED='15s')", "Dupliated options specified")
	...
	tk.MustQuery("select * from information_schema.resource_groups where name = 'x'").Check(testkit.Rows("x 2000 LOW YES(MODERATED) EXEC_ELAPSED='15s', PROCESSED_KEYS=100, ACTION=KILL <nil>"))
	tk.MustExec("alter resource group x RU_PER_SEC=2000 BURSTABLE=MODERATED QUERY_LIMIT=(EXEC_ELAPSED='15s' PROCESSED_KEYS=100 RU=100 action kill)")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'x'").Check(testkit.Rows("x 2000 LOW YES(MODERATED) EXEC_ELAPSED='15s', PROCESSED_KEYS=100, RU=100, ACTION=KILL <nil>"))
	tk.MustExec("alter resource group x RU_PER_SEC=2000 BURSTABLE=MODERATED QUERY_LIMIT=(PROCESSED_KEYS=200 RU=300 action kill)")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'x'").Check(testkit.Rows("x 2000 LOW YES(MODERATED) PROCESSED_KEYS=200, RU=300, ACTION=KILL <nil>"))
	...
	tk.MustExec("alter resource group x RU_PER_SEC=2000 BURSTABLE=MODERATED QUERY_LIMIT=(EXEC_ELAPSED='15s' action kill)")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'x'").Check(testkit.Rows("x 2000 LOW YES(MODERATED) EXEC_ELAPSED='15s', ACTION=KILL <nil>"))
	...
	tk.MustExec("CREATE RESOURCE GROUP `x_new` RU_PER_SEC=2000 PRIORITY=LOW BURSTABLE=MODERATED QUERY_LIMIT=(EXEC_ELAPSED=\"15s\" ACTION=KILL)")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'x_new'").Check(testkit.Rows("x_new 2000 LOW YES(MODERATED) EXEC_ELAPSED='15s', ACTION=KILL <nil>"))
	...
	tk.MustExec("create resource group y BURSTABLE=MODERATED RU_PER_SEC=2000 QUERY_LIMIT=(EXEC_ELAPSED='1s' action COOLDOWN WATCH EXACT duration '1h')")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'y'").Check(testkit.Rows("y 2000 MEDIUM YES(MODERATED) EXEC_ELAPSED='1s', ACTION=COOLDOWN, WATCH=EXACT DURATION='1h0m0s' <nil>"))
	...
	tk.MustQuery("select * from information_schema.resource_groups where name = 'y'").Check(testkit.Rows("y 4000 MEDIUM YES(MODERATED) EXEC_ELAPSED='1s', ACTION=COOLDOWN, WATCH=EXACT DURATION='1h0m0s' <nil>"))
	...
	tk.MustQuery("select * from information_schema.resource_groups where name = 'y'").Check(testkit.Rows("y 4000 HIGH YES(MODERATED) EXEC_ELAPSED='1s', ACTION=COOLDOWN, WATCH=EXACT DURATION='1h0m0s' <nil>"))
	...
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default 1000 MEDIUM YES(MODERATED) <nil> TASK_TYPES='lightning,br'"))
	tk.MustQuery("show create resource group default").Check(testkit.Rows("default CREATE RESOURCE GROUP `default` RU_PER_SEC=1000, PRIORITY=MEDIUM, BURSTABLE(MODERATED), BACKGROUND=(TASK_TYPES='lightning,br')"))
	...
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default 1000 MEDIUM YES(MODERATED) <nil> TASK_TYPES='br,ddl'"))
	tk.MustExec("alter resource group `default` BACKGROUND=(utilization_limit=30)")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default 1000 MEDIUM YES(MODERATED) <nil> UTILIZATION_LIMIT=30"))
	tk.MustExec("alter resource group `default` BACKGROUND=(task_types='br,ddl',utilization_limit=30)")
	tk.MustQuery("select * from information_schema.resource_groups where name = 'default'").Check(testkit.Rows("default 1000 MEDIUM YES(MODERATED) <nil> TASK_TYPES='br,ddl', UTILIZATION_LIMIT=30"))
}
...
func TestResourceGroupRunaway(t *testing.T) {
	...
	tk.MustQuery("select * from information_schema.resource_groups where name = 'rg2'").Check(testkit.Rows("rg2 2000 MEDIUM YES(MODERATED) EXEC_ELAPSED='50ms', ACTION=KILL, WATCH=EXACT DURATION='1s' <nil>"))
	tk.MustQuery("select * from information_schema.resource_groups where name = 'rg3'").Check(testkit.Rows("rg3 2000 MEDIUM YES(MODERATED) EXEC_ELAPSED='50ms', ACTION=KILL, WATCH=EXACT DURATION=UNLIMITED <nil>"))
...
func TestResourceGroupBurstLimit(t *testing.T) {
	...
	tk.MustExec("alter resource group x BURSTABLE=MODERATED RU_PER_SEC=2000")
	g = testResourceGroupNameFromIS(t, tk.Session(), "x")
	re.Equal(uint64(2000), g.RURate)
