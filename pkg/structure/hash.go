...
// HashIterator is the hash iterator.
...
// Close Implements the Iterator Close.
func (i *HashIterator) Close() { i.iter.Close() }

// NewHashIter creates a reverse hash iterator.
func NewHashIter(t *TxStructure, key []byte) (*HashIterator, error) {
	return newNewHashIter(t, key, nil)
}

func newNewHashIter(t *TxStructure, key []byte, field []byte) (*HashIterator, error) {
	var iterStart kv.Key
	dataPrefix := t.hashDataKeyPrefix(key)
	if len(field) == 0 {
		iterStart = dataPrefix
	} else {
		iterStart = t.encodeHashDataKey(key, field)
	}

	it, err := t.reader.Iter(iterStart, nil)
	if err != nil {
		return nil, errors.Trace(err)
	}
	return &HashIterator{
