...
func TestGCOldVersionPivotDeletedLeadsToReferredFKMissing(t *testing.T) {
	...
	require.Len(t, dom.InfoSchema().GetTableReferredForeignKeys(model.NewCIStr("test"), model.NewCIStr("parent")), 2)
	dom.InfoCache().Data.GCOldVersion(cutVer)
	ref := tkTxn.Session().GetInfoSchema().GetTableReferredForeignKeys(model.NewCIStr("test"), model.NewCIStr("parent"))
	require.Len(t, ref, 1)
