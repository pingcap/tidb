...
func RunGetKeptAllocators(t *testing.T) {
	...
	for i, c := range cases {
		t.Run(fmt.Sprintf("case %d", i), func(t *testing.T) {
			res := infoschema.GetKeptAllocators(c.diff, allocators)
...
func RunTableName2IDCaseSensitive(t *testing.T) {
...
func RunTableName2IDCaseSensitiveMultipleTables(t *testing.T) {
	re := internal.CreateAutoIDRequirement(t)
	defer func() {
		err := re.Store().Close()
		require.NoError(t, err)
...
func RunTableName2IDWithUnloadedTables(t *testing.T) {
	re := internal.CreateAutoIDRequirement(t)
	defer func() {
		err := re.Store().Close()
		require.NoError(t, err)
