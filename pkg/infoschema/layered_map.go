...
func (m *layeredMap[K, V]) scan(fn func(K, V) bool) {
	...
	keys := make(map[K]struct{}, len(m.layers[0]))
...
func (m *layeredMap[K, V]) compact(full bool) {
	...
	var compactIntoLayerIdx int
	if !full {
		compactIntoLayerIdx = len(m.layers) - 2
		for i := 0; i < len(m.layers)-1; i++ {
			if len(m.layers[i]) <= sums[i+1] {
				compactIntoLayerIdx = i
				break
			}
		}
		if compactIntoLayerIdx == 0 {
			full = true
		}
	}
	// merge all layers from compactIntoLayerIdx to the top layer into a new map
	mergedLayer := make(map[K]itemT[V], sums[compactIntoLayerIdx])
	for i := compactIntoLayerIdx; i < len(m.layers); i++ {
	...
	m.layers = m.layers[:compactIntoLayerIdx]
