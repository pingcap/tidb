...
// TestDecodeClusteredHandleInIndexKey covers the regression where a clustered
// (common handle) non-unique global index V1 key was decoded via codec.DecodeOne,
// producing a spurious IntHandle and later panicking in NumCols/EncodedCol.
func TestDecodeClusteredHandleInIndexKey(t *testing.T) {
	const partitionID int64 = 42

	// Common handle whose first encoded byte could be misread by codec.DecodeOne.
	commonHandleBytes, err := codec.EncodeKey(time.UTC, nil, types.NewIntDatum(2001), types.NewStringDatum("x"))
	require.NoError(t, err)
	commonHandle, err := kv.NewCommonHandle(commonHandleBytes)
	require.NoError(t, err)

	// Without partition ID prefix: plain common handle bytes.
	h, err := decodeClusteredHandleInIndexKey(commonHandle.Encoded())
	require.NoError(t, err)
	ch, ok := h.(*kv.CommonHandle)
	require.True(t, ok, "expected CommonHandle, got %T", h)
	require.True(t, ch.Equal(commonHandle))

	// With partition ID prefix: PartitionIDFlag + partition id + common handle bytes.
	suffix := []byte{PartitionIDFlag}
	suffix = codec.EncodeInt(suffix, partitionID)
	suffix = append(suffix, commonHandle.Encoded()...)
	h, err = decodeClusteredHandleInIndexKey(suffix)
	require.NoError(t, err)
	ph, ok := h.(kv.PartitionHandle)
	require.True(t, ok, "expected PartitionHandle, got %T", h)
	require.Equal(t, partitionID, ph.PartitionID)
	innerCH, ok := ph.Handle.(*kv.CommonHandle)
	require.True(t, ok, "expected inner CommonHandle, got %T", ph.Handle)
	require.True(t, innerCH.Equal(commonHandle))
	// The inner handle must support NumCols/EncodedCol (would panic for IntHandle).
	require.Equal(t, commonHandle.NumCols(), ph.Handle.NumCols())
	require.Equal(t, commonHandle.EncodedCol(0), ph.Handle.EncodedCol(0))
}
