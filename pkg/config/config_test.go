...
func TestLogConfig(t *testing.T) {
	...
	var testLoad = func(confStr string, expectedEnableErrorStack, expectedDisableErrorStack, expectedEnableTimestamp, expectedDisableTimestamp nullableBool, resultedDisableTimestamp, resultedDisableErrorVerbose bool) {
		...
		require.Equal(t, logutil.NewLogConfig("info", "text", "tidb-slow.log", "general.log", conf.Log.File, resultedDisableTimestamp, func(config *zaplog.Config) { config.DisableErrorVerbose = resultedDisableErrorVerbose }), conf.Log.ToLogConfig())
		err := f.Truncate(0)
		require.NoError(t, err)
		_, err = f.Seek(0, 0)
		require.NoError(t, err)
...
`)
	...
	// Test for log config.
	require.Equal(t, logutil.NewLogConfig("info", "text", "tidb-slow.log", "general.log", conf.Log.File, false, func(config *zaplog.Config) { config.DisableErrorVerbose = conf.Log.getDisableErrorStack() }), conf.Log.ToLogConfig())
