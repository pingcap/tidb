...
func RunAtomicBoolUnmarshal(t *testing.T) {
	type data struct {
		Ab config.ExportAtomicBool `toml:"ab"`
	}
	var d data
...
func RunNullableBoolUnmarshal(t *testing.T) {
	var nb = config.ExportNullableBool{false, false}
	data, err := json.Marshal(nb)
	require.NoError(t, err)
	err = json.Unmarshal(data, &nb)
	require.NoError(t, err)
	require.Equal(t, config.ExportNullableBoolUnset(), nb)

	nb = config.ExportNullableBool{true, false}
	data, err = json.Marshal(nb)
	require.NoError(t, err)
	err = json.Unmarshal(data, &nb)
	require.NoError(t, err)
	require.Equal(t, config.ExportNullableBoolFalse(), nb)

	nb = config.ExportNullableBool{true, true}
	data, err = json.Marshal(nb)
	require.NoError(t, err)
	err = json.Unmarshal(data, &nb)
	require.NoError(t, err)
	require.Equal(t, config.ExportNullableBoolTrue(), nb)

	// Test for UnmarshalText
	var log config.ExportLog
	_, err = toml.Decode("enable-error-stack = true", &log)
	require.NoError(t, err)
	require.Equal(t, config.ExportNullableBoolTrue(), log.EnableErrorStack)

	_, err = toml.Decode("enable-error-stack = \"\"", &log)
	require.NoError(t, err)
	require.Equal(t, config.ExportNullableBoolUnset(), log.EnableErrorStack)
	...
	require.NoError(t, err)
	require.Equal(t, config.ExportNullableBoolFalse(), log.EnableTimestamp)

	err = json.Unmarshal([]byte("{\"disable-timestamp\":null}"), &log)
	require.NoError(t, err)
	require.Equal(t, config.ExportNullableBoolUnset(), log.DisableTimestamp)
}

func RunLogConfig(t *testing.T) {
	var conf Config
	...
	var testLoad = func(confStr string, expectedEnableErrorStack, expectedDisableErrorStack, expectedEnableTimestamp, expectedDisableTimestamp config.ExportNullableBool, resultedDisableTimestamp, resultedDisableErrorVerbose bool) {
		conf = defaultConf
	...
	testLoad(`
[Log]
`, config.ExportNullableBoolUnset(), config.ExportNullableBoolUnset(), config.ExportNullableBoolUnset(), config.ExportNullableBoolUnset(), false, true)

	testLoad(`
[Log]
enable-timestamp = false
`, config.ExportNullableBoolUnset(), config.ExportNullableBoolUnset(), config.ExportNullableBoolFalse(), config.ExportNullableBoolUnset(), true, true)

	testLoad(`
[Log]
enable-timestamp = true
disable-timestamp = false
`, config.ExportNullableBoolUnset(), config.ExportNullableBoolUnset(), config.ExportNullableBoolTrue(), config.ExportNullableBoolFalse(), false, true)

	testLoad(`
[Log]
enable-timestamp = false
disable-timestamp = true
`, config.ExportNullableBoolUnset(), config.ExportNullableBoolUnset(), config.ExportNullableBoolFalse(), config.ExportNullableBoolTrue(), true, true)

	testLoad(`
[Log]
enable-timestamp = true
disable-timestamp = true
`, config.ExportNullableBoolUnset(), config.ExportNullableBoolUnset(), config.ExportNullableBoolTrue(), config.ExportNullableBoolUnset(), false, true)

	testLoad(`
[Log]
enable-error-stack = false
disable-error-stack = false
`, config.ExportNullableBoolFalse(), config.ExportNullableBoolUnset(), config.ExportNullableBoolUnset(), config.ExportNullableBoolUnset(), false, true)
}

func RunRemovedVariableCheck(t *testing.T) {
	configTest := []struct {
		options string
		err     error
	}{
...
func RunTxnTotalSizeLimitValid(t *testing.T) {
	conf := NewConfig()
	tests := []struct {
		limit uint64
		valid bool
...
func RunConflictInstanceConfig(t *testing.T) {
	t.Cleanup(func() {
		ConflictOptions = nil
...
func RunDeprecatedConfig(t *testing.T) {
	t.Cleanup(func() {
		ConflictOptions = nil
		DeprecatedOptions = nil
	})
...
func RunMaxIndexLength(t *testing.T) {
	conf := NewConfig()
...
func RunIndexLimit(t *testing.T) {
	conf := NewConfig()
...
func RunTableColumnCountLimit(t *testing.T) {
	conf := NewConfig()
...
func RunPluginAuditLog(t *testing.T) {
	conf := NewConfig()
...
func RunTokenLimit(t *testing.T) {
	storeDir := t.TempDir()
	configFile := filepath.Join(storeDir, "config.toml")
	f, err := os.Create(configFile)
	require.NoError(t, err)
...
func RunEncodeDefTempStorageDir(t *testing.T) {
	tests := []struct {
		host       string
		statusHost string
		port       uint
...
func RunModifyThroughLDFlags(t *testing.T) {
	tests := []struct {
		Edition               string
...
func RunSecurityValid(t *testing.T) {
	c1 := NewConfig()
	tests := []struct {
...
func RunTcpNoDelay(t *testing.T) {
	c1 := NewConfig()
	// check default value
	require.True(t, c1.Performance.TCPNoDelay)
}

func RunGetJSONConfig(t *testing.T) {
...
func RunConfigExample(t *testing.T) {
	conf := NewConfig()
	configFile := "config.toml.example"
...
func RunStatsLoadLimit(t *testing.T) {
	conf := NewConfig()
...
func RunGetGlobalKeyspaceName(t *testing.T) {
	conf := NewConfig()
...
func RunGetGlobalTiKVWorkerURL(t *testing.T) {
	conf := NewConfig()
...
func RunAutoScalerConfig(t *testing.T) {
	conf := NewConfig()
...
func RunInvalidConfigWithDeprecatedConfig(t *testing.T) {
	tmpDir := t.TempDir()
...
func RunKeyspaceName(t *testing.T) {
	conf := NewConfig()
	conf.KeyspaceName = "#!"
...
func RunGetTiKVConfigKeepsZeroRUV2RUScale(t *testing.T) {
	conf := NewConfig()
