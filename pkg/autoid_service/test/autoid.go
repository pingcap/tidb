...
import (
	...
	autoidsvc "github.com/pingcap/tidb/pkg/autoid_service"
...
func RunConcurrent(t *testing.T) {
	store, dom := testkit.CreateMockStoreAndDomain(t)
	tk := testkit.NewTestKit(t, store)
	cli := autoidsvc.MockForTest(store)
...
func testAPIWithKeyspace(t *testing.T, keyspaceMeta *keyspacepb.KeyspaceMeta) {
	...
	to := dest{dbID: dbInfo.ID, tblID: tbl.Meta().ID}
	var force = struct{}{}
...
func RunGRPC(t *testing.T) {
	...
	service := autoidsvc.ExportNewWithCli(addr, etcdCli, store)
	defer service.Close()

	var i int
	for !service.ExportLeaderShip().IsOwner() {
	...
	autoidsvc.ExportRegisterAutoIDAllocServer(grpcServer, service)
	go func() {
