...
import (
	...
	tablestore "github.com/pingcap/tidb/pkg/timer/tablestore"
...
func RunBuildInsertTimerSQL(t *testing.T) {
	now := time.Now()
	...
	cases := []struct {
	...
	}{
		{
			...
			record: &timerapi.TimerRecord{
				...
				ManualRequest: timerapi.ManualRequest{
				...
				EventExtra: timerapi.EventExtra{
					EventManualRequestID: "req1",
					EventWatermark:       time.Unix(456, 0),
				},
				EventID:     "e1",
				EventStatus: timerapi.SchedEventTrigger,
	...
	for _, c := range cases {
		...
		sql, args, err := tablestore.ExportedBuildInsertTimerSQL("db1", "t1", c.record)
...
func RunBuildCondCriteria(t *testing.T) {
	cases := []struct {
		cond     timerapi.Cond
		criteria string
		args     []any
		...
		{
			cond:     &timerapi.TimerCond{},
			criteria: "1",
			args:     []any{},
		},
		{
			cond: &timerapi.TimerCond{
				ID: timerapi.NewOptionalVal("1"),
			},
			criteria: "ID = %?",
			args:     []any{"1"},
		},
		{
			cond: &timerapi.TimerCond{
				Namespace: timerapi.NewOptionalVal("ns1"),
			},
			criteria: "NAMESPACE = %?",
			args:     []any{"ns1"},
		},
		{
			cond: &timerapi.TimerCond{
				Key: timerapi.NewOptionalVal("key1"),
			},
			criteria: "TIMER_KEY = %?",
			args:     []any{"key1"},
		},
		...
		{
			cond: &timerapi.TimerCond{
				Tags: timerapi.NewOptionalVal([]string{}),
			},
			criteria: "1",
			args:     []any{},
		},
		{
			cond: &timerapi.TimerCond{
				Tags: timerapi.NewOptionalVal([]string{"l1"}),
			},
		...
		{
			cond: &timerapi.TimerCond{
				Tags: timerapi.NewOptionalVal([]string{"l1", "l2"}),
			},
			criteria: "JSON_EXTRACT(TIMER_EXT, '$.tags') IS NOT NULL AND JSON_CONTAINS((TIMER_EXT->'$.tags'), %?)",
		...
		{
			cond: timerapi.And(
				timerapi.Not(&timerapi.TimerCond{}),
				&timerapi.TimerCond{
					ID: timerapi.NewOptionalVal("2"),
				},
			),
		...
		{
			cond: timerapi.Not(timerapi.And(
				&timerapi.TimerCond{
					Namespace: timerapi.NewOptionalVal("ns1"),
					Key:       timerapi.NewOptionalVal("key1"),
				},
		...
		{
			cond: timerapi.Not(&timerapi.TimerCond{
				ID: timerapi.NewOptionalVal("3"),
			}),
			criteria: "!(ID = %?)",
			args:     []any{"3"},
		},
		...
		{
			cond:     timerapi.Not(timerapi.Not(&timerapi.TimerCond{})),
			criteria: "1",
			args:     []any{},
		},
		{
	...
	for _, c := range cases {
		...
		args := make([]any, 0)
		criteria, args, err := tablestore.ExportedBuildCondCriteria(c.cond, args)
		require.NoError(t, err)
...
func RunBuildSelectTimerSQL(t *testing.T) {
	prefix := "SELECT " +
	...
	cases := []struct {
		...
		{
			cond: &timerapi.TimerCond{ID: timerapi.NewOptionalVal("2")},
			sql:  prefix + " WHERE ID = %?",
			args: []any{"2"},
		},
		{
			cond: &timerapi.TimerCond{Namespace: timerapi.NewOptionalVal("ns1"), Key: timerapi.NewOptionalVal("key1")},
			sql:  prefix + " WHERE NAMESPACE = %? AND TIMER_KEY = %?",
			args: []any{"ns1", "key1"},
		},
		{
			cond: timerapi.Or(
				&timerapi.TimerCond{ID: timerapi.NewOptionalVal("3")},
				&timerapi.TimerCond{Namespace: timerapi.NewOptionalVal("ns1")},
			),
	...
	for _, c := range cases {
		require.Equal(t, strings.Count(c.sql, "%?"), len(c.args))
		sql, args, err := tablestore.ExportedBuildSelectTimerSQL("db1", "t1", c.cond)
		require.NoError(t, err)
		require.Equal(t, c.sql, sql)
		require.Equal(t, c.args, args)
	}
...
func RunBuildUpdateCriteria(t *testing.T) {
	now := time.Now()
	var zeroTime time.Time
	cases := []struct {
		update   *timerapi.TimerUpdate
		criteria string
		args     []any
	}{
		{
		...
		{
			update: &timerapi.TimerUpdate{
				Enable: timerapi.NewOptionalVal(true),
			},
		...
		{
			update: &timerapi.TimerUpdate{
				Enable:          timerapi.NewOptionalVal(false),
				Tags:            timerapi.NewOptionalVal([]string{"l1", "l2"}),
				TimeZone:        timerapi.NewOptionalVal("Asia/Shanghai"),
				SchedPolicyType: timerapi.NewOptionalVal(timerapi.SchedEventInterval),
				SchedPolicyExpr: timerapi.NewOptionalVal("1h"),
				...
				EventData:   timerapi.NewOptionalVal([]byte("data1")),
				EventStart:  timerapi.NewOptionalVal(now),
				EventExtra: timerapi.NewOptionalVal(timerapi.EventExtra{
					EventManualRequestID: "req2",
					EventWatermark:       time.Unix(456, 0),
				}),
				Watermark:    timerapi.NewOptionalVal(now.Add(time.Second)),
				SummaryData:  timerapi.NewOptionalVal([]byte("summary")),
				CheckEventID: timerapi.NewOptionalVal("ee"),
				CheckVersion: timerapi.NewOptionalVal(uint64(1)),
			},
		...
		{
			update: &timerapi.TimerUpdate{
				EventExtra:    timerapi.NewOptionalVal(timerapi.EventExtra{EventManualRequestID: "req1"}),
				ManualRequest: timerapi.NewOptionalVal(timerapi.ManualRequest{ManualRequestID: "req2"}),
			},
		...
		{
			update: &timerapi.TimerUpdate{
				EventExtra:    timerapi.NewOptionalVal(timerapi.EventExtra{EventWatermark: time.Unix(123, 0)}),
				ManualRequest: timerapi.NewOptionalVal(timerapi.ManualRequest{ManualRequestTime: time.Unix(456, 0)}),
			},
			criteria: "TIMER_EXT = JSON_MERGE_PATCH(TIMER_EXT, %?), VERSION = VERSION + 1",
			args: []any{json.RawMessage(`{` +
		...
		{
			update: &timerapi.TimerUpdate{
				TimeZone:        timerapi.NewOptionalVal(""),
				SchedPolicyExpr: timerapi.NewOptionalVal(""),
				EventID:         timerapi.NewOptionalVal(""),
				EventData:       timerapi.NewOptionalVal([]byte(nil)),
				EventStart:      timerapi.NewOptionalVal(zeroTime),
				EventExtra:      timerapi.NewOptionalVal(timerapi.EventExtra{}),
				ManualRequest:   timerapi.NewOptionalVal(timerapi.ManualRequest{}),
				Watermark:       timerapi.NewOptionalVal(zeroTime),
				SummaryData:     timerapi.NewOptionalVal([]byte(nil)),
				Tags:            timerapi.NewOptionalVal([]string(nil)),
			},
	...
	for _, c := range cases {
		require.Equal(t, strings.Count(c.criteria, "%?"), len(c.args))
		criteria, args, err := tablestore.ExportedBuildUpdateCriteria(c.update, []any{})
		require.NoError(t, err)
		require.Equal(t, c.criteria, criteria)
		require.Equal(t, c.args, args)

		criteria, args, err = tablestore.ExportedBuildUpdateCriteria(c.update, []any{1, "2", "3"})
		require.NoError(t, err)
		require.Equal(t, c.criteria, criteria)
...
func RunBuildUpdateTimerSQL(t *testing.T) {
	timerID := "123"
	cases := []struct {
		update *timerapi.TimerUpdate
		sql    string
	...
	for _, c := range cases {
		require.Equal(t, strings.Count(c.sql, "%?"), len(c.args))
		sql, args, err := tablestore.ExportedBuildUpdateTimerSQL("db1", "tbl1", timerID, c.update)
		require.NoError(t, err)
		require.Equal(t, c.sql, sql)
		require.Equal(t, c.args, args)
	}
...
func RunBuildDeleteTimerSQL(t *testing.T) {
	sql, args := tablestore.ExportedBuildDeleteTimerSQL("db1", "tbl1", "123")
...
func RunWithSession(t *testing.T) {
	...
	core := tablestore.ExportedNewTableTimerStoreCore("", "", pool)
	resetSe := func() {
	...
	require.EqualError(t, core.ExportedWithSession(cb1), "mockErr")
	...
	sctx.On("ExecuteInternal", matchCtx, "ROLLBACK", []any(nil)).
		...
		Once()
	require.EqualError(t, core.ExportedWithSession(cb1), "mockErr1")
	sctx.AssertExpectations(t)
	...
	sctx.On("ExecuteInternal", matchCtx, "SELECT @@time_zone", []any(nil)).
		...
		Once()
	require.EqualError(t, core.ExportedWithSession(cb1), "mockErr2")
	sctx.AssertExpectations(t)
	...
	require.Panics(t, func() {
		_ = core.ExportedWithSession(cb1)
	})
	sctx.AssertExpectations(t)
	...
	require.NoError(t, core.ExportedWithSession(cb1))
	sctx.AssertExpectations(t)
	...
	require.EqualError(t, core.ExportedWithSession(cb1), "mockErr3")
	sctx.AssertExpectations(t)
	mockCb1.AssertExpectations(t)
	...
	mockSuccessInit()
	mockCb1.On("cb1", pool.se).Return(nil).Once()
	require.NoError(t, core.ExportedWithSession(func(se *syssession.Session) error {
		sctx.AssertExpectations(t)
		sctx.On("ExecuteInternal", matchCtx, "ROLLBACK", []any(nil)).
	...
	require.NoError(t, core.ExportedWithSctx(cb2))
	sctx.AssertExpectations(t)
	...
	require.EqualError(t, core.ExportedWithSctx(cb2), "mockErr4")
	sctx.AssertExpectations(t)
	mockCb2.AssertExpectations(t)
	...
	require.PanicsWithValue(t, "panic3", func() {
		_ = core.ExportedWithSctx(cb2)
	})
	sctx.AssertExpectations(t)
	mockCb2.AssertExpectations(t)
}

func RunRunInTxn(t *testing.T) {
	se := &mockSession{}
	...
	se.On("ExecuteInternal", matchCtx, "COMMIT", []any(nil)).
		Return(nil, nil).
		Once()
	require.Nil(t, tablestore.ExportedRunInTxn(context.Background(), se, func() error {
		_, err := tablestore.ExportedExecuteSQL(context.Background(), se, "insert into t value(?)", 1)
		return err
	}))
	...
	se.On("ExecuteInternal", matchCtx, "BEGIN PESSIMISTIC", []any(nil)).
		...
		Once()
	err := tablestore.ExportedRunInTxn(context.Background(), se, func() error { return nil })
	...
	se.On("ExecuteInternal", matchCtx, "ROLLBACK", []any(nil)).
		Return(nil, nil).
		Once()
	err = tablestore.ExportedRunInTxn(context.Background(), se, func() error { return errors.New("mockFuncErr") })
