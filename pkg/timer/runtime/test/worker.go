...
func checkAndMockWorkerCounters(t *testing.T, w *timerruntime.ExportedHookWorker) {
...
func checkWorkerCounterValues(t *testing.T, trigger, onPreSched, onPreSchedErr, onPreSchedDelay, onSchedEvent, onSchedEventErr int64, w *timerruntime.ExportedHookWorker) {
...
func RunWorkerStartStop(t *testing.T) {
	...
	timerruntime.newHookWorker(ctx, &wg, "g1", "h1", onlyOnceNewHook(hook), nil)
...
func prepareTimer(t *testing.T, cli timerapi.TimerClient) *timerapi.TimerRecord {
	now := time.Now()
...
func getAndCheckTriggeredTimer(
	t *testing.T, cli timerapi.TimerClient, oldTimer *timerapi.TimerRecord,
	eventID string, eventData []byte, eventStartAfter time.Time, eventStartBefore time.Time,
) *timerapi.TimerRecord {
...
func sendWorkerRequestAndCheckResp(
	t *testing.T, w *timerruntime.ExportedHookWorker, req *timerruntime.ExportedTriggerEventRequest,
	respCh <-chan *timerruntime.ExportedTriggerEventResponse, fn func(response *timerruntime.ExportedTriggerEventResponse),
) {
	select {
	case w.ch <- req:
...
func RunWorkerProcessIdleTimerSuccess(t *testing.T) {
	var wg util.WaitGroupWrapper
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	respChan := make(chan *timerruntime.ExportedTriggerEventResponse)
	...
	w := timerruntime.newHookWorker(ctx, &wg, "g1", "h1", onlyOnceNewHook(hook), nil)
	...
	hook.On("OnPreSchedEvent", mock.Anything, mock.Anything).
		Return(timerapi.PreSchedEventResult{EventData: []byte("eventdata")}, nil).Once().
	...
	request := &timerruntime.ExportedTriggerEventRequest{
		eventID: eventID,
		timer:   timer,
		store:   store,
		resp:    respChan,
	...
	sendWorkerRequestAndCheckResp(t, w, request, respChan, func(resp *timerruntime.ExportedTriggerEventResponse) {
...
func RunWorkerProcessTriggeredTimerSuccess(t *testing.T) {
	var wg util.WaitGroupWrapper
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	err := store.Update(ctx, timer.ID, &api.TimerUpdate{
		...
		EventStatus: api.NewOptionalVal(timerapi.SchedEventTrigger),
...
func RunWorkerProcessDelayOrErr(t *testing.T) {
	var wg util.WaitGroupWrapper
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	// Delay 5 seconds
	hook.On("OnPreSchedEvent", mock.Anything, mock.Anything).
		Return(timerapi.PreSchedEventResult{Delay: 5 * time.Second}, nil).Once()
	sendWorkerRequestAndCheckResp(t, w, request, respChan, func(resp *timerruntime.ExportedTriggerEventResponse) {
	...
	hook.On("OnPreSchedEvent", mock.Anything, mock.Anything).
		Return(timerapi.PreSchedEventResult{}, errors.New("mockErr")).Once()
	sendWorkerRequestAndCheckResp(t, w, request, respChan, func(resp *timerruntime.ExportedTriggerEventResponse) {
		require.False(t, resp.success)
		require.Equal(t, timer.ID, resp.timerID)
		require.Equal(t, eventID, resp.eventID)
	...
	hook.On("OnPreSchedEvent", mock.Anything, mock.Anything).
		Return(timerapi.PreSchedEventResult{}, nil).Once()
	...
	hook.On("OnPreSchedEvent", mock.Anything, mock.Anything).
		Return(timerapi.PreSchedEventResult{}, nil).Once()
	mockCore.On("Update", mock.Anything, mock.Anything, mock.Anything).
		Return(timerapi.ErrVersionNotMatch).Once()
	mockCore.On("List", mock.Anything, mock.Anything).
		Return([]*timerapi.TimerRecord(nil), errors.New("mockErr")).Once()
	sendWorkerRequestAndCheckResp(t, w, request, respChan, func(resp *timerruntime.ExportedTriggerEventResponse) {
		require.False(t, resp.success)
		require.Equal(t, timer.ID, resp.timerID)
		require.Equal(t, eventID, resp.eventID)
	...
	mockCore.On("Update", mock.Anything, mock.Anything, mock.Anything).
		Return(nil).Once()
	mockCore.On("List", mock.Anything, mock.Anything).
		Return([]*timerapi.TimerRecord(nil), nil).Once()
	sendWorkerRequestAndCheckResp(t, w, request, respChan, func(resp *timerruntime.ExportedTriggerEventResponse) {
		require.False(t, resp.success)
		require.Equal(t, timer.ID, resp.timerID)
		require.Equal(t, eventID, resp.eventID)
	...
	anotherEventIDTimer.EventStatus = timerapi.SchedEventTrigger
	...
	mockCore.On("Update", mock.Anything, mock.Anything, mock.Anything).
		Return(nil).Once()
	mockCore.On("List", mock.Anything, mock.Anything).
		Return([]*timerapi.TimerRecord{anotherEventIDTimer}, nil).Once()
	sendWorkerRequestAndCheckResp(t, w, request, respChan, func(resp *timerruntime.ExportedTriggerEventResponse) {
		require.False(t, resp.success)
		require.Equal(t, timer.ID, resp.timerID)
		require.Equal(t, eventID, resp.eventID)
...
func RunWorkerProcessManualRequest(t *testing.T) {
	var wg util.WaitGroupWrapper
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	mockCore.On("List", mock.Anything, mock.Anything).Return([]*timerapi.TimerRecord(nil), errors.New("mockErr")).Once()
	sendWorkerRequestAndCheckResp(t, w, request, respChan, func(resp *timerruntime.ExportedTriggerEventResponse) {
		require.False(t, resp.success)
		require.Equal(t, timer.ID, resp.timerID)
		require.Equal(t, eventID, resp.eventID)
	...
	sendWorkerRequestAndCheckResp(t, w, request, respChan, func(resp *timerruntime.ExportedTriggerEventResponse) {
		...
		timer.EventStatus = timerapi.SchedEventTrigger
...
func RunHookWorkerLoopPanicRecover(t *testing.T) {
	var wg util.WaitGroupWrapper
	...
	hook3.On("OnPreSchedEvent", mock.Anything, mock.Anything).Return(timerapi.PreSchedEventResult{}, nil).Once()
	hook3.On("OnSchedEvent", mock.Anything, mock.Anything).Return(nil).Once()

	w := timerruntime.newHookWorker(ctx, &wg, "g1", "h1", hookFn.Func, nil)
	waitDone(hook3.started, 5*time.Second)

	// check can process request normally after send request
	timer1 := &api.TimerRecord{ID: "1", Version: 1, EventStatus: timerapi.SchedEventIdle}
	timer2 := &api.TimerRecord{ID: timer1.ID, Version: 2, EventID: "event1", EventStatus: timerapi.SchedEventTrigger}
	mockCore, mockStore := newMockStore()
	mockCore.On("Update", mock.Anything, mock.Anything, mock.Anything).Return(nil).Once()
	mockCore.On("List", mock.Anything, mock.Anything).Return([]*timerapi.TimerRecord{timer2}, nil).Once()
	respChan := make(chan *timerruntime.ExportedTriggerEventResponse)
	req := &timerruntime.ExportedTriggerEventRequest{
		eventID: timer2.EventID,
		timer:   timer1,
		store:   mockStore,
		resp:    respChan,
	}
	sendWorkerRequestAndCheckResp(t, w, req, respChan, func(resp *timerruntime.ExportedTriggerEventResponse) {
		require.True(t, resp.success)
	...
	timerruntime.newHookWorker(ctx, &wg, "g1", "h1", hookFn.Func, nil)
...
func RunHookWorkerLoopHandleRequestPanicRecover(t *testing.T) {
	var wg util.WaitGroupWrapper
	...
	hook.On("OnPreSchedEvent", mock.Anything, mock.Anything).Return(timerapi.PreSchedEventResult{}, nil).Once()
