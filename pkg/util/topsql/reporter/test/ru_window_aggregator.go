...
func RunRUWindowAggregatorReportGranularity(t *testing.T) {
...
func RunRUWindowAggregatorCompactTo200(t *testing.T) {
...
func RunRUWindowAggregatorTakeOncePerWindow(t *testing.T) {
...
func RunRUWindowAggregatorResetCurrentWindowDropsUntilBoundary(t *testing.T) {
...
func RunRUWindowAggregatorConcurrentPressure(t *testing.T) {
	const (
		numWriters       = 16
...
func RunRUWindowAggregatorShiftsLateDataAfterWindowReported(t *testing.T) {
...
func RunLateDataUnderConcurrentReporting(t *testing.T) {
	...
	keyB := stmtstats.RUKey{
		User:       "u-b",
		SQLDigest:  stmtstats.BinaryDigest("sql-b"),
		PlanDigest: stmtstats.BinaryDigest("plan-b"),
	}
	lateKey := stmtstats.RUKey{
		User:       "u-late",
		SQLDigest:  stmtstats.BinaryDigest("sql-late"),
		PlanDigest: stmtstats.BinaryDigest("plan-late"),
	}
	...
	for _, ts := range []uint64{1, 16, 31, 46} {
		agg.addBatchToBucket(ts, stmtstats.RUIncrementMap{
			keyA: {TotalRU: 1, ExecCount: 1, ExecDuration: 1},
		})
	}
	for _, ts := range []uint64{61, 76, 91, 106} {
		agg.addBatchToBucket(ts, stmtstats.RUIncrementMap{
			keyB: {TotalRU: 2, ExecCount: 1, ExecDuration: 1},
		})
	}

	first := agg.takeReportRecords(60, 60, []byte("ks"))
	require.NotNil(t, findRURecordByDigest(first, "u-a", "sql-a", "plan-a"))

	start := make(chan struct{})
	lateDone := make(chan struct{})
	reportDone := make(chan struct{})
	var second []tipb.TopRURecord
	go func() {
		<-start
		for i := 0; i < 200; i++ {
			agg.addBatchToBucket(10, stmtstats.RUIncrementMap{
				lateKey: {TotalRU: 999, ExecCount: 1, ExecDuration: 1},
			})
		}
		close(lateDone)
	}()
	...
	require.InDelta(t, 8.0, recB.Items[0].TotalRu, 1e-9)

	require.Nil(t, agg.takeReportRecords(120, 60, []byte("ks")))

	for _, ts := range []uint64{121, 136, 151, 166} {
		agg.addBatchToBucket(ts, stmtstats.RUIncrementMap{
			{
				User:       "u-c",
				SQLDigest:  stmtstats.BinaryDigest("sql-c"),
				PlanDigest: stmtstats.BinaryDigest("plan-c"),
			}: {TotalRU: 3, ExecCount: 1, ExecDuration: 1},
		})
	}
...
func RunRUWindowAggregatorFinalReportCappedTo100x100(t *testing.T) {
...
func RunRUWindowAggregatorRegroupSparseBucketsNoPhantomPoints(t *testing.T) {
	cases := []struct {
		name       string
		interval   uint64
		expectedTS []uint64
...
func RunRUWindowAggregatorOverCapBehaviorKeepsHotKeys(t *testing.T) {
	// Risk covered: under over-cap cardinality, final report stays within 100x100 limits, preserves hot keys,
	// and does not leak already reported window data into the next report window.
	agg := newRUWindowAggregator()
	const (
		numUsers       = 130
		numSQLsPerUser = 130
	...
	batch[hotKey] = &stmtstats.RUIncrement{TotalRU: hotRU, ExecCount: 1, ExecDuration: 1}
	for _, ts := range []uint64{1, 16, 31, 46} {
		agg.addBatchToBucket(ts, batch)
	}
	...
	for i := range records {
		...
		if len(rec.SqlDigest) > 0 || len(rec.PlanDigest) > 0 {
			realUsers[rec.User]++
		} else if _, ok := realUsers[rec.User]; !ok {
			realUsers[rec.User] = 0
		}
	}
	require.LessOrEqual(t, len(realUsers), ruReportTopNUsers)
	...
	agg.addBatchToBucket(61, stmtstats.RUIncrementMap{
		{
			User:       "u-next",
			SQLDigest:  stmtstats.BinaryDigest("sql-next"),
			PlanDigest: stmtstats.BinaryDigest("plan-next"),
		}: {TotalRU: 7, ExecCount: 1, ExecDuration: 1},
	})
	next := agg.takeReportRecords(120, 60, []byte("ks"))
	require.NotEmpty(t, next)
	require.Nil(t, findRURecordByDigest(next, "u-hot", "sql-hot", "plan-hot"))
	nextRec := findRURecordByDigest(next, "u-next", "sql-next", "plan-next")
	require.NotNil(t, nextRec)
	require.Len(t, nextRec.Items, 1)
	require.InDelta(t, 7.0, nextRec.Items[0].TotalRu, 1e-9)
}
...
func makeHotspotDominatedBatch(numTailKeys int, hotRU float64) stmtstats.RUIncrementMap {
	...
	batch[stmtstats.RUKey{
		...
		SQLDigest:  stmtstats.BinaryDigest("sql-hot"),
		PlanDigest: stmtstats.BinaryDigest("plan-hot"),
	}] = &stmtstats.RUIncrement{TotalRU: hotRU, ExecCount: 1, ExecDuration: 1}
	for i := 0; i < numTailKeys; i++ {
		batch[stmtstats.RUKey{
			User:       fmt.Sprintf("u-tail-%03d", i%200),
			SQLDigest:  stmtstats.BinaryDigest(fmt.Sprintf("sql-tail-%05d", i)),
			PlanDigest: stmtstats.BinaryDigest("plan-tail"),
		}] = &stmtstats.RUIncrement{TotalRU: 1, ExecCount: 1, ExecDuration: 1}
	}
	return batch
}
...
func makeLongTailBatch(numTailUsers, numTailSQLsPerUser, numHotUsers int, hotRU float64) stmtstats.RUIncrementMap {
	totalTailKeys := numTailUsers * numTailSQLsPerUser
	batch := make(stmtstats.RUIncrementMap, totalTailKeys+numHotUsers)
	for u := 0; u < numTailUsers; u++ {
		for s := 0; s < numTailSQLsPerUser; s++ {
			batch[stmtstats.RUKey{
				User:       fmt.Sprintf("u-long-%03d", u),
				SQLDigest:  stmtstats.BinaryDigest(fmt.Sprintf("sql-long-%03d-%03d", u, s)),
				PlanDigest: stmtstats.BinaryDigest("plan-long"),
			}] = &stmtstats.RUIncrement{TotalRU: 1, ExecCount: 1, ExecDuration: 1}
		}
	}
	for i := 0; i < numHotUsers; i++ {
		batch[stmtstats.RUKey{
			User:       fmt.Sprintf("u-hot-%02d", i),
			SQLDigest:  stmtstats.BinaryDigest("sql-hot"),
			PlanDigest: stmtstats.BinaryDigest("plan-hot"),
		}] = &stmtstats.RUIncrement{
			TotalRU:      hotRU - float64(i),
			ExecCount:    1,
			ExecDuration: 1,
		}
	}
