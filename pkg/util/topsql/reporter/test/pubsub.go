...
func RunPubSubDataSink(t *testing.T) {
...
func RunNormalizeTopRUItemIntervalInvalid(t *testing.T) {
...
func RunParseTopRUSubscription(t *testing.T) {
	cases := []struct {
		name         string
		req          *tipb.TopSQLSubRequest
		enableTopRU  bool
...
func RunParseTopSQLSubscription(t *testing.T) {
	cases := []struct {
		name         string
		req          *tipb.TopSQLSubRequest
		enableTopSQL bool
...
func RunTopRUPubSub(t *testing.T) {
...
func RunSendTopSQLRecordsGating(t *testing.T) {
	records := []tipb.TopSQLRecord{{
		SqlDigest:  []byte("S1"),
		PlanDigest: []byte("P1"),
		Items: []*tipb.TopSQLRecordItem{{
...
func RunPubSubDataSinkDoSendOrderIsStable(t *testing.T) {
	setGlobalTopRUEnabled := func(enabled bool) {
		for topsqlstate.TopRUEnabled() {
			topsqlstate.DisableTopRU()
		}
