...
func RunSetupCloseAggregator(t *testing.T) {
	for range 3 {
		SetupAggregator()
...
func RunBindRUVersionProviderAfterCloseAggregator(t *testing.T) {
...
func RunRegisterUnregisterCollector(t *testing.T) {
	SetupAggregator()
...
func RunRegisterUnregisterRUCollector(t *testing.T) {
	SetupAggregator()
	defer CloseAggregator()
	time.Sleep(100 * time.Millisecond)
...
func RunAggregatorRegisterCollect(t *testing.T) {
	state.EnableTopSQL()
...
func RunAggregatorRunClose(t *testing.T) {
	a := newAggregator()
...
func RunAggregatorDisableAggregate(t *testing.T) {
	total := StatementStatsMap{}
	a := newAggregator()
	a.registerCollector(newMockCollector(func(data StatementStatsMap) {
		total.Merge(data)
...
func RunAggregatorDisableAggregateRUNoEmit(t *testing.T) {
...
func RunAggregatorRunOrderKeepsFinishedRU(t *testing.T) {
	for state.TopRUEnabled() {
		state.DisableTopRU()
	}
	state.EnableTopRU()
...
func RunAggregatorDetectsRUVersionHandover(t *testing.T) {
	for state.TopRUEnabled() {
		state.DisableTopRU()
	}
	state.EnableTopRU()
...
func RunAggregatorTopSQLTopRUCoexistenceMatrix(t *testing.T) {
	type tc struct {
		name           string
		enableTopSQL   bool
		enableTopRU    bool
...
func RunAggregatorDrainTailIncrementMatrix(t *testing.T) {
	type tc struct {
		name               string
		tailRU             float64
...
func RunDrainPushRUCapsAtMax(t *testing.T) {
	for state.TopRUEnabled() {
		state.DisableTopRU()
	}
	state.EnableTopRU()
