	...
	testCases := []*testCase{
		{
			...
			colName: "c",
			columns: []string{"a"},
			indexs:  []string{mysql.PrimaryKeyName},
			...
			indexs:  []string{mysql.PrimaryKeyName, "d"},
			colLen:  [][]int{{types.UnspecifiedLength, types.UnspecifiedLength}, {types.UnspecifiedLength}},
			fineCol: true,
		}, {
			sql: `
			CREATE TABLE jtest (
...
func RunTableStructEqual(t *testing.T) {
...
func RunSchemacmpEncode(t *testing.T) {
