...
func RunRandWithTime(t *testing.T) {
	rng1 := mathutil.NewWithTime()
...
func RunRandWithSeed(t *testing.T) {
	tests := [4]struct {
		seed  int64
		once  float64
		twice float64
	...
	for _, test := range tests {
		rng := mathutil.NewWithSeed(test.seed)
		got1 := rng.Gen()
...
func RunRandWithSeed1AndSeed2(t *testing.T) {
