...
func TestTableRange(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.exprStr, func(t *testing.T) {
			...
			result, _, _, err := ranger.BuildTableRange(conds, rctx, col.RetType, 0, 0)
			require.NoError(t, err)
...
create table t(
	...
	for _, tt := range tests {
		t.Run(tt.exprStr, func(t *testing.T) {
			...
			res, err := ranger.DetachCondAndBuildRangeForIndex(rctx, conds, cols, lengths, 0, 0)
			require.NoError(t, err)
...
func TestColumnRange(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.exprStr, func(t *testing.T) {
			...
			require.Equal(t, tt.accessConds, expression.StringifyExpressionsWithCtx(ectx, conds))
			result, _, _, err := ranger.BuildColumnRange(conds, rctx, col.RetType, tt.length, 0, 0)
			require.NoError(t, err)
			got := fmt.Sprintf("%v", result)
			require.Equal(t, tt.resultStr, got)
		})
...
create table t(
	...
	for _, tt := range tests {
		t.Run(tt.exprStr, func(t *testing.T) {
			...
			cols, lengths := plannerutil.IndexInfo2PrefixCols(tbl.Columns, selection.Schema().Columns, tbl.Indices[tt.indexPos])
			require.NotNil(t, cols)
			res, err := ranger.DetachCondAndBuildRangeForIndex(sctx.GetRangerCtx(), conds, cols, lengths, 0, 0)
			require.NoError(t, err)
			require.Equal(t, tt.accessConds, expression.StringifyExpressionsWithCtx(ectx, res.AccessConds))
			require.Equal(t, tt.filterConds, expression.StringifyExpressionsWithCtx(ectx, res.RemainedConds))
			got := fmt.Sprintf("%v", res.Ranges)
...
func TestRangeFallbackForDetachCondAndBuildRangeForIndex(t *testing.T) {
	...
	fullRangeMemUsage := res.Ranges.MemUsage()
	checkRangeFallbackAndReset(t, sctx, false)
	res, err = ranger.DetachCondAndBuildRangeForIndex(rctx, conds, cols, lengths, 0, 9)
	require.NoError(t, err)
	require.Len(t, res.Ranges, 9)
	require.Empty(t, res.RemainedConds)
	checkRangeFallbackAndReset(t, sctx, false)

	// The count guardrail is checked before expanding the next index column.
	// 3 ranges on a times 3 points on b would produce 9 ranges, so keep the
	// ranges on a and retain the remaining predicates as filters.
	res, err = ranger.DetachCondAndBuildRangeForIndex(rctx, conds, cols, lengths, 0, 8)
	require.NoError(t, err)
	checkDetachRangeResult(t, res,
	...
	require.Contains(t, warnings[0].Err.Error(), "'tidb_opt_range_max_count' exceeded when building ranges")
	checkRangeFallbackAndReset(t, sctx, false)

	res, err = ranger.DetachCondAndBuildRangeForPartition(rctx, conds, cols, lengths, 0, 8)
	require.NoError(t, err)
	checkDetachRangeResult(t, res,
		"[in(test.t1.a, 10, 20, 30)]",
		"[in(test.t1.b, 40, 50, 60) ge(test.t1.c, 70) le(test.t1.c, 80)]",
		"[[10,10] [20,20] [30,30]]")
	require.Contains(t, sctx.GetSessionVars().StmtCtx.GetWarnings()[0].Err.Error(), "'tidb_opt_range_max_count' exceeded when building ranges")
	checkRangeFallbackAndReset(t, sctx, false)
	...
	t.Run("appending to one emitted range does not corrupt peer ranges", func(t *testing.T) {
		...
		conds := selection.Conditions
		cols, lengths := plannerutil.IndexInfo2PrefixCols(tblInfo.Columns, selection.Schema().Columns, tblInfo.Indices[0])
		res, err := ranger.DetachCondAndBuildRangeForIndex(appendRctx, conds, cols, lengths, 0, 0)
		require.NoError(t, err)
		require.Len(t, res.Ranges, 9)
	...
	t.Run("appending to point-plus-tail ranges does not corrupt peer ranges", func(t *testing.T) {
		...
		appended, rangeFallback := ranger.AppendRanges2PointRanges(rctx, pointRanges, tailRanges, 0, 0)
	...
	quota := fullRangeMemUsage - 1
	res, err = ranger.DetachCondAndBuildRangeForIndex(rctx, conds, cols, lengths, quota, 0)
	require.NoError(t, err)
	checkDetachRangeResult(t, res,
	...
	quota = res.Ranges.MemUsage() - 1
	checkRangeFallbackAndReset(t, sctx, false)
	res, err = ranger.DetachCondAndBuildRangeForIndex(rctx, conds, cols, lengths, 0, 2)
	require.NoError(t, err)
	checkDetachRangeResult(t, res,
		"[]",
...
func TestRangeFallbackForBuildTableRange(t *testing.T) {
	...
	ranges, access, remained, err := ranger.BuildTableRange(conds, rctx, col.RetType, 0, 0)
	require.NoError(t, err)
	...
	checkRangeFallbackAndReset(t, sctx, false)
	ranges, access, remained, err = ranger.BuildTableRange(conds, rctx, col.RetType, 0, 4)
	require.NoError(t, err)
	...
	require.Contains(t, sctx.GetSessionVars().StmtCtx.GetWarnings()[0].Err.Error(), "'tidb_opt_range_max_count' exceeded when building ranges")
	checkRangeFallbackAndReset(t, sctx, false)
	ranges, access, remained, err = ranger.BuildTableRange(conds, rctx, col.RetType, quota, 0)
	require.NoError(t, err)
	require.Equal(t, "[[-inf,+inf]]", fmt.Sprintf("%v", ranges))
	require.Equal(t, "[]", expression.StringifyExpressionsWithCtx(ectx, access))
	require.Equal(t, "[in(test.t.a, 10, 20, 30, 40, 50)]", expression.StringifyExpressionsWithCtx(ectx, remained))
...
func TestRangeFallbackForBuildColumnRange(t *testing.T) {
	...
	require.Equal(t, 1, len(conds))
	require.Equal(t, 0, len(filters))
	ranges, access, remained, err := ranger.BuildColumnRange(conds, rctx, cola.RetType, types.UnspecifiedLength, 0, 0)
	require.NoError(t, err)
	...
	quota := ranges.MemUsage() - 1
	checkRangeFallbackAndReset(t, sctx, false)
	ranges, access, remained, err = ranger.BuildColumnRange(conds, rctx, cola.RetType, types.UnspecifiedLength, 0, 4)
	require.NoError(t, err)
	...
	ranges, access, remained, err = ranger.BuildColumnRange(conds, rctx, cola.RetType, types.UnspecifiedLength, quota, 0)
	require.NoError(t, err)
	require.Equal(t, "[[NULL,+inf]]", fmt.Sprintf("%v", ranges))
	require.Equal(t, "[]", expression.StringifyExpressionsWithCtx(ectx, access))
	require.Equal(t, "[in(test.t.a, aaa, bbb, ccc, ddd, eee)]", expression.StringifyExpressionsWithCtx(ectx, remained))
	...
	require.Equal(t, 1, len(conds))
	require.Equal(t, 0, len(filters))
	ranges, access, remained, err = ranger.BuildColumnRange(conds, rctx, colb.RetType, types.UnspecifiedLength, 1, 0)
	require.NoError(t, err)
	require.Equal(t, "[[-inf,+inf]]", fmt.Sprintf("%v", ranges))
	require.Equal(t, "[]", expression.StringifyExpressionsWithCtx(ectx, access))
...
func TestBinCollationRangeForIndex(t *testing.T) {
	...
	ranges, accessConds, remainedConds, err := ranger.DetachSimpleCondAndBuildRangeForIndex(rctx, conds, cols, lengths, 0, 0)
	require.NoError(t, err)
