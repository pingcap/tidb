...
func TestNoNullIndexChecker(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(strings.Join(c.exprStrs, ","), func(t *testing.T) {
			...
			require.NoError(t, err)
			p, err := plannercore.BuildLogicalPlanForTest(context.Background(), tk.Session(), nodeW, ret.InfoSchema)
			require.NoError(t, err)
			projection := p.(*logicalop.LogicalProjection)
			column := projection.Schema().Columns[0]
			selection := projection.Children()[0].(*logicalop.LogicalSelection)
