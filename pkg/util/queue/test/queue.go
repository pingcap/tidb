...
func RunQueue(t *testing.T) {
	t.Run("basic operations", func(t *testing.T) {
		q := queue.ExportedNewQueue[int](2)
	...
	t.Run("clear operation", func(t *testing.T) {
		q := queue.ExportedNewQueue[string](4)
		q.Push("a")
		q.Push("b")
		q.Push("c")
	...
	t.Run("panic on empty pop", func(t *testing.T) {
		...
		q := queue.ExportedNewQueue[int](1)
		q.Pop() // Should panic
	})

	t.Run("circular buffer behavior", func(t *testing.T) {
		q := queue.ExportedNewQueue[int](3)
		q.Push(1)
		q.Push(2)
		q.Pop() // Remove 1
		q.Push(3)
		...
		// Removed direct field access to queue.head
		// Removed direct field access to queue.tail
