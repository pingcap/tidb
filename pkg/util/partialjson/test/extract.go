...
func RunIter(t *testing.T) {
	...
	for _, ca := range failCases {
		i := partialjson.partialjson.ExportedNewTopLevelJSONTokenIter([]byte(ca[0]))
		_, err := i.Next(false)
		for err == nil {
			_, err = i.Next(false)
		}
	...
	for content, expected := range succCases {
		i := partialjson.partialjson.ExportedNewTopLevelJSONTokenIter([]byte(content))
		expectedIdx := 0
