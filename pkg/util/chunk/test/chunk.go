...
func RunAppendRow(t *testing.T) {
	...
	for i := range numCols {
		col2, col1 := chunk.ExportedGetColumns(chk2)[i], chunk.ExportedGetColumns(chk)[i]
		// Clear elemBuf for comparison
		elemBuf2 := chunk.ExportedGetColumnElemBuf(col2)
		elemBuf1 := chunk.ExportedGetColumnElemBuf(col1)
		defer func() {
			// Restore elemBuf after test
		}()
		_ = elemBuf2
		_ = elemBuf1
	...
	chk = newChunk(8, 8)
	row = chunk.ExportedMutRowFromValues(0, 1, 2, 3).ToRow()
	...
	require.Equal(t, 3, chunk.ExportedGetChunkNumVirtualRows(chk))
...
func RunAppendChunk(t *testing.T) {
	...
	src := chunk.ExportedNewChunkWithCapacityForTest(fieldTypes, 32)
	dst := chunk.ExportedNewChunkWithCapacityForTest(fieldTypes, 32)
...
func RunTruncateTo(t *testing.T) {
	fieldTypes := make([]*types.FieldType, 0, 3)
	fieldTypes = append(fieldTypes, types.NewFieldType(mysql.TypeFloat))
	fieldTypes = append(fieldTypes, types.NewFieldType(mysql.TypeVarchar))
	fieldTypes = append(fieldTypes, types.NewFieldType(mysql.TypeJSON))
	...
	chk := chunk.ExportedNewChunkWithCapacityForTest(fieldTypes[:1], 1)
...
func RunChunkSizeControl(t *testing.T) {
	maxChunkSize := 10
...
func newChunk(elemLen ...int) *chunk.Chunk {
	chk := &chunk.Chunk{}
	for _, l := range elemLen {
		if l > 0 {
			chk.columns = append(chk.columns, chunk.ExportedNewFixedLenColumn(l, 0))
		} else {
			chk.columns = append(chk.columns, chunk.ExportedNewVarLenColumn(0))
		}
	}
	return chk
}

func newChunkWithInitCap(capacity int, elemLen ...int) *chunk.Chunk {
	chk := &chunk.Chunk{}
	for _, l := range elemLen {
		if l > 0 {
			chk.columns = append(chk.columns, chunk.ExportedNewFixedLenColumn(l, capacity))
		} else {
			chk.columns = append(chk.columns, chunk.ExportedNewVarLenColumn(capacity))
		}
	}
	return chk
}
...
func RunCompare(t *testing.T) {
	...
	chunk := chunk.ExportedNewChunkWithCapacityForTest(allTypes, 32)
...
func RunCopyTo(t *testing.T) {
	allTypes := newAllTypes()
	chunk := chunk.ExportedNewChunkWithCapacityForTest(allTypes, 101)
	for i := range allTypes {
		chunk.AppendNull(i)
	}
	for k := range 100 {
...
func RunGetDecimalDatum(t *testing.T) {
	...
	chk := chunk.ExportedNewChunkWithCapacityForTest([]*types.FieldType{decType}, 32)
...
func RunChunkMemoryUsage(t *testing.T) {
	...
	initCap := 10
	chk := chunk.ExportedNewChunkWithCapacityForTest(fieldTypes, initCap)
	...
	colUsage[3] = (initCap+7)>>3 + 0 + initCap*chunk.ExportedSizeTime + chunk.ExportedSizeTime
	colUsage[4] = (initCap+7)>>3 + 0 + initCap*8 + 8

	expectedUsage := 0
	for i := range colUsage {
		expectedUsage += colUsage[i] + int(unsafe.Sizeof(*chunk.ExportedGetColumns(chk)[i]))
	}
	...
	colUsage[1] = (initCap+7)>>3 + (initCap+1)*8 + cap(chunk.ExportedGetColumnData(chunk.ExportedGetColumns(chk)[1])) + 0
	expectedUsage = 0
	for i := range colUsage {
		expectedUsage += colUsage[i] + int(unsafe.Sizeof(*chunk.ExportedGetColumns(chk)[i]))
	}
...
func RunSwapColumn(t *testing.T) {
	...
	chk1 := chunk.ExportedNewChunkWithCapacityForTest(fieldTypes, 1)
	chk1.AppendFloat32(0, 1)
	chk1.MakeRef(0, 1)
	chk1.AppendFloat32(2, 3)

	// chk2: column1 refers to column0
	chk2 := chunk.ExportedNewChunkWithCapacityForTest(fieldTypes, 1)
...
func RunAppendSel(t *testing.T) {
	tll := types.NewFieldType(mysql.TypeLonglong)
	chk := chunk.ExportedNewChunkWithCapacityForTest([]*types.FieldType{tll}, 1024)
...
func RunToString(t *testing.T) {
	...
	chk := chunk.ExportedNewChunkWithCapacityForTest(fieldTypes, 2)
...
func appendInt(chk *chunk.Chunk) {
	chk.Reset()
	for i := range 1000 {
		chk.AppendInt64(0, int64(i))
	}
...
func appendString(chk *chunk.Chunk) {
	chk.Reset()
	for range 1000 {
		chk.AppendString(0, "abcd")
	}
...
func appendRow(chk *chunk.Chunk, row chunk.Row) {
	chk.Reset()
	for range 1000 {
		chk.AppendRow(row)
	}
...
func BenchmarkAppendBytes1024(b *testing.B) {
	chk := chunk.ExportedNewChunkWithCapacityForTest([]*types.FieldType{types.NewFieldType(mysql.TypeString)}, 32)
...
func appendBytes(chk *chunk.Chunk, bs []byte, times int) {
	chk.Reset()
	for range times {
...
func (x *seqNumberGenerateExec) Next(chk *chunk.Chunk, resize bool) {
	if resize {
...
func RunAppendRows(t *testing.T) {
	numCols := 6
	numRows := 10
	chk := newChunk(8, 8, 0, 0, 40, 0)
	strFmt := "%d.12345"
...
func RunAppendRowsByColIdxs(t *testing.T) {
	numCols := 6
	numRows := 10
	chk := newChunk(8, 8, 0, 0, 40, 0)
	strFmt := "%d.12345"
