...
func initChunks(numChk, numRow int) ([]*chunk.Chunk, []*types.FieldType) {
...
func RunDataInDiskByRows(t *testing.T) {
	numChk, numRow := 2, 2
	chks, fields := initChunks(numChk, numRow)
	l := chunk.NewDataInDiskByChunks(fields, "")
	defer func() {
		err := l.Close()
...
func BenchmarkDataInDiskByRowsGetRow(b *testing.B) {
	...
	ptrs := make([]chunk.RowPtr, 0, b.N)
	for range min(b.N, 10000) {
		ptrs = append(ptrs, chunk.RowPtr{
...
type listInDiskWriteDisk struct {
	chunk.ExportedDataInDiskByRows
}

func (l *chunk.ExportedDiskFileReaderWriter) flushForTest() (err error) {
	err = l.file.Close()
	if err != nil {
		return
	}
...
func newDataInDiskByRowsWriteDisk(fieldTypes []*types.FieldType) (*listInDiskWriteDisk, error) {
	l := listInDiskWriteDisk{*chunk.NewDataInDiskByRows(fieldTypes)}
	disk, err := os.CreateTemp(config.GetGlobalConfig().TempStoragePath, strconv.Itoa(l.ExportedDataInDiskByRows.GetDiskTracker().Label()))
	if err != nil {
		return nil, err
	}
	dataFile := chunk.ExportedGetDataFileForRows(&l.ExportedDataInDiskByRows)
	dataFile.file = disk
	dataFile.writer = disk

	disk2, err := os.CreateTemp(config.GetGlobalConfig().TempStoragePath, "offset"+strconv.Itoa(l.ExportedDataInDiskByRows.GetDiskTracker().Label()))
	if err != nil {
		return nil, err
	}
	offsetFile := chunk.ExportedGetOffsetFileForRows(&l.ExportedDataInDiskByRows)
	offsetFile.file = disk2
	offsetFile.writer = disk2
	return &l, nil
}

func (l *listInDiskWriteDisk) GetRow(ptr chunk.RowPtr) (row chunk.ExportedRow, err error) {
	err = l.flushForTest()
	if err != nil {
		return
	}
...
func (l *listInDiskWriteDisk) flushForTest() (err error) {
	err = chunk.ExportedGetDataFileForRows(&l.ExportedDataInDiskByRows).flushForTest()
	if err != nil {
		return err
	}
	return chunk.ExportedGetOffsetFileForRows(&l.ExportedDataInDiskByRows).flushForTest()
}

func checkRow(t *testing.T, row1, row2 chunk.ExportedRow) {
...
func testDataInDiskByRows(t *testing.T, concurrency int) {
	...
	chks, fields := initChunks(numChk, numRow)
	lChecksum := chunk.NewDataInDiskByChunks(fields, "")
	defer lChecksum.Close()
	lDisk, err := newchunk.ExportedDataInDiskByChunksWriteDisk(fields)
	require.NoError(t, err)
	defer lDisk.Close()
	for _, chk := range chks {
...
func RunDataInDiskByRowsWithChecksum1(t *testing.T) {
	defer config.RestoreFunc()()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Security.SpilledFileEncryptionMethod = config.SpilledFileEncryptionMethodPlaintext
	})
	testchunk.ExportedDataInDiskByChunks(t, 1)
}

func RunDataInDiskByRowsWithChecksum2(t *testing.T) {
	defer config.RestoreFunc()()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Security.SpilledFileEncryptionMethod = config.SpilledFileEncryptionMethodPlaintext
	})
	testchunk.ExportedDataInDiskByChunks(t, 2)
}

func RunDataInDiskByRowsWithChecksum8(t *testing.T) {
	defer config.RestoreFunc()()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Security.SpilledFileEncryptionMethod = config.SpilledFileEncryptionMethodPlaintext
	})
	testchunk.ExportedDataInDiskByChunks(t, 8)
}

func RunDataInDiskByRowsWithChecksumReaderWithCache(t *testing.T) {
	defer config.RestoreFunc()()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Security.SpilledFileEncryptionMethod = config.SpilledFileEncryptionMethodPlaintext
	})
...
func RunDataInDiskByRowsWithChecksumReaderWithCacheNoFlush(t *testing.T) {
	defer config.RestoreFunc()()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Security.SpilledFileEncryptionMethod = config.SpilledFileEncryptionMethodPlaintext
	})
...
func RunDataInDiskByRowsWithChecksumAndEncrypt1(t *testing.T) {
	defer config.RestoreFunc()()
	config.UpdateGlobal(func(conf *config.Config) {
...
func RunDataInDiskByRowsWithChecksumAndEncrypt2(t *testing.T) {
	defer config.RestoreFunc()()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Security.SpilledFileEncryptionMethod = config.SpilledFileEncryptionMethodAES128CTR
	})
...
func RunDataInDiskByRowsWithChecksumAndEncrypt8(t *testing.T) {
	defer config.RestoreFunc()()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Security.SpilledFileEncryptionMethod = config.SpilledFileEncryptionMethodAES128CTR
	})
...
func RunDataInDiskByRowsWithChecksumAndEncryptReaderWithCache(t *testing.T) {
	defer config.RestoreFunc()()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Security.SpilledFileEncryptionMethod = config.SpilledFileEncryptionMethodAES128CTR
	})
...
func RunDataInDiskByRowsWithChecksumAndEncryptReaderWithCacheNoFlush(t *testing.T) {
	defer config.RestoreFunc()()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.Security.SpilledFileEncryptionMethod = config.SpilledFileEncryptionMethodAES128CTR
	})
...
func testReaderWithCache(t *testing.T) {
	...
	l := chunk.NewDataInDiskByChunks(field, "")
	err := l.Add(chk)
	require.NoError(t, err)

	// Basic test for GetRow().
	row, err := l.GetRow(chunk.RowPtr{0, 0})
	require.NoError(t, err)
