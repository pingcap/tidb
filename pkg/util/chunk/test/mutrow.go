...
func RunMutRow(t *testing.T) {
	...
	require.Equal(t, chunk.ExportedGetColumns(chunk.ExportedGetMutRowChunk(&mutRow))[0].data, chunk.ExportedGetColumns(chk)[0].data)
	mutRow.SetDatum(0, types.NewDurationDatum(dur))
	require.Equal(t, chunk.ExportedGetColumns(chunk.ExportedGetMutRowChunk(&mutRow))[0].data, chunk.ExportedGetColumns(chk)[0].data)
}

func RunIssue29947(t *testing.T) {
	allTypes := newAllTypes()
	mutRow := MutRowFromTypes(allTypes)
...
func RunMutRowShallowCopyPartialRow(t *testing.T) {
