...
func BenchmarkChecksum(b *testing.B) {
	...
	cols := []rowcodec.ColData{
		{ColumnInfo: &model.ColumnInfo{ID: 1, FieldType: *tp1}, Datum: &datums[0]},
		{ColumnInfo: &model.ColumnInfo{ID: 2, FieldType: *tp2}, Datum: &datums[1]},
		{ColumnInfo: &model.ColumnInfo{ID: 3, FieldType: *tp3}, Datum: &datums[2]},
	}
...
func BenchmarkEncodeFromOldRow(b *testing.B) {
	...
	var buf []byte
	for range b.N {
		buf, err = rowcodec.ExportedEncodeFromOldRow(&xb, nil, oldRowData, buf)
		if err != nil {
			b.Fatal(err)
		}
	}
