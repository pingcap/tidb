...
func RunEncodeLargeSmallReuseBug(t *testing.T) {
...
func RunDecodeRowWithHandle(t *testing.T) {
	handleID := int64(-1)
...
func RunEncodeKindNullDatum(t *testing.T) {
	var encoder rowcodec.Encoder
	colIDs := []int64{1, 2}
...
func RunDecodeDecimalFspNotMatch(t *testing.T) {
	var encoder rowcodec.Encoder
	colIDs := []int64{
		1,
	}
...
func RunTypesNewRowCodec(t *testing.T) {
...
func RunNilAndDefault(t *testing.T) {
	td := []testData{
		{
			1,
			types.NewFieldType(mysql.TypeLonglong),
...
func RunVarintCompatibility(t *testing.T) {
	td := []testData{
		{
			1,
			types.NewFieldType(mysql.TypeLonglong),
...
func RunCodecUtil(t *testing.T) {
	...
	newRow, err = rowcodec.ExportedEncodeFromOldRow(&rb, nil, oldRow, nil)
	require.NoError(t, err)
...
func RunOldRowCodec(t *testing.T) {
	colIDs := []int64{1, 2, 3, 4}
	tps := make([]*types.FieldType, 4)
	for i := range 3 {
		tps[i] = types.NewFieldType(mysql.TypeLonglong)
...
func RunColumnEncode(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			col := rowcodec.ColData{ColumnInfo: &model.ColumnInfo{FieldType: *tt.typ}, Datum: &tt.dat}
	...
	t.Run("nulldatum", func(t *testing.T) {
		for _, typ := range []byte{
			...
			col := rowcodec.ColData{ColumnInfo: &model.ColumnInfo{FieldType: *ft}, Datum: &dat}
...
func RunRowChecksum(t *testing.T) {
	typ1 := types.NewFieldType(mysql.TypeNull)
	dat1 := types.NewDatum(nil)
	col1 := rowcodec.ColData{ColumnInfo: &model.ColumnInfo{ID: 1, FieldType: *typ1}, Datum: &dat1}
	typ2 := types.NewFieldType(mysql.TypeLong)
	dat2 := types.NewDatum(42)
	col2 := rowcodec.ColData{ColumnInfo: &model.ColumnInfo{ID: 2, FieldType: *typ2}, Datum: &dat2}
	typ3 := types.NewFieldType(mysql.TypeVarchar)
	dat3 := types.NewDatum("foobar")
	col3 := rowcodec.ColData{ColumnInfo: &model.ColumnInfo{ID: 3, FieldType: *typ3}, Datum: &dat3}
	...
	col4 := rowcodec.ColData{ColumnInfo: &model.ColumnInfo{ID: 4, FieldType: *typ4}, Datum: &dat4}
	buf := make([]byte, 0, 64)
	for _, tt := range []struct {
		name string
		...
		t.Run(tt.name, func(t *testing.T) {
			row := rowcodec.RowData{Cols: tt.cols, Data: buf}
...
func RunEncodeDecodeRowWithChecksum(t *testing.T) {
...
func RunDecodeWithCommitTS(t *testing.T) {
