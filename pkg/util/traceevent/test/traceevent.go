...
func testTraceEventCategories(t *testing.T) {
	...
	Disable(fr, TxnLifecycle)
	require.False(t, IsEnabled(TxnLifecycle))

	Enable(fr, TxnLifecycle)
	require.True(t, IsEnabled(TxnLifecycle))

	SetCategories(fr, 0)
	require.False(t, IsEnabled(TxnLifecycle))
	SetCategories(fr, AllCategories)
	require.True(t, IsEnabled(TxnLifecycle))
}
...
func RunTraceEventModes(t *testing.T) {
	prev := CurrentMode()
	t.Cleanup(func() {
		_, _ = SetMode(prev)
	})
...
func RunRingBufferSnapshotOrder(t *testing.T) {
...
func RunRingBufferFlushTo(t *testing.T) {
...
func RunCategoryNames(t *testing.T) {
	cases := []struct {
