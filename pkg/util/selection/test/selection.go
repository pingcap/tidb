...
func RunSelectionWithRandomCase(t *testing.T) {
...
func RunSelectionWithSerialCase(t *testing.T) {
...
func randomTestCase(size int) testSlice {
	...
	for range size {
		data = append(data, rand.Int()%100) //nolint:gosec // test code, weak RNG is acceptable
	}
	return data
}
...
func benchmarkQuickSelection(b *testing.B, testCase testSlice, k int) {
	...
	copy(data, testCase)
	b.StartTimer()
	selection.QuickSelect(data, 0, len(testCase)-1, k-1)
}
