...
func newSliceIterWithCopy(data []*kv.Entry) *mock.SliceIter {
	if data == nil {
		return mock.NewSliceIter(nil)
	}

	return mock.NewSliceIter(slices.Clone(data))
}

func RunSliceIter(t *testing.T) {
	assert := assert.New(t)
	slices := []struct {
		data []*kv.Entry
	}{
