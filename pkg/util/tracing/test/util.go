...
// Define a custom type for context keys to avoid collisions with built-in string type
type contextKey string

func RunSpanFromContext(t *testing.T) {
	ctx := context.TODO()
	...
	// test tidb tracing
	collectedSpan := make([]basictracer.RawSpan, 0, 1)
...
func RunChildSpanFromContext(t *testing.T) {
	ctx := context.TODO()
...
func RunFollowFrom(t *testing.T) {
...
func RunCreateSapnBeforeSetupGlobalTracer(t *testing.T) {
	var collectedSpans []basictracer.RawSpan
...
func RunTreeRelationship(t *testing.T) {
	var collectedSpans []basictracer.RawSpan
	ctx := context.TODO()
	// first start a root span
	sp1 := tracing.NewRecordedTrace("test", func(sp basictracer.RawSpan) {
...
func RunTraceInfoFromContext(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.WithValue(ctx, contextKey("val1"), "a"))
	...
	require.Equal(t, "a", ctx.Value(contextKey("val1")))
