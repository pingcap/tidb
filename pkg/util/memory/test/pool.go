...
func (p *memory.ResourcePool) setLimit(newLimit int64) {
	atomic.StoreInt64(&p.limit, newLimit)
}

func RunPoolAllocations(t *testing.T) {
...
func RunResourcePool(t *testing.T) {
	p := NewResourcePoolDefault("test", 1)
	p.Start(nil, (100))
	p.maxUnusedBlocks = 1
...
func newResourcePoolWithLimit(name string, limiit, allocAlignSize int64) *ResourcePool {
	return NewResourcePool(
		memory.newPoolUID(), name, limiit, allocAlignSize, DefMaxUnusedBlocks, PoolActions{})
}

func RunMemoryAllocationEdgeCases(t *testing.T) {
...
func RunMultiSharedGauge(t *testing.T) {
...
func genPool(
	...
	return memory.getPoolImpl(name, parent, reservedBytes)
}
...
func getPoolUsed(
	...
	m := memory.getPoolImpl(name, parent, reservedBytes)
	if usedBytes != 0 {
		acc := m.CreateBudget()
...
func RunResourcePoolTree(t *testing.T) {
...
func RunResourcePoolUsedFromReserved(t *testing.T) {
...
func RunResourcePoolNoDeadlocks(t *testing.T) {
	root := genPool("root", nil)
	defer root.Stop()
