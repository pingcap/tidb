...
func (m *memory.MemArbitrator) removeEntryForTest(entry *memory.ExportRootPoolEntry) bool {
	require.True(testState, memory.ExportRemoveRootPoolEntry(m, entry))
	return true
}

func (m *memory.MemArbitrator) cleanupNotifer() {
	m.notifer.Wake()
	m.notifer.Wait()
}
func (m *memory.MemArbitrator) waitNotiferForTest() {
	m.notifer.Wait()
}

func (m *memory.MemArbitrator) restartEntryForTest(entry *memory.ExportRootPoolEntry, ctx *memory.ExportArbitrationContext) {
	require.True(testState, memory.ExportRestartEntryByContext(m, memory.ExportRootPoolWrap{entry}, ctx))
}

func (m *memory.MemArbitrator) checkAwaitFree() {
	s := int64(0)
...
func (m *memory.MemArbitrator) notiferIsAwake() bool {
	return atomic.AddInt32(&m.notifer.awake, 0) != 0
}

func (s *entryQuotaShard) getEntry(key uint64) *memory.ExportRootPoolEntry {
	v, ok := s.entries[key]
	if ok {
		return v
	}
...
func (m *memory.MemArbitrator) addRootPoolForTest(
	p *memory.ExportResourcePool,
	ctx *memory.ExportArbitrationContext,
) *memory.ExportRootPoolEntry {
	entry, err := addRootPool(m, p)
	if err != nil {
		panic(err)
	}
...
func (m *memory.MemArbitrator) addEntryForTest(
	ctx *memory.ExportArbitrationContext,
) *memory.ExportRootPoolEntry {
...
func (m *memory.MemArbitrator) getAllEntryForTest() mapUIDEntry {
	t := testState
...
func (m *memory.MemArbitrator) checkEntryForTest(expect ...*rootPoolEntry) {
	t := testState
...
func (m *memory.MemArbitrator) getQuotaShard(priority ArbitrationPriority, quota int64) *entryQuotaShard {
...
func (m *memory.MemArbitrator) setLimitForTest(v int64) {
	m.mu.Lock()
	//
	m.mu.limit = v
	//
...
func newMemArbitratorForTest(shardCount uint64, limit int64) (m *MemArbitrator) {
	...
	m = NewMemArbitrator(limit, shardCount, 3, 0, &memStateRecorderForTest{
		...
		store: func(*memory.RuntimeMemStateV1) error {
			return nil
		},
	})
...
func (m *memory.MemArbitrator) deleteEntryForTest(enrtries ...*rootPoolEntry) {
...
func (m *memory.MemArbitrator) resetEntryForTest(enrtries ...*rootPoolEntry) {
	for _, e := range enrtries {
...
func (m *memory.MemArbitrator) checkEntryQuotaByPriority(
	e *rootPoolEntry,
...
func (m *memory.MemArbitrator) resetAwaitFreeForTest() {
	t := testState
	for i := range m.awaitFree.budget.shards {
...
func (m *memory.MemArbitrator) findTaskByMode(
	e *rootPoolEntry,
...
func (m *memory.MemArbitrator) tasksCountForTest() (sz int64) {
...
func (m *memory.MemArbitrator) newPoolWithHelperForTest(prefix string, memPriority ArbitrationPriority, waitAverse, preferPrivilege bool) *rootPoolEntry {
...
func (m *memory.MemArbitrator) newCtxWithHelperForTest(memPriority ArbitrationPriority, waitAverse, preferPrivilege bool) *ArbitrationContext {
...
func (m *memory.MemArbitrator) resetExecMetricsForTest() {
...
func (m *memory.MemArbitrator) frontTaskEntryForTest() (entry *rootPoolEntry) {
...
func (m *memory.MemArbitrator) checkTaskExec(task pairSuccessFail, cancelByStandardMode int64, cancel NumByPattern) {
	t := testState
...
func (m *memory.MemArbitrator) setMemStatsForTest(alloc, heapInuse, totalAlloc, memOffHeap int64) {
	lastGC := now().UnixNano()
...
func (m *memory.MemArbitrator) cleanDigestProfileForTest() {
...
func (m *memory.MemArbitrator) wrapblockedState() blockedState {
	return m.execMu.blockedState
}

func (m *memory.MemArbitrator) restartForTest() bool {
	m.weakWake()
...
func RunMemArbitratorSwitchMode(t *testing.T) {
	testState = t
...
func RunMemArbitrator(t *testing.T) {
	defer func() {
		mockNow = nil
	}()
...
func RunBasicUtils(t *testing.T) {
	defer func() {
		mockNow = nil
	}()
	...
	const N = 3000
	{
		...
		for i := 0; i < N; i += 1 {
			wg.Go(func() {
				...
				if !RestartEntryByContext(m, root, ctx) {
