...
func RunCodecKey(t *testing.T) {
	...
	for i, datums := range table {
		...
		b, err := codec.ExportedEncodeKey(typeCtx.Location(), nil, datums.Input...)
		require.NoError(t, err, comment)

		args, err := codec.ExportedDecode(b, 1)
		require.NoError(t, err, comment)
		require.Equal(t, datums.Expect, args, comment)

		b, err = codec.ExportedEncodeValue(typeCtx.Location(), nil, datums.Input...)
		require.NoError(t, err, comment)
	...
	var raw types.Datum
	raw.SetRaw([]byte("raw"))
	_, err := codec.ExportedEncodeKey(typeCtx.Location(), nil, raw)
	require.Error(t, err)
}
...
func estimateValuesSize(typeCtx types.Context, vals []types.Datum) (int, error) {
	...
	for _, val := range vals {
		length, err := codec.ExportedEstimateValueSize(typeCtx, val)
		if err != nil {
			return 0, err
		}
		size += length
...
func RunCodecKeyCompare(t *testing.T) {
	table := []struct {
		Left   []types.Datum
		Right  []types.Datum
		Expect int
	...
	for _, datums := range table {
		b1, err := codec.ExportedEncodeKey(time.Local, nil, datums.Left...)
		require.NoError(t, err)

		b2, err := codec.ExportedEncodeKey(time.Local, nil, datums.Right...)
		require.NoError(t, err)
...
func RunNumberCodec(t *testing.T) {
	tblInt64 := []int64{
		math.MinInt64,
		math.MinInt32,
		math.MinInt16,
	...
	for _, intNum := range tblInt64 {
		b := codec.ExportedEncodeInt(nil, intNum)
		_, v, err := codec.ExportedDecodeInt(b)
		require.NoError(t, err)
		require.Equal(t, intNum, v)

		b = codec.ExportedEncodeIntDesc(nil, intNum)
		_, v, err = codec.ExportedDecodeIntDesc(b)
		require.NoError(t, err)
		require.Equal(t, intNum, v)

		b = codec.ExportedEncodeVarint(nil, intNum)
		_, v, err = codec.ExportedDecodeVarint(b)
		require.NoError(t, err)
		require.Equal(t, intNum, v)

		b = codec.ExportedEncodeComparableVarint(nil, intNum)
		_, v, err = codec.ExportedDecodeComparableVarint(b)
		require.NoError(t, err)
		require.Equal(t, intNum, v)
	}
	...
	for _, uintNum := range tblUint64 {
		b := codec.ExportedEncodeUint(nil, uintNum)
		_, v, err := codec.ExportedDecodeUint(b)
		require.NoError(t, err)
		require.Equal(t, uintNum, v)

		b = codec.ExportedEncodeUintDesc(nil, uintNum)
		_, v, err = codec.ExportedDecodeUintDesc(b)
		require.NoError(t, err)
		require.Equal(t, uintNum, v)

		b = codec.ExportedEncodeUvarint(nil, uintNum)
		_, v, err = codec.ExportedDecodeUvarint(b)
		require.NoError(t, err)
		require.Equal(t, uintNum, v)

		b = codec.ExportedEncodeComparableUvarint(nil, uintNum)
		_, v, err = codec.ExportedDecodeComparableUvarint(b)
		require.NoError(t, err)
		require.Equal(t, uintNum, v)
	}

	var b []byte
	b = codec.ExportedEncodeComparableVarint(b, -1)
	b = codec.ExportedEncodeComparableUvarint(b, 1)
	b = codec.ExportedEncodeComparableVarint(b, 2)
	b, i, err := codec.ExportedDecodeComparableVarint(b)
	require.NoError(t, err)
	require.Equal(t, int64(-1), i)

	b, u, err := codec.ExportedDecodeComparableUvarint(b)
	require.NoError(t, err)
	require.Equal(t, uint64(1), u)

	_, i, err = codec.ExportedDecodeComparableVarint(b)
	require.NoError(t, err)
	require.Equal(t, int64(2), i)
}

func RunNumberOrder(t *testing.T) {
	tblInt64 := []struct {
		Arg1 int64
		Arg2 int64
		Ret  int
	...
	for _, intNums := range tblInt64 {
		b1 := codec.ExportedEncodeInt(nil, intNums.Arg1)
		b2 := codec.ExportedEncodeInt(nil, intNums.Arg2)
		require.Equal(t, intNums.Ret, bytes.Compare(b1, b2))

		b1 = codec.ExportedEncodeIntDesc(nil, intNums.Arg1)
		b2 = codec.ExportedEncodeIntDesc(nil, intNums.Arg2)
		require.Equal(t, -intNums.Ret, bytes.Compare(b1, b2))

		b1 = codec.ExportedEncodeComparableVarint(nil, intNums.Arg1)
		b2 = codec.ExportedEncodeComparableVarint(nil, intNums.Arg2)
		require.Equal(t, intNums.Ret, bytes.Compare(b1, b2))
	}
	...
	for _, uintNums := range tblUint64 {
		b1 := codec.ExportedEncodeUint(nil, uintNums.Arg1)
		b2 := codec.ExportedEncodeUint(nil, uintNums.Arg2)
		require.Equal(t, uintNums.Ret, bytes.Compare(b1, b2))

		b1 = codec.ExportedEncodeUintDesc(nil, uintNums.Arg1)
		b2 = codec.ExportedEncodeUintDesc(nil, uintNums.Arg2)
		require.Equal(t, -uintNums.Ret, bytes.Compare(b1, b2))

		b1 = codec.ExportedEncodeComparableUvarint(nil, uintNums.Arg1)
		b2 = codec.ExportedEncodeComparableUvarint(nil, uintNums.Arg2)
		require.Equal(t, uintNums.Ret, bytes.Compare(b1, b2))
	}
}

func RunFloatCodec(t *testing.T) {
	tblFloat := []float64{
		-1,
		0,
		1,
	...
	for _, floatNum := range tblFloat {
		b := codec.ExportedEncodeFloat(nil, floatNum)
		_, v, err := codec.ExportedDecodeFloat(b)
		require.NoError(t, err)
		require.Equal(t, floatNum, v)

		b = codec.ExportedEncodeFloatDesc(nil, floatNum)
		_, v, err = codec.ExportedDecodeFloatDesc(b)
		require.NoError(t, err)
		require.Equal(t, floatNum, v)
	}
	...
	for _, floatNums := range tblCmp {
		b1 := codec.ExportedEncodeFloat(nil, floatNums.Arg1)
		b2 := codec.ExportedEncodeFloat(nil, floatNums.Arg2)

		ret := bytes.Compare(b1, b2)
		require.Equal(t, floatNums.Ret, ret)

		b1 = codec.ExportedEncodeFloatDesc(nil, floatNums.Arg1)
		b2 = codec.ExportedEncodeFloatDesc(nil, floatNums.Arg2)
...
func RunBytes(t *testing.T) {
	...
	for _, bytesDatum := range tblBytes {
		b := codec.ExportedEncodeBytes(nil, bytesDatum)
		_, v, err := codec.ExportedDecodeBytes(b, nil)
		require.NoError(t, err)
		require.Equalf(t, bytesDatum, v, "%v - %v - %v", bytesDatum, b, v)

		b = codec.ExportedEncodeBytesDesc(nil, bytesDatum)
		_, v, err = codec.ExportedDecodeBytesDesc(b, nil)
		require.NoError(t, err)
		require.Equalf(t, bytesDatum, v, "%v - %v - %v", bytesDatum, b, v)

		b = codec.ExportedEncodeCompactBytes(nil, bytesDatum)
		_, v, err = codec.ExportedDecodeCompactBytes(b)
		require.NoError(t, err)
	...
	for _, bytesData := range tblCmp {
		b1 := codec.ExportedEncodeBytes(nil, bytesData.Arg1)
		b2 := codec.ExportedEncodeBytes(nil, bytesData.Arg2)

		ret := bytes.Compare(b1, b2)
		require.Equal(t, bytesData.Ret, ret)

		b1 = codec.ExportedEncodeBytesDesc(nil, bytesData.Arg1)
		b2 = codec.ExportedEncodeBytesDesc(nil, bytesData.Arg2)
...
func RunTime(t *testing.T) {
	...
	for _, timeDatum := range tbl {
		...
		b, err := codec.ExportedEncodeKey(time.Local, nil, m)
		require.NoError(t, err)
		v, err := codec.ExportedDecode(b, 1)
		require.NoError(t, err)
	...
	for _, timeData := range tblCmp {
		...
		b1, err := codec.ExportedEncodeKey(time.Local, nil, m1)
		require.NoError(t, err)
		b2, err := codec.ExportedEncodeKey(time.Local, nil, m2)
		require.NoError(t, err)
...
func RunDuration(t *testing.T) {
	...
	for _, duration := range tbl {
		...
		b, err := codec.ExportedEncodeKey(time.Local, nil, types.NewDatum(m))
		require.NoError(t, err)
		v, err := codec.ExportedDecode(b, 1)
		require.NoError(t, err)
		m.Fsp = types.MaxFsp
	...
	for _, durations := range tblCmp {
		...
		b1, err := codec.ExportedEncodeKey(time.Local, nil, types.NewDatum(m1))
		require.NoError(t, err)
		b2, err := codec.ExportedEncodeKey(time.Local, nil, types.NewDatum(m2))
		require.NoError(t, err)
...
func RunDecimal(t *testing.T) {
	...
	for _, decimalNum := range tbl {
		...
		b, err := codec.ExportedEncodeKey(typeCtx.Location(), nil, types.NewDatum(dec))
		require.NoError(t, err)
		v, err := codec.ExportedDecode(b, 1)
		require.NoError(t, err)
		require.Len(t, v, 1)
	...
	for _, decimalNums := range tblCmp {
		...
		b1, err := codec.ExportedEncodeKey(typeCtx.Location(), nil, d1)
		require.NoError(t, err)
		b2, err := codec.ExportedEncodeKey(typeCtx.Location(), nil, d2)
		require.NoError(t, err)

		ret := bytes.Compare(b1, b2)
		require.Equalf(t, decimalNums.Ret, ret, "%v %x %x", decimalNums, b1, b2)

		b1, err = codec.ExportedEncodeValue(typeCtx.Location(), b1[:0], d1)
		require.NoError(t, err)
		size, err := codec.ExportedEstimateValueSize(typeCtx, d1)
		require.NoError(t, err)
	...
	for i := range floats {
		...
		b, err := codec.ExportedEncodeDecimal(nil, d.GetMysqlDecimal(), d.Length(), d.Frac())
		require.NoError(t, err)
		decs = append(decs, b)
		size, err := codec.ExportedEstimateValueSize(typeCtx, d)
		require.NoError(t, err)
	...
	_, err := codec.ExportedEncodeDecimal(nil, d, 20, 5)
	require.Truef(t, terror.ErrorEqual(err, types.ErrTruncated), "err %v", err)

	_, err = codec.ExportedEncodeDecimal(nil, d, 12, 10)
	...
	_, err = codec.ExportedEncodeValue(typeCtx.Location(), nil, decimalDatum)
...
func RunJSON(t *testing.T) {
	...
	buf, err := codec.ExportedEncode(nil, buf, originalDatums, false)
	require.NoError(t, err)

	decodedDatums, err := codec.ExportedDecode(buf, 2)
	require.NoError(t, err)
...
func RunCut(t *testing.T) {
	...
	for i, datums := range table {
		b, err := codec.ExportedEncodeKey(time.Local, nil, datums.Input...)
		require.NoErrorf(t, err, "%d %v", i, datums)

		var d []byte
		for j, e := range datums.Expect {
			d, b, err = codec.ExportedCutOne(b)
			require.NoError(t, err)
			require.NotNil(t, d)

			ed, err1 := codec.ExportedEncodeKey(time.Local, nil, e)
	...
	for i, datums := range table {
		b, err := codec.ExportedEncodeValue(time.Local, nil, datums.Input...)
		require.NoErrorf(t, err, "%d %v", i, datums)
		...
		for j, e := range datums.Expect {
			...
			ed, err1 := codec.ExportedEncodeValue(time.Local, nil, e)
			require.NoError(t, err1)
			require.Equalf(t, ed, d, "%d:%d %#v", i, j, e)
		}
		require.Len(t, b, 0)
	...
	input := 42
	b, err := codec.ExportedEncodeValue(time.Local, nil, types.NewDatum(input))
	require.NoError(t, err)
	rem, n, err := codec.ExportedCutColumnID(b)
	require.NoError(t, err)
	require.Len(t, rem, 0)
	require.Equal(t, int64(input), n)
}

func RunCutOneError(t *testing.T) {
	var b []byte
	_, _, err := codec.ExportedCutOne(b)
	require.Error(t, err)
...
func RunSetRawValues(t *testing.T) {
	datums := types.MakeDatums(1, "abc", 1.1, []byte("def"))
	rowData, err := codec.ExportedEncodeValue(time.Local, nil, datums...)
	require.NoError(t, err)

	values := make([]types.Datum, 4)
	err = codec.ExportedSetRawValues(rowData, values)
	require.NoError(t, err)

	for i, rawVal := range values {
		require.IsType(t, types.KindRaw, rawVal.Kind())
		encoded, encodedErr := codec.ExportedEncodeValue(time.Local, nil, datums[i])
		require.NoError(t, encodedErr)
		require.Equal(t, rawVal.GetBytes(), encoded)
	}
}

func RunDecodeOneToChunk(t *testing.T) {
	typeCtx := types.DefaultStmtNoWarningContext.WithLocation(time.Local)
...
func RunHashGroup(t *testing.T) {
	...
	tp1.SetFlen(20)
	tp1.SetDecimal(5)
	buf, err := codec.ExportedHashGroupKey(time.Local, 3, chk1.Column(0), buf1, tp1)
	require.Error(t, err)
	require.Len(t, buf, 3)
	...
	tp2.SetFlen(12)
	tp2.SetDecimal(10)
	buf, err = codec.ExportedHashGroupKey(time.Local, 3, chk1.Column(0), buf1, tp2)
	require.Error(t, err)
	require.Len(t, buf, 3)
}
...
func chunkForTest(t *testing.T, tz *time.Location, datums []types.Datum, tps []*types.FieldType, rowCount int) *chunk.Chunk {
	decoder := codec.ExportedNewDecoderDirect(chunk.New(tps, 32, 32), tz)
	for range rowCount {
		encoded, err := codec.ExportedEncodeValue(tz, nil, datums...)
		require.NoError(t, err)
		codec.ExportedSetDecoderBuf(decoder, make([]byte, 0, len(encoded)))
		for colIdx, tp := range tps {
			encoded, err = decoder.DecodeOne(encoded, colIdx, tp)
			require.NoError(t, err)
		}
	}
	return codec.ExportedGetDecoderChk(decoder)
}

func RunDecodeRange(t *testing.T) {
	_, _, err := codec.ExportedDecodeRange(nil, 0, nil, nil)
	require.Error(t, err)

	datums := types.MakeDatums(1, "abc", 1.1, []byte("def"))
	rowData, err := codec.ExportedEncodeValue(time.UTC, nil, datums...)
	require.NoError(t, err)

	datums1, _, err := codec.ExportedDecodeRange(rowData, len(datums), nil, nil)
	require.NoError(t, err)
	...
	for _, b := range []byte{codec.ExportedNilFlag, codec.ExportedBytesFlag, codec.ExportedMaxFlag, codec.ExportedMaxFlag + 1} {
		newData := append(rowData, b)
		_, _, err := codec.ExportedDecodeRange(newData, len(datums)+1, nil, nil)
		require.NoError(t, err)
	}
}
...
func testHashChunkRowEqual(t *testing.T, a, b any, equal bool) {
	...
	h := crc32.NewIEEE()
	err1 := codec.ExportedHashChunkRow(typeCtx, h, chk1.GetRow(0), []*types.FieldType{tp1}, []int{0}, buf1)
	sum1 := h.Sum32()
	h.Reset()
	err2 := codec.ExportedHashChunkRow(typeCtx, h, chk2.GetRow(0), []*types.FieldType{tp2}, []int{0}, buf2)
	sum2 := h.Sum32()
	require.NoError(t, err1)
	...
	e, err := codec.ExportedEqualChunkRow(typeCtx,
...
func RunHashChunkRow(t *testing.T) {
	typeCtx := types.DefaultStmtNoWarningContext.WithLocation(time.Local)
	buf := make([]byte, 1)
	datums, tps := datumsForTest()
	...
	h := crc32.NewIEEE()
	err1 := codec.ExportedHashChunkRow(typeCtx, h, chk.GetRow(0), tps, colIdx, buf)
	sum1 := h.Sum32()
	h.Reset()
	err2 := codec.ExportedHashChunkRow(typeCtx, h, chk.GetRow(0), tps, colIdx, buf)
	sum2 := h.Sum32()
...
func RunValueSizeOfSignedInt(t *testing.T) {
	...
	for _, v := range testCase {
		b := codec.ExportedEncodeSignedIntReal(b[:0], v-10, false)
		require.Equal(t, codec.ExportedValueSizeOfSignedInt(v-10), len(b))

		b = codec.ExportedEncodeSignedIntReal(b[:0], v, false)
		require.Equal(t, codec.ExportedValueSizeOfSignedInt(v), len(b))

		b = codec.ExportedEncodeSignedIntReal(b[:0], v+10, false)
		require.Equal(t, codec.ExportedValueSizeOfSignedInt(v+10), len(b))

		// Test for negative value.
		b = codec.ExportedEncodeSignedIntReal(b[:0], 0-v, false)
		require.Equal(t, codec.ExportedValueSizeOfSignedInt(0-v), len(b))

		b = codec.ExportedEncodeSignedIntReal(b[:0], 0-v+10, false)
		require.Equal(t, codec.ExportedValueSizeOfSignedInt(0-v+10), len(b))
...
func RunValueSizeOfUnsignedInt(t *testing.T) {
	...
	var b []byte
	for _, v := range testCase {
		b := codec.ExportedEncodeUnsignedIntReal(b[:0], v-10, false)
		require.Equal(t, codec.ExportedValueSizeOfUnsignedInt(v-10), len(b))

		b = codec.ExportedEncodeUnsignedIntReal(b[:0], v, false)
		require.Equal(t, codec.ExportedValueSizeOfUnsignedInt(v), len(b))
...
func RunHashChunkColumns(t *testing.T) {
	typeCtx := types.DefaultStmtNoWarningContext.WithLocation(time.Local)
	buf := make([]byte, 1)
	datums, tps := datumsForTest()
	...
	for i := range 12 {
		...
		err1 := codec.ExportedHashChunkSelected(typeCtx, vecHash, chk, tps[i], i, buf, hasNull, sel, false)
		err2 := codec.ExportedHashChunkRow(typeCtx, rowHash[0], chk.GetRow(0), tps[i:i+1], colIdx[i:i+1], buf)
		err3 := codec.ExportedHashChunkRow(typeCtx, rowHash[1], chk.GetRow(1), tps[i:i+1], colIdx[i:i+1], buf)
		err4 := codec.ExportedHashChunkRow(typeCtx, rowHash[2], chk.GetRow(2), tps[i:i+1], colIdx[i:i+1], buf)
		require.NoError(t, err1)
		require.NoError(t, err2)
...
func RunDatumHashEquals(t *testing.T) {
	now := time.Now()
