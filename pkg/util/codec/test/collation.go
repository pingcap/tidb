...
func RunHashGroupKeyCollation(t *testing.T) {
	tp := types.NewFieldType(mysql.TypeString)
	...
	buf1 := make([][]byte, n)
	buf2 := make([][]byte, n)
	buf1, err := codec.ExportedHashGroupKey(time.Local, n, chk1.Column(0), buf1, tp)
	require.NoError(t, err)

	buf2, err = codec.ExportedHashGroupKey(time.Local, n, chk2.Column(0), buf2, tp)
	require.NoError(t, err)
...
func RunHashChunkRowCollation(t *testing.T) {
	...
	for i := range n {
		h1 := crc32.NewIEEE()
		h2 := crc32.NewIEEE()
		require.NoError(t, codec.ExportedHashChunkRow(typeCtx, h1, chk1.GetRow(i), tps, cols, buf))
		require.NoError(t, codec.ExportedHashChunkRow(typeCtx, h2, chk2.GetRow(i), tps, cols, buf))
...
func RunHashChunkColumnsCollation(t *testing.T) {
	typeCtx := types.DefaultStmtNoWarningContext.WithLocation(time.Local)
	tp := types.NewFieldType(mysql.TypeString)
	n, chk1, chk2 := prepareCollationData()
	buf := make([]byte, 1)
	...
	tp.SetCollate("binary")
	require.NoError(t, codec.ExportedHashChunkColumns(typeCtx, h1s, chk1, tp, 0, buf, hasNull))
	require.NoError(t, codec.ExportedHashChunkColumns(typeCtx, h2s, chk2, tp, 0, buf, hasNull))
