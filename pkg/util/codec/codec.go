	...
	for i, idx := range buildKeyIndex {
		...
		case mysql.TypeVarchar, mysql.TypeVarString, mysql.TypeString, mysql.TypeBlob, mysql.TypeTinyBlob, mysql.TypeMediumBlob, mysql.TypeLongBlob:
			...
			for j, physicalRowIndex := range usedRows {
				...
				strLen := collator.ByteNumAfterConverted(len(column.GetBytes(physicalRowIndex)) * collator.MaxBytesOneCharacter())
		...
		case mysql.TypeEnum:
			if mysql.HasEnumSetAsIntFlag(tps[i].GetFlag()) {
				...
				for j, physicalRowindex := range usedRows {
					...
					serializedKeyLens[j] += int(sizeByteNum) + collator.ByteNumAfterConverted(len(str)*collator.MaxBytesOneCharacter())
				}
			}
		case mysql.TypeSet:
			sizeByteNum := int64(0)
			...
			for j, physicalRowindex := range usedRows {
				...
				serializedKeyLens[j] += int(sizeByteNum) + collator.ByteNumAfterConverted(len(s.Name)*collator.MaxBytesOneCharacter())
			}
		case mysql.TypeBit:
			signFlagLen := 0
			if serializeModes[i] == NeedSignFlag {
