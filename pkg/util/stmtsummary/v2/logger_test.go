	...
	for _, action := range []string{"rotate", "flush", "close", "evict"} {
		for _, transition := range []struct{ before, after string }{
			...
			t.Run(action+"/"+transition.before+"-"+transition.after, func(t *testing.T) {
				...
				raw := memoryRecord.SampleSQL
				require.Equal(t, info.LazyInfo.GetOriginalSQL(), raw)
				...
				require.Equal(t, raw, memoryRecord.SampleSQL, "a memory reader's retained record must remain raw")
			})
		}
	}
}
