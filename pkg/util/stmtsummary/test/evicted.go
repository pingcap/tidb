...
func RunMapToEvictedCountDatum(t *testing.T) {
...
func RunSimpleStmtSummaryByDigestEvicted(t *testing.T) {
...
func RunStmtSummaryByDigestEvictedElement(t *testing.T) {
...
func RunEvictedCountDetailed(t *testing.T) {
	ssMap := newStmtSummaryByDigestMap()
	ssMap.Clear()
	err := ssMap.SetRefreshInterval(60)
	require.NoError(t, err)
...
func RunNewStmtSummaryByDigestEvictedElement(t *testing.T) {
	now := time.Now().Unix()
	end := now + 60
