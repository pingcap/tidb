...
func ExtractTableHintsFromStmtNode(node ast.Node, warnHandler HintWarnInterface) []*ast.TableOptimizerHint {
	switch x := node.(type) {
	case *ast.SelectStmt:
		return x.TableHints
	case *ast.UpdateStmt:
...
func checkInsertStmtHintDuplicated(node ast.Node, warnHandler HintWarnInterface) {
	switch x := node.(type) {
	case *ast.InsertStmt:
