...
func RunChecksumReadAt(t *testing.T) {
	f := newFakeFile()

	w := newTestBuff("0123456789", 510)

	csw := checksum.ExportedNewWriter(checksum.ExportedNewWriter(checksum.ExportedNewWriter(checksum.ExportedNewWriter(f))))
	...
	assertReadAt := func(off int64, assertErr error, assertN int, assertString string) {
		cs := checksum.ExportedNewReader(checksum.ExportedNewReader(checksum.ExportedNewReader(checksum.ExportedNewReader(f))))
		r := make([]byte, 10)
...
func RunAddOneByte(t *testing.T) {
...
func testAddOneByte(t *testing.T, encrypt bool) {
	...
	for i := 0; ; i++ {
		...
		if i < 5 {
			...
			require.ErrorIs(t, err, checksum.ExportedErrChecksumFail())
		}
	}
}
...
// both the current block and the following block have errors.
func RunDeleteOneByte(t *testing.T) {
	t.Run("unencrypted", func(t *testing.T) {
...
func RunModifyOneByte(t *testing.T) {
	t.Run("unencrypted", func(t *testing.T) {
...
func RunReadEmptyFile(t *testing.T) {
	t.Run("unencrypted", func(t *testing.T) {
...
func testReadEmptyFile(t *testing.T, encrypt bool) {
	...
	for i := range 11 {
		...
		underlying = checksum.ExportedNewReader(underlying)
		r := make([]byte, 10)
...
// only the current block has error.
func RunModifyThreeBytes(t *testing.T) {
	t.Run("unencrypted", func(t *testing.T) {
...
func RunReadDifferentBlockSize(t *testing.T) {
	t.Run("unencrypted", func(t *testing.T) {
...
func testReadDifferentBlockSize(t *testing.T, encrypt bool) {
	...
	underlying = checksum.ExportedNewWriter(underlying)
...
func RunWriteDifferentBlockSize(t *testing.T) {
	t.Run("unencrypted", func(t *testing.T) {
...
func testWriteDifferentBlockSize(t *testing.T, encrypt bool) {
	...
	underlying1 = checksum.ExportedNewWriter(underlying1)
	underlying2 = checksum.ExportedNewWriter(underlying2)
...
func RunChecksumWriter(t *testing.T) {
	f := newFakeFile()

	buf := newTestBuff("0123456789", 100)
	// Write 1000 bytes and flush.
	w := checksum.ExportedNewWriter(f)
...
func RunChecksumWriterAutoFlush(t *testing.T) {
	f := newFakeFile()
...
var checkFlushedData = func(t *testing.T, f io.ReaderAt, off int64, readBufLen int, assertN int, assertErr error, assertRes []byte) {
	...
	r := checksum.ExportedNewReader(f)
