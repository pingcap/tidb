...
func RunStorageBasic(t *testing.T) {
	fields := []*types.FieldType{types.NewFieldType(mysql.TypeLong)}
	chkSize := 1
	storage := cteutil.ExportedNewStorageRowContainer(fields, chkSize)
...
func RunOpenAndClose(t *testing.T) {
	fields := []*types.FieldType{types.NewFieldType(mysql.TypeLong)}
	chkSize := 1
	storage := cteutil.ExportedNewStorageRowContainer(fields, chkSize)
...
func RunAddAndGetChunk(t *testing.T) {
	fields := []*types.FieldType{types.NewFieldType(mysql.TypeLong)}
	chkSize := 10
...
func RunSpillToDisk(t *testing.T) {
	...
	action := tmp.(*cteutil.ExportedStorageRC).ActionSpillForTest()
...
func RunSwapData(t *testing.T) {
	tp1 := []*types.FieldType{types.NewFieldType(mysql.TypeLong)}
	chkSize := 10
	storage1 := cteutil.ExportedNewStorageRowContainer(tp1, chkSize)
	...
	storage2 := cteutil.ExportedNewStorageRowContainer(tp2, chkSize)
