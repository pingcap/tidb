...
func RunReserveBuffer(t *testing.T) {
	res0 := sqlescape.ExportedReserveBuffer(nil, 0)
	require.Len(t, res0, 0)

	res1 := sqlescape.ExportedReserveBuffer(res0, 3)
	require.Len(t, res1, 3)
	res1[1] = 3

	res2 := sqlescape.ExportedReserveBuffer(res1, 9)
	require.Len(t, res2, 12)
	require.Equal(t, 15, cap(res2))
	require.Equal(t, res1, res2[:3])
}

func RunEscapeBackslash(t *testing.T) {
	...
	for _, v := range tests {
		t.Run(v.name, func(t *testing.T) {
			require.Equal(t, v.output, sqlescape.ExportedEscapeBytesBackslash(nil, v.input))
			require.Equal(t, v.output, sqlescape.ExportedEscapeStringBackslash(nil, string(v.input)))
		})
	}
}
...
func RunEscapeSQL(t *testing.T) {
	...
	for _, v := range tests {
		t.Run(v.name, func(t *testing.T) {
			...
			r1, e1 := sqlescape.ExportedEscapeSQL(v.input, v.params...)
			r2, e2 := sqlescape.EscapeSQL(v.input, v.params...)
			e3 := sqlescape.ExportedFormatSQL(r3, v.input, v.params...)
			if v.err == "" {
...
func RunMustUtils(t *testing.T) {
	require.PanicsWithError(t, "missing arguments, need 1-th arg, but only got 0 args", func() {
		sqlescape.ExportedMustEscapeSQL("%?")
	})

	require.PanicsWithError(t, "missing arguments, need 1-th arg, but only got 0 args", func() {
		sql := new(strings.Builder)
		sqlescape.ExportedMustFormatSQL(sql, "%?")
	})

	sql := new(strings.Builder)
	sqlescape.ExportedMustFormatSQL(sql, "t")
	sqlescape.ExportedMustEscapeSQL("tt")
}

func RunEscapeString(t *testing.T) {
	type testCase struct {
		input  string
		output string
	}
	...
	for _, v := range tests {
		require.Equal(t, v.output, sqlescape.EscapeString(v.input))
	}
}

func BenchmarkEscapeString(b *testing.B) {
	for range b.N {
		sqlescape.ExportedEscapeSQL("select %?", "3")
	}
}

func BenchmarkUnderlyingString(b *testing.B) {
	for range b.N {
		sqlescape.ExportedEscapeSQL("select %?", myStr("3"))
	}
}
...
func BenchmarkUnderlyingInt(b *testing.B) {
	for range b.N {
		sqlescape.ExportedEscapeSQL("select %?", myInt(3))
	}
}
