...
// MatchType is used to indicate whether query was interrupted by runaway identification or quarantine watch.
type MatchType uint

const (
	// MatchTypeWatch shows quarantine watch.
	MatchTypeWatch MatchType = iota
	// MatchTypeIdentify shows identification.
	MatchTypeIdentify
)
...
func (rm *Manager) markRunaway(resourceGroupName, originalSQL, planDigest string, action string, matchType MatchType, now *time.Time) {
	source := rm.serverID
	if !rm.syncerInitialized.Load() {
		rm.logOnce.Do(func() {
			logutil.BgLogger().Warn("runaway syncer is not initialized, so can't records about runaway")
