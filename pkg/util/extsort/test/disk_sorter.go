...
func RunDiskSorterCommon(t *testing.T) {
...
func RunDiskSorterCommonParallel(t *testing.T) {
	sorter, err := OpenDiskSorter(t.TempDir(), &DiskSorterOptions{
		WriterBufferSize:    32 * 1024,
		CompactionThreshold: 4,
		MaxCompactionDepth:  4,
...
func RunDiskSorterReopen(t *testing.T) {
	dirname := t.TempDir()
...
func RunKVStatsCollector(t *testing.T) {
	kvs := []keyValue{
		{[]byte("aa"), []byte("11")},
		{[]byte("bb"), []byte("22")},
		{[]byte("cc"), []byte("33")},
...
func RunMakeFilename(t *testing.T) {
	testCases := []struct {
		dir      string
		fileNum  int
		expected string
...
func RunParseFilename(t *testing.T) {
	testCases := []struct {
		filename   string
		expOK      bool
		expFileNum int
...
func RunSSTWriterEmpty(t *testing.T) {
	fs := vfs.Default
	dirname := t.TempDir()
	fileNum := 13
...
func RunSSTWriterError(t *testing.T) {
	fs := vfs.Default
	dirname := t.TempDir()
	fileNum := 13
...
func RunSSTReaderPool(t *testing.T) {
	fs := vfs.Default
	dirname := t.TempDir()
...
func RunSSTReaderPoolParallel(t *testing.T) {
	fs := vfs.Default
	dirname := t.TempDir()
...
func RunMergingIter(t *testing.T) {
	fs := vfs.Default
	dirname := t.TempDir()
...
func RunPickCompactionFiles(t *testing.T) {
	testCases := []struct {
...
func RunSplitCompactionFiles(t *testing.T) {
	testCases := []struct {
		files              []*fileMetadata
...
func RunBuildCompactions(t *testing.T) {
	testCases := []struct {
		files             []*fileMetadata
