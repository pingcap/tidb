...
func TestCopRuntimeStats(t *testing.T) {
	...
	aggID := 2
	tableReaderID := 3
	stats.RecordOneCopTask(tableScanID, kv.TiKV, "8.8.8.8", nil, nil, mockExecutorExecutionSummary(1, 1, 1))
	stats.RecordOneCopTask(tableScanID, kv.TiKV, "8.8.8.9", nil, nil, mockExecutorExecutionSummary(2, 2, 2))
	stats.RecordOneCopTask(aggID, kv.TiKV, "8.8.8.8", nil, nil, mockExecutorExecutionSummary(3, 3, 3))
	stats.RecordOneCopTask(aggID, kv.TiKV, "8.8.8.9", nil, nil, mockExecutorExecutionSummary(4, 4, 4))
	scanDetail := &util.ScanDetail{
		TotalKeys:                 15,
		ProcessedKeys:             10,
	...
	stats.RecordCopStats(tableScanID, kv.TiKV, scanDetail, nil)
	...
	copStats.rows = 10
	copStats.loop += 1
	copStats.consume += int64(time.Second)
	copStats.rows += int64(10)
...
func TestCopRuntimeStatsForTiFlash(t *testing.T) {
	...
	aggID := 2
	tableReaderID := 3
	stats.RecordOneCopTask(tableScanID, kv.TiFlash, "8.8.8.8", nil, nil, mockExecutorExecutionSummaryForTiFlash(1, 1, 1, 1, 8192, 0, 15, 200, 40, 10, 4, 1, 100, 50, 30000000, 20000000, 10000000, "tablescan_"+strconv.Itoa(tableScanID)))
	stats.RecordOneCopTask(tableScanID, kv.TiFlash, "8.8.8.9", nil, nil, mockExecutorExecutionSummaryForTiFlash(2, 2, 2, 1, 0, 0, 0, 2, 0, 0, 0, 0, 0, 0, 20000000, 10000000, 5000000, "tablescan_"+strconv.Itoa(tableScanID)))
	stats.RecordOneCopTask(aggID, kv.TiFlash, "8.8.8.8", nil, nil, mockExecutorExecutionSummaryForTiFlash(3, 3, 3, 1, 12000, 6000, 60, 1000, 20, 5, 1, 0, 20, 0, 0, 0, 0, "aggregation_"+strconv.Itoa(aggID)))
	stats.RecordOneCopTask(aggID, kv.TiFlash, "8.8.8.9", nil, nil, mockExecutorExecutionSummaryForTiFlash(4, 4, 4, 1, 8192, 80000, 40, 2000, 30, 1, 1, 0, 0, 0, 0, 0, 0, "aggregation_"+strconv.Itoa(aggID)))
	scanDetail := &util.ScanDetail{
		TotalKeys:                 10,
		ProcessedKeys:             10,
	...
	stats.RecordCopStats(tableScanID, kv.TiFlash, scanDetail, nil)
	stats.RecordCopStats(tableScanID, kv.TiFlash, scanDetail, nil)
	require.True(t, stats.ExistsCopStats(tableScanID))
...
func TestVectorSearchStats(t *testing.T) {
	...
	stats.RecordOneCopTask(1, kv.TiFlash, "8.8.8.8", nil, nil, execSummary)
...
func TestCopRuntimeStats2(t *testing.T) {
	...
	for range 1005 {
		stats.RecordOneCopTask(tableScanID, kv.TiKV, "8.8.8.9", nil, nil, mockExecutorExecutionSummary(2, 2, 2))
		stats.RecordCopStats(tableScanID, kv.TiKV, scanDetail, timeDetail)
	}
