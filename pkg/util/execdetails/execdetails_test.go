...
func TestUpdateRUV2MetricsFromCommitDetails(t *testing.T) {
	...
	t.Run("reporting clone leaves live metrics unchanged", func(t *testing.T) {
		live := NewRUV2Metrics()
		details := &util.CommitDetails{WriteKeys: 3, WriteSize: 66, PrewriteRegionNum: 4}
		snapshot := live.CloneWithCommitDetailsForReporting(details)

		require.Zero(t, live.WriteKeys())
		require.Zero(t, live.WriteSize())
		require.Zero(t, live.PrewriteRegionNum())
		require.Equal(t, int64(3), snapshot.WriteKeys())
		require.Equal(t, int64(66), snapshot.WriteSize())
		require.Equal(t, int64(4), snapshot.PrewriteRegionNum())

		zeroSnapshot := live.CloneWithCommitDetailsForReporting(&util.CommitDetails{})
		require.Zero(t, zeroSnapshot.WriteKeys())
		require.Zero(t, zeroSnapshot.WriteSize())
		require.Nil(t, (*RUV2Metrics)(nil).CloneWithCommitDetailsForReporting(details))
	})
}
