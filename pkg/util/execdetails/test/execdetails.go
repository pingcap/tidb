...
func defaultRUV2WeightsForTest() execdetails.ExportRUV2Weights {
	cfg := config.DefaultRUV2Config()
	return execdetails.ExportRUV2Weights{
		RUScale:                 cfg.RUScale,
...
func RunString(t *testing.T) {
	detail := &execdetails.ExportExecDetails{
		...
		CopExecDetails: execdetails.ExportCopExecDetails{
...
func RunCopRuntimeStats(t *testing.T) {
	stats := execdetails.ExportNewRuntimeStatsColl(nil)
	tableScanID := 1
	aggID := 2
	tableReaderID := 3
	...
	zeroCopStats := execdetails.ExportCopRuntimeStats{}
	require.Equal(t, "", zeroScanDetail.String())
	require.Equal(t, "", getZeroTimeDetail())
	require.Equal(t, "", zeroCopStats.String())
}

func RunRUV2MetricsSnapshotCalculateRUValues(t *testing.T) {
	weights := defaultRUV2WeightsForTest()
	metrics := execdetails.ExportNewRUV2Metrics()
	...
	t.Run("bypass keeps total zero", func(t *testing.T) {
		...
		total, detail := execdetails.ExportFormatRUV2Summary(bypassed, weights, tikvRU, tiflashRU)
		require.Empty(t, total)
		require.Empty(t, detail)
	})

	t.Run("nil metrics keep tikv and tiflash ru", func(t *testing.T) {
		var nilMetrics *execdetails.ExportRUV2Metrics
		require.Equal(t, tikvRU+tiflashRU, nilMetrics.TotalRU(weights, tikvRU, tiflashRU))
	})
}

func RunRUV2MetricsSnapshotFreezesRUValues(t *testing.T) {
	weights := defaultRUV2WeightsForTest()
	metrics := execdetails.ExportNewRUV2Metrics()
	metrics.AddResultChunkCells(1000)
...
func RunFormatRUV2MetricsIncludesRUValuesFirst(t *testing.T) {
	weights := defaultRUV2WeightsForTest()
	metrics := execdetails.ExportNewRUV2Metrics()
	metrics.AddResultChunkCells(1000)
	metrics.AddResourceManagerWriteCnt(20)
	metrics.AddTiKVCoprocessorWorkTotal("BatchTopN", 10)
	total, formatted := execdetails.ExportFormatRUV2Summary(metrics, weights, 10987, 246)
...
func RunRURuntimeStatsStringIncludesTiFlashRU(t *testing.T) {
...
func RunCopRuntimeStatsForTiFlash(t *testing.T) {
	stats := execdetails.ExportNewRuntimeStatsColl(nil)
	tableScanID := 1
	aggID := 2
	tableReaderID := 3
...
func RunVectorSearchStats(t *testing.T) {
	stats := execdetails.ExportNewRuntimeStatsColl(nil)
...
func RunRuntimeStatsWithCommit(t *testing.T) {
	commitDetail := &util.CommitDetails{
		GetCommitTsTime: time.Second,
		PrewriteTime:    time.Second,
		CommitTime:      time.Second,
...
func RunRootRuntimeStats(t *testing.T) {
	pid := 1
...
func RunFormatDurationForExplain(t *testing.T) {
	cases := []struct {
		t string
		s string
	}{
...
func RunCopRuntimeStats2(t *testing.T) {
	stats := execdetails.ExportNewRuntimeStatsColl(nil)
	tableScanID := 1
	scanDetail := &util.ScanDetail{
		TotalKeys:                 15,
...
func RunRURuntimeStatsStringV1(t *testing.T) {
	stats := &RURuntimeStats{
...
func RunRURuntimeStatsStringV1NilDetails(t *testing.T) {
	stats := &RURuntimeStats{
		Metrics:   newRUV2Metrics(),
		Weights:   defaultRUV2WeightsForTest(),
		RUVersion: rmclient.RUVersionV1,
...
func RunRURuntimeStatsStringV2(t *testing.T) {
	stats := &RURuntimeStats{
		RUDetails: util.NewRUDetails(),
		Metrics:   newRUV2Metrics(),
		Weights:   defaultRUV2WeightsForTest(),
...
func RunRURuntimeStatsStringV2ZeroRU(t *testing.T) {
	stats := &RURuntimeStats{
		Metrics:   newRUV2Metrics(),
		Weights:   defaultRUV2WeightsForTest(),
		RUVersion: rmclient.RUVersionV2,
...
func RunRURuntimeStatsStringDefaultVersion(t *testing.T) {
...
func RunRURuntimeStatsClonePreservesRUVersion(t *testing.T) {
	stats := &RURuntimeStats{
...
func RunRURuntimeStatsCloneNilPreservesZeroVersion(t *testing.T) {
	var stats *RURuntimeStats
	cloned := stats.Clone().(*RURuntimeStats)
	require.Equal(t, rmclient.RUVersion(0), cloned.RUVersion)
}

func RunRURuntimeStatsMergeRUVersion(t *testing.T) {
...
func RunRURuntimeStatsMergeKeepsExistingRUVersion(t *testing.T) {
