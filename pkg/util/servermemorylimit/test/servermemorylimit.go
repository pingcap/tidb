...
func RunMemoryUsageOpsHistory(t *testing.T) {
	...
	for i := range 3 {
		genInfo(i)
		servermemorylimit.servermemorylimit.GlobalMemoryOpsHistoryManager.recordOne(&info, time.Now(), uint64(i), uint64(2*i))
	}
	...
	rows := servermemorylimit.GlobalMemoryOpsHistoryManager.GetRows()
	...
	for i := 3; i < 53; i++ {
		genInfo(i)
		servermemorylimit.GlobalMemoryOpsHistoryManager.recordOne(&info, time.Now(), uint64(i), uint64(2*i))
	}
	rows = servermemorylimit.GlobalMemoryOpsHistoryManager.GetRows()
	...
	require.Equal(t, servermemorylimit.GlobalMemoryOpsHistoryManager.offsets, 3)
}
