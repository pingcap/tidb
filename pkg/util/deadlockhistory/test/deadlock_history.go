...
func getAllDatum(d *deadlockhistory.ExportedDeadlockHistory, columns []*model.ColumnInfo) [][]types.Datum {
	records := d.GetAll()
	rowsCount := 0
...
func RunDeadlockHistoryCollection(t *testing.T) {
	h := deadlockhistory.ExportedNewDeadlockHistory(1)
	assert.Equal(t, 0, len(h.GetAll()))
	assert.Equal(t, 0, deadlockhistory.ExportedGetHead(h))
	assert.Equal(t, 0, deadlockhistory.ExportedGetSize(h))

	rec1 := &deadlockhistory.ExportedDeadlockRecord{
		OccurTime: time.Now(),
	}
	h.Push(rec1)
	res := h.GetAll()
	...
	assert.Equal(t, 0, deadlockhistory.ExportedGetHead(h))
	assert.Equal(t, 1, deadlockhistory.ExportedGetSize(h))

	rec2 := &deadlockhistory.ExportedDeadlockRecord{
		OccurTime: time.Now(),
	}
	h.Push(rec2)
	res = h.GetAll()
	...
	h = deadlockhistory.ExportedNewDeadlockHistory(3)
	rec1 = &deadlockhistory.ExportedDeadlockRecord{
		OccurTime: time.Now(),
	}
	h.Push(rec1)
	...
	assert.Equal(t, 0, deadlockhistory.ExportedGetHead(h))
	assert.Equal(t, 2, deadlockhistory.ExportedGetSize(h))

	rec3 := &deadlockhistory.ExportedDeadlockRecord{
		OccurTime: time.Now(),
	}
	h.Push(rec3)
	res = h.GetAll()
	...
	assert.Equal(t, 0, deadlockhistory.ExportedGetHead(h))
	assert.Equal(t, 3, deadlockhistory.ExportedGetSize(h))

	// Continuously pushing items to check the correctness of the deque
	expectedItems := []*deadlockhistory.ExportedDeadlockRecord{rec1, rec2, rec3}
	expectedIDs := []uint64{1, 2, 3}
	expectedDequeHead := 0
	for range 6 {
		newRec := &deadlockhistory.ExportedDeadlockRecord{
			OccurTime: time.Now(),
		}
		h.Push(newRec)
		...
		assert.Equal(t, expectedDequeHead, deadlockhistory.ExportedGetHead(h))
		assert.Equal(t, 3, deadlockhistory.ExportedGetSize(h))
	}
...
func RunGetDatum(t *testing.T) {
	time1 := time.Date(2021, 05, 14, 15, 28, 30, 123456000, time.UTC)
	time2 := time.Date(2022, 06, 15, 16, 29, 31, 123457000, time.UTC)

	h := deadlockhistory.ExportedNewDeadlockHistory(10)
	h.Push(&deadlockhistory.ExportedDeadlockRecord{
		OccurTime:   time1,
		IsRetryable: false,
		WaitChain: []deadlockhistory.ExportedWaitChainItem{
			{
				TryLockTxn:     101,
				SQLDigest:      "sql1",
				Key:            []byte("k1"),
	...
	dummyColumnInfo := []*model.ColumnInfo{
		{Name: ast.NewCIStr(deadlockhistory.ColDeadlockIDStr)},
		{Name: ast.NewCIStr(deadlockhistory.ColOccurTimeStr)},
		{Name: ast.NewCIStr(deadlockhistory.ColRetryableStr)},
		{Name: ast.NewCIStr(deadlockhistory.ColTryLockTrxIDStr)},
		{Name: ast.NewCIStr(deadlockhistory.ColCurrentSQLDigestStr)},
		{Name: ast.NewCIStr(deadlockhistory.ColCurrentSQLDigestTextStr)},
		{Name: ast.NewCIStr(deadlockhistory.ColKeyStr)},
		{Name: ast.NewCIStr(deadlockhistory.ColKeyInfoStr)},
		{Name: ast.NewCIStr(deadlockhistory.ColTrxHoldingLockStr)},
	}
...
func RunErrDeadlockToDeadlockRecord(t *testing.T) {
	...
	expectedRecord := &deadlockhistory.ExportedDeadlockRecord{
		IsRetryable: true,
		WaitChain: []deadlockhistory.ExportedWaitChainItem{
			{
				TryLockTxn:     100,
	...
	record := deadlockhistory.ExportedErrDeadlockToDeadlockRecord(err)
...
func dummyRecord() *deadlockhistory.ExportedDeadlockRecord {
	return &deadlockhistory.ExportedDeadlockRecord{}
}

func RunResize(t *testing.T) {
	h := deadlockhistory.ExportedNewDeadlockHistory(2)
	h.Push(dummyRecord()) // id=1 inserted
	h.Push(dummyRecord()) // id=2 inserted,
	h.Push(dummyRecord()) // id=3 inserted, id=1 is removed
	assert.Equal(t, 1, deadlockhistory.ExportedGetHead(h))
	assert.Equal(t, 2, deadlockhistory.ExportedGetSize(h))
