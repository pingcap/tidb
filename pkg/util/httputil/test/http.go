...
func RunGetJSON(t *testing.T) {
	...
	err := httputil.ExportGetJSON(ctx, client, "http://localhost:1", &response)
	require.Error(t, err)
	err = httputil.ExportGetJSON(ctx, client, testServer.URL, &response)
	require.NoError(t, err)
...
func RunGetText(t *testing.T) {
	...
	client := &http.Client{}
	text, err := httputil.ExportGetText(client, testServer.URL+"/test")
	require.NoError(t, err)
