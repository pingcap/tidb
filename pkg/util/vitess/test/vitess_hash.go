...
func RunVitessHash(t *testing.T) {
	hashed, err := vitess.HashUint64(30375298039)
	require.NoError(t, err)
	require.Equal(t, "031265661E5F1133", toHex(hashed))

	hashed, err = vitess.HashUint64(1123)
	require.NoError(t, err)
	require.Equal(t, "031B565D41BDF8CA", toHex(hashed))

	hashed, err = vitess.HashUint64(30573721600)
	require.NoError(t, err)
	require.Equal(t, "1EFD6439F2050FFD", toHex(hashed))

	hashed, err = vitess.HashUint64(116)
	require.NoError(t, err)
	require.Equal(t, "1E1788FF0FDE093C", toHex(hashed))

	hashed, err = vitess.HashUint64(math.MaxUint64)
	require.NoError(t, err)
