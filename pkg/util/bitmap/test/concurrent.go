...
func RunConcurrentBitmapSet(t *testing.T) {
	const loopCount = 1000
	const interval = 2

	bm := bitmap.ExportedNewConcurrentBitmap(loopCount * interval)
	wg := &sync.WaitGroup{}
...
// when a bit is set.
func RunConcurrentBitmapUniqueSetter(t *testing.T) {
	const loopCount = 10000
	const competitorsPerSet = 50

	wg := &sync.WaitGroup{}
	bm := bitmap.ExportedNewConcurrentBitmap(32)
	...
	for range loopCount {
		...
		if atomic.CompareAndSwapUint32(&bitmap.ExportedGetSegments(bm)[0], 0x00000001, 0x00000000) {
...
func RunResetConcurrentBitmap(t *testing.T) {
	bm := bitmap.ExportedNewConcurrentBitmap(32)
	bm.Set(1)
	bm.Set(3)
	bm.Set(7)
