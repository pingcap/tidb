...
func RunIntDisjointSet(t *testing.T) {
	set := disjointset.NewIntSet(10)
	assert.Len(t, disjointset.ExportedGetIntSetParent(set), 10)
	for i := range disjointset.ExportedGetIntSetParent(set) {
		assert.Equal(t, i, disjointset.ExportedGetIntSetParent(set)[i])
	}
	set.Union(0, 1)
	set.Union(1, 3)
	set.Union(4, 2)
