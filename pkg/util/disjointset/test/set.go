...
func RunDisjointSet(t *testing.T) {
	set := disjointset.NewSet[string](10)
	assert.False(t, set.InSameGroup("a", "b"))
	assert.Len(t, disjointset.ExportedGetSetParent(set), 2)
	set.Union("a", "b")
	assert.True(t, set.InSameGroup("a", "b"))
	assert.False(t, set.InSameGroup("a", "c"))
	assert.Len(t, disjointset.ExportedGetSetParent(set), 3)
	...
	set.Union("e", "f")
	set.Union("f", "g")
	assert.Len(t, disjointset.ExportedGetSetParent(set), 7)
