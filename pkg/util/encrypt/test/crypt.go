...
func RunSQLDecode(t *testing.T) {
	...
	for _, tt := range tests {
		crypted, err := encrypt.SQLDecode(tt.str, tt.passwd)
		if tt.isError {
...
func RunSQLEncode(t *testing.T) {
	...
	for _, tt := range tests {
		...
		uncrypte, err := encrypt.SQLEncode(crypted, tt.passwd)
		if tt.isError {
			require.Errorf(t, err, "%v", tt)
			continue
		}
