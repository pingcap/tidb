...
func RunReadAt(t *testing.T) {
	ctrCipher1, err := encrypt.NewCtrCipher()
	require.NoError(t, err)
	ctrCipher2, err := encrypt.NewCtrCipher()
	require.NoError(t, err)

	readAtTestCases := []readAtTestCase{
		{
			newWriter: func(f *os.File) io.WriteCloser { return encrypt.NewWriter(f, ctrCipher1) },
			newReader: func(f *os.File) io.ReaderAt { return encrypt.NewReader(f, ctrCipher1) },
		},
		{
			newWriter: func(f *os.File) io.WriteCloser { return checksum.NewWriter(encrypt.NewWriter(f, ctrCipher1)) },
			newReader: func(f *os.File) io.ReaderAt { return checksum.NewReader(encrypt.NewReader(f, ctrCipher1)) },
		},
		{
			newWriter: func(f *os.File) io.WriteCloser { return encrypt.NewWriter(checksum.NewWriter(f), ctrCipher1) },
			newReader: func(f *os.File) io.ReaderAt { return encrypt.NewReader(checksum.NewReader(f), ctrCipher1) },
		},
		{
			newWriter: func(f *os.File) io.WriteCloser {
				return encrypt.NewWriter(encrypt.NewWriter(f, ctrCipher1), ctrCipher2)
			},
			newReader: func(f *os.File) io.ReaderAt { return encrypt.NewReader(encrypt.NewReader(f, ctrCipher1), ctrCipher2) },
		},
	}
...
func BenchmarkReadAt(b *testing.B) {
	ctrCipher, err := encrypt.NewCtrCipher()
	if err != nil {
		b.Fatal(err)
	}
	...
	readAtTestCases := []readAtTestCase{
		...
		},
		{
			...
			newWriter: func(f *os.File) io.WriteCloser {
				return checksum.NewWriter(encrypt.NewWriter(f, ctrCipher))
			},
			newReader: func(f *os.File) io.ReaderAt {
				return checksum.NewReader(encrypt.NewReader(f, ctrCipher))
			},
		},
	}
