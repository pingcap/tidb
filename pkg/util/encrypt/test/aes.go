...
func RunPad(t *testing.T) {
	p := []byte{0x0A, 0x0B, 0x0C, 0x0D}
	p, err := encrypt.PKCS7Pad(p, 8)
...
func RunAESECB(t *testing.T) {
	...
	for _, tt := range ecbAESTests {
		...
		encrypter := encrypt.ExportedNewECBEncrypter(cipher)
		d := make([]byte, len(tt.in))
		encrypter.CryptBlocks(d, tt.in)
		require.Equalf(t, toHex(tt.out), toHex(d), "%s: ECBEncrypter\nhave %x\nwant %x", test, d, tt.out)

		decrypter := encrypt.ExportedNewECBDecrypter(cipher)
		p := make([]byte, len(d))
...
func RunAESEncryptWithECB(t *testing.T) {
	tests := []struct {
		str     string
		key     string
		expect  string
...
func RunAESDecryptWithECB(t *testing.T) {
	tests := []struct {
		expect      string
		key         string
		hexCryptStr string
	...
	for _, tt := range tests {
		...
		result, err := encrypt.AESDecryptWithECB(cryptStr, key)
		if tt.isError {
			require.Error(t, err)
			continue
		}
...
func RunAESEncryptWithCBC(t *testing.T) {
	tests := []struct {
		str     string
		key     string
		iv      string
	...
	for _, tt := range tests {
		...
		crypted, err := encrypt.AESEncryptWithCBC(str, key, iv)
		if tt.isError {
			require.Errorf(t, err, "%v", tt)
			continue
		}
...
func RunAESEncryptWithOFB(t *testing.T) {
	tests := []struct {
		str     string
		key     string
		iv      string
	...
	for _, tt := range tests {
		...
		crypted, err := encrypt.AESEncryptWithOFB(str, key, iv)
		if tt.isError {
			require.Errorf(t, err, "%v", tt)
			continue
		}
...
func RunAESDecryptWithOFB(t *testing.T) {
	tests := []struct {
		str     string
		key     string
		iv      string
...
func RunAESEncryptWithCTR(t *testing.T) {
	tests := []struct {
		str     string
		key     string
		iv      string
	...
	for _, tt := range tests {
		...
		crypted, err := encrypt.AESEncryptWithCTR(str, key, iv)
		if tt.isError {
			require.Errorf(t, err, "%v", tt)
			continue
		}
...
func RunAESDecryptWithCTR(t *testing.T) {
	tests := []struct {
		str     string
		key     string
		iv      string
	...
	for _, tt := range tests {
		...
		plainText, err := encrypt.AESDecryptWithCTR(str, key, iv)
		if tt.isError {
			require.Errorf(t, err, "%v", tt)
			continue
		}
...
func RunAESDecryptWithCBC(t *testing.T) {
	tests := []struct {
		expect      string
		key         string
		iv          string
	...
	for _, tt := range tests {
		...
		result, err := encrypt.AESDecryptWithCBC(cryptStr, key, iv)
		if tt.isError {
			require.Error(t, err)
			continue
		}
...
func RunAESEncryptWithCFB(t *testing.T) {
	tests := []struct {
		str     string
		key     string
		iv      string
	...
	for _, tt := range tests {
		...
		crypted, err := encrypt.AESEncryptWithCFB(str, key, iv)
		if tt.isError {
			require.Errorf(t, err, "%v", tt)
			continue
		}
...
func RunAESDecryptWithCFB(t *testing.T) {
	tests := []struct {
		str     string
		key     string
		iv      string
...
func RunDeriveKeyMySQL(t *testing.T) {
	p := []byte("MySQL=insecure! MySQL=insecure! ")
	p = encrypt.DeriveKeyMySQL(p, 16)
