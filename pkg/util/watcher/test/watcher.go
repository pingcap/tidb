...
import (
	...
	"github.com/pingcap/tidb/pkg/util/watcher"
	"github.com/stretchr/testify/require"
)

func RunWatcher(t *testing.T) {
	var (
	...
	// start watcher
	err = ww.Start(10 * time.Millisecond)
	require.NoError(t, err)
	defer ww.Close()
	...
	go func() {
		defer wg.Done()
		assertEvent(w, oldFilePath, watcher.Create, t)
	}()
	wg.Wait()
	...
	go func() {
		defer wg.Done()
		assertEvent(w, oldFilePath, watcher.Modify, t)
	}()
	...
	go func() {
		defer wg.Done()
		assertEvent(w, oldFilePath, watcher.Chmod, t)
	}()
	...
	go func() {
		defer wg.Done()
		assertEvent(w, oldFilePath, watcher.Rename, t)
	}()
	...
	go func() {
		defer wg.Done()
		assertEvent(w, newFilePath, watcher.Remove, t)
	}()
	...
	go func() {
		defer wg.Done()
		assertEvent(w, oldFilePath, watcher.Move, t)
	}()
...
func assertEvent(w *watcher.Watcher, path string, op Op, t *testing.T) {
	for {
		select {
		case ev := <-w.Events:
			if ev.IsDirEvent() {
