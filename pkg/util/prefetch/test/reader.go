...
func RunBasic(t *testing.T) {
	...
	r := prefetch.NewReader(io.NopCloser(source), source.Size(), 3)
	buf := make([]byte, 1)
	n, err := r.Read(buf)
	...
	r = prefetch.NewReader(io.NopCloser(source), source.Size(), 100)
	buf = make([]byte, 11)
	n, err = r.Read(buf)
	require.NoError(t, err)
...
func RunConvertUnexpectedEOF(t *testing.T) {
	r := prefetch.NewReader(io.NopCloser(&unexpectedEOFReader{}), 10, 10)
	buf := make([]byte, 10)
	_, err := r.Read(buf)
...
func RunCloseBeforeDrainRead(t *testing.T) {
	data := make([]byte, 1024)
	source := bytes.NewReader(data)
	r := prefetch.NewReader(io.NopCloser(source), source.Size(), 2)
	err := r.Close()
	require.NoError(t, err)
}
...
func RunFillPrefetchBuffer(t *testing.T) {
	...
	prefetchReader := prefetch.NewReader(io.NopCloser(fragReader), int64(len(fragReader.data)), 10)
