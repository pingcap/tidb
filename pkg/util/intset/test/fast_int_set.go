...
func RunFastIntSetBasic(t *testing.T) {
	// Test Insert, Remove, Len, Has.
	fis := intset.FastIntSet{}
	fis.Insert(1)
	fis.Insert(2)
	fis.Insert(3)
...
func RunFastIntSet(t *testing.T) {
	for _, mVal := range []int{1, 8, 30, intset.ExportedSmallCutOff, 2 * intset.ExportedSmallCutOff, 4 * intset.ExportedSmallCutOff} {
		m := mVal
...
func RunFastIntSetTwoSetOps(t *testing.T) {
	rng, _ := NewTestRand()
	...
	for _, minVal := range []int{-10, -1, 0, intset.ExportedSmallCutOff, 2 * intset.ExportedSmallCutOff} {
...
func RunFastIntSetAddRange(t *testing.T) {
...
func RunGetSmallUInt64(t *testing.T) {
	// Empty set returns 0.
	var s intset.FastIntSet
	...
	s = intset.NewFastIntSet(0, 1, 3)
	v, err = s.GetSmallUInt64()
	require.NoError(t, err)
	require.Equal(t, uint64(0b1011), v)

	// All small values up to bit 5 → 0b111111 = 63.
	s = intset.NewFastIntSet(0, 1, 2, 3, 4, 5)
	v, err = s.GetSmallUInt64()
	require.NoError(t, err)
	require.Equal(t, uint64(63), v)

	// A set with a value >= intset.ExportedSmallCutOff (64) triggers an error.
	s = intset.NewFastIntSet(64)
	_, err = s.GetSmallUInt64()
	require.Error(t, err)

	// Mixed: small + large value also triggers an error.
	s = intset.NewFastIntSet(1, 64)
	_, err = s.GetSmallUInt64()
	require.Error(t, err)
}

func RunFastIntSetString(t *testing.T) {
	testCases := []struct {
		vals []int
		exp  string
	}{
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			s := intset.NewFastIntSet(tc.vals...)
