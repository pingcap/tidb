...
func (e *SQLBindExec) dropSQLBindByDigest() error {
	...
	affectedRows, err := domain.GetDomain(e.Ctx()).BindingHandle().DropGlobalBinding(sqlDigests)
	e.Ctx().GetSessionVars().StmtCtx.AddAffectedRows(affectedRows)
	return err
}
...
func (e *SQLBindExec) setBindingStatus() error {
	...
	ok, err := domain.GetDomain(e.Ctx()).BindingHandle().SetGlobalBindingStatus(e.details[0].NewStatus, sqlDigest.String())
	if err == nil && !ok {
...
func (e *SQLBindExec) setBindingStatusByDigest() error {
	...
	ok, err := domain.GetDomain(e.Ctx()).BindingHandle().SetGlobalBindingStatus(
...
func (e *SQLBindExec) createSQLBind() error {
	...
	return domain.GetDomain(e.Ctx()).BindingHandle().CreateGlobalBinding(e.Ctx(), bindings)
}
