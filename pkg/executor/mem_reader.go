...
func (m *memIndexLookUpReader) getMemRowsIter(ctx context.Context) (memRowsIter, error) {
	...
	for i, ranges := range kvRanges {
		...
		ranges, _ := distsql.TableHandlesToKVRanges(physicalTableID, handles, nil)
...
func (m *memIndexMergeReader) getMemRows(ctx context.Context) ([][]types.Datum, error) {
	...
	if m.partitionMode {
		...
		tblKVRanges, _ = distsql.TableHandlesToKVRanges(0, handles, nil)
	} else {
		tblKVRanges, _ = distsql.TableHandlesToKVRanges(getPhysicalTableID(m.table), handles, nil)
	}
