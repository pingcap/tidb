...
func TestMergePartialResult4Sum(t *testing.T) {
	...
	tests = append(tests, buildAggTesterWithFieldType(ast.AggFuncSumInt, unsignedType, nil, 5, uint64(10), uint64(9), uint64(19)))
...
func TestSum(t *testing.T) {
	tests := []aggTest{
		...
		buildAggTester(ast.AggFuncSumInt, mysql.TypeLonglong, 0, 5, nil, 10),
	}
	unsignedType := types.NewFieldType(mysql.TypeLonglong)
	unsignedType.AddFlag(mysql.UnsignedFlag)
	tests = append(tests, buildAggTesterWithFieldType(ast.AggFuncSumInt, unsignedType, nil, 5, nil, uint64(10)))
	for i, test := range tests {
		t.Run(fmt.Sprintf("%s_%d", test.funcName, i), func(t *testing.T) {
			testAggFunc(t, test)
		})
...
func TestMemSum(t *testing.T) {
	tests := []aggMemTest{
		...
		buildAggMemTester(ast.AggFuncSumInt, mysql.TypeLonglong, 0, 5,
