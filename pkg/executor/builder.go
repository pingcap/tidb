...
func (b *executorBuilder) buildTopN(v *plannercore.PhysicalTopN) exec.Executor {
	...
	if columnIdxsUsedByChild != nil && columnMissing {
		...
		// b.err = errors.Annotate(ErrBuildExecutor, "Inline projection occurs when `buildTopN` executor, columns should not missing in the child schema")
		// return nil
	}
...
func (b *executorBuilder) buildBatchPointGet(plan *plannercore.BatchPointGetPlan) exec.Executor {
	...
	e := &BatchPointGetExec{
		...
		indexUsageReporter: b.buildIndexUsageReporter(plan, false),
