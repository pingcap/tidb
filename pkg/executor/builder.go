...
func buildIndexReq(ctx sessionctx.Context, handleLen int, plans []base.PhysicalPlan) (dagReq *tipb.DAGRequest, err error) {
	indexReq, err := builder.ConstructDAGReq(ctx, plans, kv.TiKV)
	if err != nil {
		return nil, err
	}

	outputOffsets, err := buildIndexScanOutputOffsets(plans[0], handleLen)
	if err != nil {
		return nil, err
	}
...
func buildIndexLookUpPushDownReq(ctx sessionctx.Context, handleLen int, plans []base.PhysicalPlan, planUnNatureOrders map[int]int) (dagReq *tipb.DAGRequest, err error) {
	indexReq, err := builder.ConstructDAGReqForUnNatureOrderPlans(ctx, plans, planUnNatureOrders, kv.TiKV)
	if err != nil {
		return nil, err
	}

	intermediateOutputOffsets, err := buildIndexScanOutputOffsets(plans[0], handleLen)
	if err != nil {
		return nil, err
	}
...
func buildIndexScanOutputOffsets(idxScanPlan base.PhysicalPlan, handleLen int) ([]uint32, error) {
	p, ok := idxScanPlan.(*physicalop.PhysicalIndexScan)
	if !ok {
		return nil, errors.Errorf("Not the type PhysicalIndexScan: %T", idxScanPlan)
	}
	...
	for i := range handleLen {
		outputOffsets = append(outputOffsets, uint32(len(p.Index.Columns)+i))
	}

	if needExtraOutputCol {
		// need add one more column for pid or physical table id
		outputOffsets = append(outputOffsets, uint32(len(p.Index.Columns)+handleLen))
	}
...
func buildNoRangeIndexLookUpReader(b *executorBuilder, v *physicalop.PhysicalIndexLookUpReader) (*IndexLookUpExecutor, error) {
	...
	if v.IndexLookUpPushDown {
		indexReq, err = buildIndexLookUpPushDownReq(b.ctx, handleLen, v.IndexPlans, v.IndexPlansUnNatureOrders)
	} else {
		indexReq, err = buildIndexReq(b.ctx, handleLen, v.IndexPlans)
	}
	if err != nil {
		return nil, err
	}
	...
	if v.IndexLookUpPushDown {
		// currently, we do not support paging when index lookup is pushed down
		e.dctx.EnablePaging = false
	}
...
func buildNoRangeIndexMergeReader(b *executorBuilder, v *physicalop.PhysicalIndexMergeReader) (*IndexMergeReaderExecutor, error) {
	...
	for i := range partialPlanCount {
		...
		if is, ok := v.PartialPlans[i][0].(*physicalop.PhysicalIndexScan); ok {
			tempReq, err = buildIndexReq(b.ctx, ts.HandleCols.NumCols(), v.PartialPlans[i])
