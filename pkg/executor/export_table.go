...
// pkg/dxf/export can't import pkg/executor directly for SHOW CREATE
// TABLE/DATABASE rendering (this file imports pkg/dxf/export, so the reverse
// would cycle), so it exposes function vars that pkg/executor fills in here.
func init() {
	export.ShowCreateTableFunc = ConstructResultOfShowCreateTable
	export.ShowCreateDatabaseFunc = ConstructResultOfShowCreateDatabase
}
...
type exportOptions struct {
	fileSize int64
	// thread is the task's submit-time RequiredSlots estimate. 0 means auto:
	// min(8, executor node CPU).
	thread   int
	detached bool
}

// parseExportOptions parses the WITH options shared by EXPORT TABLE and
// EXPORT SCHEMA.
func parseExportOptions(userSctx sessionctx.Context, rawOpts []*plannercore.LoadDataOpt) (*exportOptions, error) {
	opts := &exportOptions{
		fileSize: defaultExportFileSize,
	}
	evalCtx := userSctx.GetExprCtx().GetEvalCtx()
	for _, opt := range rawOpts {
		...
		case "detached":
			opts.detached = true
		default:
			return nil, errors.Errorf("unknown EXPORT option %s", opt.Name)
		}
	}
	return opts, nil
}

// defaultThread returns opts.thread, or an auto default (min(8, executor
// node CPU)) when the user didn't set one: task creation rejects
// RequiredSlots larger than the executor node CPU, so the default must be
// clamped by it.
func defaultThread(ctx context.Context, opts *exportOptions) int {
	if opts.thread != 0 {
		return opts.thread
	}
	thread := 8
	if cpu, err := handle.GetCPUCountOfNode(ctx); err == nil && cpu > 0 {
		thread = min(thread, cpu)
	}
	return thread
}
