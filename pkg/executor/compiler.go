...
func (c *Compiler) Compile(ctx context.Context, stmtNode ast.StmtNode) (_ *ExecStmt, err error) {
	...
	if preparedObj != nil {
		CountStmtNode(ctx, preparedObj.PreparedAst.Stmt, sessVars.InRestrictedSQL, stmtCtx.ResourceGroupName)
	} else {
		CountStmtNode(ctx, stmtNode, sessVars.InRestrictedSQL, stmtCtx.ResourceGroupName)
	}
...
func CountStmtNode(ctx context.Context, stmtNode ast.StmtNode, inRestrictedSQL bool, resourceGroup string) {
	if inRestrictedSQL {
		return
	}
