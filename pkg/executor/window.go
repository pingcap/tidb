...
func (p *aggWindowProcessor) consumeGroupRows(ctx sessionctx.Context, rows []chunk.Row) ([]chunk.Row, error) {
	for i, windowFunc := range p.windowFuncs {
		...
		_, err := windowFunc.UpdatePartialResult(ctx.GetExprCtx(), rows, p.partialResults[i])
		if err != nil {
			return nil, err
		}
	}
...
func (p *aggWindowProcessor) appendResult2Chunk(ctx sessionctx.Context, rows []chunk.Row, chk *chunk.Chunk, remained int) ([]chunk.Row, error) {
	for remained > 0 {
		for i, windowFunc := range p.windowFuncs {
			...
			err := windowFunc.AppendFinalResult2Chunk(ctx.GetExprCtx(), p.partialResults[i], chk)
			if err != nil {
				return nil, err
			}
		}
	...
	for ; remained > 0; lastStart, lastEnd = start, end {
		...
		if start >= end {
			for i, windowFunc := range p.windowFuncs {
				...
				if slidingWindowAggFunc != nil && initializedSlidingWindow {
					err = slidingWindowAggFunc.Slide(ctx.GetExprCtx(), func(u uint64) chunk.Row {
						return rows[u]
		...
		for i, windowFunc := range p.windowFuncs {
			...
			if slidingWindowAggFunc != nil && initializedSlidingWindow {
				...
				_, err = windowFunc.UpdatePartialResult(ctx.GetExprCtx(), rows[start:end], p.partialResults[i])
			}
			if err != nil {
				return nil, err
			}
...
func (p *rangeFrameWindowProcessor) getStartOffset(ctx sessionctx.Context, rows []chunk.Row) (uint64, error) {
	...
	for ; p.lastStartOffset < numRows; p.lastStartOffset++ {
		...
		for i := range p.orderByCols {
			res, _, err = p.start.CmpFuncs[i](ctx.GetExprCtx(), p.start.CompareCols[i], p.start.CalcFuncs[i], rows[p.lastStartOffset], rows[p.curRowIdx])
			if err != nil {
				return 0, err
			}
			if res != 0 {
...
func (p *rangeFrameWindowProcessor) getEndOffset(ctx sessionctx.Context, rows []chunk.Row) (uint64, error) {
	...
	for ; p.lastEndOffset < numRows; p.lastEndOffset++ {
		...
		var err error
		for i := range p.orderByCols {
			res, _, err = p.end.CmpFuncs[i](ctx.GetExprCtx(), p.end.CalcFuncs[i], p.end.CompareCols[i], rows[p.curRowIdx], rows[p.lastEndOffset])
			if err != nil {
				return 0, err
			}
			if res != 0 {
