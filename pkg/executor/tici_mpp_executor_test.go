...
func buildMockIndexScanForTiCITests(ctx sessionctx.Context, isTiCI bool, ranges []*ranger.Range, schemaCols ...*expression.Column) *plannercore.PhysicalIndexScan {
	planCtx := ctx.GetPlanCtx()
	index := &model.IndexInfo{ID: 100, Name: pmodel.NewCIStr("idx")}
	if isTiCI {
		index.FullTextInfo = &model.FullTextIndexInfo{}
	}
	scan := (&plannercore.PhysicalIndexScan{
		Table:  &model.TableInfo{ID: 42, Name: pmodel.NewCIStr("t")},
		Index:  index,
		Ranges: ranges,
	}).Init(planCtx, 0)
	if len(schemaCols) == 0 {
...
func TestCloneMPPIndexSenderForTiCIRewritesChildScan(t *testing.T) {
	...
	clonedIndexScan, ok := cloned.Children()[0].(*plannercore.PhysicalIndexScan)
	require.True(t, ok)
	isPartition, clonedID := clonedIndexScan.IsPartition()
	require.True(t, isPartition)
	require.Equal(t, physicalTableID, clonedID)
	...
	isPartition, _ = indexScan.IsPartition()
...
func TestUseMPPExecutionTreatsTiCIIndexAsTiCIPath(t *testing.T) {
	ctx := defaultCtx()
	planCtx := ctx.GetPlanCtx()
	require.NoError(t, ctx.GetSessionVars().SetSystemVar(variable.TiDBAllowMPPExecution, variable.Off))
	...
	ticiSender := (plannercore.PhysicalExchangeSender{ExchangeType: tipb.ExchangeType_PassThrough}).Init(planCtx, &property.StatsInfo{RowCount: 1})
	ticiSender.SetChildren(ticiScan)
	ticiReader := &plannercore.PhysicalTableReader{
		TablePlans: []base.PhysicalPlan{ticiScan},
	}
	ticiReader.SetTablePlanForTest(ticiSender)
	require.True(t, useMPPExecution(ctx, ticiReader))

	normalScan := buildMockIndexScanForTiCITests(ctx, false, nil, schemaCol)
	normalSender := (plannercore.PhysicalExchangeSender{ExchangeType: tipb.ExchangeType_PassThrough}).Init(planCtx, &property.StatsInfo{RowCount: 1})
	normalSender.SetChildren(normalScan)
	normalReader := &plannercore.PhysicalTableReader{
		TablePlans: []base.PhysicalPlan{normalScan},
	}
	normalReader.SetTablePlanForTest(normalSender)
