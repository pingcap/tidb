...
func parseSlowLog(sctx sessionctx.Context, reader *bufio.Reader) ([][]types.Datum, error) {
	...
	// Initialize only the column factories so parsing stops at the reader's EOF
	// instead of continuing into slow log files on disk.
	...
	return parseLog(retriever, sctx, reader)
}
...
select * from t;`
	...
	expectRecordString := `2019-04-28 15:24:04.309074,` +
		...
		`,update t set i = 1;,null,select * from t;,,,[regionMiss rpcPD rpcTiKV]`
...
select 7;`
	...
	for k := range 2 {
		...
		for i, cas := range cases {
			...
			// The Query column is not necessarily the last output column, so
			// locate it by name before comparing row values.
			queryColumnIdx := -1
			for idx, col := range retriever.outputCols {
				if col.Name.O == variable.SlowLogQuerySQLStr {
					queryColumnIdx = idx
					break
				}
			}
			require.NotEqual(t, -1, queryColumnIdx, comment)
			if len(retriever.files) > 0 {
				...
				for i, row := range rows {
					require.Equal(t, row[queryColumnIdx].GetString(), cas.querys[i], comment)
				}
			}
