...
func RunColumnsBasic(t *testing.T) {
	notNullIntTp := types.NewFieldType(mysql.TypeLonglong)
...
func RunColumnsAllDataTypes(t *testing.T) {
	tinyTp := types.NewFieldType(mysql.TypeTiny)
...
func RunBalanceOfFilteredRows(t *testing.T) {
	intTp := types.NewFieldType(mysql.TypeLonglong)
	stringTp := types.NewFieldType(mysql.TypeVarString)
	binaryStringTp := types.NewFieldType(mysql.TypeBlob)
	buildKeyIndex := []int{0}
...
func RunUnalignmentLoad(t *testing.T) {
...
func RunSetupPartitionInfo(t *testing.T) {
