...
func RunShuffleMergeJoinInDisk(t *testing.T) {
...
func RunMergeJoinInDisk(t *testing.T) {
...
func RunVectorizedMergeJoin(t *testing.T) {
	...
	runTest := func(ts1, ts2 []int) {
		getTable := func(prefix string, ts []int) string {
			...
			for i, n := range ts {
				...
				fmt.Fprintf(&buf, "insert into %v values ", tableName)
				for j := range n {
					if j > 0 {
						buf.WriteString(", ")
					}
					fmt.Fprintf(&buf, "(%v, %v)", i, rand.Intn(10))
				}
...
//
//nolint:gosimple // generates false positive fmt.Sprintf warnings which keep aligned
func RunVectorizedShuffleMergeJoin(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
