...
func (j *leftOuterJoinProbe) buildResultForMatchedRowsAfterOtherCondition(chk, joinedChk *chunk.Chunk) {
	j.baseOuterJoinProbe.buildResultForMatchedRowsAfterOtherCondition(chk, joinedChk, 0, j.lUsed, 0, len(j.lUsed), j.rUsed, j.currentChunk.NumCols())
}

func (j *leftOuterJoinProbe) buildResultForNotMatchedRows(chk *chunk.Chunk, startProbeRow int) {
	j.baseOuterJoinProbe.buildResultForNotMatchedRows(chk, startProbeRow, 0, j.lUsed, len(j.lUsed), j.rUsed)
}
