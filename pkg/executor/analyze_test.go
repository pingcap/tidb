...
func TestAnalyzeBuildsBatchableRequests(t *testing.T) {
	...
	testfailpoint.EnableCall(t, "github.com/pingcap/tidb/pkg/executor/analyzeColumnsRequestBuilt", func(req *kv.Request) {
		snapshot := *req // BuildCopIterator clears StoreBatchSize after building the initial tasks.
	...
	// Full-sampling Analyze opts in to merged child responses.
	...
	require.True(t, ok)
	const locateRegionMaxBackoff = 1000
	assertAnalyzeRequest := func(regionCount, scanConcurrency, wantConcurrency, wantBatchSize int) {
		...
		table, err := domain.GetDomain(tk.Session()).InfoSchema().TableByName(ctx, ast.NewCIStr("test"), ast.NewCIStr("tu"))
		require.NoError(t, err)
		tableStart := tablecodec.GenTableRecordPrefix(table.Meta().ID)
		if kerneltype.IsNextGen() {
			tableStart = store.GetCodec().EncodeKey(tableStart)
		}
		location, err := tikvStore.GetRegionCache().LocateKey(tikv.NewBackoffer(ctx, locateRegionMaxBackoff), tableStart)
		require.NoError(t, err)
		cluster.SplitKeys(tableStart, tableStart.PrefixNext(), regionCount)
		// SplitKeys bypasses RegionCache, so invalidate its pre-split entry.
		...
		tk.MustExec("analyze table tu with 1 samplerate, 0 topn, 2 buckets")
		require.Equalf(t, before+1, requestCount.Load(), "N=%d C=%d", regionCount, scanConcurrency)
		request := lastRequest.Load()
		require.Equalf(t, wantConcurrency, request.Concurrency, "N=%d C=%d", regionCount, scanConcurrency)
		require.Equalf(t, wantBatchSize, request.StoreBatchSize, "N=%d C=%d", regionCount, scanConcurrency)
	}

	// N=12 is the exact no-batch gate. N=13 crosses it, but C=6 cannot batch
	// and still keep analyzeBatchMinOuterConcurrency workers; C=8 is the
	// smallest batching budget and N=17 its first batched neighbor.
	assertAnalyzeRequest(12, 6, 6, 0)
	assertAnalyzeRequest(13, 6, 6, 0)
	assertAnalyzeRequest(17, 8, 4, 1)
	...
	// With abundant Regions the outer-concurrency floor binds the batch, and
	// the 300-Region lookup also crosses the pagination boundary.
	assertAnalyzeRequest(300, 32, 4, 7)
	// Positive-only values leave the upper unsigned half empty; all rows must
	// still be analyzed.
	metaRows := tk.MustQuery("show stats_meta where db_name = 'test' and table_name = 'tu'").Rows()
	require.Len(t, metaRows, 1)
	require.Equal(t, "300", metaRows[0][5])
}
