...
func TestColumnTable(t *testing.T) {
	...
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	baseColumnCnt, err := strconv.Atoi(tk.MustQuery(`select count(*) from information_schema.columns;`).Rows()[0][0].(string))
	require.NoError(t, err)
	...
	tk.MustQuery(`select count(*) from information_schema.columns;`).Check(
		testkit.RowsWithSep("|", fmt.Sprintf("%d", baseColumnCnt+9)))
}
...
func TestInfoSchemaDDLJobs(t *testing.T) {
	...
	testfailpoint.EnableCall(t, "github.com/pingcap/tidb/pkg/ddl/onJobRunBefore", func(job *model.Job) {
		if job.SchemaState == model.StateWriteOnly && loaded.CompareAndSwap(false, true) {
			tk2.MustQuery(`SELECT JOB_TYPE, SCHEMA_STATE, table_name, STATE
				...
				"add index|write only|t0|running",
			))
			tk2.MustQuery(`SELECT JOB_TYPE, SCHEMA_STATE, table_name, STATE
	...
	tk.MustExec("create database test2")
	tk.MustExec("create table test2.t1(id int)")
	tk.MustQuery(`SELECT JOB_TYPE, SCHEMA_STATE, table_name, STATE
...
func TestInfoschemaTablesSpecialOptimizationCovered(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		// This query can take different optimization paths between infoschema v1/v2.
		{"select count(1) from (select table_name from information_schema.tables) t", true},
		{"select * from information_schema.tables", false},
		// Projection with table_catalog can also diverge between infoschema v1/v2.
