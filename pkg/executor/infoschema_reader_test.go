...
func testInfoschemaTablesWhenTableIsNotPublic(t *testing.T, enableV2 bool) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	if enableV2 {
	...
	testfailpoint.EnableCall(t, "github.com/pingcap/tidb/pkg/ddl/onJobRunBefore", func(job *model.Job) {
		if (job.Type == model.ActionCreateTable || job.Type == model.ActionDropTable) && job.TableName == "child" {
			if job.SchemaState == model.StateDeleteOnly {
				tk.MustQuery("select * from information_schema.tables where table_name = 'child'").WithUseAssert(true).Check(testkit.Rows())
				tk.MustQuery("select table_name from information_schema.tables where table_name = 'child'").WithUseAssert(true).Check(testkit.Rows())
				tk.MustQuery("select * from information_schema.referential_constraints where table_name = 'child'").WithUseAssert(true).Check(testkit.Rows())
				tk.MustQuery("select * from information_schema.partitions where table_name = 'child'").WithUseAssert(true).Check(testkit.Rows())
				tk.MustQuery("select * from information_schema.tidb_indexes where table_name = 'child'").WithUseAssert(true).Check(testkit.Rows())
				tk.MustQuery("select * from information_schema.key_column_usage where table_name = 'child'").WithUseAssert(true).Check(testkit.Rows())
				tk.MustQuery("select * from information_schema.table_constraints where table_name = 'child'").WithUseAssert(true).Check(testkit.Rows())
				tk.MustQuery("select * from information_schema.columns where table_name = 'child'").WithUseAssert(true).Check(testkit.Rows())
				tk.MustQuery("show tables").WithUseAssert(true).Check(testkit.Rows("parent"))
			}
		}
	})
	...
	tkDDL.MustExec("drop table child")

	testfailpoint.Disable(t, "github.com/pingcap/tidb/pkg/ddl/onJobRunBefore")
}

func TestInfoschemaTablesWhenTableIsNotPublic(t *testing.T) {
	testInfoschemaTablesWhenTableIsNotPublic(t, false)
	testInfoschemaTablesWhenTableIsNotPublic(t, true)
}
