...
func (e *IndexMergeReaderExecutor) Open(_ context.Context) (err error) {
	...
	if e.isCorColInTableFilter {
		e.tableRequest.Executors, err = builder.ConstructListBasedDistExec(e.Ctx().GetBuildPBCtx(), e.tblPlans, kv.TiKV)
		if err != nil {
			return err
		}
	}
		...
		util.WithRecovery(
			func() {
				...
				if e.isCorColInPartialFilters[workID] {
					...
					var err error
					if e.dagPBs[workID].Executors, err = builder.ConstructListBasedDistExec(e.Ctx().GetBuildPBCtx(), e.partialPlans[workID], kv.TiKV); err != nil {
