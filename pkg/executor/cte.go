...
func (p *cteProducer) computeChunkHash(chk *chunk.Chunk) (sel []int, err error) {
	...
	for i := 0; i < chk.NumCols(); i++ {
		if err = codec.HashChunkSelected(p.hCtx.hashVals, chk, p.hCtx.allTypes[i], i, p.hCtx.buf, p.hCtx.hasNull, hashBitMap, false); err != nil {
			return nil, err
		}
	}
	return sel, nil
...
func (p *cteProducer) checkHasDup(probeKey uint64,
	...
	for ; entry != nil; entry = entry.next {
		...
		isEqual, err := codec.EqualChunkRow(row, p.hCtx.allTypes, p.hCtx.keyColIdx, matchedRow, p.hCtx.allTypes, p.hCtx.keyColIdx)
		if err != nil {
			return false, err
		}
		if isEqual {
