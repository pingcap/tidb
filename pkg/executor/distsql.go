...
func (e *IndexLookUpExecutor) Close() error {
	if e.stats != nil {
		defer func() {
			...
			if e.indexLookUpPushDown {
				metrics.IndexLookUpExecutorRowNumberPushDown.Observe(float64(e.stats.indexScanBasicStats.GetActRows()))
			}
		}()
	}
	...
	if stats := e.RuntimeStats(); stats != nil {
		if e.indexLookUpPushDown {
			defer func() {
				metrics.IndexLookUpExecutorDurationPushDown.Observe(time.Duration(stats.GetTime()).Seconds())
			}()
		}
	}
