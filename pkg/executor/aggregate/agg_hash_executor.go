...
func (e *HashAggExec) initPartialWorkers(partialConcurrency int, finalConcurrency int, ctx sessionctx.Context) {
	for i := 0; i < partialConcurrency; i++ {
		...
		e.partialWorkers[i] = HashAggPartialWorker{
			...
			chk:                  e.NewChunkWithCapacity(e.RetFieldTypes(), e.InitCap(), e.MaxChunkSize()),
		...
		input := &HashAggInput{
			chk:        chunk.New(e.RetFieldTypes(), e.InitCap(), e.MaxChunkSize()),
...
func (e *HashAggExec) initFinalWorkers(finalConcurrency int) {
	for i := 0; i < finalConcurrency; i++ {
		...
		e.finalWorkers[i].finalResultHolderCh <- chunk.New(e.RetFieldTypes(), e.InitCap(), e.MaxChunkSize())
	}
}
