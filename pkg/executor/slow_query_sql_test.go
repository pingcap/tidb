...
# Request_unit_detail: RRU=read_rpc_count(1)*READ_BASE_COST(0.125)=0.125000; RU=RRU(0.125000)=0.125000
...
`)
	...
	tk.MustQuery("select Request_unit_v2, Request_unit_v2 + 1, Request_unit_v2_detail, Request_unit_detail from `information_schema`.`slow_query` " +
		"where query = 'select /* issue:67199 */ 1;'").
		Check(testkit.Rows(`123.45 124.45 total_ru:123.45, tidb_ru:100.00, tikv_ru:20.00, tiflash_ru:3.45 RRU=read_rpc_count(1)*READ_BASE_COST(0.125)=0.125000; RU=RRU(0.125000)=0.125000`))
}
