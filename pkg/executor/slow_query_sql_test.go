...
func TestSlowLogRUVersion(t *testing.T) {
	...
	for _, version := range []rmclient.RUVersion{rmclient.RUVersionV1, rmclient.RUVersionV2} {
		t.Run(fmt.Sprintf("v%d", version), func(t *testing.T) {
			provider := &slowLogRUVersionProvider{ResourceManagerClient: infosync.NewMockResourceManagerClient(1), version: version}
...
func (*slowLogRUVersionProvider) Put(context.Context, []byte, []byte, ...opt.MetaStorageOption) (*meta_storagepb.PutResponse, error) {
	return nil, fmt.Errorf("unexpected metadata write in RU version test")
}
