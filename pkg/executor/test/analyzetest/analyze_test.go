...
func TestAnalyzeUsesNDVRateOption(t *testing.T) {
	...
	t.Run("singleton-sketch path with ndvrate<1", func(t *testing.T) {
		...
		// True NDVs are 200/10/10. Column `a` is intentionally not pinned to a
		// single number here: the HLL-backed singleton path is approximate, but it
		// must be clearly below the exact full-scan value and clearly above the
		// raw half-sample size.
		aNDV := readNDV(tblInfo.Columns[0].ID, 0)
		require.Less(t, aNDV, int64(200))
		require.Greater(t, aNDV, int64(100))
		require.Equal(t, int64(10), readNDV(tblInfo.Columns[1].ID, 0))
		require.Equal(t, int64(10), readNDV(tblInfo.Indices[0].ID, 1))
	})
}
