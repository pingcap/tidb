...
func TestFailedAnalyzeRequest(t *testing.T) {
	...
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
	// MV indexes are analyzed by independent index-analyze tasks in v2 and still use buildStatsFromResult.
	tk.MustExec("create table t (a int, c json, index idx_c((cast(json_extract(c, _utf8mb4'$') as char(32) array))))")
	tk.MustExec(`insert into t values (1, '["a1", "a2"]'), (2, '["b1", "b2"]')`)

	fp := "github.com/pingcap/tidb/pkg/executor/buildStatsFromResult"
	require.NoError(t, failpoint.Enable(fp, `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable(fp))
	}()

	err := tk.ExecToErr("analyze table t index idx_c")
	require.Error(t, err)
	require.ErrorContains(t, err, "mock buildStatsFromResult error")
}
...
func TestAnalyzeForceV2WarningOnLegacyStats(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
	...
	tbl, err := is.TableByName(context.Background(), ast.NewCIStr("test"), ast.NewCIStr("t"))
	require.NoError(t, err)
	tk.MustExec("update mysql.stats_histograms set stats_ver = 1 where table_id = ?", tbl.Meta().ID)
	handle.Clear()
	require.NoError(t, handle.Update(context.Background(), is))
	...
	for _, row := range warnings {
		msg := row[2].(string)
		if strings.Contains(msg, "Analyze v1 has been removed. TiDB forces analyze version 2") {
			found = true
			break
		}
	}
	require.True(t, found, "expected force-v2 warning for legacy v1 stats")
}
