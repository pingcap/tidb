...
func TestShowCreateUser(t *testing.T) {
	...
	tk.MustExec("CREATE USER res1 WITH MAX_QUERIES_PER_HOUR 1")
	tk.MustQuery("SHOW CREATE USER res1").Check(testkit.Rows(`CREATE USER 'res1'@'%' IDENTIFIED WITH 'mysql_native_password' AS '' REQUIRE NONE ` +
		`WITH MAX_QUERIES_PER_HOUR 1 PASSWORD EXPIRE DEFAULT ACCOUNT UNLOCK PASSWORD HISTORY DEFAULT PASSWORD REUSE INTERVAL DEFAULT`))
	tk.MustExec("CREATE USER res2 WITH MAX_UPDATES_PER_HOUR 1")
	tk.MustQuery("SHOW CREATE USER res2").Check(testkit.Rows(`CREATE USER 'res2'@'%' IDENTIFIED WITH 'mysql_native_password' AS '' REQUIRE NONE ` +
		`WITH MAX_UPDATES_PER_HOUR 1 PASSWORD EXPIRE DEFAULT ACCOUNT UNLOCK PASSWORD HISTORY DEFAULT PASSWORD REUSE INTERVAL DEFAULT`))
	tk.MustExec("CREATE USER res3 WITH MAX_CONNECTIONS_PER_HOUR 1")
	tk.MustQuery("SHOW CREATE USER res3").Check(testkit.Rows(`CREATE USER 'res3'@'%' IDENTIFIED WITH 'mysql_native_password' AS '' REQUIRE NONE ` +
		`WITH MAX_CONNECTIONS_PER_HOUR 1 PASSWORD EXPIRE DEFAULT ACCOUNT UNLOCK PASSWORD HISTORY DEFAULT PASSWORD REUSE INTERVAL DEFAULT`))
	tk.MustExec("CREATE USER res4 WITH MAX_USER_CONNECTIONS 1")
	tk.MustQuery("SHOW CREATE USER res4").Check(testkit.Rows(`CREATE USER 'res4'@'%' IDENTIFIED WITH 'mysql_native_password' AS '' REQUIRE NONE ` +
		`WITH MAX_USER_CONNECTIONS 1 PASSWORD EXPIRE DEFAULT ACCOUNT UNLOCK PASSWORD HISTORY DEFAULT PASSWORD REUSE INTERVAL DEFAULT`))
	tk.MustExec("CREATE USER res5 WITH MAX_QUERIES_PER_HOUR 1 MAX_UPDATES_PER_HOUR 1 MAX_CONNECTIONS_PER_HOUR 1 MAX_USER_CONNECTIONS 1")
	tk.MustQuery("SHOW CREATE USER res5").Check(testkit.Rows(`CREATE USER 'res5'@'%' IDENTIFIED WITH 'mysql_native_password' AS '' REQUIRE NONE ` +
		`WITH MAX_QUERIES_PER_HOUR 1 MAX_UPDATES_PER_HOUR 1 MAX_CONNECTIONS_PER_HOUR 1 MAX_USER_CONNECTIONS 1` +
		` PASSWORD EXPIRE DEFAULT ACCOUNT UNLOCK PASSWORD HISTORY DEFAULT PASSWORD REUSE INTERVAL DEFAULT`))
}
