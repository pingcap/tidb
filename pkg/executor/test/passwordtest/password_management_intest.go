...
import (
	"bytes"
	// #nosec G505 -- legacy password hash compatibility is the subject under test
	"crypto/sha1"
	"encoding/json"
	"fmt"
	"strconv"
...
func RunValidatePassword(t *testing.T) {
...
func RunPasswordManagement(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("SET GLOBAL validate_password.enable = 1")
...
func RunFailedLoginTrackingBasic(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
...
func sha1Password(s string) []byte {
	// #nosec G401 -- legacy SHA1 password hashing is the subject under test
	crypt := sha1.New()
	crypt.Write([]byte(s))
...
func RunFailedLoginTracking(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
...
func RunFailedLoginTrackingAlterUser(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
...
func RunFailedLoginTrackingCheckPrivilges(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
...
func RunUserPassword(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
...
func RunPasswordExpiredAndTacking(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	user := "u3"
	host := "localhost"
...
func RunPasswordMySQLCompatibility(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
