...
func RunSelectCheckVisibility(t *testing.T) {
...
func RunReturnValues(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
...
func RunMemCacheReadLock(t *testing.T) {
...
func RunPartitionMemCacheReadLock(t *testing.T) {
	defer config.RestoreFunc()()
	config.UpdateGlobal(func(conf *config.Config) {
		conf.EnableTableLock = true
	})
...
func RunPointGetLockExistKey(t *testing.T) {
...
func RunWithTiDBSnapshot(t *testing.T) {
