...
func TestIssue30211(t *testing.T) {
	...
	tk.MustExec("prepare stmt30211 from 'select /*+ inl_hash_join(t1) */ * from t1 join t2 on t1.a = t2.a';")
	defer tk.MustExec("deallocate prepare stmt30211;")
	tk.MustQuery("execute stmt30211;").Sort().Check(testkit.Rows("1 1", "1 1", "2 2", "2 2"))
	tk.MustExec("set @@tidb_mem_quota_query=1;")
	tk.MustExec("set tidb_index_join_batch_size = 1;")
	tk.MustExec("SET GLOBAL tidb_mem_oom_action = 'CANCEL'")
	defer tk.MustExec("SET GLOBAL tidb_mem_oom_action='LOG'")
	err := tk.QueryToErr("execute stmt30211;")
	require.True(t, exeerrors.ErrMemoryExceedForQuery.Equal(err))
}
