...
func TestPessimisticLockOnPartitionForIndexMerge(t *testing.T) {
	...
	tk.MustQuery(`explain format='brief' select /*+ use_index_merge(t1) */ c1 from t1 join t2
		...
		"        │   ├─IndexRangeScan(Build) 3.00 cop[tikv] table:t1, partition:p0, index:c1(c1) range:[-inf,10), keep order:false",
		"        │   ├─IndexRangeScan(Build) 3.00 cop[tikv] table:t1, partition:p0, index:c2(c2) range:[-inf,10), keep order:false",
		"        │   └─TableRowIDScan(Probe) 3.00 cop[tikv] table:t1, partition:p0 keep order:false",
