...
func TestPessimisticLockOnPartitionForIndexMerge(t *testing.T) {
	...
	tk.MustQuery(`explain format='brief' select /*+ use_index_merge(t1) */ c1 from t1 join t2
		...
		"Projection 18.00 root  test.t1.c1",
		"└─SelectLock 18.00 root  for update 0",
		"  └─Projection 18.00 root  test.t1.c1, test.t1._tidb_rowid, test.t1._tidb_tid, test.t2._tidb_rowid",
		"    └─HashJoin 18.00 root  CARTESIAN inner join, other cond:ge(test.t1.c_datetime, test.t2.c_datetime)",
		...
		"      └─PartitionUnion(Probe) 6.00 root  ",
		"        ├─Projection 3.00 root  test.t1.c_datetime, test.t1.c1, test.t1._tidb_rowid, test.t1._tidb_tid",
		"        │ └─IndexMerge 3.00 root  type: union",
		"        │   ├─IndexRangeScan(Build) 3.00 cop[tikv] table:t1, partition:p0, index:c1(c1) range:[-inf,10), keep order:false, stats:pseudo",
		"        │   ├─IndexRangeScan(Build) 3.00 cop[tikv] table:t1, partition:p0, index:c2(c2) range:[-inf,10), keep order:false, stats:pseudo",
		"        │   └─TableRowIDScan(Probe) 3.00 cop[tikv] table:t1, partition:p0 keep order:false, stats:pseudo",
