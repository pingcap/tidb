...
func TestIndexMergeReaderIssue45279(t *testing.T) {
	...
	tk.MustQuery("explain select * from reproduce where c1 in (0, 1, 2, 3) or c2 in (0, 1, 2);").Check(testkit.Rows(
		"IndexMerge_12 253.90 root  type: union",
		"├─TableRangeScan_9(Build) 4.00 cop[tikv] table:reproduce range:[0,0], [1,1], [2,2], [3,3], keep order:false, stats:pseudo",
		"├─IndexRangeScan_10(Build) 250.00 cop[tikv] table:reproduce, index:ci2(c2) range:[0,2], keep order:false, stats:pseudo",
		"└─TableRowIDScan_11(Probe) 253.90 cop[tikv] table:reproduce keep order:false, stats:pseudo"))
...
func TestIndexMergeLimitNotPushedOnPartialSideButKeepOrder(t *testing.T) {
	...
	for idx := range 10 {
		valA, valB, valC, limit := idx%10, idx%8, idx%15, (idx%10)+1
