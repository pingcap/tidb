...
func RunPlanReplayerCaptureSEM(t *testing.T) {
	tempDir := t.TempDir()
	ctx := context.Background()
	storage, err := extstore.NewExtStorage(ctx, "file://"+tempDir, "")
	require.NoError(t, err)
...
func RunPlanReplayerCapture(t *testing.T) {
...
func RunPlanReplayerContinuesCapture(t *testing.T) {
	tempDir := t.TempDir()
	ctx := context.Background()
	storage, err := extstore.NewExtStorage(ctx, "file://"+tempDir, "")
	require.NoError(t, err)
...
func RunPlanReplayerDumpSingle(t *testing.T) {
	ctx := context.Background()
	tempDir := t.TempDir()
	storage, err := extstore.NewExtStorage(ctx, "file://"+tempDir, "")
	require.NoError(t, err)
...
func RunPlanReplayerDumpMultipleError(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	tk.MustExec("create table t(id int)")
...
func RunPlanReplayerDumpMultiple(t *testing.T) {
	const numStmts = 50
	const numTables = 5
	ctx := context.Background()
	tempDir := t.TempDir()
