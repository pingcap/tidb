...
func TestCTEStrictModeDataTruncation(t *testing.T) {
	...
	// Implicit recursive-member coercion (no explicit CAST): truncation should still be an error in strict mode.
	err = tk.QueryToErr(`
			WITH RECURSIVE cte AS (
				SELECT CAST('abc' AS CHAR(3)) AS s, 1 AS n
				UNION ALL
				SELECT 'abcdefgh', n + 1
			FROM cte
			WHERE n < 2
...
func TestCTENonStrictModeDataTruncation(t *testing.T) {
	...
	// Implicit recursive-member coercion (no explicit CAST) should still truncate in non-strict mode.
	tk.MustQuery(`
		WITH RECURSIVE cte AS (
			SELECT CAST('abc' AS CHAR(3)) AS s, 1 AS n
			UNION ALL
