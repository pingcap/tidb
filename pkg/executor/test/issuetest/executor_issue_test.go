...
func TestUnionIssue(t *testing.T) {
	...
	// Issue56587
	tk.MustQuery("(select quote(cast('abc' as char)) union all select '1') order by 1").Check(testkit.Rows("'abc'", "1"))
	tk.MustQuery(`select elt(2, "1", cast('abc' as char)) union all select "12" where false`).Check(testkit.Rows("abc"))
	tk.MustQuery(`(select hex(cast('1' as char)) union all select '1') order by 1;`).Check(testkit.Rows("1", "31"))
