...
func TestForeignKeyOnInsertChildTable(t *testing.T) {
	...
	tk.MustExec("create table missing_parent_child(id int primary key, a int, foreign key(a) references missing_parent_db.parent(id))")
	tk.MustExec("set foreign_key_checks=1")
	tk.MustGetDBError("insert into missing_parent_child values(1,1)", plannererrors.ErrNoReferencedRow2)
	tk.MustExec("insert into missing_parent_child values(2,null)")
	tk.MustExec("insert into missing_parent_child select 3,1 where false")
	tk.MustExec("insert ignore into missing_parent_child values(3,1),(4,null)")
	require.Equal(t, uint16(1), tk.Session().GetSessionVars().StmtCtx.WarningCount())
	tk.MustGetDBError("insert into missing_parent_child values(5,null),(6,1)", plannererrors.ErrNoReferencedRow2)
	tk.MustQuery("select id,a from missing_parent_child order by id").Check(testkit.Rows("2 <nil>", "4 <nil>"))
	tk.MustGetDBError("update missing_parent_child set a=1 where id=2", plannererrors.ErrNoReferencedRow2)
	tk.MustQuery("explain insert into missing_parent_child values(5,1)")
	tk.MustExec("create database missing_parent_db")
	tk.MustGetDBError("replace into missing_parent_child values(2,1)", plannererrors.ErrNoReferencedRow2)
	tk.MustExec("create table missing_parent_db.parent(id int primary key)")
	tk.MustExec("insert into missing_parent_db.parent values(1)")
	tk.MustExec("insert into missing_parent_child values(5,1)")
	tk.MustQuery("select id,a from missing_parent_child order by id").Check(testkit.Rows("2 <nil>", "4 <nil>", "5 1"))
	tk.MustExec("drop table missing_parent_child")
	tk.MustExec("set foreign_key_checks=0")
	tk.MustExec("create table missing_parent_child(id int primary key,a int,b int,foreign key(a,b) references missing_parent_db.composite(a,b))")
	tk.MustExec("set foreign_key_checks=1")
	tk.MustExec("insert into missing_parent_child values(1,null,1),(2,1,null),(3,null,null)")
	tk.MustGetDBError("insert into missing_parent_child values(4,1,1)", plannererrors.ErrNoReferencedRow2)
	tk.MustGetDBError("update missing_parent_child set a=1 where id=1", plannererrors.ErrNoReferencedRow2)
	tk.MustQuery("select id,a,b from missing_parent_child order by id").Check(testkit.Rows("1 <nil> 1", "2 1 <nil>", "3 <nil> <nil>"))
	tk.MustExec("drop table missing_parent_child")
	tk.MustExec("drop database missing_parent_db")
