...
func (*ruVersionResourceGroupProvider) Put(context.Context, []byte, []byte, ...opt.MetaStorageOption) (*meta_storagepb.PutResponse, error) {
...
func setDomainRUVersionForTest(t *testing.T, dom *domain.Domain, version rmclient.RUVersion) {
	...
	controller, err := rmclient.NewResourceGroupController(
		...
		&ruVersionResourceGroupProvider{ResourceManagerClient: infosync.NewMockResourceManagerClient(1), config: cfg},
		nil, 1,
	)
