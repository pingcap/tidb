...
func TestUpdateDecimalTruncationWarning(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	tk.MustExec("create table decimals (id int primary key, total decimal(20, 0) not null)")
	tk.MustExec("insert into decimals values (1, 201)")

	tk.MustExec("update /* issue:5777 */ decimals set total = total + 1.23 where id = 1")
	tk.MustQuery("show warnings").Check(testkit.Rows("Warning 1265 Data truncated for column 'total' at row 1"))

	tk.MustExec("prepare stmt from 'update decimals set total = total + ? where id = ?'")
	tk.MustExec("set @increment = 1.23, @id = 1")
	tk.MustExec("execute stmt using @increment, @id")
	tk.MustQuery("show warnings").Check(testkit.Rows("Warning 1265 Data truncated for column 'total' at row 1"))
}
