...
// TestUpdateSelfJoinExistsUsesCorrelatedSameTable ensures self-join UPDATE with EXISTS uses
// a correlated single-row inner FROM for the matched outer table instance.
func TestUpdateSelfJoinExistsUsesCorrelatedSameTable(t *testing.T) {
	...
	require.Contains(t, plan.String(), "TableDual")
}
