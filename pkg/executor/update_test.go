...
func TestUpdateOutOfRangeTimestamp(t *testing.T) {
	...
	tk.MustExec("update t set ts = '1969-01-01 00:00:00' where id = 1")
	tk.MustQuery("show warnings").Check(testkit.Rows("Warning 1292 Incorrect time value: '{1969 1 1 0 0 0 0}'"))
	tk.MustQuery("select * from t where id = 1").Check(testkit.Rows("1 0000-00-00 00:00:00"))
	tk.MustExec("update t set ts = '2039-01-01 00:00:00' where id = 2")
	tk.MustQuery("show warnings").Check(testkit.Rows("Warning 1292 Incorrect time value: '{2039 1 1 0 0 0 0}'"))
	tk.MustQuery("select * from t where id = 2").Check(testkit.Rows("2 0000-00-00 00:00:00"))
}

func TestUpdateInvalidTimestampInDST(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewSteppedTestKit(t, store)
	tk.MustExec("use test")
	tk.MustExec(`set sql_mode = ''`)
	...
	tk.MustExec("create table t (id int primary key, ts timestamp)")
	tk.MustExec("insert into t values (1, '2020-01-01 00:00:00')")
	tk.MustExec("update t set ts = '2025-03-30 02:30:00' where id = 1")
	tk.MustQuery("show warnings").Check(testkit.Rows("Warning 1292 Incorrect time value: '{2025 3 30 2 30 0 0}'"))
	tk.MustQuery("select * from t where id = 1").Check(testkit.Rows("1 0000-00-00 00:00:00"))
}
