...
// FIXME(cbc): check if such test case exists
...
func removePrivFromTablePriv(ctx context.Context, sctx sessionctx.Context,
	...
	if priv == mysql.AllPriv {
		...
		// FIXME(cbc): how to trigger this code path? It seems that 'revoke all' would not run here
		sql.Reset()
		sqlescape.MustFormatSQL(sql,
...
func removePrivFromColumnPriv(ctx context.Context, e sqlexec.SQLExecutor,
	...
	sql := new(strings.Builder)
	if priv == mysql.AllPriv {
		// FIXME(cbc): is it a possible path?
...
func (e *RevokeExec) revokeColumnPriv(ctx context.Context, internalSession sessionctx.Context, priv *ast.PrivElem, user, host string, checkTablePrivTbl bool) error {
	...
	if len(cols) == 0 && tbl != nil {
		...
		// FIXME(cbc): the behavior is the same as mysql. Check if we have such test cases
	...
	if len(cols) > 0 {
		...
		// FIXME(cbc): is this a dead code path?
	...
	if checkTablePrivTbl {
		// checks if there is another entry with key user-host-db-tbl and column_priv in mysql.Columns_priv.
		exists, err := recordExists(internalSession,
			strings.Join([]string{"SELECT * FROM %n.%n WHERE User=%? AND Host=%? AND DB=%? AND Table_name=%? AND Column_priv LIKE '%%", priv.Priv.String(), "%%';"}, ""),
			mysql.SystemDB, mysql.ColumnPrivTable, user, host, e.Level.DBName, e.Level.TableName)
		if err != nil {
			return err
		}
		if !exists {
