...
func addUnchangedKeysForLockByRow(
	...
	// The index objects of a partitioned table are bound to the logical table ID, while
	// index keys are stored under each partition's physical table ID. Use the selected
	// partition's index objects so that the generated keys (including global-index
	// partition handles and DDL NewTableID remapping) match the ones actually written.
	indexTable := t
	...
	if keySet&lockUniqueKeys > 0 {
		...
		for _, idx := range indexTable.Indices() {
			...
			// A partial index only contains rows satisfying its predicate, so a row
			// outside the predicate must not lock any (non-existent) index key.
			...
			// GenIndexKVIter enumerates the real index keys, expanding a multi-valued
			// index into one key per distinct JSON array element.
