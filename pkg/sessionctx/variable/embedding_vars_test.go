...
func TestGetOpenAIEmbeddingBaseURL(t *testing.T) {
	...
	require.NotNil(t, sv)
	beforeVersion := vardef.EmbeddingConfigVersion.Load()
	require.NoError(t, sv.SetGlobal(context.Background(), nil, "https://api.openai.com/v2/"))
	require.Equal(t, "https://api.openai.com/v2", vardef.EmbedOpenAIAPIBase.Load())
	require.Equal(t, beforeVersion+1, vardef.EmbeddingConfigVersion.Load())
	require.NoError(t, sv.SetGlobal(context.Background(), nil, "https://api.openai.com/v2"))
	require.Equal(t, beforeVersion+1, vardef.EmbeddingConfigVersion.Load())

	require.NoError(t, sv.SetGlobal(context.Background(), nil, vardef.DefTiDBEmbedOpenAIAPIBase))
	defaultVersion := vardef.EmbeddingConfigVersion.Load()
	require.NoError(t, sv.SetGlobal(context.Background(), nil, ""))
	require.Equal(t, "", vardef.EmbedOpenAIAPIBase.Load())
	require.Equal(t, vardef.DefTiDBEmbedOpenAIAPIBase, GetOpenAIEmbeddingBaseURL())
	require.Equal(t, defaultVersion, vardef.EmbeddingConfigVersion.Load())
}

func TestEmbeddingAPIKeySysVars(t *testing.T) {
	type apiKeyVar struct {
		name  string
		load  func() string
		store func(string)
	}
	...
	for _, item := range vars {
		...
		require.NoError(t, sv.SetGlobal(context.Background(), nil, value))
		require.Equal(t, value, item.load())
		require.Equal(t, beforeVersion+1, vardef.EmbeddingConfigVersion.Load())
		require.NoError(t, sv.SetGlobal(context.Background(), nil, value))
		require.Equal(t, beforeVersion+1, vardef.EmbeddingConfigVersion.Load())

		masked, err := sv.GetGlobal(context.Background(), nil)
		require.NoError(t, err)
