...
type SessionVars struct {
	...
	*UserVars
	...
	// EnablePlanCacheGenericRewrite enables the current narrow rewrite policy
	// for plan cache. It currently covers non-outer-join constant propagation,
	// predicate simplification, and ranger same-slot Eq/In fallback. It does not
	// cover broader join reasoning such as outer-join derivation or null-reject
	// inference.
	...
	// EnableRemotePlan routes eligible prepared plans to a remote TiDB node for execution.
	EnableRemotePlan bool
	// EnableRemotePlanInTxnRead routes eligible read-only prepared plans to a remote TiDB node for execution
	// inside a transaction, when the referenced tables are not dirty.
	EnableRemotePlanInTxnRead bool
	// RemotePlanFeedbackDisableAfter is the number of consecutive large-result forwards required to disable forwarding.
	RemotePlanFeedbackDisableAfter uint64
	// RemotePlanFeedbackCooldown is the cooldown period (in seconds) after feedback disables forwarding.
	RemotePlanFeedbackCooldown uint64
	// RemotePlanFeedbackNoShrinkRatio is the ratio threshold (in percent) to treat a forwarded execution as "no shrink".
	RemotePlanFeedbackNoShrinkRatio uint64
	// RemotePlanFeedbackMinLocalCallRequests is the minimum number of TiKV local-call/FFI requests observed on the remote
	// side to treat forwarding as worthwhile even when the result set is large and not shrinking.
	RemotePlanFeedbackMinLocalCallRequests uint64
...
func NewSessionVars(hctx HookContext) *SessionVars {
	vars := &SessionVars{
		...
		EnablePlanCacheGenericRewrite:          DefTiDBOptEnablePlanCacheGenericRewrite,
		HookContext:                            hctx,
