...
local RRUPanel = graphPanel.new(
  ...
  prometheus.target(
    'sum(irate(resource_manager_resource_unit_read_request_unit_sum{k8s_cluster="$k8s_cluster", tidb_cluster="$tidb_cluster", type=~"|ap", resource_group=~"$resource_group"}[30s])) by (resource_group)',
    intervalFactor=1,
    legendFormat="ap-{{resource_group}}",
    hide=true,
  )
  ...
  prometheus.target(
    'sum(irate(resource_manager_resource_unit_read_request_unit_sum{k8s_cluster="$k8s_cluster", tidb_cluster="$tidb_cluster", type=~"|ap"}[30s]))',
    intervalFactor=1,
    legendFormat="ap-total",
    hide=true,
  )
...
local WRUPanel = graphPanel.new(
  ...
  prometheus.target(
    'sum(irate(resource_manager_resource_unit_write_request_unit_sum{k8s_cluster="$k8s_cluster", tidb_cluster="$tidb_cluster", type=~"|ap", resource_group=~"$resource_group"}[30s])) by (resource_group)',
    intervalFactor=1,
    legendFormat="ap-{{resource_group}}",
    hide=true,
  )
  ...
  prometheus.target(
    'sum(irate(resource_manager_resource_unit_write_request_unit_sum{k8s_cluster="$k8s_cluster", tidb_cluster="$tidb_cluster", type=~"|ap"}[30s]))',
    intervalFactor=1,
    legendFormat="ap-total",
    hide=true,
  )
...
.addPanel(
  ...
  panels: [
    if panel.title == "Client" then
      panel { panels+: [ClientRUPanel { id: 54, gridPos: fullPanelPos { y: 4 * panelH } }] }
    else panel
