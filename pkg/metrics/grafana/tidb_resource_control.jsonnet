...
local FailedKVRequestCountPanel = graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(resource_manager_client_request_fail{k8s_cluster="$k8s_cluster", tidb_cluster="$tidb_cluster", instance=~"$tidb_instance", resource_group=~"$resource_group"}[1m])) by (instance, resource_group)',
    legendFormat="{{instance}}-{{resource_group}}",
  )
);
