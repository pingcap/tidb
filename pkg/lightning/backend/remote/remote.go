...
func NewBackend(
	...
	if err != nil {
		log.L().Error("fail to create TiKV client", zap.Error(err))
		return nil, common.ErrCreateKVClient.Wrap(err).GenWithStackByArgs()
	}
...
func (b *Backend) Close() {
	if b.duplicateDB != nil {
		...
		if err == nil {
			...
			b.logger.Warn("create iter failed", zap.Error(err))
		}
	}
...
func (b *Backend) loadDataInit(ctx context.Context, engine *engine, dataSize int64) error {
	for {
		...
		resp, err := sendRequestWithRetry(ctx, &client, "POST", url, nil, engine.retryCounter)
		if err != nil {
			return err
		}
		...
		if resp.StatusCode != http.StatusOK {
			...
			if err == nil {
				if strings.TrimSpace(string(msg)) == taskExitsMsg {
					b.logger.Info("loadData task has inited in load_data worker",
						zap.String("loadDataTask", engine.loadDataTaskID),
						zap.String("worker addr", engine.addr))
				}
				// msg format as `data size %d exceeds max data size %d`
				if strings.Contains(string(msg), taskExceedMaxSizeMsg) {
					return errors.New(string(msg) + ". Please contact support to raise threshold.")
				}
				return nil
			}
...
func (b *Backend) CloseEngine(ctx context.Context, cfg *backend.EngineConfig, engineUUID uuid.UUID) error {
	...
	if err != nil {
		...
		if !exist {
			b.logger.Error("task not found in remote", zap.String("loadDataTaskID", genLoadDataTaskID(b.keyspaceID, cfg)))
			return ErrTaskNotFound
		}
	...
	for {
		...
		case <-ticker.C:
			...
			if states.Canceled {
				...
				if strings.Contains(states.Error, "Ingest::Overlap: region has overlap data") {
					return ErrOverlap
				} else if strings.Contains(states.Error, "TooManyDuplicatedKeys") {
