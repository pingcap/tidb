...
func NewBaseKVEncoder(config *encode.EncodingConfig) (*BaseKVEncoder, error) {
	...
	return &BaseKVEncoder{
		...
		hasFullTextTiCIIndexKVs: hasFullTextTiCIIndex(meta),
	}, nil
}
...
func (e *BaseKVEncoder) tableForTiCIIndexKVs(record []types.Datum) (table.Table, error) {
	pt, ok := e.table.(table.PartitionedTable)
	if !ok {
		return e.table, nil
	}
	physicalTable, err := pt.GetPartitionByRow(e.SessionCtx.GetExprCtx().GetEvalCtx(), record)
	if err != nil {
		return nil, err
	}
	if physicalTable == nil {
		return nil, errors.Errorf("failed to locate partition for TiCI index KVs")
	}
	return physicalTable, nil
}

func (e *BaseKVEncoder) addTiCIIndexKVs(tbl table.Table, record []types.Datum, handle kv.Handle) error {
	writeBufs := e.SessionCtx.GetTableCtx().GetMutateBuffers().GetWriteStmtBufs()
	...
	for _, idx := range tbl.Indices() {
		...
		handleRestoreData := tables.TryGetHandleRestoredDataWrapper(tbl.Meta(), record, nil, idx.Meta())
		if _, err = idx.Create(e.SessionCtx.GetTableCtx(), e.SessionCtx.Txn(), indexVals, handle, handleRestoreData, table.DupKeyCheckSkip); err != nil {
			return err
		}
	}
