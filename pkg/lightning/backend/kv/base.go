...
type BaseKVEncoder struct {
	...
	// ColumnConstants holds literal string values for columns absent from source data,
	// keyed by lowercase column name (col.Name.L).
...
func NewBaseKVEncoder(config *encode.EncodingConfig) (*BaseKVEncoder, error) {
	...
	return &BaseKVEncoder{
		...
		AutoIDFn:        autoIDFn,
		ColumnConstants: config.ColumnConstants,
