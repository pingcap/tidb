...
func TestCheckPeersBusy(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		worker := local.newRegionJobWorker(context.Background(), 1, jobCh, jobOutCh, nil, nil)
		err := worker.run(ctx)
		require.NoError(t, err)
	}()
...
func TestNotLeaderErrorNeedUpdatePeers(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		worker := local.newRegionJobWorker(context.Background(), 1, jobCh, jobOutCh, &jobWg, nil)
		err := worker.run(ctx)
		require.NoError(t, err)
	}()
