...
// getReadRangesFromProps reads the statistic files to find the largest offset of
...
// data.
//
// Caller can specify multiple ascending keys and getReadRangesFromProps will return
// the offsets list per file for each key.
func getReadRangesFromProps(
	ctx context.Context,
	jobKeys [][]byte,
	paths []string,
	...
	for i := range paths {
		eg.Go(func() error {
			...
			for {
				...
				offsetsPerFile[i][keyIdx] = [2]uint64{p.offset, p.getEndOffset()}
				p, err3 = r.nextProp()
			}
		})
	}
