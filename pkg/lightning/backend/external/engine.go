...
func (e *Engine) loadRangeBatchData(
	...
	failpoint.Inject("mockLoadBatchRegionData", func(_ failpoint.Value) {
		...
		data := e.buildIngestData(ctx, kvs, nil)
		data.IncRef()
		select {
		case <-ctx.Done():
	...
	logutil.Logger(ctx).Info("load range batch done",
		zap.Int64("dataID", data.DataID()),
		zap.Int64("loadedKVs", data.LoadedKVCount()),
		zap.Int("rawLoadedKVs", len(e.memKVsAndBuffers.kvs)),
...
func (e *Engine) buildIngestData(ctx context.Context, kvs []KVPair, buf []*membuf.Buffer) *MemoryIngestData {
	return &MemoryIngestData{
		kvs:           kvs,
		ts:            e.ts,
		dataID:        e.nextDataID.Inc(),
...
// DataID returns the external engine-scoped sequence number for this data batch.
func (m *MemoryIngestData) DataID() int64 {
	return m.dataID
}

// LoadedKVCount returns the number of KVs stored in this data batch after duplicate handling.
func (m *MemoryIngestData) LoadedKVCount() int64 {
	return m.loadedKVCount
}

// ImportedKVCount returns the number of KVs imported from this data batch.
func (m *MemoryIngestData) ImportedKVCount() int64 {
	return m.dataImportedKVCount.Load()
}

// ImportedKVSize returns the number of bytes imported from this data batch.
func (m *MemoryIngestData) ImportedKVSize() int64 {
	return m.dataImportedKVSize.Load()
}
...
func (m *MemoryIngestData) DecRef() {
	if m.refCnt.Dec() == 0 {
		loadedKVs := m.LoadedKVCount()
		importedKVs := m.ImportedKVCount()
		m.kvs = nil
		...
		logger.Info("release memory ingest data",
			zap.Int64("dataID", m.DataID()),
			zap.Int64("loadedKVs", loadedKVs),
			zap.Int64("importedKVs", importedKVs),
			zap.Int64("importedBytes", m.ImportedKVSize()),
			zap.Bool("importedLoadedMatch", importedKVs == loadedKVs),
			zap.Int64("refCnt", 0),
		)
	}
}
