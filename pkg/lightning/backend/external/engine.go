...
type Engine struct {
	...
	// activeIngestData store data used generated by this engine
	activeIngestData []*atomic.Bool

	// region job worker related to this engine.
	// We store the operator.TunableOperator interface in it to avoid import cycle.
	worker atomic.Value
...
func getFilesReadConcurrency(
	...
	return getReadConcurrencyFromOffsets(ctx, statsFiles, startOffs, endOffs), startOffs, nil
}

func (e *Engine) loadRangeBatch(
	ctx context.Context,
	jobKeys [][]byte,
	...
	failpoint.Inject("mockLoadBatchRegionData", func(_ failpoint.Value) {
		...
		case outCh <- common.DataAndRanges{
			...
			e.activeIngestData = append(e.activeIngestData, data.released)
		}
...
// filterUnreleased filters out released pointers from the input slice.
func filterUnreleased(released []*atomic.Bool) []*atomic.Bool {
	res := released[:0]
...
func (e *Engine) checkConcurrencyChange(ctx context.Context, currBatchSize int) int {
	...
	if newBatchSize == currBatchSize {
		e.activeIngestData = filterUnreleased(e.activeIngestData)
...
func (e *Engine) LoadIngestData(
	...
	for start := 0; start < len(e.jobKeys)-1; {
		...
		err := e.loadRangeBatch(
			ctx,
			e.jobKeys[start:end],
			readRangesPerKey[start],
			readRangesPerKey[end-1],
			outCh,
		)
		if err != nil {
			return err
...
func (e *Engine) UpdateResource(ctx context.Context, concurrency int, memCapacity int64) error {
	...
	worker, _ := v.(operator.TunableOperator)
	...
	for {
		...
		case _, ok := <-e.readyCh:
			if ok {
				...
				logutil.BgLogger().Info("load data finished, skip update resource")
			}
			return nil
		}
	}
