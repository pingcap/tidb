...
func (s *DefaultJobSubmitter) SubmitTable(ctx context.Context, tableMeta *importsdk.TableMeta) (*ImportJob, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("generate import SQL: %w", err)
	}
...
func (s *DefaultJobSubmitter) buildImportOptions(tableMeta *importsdk.TableMeta) *importsdk.ImportOptions {
	...
	// Set max engine size for local backend
	if cfg.TikvImporter.EngineMemCacheSize > 0 {
		opts.MaxEngineSize = units.BytesSize(float64(cfg.TikvImporter.EngineMemCacheSize))
	}

	// Set record errors based on max error config
	...
	// Set CSV config for CSV format
	if opts.Format == "csv" {
		opts.CSVConfig = &cfg.Mydumper.CSV
		// Set skip rows if header is enabled
		if cfg.Mydumper.CSV.Header {
			opts.SkipRows = 1
		}
	}

	// Split file is useful for large files
