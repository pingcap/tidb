...
func TestParquetParser(t *testing.T) {
	...
	reader := newParquetParserForTest(context.Background(), t, dir, name, 0, ParquetFileMeta{})
...
func TestParquetParserLogicalTimeNanos(t *testing.T) {
	...
	require.NoError(t, WriteParquetFile(dir, name, pc, len(values)))
...
func TestParquetVariousTypes(t *testing.T) {
	...
	t.Run("logical_null_all_values_are_null", func(t *testing.T) {
		...
		require.NoError(t, WriteParquetFile(dir, name, pc, 2))
	...
	t.Run("rejects_nested_repeated_schema", func(t *testing.T) {
		...
		require.NoError(t, err)
		defer store.Close()
		_, err = NewParquetParser(context.Background(), store, func(ctx context.Context) (io.ReadSeekCloser, error) {
			return store.Open(ctx, name, nil)
		}, name, 0, ParquetFileMeta{})
	...
	t.Run("logical_timestamp_nanos_ignores_spark_rebase_marker", func(t *testing.T) {
		...
		require.NoError(t, WriteParquetFile(
			dir, name, pc, 1,
		...
		require.NoError(t, err)
		reader, err := NewParquetParser(context.Background(), store, func(ctx context.Context) (io.ReadSeekCloser, error) {
			return store.Open(ctx, name, nil)
		}, name, 0, ParquetFileMeta{Loc: asiaShanghai})
		require.NoError(t, err)
	...
	t.Run("logical_uint32_preserves_high_bits", func(t *testing.T) {
		...
		require.NoError(t, WriteParquetFile(dir, name, pc, len(physicalValues)))
		reader := newParquetParserForTest(context.Background(), t, dir, name, 0, ParquetFileMeta{})
	...
	t.Run("timestamp_and_decimal", func(t *testing.T) {
		...
		reader := newParquetParserForTest(context.Background(), t, dir, name, 0, ParquetFileMeta{Loc: time.UTC})
	...
	t.Run("logical_timestamp_nanos", func(t *testing.T) {
		...
		parser := newParquetParserForTest(context.Background(), t, dir, name, 0, ParquetFileMeta{Loc: time.UTC})
		require.NoError(t, parser.ReadRow())
...
func TestParquetAurora(t *testing.T) {
	...
	parser := newParquetParserForTest(context.TODO(), t, "examples", fileName, 0, ParquetFileMeta{})
...
func TestHiveParquetParser(t *testing.T) {
	...
	dir := "./parquet/"
	reader := newParquetParserForTest(context.TODO(), t, dir, name, 0, ParquetFileMeta{Loc: time.UTC})
	// UTC+0:00
	results := []time.Time{
...
func TestBasicReadFile(t *testing.T) {
	...
	reader := newParquetParserForTest(context.TODO(), t, dir, fileName, 0, ParquetFileMeta{})
	for i := range rowCnt {
		require.NoError(t, reader.ReadRow())
...
func TestParquetParserWholeFileInMemory(t *testing.T) {
	...
	read := func(t *testing.T, fileSize int64) (*ParquetParser, *recording.AccessStats) {
		store, accessStats := newParquetS3StoreForTest(t, fileName, data)
		parser, err := NewParquetParser(context.Background(), store, func(ctx context.Context) (io.ReadSeekCloser, error) {
			return store.Open(ctx, fileName, nil)
		}, fileName, fileSize, ParquetFileMeta{})
		require.NoError(t, err)
		t.Cleanup(func() {
			require.NoError(t, parser.Close())
		})
...
func TestParquetParserLargePagePeakMemory(t *testing.T) {
	...
	reader := newParquetParserForTest(context.Background(), t, dir, fileName, 0, ParquetFileMeta{allocator: allocator})
