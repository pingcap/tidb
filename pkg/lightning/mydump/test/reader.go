...
func RunExportStatementNoTrailingNewLine(t *testing.T) {
	dir := t.TempDir()
	...
	_, err = file.WriteString("CREATE DATABASE whatever;")
	require.NoError(t, err)
	stat, err := file.Stat()
	require.NoError(t, err)
	err = file.Close()
...
func RunExportStatementWithComment(t *testing.T) {
...
func RunExportStatementWithCommentNoTrailingNewLine(t *testing.T) {
	exportStatmentShouldBe(t, `
		/* whatever blabla
			multiple lines comment
			multiple lines comment
...
func RunExportStatementGBK(t *testing.T) {
	dir := t.TempDir()
	file, err := os.Create(filepath.Join(dir, "tidb_lightning_test_reader"))
	require.NoError(t, err)
	defer os.Remove(file.Name())

	_, err = file.WriteString("CREATE TABLE a (b int(11) COMMENT '")
	require.NoError(t, err)
...
func RunExportStatementGibberishError(t *testing.T) {
	dir := t.TempDir()
	file, err := os.Create(filepath.Join(dir, "tidb_lightning_test_reader"))
	require.NoError(t, err)
	defer os.Remove(file.Name())

	_, err = file.WriteString("\x9e\x02\xdc\xfbZ/=n\xf3\xf2N8\xc1\xf2\xe9\xaa\xd0\x85\xc5}\x97\x07\xae6\x97\x99\x9c\x08\xcb\xe8;")
	require.NoError(t, err)
	stat, err := file.Stat()
	require.NoError(t, err)
	err = file.Close()
...
func RunExportStatementHandleNonEOFError(t *testing.T) {
...
func RunExportStatementCompressed(t *testing.T) {
	dir := t.TempDir()
	file, err := os.Create(filepath.Join(dir, "tidb_lightning_test_reader"))
	require.NoError(t, err)
	defer os.Remove(file.Name())
...
func RunExportStatementMissingTrailingSemicolon(t *testing.T) {
	cases := []struct {
		name    string
		content []byte
		want    []byte
