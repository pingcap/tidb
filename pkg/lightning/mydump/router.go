...
var defaultFileRouteRules = []*config.FileRouteRule{
	...
	// Aurora/RDS snapshot-export data file pattern. The logical schema comes
	// from the database directory because part filenames contain no table name.
	{Pattern: `(?i)^(?:[^/]*/)*([^/]+)/[^/]+\.([^/]+)/(?:[0-9]+/)?part-[^/]+\.parquet$`,
		Schema: "$1", Table: "$2", Type: TypeParquet, Unescape: true},
