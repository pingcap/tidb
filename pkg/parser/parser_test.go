...
func TestMaterializedViewStatements(t *testing.T) {
	table := []testCase{
		...
		{
			"CREATE MATERIALIZED VIEW LOG ON t (a,b) TIFLASH REPLICA 1",
			true,
			"CREATE MATERIALIZED VIEW LOG ON `t` (`a`, `b`) TIFLASH REPLICA 1",
		},
		{
		...
		{
			"CREATE MATERIALIZED VIEW LOG ON t (a,b) TIFLASH REPLICA 1 PURGE IMMEDIATE",
			true,
			"CREATE MATERIALIZED VIEW LOG ON `t` (`a`, `b`) TIFLASH REPLICA 1 PURGE IMMEDIATE",
		},
		{
		...
		{
			"ALTER MATERIALIZED VIEW LOG ON t TIFLASH REPLICA 1",
			true,
			"ALTER MATERIALIZED VIEW LOG ON `t` TIFLASH REPLICA 1",
		},
		{
