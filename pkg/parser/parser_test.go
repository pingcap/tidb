...
func TestAdminStmt(t *testing.T) {
	table := []testCase{
		...
		{"admin checksum table t partition (p0)", true, "ADMIN CHECKSUM TABLE `t` PARTITION (`p0`)"},
		{"admin checksum table t partition (p0, p1)", true, "ADMIN CHECKSUM TABLE `t` PARTITION (`p0`, `p1`)"},
		{"admin checksum table db.t partition (p0)", true, "ADMIN CHECKSUM TABLE `db`.`t` PARTITION (`p0`)"},
