...
func RunProcedureVisitorCover(t *testing.T) {
	stmts := []ast.Node{
		&ast.StoreParameter{},
		&ast.ProcedureDecl{},
	}
...
func RunShowCreateProcedure(t *testing.T) {
	p := parser.New()
...
func RunProcedureVisitor(t *testing.T) {
	sqls := []string{
		"create procedure proc_2(in id bigint,in id2 varchar(100),in id3 decimal(30,2)) begin declare s varchar(100) DEFAULT FROM_UNIXTIME(1447430881);select s;SELECT * FROM `t1`;SELECT * FROM `t2`;INSERT INTO `t1` VALUES (111);END;",
...
func RunProcedureRestore(t *testing.T) {
