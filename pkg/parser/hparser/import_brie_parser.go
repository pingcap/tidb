...
func (p *HandParser) parseImportIntoStmt() ast.StmtNode {
	...
	if _, ok := p.accept('('); ok {
		...
		for {
			if tok, ok := p.accept(tokSingleAtIdent); ok {
			...
			} else if tok, ok := p.accept(tokIdentifier); ok {
	...
	if p.peek().Tp == tokStringLit {
		...
		stmt.Select = ts
		p.expect(')')
	} else if p.peek().Tp == tokSelect || p.peek().Tp == tokWith {
...
func (p *HandParser) parseBRIEOptions(stmt *ast.BRIEStmt) {
	for {
		...
		p.accept(tokEq) // optional '=' separator
