...
func (p *HandParser) parseCastTypeInternal() (*types.FieldType, bool) {
	...
	case tokCharType, tokCharacter:
		...
		if _, ok := p.accept(tokBinaryType); ok {
			if tp.GetFlen() != types.UnspecifiedLength {
				tp.SetType(mysql.TypeString)
			}
			setBinaryCastType(tp)
	...
	case tokDatetimeType, tokTimeType:
		p.next()
		var mysqlType byte
		if tok.Tp == tokDatetimeType {
	...
	case tokSigned, tokUnsigned:
		isUnsigned := tok.Tp == tokUnsigned
		p.next()
		// Accept optional INTEGER/INT keyword.
		if _, ok := p.accept(tokIntType); !ok {
			if isUnsigned {
				p.accept(tokInteger)
			}
		}
	...
	case tokDouble, tokReal:
		isReal := tok.Tp == tokReal
		p.next()
