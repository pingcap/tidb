...
func (p *HandParser) parseInsertStmt(isReplace bool) *ast.InsertStmt {
	...
	case '(': // (SELECT ...) or (VALUES ...)
		p.next()
		if p.peek().Tp == tokSelect {
			sel := p.parseSelectStmt()
			stmt.Select = p.maybeParseUnion(sel)
			p.expect(')')
			markIsInBraces(stmt.Select)
		} else if p.peek().Tp == tokValues { // (VALUES ...)
...
func (p *HandParser) parseValueList(isReplace, enforceRow bool) [][]ast.ExprNode {
	p.expectAny(tokValues, tokValue)

	var lists [][]ast.ExprNode
	for {
		// (val1, val2, ...) || ROW(val1, val2, ...)
		if _, ok := p.accept(tokRow); ok {
			// consume ROW
		} else if enforceRow {
			p.expect(tokRow)
		}
...
func (p *HandParser) parseNonTransactionalDMLStmt() ast.StmtNode {
	...
	// DRY RUN
	if _, ok := p.accept(tokDry); ok {
		p.expect(tokRun)
		stmt.DryRun = ast.DryRunSplitDml
		if ok := p.peek().Tp == tokQuery; ok {
			p.next()
