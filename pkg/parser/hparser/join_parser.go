...
func (p *HandParser) parseJoinType() (joinType ast.JoinType, natural bool, straight bool, commaJoin bool) {
	...
	case tokLeft:
		p.next()
		p.accept(tokOuter) // optional OUTER
		p.expect(tokJoin)
		return ast.LeftJoin, false, false, false
...
func (p *HandParser) parseTableSource() ast.ResultSetNode {
	...
	case '(':
		p.next()
		if p.peek().Tp == tokSelect || p.peek().Tp == tokWith || p.peek().Tp == tokTable || p.peek().Tp == tokValues {
	...
	if _, ok := p.accept(tokPartition); ok {
		...
		var names []ast.CIStr
		for {
			ident, ok := p.expect(tokIdentifier)
			if !ok {
				return nil
			}
	...
	if p.peek().Tp == tokAsOf {
		...
		if !isIdentLike(aliasTok.Tp) || aliasTok.Tp == tokStringLit {
	...
	if p.peek().Tp == tokTableSample && isResTableName {
		p.next()
		...
		} else if p.peek().Tp == tokRegion || p.peek().Tp == tokRegions {
			p.next()
...
func (p *HandParser) parseTableName() *ast.TableName {
	...
	// All keyword/identifier tokens have Tp >= tokIdentifier and carry their text in Lit.
	if !isIdentLike(tok.Tp) && tok.Tp != tokUnderscoreCS {
	...
	if p.peek().Tp == '.' && p.peekN(1).Tp != '*' {
		p.next() // consume '.'
		nextTok := p.next()
		if nextTok.Tp < tokIdentifier && nextTok.Tp != tokUnderscoreCS {
...
func (p *HandParser) parseIndexHintsInto(tn *ast.TableName) {
	for p.peek().Tp == tokUse || p.peek().Tp == tokIgnore || p.peek().Tp == tokForce {
