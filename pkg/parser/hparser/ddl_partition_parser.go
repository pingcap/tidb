...
func (p *HandParser) parsePartitionOptions() *ast.PartitionOptions {
	...
	case tokRange:
		...
		if _, ok := p.acceptAny(tokColumns, tokFields); ok {
	...
	// [SUBPARTITION BY ...]
	if p.peekKeyword(tokSubPartition, "SUBPARTITION") {
		p.next()
		p.expect(tokBy)
		...
		// [SUBPARTITIONS num]
		if _, ok := p.acceptKeyword(tokSubPartitions, "SUBPARTITIONS"); ok {
	...
	// [PARTITIONS num]
	if _, ok := p.acceptKeyword(tokPartitions, "PARTITIONS"); ok {
...
func (p *HandParser) parseSplitIndexOption() *ast.SplitIndexOption {
	...
	if _, ok := p.accept(tokPrimary); ok {
		p.expect(tokKey)
		opt.PrimaryKey = true
		...
		if tp := p.peek().Tp; tp == tokBetween || tp == tokBy {
			opt.TableLevel = true
		} else {
...
func (p *HandParser) parseSplitOption() *ast.SplitOption {
	...
	} else if _, ok := p.accept(tokBy); ok {
		// BY ...
		// (val1), (val2) ...
		for {
