...
func (p *HandParser) parseKillStmt() ast.StmtNode {
	p.next() // consume KILL (may be tokKill or tokIdentifier)
	stmt := Alloc[ast.KillStmt](p.arena)

	// Check for TIDB prefix
	if _, ok := p.acceptKeyword(tokTidb, "TIDB"); ok {
		stmt.TiDBExtension = true
	}

	// Optional CONNECTION or QUERY
	if _, ok := p.accept(tokConnection); ok {
		// CONNECTION is the default (Query=false)
	} else if _, ok := p.accept(tokQuery); ok {
		stmt.Query = true
	}
...
func (p *HandParser) parseFlushStmt() ast.StmtNode {
	...
	// Flush target
	switch p.peek().Tp {
	case tokTables, tokTable:
		p.next()
		stmt.Tp = ast.FlushTables
		// Optional table names
		if p.peek().IsIdent() && p.peek().Tp != tokWith {
			for {
...
func (p *HandParser) parseCreateBindingStmt(globalScope bool) ast.StmtNode {
	...
	} else if _, ok := p.accept(tokFrom); ok {
	...
	} else if _, ok := p.accept(tokUsing); ok {
...
func (p *HandParser) parseCalibrateResourceStmt() ast.StmtNode {
	...
	for {
		...
		switch {
		case pk.Tp == tokWorkload || pk.IsKeyword("WORKLOAD"):
			p.next()
			wlTok := p.next()
		...
		case pk.Tp == tokStartTime || pk.IsKeyword("START_TIME") ||
			pk.Tp == tokEndTime || pk.IsKeyword("END_TIME"):
			var optTp ast.DynamicCalibrateType
			if pk.Tp == tokStartTime || pk.IsKeyword("START_TIME") {
		...
		case pk.IsKeyword("DURATION"):
			...
			} else if _, ok := p.accept(tokInterval); ok {
				opt.Ts = p.parseExpression(precNone)
