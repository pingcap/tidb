...
func (p *HandParser) parseInfixExpr(left ast.ExprNode, minPrec int) ast.ExprNode {
	for {
		...
		case tokNot:
			...
			nextTp := p.peekN(1).Tp
			if nextTp != tokIn && nextTp != tokLike && nextTp != tokILike &&
				nextTp != tokBetween && nextTp != tokRegexp && nextTp != tokRlike {
				return left
			}
		...
		case tokJss, tokJuss: // -> or ->> (JSON extract / unquote+extract)
			if minPrec > precPredicate {
				return left
			}
			left = p.parseJSONExtract(left, tok.Tp == tokJuss)
			if left == nil {
				return nil
			}
			continue

		case tokMemberOf: // MEMBER OF (expr) - lexer merges MEMBER+OF into single token
			if minPrec > precPredicate {
				return left
			}
			...
			for p.peek().Tp == tokJss || p.peek().Tp == tokJuss {
				arg = p.parseJSONExtract(arg, p.peek().Tp == tokJuss)
				if arg == nil {
					return nil
				}
			}
...
func (p *HandParser) parsePrefixExpr(minPrec int) ast.ExprNode {
	...
	switch tok.Tp {
	case tokIntLit, tokFloatLit, tokDecLit, tokStringLit, tokHexLit, tokBitLit:
	...
	case tokTrue, tokFalse:
		return p.newValueExpr(p.next().Tp == tokTrue)

	case tokUtcTimestamp, tokUtcTime, tokUtcDate:
		return p.parseTimeFunc(p.peek().Lit)

	case tokUnderscoreCS:
	...
	case tokNot2:
		// tokNot2 is emitted by the lexer when HIGH_NOT_PRECEDENCE mode is set.
...
func (p *HandParser) parseLiteral() ast.ExprNode {
	...
	switch tok.Tp {
	case tokIntLit, tokFloatLit, tokDecLit:
		return p.newValueExpr(tok.Item)
	case tokStringLit:
		return p.newValueExpr(tok.Lit)
	case tokHexLit, tokBitLit:
	...
	default:
		p.error(tok.Offset, "unexpected literal token %d", tok.Tp)
		return nil
	}
}
...
func (p *HandParser) parseIdentOrFuncCall() ast.ExprNode {
	...
	if p.peek().Tp == '.' && p.peekN(1).Tp >= tokIdentifier && p.peekN(2).Tp == '(' {
		p.next()                // consume '.'
...
func (p *HandParser) parseLikeExpr(left ast.ExprNode, not bool, isLike bool) ast.ExprNode {
	...
	if _, ok := p.acceptKeyword(tokEscape, "escape"); ok {
		escTok := p.next()
...
func (p *HandParser) newValueExpr(val any) ast.ExprNode {
	return ast.NewValueExpr(val, p.charset, p.collation)
}

// newLiteralExpr creates a literal node from a parse function (hex or bit).
func (p *HandParser) newLiteralExpr(parse func(string) (any, error), lit string) ast.ExprNode {
	val, err := parse(lit)
	if err != nil {
		p.errs = append(p.errs, err)
		return p.newValueExpr(nil)
...
func (p *HandParser) parseCharsetIntroducer() ast.ExprNode {
	...
	case tokHexLit, tokBitLit:
		p.next()
		var parseFn func(string) (any, error)
		if valTok.Tp == tokHexLit {
...
func (p *HandParser) parseCurDateFunc() ast.ExprNode {
	tok := p.next() // consume builtinCurDate token
	p.expect('(')
	p.expect(')')
...
func (p *HandParser) parseMatchAgainstExpr() ast.ExprNode {
	...
	if _, ok := p.accept(tokIn); ok {
		...
		if _, ok := p.accept(tokBoolean); ok {
			p.expect(tokMode)
...
func (p *HandParser) parseDateArith(left ast.ExprNode, opCode opcode.Op) ast.ExprNode {
	if (opCode != opcode.Plus && opCode != opcode.Minus) || p.peek().Tp != tokInterval || p.peekN(1).Tp == '(' {
		return nil
	}
