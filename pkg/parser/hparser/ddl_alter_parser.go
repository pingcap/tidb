...
func (p *HandParser) parseAlterTableStmt() ast.StmtNode {
	...
	if _, ok := p.accept(tokCompact); ok {
	...
	for {
		...
		if p.peek().Tp == tokPartition && p.peekN(1).Tp == tokBy {
			break
		}
		if p.peek().Tp == tokRemove {
			break
		}
	}
...
func (p *HandParser) parseAlterTableSpec() *ast.AlterTableSpec {
	...
	case tokModify, tokChange:
		isChange := p.next().Tp == tokChange
		p.accept(tokColumn)
		if isChange {
	...
	case tokSet:
		...
		isHypo := false
		if p.peekN(1).IsKeyword("HYPO") && p.peekN(2).Tp == tokTiFlash {
			isHypo = true
		}
		if p.peekN(1).Tp == tokTiFlash || isHypo {
			p.next() // consume SET
			if isHypo {
				p.next() // consume HYPO
			}
...
func (p *HandParser) parseColumnPosition(spec *ast.AlterTableSpec) {
	...
	} else if _, ok := p.accept(tokAfter); ok {
...
func (p *HandParser) parsePartitionDef(partType ast.PartitionType) *ast.PartitionDefinition {
	...
	if _, ok := p.accept(tokValues); ok {
		...
		} else if _, ok := p.accept(tokIn); ok {
			// VALUES IN
		...
		pDef.Clause = &ast.PartitionDefinitionClauseHistory{Current: tok.Tp == tokCurrent}
	}
...
func (p *HandParser) parseSubPartitionDefs(pDef *ast.PartitionDefinition) {
	...
	if next := p.peekN(1); next.Tp != tokSubPartition && !next.IsKeyword("SUBPARTITION") {
		return
	}
	p.next() // consume '('
	for {
		if _, ok := p.acceptKeyword(tokSubPartition, "SUBPARTITION"); !ok {
			break
		}
		spd := &ast.SubPartitionDefinition{}
		if nameTok, ok := p.expectAny(tokIdentifier, tokStringLit); ok {
...
func (p *HandParser) parseSplitRegionSpec(spec *ast.AlterTableSpec) *ast.AlterTableSpec {
	if _, ok := p.accept(tokMaxValue); ok {
	...
	if _, ok := p.accept(tokTable); ok {
	...
	} else if _, ok := p.accept(tokIndex); ok {
		// SPLIT INDEX idx
	...
	} else if _, ok := p.accept(tokRegion); ok {
		...
		// But parseSplitRegionSpec is called after tokSplit.
...
func (p *HandParser) parseAlterAnalyzePartition(table *ast.TableName) *ast.AnalyzeTableStmt {
	...
	if _, ok := p.accept(tokWith); ok {
		for {
			...
			if tok, ok := p.expectAny(tokIntLit, tokDecLit, tokFloatLit); ok {
				opt.Value = ast.NewValueExpr(tok.Item, "", "")
				if _, ok := p.accept(tokBuckets); ok {
					opt.Type = ast.AnalyzeOptNumBuckets
				} else if _, ok := p.accept(tokTopN); ok {
					opt.Type = ast.AnalyzeOptNumTopN
				} else if _, ok := p.accept(tokSampleRate); ok {
					opt.Type = ast.AnalyzeOptSampleRate
				} else if _, ok := p.accept(tokSamples); ok {
					opt.Type = ast.AnalyzeOptNumSamples
				} else if _, ok := p.accept(tokCMSketch); ok {
					if _, ok := p.accept(tokDepth); ok {
						opt.Type = ast.AnalyzeOptCMSketchDepth
					} else if _, ok := p.accept(tokWidth); ok {
