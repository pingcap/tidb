...
func (p *HandParser) parseBeginStmt() ast.StmtNode {
	...
	if _, ok := p.accept(tokBegin); ok {
		// BEGIN [PESSIMISTIC | OPTIMISTIC]
		if tok, ok := p.acceptAny(tokPessimistic, tokOptimistic); ok {
			if tok.Tp == tokPessimistic {
	...
	case tokRead:
		...
		} else if _, ok := p.accept(tokOnly); ok {
			stmt.ReadOnly = true
			if _, ok := p.accept(tokAsOf); ok {
	...
	case tokWith:
		p.next()
		if _, ok := p.accept(tokConsistent); ok {
			p.expect(tokSnapshot)
			// START TRANSACTION WITH CONSISTENT SNAPSHOT — default mode
		} else if _, ok := p.accept(tokCausal); ok {
			p.expect(tokConsistency)
			p.expect(tokOnly)
...
func (p *HandParser) parseRollbackStmt() ast.StmtNode {
	...
	if _, ok := p.accept(tokTo); ok {
		...
		p.accept(tokSavepoint) // optional
		tok := p.next()
...
func (p *HandParser) parseCompletionType() ast.CompletionType {
	...
	case tokAnd:
		...
		p.expect(tokNo)
		p.expect(tokChain)
		if _, ok := p.accept(tokRelease); ok {
...
func (p *HandParser) parseAdminShow(stmt *ast.AdminStmt) ast.StmtNode {
	...
	if tbl != nil && p.peek().Tp == tokNextRowID {
		p.next()
