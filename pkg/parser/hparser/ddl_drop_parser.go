...
func (p *HandParser) parseDropStmt() ast.StmtNode {
	...
	case tokDatabase: // tokSchema alias
		p.next()
	...
	case tokPrepare:
		...
		// parseDeallocateStmt expects tokDeallocate, then tokPrepare.
		// Here we consumed tokDrop. Next is tokPrepare.
		// We can call a helper or handle it here.
		// parseDeallocateStmt: p.expect(tokDeallocate); p.accept(tokPrepare);
...
func (p *HandParser) parseDropTable(tmpKw ast.TemporaryKeyword) ast.StmtNode {
	p.acceptAny(tokTable, tokTables)
...
func (p *HandParser) parseTruncateTableStmt() ast.StmtNode {
	...
	p.expect(tokTruncate)
	p.accept(tokTable) // optional TABLE keyword
	stmt.Table = p.parseTableName()
	if stmt.Table == nil {
		return nil
	}
...
func (p *HandParser) parseAnalyzeTableStmt() ast.StmtNode {
	...
	if p.peek().Tp == tokAll && p.peekN(1).IsKeyword("COLUMNS") {
		p.next() // ALL
		p.next() // COLUMNS
...
func (p *HandParser) parseAnalyzeWithOpts(stmt *ast.AnalyzeTableStmt) {
	...
	for {
		...
		valTok, ok := p.expectAny(tokIntLit, tokDecLit)
		if !ok {
			return
		}
		opt := ast.AnalyzeOpt{}
