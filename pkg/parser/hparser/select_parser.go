...
func (p *HandParser) parseWithStmt() ast.StmtNode {
	...
	if _, ok := p.accept(tokRecursive); ok {
	...
	for {
		...
		if p.peek().Tp == '(' {
			p.next()
			for {
				colTok, ok := p.expect(tokIdentifier)
				if !ok {
					return nil
				}
...
func (p *HandParser) parseSelectOpts(stmt *ast.SelectStmt) {
	...
	for {
		...
		case tokDistinct, tokDistinctRow:
			p.next()
			if opts.ExplicitAll {
				p.error(p.peek().Offset, "Incorrect usage of ALL and DISTINCT")
				return
		...
		case tokStraightJoin:
			p.next()
			opts.StraightJoin = true
		case tokSQLCalcFoundRows:
			p.next()
		...
		case tokSQLNoCache:
			p.next()
			opts.SQLCache = false
		case tokSQLSmallResult:
			p.next()
			opts.SQLSmallResult = true
		case tokSQLBigResult:
			p.next()
			opts.SQLBigResult = true
		case tokSQLBufferResult:
			p.next()
	...
	if p.peek().Tp == tokIdentifier {
		if p.lexer.PeekN(1).Tp == '.' {
			if p.lexer.PeekN(2).Tp == '*' {
				...
				sf.WildCard = wf
				return sf
			} else if p.lexer.PeekN(2).Tp == tokIdentifier &&
				p.lexer.PeekN(3).Tp == '.' &&
				p.lexer.PeekN(4).Tp == '*' {
				// schema.table.*
				schema := p.next()
