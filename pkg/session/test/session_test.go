...
func TestRequestSource(t *testing.T) {
	...
	tk.MustExecWithContext(ctx, "insert into t values(2, 2)")

	// A SET_VAR hint overrides the session value for one statement only, including
	// inside an explicit transaction and across the prepared plan cache.
	tk.MustExecWithContext(ctx, "set @@tidb_request_source_type = 'br'")
	hintCtx := interceptor.WithRPCInterceptor(context.Background(), withCheckInterceptor("external_Select_lightning"))
	brCtx := interceptor.WithRPCInterceptor(context.Background(), withCheckInterceptor("external_Select_br"))
	tk.MustQueryWithContext(hintCtx, "select /*+ set_var(tidb_request_source_type='lightning') */ count(*) from t").Check(testkit.Rows("1"))
	tk.MustQueryWithContext(brCtx, "select count(*) from t").Check(testkit.Rows("1"))
	tk.MustExecWithContext(ctx, "begin")
	tk.MustQueryWithContext(hintCtx, "select /*+ set_var(tidb_request_source_type='lightning') */ b from t where a = 2").Check(testkit.Rows("2"))
	tk.MustQueryWithContext(brCtx, "select b from t where a = 2").Check(testkit.Rows("2"))
	tk.MustExecWithContext(ctx, "rollback")
	tk.MustExecWithContext(ctx, "prepare stmt from 'select /*+ set_var(tidb_request_source_type=\"lightning\") */ count(*) from t where a >= ?'")
	tk.MustExecWithContext(ctx, "set @a = 1")
	tk.MustQueryWithContext(hintCtx, "execute stmt using @a").Check(testkit.Rows("1"))
	tk.MustQueryWithContext(ctx, "select @@last_plan_from_cache").Check(testkit.Rows("0"))
	tk.MustQueryWithContext(hintCtx, "execute stmt using @a").Check(testkit.Rows("1"))
	tk.MustQueryWithContext(ctx, "select @@last_plan_from_cache").Check(testkit.Rows("1"))
	// An optimistic autocommit point get reuses its executor and snapshot, which must pick up the hint as well.
	tk.MustExecWithContext(ctx, "set @@tidb_txn_mode = 'optimistic'")
	tk.MustExecWithContext(ctx, "prepare point_stmt from 'select /*+ set_var(tidb_request_source_type=\"lightning\") */ b from t where a = ?'")
	tk.MustExecWithContext(ctx, "set @a = 2")
	tk.MustQueryWithContext(hintCtx, "execute point_stmt using @a").Check(testkit.Rows("2"))
	tk.MustQueryWithContext(hintCtx, "execute point_stmt using @a").Check(testkit.Rows("2"))
	tk.MustQueryWithContext(brCtx, "select b from t where a = 2").Check(testkit.Rows("2"))
	tk.MustExecWithContext(ctx, "set @@tidb_txn_mode = default")
	// The hint must not leak into the transaction's commit.
	commitCtx := interceptor.WithRPCInterceptor(context.Background(), withCheckInterceptor("external_Commit_br"))
	tk.MustExecWithContext(ctx, "begin")
	tk.MustQueryWithContext(hintCtx, "select /*+ set_var(tidb_request_source_type='lightning') */ b from t where a = 2").Check(testkit.Rows("2"))
	tk.MustExecWithContext(ctx, "insert into t values(3, 3)")
	tk.MustExecWithContext(commitCtx, "commit")
