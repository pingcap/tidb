...
func TestSkipNewerChangeCommit(t *testing.T) {
	store := testkit.CreateMockStore(t)
	testSkipNewerChangeCommit(t, store, true)
	testSkipNewerChangeCommit(t, store, false)
}

func testSkipNewerChangeCommit(t *testing.T, store kv.Storage, setOption bool) {
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	tk1 := testkit.NewTestKit(t, store)
	tk1.MustExec("use test")
	...
	// tk commit, then tk1 commit.
	tk.MustExec("commit")

	// Without kv.SkipNewerChange option, transaction conflicts and this line fails.
	if setOption {
		tk1.MustExec("commit")
		// Check the result to see partially success, with conflict keys ignored.
