...
func prepareBenchSession() (sessionapi.Session, *domain.Domain, kv.Storage) {
	...
	domain, err := sessionpkg.ExportedBootstrapSession(store)
	if err != nil {
		logutil.BgLogger().Fatal(err.Error())
	}
	...
	sessionpkg.ExportedMustExecute(se, "use test")
	return se, domain, store
}

func prepareBenchData(se sessionapi.Session, colType string, valueFormat string, valueCount int) {
	sessionpkg.ExportedMustExecute(se, "drop table if exists t")
	sessionpkg.ExportedMustExecute(se, fmt.Sprintf("create table t (pk int primary key auto_increment, col %s, index idx (col))", colType))
	sessionpkg.ExportedMustExecute(se, "begin")
	for i := range valueCount {
		sessionpkg.ExportedMustExecute(se, "insert t (col) values ("+fmt.Sprintf(valueFormat, i)+")")
	}
	sessionpkg.ExportedMustExecute(se, "commit")
}

func prepareNonclusteredBenchData(se sessionapi.Session, colType string, valueFormat string, valueCount int) {
	sessionpkg.ExportedMustExecute(se, "drop table if exists t")
	sessionpkg.ExportedMustExecute(se, fmt.Sprintf("create table t (pk int primary key /*T![clustered_index] NONCLUSTERED */ auto_increment, col %s, index idx (col))", colType))
	sessionpkg.ExportedMustExecute(se, "begin")
	for i := range valueCount {
		sessionpkg.ExportedMustExecute(se, "insert t (col) values ("+fmt.Sprintf(valueFormat, i)+")")
	}
...
func prepareSortBenchData(se sessionapi.Session, colType string, valueFormat string, valueCount int) {
	sessionpkg.ExportedMustExecute(se, "drop table if exists t")
	sessionpkg.ExportedMustExecute(se, fmt.Sprintf("create table t (pk int primary key auto_increment, col %s)", colType))
	sessionpkg.ExportedMustExecute(se, "begin")
	...
	for i := range valueCount {
		...
		sessionpkg.ExportedMustExecute(se, "insert t (col) values ("+fmt.Sprintf(valueFormat, r.Intn(valueCount))+")")
	}
	sessionpkg.ExportedMustExecute(se, "commit")
}
...
func hasPlan(ctx context.Context, b *testing.B, se sessionapi.Session, plan string) {
	...
	rows, err := sessionpkg.ExportedResultSetToStringSlice(ctx, se, rs[0])
	if err != nil {
		b.Fatal(err)
	}
	for i := range rows {
...
func BenchmarkPointGet(b *testing.B) {
	...
	sessionpkg.ExportedMustExecute(se, "create table t (pk int primary key)")
	sessionpkg.ExportedMustExecute(se, "insert t values (61),(62),(63),(64)")
	b.ResetTimer()
	...
	for range b.N {
		...
		_, err = sessionpkg.ExportedDrainRecordSet(ctx, se, rs[0], alloc)
		if err != nil {
			b.Fatal(err)
		}
...
func BenchmarkPreparedPointGet(b *testing.B) {
	...
	for range b.N {
		...
		_, err = sessionpkg.ExportedDrainRecordSet(ctx, se, rs, alloc)
		if err != nil {
			b.Fatal(err)
		}
		alloc.Reset()
...
func BenchmarkInsertWithIndex(b *testing.B) {
	...
	sessionpkg.ExportedMustExecute(se, `set @@tidb_enable_mutation_checker = 0`)
	sessionpkg.ExportedMustExecute(se, "drop table if exists t")
	sessionpkg.ExportedMustExecute(se, "create table t (pk int primary key, col int, index idx (col))")
	b.ResetTimer()
	for i := range b.N {
		sessionpkg.ExportedMustExecute(se, fmt.Sprintf("insert t values (%d, %d)", i, i))
	}
	b.StopTimer()
}
...
func BenchmarkInsertNoIndex(b *testing.B) {
	...
	sessionpkg.ExportedMustExecute(se, "drop table if exists t")
	sessionpkg.ExportedMustExecute(se, "create table t (pk int primary key, col int)")
	b.ResetTimer()
	for i := range b.N {
		sessionpkg.ExportedMustExecute(se, fmt.Sprintf("insert t values (%d, %d)", i, i))
	}
...
func BenchmarkPartitionPruning(b *testing.B) {
	...
	sessionpkg.ExportedMustExecute(se, `create table t (id int, dt datetime)
...
func BenchmarkRangeColumnPartitionPruning(b *testing.B) {
	...
	sessionpkg.ExportedMustExecute(se, build.String())
	alloc := chunk.NewAllocator()
	_, err := se.Execute(ctx, "analyze table t")
	if err != nil {
		b.Fatal(err)
...
func BenchmarkHashPartitionPruningPointSelect(b *testing.B) {
	...
	alloc := chunk.NewAllocator()
	sessionpkg.ExportedMustExecute(se, `create table t (id int, dt datetime) partition by hash(id) partitions 1024;`)
	b.ResetTimer()
	for range b.N {
...
func BenchmarkInsertIntoSelect(b *testing.B) {
	...
	sessionpkg.ExportedMustExecute(se, `set @@tidb_enable_mutation_checker = 0`)
	sessionpkg.ExportedMustExecute(se, `set @@tmp_table_size = 1000000000`)
	sessionpkg.ExportedMustExecute(se, `create global temporary table tmp (id int, dt varchar(512)) on commit delete rows`)
	sessionpkg.ExportedMustExecute(se, `create table src (id int, dt varchar(512))`)
	for range 100 {
		sessionpkg.ExportedMustExecute(se, "begin")
		for range 100 {
			sessionpkg.ExportedMustExecute(se, "insert into src values (42, repeat('x', 512)), (66, repeat('x', 512))")
		}
		sessionpkg.ExportedMustExecute(se, "commit")
	}
	...
	b.ResetTimer()
	for range b.N {
		sessionpkg.ExportedMustExecute(se, "insert into tmp select * from src")
	}
	b.StopTimer()
}
...
func BenchmarkCompileStmt(b *testing.B) {
	...
	sessionpkg.ExportedMustExecute(se, `CREATE TABLE item1 (
  ...
  z2 datetime DEFAULT NULL)`)
	...
	sessionpkg.ExportedMustExecute(se, `CREATE TABLE item2 like item1`)
...
func BenchmarkAutoIncrement(b *testing.B) {
	...
	sessionpkg.ExportedMustExecute(se, "create table auto_inc (id int unsigned key nonclustered auto_increment) shard_row_id_bits=4 auto_id_cache 1;")
	sessionpkg.ExportedMustExecute(se, "set @@tidb_enable_mutation_checker = false")
	b.ResetTimer()
	for range b.N {
		sessionpkg.ExportedMustExecute(se, "insert into auto_inc values ()")
	}
	b.StopTimer()
}
...
func BenchmarkPipelinedSimpleInsert(b *testing.B) {
	...
	sessionpkg.ExportedMustExecute(se, `create table tmp (id int, dt varchar(512))`)
	sessionpkg.ExportedMustExecute(se, `create table src (id int, dt varchar(512))`)
	for range batchNum {
		sessionpkg.ExportedMustExecute(se, "begin")
		for range batchSize {
			sessionpkg.ExportedMustExecute(se, "insert into src values (42, repeat('x', 512))")
		}
		sessionpkg.ExportedMustExecute(se, "commit")
	}
...
func BenchmarkPipelinedInsertOnDuplicate(b *testing.B) {
	...
	sessionpkg.ExportedMustExecute(se, `create table tmp (id int, dt varchar(512), unique key k1(id))`)
	sessionpkg.ExportedMustExecute(se, `create table src (id int, dt varchar(512))`)
	for i := range batchNum {
		sessionpkg.ExportedMustExecute(se, "begin")
		for lines := range batchSize {
			sessionpkg.ExportedMustExecute(se,
				fmt.Sprintf(
...
func BenchmarkPipelinedUpdate(b *testing.B) {
	...
	sessionpkg.ExportedMustExecute(se, `create table src (id int, dt varchar(128))`)
	for range batchNum {
		sessionpkg.ExportedMustExecute(se, "begin")
		for range batchSize {
			sessionpkg.ExportedMustExecute(se, "insert into src values (42, repeat('x', 128))")
		}
		sessionpkg.ExportedMustExecute(se, "commit")
	}
