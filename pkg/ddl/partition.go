...
func dropLabelRules(ctx context.Context, d *ddlCtx, schemaName, tableName string, partNames []string) error {
...
func (w *worker) onDropTablePartition(d *ddlCtx, t *meta.Meta, job *model.Job) (ver int64, _ error) {
	...
	if job.Type == model.ActionAddTablePartition || job.Type == model.ActionReorganizePartition ||
		...
		err = dropLabelRules(w.ctx, d, job.SchemaName, tblInfo.Name.L, pNames)
		if err != nil {
			job.State = model.JobStateCancelled
	...
	case model.StatePublic:
		...
		err = dropLabelRules(w.ctx, d, job.SchemaName, tblInfo.Name.L, partNames)
		if err != nil {
			job.State = model.JobStateCancelled
			return ver, errors.Wrapf(err, "failed to notify PD the label rules")
		}
