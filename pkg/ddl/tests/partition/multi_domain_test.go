...
func TestMultiSchemaReorganizePK(t *testing.T) {
	...
	initFn := func(tkO *testkit.TestKit) {
		tkO.MustExec(fmt.Sprintf(`insert into t values (%d,'%s',%d,%d,%d)`, i, "init O", 6821527184-i, 4185725186-i, 7483634197-i))
		i++
		tkO.MustExec(fmt.Sprintf(`insert into t values (%d,'%s',%d,%d,%d)`, i, "init O", 6821527184-i, 4185725186-i, 7483634197-i))
		i++
	}
	...
	loopFn := func(tkO, tkNO *testkit.TestKit) {
		...
		tkO.MustExec(fmt.Sprintf(`insert into t values (%d,'%s',%d,%d,%d)`, i, schemaState+" O", 6821527184-i, 4185725186-i, 7483634197-i))
		i++
		tkNO.MustExec(fmt.Sprintf(`insert into t values (%d,'%s',%d,%d,%d)`, i, schemaState+" NO", 6821527184-i, 4185725186-i, 7483634197-i))
		i++
	}
...
func TestMultiSchemaReorganizeNoPK(t *testing.T) {
	...
	i := 1
	initFn := func(tkO *testkit.TestKit) {
		tkO.MustExec(fmt.Sprintf(`insert into t values (%d,'%s',repeat('%d', 25),repeat('%d', 25),repeat('%d', 25))`, i, "init O", 6821527184-i, 4185725186-i, 7483634197-i))
		i++
		tkO.MustExec(fmt.Sprintf(`insert into t values (%d,'%s',repeat('%d', 25),repeat('%d', 25),repeat('%d', 25))`, i, "init O", 6821527184-i, 4185725186-i, 7483634197-i))
		i++
	}
	...
	loopFn := func(tkO, tkNO *testkit.TestKit) {
		res := tkO.MustQuery(`select schema_state from information_schema.DDL_JOBS where table_name = 't' order by job_id desc limit 1`)
		schemaState := res.Rows()[0][0].(string)
		tkO.MustExec(fmt.Sprintf(`insert into t values (%d,'%s',repeat('%d', 25),repeat('%d', 25),repeat('%d', 25))`, i, schemaState+" O", 6821527184-i, 4185725186-i, 7483634197-i))
		i++
		tkNO.MustExec(fmt.Sprintf(`insert into t values (%d,'%s',repeat('%d', 25),repeat('%d', 25),repeat('%d', 25))`, i, schemaState+" NO", 6821527184-i, 4185725186-i, 7483634197-i))
		i++
	}
	postFn := func(tkO *testkit.TestKit) {
		require.Equal(t, int(6*2+1), i)
