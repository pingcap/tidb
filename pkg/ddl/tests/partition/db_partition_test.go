...
func TestRemovePartitioningGlobalIndex(t *testing.T) {
	...
	tk.MustExec(`create table t (a int not null, b int, unique key idx_b(b), unique key idx_a(a)) partition by hash(b) partitions 3`)
	tt := external.GetTableByName(t, tk, "test", "t")
	require.Equal(t, 2, len(tt.Meta().Indices))
	...
	//tk.MustExec(`create table t2 (a int not null, b varchar(255), unique key idx_b(b), unique key idx_a(a))`)
