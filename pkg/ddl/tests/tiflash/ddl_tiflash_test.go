...
func (s *tiflashContext) CheckFlashback(tk *testkit.TestKit, t *testing.T) {
	...
	tb, err := s.dom.InfoSchema().TableByName(context.Background(), model.NewCIStr("test"), model.NewCIStr("ddltiflash"))
	require.NoError(t, err)
	require.NotNil(t, tb)
...
func TestTiFlashReplicaPartitionTableNormal(t *testing.T) {
	...
	tb2, err := s.dom.InfoSchema().TableByName(context.Background(), model.NewCIStr("test"), model.NewCIStr("ddltiflash"))
	require.NoError(t, err)
...
func CheckTableAvailableWithTableName(dom *domain.Domain, t *testing.T, count uint64, labels []string, db string, table string) {
	tb, err := dom.InfoSchema().TableByName(context.Background(), model.NewCIStr(db), model.NewCIStr(table))
	require.NoError(t, err)
	replica := tb.Meta().TiFlashReplica
...
func TestSetPlacementRuleWithGCWorker(t *testing.T) {
	...
	tb, err := s.dom.InfoSchema().TableByName(context.Background(), model.NewCIStr("test"), model.NewCIStr("ddltiflash_gc"))
	require.NoError(t, err)
...
func TestTiFlashBatchRateLimiter(t *testing.T) {
	...
	check := func(expected int, total int) {
		cnt := 0
		for i := 0; i < total; i++ {
			tb, err := s.dom.InfoSchema().TableByName(context.Background(), model.NewCIStr("tiflash_ddl_limit"), model.NewCIStr(fmt.Sprintf("t%v", i)))
			require.NoError(t, err)
...
func TestTiFlashProgress(t *testing.T) {
	...
	tb, err := s.dom.InfoSchema().TableByName(context.Background(), model.NewCIStr("tiflash_d"), model.NewCIStr("t"))
	require.NoError(t, err)
	require.NotNil(t, tb)
	...
	tb, _ = s.dom.InfoSchema().TableByName(context.Background(), model.NewCIStr("tiflash_d"), model.NewCIStr("t"))
	infosync.UpdateTiFlashProgressCache(tb.Meta().ID, 5.0)
...
func TestTiFlashProgressAvailableList(t *testing.T) {
	...
	for i := 0; i < tableCount; i++ {
		var err error
		tbls[i], err = s.dom.InfoSchema().TableByName(context.Background(), model.NewCIStr("test"), model.NewCIStr(tableNames[i]))
		require.NoError(t, err)
