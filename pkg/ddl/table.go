...
func (w *worker) onSetTableFlashReplica(jobCtx *jobContext, job *model.Job) (ver int64, _ error) {
	...
	if replicaInfo.Count > 0 {
		...
		// For example, during the process of increasing the number of TiFlash replicas from 2 to 3, or decreasing it from 3 to 2, if the `available` field of the original TiFlash replica is already `True`, its value remains unchanged. In this case, the optimizer can still choose to route queries to TiFlash for execution, avoiding any impact on service continuity.
		available := false
		if tblInfo.TiFlashReplica != nil {
