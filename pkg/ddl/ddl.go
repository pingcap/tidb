...
// Logger is the package-level logger. It will be initialized with a field
// `zap.String(logutil.LogFieldCategory, "ddl")`.
...
func (sv *schemaVersionManager) unlockSchemaVersion(jobID int64) {
	...
	if ownerID == jobID {
		...
		outer:
			for err != nil {
				Logger.Error("unlock schema version", zap.Error(err))
				select {
				case <-sv.ctx.Done():
					break outer
			...
			if err := lockInfo.se.Close(); err != nil {
				Logger.Error("close etcd session", zap.Error(err))
			}
		}
...
func (dc *ddlCtx) isOwner() bool {
	...
	Logger.Debug("check whether is the DDL owner", zap.Bool("isOwner", isOwner), zap.String("selfID", dc.uuid))
	if isOwner {
...
func (dc *ddlCtx) notifyReorgWorkerJobStateChange(job *model.Job) {
	...
	if rc == nil {
		Logger.Warn("cannot find reorgCtx", zap.Int64("Job ID", job.ID))
		return
	}
	Logger.Info("notify reorg worker the job's state",
...
func asyncNotifyEvent(d *ddlCtx, e *statsutil.DDLEvent) {
	if d.ddlEventCh != nil {
		...
		Logger.Warn("fail to notify DDL event", zap.Stringer("event", e))
	}
}
...
func (d *ddl) Stop() error {
	...
	d.close()
	Logger.Info("stop DDL", zap.String("ID", d.uuid))
	return nil
}
...
func (d *ddl) newDeleteRangeManager(mock bool) delRangeManager {
	...
	if !mock {
		...
		Logger.Info("start delRangeManager OK", zap.Bool("is a emulator", !d.store.SupportDeleteRange()))
	} else {
...
func (d *ddl) Start(ctxPool *pools.ResourcePool) error {
	Logger.Info("start DDL", zap.String("ID", d.uuid), zap.Bool("runWorker", config.GetGlobalConfig().Instance.TiDBEnableDDL.Load()))
	...
	d.ownerManager.SetBeOwnerHook(func() {
		...
		if err != nil {
			Logger.Error("error when getting the ddl history count", zap.Error(err))
		}
		d.runningJobs.clear()
	})
	...
	if err := d.stateSyncer.Init(d.ctx); err != nil {
		Logger.Warn("start DDL init state syncer failed", zap.Error(err))
		return errors.Trace(err)
	}
...
func (d *ddl) DisableDDL() error {
	if d.ownerManager.IsOwner() {
		...
		if err != nil {
			Logger.Error("error when GetAllServerInfo", zap.Error(err))
			return err
		}
		if len(serverInfo) <= 1 {
...
func (d *ddl) close() {
	...
	Logger.Info("DDL closed", zap.String("ID", d.uuid), zap.Duration("take time", time.Since(startTime)))
}
...
func (d *ddl) DoDDLJob(ctx sessionctx.Context, job *model.Job) error {
	...
	Logger.Info("start DDL job", zap.Stringer("job", job), zap.String("query", job.Query))
	...
	for {
		...
		case <-d.ctx.Done():
			Logger.Info("DoDDLJob will quit because context done")
		...
		if sessVars.SQLKiller.HandleSignal() == exeerrors.ErrQueryInterrupted {
			...
			if sessVars.StmtCtx.DDLJobID != 0 {
				se, err := d.sessPool.Get()
				if err != nil {
					Logger.Error("get session failed, check again", zap.Error(err))
					continue
				}
				...
				d.sessPool.Put(se)
				if len(errs) > 0 {
					Logger.Warn("error canceling DDL job", zap.Error(errs[0]))
				}
				if err != nil {
					Logger.Warn("Kill command could not cancel DDL job", zap.Error(err))
					continue
				}
			}
		}
		...
		d.sessPool.Put(se)
		if err != nil {
			Logger.Error("get history DDL job failed, check again", zap.Error(err))
			continue
		}
		if historyJob == nil {
			Logger.Debug("DDL job is not in history, maybe not run", zap.Int64("jobID", jobID))
			continue
		}
		...
		if historyJob.IsSynced() {
			...
			if historyJob.ReorgMeta != nil && len(historyJob.ReorgMeta.Warnings) != 0 {
				if len(historyJob.ReorgMeta.Warnings) != len(historyJob.ReorgMeta.WarningsCount) {
					Logger.Info("DDL warnings doesn't match the warnings count", zap.Int64("jobID", jobID))
				} else {
			...
			Logger.Info("DDL job is finished", zap.Int64("jobID", jobID))
			return nil
		}

		if historyJob.Error != nil {
			Logger.Info("DDL job is failed", zap.Int64("jobID", jobID))
...
func (d *ddl) startCleanDeadTableLock() {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				Logger.Info("get dead table lock failed.", zap.Error(err))
				continue
			}
			for se, tables := range deadLockTables {
				err := d.CleanDeadTableLock(tables, se)
				if err != nil {
					Logger.Info("clean dead table lock failed.", zap.Error(err))
				}
			}
		case <-d.ctx.Done():
			return
...
func (d *ddl) SwitchMDL(enable bool) error {
	...
	if err != nil {
		Logger.Warn("switch metadata lock feature", zap.Bool("enable", enable), zap.Error(err))
		return err
	}
	Logger.Info("switch metadata lock feature", zap.Bool("enable", enable))
	return nil
}
...
func (d *ddl) SwitchFastCreateTable(val bool) error {
	...
	Logger.Info("switch fast create table", zap.Bool("val", val))
	return nil
}
...
func delayForAsyncCommit() {
	...
	Logger.Info("sleep before DDL finishes to make async commit and 1PC safe",
...
func processJobs(process func(*sess.Session, *model.Job, model.AdminCommandOperator) (err error),
	...
	for tryN := uint(0); tryN < 3; tryN++ {
		...
		for _, job := range jobs {
			i, ok := jobMap[job.ID]
			if !ok {
				Logger.Debug("Job ID from meta is not consistent with requested job id,",
