...
import (
	...
	resmgr_util "github.com/pingcap/tidb/pkg/resourcemanager/util"
...
func (w *tableScanWorker) scanRecords(task TableScanTask, sender func(IndexRecordChunk)) {
	...
	var (
		...
		execDetails util.ExecDetails
	)
	var ctx context.Context = w.ctx
	if ctx.Value(util.ExecDetailsKey) == nil {
		ctx = context.WithValue(ctx, util.ExecDetailsKey, &execDetails)
	}
	err := wrapInBeginRollback(w.se, func(startTS uint64) error {
		...
		for !done {
			...
			srcChk := w.getChunk()
			done, err = fetchTableScanResult(ctx, w.copCtx.GetBase(), rs, srcChk)
...
func (w *indexIngestWorker) WriteChunk(rs *IndexRecordChunk) (count, bytes int, err error) {
