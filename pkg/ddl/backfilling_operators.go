...
func (w *tableScanWorker) scanRecords(task TableScanTask, sender func(IndexRecordChunk)) error {
	...
	err := wrapInBeginRollback(w.se, func(startTS uint64) error {
		...
		for !done {
			...
			idxResult = IndexRecordChunk{ID: task.ID, Chunk: srcChk, Done: done, tableScanRowCount: tableScanRowCount - lastTableScanRowCount, conditionPushed: conditionPushed}
			if w.cpOp != nil {
