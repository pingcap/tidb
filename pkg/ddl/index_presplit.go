...
func preSplitIndexRegions(
	...
	for i, idxInfo := range allIndexInfos {
		splitArgs, err := evalSplitDatumFromArgs(sctx.GetExprCtx(), tblInfo, idxInfo, indexPartSpecifications[i], splitOpts[i])
		if err != nil {
...
func evalSplitDatumFromArgs(
	...
	tblInfo *model.TableInfo,
	idxInfo *model.IndexInfo,
	idxPartSpecifications []*ast.IndexPartSpecification,
	opt *indexPresplitOpt,
) (*splitArgs, error) {
	if opt == nil {
		return nil, nil
	}
	if len(opt.ValueLists) > 0 {
		...
		for i, valueList := range opt.ValueLists {
			if len(valueList) > len(idxPartSpecifications) {
