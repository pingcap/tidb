...
func checkModifyGeneratedColumn(sctx sessionctx.Context, schemaName pmodel.CIStr, tbl table.Table, oldCol, newCol *table.Column, newColDef *ast.ColumnDef, pos *ast.ColumnPosition) error {
	...
	if newCol.IsGenerated() {
		...
		if err := checkGeneratedColumnDependsOnAutoEmbeddingColumn(newCol.Name.L, dependColNames, originCols); err != nil {
			return errors.Trace(err)
		}
...
func checkGeneratedColumnDependsOnAutoEmbeddingColumn(name string, dependColNames map[string]struct{}, cols []*table.Column) error {
	for _, col := range cols {
		if _, ok := dependColNames[col.Name.L]; !ok {
			continue
		}
		...
		isAutoEmbed, err := isAutoEmbeddingGeneratedColumn(col)
		if err != nil {
			return errors.Trace(err)
		}
		if isAutoEmbed {
			return dbterror.ErrUnsupportedOnGeneratedColumn.GenWithStack("generated column on an auto-embedding column is not supported: column '%s' is generated from '%s'", name, col.Name.L)
		}
	}
	return nil
}

func isAutoEmbeddingGeneratedColumn(col *table.Column) (bool, error) {
	var expr ast.ExprNode
	switch {
	case col.GeneratedExpr != nil:
		expr = col.GeneratedExpr.Internal()
	case len(col.GeneratedExprString) > 0:
		var err error
	...
	return expression.IsAutoEmbedFnCallAST(expr), nil
}
