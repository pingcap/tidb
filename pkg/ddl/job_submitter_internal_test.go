...
func TestGenIDAndInsertJobsWithRetry(t *testing.T) {
	...
	submitter := NewJobSubmitterForTest()
	for i := range threads {
		idx := i
		wg.Run(func() {
			kit := tks[idx]
	...
	gotJobs, err := getAllDDLJobs(ctx, tk.Session())
	require.NoError(t, err)
...
func TestCombinedIDAllocation(t *testing.T) {
	...
	genCreateTblJobW := func(tp model.ActionType, partitionCnt int, idAllocated bool) *JobWrapper {
		return NewJobWrapperWithArgs(
			&model.Job{
				Version: model.GetJobVerInUse(),
				Type:    tp,
			},
	...
	genCreateTblsJobW := func(idAllocated bool, partitionCounts ...int) *JobWrapper {
	...
	genCreateDBJob := func(idAllocated bool) *JobWrapper {
		info := &model.DBInfo{}
		j := &model.Job{
			Version: model.GetJobVerInUse(),
			Type:    model.ActionCreateSchema,
		}
		return NewJobWrapperWithArgs(j, &model.CreateSchemaArgs{DBInfo: info}, idAllocated)
	}

	genRGroupJob := func(idAllocated bool) *JobWrapper {
		info := &model.ResourceGroupInfo{}
		job := &model.Job{
			Version: model.GetJobVerInUse(),
			Type:    model.ActionCreateResourceGroup,
		}
		return NewJobWrapperWithArgs(job, &model.ResourceGroupArgs{
			RGInfo: info,
		}, idAllocated)
	}

	genAlterTblPartitioningJob := func(partCnt int, idAllocated bool) *JobWrapper {
	...
	genTruncPartitionJob := func(partCnt int, idAllocated bool) *JobWrapper {
		oldIDs := make([]int64, partCnt)
		return NewJobWrapperWithArgs(&model.Job{
			Version: model.GetJobVerInUse(),
			Type:    model.ActionTruncateTablePartition,
		}, &model.TruncateTableArgs{OldPartitionIDs: oldIDs}, idAllocated)
	}

	genAddPartitionJob := func(partCnt int, idAllocated bool) *JobWrapper {
		info := &model.PartitionInfo{
			Definitions: make([]model.PartitionDefinition, partCnt),
		}
		return NewJobWrapperWithArgs(&model.Job{
	...
	genReorgOrRemovePartitionJob := func(remove bool, partCnt int, idAllocated bool) *JobWrapper {
		info := &model.PartitionInfo{
			Definitions: make([]model.PartitionDefinition, partCnt),
		}
	...
	genTruncTblJob := func(partCnt int, idAllocated bool) *JobWrapper {
		j := &model.Job{
			Version: model.GetJobVerInUse(),
			Type:    model.ActionTruncateTable,
		}
		args := &model.TruncateTableArgs{OldPartitionIDs: make([]int64, partCnt)}
		return NewJobWrapperWithArgs(j, args, idAllocated)
	}
	...
	t.Run("process one by one", func(t *testing.T) {
		...
		for i, c := range cases {
			...
			require.NoError(t, submitter.GenGIDAndInsertJobsWithRetry(ctx, sess.NewSession(tk.Session()), []*JobWrapper{c.jobW}))
	...
	t.Run("process together", func(t *testing.T) {
		...
		totalRequiredCnt := 0
		jobWs := make([]*JobWrapper, 0, len(cases))
		for _, c := range cases {
