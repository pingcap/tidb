...
func rollingbackAddTiFlashIndex(w *worker, jobCtx *jobContext, job *model.Job, isVector bool) (ver int64, err error) {
	if job.SchemaState == model.StateWriteReorganization {
		...
		ver, err = w.onCreateTiFlashIndex(jobCtx, job, isVector)
	} else {
...
func convertJob2RollbackJob(w *worker, jobCtx *jobContext, job *model.Job) (ver int64, err error) {
	...
	case model.ActionAddVectorIndex:
		ver, err = rollingbackAddTiFlashIndex(w, jobCtx, job, true)
	case model.ActionAddColumnarIndex:
		ver, err = rollingbackAddTiFlashIndex(w, jobCtx, job, false)
