...
func TestCreateTableWithFullTextIndexMetadata(t *testing.T) {
	...
	tk.MustExec("create table t(a text, fulltext index idx(a))")
	...
	require.NoError(t, err)
	likeIdx := likeTbl.Meta().FindIndexByName("idx_ab")
	...
	tk.MustExec("alter table t2 drop column a")
	tk.MustQuery("show create table t2").CheckNotContain("FULLTEXT INDEX `idx_a`")
	tbl, err = dom.InfoSchema().TableByName(context.Background(), pmodel.NewCIStr("test"), pmodel.NewCIStr("t2"))
	require.NoError(t, err)
	require.Nil(t, model.FindColumnInfo(tbl.Meta().Columns, "a"))
	require.Nil(t, tbl.Meta().FindIndexByName("idx_a"))
