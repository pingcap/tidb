...
func TestAddVectorIndexSimple(t *testing.T) {
	...
	testfailpoint.Enable(t, "github.com/pingcap/tidb/pkg/ddl/MockCheckTiFlashIndexProcess", `return(1)`)
	...
	tk.MustContainErrMsg("admin cleanup index t idx", "tiflash index `idx` is not supported for cleanup index")
	...
	tk.MustContainErrMsg("alter table t alter index idx invisible", "Unsupported set tiflash index invisible")
	...
	tk.MustContainErrMsg("alter table t modify column b vector(2)", "[ddl:8200]Unsupported modify column: tiflash indexes on the column")
...
func TestAddColumnarIndexSimple(t *testing.T) {
	...
	tk.MustContainErrMsg("alter table t drop column a;",
		"can't drop column a with Columnar Key covered now")
	...
	tk.MustContainErrMsg("alter table t modify column a smallint", "[ddl:8200]Unsupported modify column: tiflash indexes on the column")
...
func testAddTiFlashIndexRollback(prepareSQL []string, addIdxSQL string, tp model.ActionType, t *testing.T) {
	...
	checkRollbackInfo := func(expectState model.JobState) {
		...
		require.NoError(t, err)
		currJob := jobs[0]
		require.Equal(t, tp, currJob.Type)
	...
	tk1 := testkit.NewTestKit(t, store)
	tk1.MustExec("use test")
	testfailpoint.Enable(t, "github.com/pingcap/tidb/pkg/ddl/MockCheckTiFlashIndexProcess", `return(0)`)
	...
	// Case4: add a tiflash index normally.
	testfailpoint.Enable(t, "github.com/pingcap/tidb/pkg/ddl/MockCheckTiFlashIndexProcess", `return(4)`)
	tk.MustExec(addIdxSQL)
	...
	testfailpoint.Disable(t, "github.com/pingcap/tidb/pkg/ddl/MockCheckTiFlashIndexProcess")
}

func TestAddVectorIndexRollback(t *testing.T) {
	testAddTiFlashIndexRollback([]string{"create table t1 (c1 int, b vector, c vector(3), unique key(c1));",
	...
	}, "alter table t1 add vector index v_idx((VEC_COSINE_DISTANCE(c))) USING HNSW COMMENT 'b comment';", model.ActionAddVectorIndex, t)
}

func TestAddColumnarIndexRollback(t *testing.T) {
	testAddTiFlashIndexRollback([]string{"create table t1 (c1 int, b int, c vector(3), unique key(c1));",
		"alter table t1 set tiflash replica 2 location labels 'a','b';",
		"insert into t1 values (1, 1, '[1,8.88,9.99]'), (2, 2, '[2,8.88,9.99]'), (3, 3, '[3,8.88,9.99]'), (4, 4, '[4,8.88,9.99]')",
	}, "alter table t1 add columnar index c_idx(b) USING INVERTED COMMENT 'b comment';", model.ActionAddColumnarIndex, t)
}
