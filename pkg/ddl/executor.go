...
func newReorgMetaFromVariables(job *model.Job, sctx sessionctx.Context) (*model.DDLReorgMeta, error) {
	...
	logutil.DDLLogger().Info("initialize reorg meta",
		...
		zap.Int("concurrency", reorgMeta.GetConcurrencyOrDefault(int(variable.GetDDLReorgWorkerCounter()))),
		zap.Int("batchSize", reorgMeta.GetBatchSizeOrDefault(int(variable.GetDDLReorgBatchSize()))),
	)
