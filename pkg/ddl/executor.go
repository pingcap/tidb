...
func checkColumnReferencedByPartialCondition(t table.Table, col *model.ColumnInfo) error {
	...
	for _, idx := range t.Indices() {
		for _, referencedCol := range idx.Meta().AffectColumn {
			if referencedCol.Offset == col.Offset {
				return dbterror.ErrModifyColumnReferencedByPartialCondition.GenWithStackByArgs(col.Name.O, idx.Meta().Name.O)
			}
		}
	}
