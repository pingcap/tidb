...
func RunDDLStatementsBackFill(t *testing.T) {
...
func RunDropTableAdminCheckTableFastCheckTable(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test;")
	...
	//nolint:all
	sessVars := seOffCtx.GetSessionVars()
	require.NoError(t, sessVars.SetSystemVar(vardef.TiDBFastCheckTable, vardef.Off))
...
func RunMaintainAffectColumns(t *testing.T) {
...
func RunJobVersionAndGlobalIndexV1SupportForNextGen(t *testing.T) {
