...
func TestTableInfoInProcessingEvent(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("USE test")
	tk.MustExec("DROP TABLE IF EXISTS " + ddl.NotifierTableName)
	...
	se := sess.NewSession(tk.Session())
	tableInfo, err := se.GetDomainInfoSchema().TableInfoByName(pmodel.NewCIStr("test"), pmodel.NewCIStr("t1"))
	require.NoError(t, err)
	...
	event1 := notifier.NewCreateTableEvent(tableInfo)
	err = notifier.PubSchemeChangeToStore(ctx, se, 1, -1, event1, s)
	require.NoError(t, err)
	...
	require.Equal(t, seenChanges[0].GetCreateTableInfo(), tableInfo)
	cancel()
	<-done
}
