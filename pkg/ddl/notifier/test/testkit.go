...
func RunPublishToTableStore(t *testing.T) {
...
func RunBasicPubSub(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
...
func RunDeliverOrderAndCleanup(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("USE test")
	tk.MustExec("DROP TABLE IF EXISTS " + metadef.NotifierTableName)
...
func RunPublishEventError(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("USE test")
	cases := []string{
...
func Test2OwnerForAShortTime(t *testing.T) {
	...
	tk2.MustExec("DELETE FROM test." + metadef.NotifierTableName) // #nosec G202 -- test code with constant table name
	close(waitCh2)
...
func RunPaginatedList(t *testing.T) {
...
func RunBeginTwice(t *testing.T) {
	conf := new(log.Config)
...
func RunHandlersSeePessimisticTxnError(t *testing.T) {
	// 1. One always fails
	...
	failHandler := func(_ context.Context, sctx sessionctx.Context, _ *notifier.SchemaChangeEvent) error {
		...
		_, err := sctx.GetSQLExecutor().Execute(ctx, "INSERT INTO test."+metadef.NotifierTableName+" VALUES(1, -1, 'some', 0)") // #nosec G202 -- test code with constant table name
		return err
	}
	// Always succeeds
...
func RunCommitFailed(t *testing.T) {
