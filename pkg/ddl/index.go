...
func buildIndexColumns(ctx *metabuild.Context, columns []*model.ColumnInfo, indexPartSpecifications []*ast.IndexPartSpecification, tiflashIndexType TiFlashIndexType) ([]*model.IndexColumn, bool, error) {
	// Build offsets.
	...
	for _, ip := range indexPartSpecifications {
		...
		if err := checkIndexColumn(col, ip.Length, ctx != nil && (!ctx.GetSQLMode().HasStrictMode() || ctx.SuppressTooLongIndexErr()), tiflashIndexType); err != nil {
...
func checkIndexColumn(col *model.ColumnInfo, indexColumnLen int, suppressTooLongKeyErr bool, tiflashIndexType TiFlashIndexType) error {
	...
	if col.FieldType.GetType() == mysql.TypeTiDBVectorFloat32 {
		...
		if tiflashIndexType != TiFlashIndexTypeVector {
...
func BuildIndexInfo(
	...
	idxInfo.Columns, idxInfo.MVIndex, err = buildIndexColumns(ctx, allTableColumns, indexPartSpecifications, tiflashIndexType)
	if err != nil {
		return nil, errors.Trace(err)
	}
	...
	if indexOption != nil {
		...
		} else if tiflashIndexType != TiFlashIndexTypeVector && indexOption.Tp == ast.IndexTypeHNSW {
...
func validateAlterIndexVisibility(ctx sessionctx.Context, indexName ast.CIStr, invisible bool, tbl *model.TableInfo) (bool, error) {
	...
	if idx.IsTiFlashLocalIndex() {
		return false, dbterror.ErrGeneralUnsupportedDDL.GenWithStackByArgs("set tiflash index invisible")
	}
	return false, nil
}
...
func checkAndBuildIndexInfo(
	...
	tiflashIndexType TiFlashIndexType, isPK bool, args *model.IndexArg,
) (*model.IndexInfo, error) {
	var err error
...
func (w *worker) onCreateTiFlashIndex(jobCtx *jobContext, job *model.Job, isVector bool) (ver int64, err error) {
	...
	indexInfo, err := checkAndBuildIndexInfo(job, tblInfo, tiFlashIndexType, false, a)
	if err != nil {
		return ver, errors.Trace(err)
	}
	...
	case model.StateWriteReorganization:
		...
		var done bool
		done, ver, err = w.checkTiFlashIndexProcessOnTiFlash(jobCtx, job, tbl, indexInfo)
		if err != nil || !done {
			return ver, err
		}
...
func (w *worker) checkTiFlashIndexProcessOnTiFlash(jobCtx *jobContext, job *model.Job, tbl table.Table, indexInfo *model.IndexInfo,
) (done bool, ver int64, err error) {
	err = w.checkTiFlashIndexProcess(jobCtx, tbl, job, indexInfo)
	if err != nil {
		if dbterror.ErrWaitReorgTimeout.Equal(err) {
			return false, ver, nil
		}
		if !isRetryableJobError(err, job.ErrorCount) {
			logutil.DDLLogger().Warn("run add tiflash index job failed, convert job to rollback", zap.Stringer("job", job), zap.Error(err))
...
func (w *worker) checkTiFlashIndexProcess(jobCtx *jobContext, tbl table.Table, job *model.Job, index *model.IndexInfo) error {
	...
	for {
		...
		case <-ticker.C:
			logutil.DDLLogger().Info(
				"index backfill state running, check tiflash index process",
		...
		isDone, notAddedIndexCnt, addedIndexCnt, err := w.checkTiFlashIndexProcessOnce(jobCtx, tbl, index.ID)
		if err != nil {
...
// checkTiFlashIndexProcessOnce checks the backfill process of a TiFlash index from TiFlash once.
func (w *worker) checkTiFlashIndexProcessOnce(jobCtx *jobContext, tbl table.Table, indexID int64) (
	isDone bool, notAddedIndexCnt, addedIndexCnt int64, err error) {
	failpoint.Inject("MockCheckTiFlashIndexProcess", func(val failpoint.Value) {
		if valInt, ok := val.(int); ok {
			logutil.DDLLogger().Info("MockCheckTiFlashIndexProcess", zap.Int("val", valInt))
			if valInt < 0 {
...
func (w *worker) onCreateIndex(jobCtx *jobContext, job *model.Job, isPK bool) (ver int64, err error) {
	...
	for _, arg := range args.IndexArgs {
		indexInfo, err := checkAndBuildIndexInfo(job, tblInfo, TiFlashIndexTypeInvalid, job.Type == model.ActionAddPrimaryKey, arg)
		if err != nil {
			job.State = model.JobStateCancelled
			return ver, errors.Trace(err)
		}
...
func onDropIndex(jobCtx *jobContext, job *model.Job) (ver int64, _ error) {
	...
	case model.StateDeleteReorganization:
		...
		if job.IsRollingback() {
			...
			dropArgs.IndexArgs[0].IsTiFlashIndex = allIndexInfos[0].IsTiFlashLocalIndex()
