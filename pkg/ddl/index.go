...
func (w *worker) onCreateIndex(d *ddlCtx, t *meta.Meta, job *model.Job, isPK bool) (ver int64, err error) {
	...
	splitOpts := make([]*IndexArgSplitOpt, 1)

	if isPK {
		// Notice: sqlMode and warnings is used to support non-strict mode.
		err = job.DecodeArgs(&unique[0], &indexNames[0], &indexPartSpecifications[0], &indexOption[0], &sqlMode, &warnings, &global[0], &splitOpts[0])
	} else {
		err = job.DecodeArgs(&unique[0], &indexNames[0], &indexPartSpecifications[0], &indexOption[0], &hiddenCols[0], &global[0], &splitOpts[0])
		if err != nil {
			err = job.DecodeArgs(&unique, &indexNames, &indexPartSpecifications, &indexOption, &hiddenCols, &global, &splitOpts)
		}
	}
	if err != nil {
		job.State = model.JobStateCancelled
...
func runIngestReorgJob(w *worker, d *ddlCtx, t *meta.Meta, job *model.Job,
	...
	bc, err = ingest.LitBackCtxMgr.Register(ctx, job.ID, allIndexInfos[0].Unique, nil, discovery, job.ReorgMeta.ResourceGroupName, 0)
	if err != nil {
...
func checkDuplicateForUniqueIndex(ctx context.Context, t table.Table, reorgInfo *reorgInfo, discovery pd.ServiceDiscovery) error {
	...
	for _, elem := range reorgInfo.elements {
		...
		if indexInfo.Unique {
			...
			if bc == nil {
				bc, err = ingest.LitBackCtxMgr.Register(ctx, reorgInfo.ID, indexInfo.Unique, nil, discovery, reorgInfo.ReorgMeta.ResourceGroupName, 0)
				if err != nil {
					return err
				}
			}
