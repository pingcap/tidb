...
func runIngestReorgJob(w *worker, d *ddlCtx, t *meta.Meta, job *model.Job,
	...
	bc, err = ingest.LitBackCtxMgr.Register(ctx, job.ID, allIndexInfos[0].Unique, discovery, job.ReorgMeta.ResourceGroupName, d.ownerManager.ID(), false)
	if err != nil {
...
func checkDuplicateForUniqueIndex(ctx context.Context, t table.Table, reorgInfo *reorgInfo, discovery pd.ServiceDiscovery) error {
	...
	for _, elem := range reorgInfo.elements {
		...
		if indexInfo.Unique {
			ctx := logutil.WithCategory(ctx, "ddl-ingest")
			if bc == nil {
				bc, err = ingest.LitBackCtxMgr.Register(ctx, reorgInfo.ID, indexInfo.Unique, discovery, reorgInfo.ReorgMeta.ResourceGroupName, "", false)
				if err != nil {
					return err
				}
			}
