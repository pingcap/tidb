...
func TestStorageClassAdmission(t *testing.T) {
	...
	config.UpdateGlobal(func(conf *config.Config) {
		conf.EnableStorageClass = false
	})
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("USE test")
	// Admission is controlled only by the server configuration.
	tk.MustGetErrCode("SET GLOBAL tidb_enable_storage_class = ON", 1193)
	...
	options := []string{
		...
		`ENGINE_ATTRIBUTE = '{"storage_class":"STANDARD"}'`,
		`ENGINE_ATTRIBUTE = '{"storage_class":{"tier":"STANDARD","names_in":["future"]}}'`,
	}
	...
	for _, option := range options {
		require.ErrorContains(t, tk.ExecToErr("CREATE TABLE denied (id INT) "+option+partition), "enable-storage-class")
		require.ErrorContains(t, tk.ExecToErr("ALTER TABLE existing "+option), "enable-storage-class")
	}

	// Reject before other options in the same ALTER statement can take effect.
	require.ErrorContains(t, tk.ExecToErr("ALTER TABLE existing COMMENT='changed' STORAGE_CLASS='IA'"), "enable-storage-class")
	...
	require.ErrorContains(t, tk.ExecToErr("ALTER TABLE existing ADD COLUMN v INT, STORAGE_CLASS='IA'"), "enable-storage-class")
	...
	for _, table := range []string{"ia", "scheduled", "future", "standard"} {
		require.ErrorContains(t, tk.ExecToErr("CREATE TABLE copied LIKE "+table), "enable-storage-class")
	...
	// Returning to STANDARD also requires explicit storage-class admission.
	require.ErrorContains(t, tk.ExecToErr("ALTER TABLE ia STORAGE_CLASS='STANDARD'"), "enable-storage-class")
	require.ErrorContains(t, tk.ExecToErr(`ALTER TABLE scheduled ENGINE_ATTRIBUTE = '{"storage_class":"STANDARD"}'`), "enable-storage-class")
	tk.MustExec("CREATE TABLE plain (id INT)")
	tk.MustExec("CREATE TABLE plain_copy LIKE plain")
	config.UpdateGlobal(func(conf *config.Config) {
		conf.EnableStorageClass = true
	})
...
func TestStorageClassVisibility(t *testing.T) {
	...
	for _, enabled := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			config.UpdateGlobal(func(conf *config.Config) {
				conf.EnableStorageClass = enabled
			})
			store := testkit.CreateMockStore(t)
			tk := testkit.NewTestKit(t, store)
