...
func RunCheckpointManager(t *testing.T) {
	...
	mgr.AddChunk(0, []byte{'0', '9'})
	mgr.AddChunk(1, []byte{'1', '9'})
	mgr.UpdateChunk(0, 100, false)
	require.False(t, mgr.IsKeyProcessed([]byte{'0', '9'}))
	mgr.FinishChunk(0, 100)
	require.NoError(t, mgr.AdvanceWatermark(false))
	require.False(t, mgr.IsKeyProcessed([]byte{'0', '9'}))
	mgr.UpdateChunk(0, 100, true)
	mgr.FinishChunk(0, 100)
	require.NoError(t, mgr.AdvanceWatermark(false))
	...
	mgr.UpdateChunk(1, 50, false)
	mgr.UpdateChunk(1, 50, true)
	mgr.FinishChunk(1, 50)
	...
	mgr.AddChunk(2, []byte{'2', '9'})
	mgr.AddChunk(3, []byte{'3', '9'})
	mgr.AddChunk(4, []byte{'4', '9'})
	mgr.UpdateChunk(2, 100, true)
	mgr.UpdateChunk(3, 100, true)
	mgr.UpdateChunk(4, 100, true)
	mgr.FinishChunk(4, 100)
	require.NoError(t, mgr.AdvanceWatermark(true))
	mgr.FinishChunk(3, 100)
	require.NoError(t, mgr.AdvanceWatermark(true))
...
func RunCheckpointManagerUpdateReorg(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	tk.MustExec("insert into mysql.tidb_ddl_reorg (job_id, ele_id, ele_type) values (1, 1, '_idx_');")
...
func RunCheckpointManagerResumeReorg(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	reorgMeta := &ingest.JobReorgMeta{
