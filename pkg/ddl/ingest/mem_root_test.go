...
func TestRiskOfDiskFull(t *testing.T) {
	...
	t.Run("check local sort free disk", func(t *testing.T) {
		err := checkLocalSortFreeDisk("/tmp/local-sort", 20*size.GB, 20*size.GB, 1, 4, size.GB, 2)
		require.NoError(t, err)

		err = checkLocalSortFreeDisk("/tmp/local-sort", 12*size.GB, 20*size.GB, 1, 3, 0, 2)
		require.Error(t, err)

		err = checkLocalSortFreeDisk("/tmp/local-sort", 9*size.GB, 10*size.GB, 2, 4, 2*size.GB, 1)
		require.Error(t, err)
		...
		err = checkLocalSortFreeDisk("/tmp/local-sort", 81*size.GB, 200*size.GB, 1, 40, 40*size.GB, 20)
		require.NoError(t, err)

		err = checkLocalSortFreeDisk("/tmp/local-sort", 80*size.GB, 200*size.GB, 1, 40, 40*size.GB, 20)
		require.Error(t, err)

		err = checkLocalSortFreeDisk("/tmp/local-sort", 21*size.GB, 200*size.GB, 1, 40, 101*size.GB, 20)
		require.NoError(t, err)

		err = checkLocalSortFreeDisk("/tmp/local-sort", size.GB, size.GB, 1, 1, 0, 1)
		require.Error(t, err)
