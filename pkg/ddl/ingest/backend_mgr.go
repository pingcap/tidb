...
func CreateLocalBackend(ctx context.Context, store kv.Storage, job *model.Job, checkDup bool, adjustedWorkerConcurrency int) (*local.BackendConfig, *local.Backend, error) {
	...
	cfg := genConfig(ctx, jobSortPath, LitMemRoot, checkDup, resGroupName, store.GetKeyspace(), concurrency, maxWriteSpeed, job.ReorgMeta.UseCloudStorage)
	...
	ddllogutil.DDLIngestLogger().Info("create local backend for adding index",
		...
		zap.Bool("check duplicate", checkDup))
