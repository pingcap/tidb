...
func TestCheckpointManager(t *testing.T) {
	...
	mgr, err := ingest.NewCheckpointManager(ctx, sessPool, 1, 1, []int64{1}, tmpFolder, &mockGetTSClient{pts: 12, lts: 34})
	require.NoError(t, err)
	defer mgr.Close()
	...
	require.NoError(t, mgr.AdvanceWatermark(true, false))
	require.False(t, mgr.IsKeyProcessed([]byte{'0', '9'}))
	...
	require.NoError(t, mgr.AdvanceWatermark(true, true)) // Mock the data is imported to the storage.
	...
	require.NoError(t, mgr.AdvanceWatermark(true, true))
	require.True(t, mgr.IsKeyProcessed([]byte{'0', '9'}))
