...
func submitTaskAndCheckSuccessForBasic(ctx context.Context, t *testing.T, taskKey string, testContext *testutil.TestContext) int64 {
	...
	return submitTaskAndCheckSuccess(ctx, t, taskKey, scope, testContext, map[proto.Step]int{
		proto.StepOne: 3,
		proto.StepTwo: 1,
	})
}
...
func RunRandomOwnerChangeWithMultipleTasks(t *testing.T) {
...
func RunFrameworkScaleInAndOut(t *testing.T) {
	c := testutil.NewTestDXFContext(t, 5, 16, true)
	seed := time.Now().UnixNano()
	t.Logf("seed: %d", seed)
	random := rand.New(rand.NewSource(seed))
...
func RunFrameworkWithQuery(t *testing.T) {
...
func RunFrameworkCancelTask(t *testing.T) {
	c := testutil.NewTestDXFContext(t, 2, 16, true)
...
func RunFrameworkSubTaskInitEnvFailed(t *testing.T) {
...
func RunOwnerChangeWhenSchedule(t *testing.T) {
...
func RunFrameworkRunSubtaskCancelOrFailed(t *testing.T) {
	c := testutil.NewTestDXFContext(t, 3, 16, true)
	scope := handle.GetTargetScope()
	registerExampleTask(t, c.MockCtrl, testutil.GetMockBasicSchedulerExt(c.MockCtrl), c.TestContext, nil)
	...
	t.Run("meet some error on run subtask", func(t *testing.T) {
		...
		task := testutil.SubmitAndWaitTask(c.Ctx, t, "key2", scope, 1)
		require.Equal(t, proto.TaskStateReverted, task.State)
	})
}

func RunFrameworkCleanUpRoutine(t *testing.T) {
...
func RunTaskCancelledBeforeUpdateTask(t *testing.T) {
	c := testutil.NewTestDXFContext(t, 1, 16, true)
...
func RunDXFAlwaysEnabledOnNextGen(t *testing.T) {
...
func RunMaxRuntimeSlots(t *testing.T) {
	c := testutil.NewTestDXFContext(t, 1, 16, true)
