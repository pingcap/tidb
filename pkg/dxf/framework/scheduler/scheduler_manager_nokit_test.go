...
type legacyCleanerCallRecorder struct {
	calls []int64
}

func (r *legacyCleanerCallRecorder) CleanUp(_ context.Context, task *proto.Task) error {
	r.calls = append(r.calls, task.ID)
	return nil
}
...
func TestSchedulerCleanFinishedTasks(t *testing.T) {
	...
	t.Run("legacy cleanup factory compatibility", func(t *testing.T) {
		ClearCleanerFactory()
		t.Cleanup(ClearCleanerFactory)
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		taskMgr := mock.NewMockTaskManager(ctrl)
		mgr := NewManager(context.Background(), nil, taskMgr, "1", proto.NodeResourceForTest)
		legacyCleaner := &legacyCleanerCallRecorder{}
		RegisterSchedulerCleanUpFactory(proto.TaskTypeExample, func() CleanUpRoutine {
			return legacyCleaner
		})
		tasks := []*proto.Task{{TaskBase: proto.TaskBase{ID: 1, Type: proto.TaskTypeExample}}}
		taskMgr.EXPECT().TransferTasks2History(mgr.ctx, tasks).Return(nil)

		transferredTaskCount, err := mgr.cleanFinishedTasks(tasks)

		require.NoError(t, err)
		require.Equal(t, 1, transferredTaskCount)
		require.Equal(t, []int64{1}, legacyCleaner.calls)
	})
}
