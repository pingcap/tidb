...
func RunUpdateTaskExtraParams(t *testing.T) {
	_, gm, ctx := testutil.InitTableTest(t)
	require.NoError(t, gm.InitMeta(ctx, ":4000", ""))
	id, err := gm.CreateTask(ctx, "key1", "test", "", 4, "", 0, proto.ExtraParams{}, []byte("test"))
	require.NoError(t, err)
...
func RunModifyTask(t *testing.T) {
	_, gm, ctx := testutil.InitTableTest(t)
	require.NoError(t, gm.InitMeta(ctx, ":4000", ""))
	...
	testfailpoint.EnableCall(t, "github.com/pingcap/tidb/pkg/dxf/framework/storage/beforeMoveToModifying", func() {
		if counter.Add(1) == 1 {
			<-ch
			<-ch
		}
	...
	var called bool
	testfailpoint.EnableCall(t, "github.com/pingcap/tidb/pkg/dxf/framework/storage/beforeModifiedTask", func() {
		if called {
			return
		}
		called = true
