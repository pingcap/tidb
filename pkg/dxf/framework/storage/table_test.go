...
func TestGetImportIntoJobInfoFromHistory(t *testing.T) {
	_, gm, ctx := testutil.InitTableTest(t)
	require.NoError(t, gm.InitMeta(ctx, ":4000", ""))
	...
	taskID, err := gm.CreateTask(ctx, taskKey, proto.ImportInto, keyspace, 8, "", 4, proto.ExtraParams{}, taskMeta)
	require.NoError(t, err)

	encodeID := testutil.InsertSubtask(t, gm, taskID, proto.ImportStepEncodeAndSort, "tidb-1",
		[]byte(`{"kv-group":"data"}`), proto.SubtaskStateSucceed, proto.ImportInto, 8)
	dataID := testutil.InsertSubtask(t, gm, taskID, proto.ImportStepWriteAndIngest, "tidb-1",
		[]byte(`{"kv-group":"data"}`), proto.SubtaskStateSucceed, proto.ImportInto, 8)
	...
	require.NoError(t, err)
	require.NoError(t, gm.TransferTasks2History(ctx, []*proto.Task{task}))

	info, err := gm.GetImportIntoJobInfoFromHistory(ctx, keyspace, jobID)
	require.NoError(t, err)
	...
	require.Equal(t, "0.5GiB", info.IndexKVSize)
	...
	require.Equal(t, "00:40:00", info.Duration.Total)
	require.Equal(t, "00:10:00", info.Duration.Encode)
	require.Equal(t, "00:30:00", info.Duration.Ingest)
	...
	_, err = gm.GetImportIntoJobInfoFromHistory(ctx, keyspace, jobID+1)
