...
// exportColumns returns the stored columns to export, in schema order. Virtual
// generated columns are rejected at submit time.
...
// rows in handle order. SetConcurrency lets the cop client fetch regions in
// parallel while distsql merges the result back into handle order.
func buildScan(ctx context.Context, exprCtx *exprstatic.ExprContext, distCtx *distsqlctx.DistSQLContext,
	...
	snapshotTS uint64, concurrency int, start, end kv.Key) (distsql.SelectResult, error) {
