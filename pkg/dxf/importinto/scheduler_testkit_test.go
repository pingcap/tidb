...
func TestSchedulerExtGlobalSort(t *testing.T) {
	t.Run("files", func(t *testing.T) { testSchedulerExtGlobalSort(t, false) })
	t.Run("query", func(t *testing.T) { testSchedulerExtGlobalSort(t, true) })
}

func testSchedulerExtGlobalSort(t *testing.T, fromQuery bool) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	sourceStep, sourceSubtasks := proto.ImportStepEncodeAndSort, 2
	if fromQuery {
		...
		sourceStep, sourceSubtasks = proto.ImportStepQuery, 1
	}
	bs, err := logicalPlan.ToTaskMeta()
	require.NoError(t, err)
	task := &proto.Task{
