...
func (s *testStorage) WalkDir(
	...
	if s.walkNames != nil {
		for i, name := range s.walkNames {
			if err := fn(name, 0); err != nil {
				return err
			}
			if s.afterWalkName != nil {
				s.afterWalkName(i)
			}
		}
		return nil
...
func TestReconcileFiles(t *testing.T) {
	...
	t.Run("mixed decisions", func(t *testing.T) {
		...
		stats, err := reconcileFiles(context.Background(), store, getter, now)
		require.NoError(t, err)
		...
		require.Zero(t, stats.missingMetadata)
	...
	t.Run("object bound keeps current callback", func(t *testing.T) {
		...
		stats, err := reconcileFiles(context.Background(), store, getter, now)
		require.NoError(t, err)
		require.Equal(t, []int{1, 1}, lookupSizes)
		require.Equal(t, int64(maxObjectsPerFlush+1), stats.deletedFiles)
		require.Len(t, store.deleteCalls, 2)
		require.Len(t, store.deleteCalls[0], maxObjectsPerFlush)
		require.Len(t, store.deleteCalls[1], 1)
	})
	...
	t.Run("later delete failure and retry", func(t *testing.T) {
		...
		require.Equal(t, int64(maxObjectsPerFlush), stats.deletedFiles)
		require.Zero(t, stats.missingMetadata)
		...
		stats, err = reconcileFiles(context.Background(), store, getter, now)
		require.NoError(t, err)
		require.Equal(t, int64(2), stats.deletedFiles)
		require.Equal(t, int64(1), stats.missingMetadata)
		require.Equal(t, int64(1), stats.filesWithoutTask)
	})
	...
	t.Run("missing metadata warning is bounded", func(t *testing.T) {
		...
		stats, err := reconcileFiles(context.Background(), store, taskInfoGetterFunc(
		...
		require.Equal(t, int64(20), stats.missingMetadata)
		require.Equal(t, int64(20), stats.filesWithoutTask)
		...
		require.Len(t, stats.firstFilesWithoutTask, maxLoggedUnexpectedFiles)

		warningLogs := logs.FilterMessage("conflict-row cleanup deleted objects without task metadata").All()
		require.Len(t, warningLogs, 1)
		require.Equal(t, int64(20), warningLogs[0].ContextMap()["missing-metadata"])
		require.Len(t, warningLogs[0].ContextMap()["file-samples"], maxLoggedUnexpectedFiles)
		summaryLogs := logs.FilterMessage("finished conflict-row cleanup reconciliation").All()
	...
	t.Run("canceled context", func(t *testing.T) {
		...
		cancel()
		stats, err := reconcileFiles(ctx, store, taskInfoGetterFunc(
			func(context.Context, []int64) (map[int64]storage.TaskCleanupInfo, error) {
				require.FailNow(t, "metadata lookup should not run")
				return nil, nil
			}), now)
	...
	t.Run("cancellation stops context-ignoring walk and later operations", func(t *testing.T) {
		t.Run("before final metadata lookup", func(t *testing.T) {
			ctx, cancel := context.WithCancel(context.Background())
			store := &testStorage{
				Storage:   objstore.NewMemStorage(),
				walkNames: []string{"conflicted-rows/1/only"},
				afterWalkName: func(int) {
					cancel()
				},
			}
		...
		t.Run("after metadata lookup", func(t *testing.T) {
			ctx, cancel := context.WithCancel(context.Background())
			store := &testStorage{
				Storage:   objstore.NewMemStorage(),
				walkNames: []string{"conflicted-rows/1/only"},
		...
		t.Run("after retained metadata lookup", func(t *testing.T) {
			ctx, cancel := context.WithCancel(context.Background())
			store := &testStorage{
				Storage:   objstore.NewMemStorage(),
				walkNames: []string{"conflicted-rows/1/only"},
			...
			stats, err := reconcileFiles(ctx, store, taskInfoGetterFunc(
				func(context.Context, []int64) (map[int64]storage.TaskCleanupInfo, error) {
					cancel()
					return map[int64]storage.TaskCleanupInfo{1: {
						ID: 1, Type: proto.ImportInto, State: proto.TaskStateRunning,
					}}, nil
				}), now)
			require.ErrorIs(t, err, context.Canceled)
			require.Empty(t, store.deleteCalls)
			require.Zero(t, stats.candidateTasks)
			require.Zero(t, stats.retainedTasks)
			require.Zero(t, stats.deletedTasks)
