...
func (s *PostProcessSpec) ToSubtaskMeta(planCtx planner.PlanCtx) ([]byte, error) {
	encodeStep := getSourceStep(planCtx.GlobalSort, s.fromQuery)
...
func generateMergeSortSpecs(planCtx planner.PlanCtx, p *LogicalPlan) ([]planner.PipelineSpec, error) {
	...
	sourceStep := getSourceStep(true, p.Plan.Query != nil)
	kvMetas, err := getSortedKVMetasOfEncodeStep(planCtx.Ctx, planCtx.PreviousSubtaskMetas[sourceStep], store)
	if err != nil {
		return nil, err
	}
...
func getSortedKVMetasForIngest(planCtx planner.PlanCtx, p *LogicalPlan, store storeapi.Storage) (map[string]*globalsort.SortedKVMeta, error) {
	...
	sourceStep := getSourceStep(true, p.Plan.Query != nil)
	kvMetasOfEncodeStep, err := getSortedKVMetasOfEncodeStep(planCtx.Ctx, planCtx.PreviousSubtaskMetas[sourceStep], store)
	if err != nil {
		return nil, err
	}
...
func generateCollectConflictsSpecs(planCtx planner.PlanCtx, p *LogicalPlan) ([]planner.PipelineSpec, error) {
	...
	defer store.Close()
	groupConflictInfos, err := collectConflictInfos(planCtx.Ctx, store, planCtx, getSourceStep(true, p.Plan.Query != nil))
	if err != nil {
		return nil, err
	}
...
func collectConflictInfos(ctx context.Context, store storeapi.Storage, planCtx planner.PlanCtx, sourceStep proto.Step) (*KVGroupConflictInfos, error) {
	m := &KVGroupConflictInfos{}
	for _, subTaskMeta := range planCtx.PreviousSubtaskMetas[sourceStep] {
		var stepMeta ImportStepMeta
		err := json.Unmarshal(subTaskMeta, &stepMeta)
		if err != nil {
			return nil, errors.Trace(err)
