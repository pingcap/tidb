...
func TestMockEmbedderOptions(t *testing.T) {
	embedder := NewMockEmbedder()
	ctx := context.Background()

	opts := map[string]any{"plus": 1.5, "delay": "1ms"}
	...
	tests := []struct {
		...
		{name: "unknown option", opts: map[string]any{"dimensions": 3}, err: "unknown option dimensions"},
		{name: "invalid plus type", opts: map[string]any{"plus": "1"}, err: "invalid type for 'plus' option"},
		{name: "invalid delay type", opts: map[string]any{"delay": 1}, err: "invalid type for 'delay' option"},
		{name: "invalid delay value", opts: map[string]any{"delay": "later"}, err: "invalid delay duration"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := embedder.CreateEmbeddings(ctx, "json", []string{"[]"}, tt.opts)
