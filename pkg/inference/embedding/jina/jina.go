...
const (
	// DefaultAPIBaseURL is the default base URL for JinaAI embeddings API.
...
func (e *Embedder) CreateEmbeddings(ctx context.Context, model string, texts []string, opts map[string]any) ([][]float32, error) {
	...
	if resp.StatusCode != http.StatusOK {
		logutil.BgLogger().Error("JinaAI API request failed",
		...
		if err := json.Unmarshal(body, &errResp); err == nil && errResp.Detail != "" {
			return nil, fmt.Errorf("JinaAI: %s", errResp.Detail)
		}
