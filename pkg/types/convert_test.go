...
func TestConvertToString(t *testing.T) {
	...
	for _, tt := range tests {
		...
		outputDatum, err := inputDatum.ConvertTo(NewCtxForText(t), ft)
...
func TestConvertToStringWithCheck(t *testing.T) {
	...
	for _, tt := range tests {
		...
		inputDatum := NewStringDatum(tt.input)
		ctx := NewCtxForText(t, tt.newFlags(StrictFlags))
...
func testSelectUpdateDeleteEmptyStringError(t *testing.T) {
	ctx := NewCtxForText(t, FlagTruncateAsWarning)
	str := ""
	expect := 0
...
func accept(t *testing.T, tp byte, value interface{}, unsigned bool, expected string) {
	...
	d := NewDatum(value)
	ctx := NewCtxForText(t, DefaultStmtFlags, FlagIgnoreTruncateErr)
...
func deny(t *testing.T, tp byte, value interface{}, unsigned bool, expected string) {
	...
	d := NewDatum(value)
	ctx := NewCtxForText(t, DefaultStmtFlags)
	casted, err := d.ConvertTo(ctx, ft)
	require.Error(t, err)
	if casted.IsNull() {
		require.Equal(t, "<nil>", expected)
...
func TestGetValidInt(t *testing.T) {
	...
	ws := &WarningsInTest{}
	ctx := NewCtxForText(t, DefaultStmtFlags|FlagTruncateAsWarning)
	ctx = ctx.WithAppendWarning(ws.AppendWarning)
	warningCount := 0
...
func TestConvertJSONToInt(t *testing.T) {
	...
	for _, tt := range tests {
		...
		ctx := NewCtxForText(t)
		casted, err := ConvertJSONToInt64(ctx, j, false)
		if tt.err {
