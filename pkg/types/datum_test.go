...
// TestToGeometry test MySQL Geometry types
//
// Geometry layout example for "POINT(1 2)":
// Full value: 0x000000000101000000000000000000F03F0000000000000040
// SRID:         00000000
// WKB:                  0101000000000000000000F03F0000000000000040
// WKB, little endinan:  01
// WKB type: point:        01000000
// WKB coord:                      000000000000F03F
// WKB coord:                                      0000000000000040
//
// See also: https://dev.mysql.com/doc/refman/8.0/en/gis-data-formats.html#gis-internal-format
func TestToGeometry(t *testing.T) {
	ft := NewFieldType(mysql.TypeGeometry)
	...
	tests := []struct {
		...
		{NewIntDatum(1), ``, false},             // wrong type
		{NewBytesDatum([]byte("0")), ``, false}, // too short
		{NewBytesDatum(
			// POINT(1 2), SRID 0
			[]byte{0x0, 0x0, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf0, 0x3f, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x40}),
			[]byte{0x0, 0x0, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf0, 0x3f, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x40},
			true,
		},
		{NewBinaryLiteralDatum([]byte{0x0}), ``, false},                // too short
		{NewBinaryLiteralDatum([]byte{0x0, 0x0, 0x0, 0x0}), ``, false}, // 4-byte srid, but 0 bytes geometry
		{NewBinaryLiteralDatum(
			// POINT(1 2), SRID 0
			[]byte{0x0, 0x0, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf0, 0x3f, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x40},
		),
			[]byte{0x0, 0x0, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf0, 0x3f, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x40},
		...
		{NewBinaryLiteralDatum(
			// POINT(1 2), SRID 0, endianness marker (byte 5, first byte of WKB) not 0x1
			[]byte{0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf0, 0x3f, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x40},
		),
			``,
			false,
		},
		{NewBinaryLiteralDatum(
			// POINT(1 2), SRID 4326
			[]byte{0xE6, 0x10, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf0, 0x3f, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x40},
		),
			[]byte{0xe6, 0x10, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf0, 0x3f, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x40},
			true,
		},
	...
	for _, tt := range tests {
		...
		if tt.success {
			require.NoError(t, err)
			require.Equal(t, tt.expected, obtain.b)
		} else {
			require.Error(t, err)
		}
	}
