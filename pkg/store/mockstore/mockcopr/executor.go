...
func evalBool(exprs []expression.Expression, row []types.Datum, ctx sessionctx.Context) (bool, error) {
	for _, expr := range exprs {
		data, err := expr.Eval(ctx, chunk.MutRowFromDatums(row).ToRow())
		if err != nil {
		...
		isBool, err = expression.HandleOverflowOnSelection(expression.NewEvalVars(ctx.(sessionctx.Context).GetSessionVars()).StmtCtx, isBool, err)
		if err != nil {
			return false, errors.Trace(err)
		}
		if isBool == 0 {
