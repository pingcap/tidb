...
// see tikv/src/coprocessor/util.rs for more detail
func RunIsPrefixNext(t *testing.T) {
...
func RunClosureExecutor(t *testing.T) {
	data, err := prepareTestTableData(keyNumber, tableID)
	require.NoError(t, err)
	store, clean, err := newTestStore("cop_handler_test_db", "cop_handler_test_log")
	require.NoError(t, err)
...
func RunMppExecutor(t *testing.T) {
	data, err := prepareTestTableData(keyNumber, tableID)
	require.NoError(t, err)
	store, clean, err := newTestStore("cop_handler_test_db", "cop_handler_test_log")
	require.NoError(t, err)
...
func RunExchSenderExecNextReturnsWhenCtxCanceledBeforeTunnelConnected(t *testing.T) {
