...
func buildCopTasks(bo *Backoffer, ranges *KeyRanges, opt *buildCopTaskOpt) ([]*copTask, error) {
	...
	if opt.skipBuckets {
		...
		for locIdx, loc := range locs {
			...
			pdLoc, pdErr := cache.RegionCache.LocateRegionByID(bo.TiKVBackoffer(), loc.Location.Region.GetID())
			if pdErr != nil {
...
func buildExceedsBoundDiagFields(
	...
	pdLoc, pdErr := regionCache.RegionCache.LocateRegionByID(bo.TiKVBackoffer(), task.region.GetID())
	if pdErr != nil {
		fields = append(fields, zap.Error(pdErr))
	} else {
		fields = append(fields,
