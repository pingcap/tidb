...
// tiflash-only rounds.
//
// Round 1's bottom-up search picks a storage engine per DataSource through
// local cost comparisons, so a plan that is homogeneous across the whole
// statement is never costed as a unit. These rounds build that missing global
// alternative and let the normal strict-`<` cost comparison decide.
//
// They arm only for statements with a join or an aggregation: a wholesale
// engine switch has little to offer a plain scan, and this keeps the extra
// rounds off the OLTP fast path. They never run when a READ_FROM_STORAGE hint
// pins an engine (the cost comparison must not override an explicit user
// choice), nor under enforced MPP (its cost discount would make the
// cross-round comparison meaningless).
func shouldTryEngineRestrictedRounds(sessVars *variable.SessionVars) bool {
	return sessVars.EnableAlternativeLogicalPlans &&
		sessVars.StmtCtx.AlternativeLogicalPlanHasJoinOrAgg &&
		!sessVars.StmtCtx.AlternativeLogicalPlanHasStoreTypeHint &&
		!sessVars.IsMPPEnforced()
}

// shouldTryTiKVOnlyRound arms whenever round 1 read from TiFlash anywhere: only
// then does an all-TiKV plan differ from the plan already chosen.
...
// shouldTryTiFlashOnlyRound arms whenever round 1 read from TiKV anywhere,
// including the all-TiKV case: local per-table costing can prefer index access
// table by table even when a whole-statement MPP plan would win, and that
// alternative is never costed otherwise. It is skipped when any table lacks a
// TiFlash access path (no replica at all is the common case, and then a
// fully-TiFlash plan cannot be built) or when MPP execution is not allowed.
...
var alternativeRounds = [...]alternativeRound{
	...
	},
	{
		...
		// read engines so every table is read from TiKV.
		name:    "tikv-only",
	...
	{
		// tiflash-only: mirror of tikv-only with only TiFlash readable, which
		// lets the physical search build the MPP plan for the whole statement.
		name:    "tiflash-only",
...
func optimize(ctx context.Context, sctx planctx.PlanContext, node *resolve.NodeW, is infoschema.InfoSchema) (base.Plan, types.NameSlice, float64, error) {
	...
	// of round 1's chosen plan rather than on a build-time signal: which engines
	// it actually read from decides which homogeneous alternative is still
	// uncosted, and whether it joins or aggregates decides whether the rebuild
	// is worth the extra optimization.
	if needRestoreLogicalPlanCtx && bestPlan != nil {
		shape := physicalop.InspectPlanShape(bestPlan)
		if shape.ReadsFromTiKV {
			sessVars.StmtCtx.MarkAlternativeLogicalPlanReadsFromTiKV()
		}
		if shape.ReadsFromTiFlash {
			sessVars.StmtCtx.MarkAlternativeLogicalPlanReadsFromTiFlash()
		}
		if shape.HasJoinOrAgg {
			sessVars.StmtCtx.MarkAlternativeLogicalPlanHasJoinOrAgg()
		}
	}
