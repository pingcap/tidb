...
func RunGetEnforcerRules(t *testing.T) {
	prop := &property.PhysicalProperty{}
	group := memo.NewGroupWithSchema(nil, expression.NewSchema())
	enforcers := old.ExportedGetEnforcerRules(group, prop)
	...
	_, ok := enforcers[0].(*old.ExportedOrderEnforcer)
	require.True(t, ok)
}

func RunNewProperties(t *testing.T) {
	prop := &property.PhysicalProperty{}
	col := &expression.Column{}
	group := memo.NewGroupWithSchema(nil, expression.NewSchema())
	prop.SortItems = append(prop.SortItems, property.SortItem{Col: col})
	enforcers := old.ExportedGetEnforcerRules(group, prop)
	orderEnforcer, _ := enforcers[0].(*old.ExportedOrderEnforcer)
