...
func RunGroupStringer(t *testing.T) {
	optimizer := old.ExportedNewOptimizer()
	old.ExportedOptimizerResetTransformationRules(optimizer, map[pattern.Operand][]old.ExportedTransformation{
	...
	for i, sql := range input {
		...
		require.NoError(t, old.ExportedOptimizerOnPhaseExploration(optimizer, ctx, group))

		group.BuildKeyInfo()
		testdata.OnRecord(func() {
			output[i].SQL = sql
			output[i].Result = old.ExportedToString(ctx.GetEvalCtx(), group)
		})
		require.Equalf(t, output[i].Result, old.ExportedToString(ctx.GetEvalCtx(), group), "case:%v, sql:%s", i, sql)
	}
}
