...
func RunImplGroupZeroCost(t *testing.T) {
	p := parser.New()
	...
	impl, err := old.ExportedOptimizerImplGroup(old.ExportedNewOptimizer(), rootGroup, prop, 0.0)
	require.NoError(t, err)
	require.Nil(t, impl)
}

func RunInitGroupSchema(t *testing.T) {
	p := parser.New()
	ctx := coretestsdk.MockContext()
	defer func() {
		domain.GetDomain(ctx).StatsHandle().Close()
...
func RunFillGroupStats(t *testing.T) {
	p := parser.New()
	ctx := coretestsdk.MockContext()
	defer func() {
		domain.GetDomain(ctx).StatsHandle().Close()
	...
	rootGroup := memo.Convert2Group(logic)
	err = old.ExportedOptimizerFillGroupStats(old.ExportedNewOptimizer(), rootGroup)
	require.NoError(t, err)
	require.NotNil(t, rootGroup.Prop.Stats)
}

func RunPreparePossibleProperties(t *testing.T) {
	p := parser.New()
	ctx := coretestsdk.MockContext()
	defer func() {
		domain.GetDomain(ctx).StatsHandle().Close()
	...
	logic, err = old.ExportedOptimizerOnPhasePreprocessing(optimizer, ctx.GetPlanCtx(), logic)
	require.NoError(t, err)
	...
	require.NoError(t, old.ExportedOptimizerOnPhaseExploration(optimizer, ctx.GetPlanCtx(), group))
	...
	aggProp := old.ExportedPreparePossibleProperties(group, propMap)
...
func RunAppliedRuleSet(t *testing.T) {
	p := parser.New()
	ctx := coretestsdk.MockContext()
	defer func() {
		domain.GetDomain(ctx).StatsHandle().Close()
	...
	old.ExportedSetBaseRulePattern(&rule.ExportedBaseRule, pattern.NewPattern(pattern.OperandProjection, pattern.EngineAll))
	old.ExportedOptimizerResetTransformationRules(optimizer, map[pattern.Operand][]old.ExportedTransformation{
