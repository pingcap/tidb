...
func (ts *TmpStr) Hash64(h base.ExportedHasher) {
	h.HashString(ts.str1)
	h.HashString(ts.str2)
}

func RunStringLen(t *testing.T) {
	hasher1 := base.ExportedNewHashEqualer()
	hasher2 := base.ExportedNewHashEqualer()
	a := TmpStr{
		str1: "abc",
		str2: "def",
	}
...
func (sa *SA) Hash64(h base.ExportedHasher) {
	h.HashInt(sa.a)
	h.HashString(sa.b)
}
...
func (sb *SB) Hash64(h base.ExportedHasher) {
	h.HashInt(sb.a)
	h.HashString(sb.b)
}
...
func RunStructType(t *testing.T) {
	hasher1 := base.ExportedNewHashEqualer()
	hasher2 := base.ExportedNewHashEqualer()
	a := SA{
		a: 1,
