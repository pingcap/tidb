...
func RunMemo(t *testing.T) {
	...
	for e := mm.GetGroups().Front(); e != nil; e = e.Next() {
		group := e.Value.(*memo.ExportedGroup)
		require.NotNil(t, group)
		require.Equal(t, memo.ExportedGroupID(cnt), memo.ExportedGetGroupID(group))
		cnt++
	}
}

func RunInsertGE(t *testing.T) {
	require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/pkg/planner/cascades/memo/MockPlanSkipMemoDeriveStats", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/pkg/planner/cascades/memo/MockPlanSkipMemoDeriveStats"))
	}()
	...
	limit.SetID(-4)
	hasher := mm.GetHasher()
	groupExpr := mm.NewGroupExpression(limit, []*memo.ExportedGroup{mm.GetRootGroup()})
	...
	require.Equal(t, memo.ExportedGetGroupID(mm.GetGroups().Back().Value.(*memo.ExportedGroup)), memo.ExportedGroupID(cnt-1))
}
...
// └───────────────────────────────┘     └──────────────────────┘
func RunMergeGroup(t *testing.T) {
	mm := memo.ExportedNewMemo()
	ctx := mock.NewContext()

	memo.ExportedSetMemoRootGroup(mm, mm.NewGroup())
	childG1 := mm.NewGroup()
	...
	srcG := mm.NewGroup()
	mm.InsertGroupExpression(mm.NewGroupExpression(sort1, []*memo.ExportedGroup{childG1}), srcG)
	dstG := mm.NewGroup()
	mm.InsertGroupExpression(mm.NewGroupExpression(sort2, []*memo.ExportedGroup{childG1}), dstG)
	...
	srcParentGE := mm.NewGroupExpression(limit1, []*memo.ExportedGroup{srcG})
	srcParentGroup := mm.NewGroup()
	mm.InsertGroupExpression(srcParentGE, srcParentGroup)

	dstParentGE := mm.NewGroupExpression(limit2, []*memo.ExportedGroup{dstG})
	...
	sort3GE := mm.NewGroupExpression(sort3, []*memo.ExportedGroup{childG1})
	existedGE, inserted := mm.InsertGroupExpression(sort3GE, dstG)
	require.False(t, inserted)
	require.NotNil(t, existedGE)
	require.Equal(t, memo.ExportedGetGroupID(existedGE.GetGroup()), memo.ExportedGroupID(4))
	require.True(t, existedGE.Inputs[0].Equals(childG1))
	require.Equal(t, memo.ExportedGetGroupExpressionHash64(existedGE), memo.ExportedGetGroupExpressionHash64(sort3GE))
	require.Equal(t, memo.ExportedGetLogicalExpressions(existedGE.GetGroup()).Len(), 2)
	existedGE.GetGroup().Check()
	require.Equal(t, memo.ExportedGetHash2ParentGroupExpr(existedGE.GetGroup()).Size(), 2)
	mask := [2]bool{}
	memo.ExportedGetHash2ParentGroupExpr(existedGE.GetGroup()).Each(func(key unsafe.Pointer, val *memo.ExportedGroupExpression) {
		if memo.ExportedGetGroupID(val.GetGroup()) == memo.ExportedGetGroupID(srcParentGroup) {
			mask[0] = true
		}
		if memo.ExportedGetGroupID(val.GetGroup()) == memo.ExportedGetGroupID(dstParentGroup) {
			mask[1] = true
		}
	})
	...
	// assert srcG
	require.Equal(t, memo.ExportedGetHash2ParentGroupExpr(srcG).Size(), 0)
	require.Equal(t, srcG.GetLogicalExpressions().Len(), 0)
	require.Equal(t, memo.ExportedGetHash2GroupExpr(srcG).Size(), 0)
	require.Equal(t, len(srcG.Operand2FirstExpr), 0)

	// assert dstG
	require.Equal(t, memo.ExportedGetHash2ParentGroupExpr(dstG).Size(), 2)
	require.Equal(t, dstG.GetLogicalExpressions().Len(), 2)
	require.Equal(t, memo.ExportedGetHash2GroupExpr(dstG).Size(), 2)
	require.Equal(t, len(dstG.Operand2FirstExpr), 1)
	mask = [2]bool{}
	memo.ExportedGetHash2ParentGroupExpr(dstG).Each(func(key unsafe.Pointer, val *memo.ExportedGroupExpression) {
		if memo.ExportedGetGroupID(val.GetGroup()) == memo.ExportedGetGroupID(srcParentGroup) {
			mask[0] = true
		}
		if memo.ExportedGetGroupID(val.GetGroup()) == memo.ExportedGetGroupID(dstParentGroup) {
	...
	// assert memo
	require.Equal(t, mm.GetGroups().Len(), 5)
	require.Equal(t, len(mm.GetGroupID2Group()), 5)
	require.Equal(t, memo.ExportedMemoGetHash2GlobalGroupExpr(mm).Size(), 4)

	// assert childG
	require.Equal(t, memo.ExportedGetHash2ParentGroupExpr(childG1).Size(), 2)
	mask = [2]bool{}
	memo.ExportedGetHash2ParentGroupExpr(childG1).Each(func(key unsafe.Pointer, val *memo.ExportedGroupExpression) {
		if memo.ExportedGetGroupID(val.GetGroup()) == memo.ExportedGetGroupID(srcG) {
			mask[0] = true
		}
		if memo.ExportedGetGroupID(val.GetGroup()) == memo.ExportedGetGroupID(dstG) {
			mask[1] = true
		}
	})
...
func RunRecursiveMergeGroup(t *testing.T) {
	mm := memo.ExportedNewMemo()
	ctx := mock.NewContext()
	...
	mm.InsertGroupExpression(mm.NewGroupExpression(proj, []*memo.ExportedGroup{childG1}), projG2)
	sort1 := logicalop.LogicalSort{ByItems: []*util.ByItems{{Expr: expression.NewOne(), Desc: true}}}.Init(ctx, 0)

	srcG := mm.NewGroup()
	mm.InsertGroupExpression(mm.NewGroupExpression(sort1, []*memo.ExportedGroup{childG1}), srcG)
	dstG := mm.NewGroup()
	mm.InsertGroupExpression(mm.NewGroupExpression(sort1, []*memo.ExportedGroup{projG2}), dstG)
	...
	sort2GE := mm.NewGroupExpression(sort2, []*memo.ExportedGroup{childG1})
	existedGE, inserted := mm.InsertGroupExpression(sort2GE, dstG)
	require.False(t, inserted)
	require.NotNil(t, existedGE)
	require.Equal(t, memo.ExportedGetGroupID(existedGE.GetGroup()), memo.ExportedGroupID(5))
	require.True(t, existedGE.Inputs[0].Equals(childG1))
	require.Equal(t, memo.ExportedGetGroupExpressionHash64(existedGE), memo.ExportedGetGroupExpressionHash64(sort2GE))
	require.Equal(t, memo.ExportedGetLogicalExpressions(existedGE.GetGroup()).Len(), 2)
	existedGE.GetGroup().Check()
	require.Equal(t, memo.ExportedGetHash2ParentGroupExpr(existedGE.GetGroup()).Size(), 1)
	var mask bool
	memo.ExportedGetHash2ParentGroupExpr(existedGE.GetGroup()).Each(func(key unsafe.Pointer, val *memo.ExportedGroupExpression) {
		if memo.ExportedGetGroupID(val.GetGroup()) == memo.ExportedGetGroupID(dstParentGroup) {
			mask = true
	...
	// assert dstG
	require.Equal(t, memo.ExportedGetHash2ParentGroupExpr(dstG).Size(), 1)
	require.Equal(t, dstG.GetLogicalExpressions().Len(), 2)
	require.Equal(t, memo.ExportedGetHash2GroupExpr(dstG).Size(), 2)
	require.Equal(t, len(dstG.Operand2FirstExpr), 1)
	mask = false
	...
	require.Equal(t, memo.ExportedGetHash2ParentGroupExpr(srcParentGroup).Size(), 0)
	require.Equal(t, memo.ExportedGetHash2GroupExpr(srcParentGroup).Size(), 0)
	require.Equal(t, memo.ExportedGetHash2ParentGroupExpr(dstParentGroup).Size(), 0)
	require.Equal(t, memo.ExportedGetHash2GroupExpr(dstParentGroup).Size(), 1)
	memo.ExportedGetHash2GroupExpr(dstParentGroup).Each(func(key *memo.ExportedGroupExpression, val *list.Element) {
		require.True(t, key.LogicalPlan == limit2)
	})

	// assert projG
	projG2.Check()
	require.Equal(t, memo.ExportedGetHash2ParentGroupExpr(projG2).Size(), 1)
	memo.ExportedGetHash2ParentGroupExpr(projG2).Each(func(key unsafe.Pointer, val *memo.ExportedGroupExpression) {
		if memo.ExportedGetGroupID(val.GetGroup()) == memo.ExportedGetGroupID(dstG) {
			mask = true
		}
	})
	...
	memo.ExportedGetHash2ParentGroupExpr(childG1).Each(func(key unsafe.Pointer, val *memo.ExportedGroupExpression) {
		...
		_, ok := memo.ExportedGetHash2GroupExpr(dstG).Get(val)
		_, ok2 := memo.ExportedGetHash2GroupExpr(projG2).Get(val)
...
func RunIteratorLogicalPlan(t *testing.T) {
	require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/pkg/planner/cascades/memo/MockPlanSkipMemoDeriveStats", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/pkg/planner/cascades/memo/MockPlanSkipMemoDeriveStats"))
	}()
	...
	iter := memo.ExportedNewIterator(mm)
