...
func RunTaskStack(t *testing.T) {
	newSS := task.ExportedNewTaskStack()
	// size of pointer to Stack{}
	require.Equal(t, int64(unsafe.Sizeof(newSS)), int64(8))
	// size of pointer to Stack.[]Task, cap + len + addr
	require.Equal(t, int64(unsafe.Sizeof(task.ExportedGetStackTasks(newSS))), int64(24))
	// size of pointer to Stack's first element Task[0]
	newSS.Push(nil)
	newSS.Push(&TestTaskImpl{a: 1})
	newSS.Push(nil)
	v := unsafe.Sizeof(task.ExportedGetStackTasks(newSS)[0])
	require.Equal(t, int64(v), int64(16))
	v = unsafe.Sizeof(task.ExportedGetStackTasks(newSS)[1])
	require.Equal(t, int64(v), int64(16))
}

func RunTaskFunctionality(t *testing.T) {
	taskTaskPool := task.ExportedGetStackPool().Get()
	require.Equal(t, len(task.ExportedGetStackTasks(taskTaskPool.(*task.ExportedStack))), 0)
	require.Equal(t, cap(task.ExportedGetStackTasks(taskTaskPool.(*task.ExportedStack))), 4)
	ts := taskTaskPool.(*task.ExportedStack)
	...
	task.ExportedGetStackPool().Put(taskTaskPool)

	// require again.
	ts = task.ExportedGetStackPool().Get().(*task.ExportedStack)
	require.Equal(t, len(task.ExportedGetStackTasks(ts)), 4)
	require.Equal(t, cap(task.ExportedGetStackTasks(ts)), 4)
	// clean the stack
	one = ts.Pop()
	buf.Reset()
	one.Desc(w)
	...
	ts.Destroy()
	ts = task.ExportedGetStackPool().Get().(*task.ExportedStack)
	require.Equal(t, len(task.ExportedGetStackTasks(ts)), 0)
	require.Equal(t, cap(task.ExportedGetStackTasks(ts)), 4)
}
...
func BenchmarkTestStack2Pointer(b *testing.B) {
	...
	fill := func() {
		for idx := range int64(1000) {
			stack.Push(&TestTaskImpl{a: idx})
		}
		for range int64(1000) {
			stack.Pop()
		}
	}
	for range b.N {
...
func BenchmarkTestStackInterface(b *testing.B) {
	stack := task.ExportedNewTaskStackWithCap(1000)
	fill := func() {
		for idx := range int64(1000) {
			stack.Push(&TestTaskImpl{a: idx})
		}
