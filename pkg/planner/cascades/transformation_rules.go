...
func (*pushDownJoin) predicatePushDown(
	...
	case logicalop.LeftOuterJoin, logicalop.LeftOuterSemiJoin, logicalop.AntiLeftOuterSemiJoin,
		...
		if join.JoinType == logicalop.RightOuterJoin {
			joinConds, remainCond = expression.PropConstOverOuterJoin(join.SCtx().GetExprCtx(), joinConds, remainCond, rightSchema, leftSchema,
				join.SCtx().GetSessionVars().AlwaysKeepJoinKey, nullSensitive)
		} else {
			joinConds, remainCond = expression.PropConstOverOuterJoin(join.SCtx().GetExprCtx(), joinConds, remainCond, leftSchema, rightSchema,
				join.SCtx().GetSessionVars().AlwaysKeepJoinKey, nullSensitive)
		}
