...
type AccessPath struct {
	...
	*/
	...
	// MatchedScanDesc records whether the cop scan must be reversed in order
	// to satisfy the PhysicalProperty matched by matchProperty. False means a
	// forward scan over the encoded keyspace already produces rows in the
	// requested order; true means the consumer should set IndexScan.Desc=true
	// so TiKV streams keys in reverse byte order. Meaningful only when
	// matchProperty returns PropMatched or PropMatchedNeedMergeSort.
	//
	// Computing this in matchProperty (per (sortItem, idxCol) pair) lets us
	// support indexes whose columns mix ASC and DESC storage directions
	// (pingcap/tidb#2519): so long as the per-column mismatch pattern is
	// uniform, the scan direction is well-defined.
	MatchedScanDesc bool
...
func (path *AccessPath) Clone() *AccessPath {
	ret := &AccessPath{
		...
		MatchedScanDesc:              path.MatchedScanDesc,
		Forced:                       path.Forced,
