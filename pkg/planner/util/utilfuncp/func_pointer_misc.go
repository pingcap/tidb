...
// GetStreamAggs will be called by baseLogicalPlan in logicalOp pkg. The logic inside covers concrete physical operators.
...
// GetHashAggs will be called by baseLogicalPlan in logicalOp pkg. The logic inside covers concrete physical operators.
...
// PruneByItems will be called by baseLogicalPlan in logicalOp pkg. The logic current exists for rule logic inside core.
// todo: (10) arenatlx, when rule is moved out of core, we should direct ref the rule.Func instead of this util func pointer.
