...
func TestChooseBestGreedyStart(t *testing.T) {
	t.Run("pick lower cost", func(t *testing.T) {
		best, startIdx, err := chooseBestGreedyStart(2, func(startIdx int) (*greedyJoinResult, error) {
			costs := []float64{100, 10}
			return &greedyJoinResult{cumCost: costs[startIdx]}, nil
		})
