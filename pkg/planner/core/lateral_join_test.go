	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectApply {
				...
				require.GreaterOrEqual(t, applyCount, tc.expectedMinApply,
					"Expected at least %d Apply nodes, got %d", tc.expectedMinApply, applyCount)
			}
		})
	}
}
...
func TestLateralJoinErrorPaths(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:        "Comma syntax with LATERAL is valid",
			sql:         "SELECT * FROM t, LATERAL (SELECT t.a) AS dt",
			expectError: false,
		},
	}
...
func TestLateralJoinComplexScenarios(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "Multiple LATERAL with comma syntax",
...
func TestLateralJoinDecorrelateWithUSINGAndON(t *testing.T) {
	...
	optimized, _, err := solver.Optimize(ctx, lp, nil)
	require.NoError(t, err)
...
SELECT * FROM hierarchy`,
		...
		{
			name: "Recursive CTE with multiple comma LATERAL and ORDER BY/LIMIT - should work",
			sql: `
WITH RECURSIVE cte AS (
  SELECT a FROM t WHERE a = 1
  UNION ALL
...
// TestLateralJoinMySQLCompatibility tests MySQL-compatible behaviors for LATERAL joins
func TestLateralJoinMySQLCompatibility(t *testing.T) {
	s := createPlannerSuite()
	defer s.Close()
	ctx := context.Background()
	...
	testCases := []struct {
		...
		{
			// RIGHT JOIN LATERAL not yet supported (planned for PR 3)
		...
		},
		{
			...
			name:        "LATERAL references derived table alias column",
			sql:         "SELECT * FROM (SELECT a FROM t) AS j, LATERAL (SELECT j.a) AS dt",
			expectError: false,
		},
		{
			// Inner table aliases from derived tables must NOT leak to LATERAL
			name:        "Inner alias t1 must not leak from derived table",
			sql:         "SELECT * FROM (SELECT t1.a FROM t AS t1 JOIN t AS t2 USING(a)) AS j, LATERAL (SELECT t1.a) AS dt",
			expectError: true,
			errorMsg:    "Unknown column",
		},
		{
			// Inner table aliases from derived tables must NOT leak to LATERAL
			name:        "Inner alias t2 must not leak from derived table",
			sql:         "SELECT * FROM (SELECT t1.a FROM t AS t1 JOIN t AS t2 USING(a)) AS j, LATERAL (SELECT t2.a) AS dt",
			expectError: true,
			errorMsg:    "Unknown column",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectError {
				require.Error(t, err, "Expected error for: %s", tc.sql)
				if tc.errorMsg != "" {
					require.Contains(t, err.Error(), tc.errorMsg, "Error message mismatch for: %s", tc.sql)
				}
			} else {
				require.NoError(t, err, "Unexpected error for: %s\nError: %v", tc.sql, err)
			}
