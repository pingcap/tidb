...
func buildDataSource2IndexScanByIndexJoinProp(
	...
	if !prop.IsSortItemEmpty() && isMatchProp(ds, indexJoinResult.chosenPath, prop).Matched() {
...
func buildDataSource2TableScanByIndexJoinProp(
	...
	if ds.TableInfo.IsCommonHandle {
		...
		if isMatchProp(ds, indexJoinResult.chosenPath, prop).Matched() {
		...
		if !prop.IsSortItemEmpty() && isMatchProp(ds, chosenPath, prop).Matched() {
