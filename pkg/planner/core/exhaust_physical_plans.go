...
func constructIndexJoinStatic(
	...
	join := physicalop.PhysicalIndexJoin{
		...
		IsOrderPreserving:          len(prop.SortItems) > 0 || (prop.PartialOrderInfo != nil && len(prop.PartialOrderInfo.SortItems) > 0), // Mark as order-preserving if satisfying ORDER BY
		OrderPreservingExpectedCnt: prop.ExpectedCnt, // Store ExpectedCnt for cost model capping
