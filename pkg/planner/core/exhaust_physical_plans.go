...
func (p *LogicalJoin) GetMergeJoin(prop *property.PhysicalProperty, schema *expression.Schema, statsInfo *property.StatsInfo, leftStatsInfo *property.StatsInfo, rightStatsInfo *property.StatsInfo) []PhysicalPlan {
	...
	if p.preferJoinType&h.PreferNoMergeJoin > 0 {
		...
		p.SCtx().GetSessionVars().StmtCtx.SkipHintWarning(ErrInternal.FastGen(
...
func (p *LogicalJoin) getHashJoins(prop *property.PhysicalProperty) (joins []PhysicalPlan, forced bool) {
	...
	if forceLeftToBuild && forceRightToBuild {
		p.SCtx().GetSessionVars().StmtCtx.SkipHintWarning(ErrInternal.FastGen("Some HASH_JOIN_BUILD and HASH_JOIN_PROBE hints are conflicts, please check the hints"))
	...
	case SemiJoin, AntiSemiJoin, LeftOuterSemiJoin, AntiLeftOuterSemiJoin:
		...
		if forceLeftToBuild || forceRightToBuild {
			...
			p.SCtx().GetSessionVars().StmtCtx.SkipHintWarning(ErrInternal.FastGen(fmt.Sprintf("We can't use the HASH_JOIN_BUILD or HASH_JOIN_PROBE hint for %s, please check the hint", p.JoinType)))
			forceLeftToBuild = false
			forceRightToBuild = false
		}
	case LeftOuterJoin:
...
func (p *LogicalJoin) tryToGetIndexJoin(prop *property.PhysicalProperty) (indexJoins []PhysicalPlan, canForced bool) {
	...
	if p.preferAny(h.PreferLeftAsINLJInner, h.PreferRightAsINLJInner) && p.preferAny(h.PreferNoIndexJoin) {
		stmtCtx.SkipHintWarning(ErrInternal.FastGen("Some INL_JOIN and NO_INDEX_JOIN hints conflict, NO_INDEX_JOIN may be ignored"))
	}
	if p.preferAny(h.PreferLeftAsINLHJInner, h.PreferRightAsINLHJInner) && p.preferAny(h.PreferNoIndexHashJoin) {
		stmtCtx.SkipHintWarning(ErrInternal.FastGen("Some INL_HASH_JOIN and NO_INDEX_HASH_JOIN hints conflict, NO_INDEX_HASH_JOIN may be ignored"))
	}
	if p.preferAny(h.PreferLeftAsINLMJInner, h.PreferRightAsINLMJInner) && p.preferAny(h.PreferNoIndexMergeJoin) {
		stmtCtx.SkipHintWarning(ErrInternal.FastGen("Some INL_MERGE_JOIN and NO_INDEX_MERGE_JOIN hints conflict, NO_INDEX_MERGE_JOIN may be ignored"))
	}
...
func (p *LogicalJoin) handleForceIndexJoinHints(prop *property.PhysicalProperty, candidates []PhysicalPlan) (indexJoins []PhysicalPlan, canForced bool) {
	...
	if prop.IsSortItemEmpty() {
		...
		p.SCtx().GetSessionVars().StmtCtx.SkipHintWarning(ErrInternal.FastGen(errMsg))
	}
...
func (p *LogicalJoin) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	if !isJoinHintSupportedInMPPMode(p.preferJoinType) {
		if hasMPPJoinHints(p.preferJoinType) {
			...
			p.SCtx().GetSessionVars().StmtCtx.SkipHintWarning(ErrInternal.FastGen("The MPP join hints are in conflict, and you can only specify join method hints that are currently supported by MPP mode now"))
			p.preferJoinType = 0
		} else {
	...
	if forceLeftToBuild || forceRightToBuild {
		...
		if !match {
			if fixedBuildSide {
				...
				p.SCtx().GetSessionVars().StmtCtx.SkipHintWarning(ErrInternal.FastGen("Some HASH_JOIN_BUILD and HASH_JOIN_PROBE hints cannot be utilized for MPP joins, please check the hints"))
			} else {
...
func (la *LogicalAggregation) getHashAggs(prop *property.PhysicalProperty) []PhysicalPlan {
	...
	if canPushDownToMPP {
		...
		if hasMppHints {
			la.SCtx().GetSessionVars().StmtCtx.SkipHintWarning(ErrInternal.FastGen(errMsg))
		}
	}
	if prop.IsFlashProp() {
...
func (la *LogicalAggregation) ResetHintIfConflicted() (preferHash bool, preferStream bool) {
	...
	if preferHash && preferStream {
		...
		la.SCtx().GetSessionVars().StmtCtx.SkipHintWarning(warning)
...
func (la *LogicalAggregation) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	if la.aggHints.PreferAggToCop {
		if !la.canPushToCop(kv.TiKV) {
			la.SCtx().GetSessionVars().StmtCtx.SkipHintWarning(
