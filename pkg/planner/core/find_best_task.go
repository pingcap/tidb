	...
	for _, candidate := range candidates {
		...
		if len(path.Ranges) == 0 {
			...
			if expression.MaybeOverOptimized4PlanCache(ds.SCtx(), path.AccessConds) {
				ds.SCtx().GetSessionVars().StmtCtx.SetSkipPlanCache(errors.Errorf("get a TableDual plan"), ds.SCtx().GetSessionVars().StmtCtx.UseCache)
			}
			dual := PhysicalTableDual{}.Init(ds.SCtx(), ds.StatsInfo(), ds.QueryBlockOffset())
			dual.SetSchema(ds.schema)
			cntPlan++
		...
		if canConvertPointGet {
			...
			if allRangeIsPoint {
				...
				if !pointGetTask.invalid() && expression.MaybeOverOptimized4PlanCache(ds.SCtx(), candidate.path.AccessConds) && !isSafePointGetPath4PlanCache(ds.SCtx(), candidate.path) {
					ds.SCtx().GetSessionVars().StmtCtx.SetSkipPlanCache(errors.New("Batch/PointGet plans may be over-optimized"), ds.SCtx().GetSessionVars().StmtCtx.UseCache)
				}
