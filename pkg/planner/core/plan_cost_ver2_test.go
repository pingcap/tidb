...
func TestCostModelShowFormula(t *testing.T) {
	...
	require.Equal(t, actual, [][]interface{}{
		{"TableReader_7", "(((cpu(0*filters(1)*tikv_cpu_factor(49.9))) + (scan(1*logrowsize(32)*tikv_scan_factor(40.7)))) + (net(1*rowsize(16)*tidb_kv_net_factor(3.96))))/15.00"},
		{"└─Selection_6", "(cpu(0*filters(1)*tikv_cpu_factor(49.9))) + (scan(1*logrowsize(32)*tikv_scan_factor(40.7)))"},
		{"  └─TableFullScan_5", "scan(1*logrowsize(32)*tikv_scan_factor(40.7))"},
	})
}
