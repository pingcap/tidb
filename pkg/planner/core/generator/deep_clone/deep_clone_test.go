...
func TestDeepCloneCode(t *testing.T) {
	updatedCode, err := GenDeepClone4LogicalOps()
	if err != nil {
		t.Errorf("Generate DeepClone code error: %v", err)
		return
	}
	currentCode, err := os.ReadFile("../../operator/logicalop/deep_clone_generated.go")
	if err != nil {
		t.Errorf("Read current deep_clone_generated.go code error: %v", err)
		return
	}
	...
	if !bytes.Equal(updatedCode, currentCode) {
		t.Errorf("deep_clone_generated.go should be updated, please run 'make gogenerate' to update it.")
	}
}
