...
func RunEncodeDecodePlan(t *testing.T) {
...
func RunNormalizedDigest(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
...
func RunExplainFormatHintRecoverableForTiFlashReplica(t *testing.T) {
...
func BenchmarkDecodePlan(b *testing.B) {
	...
	for i := range 50000 {
		...
		fmt.Fprintf(buf, "select count(1) as num,a from t where a='%v' group by a", i)
	}
	query := buf.String()
	tk.Session().GetSessionVars().PlanID.Store(0)
	tk.MustExec(query)
...
func RunBuildFinalModeAggregation(t *testing.T) {
...
func RunCloneFineGrainedShuffleStreamCount(t *testing.T) {
...
func RunImportIntoBuildPlan(t *testing.T) {
	store := testkit.CreateMockStore(t)
