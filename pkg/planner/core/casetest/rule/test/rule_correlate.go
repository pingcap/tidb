...
// RunCorrelateNullSemantics verifies that CorrelateSolver does not break
// 3-valued NULL semantics for scalar IN (LeftOuterSemiJoin).
func RunCorrelateNullSemantics(t *testing.T) {
...
// RunCorrelateAlternativeChoosesApply verifies that the correlate alternative
...
func RunCorrelateAlternativeChoosesApply(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	...
	tk.MustExec("set tidb_opt_enable_alternative_logical_plans = OFF")
	withoutAltRows := tk.MustQuery(sql).Sort().Rows()

	tk.MustExec("set tidb_opt_enable_alternative_logical_plans = ON")
	withAltRows := tk.MustQuery(sql).Sort().Rows()

	require.Equal(t, withoutAltRows, withAltRows,
		"correlate alternative must produce the same results as standard path")
}

// RunCorrelateParallelApply verifies that when the correlate alternative round
// produces an Apply plan and tidb_enable_parallel_apply is ON, the Apply is
// executed with parallel concurrency.
func RunCorrelateParallelApply(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
...
// RunCorrelateWithCostFactors verifies that when hash/merge join cost factors
...
func RunCorrelateWithCostFactors(tt *testing.T) {
