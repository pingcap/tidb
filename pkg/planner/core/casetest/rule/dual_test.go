...
func TestDual(t *testing.T) {
	testkit.RunTestUnderCascades(t, func(t *testing.T, testKit *testkit.TestKit, cascades, caller string) {
		...
		firstDual := "TableDual_9 0.00 root  rows:0"
		proj := "Projection_9 0.00 root  3->Column#4"
		secondDual := "└─TableDual_11 0.00 root  rows:0"
		nullDual := "TableDual_7 0.00 root  rows:0"
		if cascades == "on" {
			firstDual = "TableDual_8 0.00 root  rows:0"
			proj = "Projection_8 0.00 root  3->Column#4"
			secondDual = "└─TableDual_9 0.00 root  rows:0"
			nullDual = "TableDual_6 0.00 root  rows:0"
		}
