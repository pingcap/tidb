...
func TestParamSubstituteInSP(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	tk.MustExec("set tidb_cost_model_version=1")
	...
	for i, ts := range input {
		...
		//in
		tk.MustExec("create procedure sp(in pa int) begin explain format = 'brief' " + ts + "; end")
		tk.MustExec("call sp(@a)")
		tk.Res[0].Check(testkit.Rows(output[2*i+1].Plan...))
		tk.ClearProcedureRes()
		tk.MustExec("drop procedure sp")
		// inout
		tk.MustExec("create procedure sp(inout pa int) begin explain format = 'brief' " + ts + "; end")
		tk.MustExec("call sp(@a)")
		tk.Res[0].Check(testkit.Rows(output[2*i+1].Plan...))
		tk.ClearProcedureRes()
		tk.MustExec("drop procedure sp")
		// out
		tk.MustExec("create procedure sp(inout pa int) begin set pa = 1; explain format = 'brief' " + ts + "; end")
		tk.MustExec("call sp(@a)")
		tk.Res[0].Check(testkit.Rows(output[2*i+1].Plan...))
		tk.ClearProcedureRes()
		tk.MustExec("drop procedure sp")
		// stored procedure local variable
		tk.MustExec("create procedure sp() begin declare pa int; set pa = 1; explain format = 'brief' " + ts + "; end")
		tk.MustExec("call sp()")
		tk.Res[0].Check(testkit.Rows(output[2*i+1].Plan...))
		tk.ClearProcedureRes()
		tk.MustExec("drop procedure sp")
...
func TestUDVSubstituteInSP(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	tk.MustExec("set tidb_cost_model_version=1")
...
func TestUDVSubstituteInSPWithPrepare(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	tk.MustExec("set tidb_cost_model_version=1")
	...
	tk.MustExec("set @@session.tidb_enable_udv_substitute = 0")
	// currently, udv substitute is not supported in prepare statement
	for i, ts := range input {
		testdata.OnRecord(func() {
			output[i].SQL = ts
			output[i].Plan = testdata.ConvertRowsToStrings(tk.MustQuery("explain format = 'brief' " + ts).Rows())
		})
		tk.MustExec("prepare stmt from 'explain format = brief " + ts + "'")
		tk.MustExec("create procedure sp() begin execute stmt; end")
		tk.MustExec("call sp()")
		tk.Res[0].Check(testkit.Rows(output[i].Plan...))
		tk.ClearProcedureRes()
		tk.MustExec("drop procedure sp")
	}
}
