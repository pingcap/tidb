...
func TestMPPHintsScope(t *testing.T) {
	testkit.RunTestUnderCascades(t, func(t *testing.T, testKit *testkit.TestKit, cascades, caller string) {
		...
		testKit.MustQuery("show warnings").Check(testkit.Rows(`Warning 1815 The agg can not push down to the MPP side, the MPP_1PHASE_AGG() hint is invalid`))
		testKit.MustExec("select /*+ MPP_2PHASE_AGG() */ a, sum(b) from t group by a, c")
		testKit.MustQuery("show warnings").Check(testkit.Rows(`Warning 1815 The agg can not push down to the MPP side, the MPP_2PHASE_AGG() hint is invalid`))
