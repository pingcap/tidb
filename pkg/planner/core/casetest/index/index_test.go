...
func TestIndexPruneWithSharedClusteredPrefix(t *testing.T) {
	testkit.RunTestUnderCascades(t, func(t *testing.T, tk *testkit.TestKit, cascades, caller string) {
		...
		kept := runQuery(`explain format = 'brief' select label from tenant_obj
		...
		kept = runQuery("explain format = 'brief' select payload from tenant_obj where tenant_ws = 'w1'")
		...
		kept = runQuery("explain format = 'brief' select label from tenant_obj where num1 = 1")
		...
		tk.MustQuery(`explain format = 'brief' select payload from tenant_obj
		...
		kept = runQuery("explain format = 'brief' select payload from tenant_dup where tenant_ws = 'w' and a = 1")
		...
		// its larger index ID.
		kept = runQuery("explain format = 'brief' select payload from tenant_dup where tenant_ws = 'w' and a = 1 and c = 2")
