...
func TestIndexJoinWithOrderedWindowInner(t *testing.T) {
	testkit.RunTestUnderCascades(t, func(t *testing.T, tk *testkit.TestKit, cascades, caller string) {
		...
		tk.MustExec("use test")
		tk.MustExec("drop table if exists t1, t2, delivery_monitor, transfer_event")
		tk.MustExec("create table t1(a int, key(a))")
		tk.MustExec("create table t2(a int, b int, c int, key idx_abc(a, b, c))")
		tk.MustExec("create table delivery_monitor(bill_code varchar(20) not null, scan_time datetime not null, courier_code varchar(20), sign_flag int, remark3 int, remark6 int, key idx_courier_scan(courier_code, scan_time, sign_flag, remark3, remark6), key idx_bill_code(bill_code))")
		tk.MustExec("create table transfer_event(waybill_no varchar(20) not null, apply_time datetime not null, apply_type_code int, status int, receiver_name varchar(20), key idx_waybill_apply_time(waybill_no, apply_time))")
		tk.MustExec("insert into t1 values (1), (2), (3)")
		tk.MustExec("insert into t2 values (1, 1, 11), (1, 2, 12), (2, 1, 21), (2, 3, 23), (3, 4, 34)")
		tk.MustExec("insert into delivery_monitor values ('WB001', '2026-02-04 10:00:00', 'courier_001', 0, 0, 0), ('WB002', '2026-02-04 11:00:00', 'courier_001', 0, 0, 0), ('WB003', '2026-02-04 12:00:00', 'courier_001', 0, 0, 0), ('WB004', '2026-02-04 13:00:00', 'courier_002', 0, 0, 0)")
		tk.MustExec("insert into transfer_event values ('WB001', '2026-02-04 09:00:00', 4, 2, 'old_receiver_1'), ('WB001', '2026-02-04 15:00:00', 4, 2, 'new_receiver_1'), ('WB002', '2026-02-04 08:00:00', 3, 2, 'old_receiver_2'), ('WB002', '2026-02-04 16:00:00', 4, 1, 'new_receiver_2'), ('WB004', '2026-02-04 17:00:00', 4, 2, 'new_receiver_4')")
