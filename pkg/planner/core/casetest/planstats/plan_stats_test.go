...
func TestDynamicPartitionPruneUsesSelectedPartitionStatsForExplicitPartition(t *testing.T) {
	p := parser.New()
	store, dom := testkit.CreateMockStoreAndDomain(t)
	...
	require.NotEmpty(t, reader.TablePlans)
	scan := reader.TablePlans[0]
	require.Equal(t, float64(100), scan.StatsInfo().RowCount)
	require.Equal(t, int64(100), scan.StatsInfo().HistColl.RealtimeCount)
}
