...
//     be eliminated.
func (o *outerJoinEliminator) tryToEliminateOuterJoin(p *LogicalJoin, aggCols []*expression.Column, parentCols []*expression.Column, opt *coreUtil.LogicalOptimizeOp) (LogicalPlan, bool, error) {
...
func (o *outerJoinEliminator) doOptimize(p LogicalPlan, aggCols []*expression.Column, parentCols []*expression.Column, opt *coreUtil.LogicalOptimizeOp) (LogicalPlan, error) {
...
func (o *outerJoinEliminator) optimize(_ context.Context, p LogicalPlan, opt *coreUtil.LogicalOptimizeOp) (LogicalPlan, bool, error) {
	planChanged := false
...
func appendOuterJoinEliminateTraceStep(join *LogicalJoin, outerPlan LogicalPlan, parentCols []*expression.Column,
	innerJoinKeys *expression.Schema, opt *coreUtil.LogicalOptimizeOp) {
	reason := func() string {
...
func appendOuterJoinEliminateAggregationTraceStep(join *LogicalJoin, outerPlan LogicalPlan, aggCols []*expression.Column, opt *coreUtil.LogicalOptimizeOp) {
	reason := func() string {
		buffer := bytes.NewBufferString("The columns[")
