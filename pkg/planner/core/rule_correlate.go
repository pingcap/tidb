...
func (s *CorrelateSolver) correlate(ctx context.Context, p base.LogicalPlan) (base.LogicalPlan, bool, error) {
	...
	if join.JoinType == logicalop.LeftOuterSemiJoin || join.JoinType == logicalop.AntiLeftOuterSemiJoin {
		for _, eqCond := range join.EqualConditions {
			col0, col1, ok := isColOpCol(eqCond)
			if !ok {
				return p, planChanged, nil
			}
...
func isColOpCol(eqCond *expression.ScalarFunction) (*expression.Column, *expression.Column, bool) {
	args := eqCond.GetArgs()
	if len(args) != 2 {
		return nil, nil, false
	}
	col0, ok0 := args[0].(*expression.Column)
	col1, ok1 := args[1].(*expression.Column)
	return col0, col1, ok0 && ok1
}
...
func resetStatsForCorrelatedDS(p base.LogicalPlan) bool {
	...
	if ds, ok := p.(*logicalop.DataSource); ok {
		...
		if hasCorrelated {
			...
			origPaths := ds.PossibleAccessPaths
			ds.PossibleAccessPaths = make([]*util.AccessPath, len(origPaths))
			for i, ap := range origPaths {
				ds.PossibleAccessPaths[i] = freshAccessPath(ap)
			}
		}
	}
