...
func (b *PlanBuilder) buildLateralJoin(ctx context.Context, leftPlan, rightPlan base.LogicalPlan, joinNode *ast.Join) (base.LogicalPlan, error) {
	...
	switch joinNode.Tp {
	case ast.LeftJoin:
		return nil, plannererrors.ErrNotSupportedYet.GenWithStackByArgs("LATERAL with LEFT JOIN")
	case ast.RightJoin:
		return nil, plannererrors.ErrNotSupportedYet.GenWithStackByArgs("LATERAL with RIGHT JOIN")
	default:
