...
func (gc *gcSubstituter) optimize(ctx context.Context, lp LogicalPlan, opt *coreUtil.LogicalOptimizeOp) (LogicalPlan, bool, error) {
	planChanged := false
...
func tryToSubstituteExpr(expr *expression.Expression, lp LogicalPlan, candidateExpr expression.Expression, tp types.EvalType, schema *expression.Schema, col *expression.Column, opt *coreUtil.LogicalOptimizeOp) bool {
	changed := false
...
func appendSubstituteColumnStep(lp LogicalPlan, candidateExpr expression.Expression, col *expression.Column, opt *coreUtil.LogicalOptimizeOp) {
...
func SubstituteExpression(cond expression.Expression, lp LogicalPlan, exprToColumn ExprColumnMap, schema *expression.Schema, opt *coreUtil.LogicalOptimizeOp) bool {
...
func (gc *gcSubstituter) substitute(ctx context.Context, lp LogicalPlan, exprToColumn ExprColumnMap, opt *coreUtil.LogicalOptimizeOp) LogicalPlan {
	var tp types.EvalType
	switch x := lp.(type) {
