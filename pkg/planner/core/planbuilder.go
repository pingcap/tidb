	...
	for _, index := range tblInfo.Indices {
		if index.State == model.StatePublic {
			...
			if index.IsTiFlashLocalIndex() {
				...
				// So the current way to use the tiflash indexes is to require the TiFlash Replica to be available.
...
func getModifiedIndexesInfoForAnalyze(
	...
	for _, originIdx := range tblInfo.Indices {
		...
		if originIdx.IsTiFlashLocalIndex() {
			stmtCtx.AppendWarning(errors.NewNoStackErrorf("analyzing tiflash index is not supported, skip %s", originIdx.Name.L))
			continue
		}
		if allColumns {
	...
	for _, tbl := range as.TableNames {
		...
		for _, idx := range idxInfo {
			...
			if idx.IsTiFlashLocalIndex() {
				b.ctx.GetSessionVars().StmtCtx.AppendWarning(errors.NewNoStackErrorf("analyzing tiflash index is not supported, skip %s", idx.Name.L))
				continue
			}
