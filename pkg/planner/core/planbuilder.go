...
func (b *PlanBuilder) Build(ctx context.Context, node *resolve.NodeW) (base.Plan, error) {
	...
	case *ast.ExportSchemaStmt:
		return b.buildExportSchema(ctx, x)
...
func (b *PlanBuilder) buildExportSchema(_ context.Context, st *ast.ExportSchemaStmt) (base.Plan, error) {
	...
	if semv1.IsEnabled() && objstore.IsLocal(u) {
		return nil, plannererrors.ErrNotSupportedWithSem.GenWithStackByArgs("EXPORT SCHEMA to server disk")
	}
	if kerneltype.IsNextGen() && sem.IsEnabled() && objstore.IsS3Like(u) {
		if err := checkNextGenS3PathWithSem("EXPORT SCHEMA", u); err != nil {
			return nil, err
		}
		values := u.Query()
		values.Set(s3like.S3ExternalID, config.GetGlobalKeyspaceName())
	...
	// Options don't reference table columns (there's no single table to
	// resolve them against), so a plain LogicalTableDual is enough context
	// for b.rewrite, same as buildExportTable's mockTablePlan.
	mockTablePlan := logicalop.LogicalTableDual{}.Init(b.ctx, b.getSelectOffset())
	options := make([]*LoadDataOpt, 0, len(st.Options))
	for _, opt := range st.Options {
		loadDataOpt := LoadDataOpt{Name: opt.Name}
		if opt.Value != nil {
			var err error
			loadDataOpt.Value, _, err = b.rewrite(context.Background(), opt.Value, mockTablePlan, nil, true)
			if err != nil {
				return nil, err
			}
		}
	...
	for _, schema := range st.Schemas {
		var selectErr error
		if user != nil {
			selectErr = plannererrors.ErrDBaccessDenied.GenWithStackByArgs(user.AuthUsername, user.AuthHostname, schema)
		}
		b.visitInfo = appendVisitInfo(b.visitInfo, mysql.SelectPriv, strings.ToLower(schema), "", "", selectErr)
	}
