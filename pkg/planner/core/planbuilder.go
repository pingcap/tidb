...
type handleColHelper struct {
	id2HandleMapStack []map[int64][]handlecol.HandleCols
	stackTail         int
}
...
func (hch *handleColHelper) popMap() map[int64][]handlecol.HandleCols {
...
func (hch *handleColHelper) pushMap(m map[int64][]handlecol.HandleCols) {
	hch.id2HandleMapStack = append(hch.id2HandleMapStack, m)
	hch.stackTail++
}

func (hch *handleColHelper) mergeAndPush(m1, m2 map[int64][]handlecol.HandleCols) {
	newMap := make(map[int64][]handlecol.HandleCols, max(len(m1), len(m2)))
	for k, v := range m1 {
		newMap[k] = make([]handlecol.HandleCols, len(v))
		copy(newMap[k], v)
	}
	for k, v := range m2 {
		if _, ok := newMap[k]; ok {
...
func (hch *handleColHelper) tailMap() map[int64][]handlecol.HandleCols {
...
func NewPlanBuilder(opts ...PlanBuilderOpt) *PlanBuilder {
	builder := &PlanBuilder{
		...
		handleHelper:        &handleColHelper{id2HandleMapStack: make([]map[int64][]handlecol.HandleCols, 0)},
...
func BuildHandleColsForAnalyze(ctx base.PlanContext, tblInfo *model.TableInfo, allColumns bool, colsInfo []*model.ColumnInfo) handlecol.HandleCols {
	...
	case tblInfo.PKIsHandle:
		...
		handleCols = handlecol.NewIntHandleCols(&expression.Column{
			ID:      pkCol.ID,
	...
	case tblInfo.IsCommonHandle:
		...
		handleCols = handlecol.NewCommonHandleColsWithoutOffsetAlign(ctx.GetSessionVars().StmtCtx, tblInfo, pkIdx, columns)
	}
	return handleCols
}
...
func (b *PlanBuilder) buildAnalyzeFullSamplingTask(
	...
	for i, id := range physicalIDs {
		...
		if newTask.HandleCols == nil {
			...
			newTask.HandleCols = handlecol.NewIntHandleCols(colInfoToColumn(extraCol, len(newTask.ColsInfo)-1))
		}
