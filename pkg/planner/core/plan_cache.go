...
func rebuildRange(p Plan) error {
	...
	case *PointGetPlan:
		...
		if x.AccessConditions != nil {
			if x.IndexInfo != nil {
				ranges, err := ranger.DetachCondAndBuildRangeForIndex(GetRangerCtx(x.ctx), x.AccessConditions, x.IdxCols, x.IdxColLens, 0)
				if err != nil {
					return err
				}
				...
				if pkCol != nil {
					ranges, accessConds, remainingConds, err := ranger.BuildTableRange(x.AccessConditions, GetRangerCtx(x.ctx), pkCol.RetType, 0)
					if err != nil {
						return err
					}
...
func buildRangeForTableScan(sctx PlanContext, ts *PhysicalTableScan) (err error) {
	if ts.Table.IsCommonHandle {
		...
		if len(pkCols) > 0 {
			res, err := ranger.DetachCondAndBuildRangeForIndex(GetRangerCtx(sctx), ts.AccessCondition, pkCols, pkColsLen, 0)
			if err != nil {
				return err
			}
		...
		if pkCol != nil {
			ranges, accessConds, remainingConds, err := ranger.BuildTableRange(ts.AccessCondition, GetRangerCtx(sctx), pkCol.RetType, 0)
			if err != nil {
				return err
			}
...
func buildRangeForIndexScan(sctx PlanContext, is *PhysicalIndexScan) (err error) {
	...
	res, err := ranger.DetachCondAndBuildRangeForIndex(GetRangerCtx(sctx), is.AccessCondition, is.IdxCols, is.IdxColLens, 0)
	if err != nil {
		return err
	}
