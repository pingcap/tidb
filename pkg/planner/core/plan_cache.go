...
func rebuildRange(p Plan) error {
	...
	case *PointGetPlan:
		if x.TblInfo.GetPartitionInfo() != nil &&
			fixcontrol.GetBoolWithDefault(sctx.GetSessionVars().OptimizerFixControl, fixcontrol.Fix33031, false) {
			sctx.GetSessionVars().StmtCtx.UseCache = false
			return errors.New("Fix33031 fix-control set and partitioned table in cached A plan")
		}
	...
	case *BatchPointGetPlan:
		if x.TblInfo.GetPartitionInfo() != nil && fixcontrol.GetBoolWithDefault(sctx.GetSessionVars().OptimizerFixControl, fixcontrol.Fix33031, false) {
			sctx.GetSessionVars().StmtCtx.UseCache = false
			return errors.New("Fix33031 fix-control set and partitioned table in cached B plan")
		}
		// if access condition is not nil, which means it's a point get generated by cbo.
		if x.AccessConditions != nil {
			if x.IndexInfo != nil {
