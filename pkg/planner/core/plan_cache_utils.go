...
func GeneratePlanCacheStmtWithAST(ctx context.Context, sctx sessionctx.Context, isPrepStmt bool,
	paramSQL string, paramStmt ast.StmtNode, is infoschema.InfoSchema) (*plancache.PlanCacheStmt, base.Plan, int, error) {
	...
	preparedObj := &plancache.PlanCacheStmt{
...
func NewPlanCacheKey(sctx sessionctx.Context, stmt *plancache.PlanCacheStmt) (key, binding string, cacheable bool, reason string, err error) {
...
func GetPreparedStmt(stmt *ast.ExecuteStmt, vars *variable.SessionVars) (*plancache.PlanCacheStmt, error) {
	if stmt.PrepStmt != nil {
		return stmt.PrepStmt.(*plancache.PlanCacheStmt), nil
	}
	if stmt.Name != "" {
		...
		return prepStmt.(*plancache.PlanCacheStmt), nil
	}
