...
func NewPlanCacheKey(sctx sessionctx.Context, stmt *PlanCacheStmt) (key, binding string, cacheable bool, reason string, err error) {
	...
	// exprPushdownBlacklistReloadTimeStamp + hasSubquery + foreignKeyChecks + enablePlanCacheGenericPlan
	...
	hash = append(hash, bool2Byte(vars.EnablePlanCacheGenericPlan))
