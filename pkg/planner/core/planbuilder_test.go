...
func TestExtractRawImportJobFilters(t *testing.T) {
	tests := []struct {
		sql          string
		expectedID   int64
		hasID        bool
		...
		{
			sql:        "SHOW RAW IMPORT JOBS WHERE job_id = 123",
			expectedID: 123,
			hasID:      true,
		},
		{
			sql:        "SHOW RAW IMPORT JOBS WHERE job_id = '456'",
			expectedID: 456,
			hasID:      true,
		},
		{
			sql:         "SHOW RAW IMPORT JOBS WHERE group_key = 'group-1'",
			expectedKey: "group-1",
			hasKey:      true,
		},
		{
			sql:         "SHOW RAW IMPORT JOBS WHERE job_id = 123 AND group_key = 'group-1'",
			expectedID:  123,
			hasID:       true,
			expectedKey: "group-1",
			hasKey:      true,
		...
		{
			sql:          "SHOW RAW IMPORT JOBS WHERE job_id = 123 AND raw_stats IS NOT NULL",
			expectedID:   123,
			hasID:        true,
			hasRemaining: true,
		},
		{
			sql:          "SHOW RAW IMPORT JOBS WHERE group_key = 123",
			hasRemaining: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.sql, func(t *testing.T) {
			...
			show := stmt.(*ast.ShowStmt)

			jobID, groupKey, remaining := extractRawImportJobFilters(show.Where)
			require.Equal(t, tt.hasID, jobID != nil)
			if tt.hasID {
				require.Equal(t, tt.expectedID, *jobID)
			}
			require.Equal(t, tt.hasKey, groupKey != nil)
			if tt.hasKey {
				require.Equal(t, tt.expectedKey, *groupKey)
			}
			require.Equal(t, tt.hasRemaining, remaining != nil)

			originalWhere := show.Where
			builder, _ := NewPlanBuilder().Init(coretestsdk.MockContext(), nil, hint.NewQBHintHandler(nil))
			_, err = builder.Build(context.Background(), resolve.NewNodeW(show))
			require.NoError(t, err)
			require.Same(t, originalWhere, show.Where)
		})
	}
}
