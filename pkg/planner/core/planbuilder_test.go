...
func TestBuildRefreshMaterializedViewImplementFastBuildsMVMergePlan(t *testing.T) {
	sctx := MockContext()
	...
	for _, ai := range mergePlan.AggInfos {
		...
		case mvmerge.AggCount:
			...
			require.Equal(t, []int{5}, ai.Dependencies)
		case mvmerge.AggSum:
			hasSum = true
			require.Equal(t, "b", ai.ArgColName)
			require.Equal(t, []int{6, 2}, ai.Dependencies)
		}
	}
