...
func TestJoinReorderWithAddSelection(t *testing.T) {
	...
	tk.MustQuery(`explain format='brief' select 0 from t2 join(t3 join t0 a on 0) left join(t1 b left join t1 c on 0) on(c20 = b.vkey) on(c13 = a.vkey) join(select c14 d from(t2 join t3 on c12 = vkey)) e on(c3 = d) where nullif(c15, case when(c.c10) then 0 end);`).Check(testkit.Rows(
		`Projection_33 0.00 root  0->Column#26`,
		`└─HashJoin_35 0.00 root  inner join, equal:[eq(Column#27, Column#28)]`,
		`  ├─HashJoin_37(Build) 0.00 root  inner join, equal:[eq(test.t0.c3, test.t2.c14)]`,
		`  │ ├─Selection_38(Build) 0.00 root  if(eq(test.t2.c15, cast(case(test.t1.c10, 0), double BINARY)), NULL, test.t2.c15)`,
		`  │ │ └─HashJoin_40 0.00 root  left outer join, equal:[eq(test.t3.c20, test.t1.vkey)]`,
		`  │ │   ├─HashJoin_42(Build) 0.00 root  inner join, equal:[eq(test.t0.vkey, test.t2.c13)]`,
		`  │ │   │ ├─TableDual_47(Build) 0.00 root  rows:0`,
		`  │ │   │ └─TableReader_46(Probe) 9990.00 root  data:Selection_45`,
		`  │ │   │   └─Selection_45 9990.00 cop[tikv]  not(isnull(test.t2.c13))`,
		`  │ │   │     └─TableFullScan_44 10000.00 cop[tikv] table:t2 keep order:false, stats:pseudo`,
		`  │ │   └─HashJoin_48(Probe) 9990.00 root  CARTESIAN left outer join`,
		`  │ │     ├─TableDual_53(Build) 0.00 root  rows:0`,
		`  │ │     └─TableReader_52(Probe) 9990.00 root  data:Selection_51`,
		`  │ │       └─Selection_51 9990.00 cop[tikv]  not(isnull(test.t1.vkey))`,
		`  │ │         └─TableFullScan_50 10000.00 cop[tikv] table:b keep order:false, stats:pseudo`,
		`  │ └─Projection_54(Probe) 9990.00 root  test.t2.c14, cast(test.t2.c12, double BINARY)->Column#27`,
		`  │   └─TableReader_57 9990.00 root  data:Selection_56`,
		`  │     └─Selection_56 9990.00 cop[tikv]  not(isnull(test.t2.c14))`,
		`  │       └─TableFullScan_55 10000.00 cop[tikv] table:t2 keep order:false, stats:pseudo`,
		`  └─Projection_58(Probe) 10000.00 root  cast(test.t3.vkey, double BINARY)->Column#28`,
