...
func TestIssue59902(t *testing.T) {
	...
	tk.MustQuery("explain format='brief' select t1.b,(select count(*) from t2 where t2.a=t1.a) as a from t1 where t1.a=1;").
		Check(testkit.Rows(
			...
			"└─MergeJoin 1.00 root  left outer join, left key:test.t1.a, right key:test.t2.a",
			"  ├─StreamAgg(Build) 8.00 root  group by:test.t2.a, funcs:count(Column#14)->Column#9, funcs:firstrow(test.t2.a)->test.t2.a",
