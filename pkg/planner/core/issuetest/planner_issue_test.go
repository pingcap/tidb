...
func TestJoinReorderWithAddSelection(t *testing.T) {
	...
	tk.MustQuery(`explain select 0 from t2 join(t3 join t0 a on 0) left join(t1 b left join t1 c on 0) on(c20 = b.vkey) on(c13 = a.vkey) join(select c14 d from(t2 join t3 on c12 = vkey)) e on(c3 = d) where nullif(c15, case when(c.c10) then 0 end);`).Check(testkit.Rows(
		...
		`└─HashJoin_49 0.00 root  inner join, equal:[eq(Column#27, Column#28)]`,
		`  ├─HashJoin_63(Build) 0.00 root  inner join, equal:[eq(test.t0.c3, test.t2.c14)]`,
		`  │ ├─Selection_64(Build) 0.00 root  if(eq(test.t2.c15, cast(case(test.t1.c10, 0), double BINARY)), NULL, test.t2.c15)`,
		`  │ │ └─HashJoin_73 0.00 root  left outer join, left side:HashJoin_87, equal:[eq(test.t3.c20, test.t1.vkey)]`,
		`  │ │   ├─HashJoin_87(Build) 0.00 root  inner join, equal:[eq(test.t0.vkey, test.t2.c13)]`,
		`  │ │   │ ├─TableDual_96(Build) 0.00 root  rows:0`,
		`  │ │   │ └─TableReader_95(Probe) 9990.00 root  data:Selection_94`,
		`  │ │   │   └─Selection_94 9990.00 cop[tikv]  not(isnull(test.t2.c13))`,
		`  │ │   │     └─TableFullScan_93 10000.00 cop[tikv] table:t2 keep order:false, stats:pseudo`,
		`  │ │   └─HashJoin_103(Probe) 9990.00 root  CARTESIAN left outer join, left side:TableReader_107`,
		`  │ │     ├─TableDual_108(Build) 0.00 root  rows:0`,
		`  │ │     └─TableReader_107(Probe) 9990.00 root  data:Selection_106`,
		`  │ │       └─Selection_106 9990.00 cop[tikv]  not(isnull(test.t1.vkey))`,
		`  │ │         └─TableFullScan_105 10000.00 cop[tikv] table:b keep order:false, stats:pseudo`,
		`  │ └─Projection_114(Probe) 9990.00 root  test.t2.c14, cast(test.t2.c12, double BINARY)->Column#27`,
		`  │   └─TableReader_118 9990.00 root  data:Selection_117`,
		`  │     └─Selection_117 9990.00 cop[tikv]  not(isnull(test.t2.c14))`,
		`  │       └─TableFullScan_116 10000.00 cop[tikv] table:t2 keep order:false, stats:pseudo`,
		`  └─Projection_121(Probe) 10000.00 root  cast(test.t3.vkey, double BINARY)->Column#28`,
		`    └─TableReader_124 10000.00 root  data:TableFullScan_123`,
		`      └─TableFullScan_123 10000.00 cop[tikv] table:t3 keep order:false, stats:pseudo`))
}
