...
func TestIssue59902(t *testing.T) {
	...
	tk.MustQuery("explain select t1.b,(select count(*) from t2 where t2.a=t1.a) as a from t1 where t1.a=1;").
		Check(testkit.Rows(
			...
			"  ├─Point_Get_39(Build) 1.00 root table:t1 handle:1",
			"  └─HashAgg_17(Probe) 1.00 root  group by:test.t2.a, funcs:count(Column#10)->Column#9, funcs:firstrow(test.t2.a)->test.t2.a",
			"    └─IndexReader_18 1.00 root  index:HashAgg_16",
			"      └─HashAgg_16 1.00 cop[tikv]  group by:test.t2.a, funcs:count(1)->Column#10",
