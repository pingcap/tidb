...
func (*ppdSolver) optimize(_ context.Context, lp LogicalPlan, opt *coreUtil.LogicalOptimizeOp) (LogicalPlan, bool, error) {
	planChanged := false
	_, p := lp.PredicatePushDown(nil, opt)
	return p, planChanged, nil
}

func addSelection(p LogicalPlan, child LogicalPlan, conditions []expression.Expression, chIdx int, opt *coreUtil.LogicalOptimizeOp) {
	if len(conditions) == 0 {
...
func (p *baseLogicalPlan) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) ([]expression.Expression, LogicalPlan) {
	if len(p.children) == 0 {
...
func (p *LogicalSelection) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) ([]expression.Expression, LogicalPlan) {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalUnionScan) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) ([]expression.Expression, LogicalPlan) {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (ds *DataSource) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) ([]expression.Expression, LogicalPlan) {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalTableDual) PredicatePushDown(predicates []expression.Expression, _ *coreUtil.LogicalOptimizeOp) ([]expression.Expression, LogicalPlan) {
	return predicates, p
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalJoin) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) (ret []expression.Expression, retPlan LogicalPlan) {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalExpand) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) (ret []expression.Expression, retPlan LogicalPlan) {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalProjection) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) (ret []expression.Expression, retPlan LogicalPlan) {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalUnionAll) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) (ret []expression.Expression, retPlan LogicalPlan) {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (la *LogicalAggregation) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) (ret []expression.Expression, retPlan LogicalPlan) {
	var condsToPush []expression.Expression
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalLimit) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) ([]expression.Expression, LogicalPlan) {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalMaxOneRow) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) ([]expression.Expression, LogicalPlan) {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalWindow) PredicatePushDown(predicates []expression.Expression, opt *coreUtil.LogicalOptimizeOp) ([]expression.Expression, LogicalPlan) {
	canBePushed := make([]expression.Expression, 0, len(predicates))
	canNotBePushed := make([]expression.Expression, 0, len(predicates))
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalMemTable) PredicatePushDown(predicates []expression.Expression, _ *coreUtil.LogicalOptimizeOp) ([]expression.Expression, LogicalPlan) {
	if p.Extractor != nil {
...
func appendTableDualTraceStep(replaced LogicalPlan, dual LogicalPlan, conditions []expression.Expression, opt *coreUtil.LogicalOptimizeOp) {
	action := func() string {
...
func appendSelectionPredicatePushDownTraceStep(p *LogicalSelection, conditions []expression.Expression, opt *coreUtil.LogicalOptimizeOp) {
	action := func() string {
...
func appendDataSourcePredicatePushDownTraceStep(ds *DataSource, opt *coreUtil.LogicalOptimizeOp) {
...
func appendAddSelectionTraceStep(p LogicalPlan, child LogicalPlan, sel *LogicalSelection, opt *coreUtil.LogicalOptimizeOp) {
	reason := func() string {
		return ""
	}
	action := func() string {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalCTE) PredicatePushDown(predicates []expression.Expression, _ *coreUtil.LogicalOptimizeOp) ([]expression.Expression, LogicalPlan) {
...
func (p *LogicalSequence) PredicatePushDown(predicates []expression.Expression, op *coreUtil.LogicalOptimizeOp) ([]expression.Expression, LogicalPlan) {
