...
func TestBitColumnPushDown(t *testing.T) {
	...
	rows := [][]any{
		...
		{"└─Apply_17", "root", "CARTESIAN inner join, other cond:gt(test.t1.b, Column#9)"},
		...
		{"  └─Selection_21(Probe)", "root", "not(isnull(Column#9))"},
		{"    └─StreamAgg_23", "root", "funcs:min(test.t2.b)->Column#9"},
...
func TestIssue54213(t *testing.T) {
  ...
  KEY ab (a,b))`)
	tk.MustQuery(`explain select count(1) from (select /*+ force_index(tb, ab) */ 1 from tb where a=1 and b=1 limit 100) a`).Check(
		testkit.Rows("StreamAgg_11 1.00 root  funcs:count(1)->Column#7",
