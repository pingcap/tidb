...
var OptimizeAstNode func(ctx context.Context, sctx sessionctx.Context, node ast.Node, is infoschema.InfoSchema) (operator.Plan, types.NameSlice, error)
...
func BuildLogicalPlanForTest(ctx context.Context, sctx sessionctx.Context, node ast.Node, infoSchema infoschema.InfoSchema) (operator.Plan, error) {
...
func doOptimize(
	...
	flag uint64,
	logic LogicalPlan,
) (LogicalPlan, operator.PhysicalPlan, float64, error) {
...
func DoOptimize(
	...
	flag uint64,
	logic LogicalPlan,
) (operator.PhysicalPlan, float64, error) {
	sessVars := sctx.GetSessionVars()
...
func mergeContinuousSelections(p operator.PhysicalPlan) {
...
func postOptimize(ctx context.Context, sctx PlanContext, plan operator.PhysicalPlan) (operator.PhysicalPlan, error) {
...
func generateRuntimeFilter(sctx PlanContext, plan operator.PhysicalPlan) {
...
func prunePhysicalColumns(sctx PlanContext, plan operator.PhysicalPlan) error {
...
func prunePhysicalColumnsInternal(sctx PlanContext, plan operator.PhysicalPlan) error {
	var err error
	switch x := plan.(type) {
...
func tryEnableLateMaterialization(sctx PlanContext, plan operator.PhysicalPlan) {
...
*/
func countStarRewrite(plan operator.PhysicalPlan) {
...
func countStarRewriteInternal(plan operator.PhysicalPlan) {
...
func handleFineGrainedShuffle(ctx context.Context, sctx PlanContext, plan operator.PhysicalPlan) {
...
func setupFineGrainedShuffle(ctx context.Context, sctx PlanContext, streamCountInfo *tiflashClusterInfo, tiflashServerCountInfo *tiflashClusterInfo, plan operator.PhysicalPlan) {
	if tableReader, ok := plan.(*PhysicalTableReader); ok {
		if _, isExchangeSender := tableReader.tablePlan.(*PhysicalExchangeSender); isExchangeSender {
...
func setupFineGrainedShuffleInternal(ctx context.Context, sctx PlanContext, plan operator.PhysicalPlan, helper *fineGrainedShuffleHelper, streamCountInfo *tiflashClusterInfo, tiflashServerCountInfo *tiflashClusterInfo) {
	switch x := plan.(type) {
	case *PhysicalWindow:
...
func propagateProbeParents(plan operator.PhysicalPlan, probeParents []operator.PhysicalPlan) {
	...
	case *PhysicalApply, *PhysicalIndexJoin, *PhysicalIndexHashJoin, *PhysicalIndexMergeJoin:
		if join, ok := plan.(interface{ getInnerChildIdx() int }); ok {
			...
			newParents := make([]operator.PhysicalPlan, len(probeParents), len(probeParents)+1)
...
func enableParallelApply(sctx PlanContext, plan operator.PhysicalPlan) operator.PhysicalPlan {
...
func physicalOptimize(logic LogicalPlan, planCounter *PlanCounterTp) (plan operator.PhysicalPlan, cost float64, err error) {
	if logic.SCtx().GetSessionVars().StmtCtx.EnableOptimizerDebugTrace {
		debugtrace.EnterContextCommon(logic.SCtx())
		defer debugtrace.LeaveContextCommon(logic.SCtx())
	}
...
func eliminateUnionScanAndLock(sctx PlanContext, p operator.PhysicalPlan) operator.PhysicalPlan {
	var pointGet *PointGetPlan
	var batchPointGet *BatchPointGetPlan
	var physLock *PhysicalLock
	var unionScan *PhysicalUnionScan
	iteratePhysicalPlan(p, func(p operator.PhysicalPlan) bool {
		if len(p.Children()) > 1 {
			return false
		}
		switch x := p.(type) {
	...
	return transformPhysicalPlan(p, func(p operator.PhysicalPlan) operator.PhysicalPlan {
		if p == physLock {
			return p.Children()[0]
		}
		if p == unionScan {
...
func iteratePhysicalPlan(p operator.PhysicalPlan, f func(p operator.PhysicalPlan) bool) {
	if !f(p) {
		return
	}
	for _, child := range p.Children() {
...
func transformPhysicalPlan(p operator.PhysicalPlan, f func(p operator.PhysicalPlan) operator.PhysicalPlan) operator.PhysicalPlan {
...
func disableReuseChunkIfNeeded(sctx PlanContext, plan operator.PhysicalPlan) {
...
func checkOverlongColType(sctx PlanContext, plan operator.PhysicalPlan) bool {
	if plan == nil {
		return false
	}
	switch plan.(type) {
