...
func buildRangeForTableScan(sctx base.PlanContext, ts *physicalop.PhysicalTableScan) (err error) {
	if ts.Table.IsCommonHandle {
		...
		if len(pkCols) > 0 {
			res, err := ranger.DetachCondAndBuildRangeForIndex(sctx.GetRangerCtx(), ts.AccessCondition, pkCols, pkColsLen, 0, 0)
			if err != nil {
				return err
			}
		...
		if pkCol != nil {
			ranges, accessConds, remainingConds, err := ranger.BuildTableRange(ts.AccessCondition, sctx.GetRangerCtx(), pkCol.RetType, 0, 0)
			if err != nil {
				return err
			}
...
func buildRangesForPointGet(sctx base.PlanContext, x *physicalop.PointGetPlan) (err error) {
	...
	if x.AccessConditions != nil {
		if x.IndexInfo != nil {
			ranges, err := ranger.DetachCondAndBuildRangeForIndex(x.SCtx().GetRangerCtx(), x.AccessConditions, x.IdxCols, x.IdxColLens, 0, 0)
			if err != nil {
				return err
			}
			...
			if pkCol != nil {
				ranges, accessConds, remainingConds, err := ranger.BuildTableRange(x.AccessConditions, x.SCtx().GetRangerCtx(), pkCol.RetType, 0, 0)
				if err != nil {
					return err
				}
...
func buildRangesForBatchGet(sctx base.PlanContext, x *physicalop.BatchPointGetPlan) (err error) {
	...
	if x.AccessConditions != nil {
		if x.IndexInfo != nil {
			ranges, err := ranger.DetachCondAndBuildRangeForIndex(x.GetCtx().GetRangerCtx(), x.AccessConditions, x.IdxCols, x.IdxColLens, 0, 0)
			if err != nil {
				return err
			}
			...
			if pkCol != nil {
				ranges, accessConds, remainingConds, err := ranger.BuildTableRange(x.AccessConditions, x.GetCtx().GetRangerCtx(), pkCol.RetType, 0, 0)
				if err != nil {
					return err
				}
...
func buildRangeForIndexScan(sctx base.PlanContext, is *physicalop.PhysicalIndexScan) (err error) {
	...
	res, err := ranger.DetachCondAndBuildRangeForIndex(sctx.GetRangerCtx(), is.AccessCondition, is.IdxCols, is.IdxColLens, 0, 0)
	if err != nil {
		return err
	}
