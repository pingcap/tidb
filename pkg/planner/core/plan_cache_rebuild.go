...
func updateRange(p base.PhysicalPlan, ranges ranger.Ranges, rangeInfo string) {
	...
	case *PhysicalTableReader:
		updateRange(x.FlatPushedTablePlans()[0], ranges, rangeInfo)
	case *PhysicalIndexReader:
		updateRange(x.FlatPushedIndexPlans()[0], ranges, rangeInfo)
...
func rebuildRange(p base.Plan) error {
	...
	case *PhysicalTableReader:
		err = rebuildRange(x.FlatPushedTablePlans()[0])
		if err != nil {
			return err
		}
	case *PhysicalIndexReader:
		err = rebuildRange(x.FlatPushedIndexPlans()[0])
		if err != nil {
			return err
		}
	case *PhysicalIndexLookUpReader:
