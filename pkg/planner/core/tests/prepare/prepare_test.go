...
func TestPrepareCacheForPartition(t *testing.T) {
	...
	for _, pruneMode := range []string{string(variable.Static), string(variable.Dynamic)} {
		...
		tk.MustExec("create table t_index_read (id int, k int, c varchar(10), primary key (id, k), unique key (k) global) partition by hash(id+k) partitions 10")
		...
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		// Global Index
		tk.MustExec(`prepare gistmt1 from 'select id,k,c from t_index_read where k = ?'`)
		tk.MustExec("set @k=2")
		tk.MustQuery("execute gistmt1 using @k").Check(testkit.Rows("1 2 abc"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=6")
		tk.MustQuery("execute gistmt1 using @k").Check(testkit.Rows("5 6 xyz"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		tk.MustExec(`prepare gistmt2 from 'select id,k,c from t_index_read where k in (?,?)'`)
		tk.MustExec("set @k=2, @k2=4")
		tk.MustQuery("execute gistmt2 using @k, @k2").Sort().Check(testkit.Rows("1 2 abc", "3 4 def"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=6")
		tk.MustQuery("execute gistmt2 using @k, @k2").Sort().Check(testkit.Rows("3 4 def", "5 6 xyz"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		tk.MustExec(`prepare gistmt3 from 'select id,k,c from t_index_read where k >= ? and k <= ?'`)
		tk.MustExec("set @k=2, @k2=4")
		tk.MustQuery("execute gistmt3 using @k, @k2").Sort().Check(testkit.Rows("1 2 abc", "3 4 def"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=4, @k2=6")
		tk.MustQuery("execute gistmt3 using @k, @k2").Sort().Check(testkit.Rows("3 4 def", "5 6 xyz"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))

		// Test for TableScan.
		tk.MustExec("drop table if exists t_table_read")
		tk.MustExec("create table t_table_read (id int, k int, c varchar(10), primary key(id), unique key (k) global) partition by hash(id) partitions 10")
		...
		tk.MustExec("create table t_range_index (id int, k int, c varchar(10), primary key(id), unique key (k) global) partition by range(id) ( PARTITION p0 VALUES LESS THAN (4), PARTITION p1 VALUES LESS THAN (14),PARTITION p2 VALUES LESS THAN (20) )")
		tk.MustExec("insert into t_range_index values (1, 2, 'abc'), (5, 4, 'def'), (13, 6, 'xyz'), (17, 7, 'hij')")
		...
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		// Global Index
		tk.MustExec(`prepare gistmt4 from 'select id,k,c from t_range_index where k = ?'`)
		tk.MustExec("set @k=2")
		tk.MustQuery("execute gistmt4 using @k").Check(testkit.Rows("1 2 abc"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=6")
		tk.MustQuery("execute gistmt4 using @k").Check(testkit.Rows("13 6 xyz"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		tk.MustExec(`prepare gistmt5 from 'select id,k,c from t_range_index where k in (?,?)'`)
		tk.MustExec("set @k=2, @k2=4")
		tk.MustQuery("execute gistmt5 using @k, @k2").Sort().Check(testkit.Rows("1 2 abc", "5 4 def"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=6")
		tk.MustQuery("execute gistmt5 using @k, @k2").Sort().Check(testkit.Rows("13 6 xyz", "5 4 def"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		tk.MustExec(`prepare gistmt6 from 'select id,k,c from t_range_index where k >= ? and k <= ?'`)
		tk.MustExec("set @k=2, @k2=4")
		tk.MustQuery("execute gistmt6 using @k, @k2").Sort().Check(testkit.Rows("1 2 abc", "5 4 def"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=4, @k2=6")
		tk.MustQuery("execute gistmt6 using @k, @k2").Sort().Check(testkit.Rows("13 6 xyz", "5 4 def"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))

		tk.MustExec("drop table if exists t_range_table")
		tk.MustExec("create table t_range_table (id int, k int, c varchar(10), unique key (k) global) partition by range(id) ( PARTITION p0 VALUES LESS THAN (4), PARTITION p1 VALUES LESS THAN (14),PARTITION p2 VALUES LESS THAN (20) )")
		tk.MustExec("insert into t_range_table values (1, 2, 'abc'), (5, 4, 'def'), (13, 6, 'xyz'), (17, 7, 'hij')")
		...
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		// Global Index
		tk.MustExec(`prepare gistmt4 from 'select id,k,c from t_range_table where k = ?'`)
		tk.MustExec("set @k=2")
		tk.MustQuery("execute gistmt4 using @k").Check(testkit.Rows("1 2 abc"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=6")
		tk.MustQuery("execute gistmt4 using @k").Check(testkit.Rows("13 6 xyz"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		tk.MustExec(`prepare gistmt5 from 'select id,k,c from t_range_table where k in (?,?)'`)
		tk.MustExec("set @k=2, @k2=4")
		tk.MustQuery("execute gistmt5 using @k, @k2").Sort().Check(testkit.Rows("1 2 abc", "5 4 def"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=6")
		tk.MustQuery("execute gistmt5 using @k, @k2").Sort().Check(testkit.Rows("13 6 xyz", "5 4 def"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		tk.MustExec(`prepare gistmt6 from 'select id,k,c from t_range_table where k >= ? and k <= ?'`)
		tk.MustExec("set @k=2, @k2=4")
		tk.MustQuery("execute gistmt6 using @k, @k2").Sort().Check(testkit.Rows("1 2 abc", "5 4 def"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=4, @k2=6")
		...
		tk.MustExec("create table t_list_index (id int, k int, c varchar(10), primary key(id), unique key (k) global) partition by list (id*2-id) ( PARTITION p0 VALUES IN (1,2,3,4), PARTITION p1 VALUES IN (5,6,7,8),PARTITION p2 VALUES IN (9,10,11,12))")
		...
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		// Global Index
		tk.MustExec(`prepare gistmt4 from 'select id,k,c from t_list_index where k = ?'`)
		tk.MustExec("set @k=1")
		tk.MustQuery("execute gistmt4 using @k").Check(testkit.Rows("1 1 abc"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=5")
		tk.MustQuery("execute gistmt4 using @k").Check(testkit.Rows("5 5 def"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		tk.MustExec(`prepare gistmt5 from 'select id,k,c from t_list_index where k in (?,?)'`)
		tk.MustExec("set @k=1, @k2=12")
		tk.MustQuery("execute gistmt5 using @k, @k2").Sort().Check(testkit.Rows("1 1 abc", "12 12 hij"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=9")
		tk.MustQuery("execute gistmt5 using @k, @k2").Sort().Check(testkit.Rows("12 12 hij", "9 9 xyz"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))
		tk.MustExec(`prepare gistmt6 from 'select id,k,c from t_list_index where k >= ? and k <= ?'`)
		tk.MustExec("set @k=1, @k2=5")
		tk.MustQuery("execute gistmt6 using @k, @k2").Sort().Check(testkit.Rows("1 1 abc", "5 5 def"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows("0"))
		tk.MustExec("set @k=4, @k2=12")
		tk.MustQuery("execute gistmt6 using @k, @k2").Sort().Check(testkit.Rows("12 12 hij", "5 5 def", "9 9 xyz"))
		tk.MustQuery("select @@last_plan_from_cache").Check(testkit.Rows(planCacheUsed))

		// Test for list columns partition
		tk.MustExec("drop table if exists t_list_index")
		tk.MustExec("create table t_list_index (id int, k int, c varchar(10), primary key(id), unique key (k) global) partition by list columns (id) ( PARTITION p0 VALUES IN (1,2,3,4), PARTITION p1 VALUES IN (5,6,7,8),PARTITION p2 VALUES IN (9,10,11,12))")
		tk.MustExec("insert into t_list_index values (1, 1, 'abc'), (5, 5, 'def'), (9, 9, 'xyz'), (12, 12, 'hij')")
		tk.MustExec(`analyze table t_list_index`)
