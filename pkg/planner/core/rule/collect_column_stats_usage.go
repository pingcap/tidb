...
func (c *columnStatsUsageCollector) collectFromPlan(askedColGroups [][]*expression.Column, lp base.LogicalPlan, accumulatedJoinCols []*expression.Column, accumulatedOrderingCols []*expression.Column) {
	...
	if c.interestingColsByDS != nil {
		...
		case *logicalop.LogicalJoin:
			...
			currentJoinCols = append(currentJoinCols, expression.ExtractColumnsFromExpressions(expression.ScalarFuncs2Exprs(x.EqualConditions), nil, false)...)
			currentJoinCols = append(currentJoinCols, expression.ExtractColumnsFromExpressions(x.OtherConditions, nil, false)...)
		...
		case *logicalop.LogicalAggregation:
			...
			currentOrderingCols = append(currentOrderingCols, expression.ExtractColumnsFromExpressions(x.GroupByItems, nil, false)...)
			...
			for _, aggFunc := range x.AggFuncs {
				if aggFunc.Name == ast.AggFuncMin || aggFunc.Name == ast.AggFuncMax {
					currentOrderingCols = append(currentOrderingCols, expression.ExtractColumnsFromExpressions(aggFunc.Args, nil, false)...)
				}
			}
		}
	}
