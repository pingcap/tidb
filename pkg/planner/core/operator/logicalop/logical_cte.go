...
func (p *LogicalCTE) DeriveStats(_ []*property.StatsInfo, selfSchema *expression.Schema, _ []*expression.Schema, reloads []bool) (*property.StatsInfo, bool, error) {
	...
	var err error
	// Use the CTE seed plan's PlanContext so any CTE-scoped ExprCtx overrides are applied during optimization.
	cteCtx := p.SCtx()
...
func MakeCTEStrictTruncateErrPlanCtx(pctx base.PlanContext) base.PlanContext {
	...
	return planctx.WithExprCtx(pctx, overrideExprCtx)
}
