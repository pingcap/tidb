...
func (la *LogicalAggregation) DeriveStats(childStats []*property.StatsInfo, selfSchema *expression.Schema, childSchema []*expression.Schema,
	...
	if la.StatsInfo() != nil {
		...
		la.StatsInfo().GroupNDVs = la.getGroupNDVs(colGroups, childProfile, gbyCols, inMemo)
...
func (*LogicalAggregation) getGroupNDVs(colGroups [][]*expression.Column, childProfile *property.StatsInfo, gbyCols []*expression.Column, inMemo bool) []property.GroupNDV {
	if len(colGroups) == 0 && !inMemo {
		// previously, if parent doesn't ask for column groups, we don't maintain the group ndv info in leaves.
		return nil
	}
