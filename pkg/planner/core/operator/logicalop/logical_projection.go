				...
				if col.ID == 0 && p.Schema().Contains(col) {
					...
					if !p.Children()[0].Schema().Contains(col) {
						...
						newTopN := LogicalTopN{
							Count:            topN.Count,
							Offset:           topN.Offset,
							ByItems:          make([]*util.ByItems, 0, len(p.Schema().Columns)),
							PreferLimitToCop: topN.PreferLimitToCop,
							PartitionBy:      topN.GetPartitionBy(),
						}.Init(topN.SCtx(), topN.QueryBlockOffset())
						for idx, byitem := range p.Schema().Columns {
							by := &util.ByItems{
								Expr: byitem,
								Desc: topN.ByItems[idx].Desc,
							}
							newTopN.ByItems = append(newTopN.ByItems, by)
						}
						return newTopN.AttachChild(p, opt)
					}
				}
			}
		}
