...
func (p *BaseLogicalPlan) RecursiveDeriveStats(colGroups [][]*expression.Column) (*property.StatsInfo, error) {
	...
	return p.self.DeriveStats(childStats, p.self.Schema(), childSchema, colGroups, false)
}

// DeriveStats implements LogicalPlan.<11th> interface.
func (p *BaseLogicalPlan) DeriveStats(childStats []*property.StatsInfo, selfSchema *expression.Schema, _ []*expression.Schema, _ [][]*expression.Column, _ bool) (*property.StatsInfo, error) {
	if len(childStats) == 1 {
