...
func (ds *DataSource) DeriveStats(_ []*property.StatsInfo, _ *expression.Schema, _ []*expression.Schema, _ []bool) (*property.StatsInfo, bool, error) {
	...
	if ds.StatsInfo() != nil {
		...
		ds.SetStats(ds.TableStats.Scale(ds.SCtx().GetSessionVars(), selectivity))
		return ds.StatsInfo(), false, nil
	}
	...
	ds.SetStats(ds.deriveStatsByFilter(ds.PushedDownConds, ds.AllPossibleAccessPaths))
	...
	if err := utilfuncp.GenerateIndexMergePath(ds); err != nil {
		return nil, false, err
	}

	if ds.SCtx().GetSessionVars().StmtCtx.EnableOptimizerDebugTrace {
		utilfuncp.DebugTraceAccessPaths(ds.SCtx(), ds.PossibleAccessPaths)
	}
	indexForce := false
...
func (ds *DataSource) initStats() {
	...
	ds.TableStats.GroupNDVs = ds.getGroupNDVs()
...
func (ds *DataSource) getGroupNDVs() []property.GroupNDV {
...
func (ds *DataSource) DeriveTablePathStats(path *util.AccessPath, conds []expression.Expression, isIm bool) error {
	...
	if !isIm {
		...
		ds.adjustCountAfterAccess(path)
	}
	return err
}
...
func (ds *DataSource) deriveCommonHandleTablePathStats(path *util.AccessPath, conds []expression.Expression, isIm bool) error {
	...
	if err := stats.DetachCondAndBuildRangeForPath(ds.SCtx(), path, conds, ds.TableStats.HistColl); err != nil {
		return err
	}
...
// FillIndexPath fill the index path's stats.
func (ds *DataSource) FillIndexPath(path *util.AccessPath, conds []expression.Expression) error {
	...
	err := stats.DetachCondAndBuildRangeForPath(ds.SCtx(), path, conds, ds.TableStats.HistColl)
	return err
}

// adjustCountAfterAccess adjusts the CountAfterAccess when it's less than the estimated table row count.
func (ds *DataSource) adjustCountAfterAccess(path *util.AccessPath) {
...
// SplitIndexFilterConditions split the index filter conditions.
