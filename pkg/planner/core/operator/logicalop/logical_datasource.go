...
func (ds *DataSource) FindBestTask(prop *property.PhysicalProperty, planCounter *base.PlanCounterTp, opt *optimizetrace.PhysicalOptimizeOp) (t base.Task, cntPlan int64, err error) {
	return utilfuncp.FindBestTask4LogicalDS(ds, prop, planCounter, opt)
}
...
func (ds *DataSource) PredicateSimplification(*optimizetrace.LogicalOptimizeOp) base.LogicalPlan {
	...
	p.PushedDownConds = ruleutil.ApplyPredicateSimplification(p.SCtx(), p.PushedDownConds)
	p.AllConds = ruleutil.ApplyPredicateSimplification(p.SCtx(), p.AllConds)
	return p
}
...
func (ds *DataSource) DeriveStats(_ []*property.StatsInfo, _ *expression.Schema, _ []*expression.Schema, colGroups [][]*expression.Column) (*property.StatsInfo, error) {
	return utilfuncp.DeriveStats4DS(ds, colGroups)
}
...
// GetPKIsHandleCol	returns the handle column if the PKIsHandle is true, otherwise returns the ExtraHandleColumn.
...
// NewExtraHandleSchemaCol creates a new column for extra handle column.
...
// IsIndexCoveringColumns returns true if the column is an integer type.
...
// IsIndexCoveringCondition checks if the index is covering the condition.
...
// IsSingleScan checks if the DataSource can be retrieved by a single index scan.
