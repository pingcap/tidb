...
func (p *PhysicalIndexReader) RequestFTSBM25ScoreColumn() (*expression.Column, bool, error) {
	...
	outputCol, err := scoreCol.ResolveIndices(indexScan.Schema())
	if err != nil {
	...
	p.PhysicalSchemaProducer.SetSchema(schema)
	p.OutputColumns = append(p.OutputColumns, outputCol.(*expression.Column))
