...
func (c *CollectPredicateColumnsPoint) Optimize(_ context.Context, plan base.LogicalPlan, _ *optimizetrace.LogicalOptimizeOp) (base.LogicalPlan, bool, error) {
	...
	// opNum is collected via the common stats load rule, some operators may not be cleaned like proj for later rule.
	// opNum is not that accurate, but it's enough for the memo hashmap's init capacity.
