...
func (collectPredicateColumnsPoint) optimize(_ context.Context, plan LogicalPlan, _ *coreUtil.LogicalOptimizeOp) (LogicalPlan, bool, error) {
	planChanged := false
...
func (syncWaitStatsLoadPoint) optimize(_ context.Context, plan LogicalPlan, _ *coreUtil.LogicalOptimizeOp) (LogicalPlan, bool, error) {
	planChanged := false
	if plan.SCtx().GetSessionVars().InRestrictedSQL {
		return plan, planChanged, nil
	}
