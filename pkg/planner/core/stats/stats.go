...
func DeriveStats(lp base.LogicalPlan, childStats []*property.StatsInfo, selfSchema *expression.Schema, childSchema []*expression.Schema, reloads []bool) (*property.StatsInfo, bool, error) {
	switch l := lp.(type) {
	case *logicalop.LogicalTableScan:
		return deriveStats4LogicalTableScan(l)
	case *logicalop.LogicalIndexScan:
		return deriveStats4LogicalIndexScan(l, selfSchema)
	}
}

// deriveStats4LogicalTableScan derives statistics for LogicalTableScan operator.
...
func loadTableStats(ctx sessionctx.Context, tblInfo *model.TableInfo, pid int64) {
	...
	pctx := ctx.GetPlanCtx()
	tableStats := statsutil.GetStatsTable(pctx, tblInfo, pid)
