...
func getProj(ctx sessionctx.Context, p PhysicalPlan) *PhysicalProjection {
	...
	}.Init(ctx, p.StatsInfo(), p.SelectOffset())
...
func buildIndexLookUpTask(ctx sessionctx.Context, t *copTask) *rootTask {
	...
	}.Init(ctx, t.tablePlan.SelectOffset())
	...
	if t.needExtraProj && !aggPushedDown {
		...
		proj := PhysicalProjection{Exprs: expression.Column2Exprs(schema.Columns)}.Init(ctx, p.StatsInfo(), t.tablePlan.SelectOffset(), nil)
...
func (t *copTask) convertToRootTaskImpl(ctx sessionctx.Context) *rootTask {
	...
	if t.idxMergePartPlans != nil {
		p := PhysicalIndexMergeReader{
			...
			KeepOrder:          t.keepOrder,
		}.Init(ctx, t.idxMergePartPlans[0].SelectOffset())
		p.PartitionInfo = t.partitionInfo
		setTableScanToTableRowIDScan(p.tablePlan)
		newTask.p = p
		...
		if t.needExtraProj {
			schema := t.originSchema
			proj := PhysicalProjection{Exprs: expression.Column2Exprs(schema.Columns)}.Init(ctx, p.StatsInfo(), t.idxMergePartPlans[0].SelectOffset(), nil)
			proj.SetSchema(schema)
			proj.SetChildren(p)
			newTask.p = proj
		}
	...
	if t.indexPlan != nil && t.tablePlan != nil {
		...
		p := PhysicalIndexReader{indexPlan: t.indexPlan}.Init(ctx, t.indexPlan.SelectOffset())
		p.PartitionInfo = t.partitionInfo
		...
		if t.needExtraProj && !aggPushedDown {
			proj := PhysicalProjection{Exprs: expression.Column2Exprs(t.originSchema.Columns)}.Init(ts.SCtx(), ts.StatsInfo(), ts.SelectOffset(), nil)
...
func (t *copTask) handleRootTaskConds(ctx sessionctx.Context, newTask *rootTask) {
	if len(t.rootTaskConds) > 0 {
		...
		sel := PhysicalSelection{Conditions: t.rootTaskConds}.Init(ctx, newTask.p.StatsInfo().Scale(selectivity), newTask.p.SelectOffset())
...
func (p *PhysicalLimit) attach2Task(tasks ...task) task {
	...
	if cop, ok := t.(*copTask); ok {
		suspendLimitAboveTablePlan := func() {
			...
			pushedDownLimit := PhysicalLimit{PartitionBy: newPartitionBy, Count: newCount}.Init(p.SCtx(), stats, p.SelectOffset())
		...
		stats := deriveLimitStats(childProfile, float64(newCount))
		pushedDownLimit := PhysicalLimit{Count: newCount, PartitionBy: newPartitionBy}.Init(p.SCtx(), stats, p.SelectOffset())
...
func (p *PhysicalLimit) sinkIntoIndexLookUp(t task) bool {
	...
	if p.Schema().Len() != reader.Schema().Len() {
		...
		}.Init(p.SCtx(), p.StatsInfo(), p.SelectOffset(), nil)
...
func (p *PhysicalTopN) getPushedDownTopN(childPlan PhysicalPlan) *PhysicalTopN {
	...
	topN := PhysicalTopN{
		...
		Count:       newCount,
	}.Init(p.SCtx(), stats, p.SelectOffset(), p.GetChildReqProps(0))
...
func (p *basePhysicalAgg) convertAvgForMPP() *PhysicalProjection {
	...
	}.Init(p.SCtx(), p.StatsInfo(), p.SelectOffset(), p.GetChildReqProps(0).CloneEssentialFields())
...
func (p *basePhysicalAgg) newPartialAggregate(copTaskType kv.StoreType, isMPPTask bool) (partial, final PhysicalPlan) {
	...
	if p.TP() == plancodec.TypeStreamAgg {
		...
		}.initForStream(p.SCtx(), p.StatsInfo(), p.SelectOffset(), prop)
	...
	finalAgg := basePhysicalAgg{
		...
		GroupByItems: finalPref.GroupByItems,
		MppRunMode:   p.MppRunMode,
	}.initForHash(p.SCtx(), p.StatsInfo(), p.SelectOffset(), prop)
	finalAgg.schema = finalPref.Schema
...
func (p *PhysicalHashAgg) adjust3StagePhaseAgg(partialAgg, finalAgg PhysicalPlan, canUse3StageAgg bool,
	...
	}.Init(p.SCtx(), stats, mpp.p.SelectOffset())
	...
	cloneHashAgg.Plan = base.NewBasePlan(cloneHashAgg.SCtx(), cloneHashAgg.TP(), cloneHashAgg.SelectOffset())
	...
	proj4Partial := new(PhysicalProjection).Init(p.SCtx(), mpp.p.StatsInfo(), mpp.p.SelectOffset())
...
func (t *mppTask) convertToRootTaskImpl(ctx sessionctx.Context) *rootTask {
	...
	if len(t.rootTaskConds) > 0 {
		...
		sel := PhysicalSelection{Conditions: t.rootTaskConds}.Init(ctx, rt.p.StatsInfo().Scale(selectivity), rt.p.SelectOffset())
		sel.fromDataSource = true
