...
func getProj(ctx sessionctx.Context, p PhysicalPlan) *PhysicalProjection {
	...
	}.Init(ctx, p.StatsInfo(), p.QBOffset())
...
func buildIndexLookUpTask(ctx sessionctx.Context, t *copTask) *rootTask {
	...
	if t.needExtraProj && !aggPushedDown {
		...
		proj := PhysicalProjection{Exprs: expression.Column2Exprs(schema.Columns)}.Init(ctx, p.StatsInfo(), t.tablePlan.QBOffset(), nil)
...
func (t *copTask) convertToRootTaskImpl(ctx sessionctx.Context) *rootTask {
	...
	if t.idxMergePartPlans != nil {
		p := PhysicalIndexMergeReader{
			...
			KeepOrder:          t.keepOrder,
		}.Init(ctx, t.idxMergePartPlans[0].QBOffset())
		p.PartitionInfo = t.partitionInfo
		setTableScanToTableRowIDScan(p.tablePlan)
		newTask.p = p
		...
		if t.needExtraProj {
			schema := t.originSchema
			proj := PhysicalProjection{Exprs: expression.Column2Exprs(schema.Columns)}.Init(ctx, p.StatsInfo(), t.idxMergePartPlans[0].QBOffset(), nil)
			proj.SetSchema(schema)
			proj.SetChildren(p)
			newTask.p = proj
		}
	...
	if t.indexPlan != nil && t.tablePlan != nil {
		...
		p := PhysicalIndexReader{indexPlan: t.indexPlan}.Init(ctx, t.indexPlan.QBOffset())
		p.PartitionInfo = t.partitionInfo
		...
		if t.needExtraProj && !aggPushedDown {
			proj := PhysicalProjection{Exprs: expression.Column2Exprs(t.originSchema.Columns)}.Init(ts.SCtx(), ts.StatsInfo(), ts.QBOffset(), nil)
...
func (t *copTask) handleRootTaskConds(ctx sessionctx.Context, newTask *rootTask) {
	if len(t.rootTaskConds) > 0 {
		...
		sel := PhysicalSelection{Conditions: t.rootTaskConds}.Init(ctx, newTask.p.StatsInfo().Scale(selectivity), newTask.p.QBOffset())
...
func (p *PhysicalLimit) attach2Task(tasks ...task) task {
	...
	if cop, ok := t.(*copTask); ok {
		suspendLimitAboveTablePlan := func() {
			...
			pushedDownLimit := PhysicalLimit{PartitionBy: newPartitionBy, Count: newCount}.Init(p.SCtx(), stats, p.QBOffset())
		...
		stats := deriveLimitStats(childProfile, float64(newCount))
		pushedDownLimit := PhysicalLimit{Count: newCount, PartitionBy: newPartitionBy}.Init(p.SCtx(), stats, p.QBOffset())
...
func (p *PhysicalLimit) sinkIntoIndexLookUp(t task) bool {
	...
	if p.Schema().Len() != reader.Schema().Len() {
		...
		}.Init(p.SCtx(), p.StatsInfo(), p.QBOffset(), nil)
...
func (p *PhysicalTopN) getPushedDownTopN(childPlan PhysicalPlan) *PhysicalTopN {
	...
	topN := PhysicalTopN{
		...
		Count:       newCount,
	}.Init(p.SCtx(), stats, p.QBOffset(), p.GetChildReqProps(0))
...
func (p *basePhysicalAgg) convertAvgForMPP() *PhysicalProjection {
	...
	}.Init(p.SCtx(), p.StatsInfo(), p.QBOffset(), p.GetChildReqProps(0).CloneEssentialFields())
...
func (p *basePhysicalAgg) newPartialAggregate(copTaskType kv.StoreType, isMPPTask bool) (partial, final PhysicalPlan) {
	...
	if p.TP() == plancodec.TypeStreamAgg {
		...
		}.initForStream(p.SCtx(), p.StatsInfo(), p.QBOffset(), prop)
	...
	finalAgg := basePhysicalAgg{
		...
		GroupByItems: finalPref.GroupByItems,
		MppRunMode:   p.MppRunMode,
	}.initForHash(p.SCtx(), p.StatsInfo(), p.QBOffset(), prop)
	finalAgg.schema = finalPref.Schema
...
func (p *PhysicalHashAgg) adjust3StagePhaseAgg(partialAgg, finalAgg PhysicalPlan, canUse3StageAgg bool,
	...
	}.Init(p.SCtx(), stats, mpp.p.QBOffset())
	...
	cloneHashAgg.Plan = base.NewBasePlan(cloneHashAgg.SCtx(), cloneHashAgg.TP(), cloneHashAgg.QBOffset())
	...
	proj4Partial := new(PhysicalProjection).Init(p.SCtx(), mpp.p.StatsInfo(), mpp.p.QBOffset())
...
func (t *mppTask) convertToRootTaskImpl(ctx sessionctx.Context) *rootTask {
	...
	if len(t.rootTaskConds) > 0 {
		...
		sel := PhysicalSelection{Conditions: t.rootTaskConds}.Init(ctx, rt.p.StatsInfo().Scale(selectivity), rt.p.QBOffset())
		sel.fromDataSource = true
