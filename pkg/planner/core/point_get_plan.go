...
func tryWhereIn2BatchPointGet(ctx base.PlanContext, selStmt *ast.SelectStmt, resolveCtx *resolve.Context) *physicalop.BatchPointGetPlan {
	...
	if tbl.SoftdeleteInfo != nil && tbl.SoftdeleteInfo.Enable && ctx.GetSessionVars().SoftDeleteRewrite {
		return nil
	}
			...
			if field.WildCard != nil {
				...
				for _, col := range tbl.Columns {
					...
					if (tbl.SoftdeleteInfo != nil &&
						tbl.SoftdeleteInfo.Enable &&
						col.Name.L == model.ExtraSoftDeleteTimeName.L) ||
						(tbl.IsActiveActive &&
