...
func (p *rangePruner) extractDataForPrune(sctx sessionctx.Context, expr expression.Expression) (dataForPrune, bool) {
	...
	constLevel := constExpr.ConstLevel()
	if constLevel == expression.ConstNone {
		return ret, false
	}
	if sctx.GetSessionVars().StmtCtx.UseCache && constLevel != expression.ConstStrict {
		// When use cache, expression will be shared across contexts.
		// We should make sure it is always an constant
		return ret, false
	}
