...
func (s *partitionProcessor) processHashOrKeyPartition(ds *DataSource, pi *model.PartitionInfo, opt *logicalOptimizeOp) (LogicalPlan, error) {
	...
	tableDual := LogicalTableDual{RowCount: 0}.Init(ds.SCtx(), ds.SelectOffset())
...
func (s *partitionProcessor) makeUnionAllChildren(ds *DataSource, pi *model.PartitionInfo, or partitionRangeOR, opt *logicalOptimizeOp) (LogicalPlan, error) {
	...
	for _, r := range or {
		for i := r.start; i < r.end; i++ {
			...
			// Not a deep copy.
			newDataSource := *ds
			newDataSource.baseLogicalPlan = newBaseLogicalPlan(ds.SCtx(), plancodec.TypeTableScan, &newDataSource, ds.SelectOffset())
	...
	unionAll := LogicalPartitionUnionAll{}.Init(ds.SCtx(), ds.SelectOffset())
