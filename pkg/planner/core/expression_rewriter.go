...
func (er *expressionRewriter) handleCompareSubquery(ctx context.Context, planCtx *exprRewriterPlanCtx, v *ast.CompareSubqueryExpr) (ast.Node, bool) {
	...
	noDecorrelate := isNoDecorrelate(planCtx, corCols, false, hintFlags)
...
func (er *expressionRewriter) handleExistSubquery(ctx context.Context, planCtx *exprRewriterPlanCtx, v *ast.ExistsSubqueryExpr) (ast.Node, bool) {
	...
	corCols := coreusage.ExtractCorColumnsBySchema4LogicalPlan(np, planCtx.plan.Schema())
	noDecorrelate := isNoDecorrelate(planCtx, corCols, true, hintFlags)
...
func isNoDecorrelate(planCtx *exprRewriterPlanCtx, corCols []*expression.CorrelatedColumn, overrideWithVar bool, hintFlags uint64) bool {
	...
	} else if overrideWithVar && len(corCols) > 0 { // overrideWithVar allows scalar and exists to force decorrelate
		// If both hash join and merge join cost factors are > 1, we can apply NO_DECORRELATE.
		hashJoinCostFactor := planCtx.builder.ctx.GetSessionVars().HashJoinCostFactor > 1
		mergeJoinCostFactor := planCtx.builder.ctx.GetSessionVars().MergeJoinCostFactor > 1
		if hashJoinCostFactor && mergeJoinCostFactor {
			noDecorrelate = true
		}
	}
...
func (er *expressionRewriter) handleScalarSubquery(ctx context.Context, planCtx *exprRewriterPlanCtx, v *ast.SubqueryExpr) (ast.Node, bool) {
	...
	noDecorrelate := isNoDecorrelate(planCtx, correlatedColumn, true, hintFlags)
