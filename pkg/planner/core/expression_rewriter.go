...
func (b *PlanBuilder) rewriteInsertOnDuplicateUpdate(ctx context.Context, exprNode ast.ExprNode, mockPlan base.LogicalPlan, insertPlan *Insert) (expression.Expression, error) {
	...
	expr, _, err := rewriteExprNode(rewriter, exprNode, true, nil)
	return expr, err
}
...
func (b *PlanBuilder) getExpressionRewriter(ctx context.Context, p base.LogicalPlan) (rewriter *expressionRewriter) {
	...
	if len(b.rewriterPool) < b.rewriterCounter {
		rewriter = &expressionRewriter{
			...
			planCtx: &exprRewriterPlanCtx{plan: p, builder: b, curClause: b.curClause, rollExpand: b.currentBlockExpand, columnVisited: make([]*ast.ColumnName, 0, 8)},
		}
...
// rewriteVectorSearchFuncCallWithEmbedding rewrites VEC_EMBED_XX_DISTANCE() function family
// to VEC_XX_DISTANCE() according to the auto-embedding info in the referred column.
// It should return true if for the given FuncCallExpr a rewrite is performed so that original behavior is skipped.
// Otherwise it should return false to indicate (the caller) that original behavior needs to be performed.
func (er *expressionRewriter) rewriteVectorSearchFuncCallWithEmbedding(v *ast.FuncCallExpr) bool {
	var distanceFn string
	switch v.FnName.L {
	case ast.VecEmbedL1Distance:
		distanceFn = ast.VecL1Distance
	case ast.VecEmbedL2Distance:
		distanceFn = ast.VecL2Distance
	case ast.VecEmbedNegativeInnerProduct:
		distanceFn = ast.VecNegativeInnerProduct
	case ast.VecEmbedCosineDistance:
		distanceFn = ast.VecCosineDistance
	default:
		return false
	}
	...
	// For VEC_EMBED_XX_DISTANCE functions, we expect arguments to be:
	// Arg0: Must be a generated embedding column
	// Arg1: Any expression is allowed

	stackLen := len(er.ctxStack)
	args := []expression.Expression{
		er.ctxStack[stackLen-2], // LHS
		er.ctxStack[stackLen-1], // RHS
	}
	er.ctxStackPop(len(v.Args))
	...
	if vecArg.GeneratedExprString == "" {
		er.err = fmt.Errorf("%s() first argument must be a vector embedding column generated by EMBED_TEXT()", v.FnName.O)
		return true
	}
	if !vecArg.RetType.EvalType().IsVectorKind() {
		er.err = fmt.Errorf("%s() first argument must be a vector embedding column generated by EMBED_TEXT()", v.FnName.O)
		return true
	}
	astExpr, err := generatedexpr.ParseExpression(vecArg.GeneratedExprString)
	if err != nil {
		er.err = errors.Trace(err)
		return true
	}
	embedInfo, err := expression.ExtractAutoEmbedInfoFromAST(astExpr)
	if err != nil {
		er.err = fmt.Errorf("column %s cannot be used in %s: %s",
			vecArg.OrigName,
			v.FnName.O,
			err.Error())
		return true
	...
	// Rewrite rule:
	// Before: VEC_EMBED_COSINE_DISTANCE(vec, query)
	// After:  VEC_COSINE_DISTANCE(vec, TEXT_EMBED(<model>, query, <config>))

	// Build TEXT_EMBED(<model>, query, <config>)
	textEmbedArgs := make([]expression.Expression, 0, 3)
	textEmbedArgs = append(textEmbedArgs, &expression.Constant{
		Value:   types.NewDatum(embedInfo.ModelNameWithProvider),
		RetType: types.NewFieldType(mysql.TypeString),
	})
	textEmbedArgs = append(textEmbedArgs, args[1])
	if len(embedInfo.OptsInJSON) > 0 {
		textEmbedArgs = append(textEmbedArgs, &expression.Constant{
			Value:   types.NewDatum(embedInfo.OptsInJSON),
			RetType: types.NewFieldType(mysql.TypeString),
		})
	}
	fnInit := func(sf *expression.ScalarFunction) (*expression.ScalarFunction, error) {
		sf.Function.(*expression.BuiltinEmbedTextSig).IsFromVecSearch = true
		return sf, nil
	}
	...
	// Build VEC_COSINE_DISTANCE(vec, ...)
	vecSearchArgs := make([]expression.Expression, 2)
	vecSearchArgs[0] = vecArg
	vecSearchArgs[1] = funcTextEmbed
	funcVecSearch, err := er.newFunction(
		distanceFn,
		&v.Type,
		vecSearchArgs...)
	if err != nil {
	...
	er.ctxStackAppend(funcVecSearch, types.EmptyName)
	return true
}
...
func (er *expressionRewriter) funcCallToExpression(v *ast.FuncCallExpr) {
	...
	if er.rewriteVectorSearchFuncCallWithEmbedding(v) {
		return
	}
...
func (er *expressionRewriter) toColumn(v *ast.ColumnName) {
	...
	if idx >= 0 {
		...
		// We don't need to set the columnVisited here, because the er.planCtx
		return
	}
