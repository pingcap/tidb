...
func rewriteAstExprWithPlanCtxWithVisitInfos(sctx base.PlanContext, expr ast.ExprNode, schema *expression.Schema,
	names types.NameSlice, allowCastArray bool) (expression.Expression, []visitInfo, error) {
...
// priv: records the type of privilege that needs to be checked, UsagePriv means no need to check.
...
func (b *PlanBuilder) appendColumnsToVisitedInfo(columnVisited []*ast.ColumnName, priv *checkedPrivilege) {
	if priv == nil || priv.columnPriv == mysql.UsagePriv {
		return
	}
	...
	if priv.unfoldFromWildcard {
		for _, colName := range columnVisited {
			b.visitInfo = appendVisitInfo(b.visitInfo,
				...
				colName.Table.L,
				colName.Name.L,
				plannererrors.ErrTableaccessDenied.FastGenByArgs(strings.ToUpper(mysql.Priv2Str[priv.columnPriv]), user, host, colName.Table.L),
			)
		}
	} else {
		for _, colName := range columnVisited {
			b.visitInfo = appendVisitInfo(b.visitInfo,
				...
				colName.Table.L,
				colName.Name.L,
				plannererrors.ErrColumnaccessDenied.FastGenByArgs(strings.ToUpper(mysql.Priv2Str[priv.columnPriv]), user, host, colName.Name.L, colName.Table.L),
			)
		}
	}
}
