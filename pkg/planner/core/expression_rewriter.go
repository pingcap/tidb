...
// rewriteVectorSearchFuncCallWithEmbedding rewrites VEC_EMBED_*_DISTANCE() functions
// to VEC_*_DISTANCE() using the EMBED_TEXT metadata stored on a generated vector column.
func (er *expressionRewriter) rewriteVectorSearchFuncCallWithEmbedding(v *ast.FuncCallExpr) bool {
	...
	if !ok || vecArg.GeneratedExprString == "" || !vecArg.RetType.EvalType().IsVectorKind() {
		...
		er.err = fmt.Errorf("%s() first argument must be a vector embedding column generated by EMBED_TEXT(), got %s", v.FnName.O, argName)
		return true
	}
	...
	if err != nil {
		...
		if colName == "" {
			colName = vecArg.StringWithCtx(er.sctx.GetEvalCtx(), errors.RedactLogDisable)
		}
		er.err = errors.Annotatef(err, "column %s cannot be used in %s", colName, v.FnName.O)
		return true
	}
