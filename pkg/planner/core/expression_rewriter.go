...
func (er *expressionRewriter) handleScalarSubquery(ctx context.Context, planCtx *exprRewriterPlanCtx, v *ast.SubqueryExpr) (ast.Node, bool) {
	...
	if planCtx.builder.disableSubQueryPreprocessing || len(coreusage.ExtractCorrelatedCols4LogicalPlan(np)) > 0 || hasCTEConsumerInSubPlan(np) {
		planCtx.builder.onAutoEmbedOpaqueBoundary(np.Schema())
...
func (er *expressionRewriter) autoEmbedVectorSearchArg(fnName string, arg expression.Expression) (*expression.Column, *expression.AutoEmbedInfo, error) {
	...
	var embedInfo *expression.AutoEmbedInfo
	hasAutoEmbedInfo := false
	if ok && er.planCtx != nil && er.planCtx.builder != nil {
		embedInfo, hasAutoEmbedInfo = er.planCtx.builder.autoEmbedTracker().LookupColumn(vecArg)
	}
	argName := "<unknown column>"
	if ok {
		argName = vecArg.OrigName
		if argName == "" {
			argName = vecArg.StringWithCtx(er.sctx.GetEvalCtx(), errors.RedactLogDisable)
		}
	}
	if ok && vecArg.RetType != nil && vecArg.RetType.EvalType().IsVectorKind() && hasAutoEmbedInfo {
		if intest.InTest {
			er.planCtx.builder.autoEmbedOracle().recordVectorSearchHit(er.planCtx.plan, fnName, argName, vecArg, embedInfo)
		}
		return vecArg, embedInfo, nil
	}

	return nil, nil, fmt.Errorf("%s() first argument must be a vector embedding column generated by EMBED_TEXT(), got %s", fnName, argName)
}

func (er *expressionRewriter) buildEmbedTextForVectorSearch(embedInfo *expression.AutoEmbedInfo, queryArg expression.Expression) (expression.Expression, error) {
	...
	textEmbedArgs = append(textEmbedArgs, queryArg)
	if embedInfo.OptsInJSON != "" {
		textEmbedArgs = append(textEmbedArgs, &expression.Constant{
	...
	return er.newFunctionWithInit(ast.EmbedText, types.NewFieldType(mysql.TypeUnspecified), fnInit, textEmbedArgs...)
}
...
func (er *expressionRewriter) rewriteVectorSearchFuncCallWithEmbedding(v *ast.FuncCallExpr) bool {
	...
	args := []expression.Expression{
		er.ctxStack[stackLen-2],
		er.ctxStack[stackLen-1],
	}
	er.ctxStackPop(len(v.Args))

	vecArg, embedInfo, err := er.autoEmbedVectorSearchArg(v.FnName.O, args[0])
	if err != nil {
		er.err = err
		return true
	}
	embedExpr, err := er.buildEmbedTextForVectorSearch(embedInfo, args[1])
	if err != nil {
		er.err = err
		return true
	}

	vecSearchArgs := []expression.Expression{vecArg, embedExpr}
	function, err := er.newFunction(distanceFn, &v.Type, vecSearchArgs...)
	if err != nil {
		er.err = err
		return true
	}
