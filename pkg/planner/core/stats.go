...
func splitMLogCommitTSFilterSelectivity(
	...
	hasCommitTSFilter := false
	for _, cond := range conds {
		if isMLogCommitTSFilter(ds, estimation, cond) {
			hasCommitTSFilter = true
			continue
		}
		remainingConds = append(remainingConds, cond)
	}
	if !hasCommitTSFilter {
		return conds, 0, false
	}
	selectivity, ok := estimateMLogCommitTSSelectivity(estimation, ds.TableInfo)
	if !ok {
		return conds, 0, false
	}
...
// isMLogCommitTSFilter only matches the commit-ts predicates generated by mview.Build for the refresh window.
// User-specified `_tidb_commit_ts` predicates must stay in the normal selectivity path.
func isMLogCommitTSFilter(ds *logicalop.DataSource, estimation *planctx.MLogCommitTSEstimation, expr expression.Expression) bool {
	...
	if !ok || col.ID != model.ExtraCommitTSID {
		return false
	}
	value, isNull, ok := expression.GetUint64FromConstant(ds.SCtx().GetExprCtx().GetEvalCtx(), args[1])
	if !ok || isNull {
		return false
	}
	switch sf.FuncName.L {
	case ast.GT:
		return value == estimation.RefreshLowerTSO
	case ast.LE:
		return estimation.HasRefreshUpperTSOFilter && value == estimation.RefreshUpperTSO
	default:
		return false
	}
}
...
func estimateMLogCommitTSSelectivity(
	...
	if estimation.RefreshUpperTSO == 0 || estimation.RetainedUpperTSO == 0 {
		return 0, false
	}
	...
	effectiveRefreshLowerTSO := estimation.RefreshLowerTSO
	if retainedLowerTSO > effectiveRefreshLowerTSO {
		effectiveRefreshLowerTSO = retainedLowerTSO
	}
	...
	refreshLowerPhysical := oracle.ExtractPhysical(effectiveRefreshLowerTSO)
	refreshUpperPhysical := oracle.ExtractPhysical(estimation.RefreshUpperTSO)
	retainedDuration := retainedUpperPhysical - retainedLowerPhysical
	refreshDuration := refreshUpperPhysical - refreshLowerPhysical
	if retainedDuration <= 0 || refreshDuration <= 0 {
		return 0, false
	}

	selectivity := float64(refreshDuration) / float64(retainedDuration)
	if selectivity < 0 {
		selectivity = 0
