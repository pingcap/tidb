...
func detachCondAndBuildOptimalRangeForIndex(
	...
	// If there are no IN-lists to optimize (all are equality conditions) or the fix control
	// is not enabled, return early
	if res.EqOrInCount <= res.EqCondCount || !fixcontrol.GetBoolWithDefault(sctx.GetSessionVars().OptimizerFixControl, fixcontrol.Fix63487, false) {
	...
	bestCost := cost.SeekFactor*float64(len(res.Ranges)) + rowCount.Est

	for cutoff := res.EqCondCount; cutoff < res.EqOrInCount; cutoff++ {
		limitedCols := cols[:cutoff]
		limitedLens := colLens[:cutoff]

		altRes, err := ranger.DetachCondAndBuildRangeForIndex(
			sctx.GetRangerCtx(), conds, limitedCols, limitedLens,
			sctx.GetSessionVars().RangeMaxSize)
		if err != nil {
			continue
		}

		altIndexCols := limitedCols
		if len(altIndexCols) > len(limitedLens) {
			altIndexCols = altIndexCols[0:len(limitedLens)]
		}
		...
		altCost := cost.SeekFactor*float64(len(altRes.Ranges)) + altRowCount.Est
