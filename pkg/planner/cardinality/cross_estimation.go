...
func crossEstimateRowCount(sctx planctx.PlanContext,
	...
	if idxExists {
		rangeCount, err = GetRowCountByIndexRanges(sctx, dsTableStats.HistColl, idxID, convertedRanges, nil)
	} else {
...
func getColumnRangeCounts(sctx planctx.PlanContext, colID int64, ranges []*ranger.Range, histColl *statistics.HistColl, idxID int64) ([]float64, bool) {
	...
	for i, ran := range ranges {
		if idxID >= 0 {
			...
			count, err = GetRowCountByIndexRanges(sctx, histColl, idxID, []*ranger.Range{ran}, nil)
		} else {
