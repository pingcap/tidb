...
func getIndexRowCountForStatsV2(sctx planctx.PlanContext, idx *statistics.Index, coll *statistics.HistColl, indexRanges []*ranger.Range, realtimeRowCount, modifyCount int64) (totalCount statistics.RowEstimate, err error) {
		...
		if (outOfRangeOnIndex(idx, l) && !(isSingleColIdx && lowIsNull)) || outOfRangeOnIndex(idx, r) {
			...
			if idx.StatsVer == statistics.Version2 {
				...
				if isSingleColRange && c != nil && c.Histogram.NDV > 0 {
					...
					count.AddAll(c.Histogram.OutOfRangeRowCount(sctx, &indexRange.LowVal[0], &indexRange.HighVal[0], realtimeRowCount, modifyCount, histNDV))
				} else {
					// TODO: Extend original datatype out-of-range estimation to multi-column
					histNDV -= int64(idx.TopN.Num())
					count.AddAll(idx.Histogram.OutOfRangeRowCount(sctx, &l, &r, realtimeRowCount, modifyCount, histNDV))
				}
			} else {
				count.AddAll(idx.Histogram.OutOfRangeRowCount(sctx, &l, &r, realtimeRowCount, modifyCount, histNDV))
			}
	...
	totalCount.Est = mathutil.Clamp(totalCount.Est, min, float64(realtimeRowCount))
	totalCount.MinEst = mathutil.Clamp(totalCount.MinEst, min, float64(realtimeRowCount))
	totalCount.MaxEst = mathutil.Clamp(totalCount.MaxEst, min, float64(realtimeRowCount))
	return totalCount, nil
}
