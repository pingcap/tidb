...
func scanJobStatus(rows *sql.Rows) (*JobStatus, error) {
	...
	// Parse times
	createTime, _ := time.Parse(timeLayout, createTimeStr)
	var startTime, endTime, updateTime time.Time
	if startTimeStr.Valid {
		startTime, _ = time.Parse(timeLayout, startTimeStr.String)
	}
	if endTimeStr.Valid {
		endTime, _ = time.Parse(timeLayout, endTimeStr.String)
	}
	if updateTimeStr.Valid {
		updateTime, _ = time.Parse(timeLayout, updateTimeStr.String)
	}
...
func scanGroupStatus(rows *sql.Rows) (*GroupStatus, error) {
	...
	parseTime := func(ns sql.NullString) time.Time {
		...
		t, _ := time.Parse(timeLayout, ns.String)
		return t
	}
	...
	return &GroupStatus{
		...
		Failed:             failed,
		Cancelled:          cancelled,
		FirstJobCreateTime: parseTime(firstCreateTime),
		LastJobUpdateTime:  parseTime(lastUpdateTime),
	}, nil
}
