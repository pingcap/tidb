...
type infoSchemaScanStatsContextKey struct{}

// InfoSchemaScanAllocationStats records exact data-flow counts and allocation
// requests owned by one InfoSchema MetaKV scan. It is not safe for concurrent
// use; one instance belongs to one statement and the scan is sequential.
type InfoSchemaScanAllocationStats struct {
	ProcessTotalAllocBytes          uint64 `json:"process_total_alloc_bytes"`
	ProcessMallocCount              uint64 `json:"process_malloc_count"`
	ProcessFreeCount                uint64 `json:"process_free_count"`
	ProcessHeapAllocDeltaBytes      int64  `json:"process_heap_alloc_delta_bytes"`
	ScannerNextCalls                uint64 `json:"scanner_next_calls"`
	ScanResponseCount               uint64 `json:"scan_response_count"`
	ScanResponseBytes               uint64 `json:"scan_response_bytes"`
	DecodeBufferAllocatedBytes      uint64 `json:"decode_buffer_allocated_bytes"`
	PairSliceAllocatedBytes         uint64 `json:"pair_slice_allocated_bytes"`
	PairObjectAllocatedBytes        uint64 `json:"pair_object_allocated_bytes"`
	PairKeyValueAllocatedBytes      uint64 `json:"pair_key_value_allocated_bytes"`
	KVPairCount                     uint64 `json:"kv_pair_count"`
	KVKeyBytes                      uint64 `json:"kv_key_bytes"`
	KVValueBytes                    uint64 `json:"kv_value_bytes"`
	HashDecodeAllocatedBytes        uint64 `json:"hash_decode_allocated_bytes"`
	TableInfoCount                  uint64 `json:"table_info_count"`
	TableInfoJSONBytes              uint64 `json:"table_info_json_bytes"`
	FastTableInfoDecodeCount        uint64 `json:"fast_table_info_decode_count"`
	FallbackTableInfoDecodeCount    uint64 `json:"fallback_table_info_decode_count"`
	ColumnInfoCount                 uint64 `json:"column_info_count"`
	ColumnStringCloneAllocatedBytes uint64 `json:"column_string_clone_allocated_bytes"`
	PrivilegeCheckCount             uint64 `json:"privilege_check_count"`
	OutputRowCount                  uint64 `json:"output_row_count"`
	OutputDatumPayloadBytes         uint64 `json:"output_datum_payload_bytes"`
	RowBufferAllocatedBytes         uint64 `json:"row_buffer_allocated_bytes"`
}

// ObserveReusableScanStats implements tikvrpc.ReusableScanStatsObserver.
func (s *InfoSchemaScanAllocationStats) ObserveReusableScanStats(delta tikvrpc.ReusableScanStats) {
	if s == nil {
		return
	}
	s.ScannerNextCalls += delta.ScannerNextCalls
	s.ScanResponseCount += delta.ResponseCount
	s.ScanResponseBytes += delta.ResponseBytes
	s.DecodeBufferAllocatedBytes += delta.DecodeBufferAllocatedBytes
	s.PairSliceAllocatedBytes += delta.PairSliceAllocatedBytes
	s.PairObjectAllocatedBytes += delta.PairObjectAllocatedBytes
	s.PairKeyValueAllocatedBytes += delta.PairKeyValueAllocatedBytes
	s.KVPairCount += delta.PairCount
	s.KVKeyBytes += delta.KeyBytes
	s.KVValueBytes += delta.ValueBytes
}

// WithInfoSchemaScanStats attaches statement-owned scan counters to ctx.
func WithInfoSchemaScanStats(ctx context.Context, stats *InfoSchemaScanAllocationStats) context.Context {
	if stats == nil {
		return ctx
	}
	return context.WithValue(ctx, infoSchemaScanStatsContextKey{}, stats)
}

// InfoSchemaScanStatsFromContext returns statement-owned scan counters.
func InfoSchemaScanStatsFromContext(ctx context.Context) *InfoSchemaScanAllocationStats {
	stats, _ := ctx.Value(infoSchemaScanStatsContextKey{}).(*InfoSchemaScanAllocationStats)
	return stats
}
