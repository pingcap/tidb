...
func TestUpdateCopRuntimeStats(t *testing.T) {
	...
	_, scanDetailRecords, scanObservedTasks, scanExpectedTasks := ctx.GetSessionVars().StmtCtx.RuntimeStatsColl.GetCopScanDetailAndCoverage(scanPlanID)
	_, parentDetailRecords, parentObservedTasks, parentExpectedTasks := ctx.GetSessionVars().StmtCtx.RuntimeStatsColl.GetCopScanDetailAndCoverage(parentPlanID)
	require.Zero(t, scanDetailRecords)
	require.Equal(t, int32(1), scanObservedTasks)
	require.Equal(t, int32(1), scanExpectedTasks)
	require.Equal(t, int32(1), parentDetailRecords)
	require.Equal(t, int32(1), parentObservedTasks)
	require.Equal(t, int32(1), parentExpectedTasks)
	...
	_, parentDetailRecords, parentObservedTasks, parentExpectedTasks = ctx.GetSessionVars().StmtCtx.RuntimeStatsColl.GetCopScanDetailAndCoverage(parentPlanID)
	require.Zero(t, scanDetailRecords)
	require.Equal(t, int32(2), scanObservedTasks)
	require.Equal(t, int32(2), scanExpectedTasks)
	require.Equal(t, int32(2), parentDetailRecords)
	require.Equal(t, int32(2), parentObservedTasks)
	require.Equal(t, int32(2), parentExpectedTasks)
	...
	_, parentDetailRecords, parentObservedTasks, parentExpectedTasks = ctx.GetSessionVars().StmtCtx.RuntimeStatsColl.GetCopScanDetailAndCoverage(parentPlanID)
	require.Equal(t, int32(1), parentDetailRecords)
	require.Zero(t, parentObservedTasks)
	require.Equal(t, int32(1), parentExpectedTasks)
	...
	_, parentDetailRecords, _, parentExpectedTasks = ctx.GetSessionVars().StmtCtx.RuntimeStatsColl.GetCopScanDetailAndCoverage(parentPlanID)
	require.Equal(t, int32(2), parentDetailRecords)
