...
func RunTryLockRemote(t *testing.T) {
...
func RunConflictLock(t *testing.T) {
	ctx := context.Background()
	strg, pth := createMockStorage(t)
	lock, err := objstore.TryLockRemote(ctx, strg, "test.lock", "This file is mine!")
	require.NoError(t, err)
...
func RunConcurrentLock(t *testing.T) {
	ctx := context.Background()
	strg, path := createMockStorage(t)
...
func RunUnlockOnCleanUp(t *testing.T) {
