...
func TestSetBackendCredentials(t *testing.T) {
	...
	backend := &backuppb.S3{}
	require.NoError(t, setBackendCredentials(stdctx.Background(), backend, provider, true))
	...
	require.NoError(t, setBackendCredentials(stdctx.Background(), backend, provider, false))
	...
	require.ErrorContains(t, setBackendCredentials(stdctx.Background(), backend, errorProvider, true), "credentials unavailable")
