...
func RunExternalFileWriter(t *testing.T) {
	dir := t.TempDir()
...
func RunCompressReaderWriter(t *testing.T) {
	dir := t.TempDir()
	...
	testFn := func(test *testcase, t *testing.T) {
		...
		file, err := os.Open(filepath.Join(dir, fileName)) // #nosec G304 -- test-controlled path
		require.NoError(t, err)
...
func RunNewCompressReader(t *testing.T) {
