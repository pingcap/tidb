...
func RunExtractPlanWithoutHistoryView(t *testing.T) {
	tempDir := t.TempDir()
...
func RunExtractWithoutStmtSummaryPersistedEnabled(t *testing.T) {
	tempDir := t.TempDir()
	ctx := context.Background()
	storage, err := extstore.NewExtStorage(ctx, "file://"+tempDir, "")
	require.NoError(t, err)
...
func RunExtractHandlePlanTask(t *testing.T) {
	tempDir := t.TempDir()
	ctx := context.Background()
	storage, err := extstore.NewExtStorage(ctx, "file://"+tempDir, "")
	require.NoError(t, err)
