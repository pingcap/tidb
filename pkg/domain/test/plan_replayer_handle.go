...
func RunPlanReplayerHandleCollectTask(t *testing.T) {
...
func RunPlanReplayerHandleDumpTask(t *testing.T) {
	tempDir := t.TempDir()
...
func RunPlanReplayerGC(t *testing.T) {
	ctx := context.Background()
	store, dom := testkit.CreateMockStoreAndDomain(t)
	tk := testkit.NewTestKit(t, store)
...
func RunInsertPlanReplayerStatus(t *testing.T) {
	tempDir := t.TempDir()
	ctx := context.Background()
	storage, err := extstore.NewExtStorage(ctx, "file://"+tempDir, "")
	require.NoError(t, err)
