...
func (do *Domain) initResourceGroupsController(ctx context.Context, pdClient pd.Client, uniqueID uint64) error {
	...
	control, err := rmclient.NewResourceGroupController(ctx, uniqueID, pdClient, nil, constants.NullKeyspaceID, rmclient.WithMaxWaitDuration(runaway.MaxWaitDuration))
	if err != nil {
		return err
	}
	control.Start(ctx)
