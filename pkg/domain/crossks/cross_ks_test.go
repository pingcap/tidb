...
func TestDomainAcquireKSRuntimeHandle(t *testing.T) {
	...
	t.Run("optimize query with target keyspace statistics", func(t *testing.T) {
		...
		// The query uses execution time, so data committed after submission is visible.
		tk.MustExec("insert into query_source(g,v) values (1,40),(3,50)")
		...
		for _, expected := range []map[int64]int64{
			{1: 3, 2: 1, 3: 1},
			{1: 3, 2: 1, 3: 1, 4: 1},
		} {
			func() {
