...
func parseAttrs(data []byte) (map[string]string, string, error) {
	if variable.ConnectAttrsSize.Load() == 0 {
	...
	attrs, warningsText := applyConnAttrsPolicyAndMetrics(decoded, variable.ConnectAttrsSize.Load())
...
func updateConnectAttrsLongestSeen(totalSize int64) {
	...
	for {
		old := variable.ConnectAttrsLongestSeen.Load()
		if totalSize <= old {
			break
		}
		if variable.ConnectAttrsLongestSeen.CompareAndSwap(old, totalSize) {
			break
		}
	}
}
