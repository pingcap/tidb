...
func (cc *clientConn) increaseUserConnectionsCount() error {
	...
	limit := calculateConnectionLimit(globalLimit, uint32(userLimit))

	cc.server.userResLock.Lock()
	defer cc.server.userResLock.Unlock()
	ur, ok := cc.server.userResource[targetUser]
	if !ok {
		cc.server.userResource[targetUser] = &userResourceLimits{connections: 1}
		cc.countedUser = targetUser
		return nil
	}
