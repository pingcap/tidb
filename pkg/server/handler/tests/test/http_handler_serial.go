...
func RunPostSettings(t *testing.T) {
...
func RunAllServerInfo(t *testing.T) {
	ts := createBasicHTTPHandlerTestSuite()
	ts.startServer(t)
	defer ts.stopServer(t)
...
func RunRegionsFromMeta(t *testing.T) {
	ts := createBasicHTTPHandlerTestSuite()
	ts.startServer(t)
	defer ts.stopServer(t)
...
func RunTiFlashReplica(t *testing.T) {
	ts := createBasicHTTPHandlerTestSuite()
	ts.startServer(t)
	ts.prepareData(t)
	defer ts.stopServer(t)
	...
	resp, err = ts.PostStatus("/tiflash/replica-deprecated", "application/json", bytes.NewBufferString(`{"id":184,"region_count":3,"flash_region_count":3}`))
	require.NoError(t, err)
	...
	resp, err = ts.PostStatus("/tiflash/replica-deprecated", "application/json", bytes.NewBufferString(req))
	require.NoError(t, err)
	require.NotNil(t, resp)
	body, err = io.ReadAll(resp.Body)
	require.NoError(t, err)
...
func RunDebugRoutes(t *testing.T) {
	ts := createBasicHTTPHandlerTestSuite()
	ts.startServer(t)
	defer ts.stopServer(t)
...
func RunFailpointHandler(t *testing.T) {
	ts := createBasicHTTPHandlerTestSuite()
...
func RunTestHandler(t *testing.T) {
	ts := createBasicHTTPHandlerTestSuite()
...
func RunServerInfo(t *testing.T) {
	ts := createBasicHTTPHandlerTestSuite()
	ts.startServer(t)
	defer ts.stopServer(t)
...
func RunGetSchemaStorage(t *testing.T) {
	ts := createBasicHTTPHandlerTestSuite()
	ts.startServer(t)
	ts.prepareData(t)
	defer ts.stopServer(t)
...
func RunIngestParam(t *testing.T) {
	ts := createBasicHTTPHandlerTestSuite()
	ts.startServer(t)
	defer ts.stopServer(t)
	...
	for _, tc := range testCases {
		t.Run(tc.url, func(t *testing.T) {
			...
			resp, err = ts.PostStatus(tc.url, "", bytes.NewBufferString(fmt.Sprintf(`{"value": %v}`, tc.modVal)))
			require.NoError(t, err)
			require.NotNil(t, resp)
			defer func() { require.NoError(t, resp.Body.Close()) }()
			require.Equal(t, http.StatusOK, resp.StatusCode)
