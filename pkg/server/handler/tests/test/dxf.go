...
func RunDXFAPI(t *testing.T) {
	...
	t.Run("schedule status api", func(t *testing.T) {
		...
		runAndCheckReqFn(t, http.StatusBadRequest, "This api only support GET method", func() (*http.Response, error) {
			return ts.PostStatus("/dxf/schedule/status", "", bytes.NewBufferString(""))
		})
		// success
	...
	t.Run("schedule api", func(t *testing.T) {
		...
		for _, c := range []struct {
			...
			body := runAndCheckReqFn(t, http.StatusOK, "", func() (*http.Response, error) {
				return ts.PostStatus(fmt.Sprintf("/dxf/schedule?action=%s", c.action), "", bytes.NewBufferString(""))
			})
	...
	t.Run("active task api", func(t *testing.T) {
		runAndCheckReqFn(t, http.StatusBadRequest, "This api only support GET method", func() (*http.Response, error) {
			return ts.PostStatus("/dxf/task/active", "", bytes.NewBufferString(""))
		})
	...
	t.Run("import-into history job info api", func(t *testing.T) {
		...
		t.Run("validation", func(t *testing.T) {
			runAndCheckReqFn(t, http.StatusBadRequest, "This api only support GET method", func() (*http.Response, error) {
				return ts.PostStatus("/dxf/import-into/history/job/ks1/9527", "", bytes.NewBufferString(""))
			})
	...
	t.Run("task max_runtime_slots api", func(t *testing.T) {
		...
		for _, c := range [][2]string{
			...
			runAndCheckReqFn(t, http.StatusBadRequest, errMsg, func() (*http.Response, error) {
				return ts.PostStatus(path, "", bytes.NewBufferString(""))
			})
		}

		body := runAndCheckReqFn(t, http.StatusOK, "", func() (*http.Response, error) {
			return ts.PostStatus(fmt.Sprintf("/dxf/task/%d/max_runtime_slots?value=6&target_step=3", id), "", bytes.NewBufferString(""))
		})
		out := struct {
...
func RunDXFScheduleTuneAPI(t *testing.T) {
	if kerneltype.IsClassic() {
	...
	for _, v := range []float64{0.9, 10.1} {
		runAndCheckReqFn(t, http.StatusBadRequest, "is out of range", func() (*http.Response, error) {
			return ts.PostStatus(fmt.Sprintf("/dxf/schedule/tune?keyspace=SYSTEM&amplify_factor=%f", v), "", bytes.NewBufferString(""))
		})
	}
	// success
	body := runAndCheckReqFn(t, http.StatusOK, "", func() (*http.Response, error) {
		return ts.PostStatus("/dxf/schedule/tune?keyspace=SYSTEM&amplify_factor=2&ttl=10h", "", bytes.NewBufferString(""))
	})
