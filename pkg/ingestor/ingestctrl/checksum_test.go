...
func TestDoChecksum(t *testing.T) {
	...
	checksum, err := manager.Checksum(context.Background(), &importdef.TableInfo{DB: "test", Name: "t"}, nil)
	require.NoError(t, err)
...
func TestIncreaseGCLifeTimeFail(t *testing.T) {
	...
	for range 5 {
		wg.Run(func() {
			_, errChecksum := manager.Checksum(context.Background(), &importdef.TableInfo{DB: "test", Name: "t"}, nil)
...
func TestDoChecksumWithTikv(t *testing.T) {
	...
	for i := 0; i <= maxErrorRetryCount; i++ {
		...
		_, err := checksumExec.Checksum(ctx, &importdef.TableInfo{DB: "test", Name: "t", Core: tableInfo}, nil)
...
func TestTiKVChecksumManagerRejectsPartitionScope(t *testing.T) {
	...
	checksumExec := &TiKVChecksumManager{manager: newGCTTLManager(pdClient, lightningServicePrefix), client: kvClient}
	_, err := checksumExec.Checksum(context.Background(), &importdef.TableInfo{DB: "test", Name: "t"}, []string{"p0"})
	require.Error(t, err)
...
func TestDoChecksumWithErrorAndLongOriginalLifetime(t *testing.T) {
	...
	manager := NewTiDBChecksumExecutor(db)
	_, err = manager.Checksum(context.Background(), &importdef.TableInfo{DB: "test", Name: "t"}, nil)
