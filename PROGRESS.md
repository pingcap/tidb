...
> 当前焦点 / 下一步：①已推送至 0e908e13d7（含进度日志）②等用户的分区对照查询结论后验证 Rust 裁剪 ③expr-builtin：CASE/IF laziness + NULLIF 规则、字符串族扫尾 ④chunk A-1、parser #11（结构性）。注意：另一会话 ca9bc95d09 修了 parser 测试注册——重放我批次时需重跑 parser/lexer 套件（已验证全绿）。
