...
func (s *testPrivilegeSuite) TestAnalyzeTable(c *C) {
	...
	c.Assert(terror.ErrorEqual(err, core.ErrTableaccessDenied), IsTrue)
	c.Assert(err.Error(), Equals, "[DB:planner:1142] INSERT command denied to user 'anobody'@'%' for table 't1'")

	_, err = se.Execute(context.Background(), "select * from t1")
	c.Assert(err.Error(), Equals, "[DB:planner:1142] SELECT command denied to user 'anobody'@'%' for table 't1'")
