...
func TestAlterUserStmt(t *testing.T) {
	...
	tk.MustQuery(`SELECT User_attributes FROM mysql.user WHERE User = "semuser1"`).Check(testkit.Rows("{\"metadata\": {\"resource_group\": \"rg1\"}}"))
...
func TestCreateDropUser(t *testing.T) {
	...
	tk.MustQuery(`SELECT User_attributes FROM mysql.user WHERE User = "usr1"`).Check(testkit.Rows("{\"metadata\": {\"resource_group\": \"default\"}}"))
	tk.MustExec(`DROP USER usr1`)

	tk.MustExec(`CREATE USER usr1 RESOURCE GROUP 'rg1'`)
	tk.MustQuery(`SELECT User_attributes FROM mysql.user WHERE User = "usr1"`).Check(testkit.Rows("{\"metadata\": {\"resource_group\": \"rg1\"}}"))
	tk.MustExec(`DROP USER usr1`)
}
