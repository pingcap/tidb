...
func (p *UserPrivileges) ConnectionVerification(user, host string, authentication, salt []byte, tlsState *tls.ConnectionState) (u string, h string, success bool) {
	...
	if globalPriv != nil {
		if !p.checkSSL(globalPriv, tlsState) {
			logutil.Logger(context.Background()).Error("global priv check ssl fail",
				zap.String("user", user), zap.String("host", host))
			success = false
			return
		}
...
func (p *UserPrivileges) checkSSL(priv *globalPrivRecord, tlsState *tls.ConnectionState) bool {
	if priv.Broken {
		logutil.Logger(context.Background()).Debug("ssl check failure, due to broken global_priv record",
	...
	case SslTypeAny:
		r := tlsState != nil
		if !r {
			logutil.Logger(context.Background()).Debug("ssl check failure, require ssl but not use ssl",
				zap.String("user", priv.User), zap.String("host", priv.Host))
		}
		return r
	case SslTypeX509:
		if tlsState == nil {
			logutil.Logger(context.Background()).Debug("ssl check failure, require x509 but not use ssl",
				zap.String("user", priv.User), zap.String("host", priv.Host))
			return false
		}
		hasCert := false
		...
		if !hasCert {
			logutil.Logger(context.Background()).Debug("ssl check failure, require x509 but no verified cert",
				zap.String("user", priv.User), zap.String("host", priv.Host))
		}
		return hasCert
	case SslTypeSpecified:
		if tlsState == nil {
			logutil.Logger(context.Background()).Debug("ssl check failure, require subject/issuer/cipher but not use ssl",
				zap.String("user", priv.User), zap.String("host", priv.Host))
			return false
		}
		if len(priv.Priv.SSLCipher) > 0 && priv.Priv.SSLCipher != util.TLSCipher2String(tlsState.CipherSuite) {
			logutil.Logger(context.Background()).Debug("ssl check failure for cipher", zap.String("user", priv.User), zap.String("host", priv.Host),
		...
		for _, chain := range tlsState.VerifiedChains {
			...
			if len(priv.Priv.X509Issuer) > 0 {
				...
				if priv.Priv.X509Issuer == given {
					...
					matchIssuer = fail
					logutil.Logger(context.Background()).Debug("ssl check failure for issuer", zap.String("user", priv.User), zap.String("host", priv.Host),
			...
			if len(priv.Priv.X509Subject) > 0 {
				...
				if priv.Priv.X509Subject == given {
					...
					matchSubject = fail
					logutil.Logger(context.Background()).Debug("ssl check failure for subject", zap.String("user", priv.User), zap.String("host", priv.Host),
		...
		if !checkResult && !hasCert {
			logutil.Logger(context.Background()).Debug("ssl check failure, require issuer/subject but no verified cert",
				zap.String("user", priv.User), zap.String("host", priv.Host))
		}
		return checkResult
	default:
