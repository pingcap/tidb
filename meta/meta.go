...
func (m *Meta) reorgJobPhysicalID(id int64) []byte {
	b := make([]byte, 8, 12)
	binary.BigEndian.PutUint64(b, uint64(id))
	b = append(b, "_pid"...)
	return b
...
func (m *Meta) UpdateDDLReorgHandle(job *model.Job, startHandle, endHandle, physicalID int64) error {
	err := m.txn.HSet(mDDLJobReorgKey, m.reorgJobStartHandle(job.ID), []byte(strconv.FormatInt(startHandle, 10)))
	if err != nil {
		return errors.Trace(err)
	}
	...
	err = m.txn.HSet(mDDLJobReorgKey, m.reorgJobPhysicalID(job.ID), []byte(strconv.FormatInt(physicalID, 10)))
	return errors.Trace(err)
}
...
func (m *Meta) RemoveDDLReorgHandle(job *model.Job) error {
	...
	if err = m.txn.HDel(mDDLJobReorgKey, m.reorgJobPhysicalID(job.ID)); err != nil {
...
func (m *Meta) GetDDLReorgHandle(job *model.Job) (startHandle, endHandle, physicalID int64, err error) {
	...
	physicalID, err = m.txn.HGetInt64(mDDLJobReorgKey, m.reorgJobPhysicalID(job.ID))
	if err != nil {
		err = errors.Trace(err)
		return
	}
	// endHandle or physicalID may be 0, because older version TiDB (without table partition) doesn't store them.
	// update them to table's in this case.
	if endHandle == 0 || physicalID == 0 {
		if job.ReorgMeta != nil {
			endHandle = job.ReorgMeta.EndHandle
		} else {
			endHandle = math.MaxInt64
		}
		physicalID = job.TableID
		log.Warnf("new TiDB binary running on old TiDB ddl reorg data, partition %v [%v %v]", physicalID, startHandle, endHandle)
	}
	return
}
