...
func (hg *Histogram) SaveToStorage(ctx context.Context, tableID int64, count int64, isIndex int) error {
	...
	_, err := exec.Execute(ctx.GoCtx(), "begin")
	if err != nil {
	...
	_, err = exec.Execute(ctx.GoCtx(), replaceSQL)
	if err != nil {
		return errors.Trace(err)
	}
	...
	_, err = exec.Execute(ctx.GoCtx(), deleteSQL)
	if err != nil {
		return errors.Trace(err)
	}
	for i, bucket := range hg.Buckets {
		...
		_, err = exec.Execute(ctx.GoCtx(), insertSQL)
		if err != nil {
			return errors.Trace(err)
		}
	}
	_, err = exec.Execute(ctx.GoCtx(), "commit")
	return errors.Trace(err)
}
