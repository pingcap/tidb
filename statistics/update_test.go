...
func (s *testStatsUpdateSuite) TestSingleSessionInsert(c *C) {
	...
	h.Update(is)
	stats1 := h.GetTableStats(tableInfo1, tbl1.GetID())
	c.Assert(stats1.Count, Equals, int64(rowCount1))

	tbl2, err := is.TableByName(model.NewCIStr("test"), model.NewCIStr("t2"))
	c.Assert(err, IsNil)
	tableInfo2 := tbl2.Meta()
	stats2 := h.GetTableStats(tableInfo2, tbl2.GetID())
...
func (s *testStatsUpdateSuite) TestRollback(c *C) {
	...
	stats := h.GetTableStats(tableInfo, tbl.GetID())
...
func (s *testStatsUpdateSuite) TestUpdateErrorRate(c *C) {
	...
	c.Assert(err, IsNil)
	tblInfo := table.Meta()
	tbl := h.GetTableStats(tblInfo, table.GetID())
