...
func (s *testStatsCacheSuite) TestStatsCache(c *C) {
	...
	c.Assert(err, IsNil)
	defer store.Close()
	ctx, err := tidb.CreateSession(store)
	c.Assert(err, IsNil)
	...
	c.Assert(err, IsNil)
	tableInfo := tbl.Meta()
	statsTbl := statistics.GetStatsHandle(ctx).GetTableStats(tableInfo)
...
func (s *testStatsCacheSuite) TestStatsStoreAndLoad(c *C) {
	...
	testKit.MustExec("analyze table t")
	statsTbl1 := statistics.GetStatsHandle(ctx).GetTableStats(tableInfo)

	do.StatsHandle(ctx).Clear()
	do.StatsHandle(ctx).Update(is)
	statsTbl2 := statistics.GetStatsHandle(ctx).GetTableStats(tableInfo)
