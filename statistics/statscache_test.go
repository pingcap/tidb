...
func (s *testStatsCacheSuite) TestDDL(c *C) {
	...
	rs := testKit.MustQuery("select count(*) from mysql.stats_histograms where table_id = ? and hist_id = 3", tableInfo.ID)
	rs.Check(testkit.Rows("1"))
	rs = testKit.MustQuery("select count(*) from mysql.stats_buckets where table_id = ? and hist_id = 3", tableInfo.ID)
	rs.Check(testkit.Rows("1"))
