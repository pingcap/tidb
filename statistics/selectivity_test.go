...
func (s *testStatsSuite) TestPrimaryKeySelectivity(c *C) {
	...
	testKit.MustQuery(`explain select * from t where a > "t"`).Check(testkit.Rows(
		...
		"├─IndexScan_8 3333.33 cop[tikv] table:t, index:a, range:(\"t\",+inf], keep order:false, stats:pseudo",
		"└─TableScan_9 3333.33 cop[tikv] table:t, keep order:false, stats:pseudo"))
	testKit.MustExec("drop table t")
