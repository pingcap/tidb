...
func (coll *HistColl) Selectivity(ctx sessionctx.Context, exprs []expression.Expression, paths []*util.AccessPath) (float64, error) {
	...
	id2Path := make(map[int64]*util.AccessPath)
	for _, path := range paths {
		if path.IsTablePath {
			continue
		}
		id2Path[path.Index.ID] = path
	}
	for id, idxInfo := range coll.Indices {
		...
		if len(idxCols) > 0 {
			...
			maskCovered, ranges, partCover, err := getMaskAndRanges(ctx, remainedExprs, ranger.IndexRangeType, id2Path[idxInfo.Info.ID], lengths, idxCols...)
			if err != nil {
				return 0, errors.Trace(err)
			}
...
func getMaskAndRanges(ctx sessionctx.Context, exprs []expression.Expression, rangeType ranger.RangeType, path *util.AccessPath,
