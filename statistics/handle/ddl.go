...
func (h *Handle) insertColStats2KV(physicalID int64, colInfos []*model.ColumnInfo) (err error) {
	...
	if h.mu.ctx.GetSessionVars().StmtCtx.AffectedRows() > 0 {
		...
		for _, colInfo := range colInfos {
			...
			value, err = value.ConvertTo(h.mu.ctx.GetSessionVars().StmtCtx, &colInfo.FieldTypeBuilder)
			if err != nil {
				return
			}
			if value.IsNull() {
				...
				value, err = value.ConvertTo(h.mu.ctx.GetSessionVars().StmtCtx, types.NewFieldTypeBuilder(mysql.TypeBlob))
				if err != nil {
					return
				}
