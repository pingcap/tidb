...
func (h *Handle) initStatsMeta4Chunk(is infoschema.InfoSchema, cache *cache.StatsCache, iter *chunk.Iterator4Chunk) {
	for row := iter.Begin(); row != iter.End(); row = iter.Next() {
		...
		cache.Put(physicalID, tbl, false) // put this table again since it is updated
	}
}
...
func (h *Handle) initStatsHistograms4ChunkLite(is infoschema.InfoSchema, cache *cache.StatsCache, iter *chunk.Iterator4Chunk) {
	for row := iter.Begin(); row != iter.End(); row = iter.Next() {
		...
		table, ok := cache.Get(tblID, false)
		if !ok {
			continue
		}
		...
		cache.Put(tblID, table, false) // put this table again since it is updated
	}
}
...
func (*Handle) initStatsTopN4Chunk(cache *cache.StatsCache, iter *chunk.Iterator4Chunk) {
	...
	for row := iter.Begin(); row != iter.End(); row = iter.Next() {
		table, ok := cache.Get(row.GetInt64(0), false)
		if !ok {
			continue
		}
		...
		cache.Put(table.PhysicalID, table, false) // put this table again since it is updated
	}
...
func (*Handle) initStatsBuckets4Chunk(cache *cache.StatsCache, iter *chunk.Iterator4Chunk) {
	for row := iter.Begin(); row != iter.End(); row = iter.Next() {
		...
		table, ok := cache.Get(tableID, false)
		if !ok {
			continue
		}
		...
		cache.Put(tableID, table, false) // put this table again since it is updated
	}
}
