...
func (h *Handle) initStatsHistograms4Chunk(is infoschema.InfoSchema, cache *statsCache, iter *chunk.Iterator4Chunk) {
	for row := iter.Begin(); row != iter.End(); row = iter.Next() {
		...
		lastAnalyzePos := row.GetDatum(11, types.NewFieldTypeBuilder(mysql.TypeBlob))
		...
		if row.GetInt64(1) > 0 {
			...
			hist := statistics.NewHistogram(id, ndv, nullCount, version, types.NewFieldTypeBuilder(mysql.TypeBlob), chunk.InitialCapacity, 0)
