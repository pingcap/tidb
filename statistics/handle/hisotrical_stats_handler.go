...
func (h *Handle) recordHistoricalStatsMeta(tableID int64, version uint64, source string) error {
	h.mu.Lock()
	defer h.mu.Unlock()
	return recordHistoricalStatsMeta(h.mu.ctx, tableID, version, source)
}
