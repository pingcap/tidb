...
func newStatsCache(memQuota int64) statsCache {
	if memQuota > 0 {
		return statsCache{
			kvCache: newInternalLRUCache(memQuota),
		}
	}
	return statsCache{
		kvCache: &mapCache{
...
func (m *mapCache) CalculateCost() {
	...
	metrics.StatsCacheMemUsage.WithLabelValues("cache").Set(float64(m.memUsage))
}
