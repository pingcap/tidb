...
func (s *testStatisticsSuite) TestBuild(c *C) {
	...
	count, err = col.lessRowCount(sc, types.NewIntDatum(2000), false)
	c.Check(err, IsNil)
	c.Check(int(count), Equals, 19964)
	count, err = col.greaterRowCount(sc, types.NewIntDatum(2000), false)
	c.Check(err, IsNil)
	c.Check(int(count), Equals, 80034)
	count, err = col.lessRowCount(sc, types.NewIntDatum(200000000), false)
	c.Check(err, IsNil)
	c.Check(int(count), Equals, 100000)
	count, err = col.greaterRowCount(sc, types.NewIntDatum(200000000), false)
	c.Check(err, IsNil)
	...
	c.Check(err, IsNil)
	c.Check(int(count), Equals, 1)
	count, err = col.lessRowCount(sc, encodeKey(types.NewIntDatum(20000)), false)
	c.Check(err, IsNil)
	...
	c.Check(err, IsNil)
	c.Check(int(count), Equals, 1)
	count, err = col.lessRowCount(sc, types.NewIntDatum(20000), false)
	c.Check(err, IsNil)
