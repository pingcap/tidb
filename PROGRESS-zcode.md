...
- 新面批次: tidb-br 三个声明完整包审计 (br/pkg/streamhelper/spans, br/pkg/rtree, br/pkg/restore/utils @ a85e0fd5df)。零 behavior 分歧; 测试parity声明逐包验证成立 (spans 4/4, rtree 8/8+fuzz/bench 按政策, restore/utils 16/16, 案例表逐字节保留)。
  生产面匹配: spans join/Valued 排序/mergeWithOverlap 裁剪与再熔合 (含 Go 索引删除晚于 start-key 变异的 quirk)/Collapse 运算符优先级; rtree Intersect 端键表/Find/getOverlaps 上升停/GetIncompleteRange 四条款尾判/MergedRanges 索引算术/NeedsMerge 矩阵/API-V2 keyspace 裁剪/逐字错误文本; restore/utils RewriteRules 生命周期/SetTimeRangeFilter (含 Go "Wrtie" typo)/ValidateFileRewriteRule 十六进制消息/空旧前缀前插/GetRewriteTableID 0 回退/MergeAndRewriteFileRanges 分组与 CF 计数。
  cosmetics (收据记录): 错误文本 Debug vs Display 形状、两处 log.Warn 丢弃、summary 收集器、BTreeMap 确定序。纯文档批次按新规程不推送, 本地提交。
- 规程更新: journal (PROGRESS-zcode.md) 只留本地, 不推送; 无实质代码改动的提交不推送。批量推送仅限含代码修改的 commit。
- 下轮恢复点: (1) 只读收敛核查或新面; (2) F2/F3-seam live 阻塞; (3) DST 排队; (4) dbsid 分叉待协调。
