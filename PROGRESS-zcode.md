...
- 推送恢复: 兄弟 revert (c5776e1e7bf) 波及并丢失了 ddl-resourcegroup 批次 (代码+收据), 已重新 fast-forward 推送恢复 (028d8c32d84)。规程补充: 共享分支被 revert/rewrite 后, 已推送批次可能丢失 —— 每轮 ls-remote 对账时同时抽查最近批次的标志性内容是否仍在服务端。
- 下轮恢复点: (1) 只读收敛核查或新面; (2) unistore 测试目标随兄弟 distsql 修复回补; (3) pd-client 外部 pin 待决; (4) F2/F3-seam live 阻塞; (5) dbsid 分叉待协调。
