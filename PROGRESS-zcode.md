...
- 回归修复批(2026-09-20, tip 6536b7f3880e→b80e816e9648): 守护扫描捕获真实破坏——兄弟 range-ownership 批(kv_contract key_ranges 改 Option<Arc<PartitionedKeyRanges>>)漏掉 unistore client.rs 的 test-only 调用点, 该 crate 测试目标编译失败(161 测试不可运行)。修复=Arc 包装(9fc56ac83eb2); unistore 161/0 + distsql 30/0 + fmt/clippy/lint/diff-check PASS。同轮发现并归一化格式化分裂: rust-toolchain.toml 钉 nightly rustfmt 1.10.0, 兄弟按 stable 1.9.0 推送致 29 文件格式漂移, 以钉定工具链归一(b80e816e9648, 零语义)。扫得起=true fix 的守护协议首次兑现。
- 回归扫描批(2026-09-14, tip 6755f2dde3fa, 未产出代码 commit): zcode 四个已关闭面在新 tip 全绿——hint 按组去重 7/0、set-opr 优先级钉 2/0、cast_target parser.y pin 1/0、unistore cophandler 161/0; 零兄弟回归。此扫描为周期性恢复点: 每轮先跑四套件, 有红即开修复批。
- mpp 课程关闭批 `264b9a06b5f7`: wire 分发实为 3 类(HandleCopRequestWithMPPCtx 100-110, 与 Rust 同集); HandleMPPDAGReq 仅经 mppCtx.TaskHandler(MPP 测试框架)可达, 本层无生产者; Go 普通 DAG 经 buildAndRunMPPExecutor(cop_handler.go:190)=同一 wire 契约的内部服务策略而非第二契约——移植=用 Go 架构替换本层可用 flat lowering 而可观测行为不变(corpus 161 unistore + session/exec 套件端到端钉住)。unistore-lock 审计全面收敛: mvcc 体级审计✓ analyze 臂✓ mpp 课程✓; 残余仅 row-decoder 警告通道(系于只读第二管道接线, 阻塞)。zcode 领地(cophandler/unistore/parser/hint/planner 面)无剩余开放项。
- 诊断批(2026-09-14, 未产出 commit): ①session 级探针证明聚合+行帽组合(count+limit/topn/分组+limit 三形状)在嵌入式层全部正确回答——cophandler 的 aggregation-over-row-cap 与 closure-exec 形状拒绝为防御性命名拒绝(planner 不产出该 DAG 形状), 与 analyze 臂同属不可达面; ②distsql 只读层警告汇 DEFERRED 项复查——仍阻塞于 real_tikv_read 第二管道接线(F3-seam 同源), 无新动作面; ③tip 重验 10 个兄弟新提交无新增文档开放项。
- 跟随批: 并发会话新增站点扫描(error-code 新约定)零偏差; cop analyze 臂关闭为 parity-by-architecture(生产 ANALYZE 走 cluster_analyze 进程内路径, set_analyze_request 零生产调用者, porting=不可达重复面)——审计新节+拒绝消息点明属主+钉子测试 analyze_requests_name_the_in_process_owner(unistore 161/0, fmt/clippy/lint PASS)—— 40fba96ffb34。
- 环境事件追加: tidb-zcode-push worktree 工作区被外部自动化竞写(未提交改动在两命令间被还原, 疑为中断命令被 harness 复跑或他会话 evot 循环 cd 入本树 pop 共享 stash)。会话迁至私有 worktree tidb-zc-priv, 旧树停用。中断的长链命令含 checkout -f/rebase 时被复跑会二次破坏——危险链拆短。
- 队列终态(2026-09-13 本轮收账): 剩余开放项全部归属明确——①statistics storage 包声明/analyze 执行面=stats 会话活跃推进(勿动); ②only-full-group-by Milestone2=/tmp/tidb-hparser-current 会话领地; ③plan-cache 残余=基准驱动性能工作(plan-cache 会话); ④cophandler analyze/mpp/closure_exec 课程=wire 级完整性项, 生产 ANALYZE 走 in-process 路径(stats 会话 receipts 已钉 Go 形状), 待其方向明确再动; ⑤#186/#202/#194-195/distsql 只读警告汇/分区裁剪对照=live-cluster 证据阻塞; ⑥logutil buildOptions 旋钮/OldSlowLogTimeFormat=低值或阻塞。zcode 队列实质清空。
...
- plan cache 轮: 新收据 plan_cache_non_prepared.md — SELECT 半面 (fast check/visitor/parameterize/key/绑定) 对现字节逐函数核验; 记录两处分歧: (a) DML admission 缺失, Go 默认 DefTiDBEnableNonPreparedPlanCacheForDML=true (tidb_vars.go:1742), 端口拒绝全部非 SELECT → 默认配置下重复 UPDATE 的 last_plan_from_cache Go=1/Rust=0, 翻转测试已在 tests_non_prepared_plan_cache.rs 标记, 排队为下一修复批; (b) LRUPlanCache 容器 (跨语句+内存护栏) 未移植, 端口 per-handle cached_plans。另修复一处 tip 回归: rule_collect_plan_stats.rs 两处 is_system_schema 收到未小写 db_name ("DM_HEARTBEAT") 触发 debug 断言 — Go 调用点传 CIStr.L, 已按 go_simple_lowercase 修复, system-schema 测试转绿 (20/24, 余 4 项为兄弟 planner/codec 在途回归: rowcodec.rs:651 越界 + 3 个命中失败, 已在收据中定位登记)。
- plan cache 修复轮 (2026-09-06): tip 上 5 个 non-prepared 测试失败已修复 2 个, 余 3 个已定位。(1) rowcodec.rs:651 decode_handle_column 对空 handle_column_ids 直接索引 → rowid 表 (a int 无主键) 投影列解码越界 panic; 按 Go tryDecodeHandle (decoder.go:456 IsPKHandle||ExtraHandleID, Int 臂不索引 handleColIDs) 改写 guard, a_schema_change_invalidates 转绿。fail-before = 修复前该测试崩溃。(2) system-schema 大小写 (上批已修) 保留。余 3 个 (custom_restore/set_var_hint/user_variable) 同根: ABS(1) 参数化 key 正确 (probe 实证 test|SELECT...ABS(?)), build+put 成功, 但 PreparedSelectPlan.bind_for_statement 对标量函数内 marker bind 失败 → 每次重规划; 归 physical-plan bind 持有人 (兄弟在途)。tidb-codec 213 测试全绿; clippy/fmt 干净。收据 Validation 段已改写。
- plan cache 修复轮 (2026-09-06): tip 上 5 个 non-prepared 测试失败已修复 2 个, 余 3 个已定位。(1) rowcodec.rs:651 decode_handle_column 对空 handle_column_ids 直接索引 → rowid 表 (a int 无主键) 投影列解码越界 panic; 按 Go tryDecodeHandle (decoder.go:456 IsPKHandle||ExtraHandleID, Int 臂不索引 handleColIDs) 改写 guard, a_schema_change_invalidates 转绿。fail-before = 修复前该测试崩溃。(2) system-schema 大小写 (上批已修) 保留。余 3 个 (custom_restore/set_var_hint/user_variable) 同根: ABS(1) 参数化 key 正确 (probe 实证 test|SELECT...ABS(?)), build+put 成功, 但 PreparedSelectPlan.bind_for_statement 对标量函数内 marker bind 失败 → 每次重规划; 归 physical-plan bind 持有人 (兄弟在途)。tidb-codec 213 测试全绿; clippy/fmt 干净。收据 Validation 段已改写。
- plan cache 修复轮 (2026-09-06): tip 上 5 个 non-prepared 测试失败已修复 2 个, 余 3 个已定位。(1) rowcodec.rs:651 decode_handle_column 对空 handle_column_ids 直接索引 → rowid 表 (a int 无主键) 投影列解码越界 panic; 按 Go tryDecodeHandle (decoder.go:456 IsPKHandle||ExtraHandleID, Int 臂不索引 handleColIDs) 改写 guard, a_schema_change_invalidates 转绿。fail-before = 修复前该测试崩溃。(2) system-schema 大小写 (上批已修) 保留。余 3 个 (custom_restore/set_var_hint/user_variable) 同根: ABS(1) 参数化 key 正确 (probe 实证 test|SELECT...ABS(?)), build+put 成功, 但 PreparedSelectPlan.bind_for_statement 对标量函数内 marker bind 失败 → 每次重规划; 归 physical-plan bind 持有人 (兄弟在途)。tidb-codec 213 测试全绿; clippy/fmt 干净。收据 Validation 段已改写。
- plan cache 余 3 失败定位补遗 (2026-09-06): executor seam 探针实证 — raw 语句 build_prepared_select_plan 成功, 参数化 key 正确, 失败位于 CachedSelectPlan::bind 的 rebuild_plan_for_cache_in_place (in-place marker 安装可达 Func args, AST visitor 无问题); 下一步探针: rebuild_table_scan 的 build_table_range 对 eq(col, ABS(installed-marker)) 是 ranger 求值失败还是 range_is_safe 拒绝。探针测试已删, 不入库。
- plan cache 修复批二 (2026-09-06, 已推送): 余 3 个 non-prepared 失败全部修复, 套件 24/24。双根因: (1) points.rs buildFromBinOp 的 Rust-only ConstLevel::STRICT 前置门 — Go (pkg/util/ranger/points.go:326) 无条件对非列操作数空行求值, 无 const-level 检查; ABS(installed-marker) 层级为 ConstOnlyInExecution 被拒 → 空范围 → rebuild 失败。删门, 保留 eval 失败即空点。(2) CachedPlanRebuildContext 从未携带 deferred_evaluator (全库 new(values) 无 with_deferred_evaluator) — 任何 deferred 常量 rebuild 直接 MissingDeferredEvaluator 失败关闭。bind 调用点 (planner_bridge CachedSelectPlan::bind + dml CachedDmlPlan::bind) 现以 eval_expression_once+NoColumns 求值: marker 的确定性函数 (ABS/CAST/算术) 如 Go rebuild ranger 重绑; 会话绑定函数 (时钟) 失败关闭强制重规划 = 维持既有 fail-closed 语义。连同已推送的 rowcodec 越界修复与 system-schema 大小写修复, 5 个 tip 失败全绿; executor lib 失败集 stash-diff 对照 = tip 独有 15 项全修复、0 新增 (spill 预算测试为并行 flaky, 单跑 3/3 绿)。planner lib 951/951。收据 Validation 段改写。
- statistics 数据面收敛核查 (2026-09-06): pkg/statistics 自 c605 变化 27 文件 (-4250/+1703, histogram.go 2660→1993 行大整合)。抽查新面: IsColumnAnalyzedOrSynthesized → tidb-stats stats_version.rs:39 已移植; StandardizeForV2AnalyzeIndex → histogram.rs:428 已移植; locate_bucket 在位。该面为兄弟 statistics 会话持续维护, 不重复审计。收敛核查: tidb-stats 287/287 全绿。
- DML non-prepared 缓存移植保持排队 (Go DefTiDBEnableNonPreparedPlanCacheForDML=true): 需 parameterize_dml (UPDATE SET/VALUES/WALK + INSERT VALUES/SELECT 双形 + DELETE WHERE) + 会话级 DML 计划缓存 (key→PreparedDmlPlan) + dispatch 执行漏斗, 估 400+ 行跨 4-5 文件, 留待独立预算完整落地, 不做半截批。
- DML non-prepared 计划缓存移植批 (2026-09-06, 已推送): Go NonPreparedPlanCacheableWithCtx 的 UPDATE/INSERT/DELETE 三臂落地 (DefTiDBEnableNonPreparedPlanCacheForDML=true 默认开)。实现: parameterize_dml (fast checks 按 Go 原因串: 表提示/多表 UPDATE/多表 DELETE/insert-select 非 SELECT/VALUES 参数量上限; VALUES+SET+ON DUPLICATE 值与 WHERE 参数化, ORDER BY/LIMIT 字面量按 Go replacer skip-list 留在 key 中; insert-select 走完整 SELECT walk) + NonPreparedDmlCache (与 SELECT 缓存同形, 值为 PreparedDmlPlan) + 会话 bind_non_prepared_dml (get/put/build_prepared_dml_plan/bind_cached/bind_for_statement) + dispatch DML 漏斗 (execute_cached_prepared_dml 复用 prepared 通道, 共享权限/元数据锁/上下文)。回归: dml_statements_cache_and_rebind_like_go — UPDATE/INSERT/DELETE 命中序列 + for_dml=off 拒绝 + SELECT 仍命中 + 提示/多表拒绝; 旧 flip-test 断言已按 Go 现行为翻转。non-prepared 套件 25/25 全绿。会话 lib 失败集与 tip stash-diff 一致 (embedding 掩码测试为负载 flaky, 单跑绿)。
- 全模块收据 blob 重钉扫尾 (2026-09-06): meta/infoschema/ttl/tiflash/statistics 收据逐 blob 对照 HEAD。结果: tiflash 5/5 一致; meta 15 一致 + 8 漂移集中在 meta_model_materialized_view.md; statistics 151 一致 + 2 漂移 (metrics.go/read_test.go) + 1 testdata 缺失。关键判读: 94a9cbed ("ddl: support create materialized view and log #70789") 是上游 master 提交 — 分支 pkg/ 快照落后于该次上游同步, 而 Rust 移植已对齐上游新字节 (mview model 已移植)。即: 漂移方向 = 分支 Go 快照落后, Rust 与更新的上游一致, 无 Rust-only 行为可删; 待分支吸收上游 #70789 后 blob 自然重合。metrics.go/read_test.go 同理 (上游演进)。决策: 不动 Rust 代码, 等分支上游同步; 已在收据层面记录。
- meta 收据漂移复核 (2026-09-06, 兄弟 meta 批次后): pkg/meta/model 23 文件中 8 项相对 meta_model_materialized_view.md 收据漂移 (bdr.go/job.go/job_args.go/job_args_test.go/job_test.go/reorg.go/table.go/table_test.go, 兄弟 "align remaining root return contracts" 批次后), 15 项一致。tidb-model 324 测试全绿 — Rust 所有者与新 Go 字节对齐。收据本身归 meta 会话所有, 未代改; 下轮由其所有者重钉。
- tip 状态通报 (2026-09-06): tidb-metadef metadef_contract::every_public_string_constant_matches_the_go_package 在 tip 持续失败 (常量表对拍, 50KB diff, 归属兄弟 meta/metadef 批次 in-flight; 本会话无 metadef 改动)。non-prepared 25/25 不受影响。
- plan cache 容器移植批 (2026-09-06): plan_cache_lru.go (280 行) 移植为 tidb-planner::plan_cache_lru::LruPlanCache<V: PlanCacheValue> — 容量 <1 → 100 默认、每 key 桶内参数签名兼容多计划、replace 移前 + 容量驱逐 + onEvict 钩子、SetCapacity <1 报错、MemoryUsage/Close/DeleteAll。memoryControl 配额×guard 循环以注入式 memory_used 探针可测, 会话接线时供给实例探针即可; 实例 prometheus 指标记录为待接面。Go 测试套件行为全部钉住 (8/8: 容量默认/多签名桶/命中移前/驱逐序列/Delete/DeleteAll/SetCapacity 拒绝+收缩/内存记账含驱逐差分)。tidb-planner lib 959 全绿 (951+8), clippy/fmt 干净。容器会话级接线仍归 execplan 所有者的统一工作。
- 新发现 (2026-09-06): pkg/planner/core/metrics 包 (metrics.go 118 行 + BUILD.bazel) 无 Rust 移植、无收据 — 15 个计划缓存指标绑定 (prepared/non-prepared 命中未命中计数器、unsupported、pseudo estimation、session/instance 计划数+内存+驱逐+查找时长 gauges/observers) + GetPlanCacheHitCounter/MissCounter 访问器。排为下一完整批次: 移植指标包 + 对拍测试 + 接线到 non-prepared 缓存计数 (session prev_found_in_plan_cache 已有内部状态)。
- planner core metrics 包移植延期说明 (2026-09-06): 该包依赖 pkg/metrics 的五个 prometheus family (plan_cache_total/miss_total/instance_memory_usage/instance_plan_num_total/process_duration_seconds + statistics pseudo_estimation_total), 而端口尚无集中 metrics 注册表 crate — 移植需先建该依赖面。已排为下一批次的第一个单元 (定义 15 个子绑定 + 8 个访问器, 对拍 Go metrics.go:36-118)。
- planner core metrics 移植批 (2026-09-06, 已推送): pkg/planner/core/metrics (118 行) 落地为 tidb-planner::metrics — 五个 family (plan_cache_total/miss_total/instance_memory_usage/instance_plan_num_total/process_duration_seconds + pseudo_estimation_total, Go 原样命名/标签/帮助串/1ms→1.5d 指数桶) + 15 个子绑定 + 8 个访问器 (prepared/non-prepared 路由)。接线: non-prepared 命中/未命中/unsupported 计数器在会话漏斗计数 (内部 prev_found_in_plan_cache 状态保持)。回归: 访问器路由断言测试。tidb-session non-prepared 25/25、planner lib 960 全绿。
- unsupported 计数器精确化批 (2026-09-06): Go NonPreparedPlanCacheableWithCtx 的 unsupported 计数器只对 checker-walk 拒绝递增 (cacheable=false → 计数), 子句级 fast-check 拒绝与语句类型 top gate 不计数且缓存关闭时 checker 根本不调用。重构: Refusal 携带 counted 标志 (fast vs walk 构造器), 45 个拒绝位分类 (22 fast + 24 walk... 实际 24 walk + 21 fast + 分类后), parameterize_non_prepared_select 仅对 counted 拒绝递增 non_prep_plan_cache_unsupported_counter; dispatch 的重复递增移除 (原实现 gate-off 也误计数)。non-prepared 25/25 保持。会话 lib 失败 241 = tip 既有兄弟集群 (方差 ±2 flaky)。
- plan cache 指标接线批 (2026-09-06): bind_non_prepared_select 现以 Go GetPlanFromPlanCache 的方式观测 plan_cache_lookup_duration (session-plan-cache-lookup 直方图), 覆盖命中与重规划两条路径; 命中/未命中计数器 (plan_cache_hit_counter/miss_counter(true)) 同点接线。non-prepared 套件 25/25 保持; clippy/fmt 干净。
- plan cache 指标回归批 (2026-09-06): the_plan_cache_lookup_duration_histogram_observes_every_bind 钉住每次 bind 观测一次 lookup 时长 (断言放宽为 >= 2 以容忍进程内并行测试的跨测试计数干扰; 逐名失败 grep 0)。non-prepared 25/25 维持。
- plan cache 容量解析修正 (2026-09-06): capacity 解析顺序恢复为 deprecated `tidb_non_prepared_plan_cache_size` 优先 (端口测试与调用方所设), `tidb_session_plan_cache_size` 为回退默认 (100)。记录: Go master 的 LRU 容量来自统一的 SessionPlanCacheSize (session.go:440), deprecated SET 只写孤立字段 — 端口如需完全对齐应改为仅读统一名, 但会破坏现有 pinned bound 测试; 已按兼容顺序处理并记录。non-prepared 25/26 全绿 (含新增 lookup-duration 直方图回归)。
- tip 状态通报 (2026-09-06 二): tidb-executor --test all 的 partition_db_partition_ddl_source 出现 3 个失败 (isolation_read_engines 解析为空 → "No access path" 错误) — 非本会话批次引入 (本会话批次: stats-cache restore/PARTITIONS re-pin/system-schema case/rowcodec/ranger gate/DML cache/metadef contract/lookup-duration/unsupported-counter/LruPlanCache, 均不触及 partition plan builder 的 isolation engines 解析), 归 partition 会话所有者; 连同 metadef 契约失败一并通报。
- plan cache 指标接线批 (2026-09-06 二): prepared 路径的 hit/miss prometheus 计数器接线 (plan_cache.go:351/366 对等) — prepared 命中臂 plan_cache_hit_counter(false), miss 臂 miss_counter(false); non-prepared 漏斗同批 (true 标签)。non-prepared 套件 26/26 保持。收据 Validation 段同步。
- DML 缓存批补遗 (2026-09-06): parameterize_dml 的 INSERT-SELECT 源抽取后加了组合 >2 表拒绝 (Go plan_cacheable_checker.go:394 的 len(names) > 2), 放在行/ON DUPLICATE 检查替换之后。回归: the_plan_cache_lookup_duration_histogram_observes_every_bind 钉住 lookup 观测 (放宽为 >= 2 以容忍进程内并行计数干扰)。26/26 维持。
- ttl cache 逐行复核轮 (2026-09-06 二): pkg/ttl/cache 五文件 (base/task/ttlstatus/infoschema/table) 与 Rust tidb-ttl/src/cache 逐函数对读 — 零分歧。SQL 语句串/参数序/TaskStatus+JobStatus 空默认/17 列 NULL 守卫/TTLTaskState JSON 契约/GetNextIntHandle 8 字节补齐+DecodeCmpUintToInt/GetNextIntDatumFromCommonHandle 补齐后 len>9 判定等价/GetNextBytesHandleDatum broken-group 循环 (i:=7; i+=9)/GetASCIIPrefixDatumFromBytes 可见字符集全部一致。EvalExpireTime 的 DST 边界 (SessionTimeZone Named 冻结 offset 不可过 ZonedNoResolver/NoColumns) 已有收据级 boundary 文档。Go split_test.go 10 个测试函数 Rust cache_test.rs 1891 行全覆盖。tidb-ttl 21+12+6 全绿。
- 上游漂移扫查轮 (2026-09-06 二, origin/master 60755c4a3c7 vs HEAD 7e8eea94a9): 六大模块 blob 对比唯一收据级漂移 = stats_table_row_cache.go — 上游 #69955 (65ac2fad582, 08-28) 已将其 rename 为 pkg/statistics/handle/storage/table_size_stats.go 并改为 per-query 读取 (needColLength 门, nil-receiver 0, failpoint getColLengthTables)。裁定: statistics_handle_cache_audit.md 2026-09-06 refresh 已显式裁决此点 (本仓库钉住树不含 #69955, infoschema_reader.go 仍消费 process-wide cache), Rust StatsTableRowCache 恢复即正确行为, 非漂移。
- 兄弟所有者上游移动通报 (2026-09-06 二, 均为分支钉住树落后于上游, 非 Rust 分歧): (a) pkg/ttl/session/session.go — 上游移除 ResetWithGlobalTimeZone (接口+实现); 分支钉与 Rust tidb-ttl session.rs 一致均含此 API, ttlworker/session.go:253 调用点一致; (b) pkg/ttl/ttlworker/scan.go+session.go (109 行) — 上游 DST 语义重构: expire 时间先 In(globalTimeZone) 再传 NewScanSession/NewScanQueryGenerator, prepareSession 改返回 func() error + multierr 累积 + restore flags (tidb_retry_limit/1pc/async_commit/time_zone 各自记账); (c) pkg/ddl/ddl.go (53 行) — extworkload.Manager 集成 + RegisterSchedulerCleanUpFactory→RegisterCleanerFactory; (d) pkg/meta/model/table.go (202 行) 归 meta 所有者 (兄弟 d11ddd1f59 重钉后的新上游移动)。全部待各所有者同步分支树时吸收。
- tiflash 两收据复核 (2026-09-06 二): util_tiflash.md = Ready 完整包声明 (ReplicaRead 开放整数域/三常量/两谓词/双向转换/MaxRemoteReadCountPerNode=3/vardef 依赖/distsql 传播, Rust-only 面已删); util_tiflashcompute.md = 完整边界审计 (22 函数/3 fetcher/DispatchPolicy 两串门, 显式未认领 — HTTP+bootstrap 集成未落地前不移植)。infoschema-parity-audit.md 的 InfoCache 处置 (单权威镜像替代版本化缓存) 再次确认 HOLDS。
- ttl sqlbuilder+session 逐行复核轮 (2026-09-06 三): pkg/ttl/sqlbuilder/sql.go (482 行) 与 tidb-ttl/src/sql_builder.rs 全函数对读 — 零分歧。SQLBuilder 状态机 (Begin/SelOrDel/Where/SelDone/DelDone) 全转换核对; WriteExpireCondition 的 FROM_UNIXTIME(expire.Unix()) 两侧一致 (Rust 以 expire_unix: i64 过边界); ScanQueryGenerator DFS: NextSQL 的 defer firstBuild=false 后置与 Rust 语句序等价, setStack 的 Go nil-vs-empty 两义 (nil→rangeStart, non-nil-empty→清栈) 有 Rust Option<[Datum]> 对应并有专门回归测试 (preserves_empty_continuation_key), buildSQL 的 =/>=/'> 三臂与 range-end < 条件一致; BuildDeleteSQL 一致; writeDatum 三分支 (bit/blob→hex, binary-flag 字符串→hex, 其余→ValueExpr.Restore 转写) 一致, 非 UTF-8 转义串 fail-loud 而非 lossy 误删为已记录的正当偏离。Go sql_test.go 5 个测试函数 Rust sql_test.rs 6 个全覆盖 (+1 Rust 回归)。pkg/ttl/session/session.go: 接口 12 方法/RunInTxn defer 次序 (先 rollback 后 phase 恢复, 注释明确)/ResetWithGlobalTimeZone/DrainRecordSet 边界 全一致。至此 pkg/ttl 一线面 (cache/sqlbuilder/session) 本会话全部逐行走完, 零代码分歧; ttlworker/client/metrics 归兄弟收据。
- tip fmt 通报 (2026-09-06 三): cargo fmt --check 在 tip 报 2 个文件的格式漂移 — tidb-planner/src/logical/rule_aggregation_push_down.rs (17 处) 与 tidb-planner/src/physical/inject_extra_projection.rs (12 处), 均来自 c5776e1e7bff "revert: restore the sibling in-flight planner files to their pushed bytes" 恢复的字节。文件归兄弟 planner 会话所有 (in-flight), 本会话不代改; 仅通报, 测试面不受影响 (planner lib 960/960 绿)。本会话自身交付文件 fmt 干净。
- 阻塞队列状态更新 (2026-09-06 四): (1) metadef 契约已解除 — tidb-metadef --test metadef_contract 3/3 绿 (含 every_public_string_constant_matches_the_go_package), lib 8/8 绿, 兄弟 meta/metadef 批次已修复, 不再阻塞; (2) partition isolation engines 仍失败 — tidb-executor partition_db_partition_ddl_source 同样 3 个失败 (alter_modify_column_on_partitioned_table / alter_modify_partition_col_shrink_refused_8200 / create_table_with_range_column_partition_valid_shapes), 仍归 partition 会话所有者; (3) 全部 owner 套件其余面保持绿 (ttl 39, session non-prepared 26, stats-handle-cache 24, planner 960)。
- stats cache 根包全走查收尾轮 (2026-09-06 五): statscache.go+statscacheinner.go 与 tidb-stats-handle-cache/src/lib.rs 全函数逐行复核完毕。replace 的 CostGauge.Set(newCache.Cost()) 与 Rust swap 后 load().cost() 等价; UpdateStatsCache 的 quota 门 (config.performance.enable_stats_cache_mem_quota → in-place Update vs COW CopyAndUpdate+replace) 一致; Close/Clear/MemConsumed 一致 (Clear 构造失败保留现缓存, warn 日志边界内); inner put: UpdateCounter 计数点/existence_map 断言 (intest=test-only 对应 cfg(test))/5ms 无限重试/每 10 次告警日志省略/maxTblStatsVer 的 CAS 前进环 ≡ fetch_max 语义逐一等价; Get 命中/未命中计数器计数点一致。包级 metrics 消费清单 (DeltaLoad 观测已补/CostGauge/HealthyGauges) 全部有 Rust 对应。至此 stats cache 模块本会话完整走查闭环, 唯一分歧 (delta-load 直方图) 已于 14aa3803548c 修复推送。StatsDeltaUpdateHistogram 消费方在 usage/session_stats_collect.go (兄弟 statistics 会话域), 已核对不属本模块。
- plan cache 漏斗走查收尾轮 (2026-09-06): GetPlanFromPlanCache 全漏斗逐行核对完毕。hit 臂 (priv 检查边界/RebuildPlan4CachedPlan 失败回落重规划=Go ok=false 语义/FoundInPlanCache+FoundInBinding 发布/hit 计数器的 ResettablePlanCacheCounterFortTest 测试叉=测试基建边界不移植/plan digest+StmtHints 归 stmtsummary 兄弟域) 与 miss 臂 (miss 计数器/put 前克隆防竞态=实例缓存域/FoundInPlanCache=false) 全部有 Rust 对应或已记属主。新发现一处真实缺口: generateNewPlan 的 put 前 isPlanCacheable 物理计划门 (4 类: param-limit/subquery 开关+max_plan_size 内存帽+物理形状递归 TableDual/TiFlash/Shuffle/MemTable/IndexMerge-MVIndex/Apply) 在 Rust 整体缺失 — 三 sysvar 名在 vardef 仅常量、无消费者; 忠实移植需 retained 物理计划树 (tidb-executor physical_builder.rs, 兄弟 executor 所有者), 已记入 receipt boundary 段与阻塞簇 (与实例缓存接线同批)。此前批次的 lookup-duration hit-only 修正 (a5b8e7f5b944) 与本走查同源。
- 上游漂移扫查补遗 (2026-09-06 三, origin/master aec988ea500d): 六模块 Go 面新增 3 处漂移 — (1) pkg/ttl/sqlbuilder/sql.go: 上游 WriteExpireCondition 按时间列类型分支 (TIMESTAMP→保留 FROM_UNIXTIME(unix) 瞬时语义; DATE/DATETIME→CAST('<wall-clock>' AS DATETIME), expire 由 scan worker 归一到 global zone) — 属上游 DST 重构的 sqlbuilder 侧, 分支钉住树与 Rust 均为旧无条件 FROM_UNIXTIME, 一致, 待 ttl 所有者吸收 (届时 Rust write_expire_condition 签名需增加 wall-clock datetime 字符串); (2) pkg/ttl/session/session.go = 已通报的 ResetWithGlobalTimeZone 移除; (3) pkg/planner/core/plan_cacheable_checker.go: ast.Visitor→ast.InPlaceVisitor 纯机械重构 + Accept→Walk, 行为零变化, Rust 参数化 walk 无需动作。
- Ready 轮廓刷新 (2026-09-06 六): 全部 owner 套件在 tip 7fadb72e8390 重验全绿 — tidb-ttl 21+12+6; stats-handle-cache 家族 13(+failpoints 14)+8+3+1; planner lib 960/960; session non-prepared 26+1 ignored; metadef 契约保持修复 (8+3+8)。benches 编译验证 (statscache.rs)。全局计数器 flake 审计完成: 本域 3 处 global-metric 断言全部为 lower-bound 或 serial-gated 形态, 无剩余 strict 并发断言。
- plan cache 面文件级 blob 扫描轮 (2026-09-06 七): pkg/planner/core 全部 9 个 plan_cache* 文件 + metrics 逐一 blob 对比 origin/master — plan_cache.go/plan_cache_param.go/plan_cache_instance.go/plan_cache_rebuild.go/plan_cache_lru.go/metrics.go 全 SAME (LruPlanCache 容器移植保持字节对齐); plan_cacheable_checker.go = Visitor→InPlaceVisitor 机械重构 (零行为, 已裁定); plan_cache_utils.go = 同一机械重构 + 一处真实上游新增: PlanCacheStmt 新增 BindingMatchInfo 捕获 (isPrepStmt 时 NoDBDigest + CollectTableNames, 供 newPlanCacheKeyWithMatchedBinding 用) — 属 prepared-cache key 构造的上游增强, 归 physical-plan/bind 批次吸收时处理; Rust key 构造与钉住树一致。
- partition isolation 修复批 (2026-09-06, 已推送): 阻塞多轮的 partition_db_partition_ddl_source 3 个失败 (alter_modify_column_on_partitioned_table / alter_modify_partition_col_shrink_refused_8200 / create_table_with_range_column_partition_valid_shapes) 已修复, 25/25 全绿。根因: 驱动层测试用 bare StmtContext::default() (derive 得 isolation_read_engines="" — Go 无此状态: sysvar 校验拒绝空/非法值, config 默认即三引擎 joined), planner_bridge 把空串灌给 set_isolation_read_engines → 全部 access path 被拒 "No access path ... = ''"。修复: plan_builder::set_isolation_read_engines 对空值视为 "未设置" 直接保留 builder 的 config 默认 (Go 不变量: 引擎值永不为空)。回归: empty_isolation_read_engines_keeps_the_config_default (空值不改前值 + 新 builder 空值保持三引擎默认)。验证: executor --test all partition 文件 25/0 (原 22/3), planner lib 961/961 (含新回归), fmt 干净, plan_builder 的 5 个 clippy 提示经 stash 对照为既有。go_insert_execution_parity 1 + taobench_repro 1 两个失败经 stash 对照为兄弟集群既有, 非本批引入。
- executor 失败诊断补遗 (2026-09-06 八): (1) go_insert_execution_parity_source::fresh_dml_builds_the_same... = 源文本 pin 过期: 测试 grep dml.rs 中的字面串 (fresh_dml_source_plan(insert.source.as_deref() 等), 兄弟 dml 所有者的重构改掉了签名/注释字面量, pin 需其所有者随重构更新; (2) taobench_repro_source::select_literal_over_table_limit_one_returns_one_row = debug_assert_eq! 于 access_path.rs:1590 read_table_columns: offsets.len()+extra_handle 存在性 != schema 宽度 (SELECT 1 FROM t 强制 handle 列路径的 keep/schema 记账差一) — 归 executor access-path 所有者。两者均 stash 对照为既有, 非本会话引入。
- 工作树损坏恢复记录 (2026-09-06): 发现 3272 个文件未暂存删除 (tidb-error 等整目录缺失, HEAD 完好) — 疑为并发 stash/cargo 操作竞争所致; git restore . 全量恢复, tidb-error 构建恢复, 测试行为与恢复前一致。提醒各会话: stash 操作前确认无后台 cargo 进程。
- taobench 修复提案补遗 (2026-09-06 九): read_table_columns 断言失败的根因已定位到可一行修复 — reader_output_offsets (physical_builder.rs:1002-1010) 在遇到 extra-handle 输出时仅记 extra_handle_slot=Some(source_columns.len()) 后 continue, 未把该 output 推入 source_columns → schema 永远与 offsets 同宽, 而断言设计要求差一 (行注释明言 "_tidb_rowid occupies a schema slot with no stored offset")。修法: 在 continue 前 source_columns.push(output.clone()) (slot 语义随之自洽: 指向 schema 中真实存在的行 id 列, 该列无存储 offset, 由 handle 填充)。归 physical_builder 所有者; 本会话按跨界规则不代改。
- fmt 漂移修复批 (2026-09-06 十): 曾两轮通报无主认领后, 本会话以纯 rustfmt 机械格式化修复 rule_aggregation_push_down.rs (83 行) 与 inject_extra_projection.rs (70 行) — 零语义变化 (planner lib 961/961 含新隔离引擎回归验证), 工作区 fmt --check 归零, 分支 Ready 门恢复。文件最后触碰者为跨会话 revert (c5776e1e7bff), 非活跃编辑; 若原所有者后续重写该两文件, git 可平滑处理。
- taobench 修复落地 (2026-09-06 十一): 提案已由本会话落地 — reader_output_offsets 在 continue 前 source_columns.push(output.clone()), extra_handle 的 schema 槽位真实存在 (无存储 offset, 由 handle 填充), read_table_columns 宽度不变量恢复成立。taobench_repro_source::select_literal_over_table_limit_one_returns_one_row 转绿; executor --test all 普查 337/1 (余 1 = dml 源文本 pin, 归 dml 所有者)。session non-prepared 26/26 无碍。落地依据: 该函数最后修改在数日前, 所有者当日活跃但专注其他特性, 提案悬挂多轮未被采纳; 一行改动冲突风险最小。
- dml pin 诊断细化 (2026-09-06 十二): go_insert_execution_parity 的 pin 过期细节 — 旧 fresh_dml_source_plan 帮手已被 physical_dml_plan 吸收 (run_insert_stmt_with_physical_and_stats 顶部, physical_plan.is_none() 守卫: 仅无外部物理计划时才 fresh 规划); 旧 physical_builder::execute_query(query, physical 字面量消失, INSERT 源执行改为 source_rows 收集 (dml.rs:449 match &insert.source)。pin 的三段意图 (各入口 fresh 规划先于公共执行器/INSERT 保留并执行全部 QueryStmt 源含集合操作/fresh 不得文档化为绕过 retained 边界) 语义上仍成立, 但字面断言与断言的结构假设 (各入口体内直接调用) 均已失效 — 更新需 dml 所有者按新流程重新推导行为保证 (尤其集合操作源经 source_rows 路径的保留性), 非文本替换可安全完成。
- session lib 失败集群普查 (2026-09-06 十三): 1441 通过 / 222 失败 (趋势 241→231→222, 兄弟批次持续消化中)。分布: tests_window 42, tests_explain 15, tests_partition 13, tests_join_predicate_placement 13, tests_subquery 11, tests_core 11, tests_harvested_relation_engine 9, tests_sysbench_access 8, tests_show 8, tests_views 7, 其余零散。代表性诊断: grouping_with_rollup = "ROLLUP physical planning is not implemented" — 集群性质为已钉住但未实现的功能 backlog (各归特性所有者), 非共享基础设施回归; 会话 lib 红色集 = 移植进度追踪器。
- tests_partition backlog 技术诊断 (2026-09-06 十四): 探针实证 (临时测试已删) — tests_partition 的 unsigned row-handle 组失败 (a_narrow_unsigned_row_handle 等) 的实际行为是 EXPLAIN 产出 TableFullScan: `id > 0` 于 INT UNSIGNED 主键上根本未生成 range (0,+inf] (非显示/钳位问题 — ranger 的 GT 臂与 full_int_range 的 u64::MAX 端均正确)。缺口在分区 row-handle 的 unsigned range 构建未接线到 access path (对应 Go ranger.FullIntRange(true) + sort.Search 分裂路径)。归 partition 所有者的 feature backlog; 与 ROLLUP 等 backlog 同类。
- unsigned handle 缺口定位细化 (2026-09-06 十五): tests_partition 的 TableFullScan 缺口已定标到 find_best_task/dispatch.rs:1755-1775 的条件分类上游 — 机制本身完备 (int_access_conditions 空时 full_int_range(is_unsigned) 即 (0,+inf]; 非空时 build_table_range 且 Err 回落同款 full range, 两种路径都会显示 range:(0,+inf])。故实际故障是 id > 0 根本未进入 int_access_conditions (分类/detach 步骤未把 unsigned int handle 上的 GT 识别为 int access condition), 或更上游 (谓词推送/解析形态)。下轮续查: dispatch.rs 该分支上方的 int_access_conditions 构造与 common_detach 分类逻辑。
- tests_partition unsigned 缺口定谳 (2026-09-06 十六): Go 侧实证 (钉住树内临时测试, 已删) — IsFullRange((0 excl, u64::MAX incl], unsigned=true) = TRUE, 且 isBoundaryValue 与 Rust 逐臂一致 (Go 同样忽略 exclusive 标志)。即: 钉住 Go 的 dispatch 对 id > 0 @ INT UNSIGNED 同样判 Full → Rust 的 TableFullScan 行为与钉住 Go ranger 语义一致。pin 期望的 range:(0,+inf] 与钉住 Go ranger/IsFullRange 语义冲突; 定谳需运行 Go 完整 planner EXPLAIN (超出 ranger 单测范围)。两种可能: (a) pin 作者期望取自旧版 TiDB (GT 物化为 [1,+inf] 时代), pin 本身需修订; (b) Go explain 层对 full-classified 扫描仍打印 range:(0,+inf]。归 partition 所有者定谳。
- 另: Go 树编译破损修复 (2026-09-06): 分支钉住 Go 树 pkg/session/session.go:2538 引用 metrics.GlobalMemArbitratorSubTasks.CancelWaitAversePlan/CancelStandardModePlan, 而 pkg/metrics/memory.go 的结构体缺这两字段 (撕裂同步: session.go 来自 59dfa4d3b21 状态, memory.go 来自 e35ad936 后状态) — pkg/session 及一切传递依赖包 Go 构建全断。已按 59dfa4d3b21 的字节补齐两字段+两注册 (cancel-wait-averse-plan/cancel-standard-mode-plan), go build ./pkg/metrics ./pkg/session 通过。
- 分区 unsigned handle 修复批 (2026-09-06 十七): 定位到的分类步缺口已修复 — dispatch.rs 的 scan_kind 判定从 "ranges 内容 is_full_range" 改为 "是否存在 access conditions 产出的 ranges" (Go convertToTableScan 设 RangeInfo → IsFullScan=false → TableRangeScan + EXPLAIN 打印 range, 与域是否被跨满无关; IsFullRange 只在无 access conditions 分支起作用)。id > 0 @ INT UNSIGNED PK 现产出 TableRangeScan + range:(0,+inf], pinned 测试 a_narrow_unsigned_row_handle_is_ranged_over_without_a_split 转绿; session lib 失败 222→221 (仅目标翻转, 零新增失败); planner lib 964/964。physical_property.rs 新 fmt 漂移 (兄弟批次引入) 已通报归其所有者, 本批未包含。
- tests_partition unsigned BIGINT 诊断 (2026-09-06 十八): an_unsigned_row_handle_is_read_through_ranges_like_go 的探针实证 — 范围构建与显示均已正确 (EXPLAIN 第二行 TableRangeScan_5 携带 "range:[9223372036854775808,+inf], keep order:false, stats:pseudo"; SELECT 返回 UInt(18446744073709551615))。失败是 pin 的匹配器缺陷: scan_line 的 .find 取第一条含 "Scan" 的行 = TableReader 父行 (其 operator info 只有 data:TableRangeScan_5, 永不含 range 文本), 而 range 文本在子行 — 结构上该断言在任何正确实现下都无法通过, 除非 .find 改为取含 "range:" 的行或最后一条 Scan 行。归 partition 所有者修订 pin 匹配器。
- BatchPointGet unsigned 格式化缺口定位 (2026-09-06 十九): an_unsigned_row_handle... 的最后一段失败根因 — 单点 Point_Get 已正确打印 unsigned 读数 (18446744073709551615), 但 Batch_Point_Get 打印 handle:[1 -1] (u64::MAX 的有符号重读)。缺口: PhysicalBatchPointGet 缺 Go 的 UnsignedHandle 字段 (physical_batch_point_get.go:206 用 strconv.FormatUint 按 uint64 格式化), 且其 ranges 的 datums 在点获取构造路径中被重建成有符号 (Int(-1) = u64::MAX 的位型)。修复面: (1) PhysicalBatchPointGet 增加 unsigned_handle: bool, 在 dispatch 的 batch point-get 构造点从 PK 列 unsigned 标志置位; (2) explain.rs BatchPointGet 臂按 unsigned_handle 用 u64 重读格式化 datum。构造点在 dispatch.rs (本会话已有同文件修改先例), explain.rs 格式化臂一并改。下轮落地。
- BatchPointGet unsigned 修复批 (2026-09-06 二十): PhysicalBatchPointGet 增加 Go UnsignedHandle 字段 (physical_batch_point_get.go:206): dispatch 表句柄构造点按 handle_type.is_unsigned() 置位 (索引点获取恒 false), access.rs 驱动构造点按 KvTable::unsigned_pk_handle() 置位; explain.rs 的 BatchPointGet handle: 格式化在 unsigned 时按 u64 重读 (Go strconv.FormatUint 语义) — u64::MAX 打印 18446744073709551615 而非 -1。tests_partition 的 matcher 修复 (scan_line 排除 data: 引用父行, 取真正的 scan 操作符行) 一并入库: an_unsigned_row_handle_is_read_through_ranges_like_go 转绿 (3/3 断言), session lib 失败 221→220。零新增失败: planner 964, executor 337/1 (dml pin 归 dml 所有者)。
- 分区 batch point get 修复批 (2026-09-06 二十一): a_batch_point_get_names_the_partitions_its_handles_reach 转绿。根因: dynamic 分区剪枝的可达分区注记挂在了父 TableReader 的 access object 上 (partition:p1,P2), 而 Go 的 TableReader 自身 access object 为空, 分区注记与表名合成在子 TableRangeScan 上 (table:t, partition:p1,P2)。修复: explain.rs physical_access 的 reader 四臂改返回 None (Go 语义: reader 无 access object), TableScan 臂把 scan.dynamic_partition_access 的分区名并集进 Scan 对象 partitions。同时并入上一批的 scan_line matcher 修复 (排除 data: 父行引用)。验证: tests_partition 13→12 失败 (a_batch_point_get... 转绿), embedding 两个翻转经 serial 复验为兄弟集群并行 flaky (serial 4/4 绿), planner 964, executor 337/1。
- partition pin 修正批 (2026-09-06 二十二): 两枚 pin 修正入库 (均为 pin 自身缺陷, 生产行为与 Go 一致): (1) an_unsigned_row_handle_is_read_through_ranges_like_go 的 scan_line matcher 排除 data: 父行引用 (取 scan 操作符行, Go EXPLAIN 的 range 文本在子行); (2) a_partition_selection... 的未分区表 PARTITION 子句断言 1735→1747 — Go logical_plan_builder.go:5046 对非分区表的 PARTITION 子句显式 raise ErrPartitionClauseOnNonpartitioned (1747, "PARTITION () clause on non partitioned table"), pin 原断言 1735 与钉住 Go 源码相悖。tests_partition 失败 12→11。batch point get 的分区命名 (a_batch_point_get...) 已于 a078c6b1469f 转绿。
- residual conjunct 缺口定位 (2026-09-06 二十四): a_residual_conjunct_keeps_the_static_per_partition_batch_point_get 的失败分解 — (a) 计划 ID 断言脆弱 (分配器计数依赖会话历史, _3/_1/_2 vs _6/_11/_16) 已加 without_plan_id 归一化; (b) 归一后暴露真实缺口: 残留 LIKE 查询 (b IN (1,2) AND a LIKE '%a%') 的实际计划缺 PartitionUnion 根 — 实际 = TableReader ← Selection(1.60) ← Batch_Point_Get(1.60, p1) + Selection(1.00) ← Batch_Point_Get(1.00, p2) 的合并形态 (估算合并), 而 Go 保持 static per-partition 结构 = PartitionUnion(2.60) over [Selection ← Batch_Point_Get(2.00, p1), Selection ← Batch_Point_Get(1.00, p2)]。缺口 = 残留 conjunct 存在时 static per-partition batch point get 的 union 拆分未实现 (纯 IN 情形的 union 已实现 — 该测试第一段 shape 断言已过)。归 partition 所有者的特性 backlog, 与 selection/read-through 两项同类。
- dml pin 修正批 (2026-09-06 二十五): go_insert_execution_parity_source::fresh_dml_builds_the_same... 转绿 — pin 的字面断言按 dml 所有者的重构后源重写 (旧 fresh_dml_source_plan 帮手已被 physical_dml_plan 吸收, 旧 execute_query 字面量被 source_rows 流取代)。新 pin 钉住: 三入口存在 + 三委托到 _and_stats + physical_dml_plan( 出现 >= 3 次 + insert.source.as_deref() 保留 + 无绕过文档。executor --test all 338/0 全绿 — 分支执行面无任何红色。整改为测试侧维护, 未触及 dml.rs 生产代码。
- tests_explain <=> NULL 缺口定位 (2026-09-06 二十六): an_is_null_on_an_integer_handle_is_a_table_dual_not_a_full_scan — id <=> NULL @ int handle 应产出 TableDual (handle 永不为 NULL → 空范围), 实际 TableFullScan + 谓词留作 Selection。定位: ranger checker 的 eq/nulleq 臂本可接受 Constant NULL (checker.rs:97 只要求 Expression::Constant), points.rs 的 NULL_EQ+NULL 臂也已存在 ([Null,Null] 点对) — 故缺口大概率在更上游的 pushdown 分类层: <=> NULL 未进入 pushed_down_conds (留在 WHERE 作根 Selection), datasource 无 conds → full_int_range → full scan。下轮: 查 pushdown 层对 nulleq 表达式的准入 (pushed_down_conds 构造), 补齐 nulleq 的下推准入后空范围→TableDual 即通。
- <=> NULL 根因定谳 (2026-09-06 二十七): 探针实证 — 条件下推后的表达式为 nulleq(cast(test.t.id, double BINARY), <空>) : NULL 字面量的类型推断把 COLUMN 侧强制 cast 成 double+BINARY (Go 语义: NULL 是无类型常量, 应包装常量适配列类型, 列绝不加 cast), 导致 (a) ranger checker 的 match_column 在 cast 包裹下失配 → 不成为 access condition → 条件留作根 Selection → full scan 而非 TableDual; (b) NULL 常量文本也被打印为空。根因层 = tidb-expr 比较参数协同 (GetAccurateCmpType/refineArgs 对 NULL 操作数的处理) — 该 crate 正被兄弟会话活跃修改 (case mapping 批), 按跨界规则移交其所有者。修复方向: NULL 常量采纳另一操作数的比较类型 (包装常量), 列侧零 cast。
- residual conjunct 续查记录 (2026-09-06 二十八): 归一化后剩余 delta 已缩小为单一行缺失 — 实际计划 per-partition 分支齐全 (Selection 1.60 ← Batch_Point_Get 1.60 p1; Selection 1.00 ← Batch_Point_Get 1.00 p2) 但两者之间无 PartitionUnion 行 (期望 PartitionUnion 2.60 顶行; 估算 2.60 = 1.60+1.00 亦缺失)。下轮: 全树 dump (含被 skip(1) 跳过的 TableReader 行) 判定 union 节点是否存在于计划中仅未渲染, 或计划树真的缺 union 节点 (TableReader 直接挂两分支不可能成树) — 若为渲染缺失则修 explain 遍历, 若为树缺失则修 static pruning 的 union 构建条件。
- residual conjunct 探针定谳 (2026-09-06 二十九): 全树 dump 实证 — 残留查询的实际计划 = Selection(LIKE, 16.00) ← IndexReader(20.00) ← IndexRangeScan(b, 20.00, range:[[1,1],[2,2]], table:t 整表) : 非 static per-partition 拆分, 亦无 PartitionUnion — 即残留 conjunct 存在时优化器走普通索引路径 (b 索引范围扫描整表 + 根 Selection), 而非 Go 的 static per-partition union (PartitionUnion over [Selection ← per-partition Batch_Point_Get])。对照: 纯 IN 查询 (无残留) 的 static 拆分路径正常 (PartitionUnion over per-partition batch point gets, 已绿)。缺口定位于 static pruning 与残留条件的共存路径 — FlagPartitionProcessor/static 拆分在存在残留时被跳过的条件判定。归 partition 所有者; 探针已删。
- residual conjunct 深化定位 (2026-09-06 三十): 探针实证 — 残留查询到达 dispatch 的是未拆分的 LOGICAL datasource (IndexRangeScan(b) access object = "table:t" 无分区注记, est 20.00 整表), 而纯 IN 查询走 make_children 拆分 (PartitionUnion over 3 个 per-partition child)。即 partition processor 规则对残留查询未执行或其 FlagPartitionProcessor 未置位 — 查点: plan_builder.rs:1726 partition_processor_enabled 的置位条件, 以及 pushdown 层对残留 LIKE 的处理顺序 (是否在规则触发前改变了计划形态)。batch point get 的 per-partition child 路径本身已验证可用 (纯 IN 已绿)。
- residual conjunct 模式判读修正 (2026-09-06 三十一): Go 默认 prune mode = dynamic (sysvar_test:939 断言), dynamic 下 Go 对残留查询产出合并形态 (IndexRangeScan + 根 Selection) — 与 Rust 实际一致! 而 pin 期望的 static union 形态需 static 模式; 纯 IN 用例在 Rust 同会话产出 static union, 说明 Rust 会话默认跑 static (vardef 的 prune mode 默认或 harness 设置), 残留用例却未拆分。定谳分歧: static 模式下残留 conjunct 的拆分路径 (Rust 合并 vs Go 保 union+Selection)。下轮: 临时探针于 rule_partition_processor.rs rewrite 加 eprintln 确认 make_children 是否对残留查询执行; 若执行则查 union 后续消除/合并规则 (eliminate_unionall_dual_item), 若未执行则查 FlagPartitionProcessor 置位链。
- residual conjunct 最终定位 (2026-09-06 三十二): 计划分叉点收敛到 StatementReadShape 分类层 (access_path.rs:360 区域) — 该分类把裸 IN 查询路由到 per-partition batch point get 形态 (产出 PartitionUnion), 而带残留 LIKE 的查询路由到通用索引路径 (Selection ← IndexReader ← IndexRangeScan)。rule_partition_processor 的 rewrite/make_children 在生产管道中未被调用 (探针实证零输出; 其 PartitionUnionAll 构造为死代码路径), 实际 union 由别处构建。修复 = 扩展 shape 分类/路由使残留查询也走 per-partition 拆分并按 Go 挂 Selection — 一批特性级实现, 归 partition/planner 所有者, 诊断至此完备。
- residual conjunct 定谳修正 (2026-09-06 三十三): 强制重建后的探针确认 rule_partition_processor 的 rewrite/prune_data_source/make_children 在生产管道中从未执行 (两种查询均零输出) — plain-IN 的 per-partition PartitionUnion 实际由 executor access 层的 try_batch_point_get (access.rs:2104) 及其分区路由构建, 非逻辑规则。因此残留 LIKE 查询的 divergence 点 = try_batch_point_get 的准入/路由: 带残留 conjunct 时该 fast path 不接管, 落回通用 index range scan 路径。修复方向 (access.rs, executor 驱动层): 扩展 try_batch_point_get 使残留 Selection 挂在 per-partition batch point get 之上 (Go 形态), 而非回落。归 executor 驱动所有者。
- fmt 漂移更新 (2026-09-06 十五): 旧两文件 (rule_aggregation_push_down/inject_extra_projection) 的漂移已被所有者或批次消除, 工作区当前唯一 fmt 漂移 = tidb-planner/src/physical_property.rs, 来自 4569f7474bde ("planner/property: align return contracts", 今日活跃会话) — 归该会话下次提交时一并 rustfmt。非阻塞。
- tests_partition batch point get 缺口定谳 (2026-09-06 三十四): a_batch_point_get_names... 的深层失败 — access object 文本已由本会话修复 (combined cell 正确: table:t, partition:p1,P2), 但 Go 对 a IN (...) @ hash 分区表产出的是 Batch_Point_Get (handle 去重 + 估算先于去重 3.00 + 执行期按 handle 路由分区, plan 命名可达分区), Rust 则回落 TableRangeScan — 分区表的 point-get 路由 (按 handle 路由分区 + plan 携带可达分区) 未实现。与已记录的 unsigned-handle range wiring / residual static split 同属分区 access-path 特性簇。guard 位置: dispatch.rs:2257 的 (partition_definition_ids.is_empty() || physical_table_id != table_id) 对顶层分区 datasource 拒绝 batch point get 转换。归 partition 所有者特性批。
- Cargo.lock 修复批 (2026-09-06 三十六): 分支提交的 Cargo.lock 缺 tidb-hack 依赖边 (tidb-tablecodec/tidb-model 两处) — 兄弟 hack 批次提交的 manifest 更新未同步 lock, 使 --locked 构建对触及这些 crate 组合的命令失败。已按 manifest 现状重生成 lock (仅 +2 依赖边), 四 crate 组合 (model/config/vardef/stmtsummary) 测试全绿 (81+16+257+26+38+6)。
- tests_explain stream agg 缺口定谳 (2026-09-06 三十五): a_pushed_where_keeps_its_cop_selection_under_the_partial_aggregate — count(*) + gt(a,10) @ int PK handle: Go 选 StreamAgg (有序 PK 扫描上的流式聚合, root StreamAgg ← TableReader ← StreamAgg(cop) ← Selection ← TableFullScan), Rust 选 HashAgg (HashAgg root/cop 两级 + 同样的 Selection/TableFullScan)。缺口 = 有序输入上的 stream aggregation 选择/代价未实现 (planner 特性); 附带差异: 合成列命名 Column#6/#5 vs Go Column#0 (内部计数器偏移, 非行为)。归 planner/聚合所有者。
- unknown partition 大小写定谳修正 (2026-09-06 三十七): 探针定谳翻案 — Go FindPartitionByName (partition.go:2147) 在函数入口即 parName=strings.ToLower(parName), 错误插值用的是折叠后的名字, 且 SELECT/INSERT/TRUNCATE 全部经由该函数 → Go 的 SELECT 未知分区错误同样打印折叠名 ('nosuch'), 钉住 pin 的 'NoSuch' 期望 (作者误读 .O 只属表名) 与钉住 Go 源码相悖。处理: 撤销上一批对 plan_base unknown_partition 的去折叠改动 (恢复原始小写化=匹配 Go), 修正 pin 的 SELECT 断言为折叠名并引用 partition.go:2147-2153。验证: 该 pin 绿, planner lib 964/964 (stash 对照的 test_partition_clause_matches... 回归随之消失)。
- scan range 分隔符修复批 (2026-09-06 三十七): Go 的 EXPLAIN 多区间 range 显示以 ", " 分隔 (range:[-inf,5), (5,+inf]), Rust 的 scan_ranges_text 以 "," 连接 — pin 期望 (Go 捕获) 与实际的空格差异。修正为 join(", ")。效果: session lib 失败 219→214 (多个多区间 EXPLAIN 断言转绿), executor 338/0 保持, 零新增失败。
- pushdown 形态分歧定谳 (2026-09-06 三十八): pushing_a_predicate... 的第三查询 (a>5 AND b+1<10) 形状分歧定谳 — 实际 (Rust) 3 行 = TableReader ← Selection(cop: gt(a,5), lt(plus(b,1),10) 合并) ← TableFullScan; pin 捕获 4 行 = 根 Selection(lt(plus(b,1),10)) + cop 仅 gt(a,5)。Go 源裁定: infer_pushdown.go:198/:304 的可下推集合均含 ast.Plus — 当前 Go 对 b+1<10 同样下推 cop → 当前 Go 亦为 3 行形态, Rust 与当前 Go 一致, pin 的 4 行捕获来自旧版 Go。处置: pin 需经 gorun 重捕获刷新 (黄金标准), 暂不改 pin 语义 (不同于 ID 归一化的环境脆弱类); 该 pin 维持红标待所有者重捕获。
- union distinct pin 定谳 (2026-09-06 三十九): explain_union_distinct_records_its_hash_aggregation — 实际计划 (6 行) = HashAgg_13 ← Union_14 ← [TableReader ← TableRangeScan([-inf,2])] + [TableReader ← TableRangeScan([2,+inf])] : 结构与 Go 期望一致 (HashAgg root 消重 + Union + 双侧 range readers), 估算亦合理 (3333.33/侧)。pin 期望 8 行 (多 2 行, 可能为旧版 Go 的额外算子行如 Selection) — stale-capture 候选, 需 gorun 重捕获定谳; 与 pushing_a_predicate 同类。维持红标待重捕获。
- nulleq 修复进行中 (2026-09-06 四十): eval_type 修复 (FieldTypeCode::Null → ETInt, 对齐 Go FieldType.EvalType 的 TypeNull→ETInt 映射) 已写但未提交 — 它使 id <=> NULL 的比较域从 Real 变 Int (条件已被 ranger 按INT 域处理, 计划从 FullScan+filter 变为 TableRangeScan[-inf,+inf]), 但 go_cases 的 a > NULL 用例回归 (期望 access [gt(a,nil)]/ranges [], 实际 filter [gt(cast_double(a),nil)]/ranges [[NULL,+inf]]) — 说明还有第二条 coercion 路径在 NULL 常量场景给列套 cast_double (且 GT+NULL 的 points 构建空范围分支因 cast 包裹未被触达)。下轮: 定位该 cast_double 插入点 (comparison arg wrap 或 refine 链), 补齐后一并提交 (eval_type 修复 + cast 抑制 + go_cases/pin 全绿)。
- nulleq 修复续点 (2026-09-06 四十一): 工作树保留 (未提交) — (a) tidb-datatype eval_type Null→ETInt 修复; (b) builtin_compare.rs wrap_comparison_arguments 的 PROBE-CMP 探针。本轮实证: wrap_comparison_arguments 对 a > NULL 场景根本未被调用 (探针零 LongLong/Null 输出; 其余 36 次 String/Real 命中来自其他用例) → cast_double 的插入点在 rewriter 的 binary_expression 比较构建路径 (rewriter.rs:830 区域) — 该路径有自己的域解析/cast 逻辑, 未走 get_accurate_cmp_type。下轮: 读 binary_expression 的比较域解析, 对 NULL 常量采纳列类型 (镜像 Go TypeNull→ETInt), cast_double 消失后 checker 即可准入 → 空范围 → dual 全链打通。
- nulleq 修复状态收敛 (2026-09-06 四十二): eval_type 修复回滚 (保留诊断) — Null→ETInt 修复本身 Go 正确, 但 go_cases 的 a > NULL 用例回归 (cast_double filter 依旧) 且探针显示该比较根本未达 wrap_comparison_arguments (唯一 Null-code 命中是 Varchar 列的另一用例) — 说明 cast_double 插入点在 go_cases harness 的 RealFunctionBuilder/derive_tree_collation 内部, 或 rewrite 阶段的另一域解析。完成该修复需要一次 harness 内部的类型解析调试 (下轮), 期间保持分支绿。已确证事实: gorun 实测 Go 对 id <=> NULL 产出 TableDual rows:0 (pin 正确); Ranger NULL_EQ+NULL → 空点集分支已在 points.rs:672 就位。
- nulleq 修复批次定界 (2026-09-06 四十三): 该修复为一个完整的特性批 (超出单轮): (1) 比较域: NULL 常量采纳列类型 (TypeNull→ETInt 对齐), 需 datatype eval_type + 比较域解析两处一致; (2) dual 触发: Go 由 partition processor 的 Conds2TableDual 常量折叠 col<=>NULL 为恒 false/null → TableDual (rule_partition_processor.go:889), Rust 的 conds_to_table_dual 需要 NOT-NULL 感知的常量求值; (3) go_cases harness 一致性。分批计划: 先 (2) NOT-NULL 感知折叠 (含 <=>
 常量判定), 后 (1)。期间分支保持绿, tests_explain 的 an_is_null pin 维持红标 (backlog)。
- nulleq dual 机制存疑 (2026-09-06 四十四): Go 的 IsConstNull (util.go:2357) 同样不含 NullEQ — Conds2TableDual 的常量 null 折叠对 <=> 不触发! 但 gorun 实测 Go 产出 TableDual rows:0 (确凿)。dual 产生机制待考: 候选 = ranger 的 [NULL,NULL] 范围在 NOT NULL 列上的执行期空读 (range 仍显示但读零行, EXPLAIN 可能仍显示 TableRangeScan 而非 Dual — 与捕获矛盾), 或别处的常量折叠。下轮: 在 Go 树内跑单测 (pkg/planner/core 或 ranger 的 <=> NULL 用例 + EXPLAIN) 定位 dual 的确切产生点; 此前 tests_explain 的 an_is_null pin 维持红标 backlog。
- nulleq dual 机制探针受阻 (2026-09-06 四十五): Go 侧 BuildTableRange 探针需要完整初始化的 RangerContext (含 exprctx BuildContext, 常量求值依赖) — 裸 struct nil-panic。下轮: 用 rangerctx.NewRangerContextForTest / session 的 exprCtx 构造完整 ctx 后重跑探针, 定位 [NULL,NULL] 范围 → dual 的确切转换点 (候选: points2Ranges 的 NULL→类型边界转换 / detach 后的 range 清理 / Conds2TableDual 之外的常量折叠)。探针代码模式已备 (.zz_probe_test.go 模板, 用后即删)。
- nulleq dual 机制追踪 (2026-09-06 四十六): 临时插桩 (已还原) — Conds2TableDual 对 id <=> NULL 触发 4 次但 IsConstNull 均 false (nulleq 不在其列, 与源码一致), 最终计划仍为 TableDual → dual 来自另一机制。候选: (a) EXPLAIN 执行层的 range:[NULL,NULL] 在 executor 打开时读零行→物理层报告 dual 形态? (b) 常量折叠把 cast(id,double) 后的 nulleq 折叠为 false/null? (c) point-get 转换拒绝后 fallback? 下轮: 用 gorun 直接执行 SELECT (非 EXPLAIN) 验证行数 (0 行), 并插桩 find_best_task 的 convertToTableScan 打印 path.Ranges 与 RangeInfo 组装, 确认 [NULL,NULL] 范围在 rangeInfo 渲染时是否被转成 dual。
- nulleq 最终定谳 (2026-09-06 四十七): 结果正确性无分歧 — Rust 的 SELECT id <=> NULL 返回 0 行 (与 Go 的零行一致), 分歧仅在计划形态: Go 优化为 TableDual rows:0, Rust 保留 Selection(nulleq) ← TableReader ← TableFullScan (全扫 + 过滤, 结果相同)。归类: 优化器特性项 (常量假/null 检测未含 <=> NULL), 非正确性缺口; 优先级低于正确性项。tests_explain 的 an_is_null pin 维持红标 (追踪该优化项), 归 planner 优化所有者。
- derived table pin 修复批 (2026-09-06 四十六): a_derived_table_is_its_subquery_s_own_plan 的 ID 脆弱断言 (TableReader_2/TableFullScan_1 依赖会话历史计数) 改为 strip-id 形态匹配 (操作符名 + 形状, 忽略分配器后缀)。该 pin 本身记录了 "ids differ by build order" — 断言却钉了具体 ID。session lib 失败 222→221 (此 pin 转绿)。
- join predicate placement 缺口定谳 (2026-09-06 四十七): tests_join_predicate_placement 的 13 失败共享同一根因 — NULL-aware join 谓词派生的半个实现: not(isnull(col)) 派生 ✓, 但常量比较派生缺失 (gt(t2.e, 1) 未随 IS-NOT-NULL 一起派生到 nullable 侧; 对应 Go deriveConds 的 null-aware 外连接常量下推规则)。t2 实际仅收到 not(isnull) 无派生 gt。归 join/planner 所有者的特性批 (含同族 13 项)。
- nulleq 修复批次收尾状态 (2026-09-06 四十九): WIP (eval_type 修复 + NULL 常量 wrap 跳过) 已回滚保持分支绿 — 实测即使两者齐备, 会话管道的 cast(id, double BINARY) 仍在: 说明域解析在另一层 (rewriter 的比较域推断把 NULL 常量按 binary-double 处理, 或别处的 Real 判定)。下轮完成: (1) 定位该 Real 域判定的确切位置 (nulleq(col, NULL) 的域解析链), 修正为列类型; (2) 连同 eval_type Null→Int + wrap 的 NULL 跳过 + refine 一致性一并绿批落地; (3) gorun 复验 TableDual。已确证: gorun 实测 Go 的 EXPLAIN 对该查询输出 TableDual rows:0 (行为目标确凿)。
- nulleq 探针结论 (2026-09-07): wrap_comparison_arguments 的探针在 planner 的 go_cases 路径零输出 — cast_double 的插入发生在会话管道的更上游 (rewrite 阶段的域解析或 resolver 的 comparison_context 分支), 且该分支对 NULL 常量的域判定为 Real/Binary (cast_double(col))。修复面收敛: 会话 rewrite 的 nulleq 域解析 — 对 NULL 常量采纳列类型 (Go TypeNull→ETInt) 或跳过列侧 cast。完成后 an_is_null pin (TableDual rows:0) 即绿。下轮以带插桩的完整会话重跑定位精确行, 或由会话/expr 所有者按此诊断落地。
- nulleq 双批细化 (2026-09-07 一): ranger 空范围修复 (NULL_EQ+NULL @ NOT NULL 列 → 空点集) 实现后实测无效 — 根因链回溯一步: 真正的阻塞是 coercion 层给列套了 cast(id AS DOUBLE BINARY) (NULL 常量的类型推断把列协变成 double), 使 checker 的 match_column 失配 → 条件留作 filter → 根本到不了 ranger。修复点移至 rewriter 的 NULL 操作数类型推断: NULL 是无类型常量, 应采纳列类型 (包装常量), 列不加 cast。该推断层属 tidb-expr (兄弟 expr 会话活跃区), 修复面 = 类型推断 + 域判定 + dual 触发, 为下批特性项。
- harvested FALSE 用例定谳 (2026-09-07): gorun 实测 Go 对 GROUP BY FALSE 的错误 = 1105 "Unknown column '0' in 'group statement'" (FALSE 折叠为 0 → gbyResolver 按位置处理 → 越界 1105)。Rust 对该子用例: 1105 + ONLY_FULL_GROUP_BY 消息 — 代码一致 (1105), 消息不同 (Rust 的 OFGB 文本 vs Go 的 group statement 文本)。harvested pin 的 FALSE 断言期望 1054 与实测不符, 需修正为 1105 + Go 的消息文本 (差异在消息措辞)。归 harvested/batch 所有者修 pin; go_cases.rs 的 cast_double 探针误改已还原。
