...
func (s *testTableSuite) TestStmtSummaryTable(c *C) {
	...
	).Check(testkit.Rows("select test test.t t:k 1 2 0 0 0 0 0 0 0 0 0 select * from t where a=2 \tIndexLookUp_11\troot\t100\t\n" +
		"\t├─IndexScan_9 \tcop \t100\ttable:t, index:a, range:[2,2], keep order:false, stats:pseudo\n" +
		"\t└─TableScan_10\tcop \t100\ttable:t, keep order:false, stats:pseudo"))
	...
	tk.MustQuery(`select stmt_type, schema_name, table_names, index_names, exec_count, cop_task_num, avg_total_keys,
		...
		from performance_schema.events_statements_summary_by_digest
		where digest_text like 'select * from t%'`,
	).Check(testkit.Rows("select test test.t t:k 2 4 0 0 0 0 0 0 0 0 0 select * from t where a=2 \tIndexLookUp_11\troot\t100\t\n" +
		"\t├─IndexScan_9 \tcop \t100\ttable:t, index:a, range:[2,2], keep order:false, stats:pseudo\n" +
		"\t└─TableScan_10\tcop \t100\ttable:t, keep order:false, stats:pseudo"))
	...
	tk.MustQuery(`select stmt_type, schema_name, table_names, index_names, exec_count, cop_task_num, avg_total_keys,
		...
		from performance_schema.events_statements_summary_by_digest
		where digest_text like 'select * from t%'`,
	).Check(testkit.Rows("select test test.t t:k 1 2 0 0 0 0 0 0 0 0 0 select * from t where a=2 \tIndexLookUp_11\troot\t1000\t\n" +
		"\t├─IndexScan_9 \tcop \t1000\ttable:t, index:a, range:[2,2], keep order:false, stats:pseudo\n" +
		"\t└─TableScan_10\tcop \t1000\ttable:t, keep order:false, stats:pseudo"))
	...
	tk.MustQuery(`select stmt_type, schema_name, table_names, index_names, exec_count, cop_task_num, avg_total_keys,
		...
		from performance_schema.events_statements_summary_by_digest
		where digest_text like 'select * from t%'`,
	).Check(testkit.Rows("select test test.t t:k 2 4 0 0 0 0 0 0 0 0 0 select * from t where a=2 \tIndexLookUp_11\troot\t1000\t\n" +
		"\t├─IndexScan_9 \tcop \t1000\ttable:t, index:a, range:[2,2], keep order:false, stats:pseudo\n" +
		"\t└─TableScan_10\tcop \t1000\ttable:t, keep order:false, stats:pseudo"))
