...
func (s *testTableSuite) TestSomeTables(c *C) {
	...
	tk.MustQuery("select * from information_schema.PROCESSLIST order by ID;").Check(
		testkit.Rows(
			...
			fmt.Sprintf("2 user-2 localhost <nil> Init DB 9223372036 2 %s 0 0 07-29 03:26:05.158(410090409861578752)", strings.Repeat("x", 101)),
		))
	tk.MustQuery("SHOW PROCESSLIST;").Sort().Check(
		testkit.Rows(
...
func (s *testClusterTableSuite) TestSelectClusterTable(c *C) {
	...
	tk.MustQuery("select * from `CLUSTER_PROCESSLIST`").Check(testkit.Rows("1 root 127.0.0.1 <nil> Query 9223372036 0 0 <nil> 0  :10080"))
