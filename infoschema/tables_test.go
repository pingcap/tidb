...
func (s *testTableSuite) TestStmtSummaryTable(c *C) {
	...
	tk.MustExec("create table h(a int, b int)")
	tk.MustExec("create index idx on h (a)")

	// Test hints
	tk.MustQuery("select /*+ use_index(@`sel_1` `test`.`h` `idx`), max_execution_time(10) */ * from h where a=2")
	tk.MustQuery(`select hints from information_schema.statements_summary
		where digest_text like 'select * from h%'`,
	).Check(testkit.Rows("use_index(@`sel_1` `test`.`h` `idx`), max_execution_time(10)"))
