...
func (s *testTableSuite) TestSomeTables(c *C) {
	...
	tk.MustQuery("select * from information_schema.PROCESSLIST order by ID;").Sort().Check(
		testkit.Rows(
			...
			fmt.Sprintf("3 user-3 127.0.0.1:12345 test Quit 9223372036 %s %s abc3 0 0 ", "in transaction", "check port"),
		))
	tk.MustQuery("SHOW PROCESSLIST;").Sort().Check(
		testkit.Rows(
			...
			fmt.Sprintf("3 user-3 127.0.0.1:12345 test Quit 9223372036 %s %s", "in transaction", "check port"),
		))
