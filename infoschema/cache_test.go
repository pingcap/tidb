...
func (s *testInfoCacheSuite) TestInsert(c *C) {
	...
	ic.Insert(is2)
	c.Assert(ic.GetVersion(2), NotNil)
	...
	c.Assert(ic.GetVersion(5), NotNil)
	c.Assert(ic.GetVersion(2), NotNil)
	c.Assert(ic.GetVersion(0), NotNil)
...
func (s *testInfoCacheSuite) TestGetVersion(c *C) {
	...
	c.Assert(ic.GetVersion(1), Equals, is1)
	c.Assert(ic.GetVersion(3), Equals, is3)
	c.Assert(ic.GetVersion(0), IsNil, Commentf("index == 0, but not found"))
	c.Assert(ic.GetVersion(2), IsNil, Commentf("index in the middle, but not found"))
	c.Assert(ic.GetVersion(4), IsNil, Commentf("index == length, but not found"))
}

func (s *testInfoCacheSuite) TestGet(c *C) {
	ic := infoschema.NewCache(16)
	c.Assert(ic, NotNil)
	c.Assert(ic.Get(), IsNil)

	is1 := infoschema.MockInfoSchemaWithSchemaVer(nil, 1)
	ic.Insert(is1)
	c.Assert(ic.Get(), Equals, is1)

	// newer change the newest
	is2 := infoschema.MockInfoSchemaWithSchemaVer(nil, 2)
	ic.Insert(is2)
	c.Assert(ic.Get(), Equals, is2)
