...
var (
	nullAddr = arenaAddr{math.MaxUint32, math.MaxUint32}
...
func (addr arenaAddr) store(dst []byte) {
	endian.PutUint32(dst, addr.idx)
	endian.PutUint32(dst[4:], addr.off)
}

func (addr *arenaAddr) load(src []byte) {
...
func (a *arena) alloc(size int, align bool) (arenaAddr, []byte) {
...
func (a *arena) allocInLastBlock(size int, align bool) (arenaAddr, []byte) {
	idx := len(a.blocks) - 1
...
func (a *arenaBlock) alloc(size int, align bool) (uint32, []byte) {
	offset := a.length
	if align {
...
func (a *arena) truncate(snap *arenaCheckpoint) {
...
func (l *memdbVlog) revertToCheckpoint(db *memdb, cp *arenaCheckpoint) {
	cursor := l.checkpoint()
	for !cp.isSamePosition(&cursor) {
		...
		if hdr.oldValue.isNull() {
			...
			if keptFlags == 0 {
				db.deleteNode(hdr.nodeAddr, node)
			} else {
...
func (l *memdbVlog) inspectKVInLog(db *memdb, head, tail *arenaCheckpoint, f func(Key, KeyFlags, []byte)) {
	cursor := *tail
	for !head.isSamePosition(&cursor) {
		cursorAddr := arenaAddr{idx: uint32(cursor.blocks - 1), off: uint32(cursor.offsetInBlock)}
...
func (l *memdbVlog) moveCursor(cursor *arenaCheckpoint, hdr *memdbVlogHdr) {
...
func (l *memdbVlog) canModify(cp *arenaCheckpoint, addr arenaAddr) bool {
	if cp == nil {
		return true
	}
