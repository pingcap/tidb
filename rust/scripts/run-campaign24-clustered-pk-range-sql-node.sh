...
for prerequisite in tiup cargo curl jq nc lsof pgrep ps awk sed seq grep sort tail mktemp mkfifo openssl chmod date kill perl; do
  if ! command -v "${prerequisite}" >/dev/null 2>&1; then
    echo "missing Campaign 24 prerequisite: ${prerequisite}" >&2
    exit 1
  fi
done

MYSQL_CLIENT=${C24_MYSQL_CLIENT:-mysql}
if ! command -v "${MYSQL_CLIENT}" >/dev/null 2>&1; then
  echo "C24_MYSQL_CLIENT must name an executable stock MySQL or MariaDB client" >&2
  exit 1
fi
MYSQL_PLUGIN_ARGS=()
if [[ -n "${C24_MYSQL_PLUGIN_DIR:-}" ]]; then
  if [[ ! -f "${C24_MYSQL_PLUGIN_DIR}/mysql_native_password.so" ]]; then
    echo "C24_MYSQL_PLUGIN_DIR does not contain mysql_native_password.so" >&2
    exit 1
  fi
  MYSQL_PLUGIN_ARGS=(--plugin-dir="${C24_MYSQL_PLUGIN_DIR}")
else
...
GO_TIDB_SERVER=${C24_GO_TIDB_SERVER:-}
if [[ -z "${GO_TIDB_SERVER}" || ! -x "${GO_TIDB_SERVER}" ]]; then
  echo "C24_GO_TIDB_SERVER must name an executable failpoint-enabled tidb-server" >&2
  exit 1
fi
if ! LC_ALL=C grep -a -q 'beforeCommitSecondaries' "${GO_TIDB_SERVER}"; then
  echo "C24_GO_TIDB_SERVER does not contain the required client-go commit failpoint" >&2
  exit 1
fi
TAG="campaign24-clustered-pk-range-sql-node-${$}-$(date +%s)"
PORT_OFFSET=${C24_PORT_OFFSET:-43000}
if [[ ! "${PORT_OFFSET}" =~ ^[0-9]+$ ]] || [[ "${PORT_OFFSET}" -gt 44375 ]]; then
  echo "C24_PORT_OFFSET must be an unsigned integer no greater than 44375" >&2
  exit 1
fi
...
RUNTIME_DIR=
AUTH_FILE=
AUTH_USER=campaign24
AUTH_PASSWORD=${C24_AUTH_PASSWORD:-campaign24-native-password}
PLAYGROUND_PID=
...
PROCESS_STOP_TIMEOUT=${C24_PROCESS_STOP_TIMEOUT:-30}
PHASE_TIMEOUT=${C24_PHASE_TIMEOUT:-120}
...
for port in "${PD_PORT}" "${GO_SQL_PORT}" "${TIKV_SEED_PORT}" \
  ...
  if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
    echo "refusing occupied Campaign 24 port ${port}; set C24_PORT_OFFSET" >&2
    exit 1
  fi
done
...
if [[ ! "${AUTH_HASH_HEX}" =~ ^[0-9A-F]{40}$ ]]; then
  echo "could not derive Campaign 24 native-password stage-two hash" >&2
  exit 1
fi
...
if [[ $(printf '%s\n' "${PD_ENDPOINTS}" | sed '/^$/d' | awk 'END { print NR + 0 }') != 3 \
  ...
  echo "Campaign 24 topology did not expose exactly three unique PD client/peer and TiKV service/status endpoints" >&2
  exit 1
fi
...
mysql_go <<'SQL'
DROP DATABASE IF EXISTS campaign24;
CREATE DATABASE campaign24;
CREATE TABLE campaign24.rows (
  id BIGINT PRIMARY KEY CLUSTERED,
  balance BIGINT NOT NULL
);
INSERT INTO campaign24.rows VALUES (-7, 913), (0, -2048), (42, 77);
CREATE TABLE campaign24.lock_secondary (
  id BIGINT PRIMARY KEY CLUSTERED,
  value BIGINT NOT NULL
);
INSERT INTO campaign24.lock_secondary VALUES (1, 1);
SET SESSION tidb_wait_split_region_finish = 1;
SPLIT TABLE campaign24.lock_secondary BY (1);
SQL
TABLE_ID=$(mysql_go -Nse \
  "select tidb_table_id from information_schema.tables where table_schema='campaign24' and table_name='rows'")
if [[ ! "${TABLE_ID}" =~ ^[0-9]+$ ]] || [[ "${TABLE_ID}" =~ ^0+$ ]]; then
  echo "Go TiDB did not resolve the physical table ID" >&2
  exit 1
fi
LOCK_SECONDARY_TABLE_ID=$(mysql_go -Nse \
  "select tidb_table_id from information_schema.tables where table_schema='campaign24' and table_name='lock_secondary'")
...
"${RUST_SERVER}" --path "${PD_ADDR}" --store tikv \
  ...
  --database campaign24 --table rows --table-id "${TABLE_ID}" \
...
if [[ -z "${READY_JSON}" ]] || ! printf '%s\n' "${READY_JSON}" | jq -e \
  ...
  '(.table_id | tostring) == $table_id and (.cluster_id | tostring) == $cluster_id
   and .database == "campaign24" and .table == "rows"
  ...
  echo "Rust readiness omitted Campaign 24 process identity" >&2
  printf '%s\n' "${READY_JSON}" >&2
  exit 1
fi
...
if [[ "${PERSISTENT_ADMITTED}" != true ]]; then
  echo "Campaign 24 persistent authenticated stock client was not admitted" >&2
  sed -n '1,160p' "${PERSISTENT_CLIENT_ERROR}" >&2
  exit 1
fi
...
run_exact_query_phase pre_transfer_less_than "${ADDRESS_A}" \
  "${QUERY_ID_LT}" "${REFERENCE_HEADER}" "${ROWS_ID_LT}" '[0,1]' 0 \
  "${RANGE_ID_LT}" '["TableScan"]'
run_exact_query_phase pre_transfer_less_equal "${ADDRESS_A}" \
  "${QUERY_ID_LE}" "${REFERENCE_HEADER}" "${ROWS_ID_LE}" '[0,1]' 0 \
  "${RANGE_ID_LE}" '["TableScan"]'
run_exact_query_phase pre_transfer_bounded_intersection "${ADDRESS_A}" \
  "${QUERY_BOUNDED}" "${REFERENCE_HEADER}" "${ROWS_BOUNDED}" '[0,1]' 0 \
  "${RANGE_BOUNDED}" '["TableScan"]'
run_exact_query_phase pre_transfer_minimum_point "${ADDRESS_A}" \
  "${QUERY_MIN}" "${REFERENCE_HEADER}" "" '[0,1]' 0 \
  "${RANGE_MIN}" '["TableScan"]'
run_exact_query_phase pre_transfer_maximum_point "${ADDRESS_A}" \
...
LOCK_SECONDARY_KEY=$(mysql_go -Nse \
  "SELECT tidb_encode_record_key('campaign24', 'lock_secondary', 1);")
...
(
  ...
  exec "${MYSQL_CLIENT}" --protocol=tcp -h 127.0.0.1 -P "${GO_SQL_PORT}" \
    ...
    "SET SESSION tidb_enable_async_commit = 0; SET SESSION tidb_enable_1pc = 0; BEGIN PESSIMISTIC; UPDATE campaign24.lock_secondary SET value = value + 1 WHERE id = 1; UPDATE campaign24.rows SET balance = balance + 1 WHERE id = -7; SELECT '${LOCK_MARKER}'; COMMIT;"
...
for _ in $(seq 1 100); do
  MAIN_MVCC=$(mysql_go -Nse \
    "SELECT tidb_mvcc_info(tidb_encode_record_key('campaign24', 'rows', -7));")
  HELPER_MVCC=$(mysql_go -Nse \
    "SELECT tidb_mvcc_info(tidb_encode_record_key('campaign24', 'lock_secondary', 1));")
  if printf '%s\n' "${MAIN_MVCC}" | jq -e \
...
for probe_attempt in $(seq 1 10); do
  ...
  (
    ...
    exec "${MYSQL_CLIENT}" --protocol=tcp -h 127.0.0.1 -P "${GO_SQL_PORT}" \
      ...
      'SELECT balance FROM campaign24.rows WHERE id = -7;'
...
if [[ "${LOCK_BARRIER_READY}" != true ]]; then
  echo "stock Go-MySQL probe did not block on the real campaign24.rows primary prewrite" >&2
...
BLOCK_SNAPSHOT=$(grep -F '"event":"query_snapshot"' "${RUST_LOG}" \
  ...
  | tail -1)
if [[ -z "${BLOCK_SNAPSHOT}" ]] \
  || ! range_snapshot_matches "${BLOCK_SNAPSHOT}" '["TableScan"]' \
    "${RANGE_ID_EQ}" 0 '[0,1]'; then
  echo "blocked id=-7 query did not publish its exact point range before waiting" >&2
  printf '%s\n' "${BLOCK_SNAPSHOT}" >&2
  exit 1
fi
...
if ! printf '%s\n' "${SHUTDOWN_EVENTS}" | jq -e \
  ...
  echo "Campaign 24 shutdown stages were not successful, ordered, and balanced" >&2
  printf '%s\n' "${SHUTDOWN_EVENTS}" >&2
  exit 1
fi
