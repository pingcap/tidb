...
for prerequisite in tiup cargo curl jq nc lsof pgrep ps awk sed seq grep sort tail mktemp mkfifo openssl chmod date kill perl; do
  if ! command -v "${prerequisite}" >/dev/null 2>&1; then
    echo "missing Campaign 22 prerequisite: ${prerequisite}" >&2
    exit 1
  fi
done

MYSQL_CLIENT=${C22_MYSQL_CLIENT:-mysql}
if ! command -v "${MYSQL_CLIENT}" >/dev/null 2>&1; then
  echo "C22_MYSQL_CLIENT must name an executable stock MySQL or MariaDB client" >&2
  exit 1
fi
MYSQL_PLUGIN_ARGS=()
if [[ -n "${C22_MYSQL_PLUGIN_DIR:-}" ]]; then
  if [[ ! -f "${C22_MYSQL_PLUGIN_DIR}/mysql_native_password.so" ]]; then
    echo "C22_MYSQL_PLUGIN_DIR does not contain mysql_native_password.so" >&2
    exit 1
  fi
  MYSQL_PLUGIN_ARGS=(--plugin-dir="${C22_MYSQL_PLUGIN_DIR}")
else
...
GO_TIDB_SERVER=${C22_GO_TIDB_SERVER:-}
if [[ -z "${GO_TIDB_SERVER}" || ! -x "${GO_TIDB_SERVER}" ]]; then
  echo "C22_GO_TIDB_SERVER must name an executable failpoint-enabled tidb-server" >&2
  exit 1
fi
if ! LC_ALL=C grep -a -q 'beforeCommitSecondaries' "${GO_TIDB_SERVER}"; then
  echo "C22_GO_TIDB_SERVER does not contain the required client-go commit failpoint" >&2
  exit 1
fi
TAG="campaign22-topology-churn-sql-node-${$}-$(date +%s)"
PORT_OFFSET=${C22_PORT_OFFSET:-43000}
if [[ ! "${PORT_OFFSET}" =~ ^[0-9]+$ ]] || [[ "${PORT_OFFSET}" -gt 44375 ]]; then
  echo "C22_PORT_OFFSET must be an unsigned integer no greater than 44375" >&2
  exit 1
fi
...
RUNTIME_DIR=
AUTH_FILE=
AUTH_USER=campaign22
AUTH_PASSWORD=${C22_AUTH_PASSWORD:-campaign22-native-password}
PLAYGROUND_PID=
...
PROCESS_STOP_TIMEOUT=${C22_PROCESS_STOP_TIMEOUT:-30}
PHASE_TIMEOUT=${C22_PHASE_TIMEOUT:-120}
...
merge_owned_pids() {
  {
    ...
    for pid in ${OWNED_PIDS}; do
      ...
      if [[ "${command}" == *"${TAG}"* || "${command}" == *"${TAG_DIR}"* ]]; then
        printf '%s\n' "${pid}"
      fi
    done
...
terminate_pid_group() {
  ...
  if ! wait_for_pids_until "${deadline}" "${running_pids[@]}"; then
    ...
    if ! wait_for_pids_until "${deadline}" "${running_pids[@]}"; then
      echo "Campaign 22 cleanup failed: ${label} remained alive after SIGKILL" >&2
      return 1
    fi
  fi
  for pid in "$@"; do
  ...
  if [[ "${forced}" == true ]]; then
    echo "Campaign 22 cleanup failed: ${label} required SIGKILL" >&2
    return 1
  fi
  return 0
}
...
terminate_playground_supervisor() {
  ...
  if ! wait_for_pids_until "${deadline}" "${pid}"; then
    # TiUP's interactive supervisor can retain the PID of the TiKV process that
    # this test deliberately killed. Once every exact service descendant is
    # already down, forcing only that supervisor is bounded harness cleanup,
    # not a SQL-node or cluster-service shutdown failure.
    local command
    local descendants
    command=$(ps -ww -p "${pid}" -o command= 2>/dev/null || true)
    ...
    if [[ "${PLAYGROUND_CHILD_DETACHED}" != true \
      ...
      echo "Campaign 22 cleanup failed: TiUP supervisor could not be safely contained" >&2
      return 1
    fi
    kill -KILL "${pid}" 2>/dev/null || true
    deadline=$(( $(date +%s) + PROCESS_STOP_TIMEOUT ))
    if ! wait_for_pids_until "${deadline}" "${pid}"; then
      echo "Campaign 22 cleanup failed: TiUP supervisor remained alive after bounded force" >&2
      return 1
    fi
    echo "Campaign 22 cleanup contained the stale TiUP supervisor after all services stopped" >&2
  fi
  wait "${pid}" 2>/dev/null || true
  return 0
}
...
cleanup() {
  ...
  if nc -z -w 1 127.0.0.1 "${RUST_SQL_PORT}" >/dev/null 2>&1; then
    echo "Campaign 22 cleanup failed: Rust SQL node ${RUST_SQL_ADDR} remains reachable" >&2
    cleanup_failed=true
  fi
  ...
  if [[ -n "${registered_rows}" ]] || [[ -d "${TAG_DIR}" ]]; then
    if ! tiup clean "${TAG}" --all >/dev/null 2>&1; then
      echo "Campaign 22 cleanup failed: tiup clean failed for ${TAG}" >&2
      cleanup_failed=true
    fi
  fi
  ...
  if [[ "${processes_cleaned}" != true ]]; then
    echo "Campaign 22 cleanup failed: owned process or TiUP registry row remains" >&2
    cleanup_failed=true
  fi
  ...
  for endpoint in ${PD_ENDPOINTS}; do
    if curl -sf --max-time 1 "${endpoint}/pd/api/v1/version" >/dev/null; then
      echo "Campaign 22 cleanup failed: PD endpoint ${endpoint} remains reachable" >&2
      cleanup_failed=true
    fi
  done
  for endpoint in ${PD_PEER_ENDPOINTS}; do
    ...
    if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
      echo "Campaign 22 cleanup failed: PD peer endpoint ${endpoint} remains reachable" >&2
      cleanup_failed=true
    fi
  done
  local address
  for address in ${STORE_ADDRESSES}; do
    ...
    if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
      echo "Campaign 22 cleanup failed: TiKV ${address} remains reachable" >&2
      cleanup_failed=true
    fi
  done
  for address in ${STORE_STATUS_ADDRESSES}; do
    ...
    port=$(address_port "${address}")
    if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
      echo "Campaign 22 cleanup failed: TiKV status endpoint ${address} remains reachable" >&2
      cleanup_failed=true
    fi
  done
  if nc -z -w 1 127.0.0.1 "${GO_SQL_PORT}" >/dev/null 2>&1; then
    echo "Campaign 22 cleanup failed: Go TiDB ${GO_SQL_ADDR} remains reachable" >&2
    cleanup_failed=true
  fi
  if nc -z -w 1 127.0.0.1 "${GO_STATUS_PORT}" >/dev/null 2>&1; then
    echo "Campaign 22 cleanup failed: Go TiDB status port remains reachable" >&2
    cleanup_failed=true
  fi
  if curl -sf --max-time 1 "http://${PD_ADDR}/pd/api/v1/version" >/dev/null; then
    echo "Campaign 22 cleanup failed: PD seed ${PD_ADDR} remains reachable" >&2
    cleanup_failed=true
  fi
  ...
  if [[ "${cleanup_failed}" == false ]]; then
    ...
    if [[ -e "${TAG_DIR}" || -e "${RUNTIME_DIR}" || -e "${AUTH_FILE}" ]]; then
      echo "Campaign 22 cleanup failed: owned data/auth/runtime path remains" >&2
      cleanup_failed=true
    fi
  fi
  ...
  if [[ "${cleanup_failed}" == false ]] && [[ "${original_status}" -eq 0 ]]; then
    ...
    echo "Campaign 22 cleanup proof passed: tag processes stopped, endpoints unreachable, data/auth/runtime removed"
  else
    echo "Campaign 22 retained logs: ${PLAYGROUND_LOG} ${RUST_LOG} ${MYSQL_LOG} ${RESTART_LOG}" >&2
  fi
...
query_output_is_exact() {
  ...
  [[ "${header}" == $'amount\tid' ]] || return 1
  local rows
  rows=$(tail -n +2 "${output}" | sed '/^[[:space:]]*$/d' | sort -t $'\t' -k2,2n)
  [[ "${rows}" == $'913\t-7\n-2048\t0\n77\t42' ]]
}
...
wait_for_leader_serving() {
  ...
  while [[ $(date +%s) -lt "${deadline}" ]]; do
    if leader_facts_match "${target}" "${expected_address}" "${expected_peer}" \
      && mysql_go -B -e "${QUERY}" >"${output}" 2>"${error}" \
      && query_output_is_exact "${output}" \
...
run_exact_query_phase() {
  ...
  if ! query_output_is_exact "${output}"; then
...
for port in "${PD_PORT}" "${GO_SQL_PORT}" "${TIKV_SEED_PORT}" \
  ...
  if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
    echo "refusing occupied Campaign 22 port ${port}; set C22_PORT_OFFSET" >&2
    exit 1
  fi
done
...
if [[ ! "${AUTH_HASH_HEX}" =~ ^[0-9A-F]{40}$ ]]; then
  echo "could not derive Campaign 22 native-password stage-two hash" >&2
  exit 1
fi
...
if [[ $(printf '%s\n' "${PD_ENDPOINTS}" | sed '/^$/d' | awk 'END { print NR + 0 }') != 3 \
  ...
  echo "Campaign 22 topology did not expose exactly three unique PD client/peer and TiKV service/status endpoints" >&2
  exit 1
fi
for endpoint in ${PD_ENDPOINTS}; do
...
CREATE TABLE campaign20.lock_secondary (
  id BIGINT PRIMARY KEY CLUSTERED,
  value BIGINT NOT NULL
);
INSERT INTO campaign20.lock_secondary VALUES (1, 1);
SET SESSION tidb_wait_split_region_finish = 1;
SPLIT TABLE campaign20.lock_secondary BY (1);
SQL
...
LOCK_SECONDARY_TABLE_ID=$(mysql_go -Nse \
  "select tidb_table_id from information_schema.tables where table_schema='campaign20' and table_name='lock_secondary'")
...
if [[ -z "${READY_JSON}" ]] || ! printf '%s\n' "${READY_JSON}" | jq -e \
  ...
  echo "Rust readiness omitted Campaign 22 process identity" >&2
  printf '%s\n' "${READY_JSON}" >&2
  exit 1
fi
...
if [[ "${PERSISTENT_ADMITTED}" != true ]]; then
  echo "Campaign 22 persistent authenticated stock client was not admitted" >&2
  sed -n '1,160p' "${PERSISTENT_CLIENT_ERROR}" >&2
  exit 1
fi
...
run_exact_query_phase pre_transfer_verified "${ADDRESS_A}"
...
run_exact_query_phase transferred_to_b_convergence "${ADDRESS_B}" converge
run_exact_query_phase transferred_to_b "${ADDRESS_B}"
...
run_exact_query_phase failed_over_to_c_convergence "${ADDRESS_C}" converge
run_exact_query_phase failed_over_to_c "${ADDRESS_C}"
...
run_exact_query_phase returned_to_b_convergence "${ADDRESS_B}" converge
run_exact_query_phase returned_to_b "${ADDRESS_B}"
...
LOCK_SECONDARY_KEY=$(mysql_go -Nse \
  "SELECT tidb_encode_record_key('campaign20', 'lock_secondary', 1);")
...
(
  ...
  exec "${MYSQL_CLIENT}" --protocol=tcp -h 127.0.0.1 -P "${GO_SQL_PORT}" \
    ...
    "SET SESSION tidb_enable_async_commit = 0; SET SESSION tidb_enable_1pc = 0; BEGIN PESSIMISTIC; UPDATE campaign20.lock_secondary SET value = value + 1 WHERE id = 1; UPDATE campaign20.rows SET balance = balance + 1 WHERE id = -7; SELECT '${LOCK_MARKER}'; COMMIT;"
...
for _ in $(seq 1 100); do
  MAIN_MVCC=$(mysql_go -Nse \
    "SELECT tidb_mvcc_info(tidb_encode_record_key('campaign20', 'rows', -7));")
  HELPER_MVCC=$(mysql_go -Nse \
    "SELECT tidb_mvcc_info(tidb_encode_record_key('campaign20', 'lock_secondary', 1));")
  if printf '%s\n' "${MAIN_MVCC}" | jq -e \
...
for probe_attempt in $(seq 1 10); do
  ...
  (
    ...
    exec "${MYSQL_CLIENT}" --protocol=tcp -h 127.0.0.1 -P "${GO_SQL_PORT}" \
      ...
      'SELECT balance FROM campaign20.rows WHERE id = -7;'
...
if [[ "${LOCK_BARRIER_READY}" != true ]]; then
  echo "stock Go-MySQL probe did not block on the real campaign20.rows primary prewrite" >&2
...
BLOCK_ACTIVITIES=$(grep -F '"event":"query_activity"' "${RUST_LOG}" \
  | tail -n +$((BLOCK_BEFORE_ACTIVITIES + 1)) | jq -s '.')
if ! pid_is_running "${PERSISTENT_CLIENT_PID}" \
  || [[ $(awk 'END { print NR + 0 }' "${PERSISTENT_CLIENT_OUTPUT}") -ge $((BLOCK_BEFORE_OUTPUT_LINES + 4)) ]] \
...
if [[ "${BLOCK_CLIENT_STATUS}" -eq 0 \
  || $(awk 'END { print NR + 0 }' "${PERSISTENT_CLIENT_OUTPUT}") -ge $((BLOCK_BEFORE_OUTPUT_LINES + 4)) ]]; then
...
if ! printf '%s\n' "${SHUTDOWN_EVENTS}" | jq -e \
  ...
  echo "Campaign 22 shutdown stages were not successful, ordered, and balanced" >&2
  printf '%s\n' "${SHUTDOWN_EVENTS}" >&2
  exit 1
fi
