...
TAG="realtikv-lock-recovery-${$}"
PORT_OFFSET=${LOCK_RECOVERY_PORT_OFFSET:-31000}
...
TIDB_SERVER=${LOCK_RECOVERY_TIDB_SERVER:-}
MYSQL_CLIENT=${LOCK_RECOVERY_MYSQL_CLIENT:-mysql}
...
cleanup() {
  ...
  if ! tiup clean "${TAG}" --all >/dev/null 2>&1; then
    echo "lock-recovery cleanup failed: tiup clean failed for ${TAG}" >&2
    cleanup_failed=true
  fi
  ...
  if [[ "${cleaned}" != true ]]; then
    echo "lock-recovery cleanup failed: owned process or registry row remains" >&2
    cleanup_failed=true
  fi
  ...
  for address in ${STORE_ADDRESSES}; do
    ...
    if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
      echo "lock-recovery cleanup failed: TiKV ${address} remains reachable" >&2
      cleanup_failed=true
    fi
  done
  if nc -z -w 1 127.0.0.1 "${KV_PORT}" >/dev/null 2>&1; then
    echo "lock-recovery cleanup failed: TiKV 127.0.0.1:${KV_PORT} remains reachable" >&2
    cleanup_failed=true
  fi
  if nc -z -w 1 127.0.0.1 "${DB_PORT}" >/dev/null 2>&1; then
    echo "lock-recovery cleanup failed: TiDB ${DB_ADDR} remains reachable" >&2
    cleanup_failed=true
  fi
  ...
  if [[ "${cleanup_failed}" == false ]] && [[ "${original_status}" -eq 0 ]]; then
    ...
    echo "lock-recovery retained logs: ${PLAYGROUND_LOG} ${RUST_LOG}" >&2
  fi
...
if nc -z -w 1 127.0.0.1 "${PD_PORT}" >/dev/null 2>&1 \
  ...
  echo "refusing occupied lock-recovery endpoints; set LOCK_RECOVERY_PORT_OFFSET" >&2
  exit 1
fi
...
export LOCK_RECOVERY_PD_ADDR="${PD_ADDR}"
export LOCK_RECOVERY_LOCK_TABLE_ID
LOCK_RECOVERY_LOCK_TABLE_ID=$("${MYSQL_CLIENT}" --protocol=tcp -h 127.0.0.1 -P "${DB_PORT}" -uroot -Nse \
  "select tidb_table_id from information_schema.tables where table_schema='campaign13_lock' and table_name='locked_secondary'")
export LOCK_RECOVERY_CURRENT_TS
LOCK_RECOVERY_CURRENT_TS=$("${MYSQL_CLIENT}" --protocol=tcp -h 127.0.0.1 -P "${DB_PORT}" -uroot -Nse \
  'begin; select @@tidb_current_ts; rollback')
if [[ ! "${LOCK_RECOVERY_LOCK_TABLE_ID}" =~ ^[0-9]+$ ]] || [[ ! "${LOCK_RECOVERY_CURRENT_TS}" =~ ^[0-9]+$ ]]; then
...
if [[ "${MARKER}" != *"status=committed"* ]] \
  ...
  echo "lock-recovery marker did not prove committed-primary lock recovery" >&2
  tail -180 "${RUST_LOG}" >&2
  exit 1
fi

echo "lock-recovery lock recovery passed: ${MARKER}"
