...
"${RUST_SERVER}" --path "${PD_ADDR}" --store tikv \
  ...
  --column balance:2:stored-not-null >"${RUST_LOG}" 2>&1 &
RUST_PID=$!
...
if ! QUERY_OUTPUT=$("${MYSQL_CLIENT}" --protocol=tcp -h 127.0.0.1 \
  -P "${RUST_SQL_PORT}" -uroot --connect-timeout=5 "${MYSQL_PLUGIN_ARGS[@]}" -B \
  -e 'SELECT balance AS amount, id FROM campaign20.rows' 2>"${MYSQL_LOG}"); then
...
if [[ "${QUERY_HEADER}" != $'amount\tid' ]]; then
  echo "Rust SQL node did not preserve the requested alias and projection header order" >&2
...
if [[ "${NORMALIZED_ROWS}" != $'913\t-7\n-2048\t0\n77\t42' ]]; then
  echo "Rust SQL node did not return exactly the seeded real-TiKV stored-value pairs" >&2
  printf 'expected (amount,id):\n913\\t-7\n-2048\\t0\n77\\t42\nactual:\n%s\n' "${NORMALIZED_ROWS}" >&2
  tail -200 "${RUST_LOG}" >&2
  exit 1
fi
...
if ! printf '%s\n' "${TRANSPORT_JSON}" | jq -e \
  '(.located_region_ids | type) == "array" and (.located_region_ids | length) > 0
   and (.dispatched_region_ids | type) == "array" and (.dispatched_region_ids | length) > 0
   and .batch_attempts >= 1 and .unary_attempts == 0' \
  >/dev/null; then
  echo "Rust query did not prove an actual BatchCommands-first dispatch over PD-located and dispatched regions" >&2
  printf '%s\n' "${TRANSPORT_JSON}" >&2
  exit 1
fi
