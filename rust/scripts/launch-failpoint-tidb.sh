...
# TiDB process. The live join proof needs an actual prewritten-secondary fixture, so
...
: "${MULTI_RELATION_SQL_GO_TIDB_SERVER:?MULTI_RELATION_SQL_GO_TIDB_SERVER must name the failpoint-enabled TiDB binary}"
...
exec "${MULTI_RELATION_SQL_GO_TIDB_SERVER}" "$@"
