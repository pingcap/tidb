...
MAPPINGS: list[Mapping] = [
    ...
    # Go splits the executor across sub-packages that this tree folds back into
    # `tidb-exec`/`tidb-executor`. Naming only `pkg/executor` dropped their Go
    # tests out of the DENOMINATOR entirely, which reads as coverage but is
    # silence.
    Mapping("pkg/executor/join", ["rust/crates/tidb-exec/src",
                                  "rust/crates/tidb-executor/src/hash_join.rs",
                                  "rust/crates/tidb-executor/src/join.rs"], "LAYOUT", 2),
    Mapping("pkg/executor/sortexec", ["rust/crates/tidb-exec/src/order.rs",
                                      "rust/crates/tidb-executor/src"], "LAYOUT", 2),
    Mapping("pkg/executor/aggregate", ["rust/crates/tidb-exec/src/aggregate",
                                       "rust/crates/tidb-executor/src/hash_agg.rs"], "LAYOUT", 2),
    Mapping("pkg/expression/aggregation", ["rust/crates/tidb-exec/src/aggregate",
                                           "rust/crates/tidb-expr/src"], "LAYOUT", 2),
    # Schema mutation and the catalog it writes. These packages own every bug
    # class of the form "a component accepts a shape another then discards it"
    # (AUTO_INCREMENT, PARTITION BY, TEMPORARY, partial indexes), so leaving
    # them unmapped hid exactly the tests that would have caught them.
    Mapping("pkg/ddl", ["rust/crates/tidb-executor/src/ddl",
                        "rust/crates/tidb-executor/src/ddl.rs",
                        "rust/crates/tidb-exec/src/cluster_ddl.rs",
                        "rust/crates/tidb-session/src"], "LAYOUT", 2,
            "cluster_ddl.rs module doc names pkg/ddl/create_table.go as its source"),
    Mapping("pkg/meta/autoid", ["rust/crates/tidb-exec/src/cluster_auto_id.rs",
                                "rust/crates/tidb-exec/src/reserved_row_id.rs",
                                "rust/crates/tidb-session/src/tests_auto_increment.rs"],
            "LAYOUT", 3, "cluster_auto_id.rs module doc names the Go auto-id meta keys"),
    Mapping("pkg/table/tables", ["rust/crates/tidb-executor/src",
                                 "rust/crates/tidb-session/src"], "LAYOUT", 2),
    Mapping("pkg/table", ["rust/crates/tidb-executor/src",
                          "rust/crates/tidb-session/src"], "LAYOUT", 2),
    Mapping("pkg/table/temptable", ["rust/crates/tidb-session/src",
                                    "rust/crates/tidb-executor/src"], "LAYOUT", 2),
    Mapping("pkg/infoschema", ["rust/crates/tidb-session/src/infoschema.rs",
                               "rust/crates/tidb-exec/src/cluster_catalog.rs",
                               "rust/crates/tidb-exec/src/catalog_reload.rs"], "LAYOUT", 2),
    Mapping("pkg/privilege/privileges", ["rust/crates/tidb-session/src/privilege",
                                         "rust/crates/tidb-session/src/privilege.rs",
                                         "rust/crates/tidb-session/src/tests_grants",
                                         "rust/crates/tidb-exec/src/cluster_privilege_load.rs"],
            "LAYOUT", 2, "privilege.rs module doc names pkg/privilege/privileges as its seam"),
    Mapping("pkg/planner/core/rule", ["rust/crates/tidb-planner/src"], "LAYOUT", 2),
    Mapping("pkg/store/copr", ["rust/crates/tidb-distsql/src",
                               "rust/crates/tidb-txnkv/src"], "LAYOUT", 2),
