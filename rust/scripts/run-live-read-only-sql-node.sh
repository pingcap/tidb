...
for prerequisite in tiup cargo curl jq nc pgrep ps awk sed seq grep sort; do
  if ! command -v "${prerequisite}" >/dev/null 2>&1; then
    echo "missing read-only SQL-node prerequisite: ${prerequisite}" >&2
    exit 1
  fi
done

MYSQL_CLIENT=${READ_ONLY_SQL_MYSQL_CLIENT:-mysql}
if ! command -v "${MYSQL_CLIENT}" >/dev/null 2>&1; then
  echo "READ_ONLY_SQL_MYSQL_CLIENT must name an executable stock MySQL client" >&2
  exit 1
fi
MYSQL_PLUGIN_ARGS=()
if [[ -n "${READ_ONLY_SQL_MYSQL_PLUGIN_DIR:-}" ]]; then
  if [[ ! -f "${READ_ONLY_SQL_MYSQL_PLUGIN_DIR}/mysql_native_password.so" ]]; then
    echo "READ_ONLY_SQL_MYSQL_PLUGIN_DIR does not contain mysql_native_password.so" >&2
    exit 1
  fi
  MYSQL_PLUGIN_ARGS=(--plugin-dir="${READ_ONLY_SQL_MYSQL_PLUGIN_DIR}")
else
...
PORT_OFFSET=${READ_ONLY_SQL_PORT_OFFSET:-39000}
if [[ ! "${PORT_OFFSET}" =~ ^[0-9]+$ ]] || [[ "${PORT_OFFSET}" -gt 45375 ]]; then
  echo "READ_ONLY_SQL_PORT_OFFSET must be an unsigned integer no greater than 45375" >&2
  exit 1
fi
...
cleanup() {
  ...
  if nc -z -w 1 127.0.0.1 "${RUST_SQL_PORT}" >/dev/null 2>&1; then
    echo "read-only SQL-node cleanup failed: Rust SQL node ${RUST_SQL_ADDR} remains reachable" >&2
    cleanup_failed=true
  fi
  ...
  if ! tiup clean "${TAG}" --all >/dev/null 2>&1; then
    echo "read-only SQL-node cleanup failed: tiup clean failed for ${TAG}" >&2
    cleanup_failed=true
  fi
  ...
  if [[ "${processes_cleaned}" != true ]]; then
    echo "read-only SQL-node cleanup failed: owned process or TiUP registry row remains" >&2
    cleanup_failed=true
  fi
  ...
  for address in ${STORE_ADDRESSES}; do
    ...
    if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
      echo "read-only SQL-node cleanup failed: TiKV ${address} remains reachable" >&2
      cleanup_failed=true
    fi
  done
  if nc -z -w 1 127.0.0.1 "${TIKV_SEED_PORT}" >/dev/null 2>&1; then
    echo "read-only SQL-node cleanup failed: TiKV seed 127.0.0.1:${TIKV_SEED_PORT} remains reachable" >&2
    cleanup_failed=true
  fi
  if nc -z -w 1 127.0.0.1 "${GO_SQL_PORT}" >/dev/null 2>&1; then
    echo "read-only SQL-node cleanup failed: Go TiDB ${GO_SQL_ADDR} remains reachable" >&2
    cleanup_failed=true
  fi
  if nc -z -w 1 127.0.0.1 "${GO_STATUS_PORT}" >/dev/null 2>&1; then
    echo "read-only SQL-node cleanup failed: Go TiDB status port remains reachable" >&2
    cleanup_failed=true
  fi
  if curl -sf --max-time 1 "http://${PD_ADDR}/pd/api/v1/version" >/dev/null; then
    echo "read-only SQL-node cleanup failed: PD ${PD_ADDR} remains reachable" >&2
    cleanup_failed=true
  fi
  ...
  if [[ "${cleanup_failed}" == false ]]; then
    rm -rf -- "${TAG_DIR}"
    if [[ -e "${TAG_DIR}" ]]; then
      echo "read-only SQL-node cleanup failed: owned data directory remains" >&2
      cleanup_failed=true
    fi
  fi
  if [[ "${cleanup_failed}" == false ]] && [[ "${original_status}" -eq 0 ]]; then
    ...
    echo "read-only SQL-node retained logs: ${PLAYGROUND_LOG} ${RUST_LOG} ${MYSQL_LOG}" >&2
  fi
...
cd "${RUST_ROOT}"
if [[ -z "${READ_ONLY_SQL_RUST_SERVER:-}" ]]; then
  CARGO_BUILD_JOBS=12 cargo build -j12 -p tidb-server --bin tidb-server
  RUST_SERVER="${RUST_ROOT}/target/debug/tidb-server"
else
  RUST_SERVER=${READ_ONLY_SQL_RUST_SERVER}
fi
if [[ ! -x "${RUST_SERVER}" ]]; then
  echo "read-only SQL-node Rust server is not executable: ${RUST_SERVER}" >&2
  exit 1
fi
...
for port in "${PD_PORT}" "${GO_SQL_PORT}" "${TIKV_SEED_PORT}" \
  ...
  if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
    echo "refusing occupied read-only SQL-node port ${port}; set READ_ONLY_SQL_PORT_OFFSET" >&2
    exit 1
  fi
done
...
if [[ ! "${TABLE_ID}" =~ ^[0-9]+$ ]] || [[ "${TABLE_ID}" =~ ^0+$ ]]; then
  echo "Go TiDB did not resolve the read-only SQL-node physical table ID" >&2
  exit 1
fi
...
echo "read-only SQL-node live SQL-node proof passed: stock MySQL client read [-7,0,42] from real TiKV; table_id=${TABLE_ID}; pd_cluster_id=${PD_CLUSTER_ID}; snapshot_ts=${SNAPSHOT_TS}; region_ids=${REGION_IDS}; batch_attempts=${BATCH_ATTEMPTS}; active_connections=0"
