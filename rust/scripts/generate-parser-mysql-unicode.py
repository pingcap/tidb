...
def generate_ranges(names: list[str], calls: list[str]) -> str:
...
'''
    with tempfile.TemporaryDirectory(prefix="parser-mysql-unicode-") as directory:
        ...
        return checked_output("go", "run", source.as_posix())


def generate(names: list[str]) -> str:
    body = generate_ranges(
        names,
        [
...
def generate_unicode_categories(names: list[str]) -> str:
    body = generate_ranges(
        names,
        [
            'emit("UNICODE_UPPERCASE_LETTERS", unicode.IsUpper)',
            'emit("UNICODE_LOWERCASE_LETTERS", unicode.IsLower)',
        ],
    )
...
def main() -> None:
    ...
    category_target = rust_root / "crates/tidb-mysql/src/unicode_category.rs"
    category_generated = generate_unicode_categories(names)
    if category_target.is_file() and category_target.read_text() == category_generated:
        pass
    elif args.check:
        raise SystemExit(f"generated Unicode categories are stale in {category_target}")
    else:
        category_target.write_text(category_generated)
