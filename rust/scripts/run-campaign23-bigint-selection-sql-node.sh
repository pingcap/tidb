...
MYSQL_CLIENT=${C23_MYSQL_CLIENT:-mysql}
if ! command -v "${MYSQL_CLIENT}" >/dev/null 2>&1; then
  echo "C23_MYSQL_CLIENT must name an executable stock MySQL or MariaDB client" >&2
  exit 1
fi
MYSQL_PLUGIN_ARGS=()
if [[ -n "${C23_MYSQL_PLUGIN_DIR:-}" ]]; then
  if [[ ! -f "${C23_MYSQL_PLUGIN_DIR}/mysql_native_password.so" ]]; then
    echo "C23_MYSQL_PLUGIN_DIR does not contain mysql_native_password.so" >&2
    exit 1
  fi
  MYSQL_PLUGIN_ARGS=(--plugin-dir="${C23_MYSQL_PLUGIN_DIR}")
else
...
GO_TIDB_SERVER=${C23_GO_TIDB_SERVER:-}
if [[ -z "${GO_TIDB_SERVER}" || ! -x "${GO_TIDB_SERVER}" ]]; then
  echo "C23_GO_TIDB_SERVER must name an executable failpoint-enabled tidb-server" >&2
  exit 1
fi
if ! LC_ALL=C grep -a -q 'beforeCommitSecondaries' "${GO_TIDB_SERVER}"; then
  echo "C23_GO_TIDB_SERVER does not contain the required client-go commit failpoint" >&2
  exit 1
fi
TAG="campaign23-bigint-selection-sql-node-${$}-$(date +%s)"
PORT_OFFSET=${C23_PORT_OFFSET:-43000}
if [[ ! "${PORT_OFFSET}" =~ ^[0-9]+$ ]] || [[ "${PORT_OFFSET}" -gt 44375 ]]; then
  echo "C23_PORT_OFFSET must be an unsigned integer no greater than 44375" >&2
  exit 1
fi
...
RUNTIME_DIR=
AUTH_FILE=
AUTH_USER=campaign23
AUTH_PASSWORD=${C23_AUTH_PASSWORD:-campaign23-native-password}
PLAYGROUND_PID=
...
PROCESS_STOP_TIMEOUT=${C23_PROCESS_STOP_TIMEOUT:-30}
PHASE_TIMEOUT=${C23_PHASE_TIMEOUT:-120}
...
terminate_pid_group() {
  ...
  if ! wait_for_pids_until "${deadline}" "${running_pids[@]}"; then
    ...
    if ! wait_for_pids_until "${deadline}" "${running_pids[@]}"; then
      echo "Campaign 23 cleanup failed: ${label} remained alive after SIGKILL" >&2
      return 1
    fi
  fi
  for pid in "$@"; do
  ...
  if [[ "${forced}" == true ]]; then
    echo "Campaign 23 cleanup failed: ${label} required SIGKILL" >&2
    return 1
  fi
  return 0
}
...
terminate_playground_supervisor() {
  ...
  if ! wait_for_pids_until "${deadline}" "${pid}"; then
    ...
    if [[ "${PLAYGROUND_CHILD_DETACHED}" != true \
      ...
      echo "Campaign 23 cleanup failed: TiUP supervisor could not be safely contained" >&2
      return 1
    fi
    kill -KILL "${pid}" 2>/dev/null || true
    deadline=$(( $(date +%s) + PROCESS_STOP_TIMEOUT ))
    if ! wait_for_pids_until "${deadline}" "${pid}"; then
      echo "Campaign 23 cleanup failed: TiUP supervisor remained alive after bounded force" >&2
      return 1
    fi
    echo "Campaign 23 cleanup contained the stale TiUP supervisor after all services stopped" >&2
  fi
  wait "${pid}" 2>/dev/null || true
  return 0
}
...
cleanup() {
  ...
  if nc -z -w 1 127.0.0.1 "${RUST_SQL_PORT}" >/dev/null 2>&1; then
    echo "Campaign 23 cleanup failed: Rust SQL node ${RUST_SQL_ADDR} remains reachable" >&2
    cleanup_failed=true
  fi
  ...
  if [[ -n "${registered_rows}" ]] || [[ -d "${TAG_DIR}" ]]; then
    if ! tiup clean "${TAG}" --all >/dev/null 2>&1; then
      echo "Campaign 23 cleanup failed: tiup clean failed for ${TAG}" >&2
      cleanup_failed=true
    fi
  fi
  ...
  if [[ "${processes_cleaned}" != true ]]; then
    echo "Campaign 23 cleanup failed: owned process or TiUP registry row remains" >&2
    cleanup_failed=true
  fi
  ...
  for endpoint in ${PD_ENDPOINTS}; do
    if curl -sf --max-time 1 "${endpoint}/pd/api/v1/version" >/dev/null; then
      echo "Campaign 23 cleanup failed: PD endpoint ${endpoint} remains reachable" >&2
      cleanup_failed=true
    fi
  done
  for endpoint in ${PD_PEER_ENDPOINTS}; do
    ...
    if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
      echo "Campaign 23 cleanup failed: PD peer endpoint ${endpoint} remains reachable" >&2
      cleanup_failed=true
    fi
  done
  local address
  for address in ${STORE_ADDRESSES}; do
    ...
    if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
      echo "Campaign 23 cleanup failed: TiKV ${address} remains reachable" >&2
      cleanup_failed=true
    fi
  done
  for address in ${STORE_STATUS_ADDRESSES}; do
    ...
    port=$(address_port "${address}")
    if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
      echo "Campaign 23 cleanup failed: TiKV status endpoint ${address} remains reachable" >&2
      cleanup_failed=true
    fi
  done
  if nc -z -w 1 127.0.0.1 "${GO_SQL_PORT}" >/dev/null 2>&1; then
    echo "Campaign 23 cleanup failed: Go TiDB ${GO_SQL_ADDR} remains reachable" >&2
    cleanup_failed=true
  fi
  if nc -z -w 1 127.0.0.1 "${GO_STATUS_PORT}" >/dev/null 2>&1; then
    echo "Campaign 23 cleanup failed: Go TiDB status port remains reachable" >&2
    cleanup_failed=true
  fi
  if curl -sf --max-time 1 "http://${PD_ADDR}/pd/api/v1/version" >/dev/null; then
    echo "Campaign 23 cleanup failed: PD seed ${PD_ADDR} remains reachable" >&2
    cleanup_failed=true
  fi
  ...
  if [[ "${cleanup_failed}" == false ]]; then
    ...
    if [[ -e "${TAG_DIR}" || -e "${RUNTIME_DIR}" || -e "${AUTH_FILE}" ]]; then
      echo "Campaign 23 cleanup failed: owned data/auth/runtime path remains" >&2
      cleanup_failed=true
    fi
  fi
  ...
  if [[ "${cleanup_failed}" == false ]] && [[ "${original_status}" -eq 0 ]]; then
    ...
    echo "Campaign 23 cleanup proof passed: tag processes stopped, endpoints unreachable, data/auth/runtime removed"
  else
    echo "Campaign 23 retained logs: ${PLAYGROUND_LOG} ${RUST_LOG} ${MYSQL_LOG} ${RESTART_LOG}" >&2
  fi
...
for port in "${PD_PORT}" "${GO_SQL_PORT}" "${TIKV_SEED_PORT}" \
  ...
  if nc -z -w 1 127.0.0.1 "${port}" >/dev/null 2>&1; then
    echo "refusing occupied Campaign 23 port ${port}; set C23_PORT_OFFSET" >&2
    exit 1
  fi
done
...
if [[ ! "${AUTH_HASH_HEX}" =~ ^[0-9A-F]{40}$ ]]; then
  echo "could not derive Campaign 23 native-password stage-two hash" >&2
  exit 1
fi
...
if [[ $(printf '%s\n' "${PD_ENDPOINTS}" | sed '/^$/d' | awk 'END { print NR + 0 }') != 3 \
  ...
  echo "Campaign 23 topology did not expose exactly three unique PD client/peer and TiKV service/status endpoints" >&2
  exit 1
fi
for endpoint in ${PD_ENDPOINTS}; do
...
mysql_go <<'SQL'
DROP DATABASE IF EXISTS campaign23;
CREATE DATABASE campaign23;
CREATE TABLE campaign23.rows (
  id BIGINT PRIMARY KEY CLUSTERED,
  balance BIGINT NOT NULL
);
INSERT INTO campaign23.rows VALUES (-7, 913), (0, -2048), (42, 77);
CREATE TABLE campaign23.lock_secondary (
  id BIGINT PRIMARY KEY CLUSTERED,
  value BIGINT NOT NULL
);
INSERT INTO campaign23.lock_secondary VALUES (1, 1);
SET SESSION tidb_wait_split_region_finish = 1;
SPLIT TABLE campaign23.lock_secondary BY (1);
SQL
TABLE_ID=$(mysql_go -Nse \
  "select tidb_table_id from information_schema.tables where table_schema='campaign23' and table_name='rows'")
if [[ ! "${TABLE_ID}" =~ ^[0-9]+$ ]] || [[ "${TABLE_ID}" =~ ^0+$ ]]; then
  echo "Go TiDB did not resolve the physical table ID" >&2
  exit 1
fi
LOCK_SECONDARY_TABLE_ID=$(mysql_go -Nse \
  "select tidb_table_id from information_schema.tables where table_schema='campaign23' and table_name='lock_secondary'")
...
"${RUST_SERVER}" --path "${PD_ADDR}" --store tikv \
  ...
  --database campaign23 --table rows --table-id "${TABLE_ID}" \
...
if [[ -z "${READY_JSON}" ]] || ! printf '%s\n' "${READY_JSON}" | jq -e \
  ...
  '(.table_id | tostring) == $table_id and (.cluster_id | tostring) == $cluster_id
   and .database == "campaign23" and .table == "rows"
  ...
  echo "Rust readiness omitted Campaign 23 process identity" >&2
  printf '%s\n' "${READY_JSON}" >&2
  exit 1
fi
...
if [[ "${PERSISTENT_ADMITTED}" != true ]]; then
  echo "Campaign 23 persistent authenticated stock client was not admitted" >&2
  sed -n '1,160p' "${PERSISTENT_CLIENT_ERROR}" >&2
  exit 1
fi
...
run_exact_query_phase pre_transfer_not_equal "${ADDRESS_A}" \
  "${QUERY_ID_NE}" "${REFERENCE_HEADER}" "${ROWS_ID_NE}" '[0,1]' 1
run_exact_query_phase pre_transfer_reversed_operand "${ADDRESS_A}" \
  "${QUERY_REVERSED}" "${REFERENCE_HEADER}" "${ROWS_REVERSED}" '[0,1]' 1
run_exact_query_phase pre_transfer_empty_result "${ADDRESS_A}" \
  "${QUERY_EMPTY}" "${REFERENCE_HEADER}" "" '[0,1]' 1
run_exact_query_phase pre_transfer_and_conjunction "${ADDRESS_A}" \
...
run_exact_query_phase transferred_to_b_convergence "${ADDRESS_B}" \
...
run_exact_query_phase failed_over_to_c_convergence "${ADDRESS_C}" \
...
run_exact_query_phase returned_to_b_convergence "${ADDRESS_B}" \
...
LOCK_SECONDARY_KEY=$(mysql_go -Nse \
  "SELECT tidb_encode_record_key('campaign23', 'lock_secondary', 1);")
...
(
  ...
  exec "${MYSQL_CLIENT}" --protocol=tcp -h 127.0.0.1 -P "${GO_SQL_PORT}" \
    ...
    "SET SESSION tidb_enable_async_commit = 0; SET SESSION tidb_enable_1pc = 0; BEGIN PESSIMISTIC; UPDATE campaign23.lock_secondary SET value = value + 1 WHERE id = 1; UPDATE campaign23.rows SET balance = balance + 1 WHERE id = -7; SELECT '${LOCK_MARKER}'; COMMIT;"
...
for _ in $(seq 1 100); do
  MAIN_MVCC=$(mysql_go -Nse \
    "SELECT tidb_mvcc_info(tidb_encode_record_key('campaign23', 'rows', -7));")
  HELPER_MVCC=$(mysql_go -Nse \
    "SELECT tidb_mvcc_info(tidb_encode_record_key('campaign23', 'lock_secondary', 1));")
  if printf '%s\n' "${MAIN_MVCC}" | jq -e \
...
for probe_attempt in $(seq 1 10); do
  ...
  (
    ...
    exec "${MYSQL_CLIENT}" --protocol=tcp -h 127.0.0.1 -P "${GO_SQL_PORT}" \
      ...
      'SELECT balance FROM campaign23.rows WHERE id = -7;'
...
if [[ "${LOCK_BARRIER_READY}" != true ]]; then
  echo "stock Go-MySQL probe did not block on the real campaign23.rows primary prewrite" >&2
...
BLOCK_SNAPSHOT=$(grep -F '"event":"query_snapshot"' "${RUST_LOG}" \
  ...
  | jq -c --arg connection "${PERSISTENT_CONNECTION_ID}" --arg query "${BLOCK_QUERY_ID}" \
    'select((.connection_id | tostring) == $connection and (.query_id | tostring) == $query)' \
  | tail -1)
if [[ -z "${BLOCK_SNAPSHOT}" ]] || ! printf '%s\n' "${BLOCK_SNAPSHOT}" | jq -e \
  '.executor_kinds == ["TableScan", "Selection"]
   and .predicate_count == 1 and .output_offsets == [0, 1]' >/dev/null; then
  echo "blocked id=-7 query did not publish its Selection plan before waiting" >&2
  printf '%s\n' "${BLOCK_SNAPSHOT}" >&2
  exit 1
fi
...
if ! printf '%s\n' "${SHUTDOWN_EVENTS}" | jq -e \
  ...
  echo "Campaign 23 shutdown stages were not successful, ordered, and balanced" >&2
  printf '%s\n' "${SHUTDOWN_EVENTS}" >&2
  exit 1
fi
