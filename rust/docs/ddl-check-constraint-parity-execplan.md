...
Revision note (2026-08-31): read all four pinned `pkg/ddl/session` artifacts and transcreated the package as a distinct crate rather than reusing the behaviorally broader `pkg/session/syssession` carrier. The exact transaction wrapper, internal request source, drain/close/metric path, test failpoint handshake, checkout/reset/registry ordering, and destroy type switch are represented; all four upstream pool tests now execute and the old ignored gap-only test file was removed.

Revision note (2026-08-31): read and transcreated the complete generated three-artifact `pkg/ddl/mock` package using the same native recorder contract as the completed sqlexec mock package. The source-owned SchemaLoader and system-table Manager traits now have concrete Rust implementations; the owner scheduler performs Go's retry-until-success-or-cancel reload before processing jobs, the full unsynced tracker is present, and the two former ignored scheduler gaps now execute as tests.

Revision note (2026-08-31): the complete six-artifact `pkg/dxf/operator` prerequisite is now transcreated before `pkg/ddl/testutil`. Native unbuffered channels and threads preserve the package's compose, cancellation, panic recovery, tuning, pipeline lifecycle, and wrapper behavior; its sole upstream pipeline test executes both the successful `hit` collection and injected-error branches. The remaining DDL testutil helpers are not claimed until their concrete session/domain/meta owners can be completed atomically.
