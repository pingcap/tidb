...
1. **F13** — row format v2 wrote the decimal's natural precision/scale instead
   of the column's declared `(flen, decimal)`. `DECIMAL(10,4)` holding `11.99`
   is 7 bytes in Go and was 5 in Rust. Every decimal column in every v2 row.
   Fixed.
...
### F13 — rank 1 — row format v2 wrote the decimal's *natural* precision/scale, not the column's declared shape — FIXED
...
**Fix.** `Decimal` carries the declared `(flen, decimal)` pair beside the value
(`crates/tidb-datatype/src/decimal.rs`), which is where Go keeps it —
`Datum.length`/`Datum.decimal`, never inside `MyDecimal`. It is stamped in
`convert_to_decimal_target` (`crates/tidb-datatype/src/datum_convert.rs`), the
port of `convertToMysqlDecimal`, and read back by the row-v2 encoder through
`Decimal::storage_shape`, whose `(0, 0)` for an unstamped value is exactly the
unset `Datum.length` that `EncodeDecimal` resolves to `PrecisionAndFrac`.
`Decimal::new_with_storage` is the one construction point, so every parse,
arithmetic and rounding result resets the pair to unstamped — a computed
expression result keeps its natural shape, as in Go.

`PrecisionAndFrac` deliberately still reports the value's own shape: a payload
written at `(10, 4)` answers `(4, 2)`, confirmed against Go. That is the whole
reason a round trip is blind here.

**Fixture status.** Covered by
`crates/tidb-codec/tests/decimal_column_shape_go_vectors.rs` against
`difftests/transaction-tests/fixtures/decimal_column_rows.hex`, generated by
`generate_decimal_column_rows.go` beside it. A mutation probe that neuters the
stamping in place fails exactly the three declared-shape vectors and leaves all
119 other `tidb-codec` tests — including every self-round-trip row test — and
all 277 `tidb-datatype` tests green. That pair is the measurement of what the
round trips could not see.
