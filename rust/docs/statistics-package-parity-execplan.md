...
- [ ] Complete pinned `pkg/statistics/handle/globalstats`; production behavior and every original test/support/benchmark artifact are now mapped to executable Rust receipts, and the focused WIP gates pass. The package-level Ready gate remains open, so this is not yet an atomic completion claim.
