...
Revision note (2026-08-30): reconciled pinned `globalstats.TestDDLPartition4GlobalStats`. The first source run failed at DDL admission because the cluster route refused `ALTER TABLE ... TRUNCATE PARTITION`; after the shared partition executor was wired into cluster DDL, the next run reached the statistics assertion but retained global count 15 instead of 11. Pinned Go's DDL subscriber creates empty metadata rows for replacement partition IDs, version-marks retired rows for delayed GC, and applies `(Count=count, Delta=-count)` to the logical row. Rust now performs that same follow-up statistics transaction and republishes the affected cache entries. The source sequence passes with fresh IDs for p2/p4, immediate logical count 11, seven visible metadata rows after re-analysis, and final count 11. This closes one original globalstats test behavior but does not complete the remaining package inventory.
