...
Revision note (2026-08-29): added the shared statistics core for pinned `analyzeIndexProcessor` and `AnalyzeIndexExec.buildStatsFromResult`. It consumes ordered encoded index-column slices, excludes one-column NULLs into `NullCount`, retains NULL-bearing composite tuples, inserts every progressively longer prefix into the default 5-by-2048 CMS, tracks the full tuple in FM/TopN, removes selected TopN values from the histogram, and applies stats-v2 index standardization. The logical global-index storage scan, independent-save policy, and planner routing remain outstanding, so this is explicit seed evidence rather than an executor or statistics-package completion claim.
