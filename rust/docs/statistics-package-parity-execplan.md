...
- [ ] Complete pinned persisted ANALYZE options across table and partition targets; cluster static mode now resolves Go-ordered physical targets, analyzes each requested partition by physical ID, and saves logical plus partition option rows after successful execution. The in-process path persists and inherits table/static-partition rows and honors the live persistence gate. Cluster dynamic global-statistics merge and partition FM-sketch storage remain open.
