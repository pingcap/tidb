...
- Observation: LIST COLUMNS inherited scalar LIST's unconditional DEFAULT retention and compared tuples through a collation-blind encoded key.
  Evidence: pinned `locateColumnPartitionsByCondition` omits DEFAULT for a complete one-column point with an explicit owner, but retains it for partial multi-column points and ranges; `ForListColumnPruning.genKey` uses the column type. Rust now follows those ownership and collation rules for ranger tuples. The broader per-column CNF/DNF location-helper flow remains the next LIST COLUMNS dependency slice.
