...
- [ ] Complete `resolveOptimizeHint`'s ordinary index-hint dependency (completed: table-syntax `USE`/`FORCE`/`IGNORE INDEX` now filters the ordinary planner's public paths, preserves forced index identities, handles PRIMARY, empty USE, ignored paths, undetermined-path fallback, scan scope, and 1176; remaining: comment-style hint matching/warnings, per-partition filtering/re-enumeration, order flags, lookup-pushdown metadata, then delete the executor-only fast-path resolver).
...
- Observation: Rust resolved ordinary index hints only inside the executor's fast-point path, while the ordinary planner enumerated every public path.
  Evidence: pinned Go runs both fresh and fast-plan decisions through `getPossibleAccessPaths`; Rust's `index_hints.rs` guarded point/batch-point candidates, but `PlanBuilder::build_data_source` called the explicitly hint-free enumerator. Table-syntax hint filtering now owns the ordinary candidate list; the executor resolver remains only until comment hints and fast-plan consumers share this planner owner.
