...
- [ ] Complete `pkg/planner/core/rule/util` as the first atomic package (all inventoried behavior is now owned: centralized expression replacement, column-set tests, nullable-key max-one-row behavior, metadata-based unique-index key derivation, flag hook, iterative key-info portal, and both predicate-simplification hook signatures including validity-filter forwarding; removed duplicate CTE/projection/index-key bodies; remaining: Ready validation and the package receipt).
...
Work is in progress. Static partition planning is integrated and pushed. The nested `rule/util` package has no remaining known behavior or inventory gap, but its atomic completion claim and the parent `rule` package remain withheld until the Ready validation receipt is recorded.
