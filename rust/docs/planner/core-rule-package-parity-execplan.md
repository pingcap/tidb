...
- [ ] Complete `resolveOptimizeHint`'s ordinary index-hint dependency (completed: table-syntax and comment-style `USE`/`FORCE`/`IGNORE INDEX` share ordinary public-path filtering; query-block/database/alias matching, unique prefix resolution, 1176/1815 warnings, partition-scoped re-resolution, global-index removal, PRIMARY, empty USE, ignored/undetermined fallbacks, forced identities, and ORDER/NO_ORDER candidate admission are preserved; pinned `indexIsAvailableByHints` now owns fast point/batch-point admission and the broader 685-line executor resolver is deleted; lookup-pushdown now preserves positive/negative conflicts, all ten pinned support gates and warning texts, hint-only/affinity-force/force policy, keep-order refusal, metadata/session propagation, and execution through the ordinary physical reader; remaining: Go's `buildPushDownIndexLookUpPlan` flattened physical-tree representation, table-side zeroed statistics, and its original planbuilder test must be implemented before this dependency is complete).
...
- Observation: lookup-pushdown eligibility is statement state, not an executor-local hint boolean.
  Evidence: pinned `checkIndexLookUpPushDownSupported` reads table encoding/global/temp/cache/MV metadata followed by isolation, replica-read, stale/snapshot, and max-keys session facts in a fixed warning order; `checkAutoForceIndexLookUpPushDown` additionally reads the three-valued policy and table affinity. Rust now snapshots those facts once at the statement boundary and propagates the resulting Go origin (`Hint` or `SysVar`) through `CopTask` to the ordinary physical reader. The remaining mismatch is representational and explain-visible: Rust has not yet built Go's flattened `LocalIndexLookUp` physical subtree or zeroed the retained table-side statistics.
