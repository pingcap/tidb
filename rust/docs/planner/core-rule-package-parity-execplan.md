...
- [ ] Complete the root pinned `pkg/statistics/handle/usage` package. Its full inventory is `BUILD.bazel`; production `index_usage.go`, `predicate_column.go`, and `session_stats_collect.go`; and support/tests `export_test.go`, `index_usage_integration_test.go`, `predicate_column_test.go`, and `session_stats_collect_test.go`. Implemented so far: the session collector/list, table-delta and column-usage maps, sweep/reset/deferred failure merge, earliest `InitTime`, sorted 2,048-row column batches and twelve-hour throttle, sorted 100,000-table delta batches, locked-table/partition/global rules, positive/negative `stats_meta` writes, post-commit historical-meta recording, DML row/count collection, transaction commit/rollback/retry/savepoint lifecycle, the node-global index collector wrapper, Domain-owned scheduling (unconditional 30-minute index GC; positive-lease `100*lease+jitter` column dumps and `20*lease+jitter` delta dumps; forced delta flush at shutdown), statement/session collector lifecycle, ordinary plus prepared PointGet/BatchPointGet close-time reporting using physical Get/BatchGet RPC counts (including retries), and ordinary TableReader/IndexReader/IndexLookup close-time reporting from TiKV execution summaries. The reporter preserves exact pseudo-stat fallback, logical/physical table IDs, clustered-handle resolution, and statement query deduplication. Completion remains withheld until IndexMerge partial-path reporting and the original integration matrix are wired, then the root-package receipt passes Ready validation.
