...
- [x] (2026-09-12) Tried and set aside: Go-style spinning before parking in the worker pool (`stealWork`/`wakep`). The first version skipped the wake whenever a worker was spinning, and a spinner that took one task left a second queued task unwoken (Go's `resetspinning` wakes another worker); Q18 hung two runs in six. A corrected version is queued for its own A/B; the first showed no gain on Q9 or Q13.
