...
fn is_show_stats_histograms(sql: &str) -> bool {
    let mut words = sql.trim_start().split_ascii_whitespace();
    matches!(
        (words.next(), words.next()),
        (Some(show), Some(stats_histograms))
            if show.eq_ignore_ascii_case("SHOW")
                && stats_histograms.eq_ignore_ascii_case("STATS_HISTOGRAMS")
    )
}

#[test]
fn show_stats_histograms_static_go_rows_match() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_show_stats_histograms(&record.input.sql)
        })
        .collect();
