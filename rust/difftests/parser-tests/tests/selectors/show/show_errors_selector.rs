...
fn is_show_errors(sql: &str) -> bool {
    let mut words = sql.trim_start().split_ascii_whitespace();
    matches!(
        (words.next(), words.next()),
        (Some(show), Some(errors))
            if show.eq_ignore_ascii_case("SHOW")
                && errors.trim_end_matches(';').eq_ignore_ascii_case("ERRORS")
    )
}

#[test]
fn show_errors_static_go_rows_match() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_show_errors(&record.input.sql)
        })
        .collect();
