...
fn is_show_stats_buckets(sql: &str) -> bool {
    let mut words = sql.trim_start().split_ascii_whitespace();
    matches!(
        (words.next(), words.next()),
        (Some(show), Some(stats_buckets))
            if show.eq_ignore_ascii_case("show")
                && stats_buckets.trim_end_matches(';').eq_ignore_ascii_case("stats_buckets")
    )
}

#[test]
fn show_stats_buckets_static_go_rows_match() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_show_stats_buckets(&record.input.sql)
        })
        .collect();
