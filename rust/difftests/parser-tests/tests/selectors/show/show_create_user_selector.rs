...
fn is_show_create_user(sql: &str) -> bool {
    let mut words = sql.trim_start().split_ascii_whitespace();
    matches!(
        (words.next(), words.next(), words.next()),
        (Some(show), Some(create), Some(user))
            if show.eq_ignore_ascii_case("SHOW")
                && create.eq_ignore_ascii_case("CREATE")
                && user.eq_ignore_ascii_case("USER")
    )
}

#[test]
fn show_create_user_static_go_rows_match() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_show_create_user(&record.input.sql)
        })
        .collect();
