...
fn is_grant_identified_by(sql: &str) -> bool {
    let upper = sql.trim_start().to_ascii_uppercase();
    upper.starts_with("GRANT ") && upper.contains(" IDENTIFIED BY ")
}

#[test]
fn grant_identified_by_static_go_rows_match() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_grant_identified_by(&record.input.sql)
        })
        .collect();
