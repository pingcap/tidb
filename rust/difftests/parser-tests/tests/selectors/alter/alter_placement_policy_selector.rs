...
fn is_alter_placement_policy(sql: &str) -> bool {
    let mut words = sql.trim_start().split_ascii_whitespace();
    matches!(
        (words.next(), words.next(), words.next()),
        (Some(alter), Some(placement), Some(policy))
...
#[test]
fn alter_placement_policy_static_go_rows_match() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_alter_placement_policy(&record.input.sql)
        })
        .collect();
