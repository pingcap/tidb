...
fn is_grouped_literal_default(sql: &str) -> bool {
    let normalized = sql.to_ascii_lowercase();
    normalized.starts_with("alter table ")
        && normalized.contains(" add column(")
        && normalized.contains(" default '")
        && (normalized.contains(" tinyint ") || normalized.contains(" datetime "))
}

#[test]
fn alter_add_column_literal_defaults_match_go() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_grouped_literal_default(&record.input.sql)
        })
        .collect();
