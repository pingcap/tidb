...
fn is_exchange_partition(sql: &str) -> bool {
    let Some(action) = after_table_name(sql) else {
        return false;
    };
    let prefix = "exchange partition";
    action
        .get(..prefix.len())
        .is_some_and(|value| value.eq_ignore_ascii_case(prefix))
        && action
...
#[test]
fn alter_exchange_partition_lexical_one_statement_matches_go() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_exchange_partition(&record.input.sql)
        })
        .collect();
    assert_eq!(selected.len(), 122, "source-backed selector drifted");
