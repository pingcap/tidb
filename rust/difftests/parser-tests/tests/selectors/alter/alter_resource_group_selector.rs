...
/// Select Go's `ALTER RESOURCE GROUP` production without conflating it with
/// resource-group CREATE/DROP or generic ALTER TABLE statements.
fn is_alter_resource_group(sql: &str) -> bool {
    let mut words = sql.trim_start().split_ascii_whitespace();
    matches!(
        (words.next(), words.next(), words.next()),
        (Some(alter), Some(resource), Some(group))
...
#[test]
fn alter_resource_group_static_go_rows_match() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_alter_resource_group(&record.input.sql)
        })
        .collect();
