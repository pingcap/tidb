...
/// Returns the first ALTER TABLE action without classifying quoted table paths.
...
/// Exact table-level CACHE/NOCACHE actions. The word boundary keeps them
/// separate from unrelated longer identifiers.
fn is_table_cache_action(sql: &str) -> bool {
    after_table_name(sql).is_some_and(|action| {
        ["cache", "nocache"].iter().any(|keyword| {
            action
                .get(..keyword.len())
...
#[test]
fn alter_table_cache_static_go_rows_match() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_table_cache_action(&record.input.sql)
        })
        .collect();
