...
fn is_stats_lock(sql: &str) -> bool {
    let mut words = sql.trim_start().split_ascii_whitespace();
    matches!(
        (words.next(), words.next()),
        (Some(operation), Some(stats))
            if (operation.eq_ignore_ascii_case("LOCK") || operation.eq_ignore_ascii_case("UNLOCK"))
                && stats.eq_ignore_ascii_case("STATS")
    )
}

#[test]
fn lock_and_unlock_stats_static_go_rows_match() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_stats_lock(&record.input.sql)
        })
        .collect();
