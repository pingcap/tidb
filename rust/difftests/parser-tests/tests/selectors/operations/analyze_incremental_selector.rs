...
fn is_analyze_incremental(sql: &str) -> bool {
    let mut words = sql.trim_start().split_ascii_whitespace();
    matches!(
        (words.next(), words.next()),
        (Some(analyze), Some(incremental))
            if analyze.eq_ignore_ascii_case("analyze")
                && incremental.eq_ignore_ascii_case("incremental")
    )
}

#[test]
fn analyze_incremental_static_go_rows_match() {
    ...
    let selected: Vec<_> = records
        ...
        .filter(|record| {
            record.outcome == GoOutcome::Accepted
                ...
                && is_analyze_incremental(&record.input.sql)
        })
        .collect();
