...
const SOURCE_ROW: (&str, usize) = ("tests/integrationtest/t/cte.test", 328);

#[test]
fn cte_scalar_union_source_row_matches_go() {
    ...
    let statement =
        tidb_parser::parse(&record.input.sql).expect("parse scalar subquery with UNION source row");
    assert_eq!(
