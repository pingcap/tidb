...
const SOURCE_ROW: (&str, usize) = ("tests/integrationtest/t/expression/issues.test", 1900);

#[test]
fn exists_set_operation_source_row_matches_go() {
    ...
    let statement =
        tidb_parser::parse(&record.input.sql).expect("parse EXISTS set-operation source row");
    assert_eq!(
