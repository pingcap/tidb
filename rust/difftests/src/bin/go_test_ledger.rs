...
//! test entry point, each reachable testify suite parent/method invocation,
//! AST-discovered fixture/file access (including unresolved
...
/// invalid test-like declarations. The ledger promotes valid runners and each
/// reachable testify parent/method invocation to `go_test` obligations.
#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd)]
struct GoTestDeclaration {
    ...
    name: String,
    category: String,
    actionable_test_obligation: bool,
...
/// A byte-for-byte fingerprint of one checked-in artifact that participates in
/// an original test obligation. The entry-point inventory catches names and
/// signatures, while this complementary inventory catches assertion-body,
/// fixture, runner, and expected-result edits that preserve those anchors.
#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd)]
...
impl GoTestDeclaration {
    fn is_test_obligation(&self) -> bool {
        self.actionable_test_obligation
            && matches!(
                self.category.as_str(),
                "Test" | "Benchmark" | "Fuzz" | "Example" | "TestMain" | "TestSuiteMethod"
            )
    }

    fn obligation_names(&self) -> Vec<String> {
        if self.category == "TestSuiteMethod" {
            return self
                .suite_parents
                .iter()
                .map(|parent| format!("{parent}/{}", self.name))
                .collect();
        }
        vec![self.name.clone()]
    }
...
fn render_go_test_declaration_inventory(declarations: &[GoTestDeclaration]) -> String {
    ...
    text.push_str("# source_path\tsource_line\tsource_column\treceiver\tfunction_name\tcategory\tactionable_test_obligation\tsuite_parents\n");
    for declaration in declarations {
        text.push_str(&format!(
            ...
            declaration.actionable_test_obligation,
            if declaration.suite_parents.is_empty() {
                "-".to_owned()
            } else {
                declaration.suite_parents.join(",")
            },
        ));
    }
    text
...
fn parse_go_test_declaration_inventory(
    ...
    for (index, line) in source.lines().enumerate() {
        ...
        let actionable_test_obligation = fields[6].parse::<bool>().map_err(|error| {
            format!(
                "{}:{}: actionable_test_obligation must be true or false, got {:?}: {error}",
                path.display(),
                index + 1,
                fields[6]
            )
        ...
        let suite_parents = if fields[7] == "-" {
            ...
            {
                return Err(format!(
                    "{}:{}: suite parents must be non-empty, unique, and sorted",
                    path.display(),
                    index + 1
                ));
            }
        ...
        if (fields[5] == "TestSuiteMethod") != !suite_parents.is_empty() {
            return Err(format!(
                "{}:{}: only TestSuiteMethod declarations may name suite parents",
                path.display(),
                index + 1
            ));
        }
...
fn test_artifacts(entries: &BTreeSet<Entry>) -> BTreeSet<&str> {
    entries.iter().map(|entry| entry.path.as_str()).collect()
}

fn collect_go_test_artifacts(
    root: &Path,
    entries: &BTreeSet<Entry>,
) -> Result<Vec<GoTestArtifact>, String> {
    test_artifacts(entries)
        .into_iter()
        .map(|path| {
            let bytes = fs::read(root.join(path))
                .map_err(|error| format!("read test artifact {path}: {error}"))?;
            Ok(GoTestArtifact {
                path: path.to_owned(),
                sha256: sha256(&bytes),
            })
...
fn render_go_test_artifact_inventory(artifacts: &[GoTestArtifact]) -> String {
    let mut text =
        String::from("# Generated by cargo run -p difftest --bin go_test_ledger -- --write.\n");
    text.push_str("# artifact_path\tsha256\n");
    for artifact in artifacts {
...
fn parse_go_test_artifact_inventory(
    path: &Path,
    source: &str,
) -> Result<Vec<GoTestArtifact>, String> {
    let mut artifacts = Vec::new();
    ...
    for (index, line) in source.lines().enumerate() {
        ...
        {
            return Err(format!(
                "{}:{}: artifact path and lowercase SHA-256 must be present",
                path.display(),
                index + 1
            ));
        }
        let artifact = GoTestArtifact {
            path: fields[0].to_owned(),
            sha256: fields[1].to_owned(),
        };
        if !seen_paths.insert(artifact.path.clone()) {
            return Err(format!(
                "{}:{}: test artifact record must be unique",
                path.display(),
                index + 1
            ));
        }
    ...
    {
        return Err(format!(
            "{}: test artifact records must be strictly sorted by path",
            path.display()
        ));
    }
    Ok(artifacts)
...
fn checked_go_test_artifact_inventory(
    root: &Path,
    entries: &BTreeSet<Entry>,
    mode: &str,
) -> Result<Vec<GoTestArtifact>, String> {
    let generated = collect_go_test_artifacts(root, entries)?;
    let want = render_go_test_artifact_inventory(&generated);
    let inventory = root.join(GO_ARTIFACT_INVENTORY_RELATIVE_PATH);
    if mode == "--write" {
        fs::create_dir_all(inventory.parent().expect("inventory path has a parent"))
            .map_err(|error| format!("create test artifact inventory directory: {error}"))?;
        fs::write(&inventory, &want)
            .map_err(|error| format!("write {}: {error}", inventory.display()))?;
        return Ok(generated);
    }
    ...
    if got != want {
        return Err(format!(
            "{} is stale: an original test artifact changed without a ledger regeneration. Regenerate the ledger, then triage the changed test contract before assigning ports.",
            inventory.display()
        ));
    }
    parse_go_test_artifact_inventory(&inventory, &got)
}
    ...
    for path in files {
        ...
        if is_go_test {
            ...
            let parent_tests = declarations
                .iter()
                .filter(|declaration| declaration.path == rel && declaration.is_test_obligation())
                .filter(|declaration| declaration.category != "TestSuiteMethod")
            ...
            {
                if declaration.is_test_obligation() {
                    for name in declaration.obligation_names() {
                        entries.insert(Entry {
                            kind: "go_test",
                            path: rel.clone(),
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // `--check-inventory` prove only the mechanical upstream test surface.
    // `--write-evidence` is deliberately narrower than `--write`: it first
    // requires that mechanical inventories are current, then atomically
    // refreshes only the rendered evidence overlay for campaign promotion.
    if mode == "--write" || mode == "--check-inventory" {
        return Ok(());
    }
    ...
    if mode == "--write-evidence" {
        fs::write(&ledger, want).map_err(|error| format!("write {}: {error}", ledger.display()))?;
        print_summary(&entries, &evidence);
        return Ok(());
    }
...
mod tests {
    ...
    #[test]
    fn test_artifact_fingerprints_bind_test_bodies_and_inputs() {
        let test = Entry {
            kind: "go_test",
            path: "pkg/example/example_test.go".to_owned(),
            line: 12,
            name: "TestExample".to_owned(),
            ring: "result",
        };
        let fixture = Entry {
            kind: "testdata_artifact",
            path: "pkg/example/testdata/expected.txt".to_owned(),
            line: 0,
            name: "expected.txt".to_owned(),
            ring: "result",
        };
        let entries = BTreeSet::from([test.clone(), fixture.clone(), test]);
        assert_eq!(
            test_artifacts(&entries),
            BTreeSet::from([
                "pkg/example/example_test.go",
                "pkg/example/testdata/expected.txt",
            ])
        );
        assert_ne!(
            sha256(b"require.Equal(t, want, got)"),
            sha256(b"require.Equal(t, want, changed)"),
            "an assertion-body edit must invalidate the artifact baseline"
        );

        let artifacts = vec![
            GoTestArtifact {
                path: "pkg/example/example_test.go".to_owned(),
                sha256: sha256(b"test body"),
            },
            GoTestArtifact {
                path: "pkg/example/testdata/expected.txt".to_owned(),
                sha256: sha256(b"expected result"),
            },
        ];
        let rendered = render_go_test_artifact_inventory(&artifacts);
        assert_eq!(
            parse_go_test_artifact_inventory(Path::new("artifact.tsv"), &rendered).unwrap(),
            artifacts
        );
        let malformed = rendered.replacen(
            &sha256(b"test body"),
    ...
    fn ast_inventory_ignores_comments_and_validates_runner_signatures() {
        ...
        for (name, category) in [
            ...
            assert!(declaration.actionable_test_obligation);
        ...
        for name in [
            ...
            assert!(
                !find("valid_test.go", name).actionable_test_obligation,
        ...
        let suite_child = find("valid_test.go", "TestChild");
        assert_eq!(suite_child.category, "TestSuiteMethod");
        assert!(suite_child.actionable_test_obligation);
        assert_eq!(
            suite_child.suite_parents,
            ["TestReachableSuite", "TestReachableSuiteAgain"]
        );
        assert!(
            !find("valid_test.go", "TestInvalidChild").actionable_test_obligation,
            "suite methods with parameters are declarations, not testify obligations"
        );
        ...
        assert!(ledger_entries.iter().any(|entry| {
            entry.kind == "go_test"
                && entry.path == "valid_test.go"
                && entry.name == "TestReachableSuite/TestChild"
        }));
        assert!(ledger_entries.iter().any(|entry| {
            entry.kind == "go_test"
                && entry.path == "valid_test.go"
                && entry.name == "TestReachableSuiteAgain/TestChild"
        }));
        for name in [
            "TestInvalid",
            "BenchmarkInvalid",
