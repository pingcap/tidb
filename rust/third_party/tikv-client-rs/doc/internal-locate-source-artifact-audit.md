...
| Sender test identity and diagnostics correction | A repository-wide scan after the first closeout found six `src/region_request.rs` identities still generated by a forwarding macro that the earlier file-scoped scan omitted. The macro is removed. `TestRegionRequestSenderString`, `TestRegionRequestStats`, `TestGetErrMsg`, `TestRPCContextString`, and `TestBackoffErrWithRPCContext` now execute their own source behavior directly; `TestLogging` sends a real Get through the retry plan and observes the hintless-NotLeader result. The stronger context ports also added production `RpcContext` derivation, complete protobuf text, endpoint-specific access indexes, proxy details, deepest transport-cause extraction, and contextual TiKV/TiFlash retry reasons. |
| Complete unit-test ownership correction | A later whole-package body scan showed that the receipt's broader directness wording was still wrong: exact identities in `locate.rs`, `region_cache.rs`, and `request/plan.rs` could be independently selected but some did nothing except invoke an aggregate assertion helper. Every affected identity now performs source-specific setup, action, and assertion under its exact source name. Shared non-test helpers remain supplemental fixture or matrix support; no exact identity consists only of a helper call. `TestBatchClientSendLoopPanic` now owns the panic-recovery body itself and the redundant generic panic test is removed. The repository-wide gate reports 142 unique identities, zero differences, zero duplicates, no forwarding macro, no registered test-to-test call, and no one-call-only exact wrapper. |
...
- Mechanical declaration reconciliation: 142 expected source tests, zero missing, zero extra, zero duplicates, zero forwarding macros, zero registered test-to-test calls, and zero one-call-only exact wrappers.
...
- Canonical `make unit-test`: 1,274 no-default workspace tests passed with two configured skips; 1,249 all-feature workspace-library tests passed with six configured skips. The one-test reduction from the preceding package baseline removes the redundant generic panic-recovery test after its body moved under `TestBatchClientSendLoopPanic`; all 142 source identities remain independently executable.
