...
| `TestGetAndBatchGetWithReturnCommitTS`, `TestSnapshotCache` | `source_snapshot_return_commit_ts_refetches_unknown_cached_entries`, `source_snapshot_return_commit_ts_rejects_unknown_nonempty_entries`, `source_point_get_caches_value_before_missing_commit_ts_error`, `source_batch_get_does_not_cache_a_missing_commit_ts_response`, and cache hit/miss/reset tests execute every meaningful branch. |
| `TestSkipLargeTxnLock`, `TestPointGetSkipTxnLock` | `source_max_timestamp_point_get_omits_locks_after_the_first_transaction`, pair/response lock retries, and the completed `txnkv/txnlock` async-commit status tests cover lock omission, read-through hints, and committed-value recovery. |
| `TestSnapshotThreadSafe` | `source_external_snapshot_thread_safe_workload_uses_native_shared_ownership` executes the exact 5 tasks × 30 Get/BatchGet loop at `MaxUint64`, including a missing key, and statically requires `Snapshot<MockPdClient>: Send + Sync`. |
| `TestSnapshotRuntimeStats` | `snapshot_stats::{clone_and_merge_preserve_independent_rpc_totals,display_matches_client_go_runtime_stat_format,read_pool_details_merge_clone_and_format_like_client_go,duration_formatting_matches_client_go_precision_rules}` plus region-retry/runtime-SLI tests cover its full assertion sequence. |
...
| `TestSnapshotCacheBypassMaxUint64`, `TestReplicaReadAdjuster` | The two max-timestamp no-cache tests cover Get, BatchGet, and option variants. Replica adjustment is tested per physical region and per locked-key retry; the async true/false switch is tested from client configuration through callback accounting. |
| `snapshot_fail_test.go`: `TestBatchGetResponseKeyError`, `TestScanResponseKeyError` | `source_external_batch_get_response_error_retries_the_complete_key_set` and `source_snapshot_scanner_retries_incomplete_response_after_top_level_lock` prove that incomplete pairs do not escape and the appropriate complete request is retried. |
| `TestRetryMaxTsPointGetSkipLock`, `TestRetryPointGetResolveTS` | Latest point-lock omission, one-status-check behavior, committed/resolved hint propagation, and no second-transaction resolution execute in deterministic lock/status mocks. |
| `TestCommitTSRequiredAssertion` (seven named rows) | Ordinary Get/BatchGet cache and physical zero-commit-TS branches, cache-before-error versus no-batch-cache behavior, and BufferBatchGet's option-ignoring behavior are executable across the five commit-TS regressions and `batch_get_from_buffer_with_options`. |
| `TestResetSnapshotTS`, `TestSnapshotUseResolveForRead` (async commit false/true) | Timestamp reset tests clear cached reads and resolved hints while preserving start TS/committed hints; read-through lock tests cover ordinary and async-commit status recovery. |
| `scan_test.go`: `TestScan` (rows 1, 256, 257, 768; split at 123/456) | `source_external_scan_suite_covers_default_batch_and_region_boundaries` ports the exact row/split matrix and adds bounded, key-only toggle, and reverse assertions. It fails on the pre-fix eager multi-region implementation by returning 691/768 rows. |
| `scan_mock_test.go`: `TestScanMultipleRegions`, `TestReverseScan` | The same external-matrix port runs reverse requests with source batch size 10 across the source-shaped split topology; iterator tests separately prove eager first fetch, validity/close, bounds, and empty-region crossing. |
| `split_test.go`: `TestSplitBatchGet`, `TestBatchGetUsingAsyncAPI`, `TestStaleEpoch` | Stale relocation/epoch-cache tests, multi-region existing/missing values, callback result classes, response/pair locks, wait-all errors, and transport/region retry tests execute locally. Root split/store orchestration remains owned by `tikv`. |
| `pipelined_memdb_test.go`: ten suite methods | Snapshot-owned buffer-tier reads, delete retention, prefetch/cache, own-lock skipping, and gate/error behavior execute here and in `unionstore`; generation flush/block, commit, rollback, PK-failure, and TTL branches remain atomically ported and receipted by `txnkv/transaction`, not double-counted here. |
