...
| `trace` | public `src/trace.rs` plus request, region-cache, 2PC, and lock-resolver consumers | complete | Reopened atomic receipt: [`trace-source-artifact-audit.md`](trace-source-artifact-audit.md). All four artifacts/424 lines, all eight exact independently selectable unit-test ports, and all five direct importers are assigned. Red/green gates restore open future category values, commit response-error event omission, logical forwarding addresses, gogo region-error text, and the send/admission/transport/settlement/result envelope. Exact Go normal/race suites pass in legacy and NextGen modes; focused 16-test Rust matrices, complete 1,406/1,370 unit matrices, strict check/Clippy/private rustdoc, and 51 doctests pass. |
...
The earlier coarse receipt is superseded by the 2026-08-26 reopened atomic re-audit in [`trace-source-artifact-audit.md`](trace-source-artifact-audit.md). That receipt owns the immutable four-artifact/424-line inventory, all eight original tests as exact direct Rust identities, all five direct importers, exact Go 1.25.12 normal/race execution in both build configurations, open numeric category compatibility, complete wire/event/interceptor integration, and final pinned-nightly validation.
