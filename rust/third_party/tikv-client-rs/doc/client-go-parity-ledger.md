...
| `internal/mockstore/mocktikv` | hidden `src/mock/mocktikv`, reusable `unistore` engine crate, mock stream adapters | in-progress | Prior atomic receipt: [`internal-mockstore-mocktikv-source-artifact-audit.md`](internal-mockstore-mocktikv-source-artifact-audit.md). Downstream execution found that transactional Get does not set `GetResponse.not_found`, so an absent key is decoded as an empty value. The row is reopened until the source-derived external regression passes; the reusable engine and all other inventoried behavior remain assigned. |
...
| `txnkv/transaction` | `src/transaction/{transaction,txn_file,client,requests,lowering,buffer,unionstore}.rs`, request/error/PD integration | in-progress | Prior 16-artifact/11,766-line receipt: [`txnkv-transaction-source-artifact-audit.md`](txnkv-transaction-source-artifact-audit.md). Downstream TiDB-driver execution proved the native transaction still commits from a separate flat `Buffer`, so writes staged through the completed `MemDb`/`GetMemBuffer` contract require an out-of-tree drain. The package is reopened until transaction reads, flags, staging, and commit share one MemDB and the injected-client test surface is externally usable. |
