...
| `txnkv/transaction` | `src/transaction/{transaction,client,requests,lowering,buffer,unionstore}.rs` | seed | Atomic inventory: production `2pc.go` (2,466), `batch_getter.go` (138), `binlog.go` (52), `cleanup.go` (117), `commit.go` (283), `pessimistic.go` (708), `pipelined_flush.go` (537), `prewrite.go` (666), `txn.go` (2,146), and `txn_file.go` (1,501); always-built support `test_probe.go` (478) and `test_util.go` (518); tests `2pc_test.go` (181), `batch_getter_test.go` (143), `txn_file_test.go` (1,477), and `txn_test.go` (355). There are no package-local non-Go fixtures, generated inputs, or build variants. Rust has substantial compatible transaction, 2PC, pessimistic, pipelined-flush, batch-get, cleanup/commit/prewrite, buffer, and request-lowering seeds, backed by the complete unionstore/latch/locate/tikvrpc packages, but the complete txn-file protocol/lifecycle, binlog surface, committer actions/state transitions, failpoints/probes, all source tests, and differential real-TiKV validation are not yet accounted for. The package still depends on incomplete `tikv` and lock/snapshot paths; no completion claim is made. |
