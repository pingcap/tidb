...
- `internal/locate/pd_codec.go`, `region_cache.go`, and locate tests own PD boundary coding and special non-retry treatment of `IsDecodeError`. Rust's `src/pd/codec.rs` uses the complete codecs and now returns typed decode errors; retry policy remains part of the still-in-progress `internal/locate` receipt.
