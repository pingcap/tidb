...
| pool and lifecycle | `StoreRuntime` directly owns cancellation and joins both workers before closing compatible etcd and transport owners. `Spool` provides bounded nonblocking admission and source-compatible `Run` success/closed behavior for external users, including zero-capacity and post-close behavior from the pinned `gp` dependency. Root transport shutdown now cancels and joins the region cache before retiring any cached TiKV client, exactly matching `KVStore.Close`; a client close callback observes and asserts that order. |
...
| --- | --- |
| `TestSplitRegionsPreservesLegacyKeyErrorBehavior` | `tikv::tests::source_test_split_regions_preserves_legacy_key_error_behavior` |
| `TestSplitTxnFileRegionsResolvesLockAndRetries` | `tikv::tests::source_test_split_txn_file_regions_resolves_lock_and_retries`, plus the parent-budget cancellation regression |
| `TestSplitTxnFileRegionsSplitsWithoutScattering` | `tikv::tests::source_test_split_txn_file_regions_splits_without_scattering` |
| `TestMinSafeTsFromStores` | `tikv::tests::source_test_min_safe_ts_from_stores` |
| `TestHandleSplitRegionKeyErrorsExpandsSharedLockHolders` | `tikv::tests::source_test_handle_split_region_key_errors_expands_shared_lock_holders` |
| `TestMinSafeTsFromStoresWithAllZeros` | `tikv::tests::source_test_min_safe_ts_from_stores_with_all_zeros`; two deterministic refresh rounds retain the source four-request gate |
| `TestMinSafeTsFromStoresWithSomeZeros` | `tikv::tests::source_test_min_safe_ts_from_stores_with_some_zeros`; two deterministic refresh rounds retain the source four-request gate |
| `TestMinSafeTsFromPD` | `tikv::tests::source_test_min_safe_ts_from_pd` |
| `TestMinSafeTsFromPDByStores` | `tikv::tests::source_test_min_safe_ts_from_pd_by_stores` |
| `TestMinSafeTsFromMixed1` | `tikv::tests::source_test_min_safe_ts_from_mixed1` |
| `TestMinSafeTsFromMixed2` | `tikv::tests::source_test_min_safe_ts_from_mixed2` |
| `TestErrorHalfwayInNewKVStore` | `tikv::tests::source_test_error_halfway_in_new_kv_store` |
| `TestKVStoreCloseCheckRegionCacheClosedBeforePDClose` | `pd::client::test::source_test_kv_store_close_check_region_cache_closed_before_pd_close` |
...
The final package gate uses official Go 1.25.12 for the pinned source and `nightly-2026-08-22-aarch64-apple-darwin` for Rust. Exact `go test ./tikv -count=1` and `go test -race ./tikv -count=1` pass in 32.051 and 32.095 seconds; the race link emits only the known macOS malformed `LC_DYSYMTAB` warning. Mechanical reconciliation verifies all 17 artifacts/3,895 lines, the suite entry, every one of its 12 independently named method ports, the independently named lifecycle test, the `TestMain` ownership disposition, and all 35 direct importers.

The focused root matrix passes 29 tests in both no-default and all-feature configurations, and the close-order port passes separately in both. All source-derived tests pass 769/769 and 766/766. The complete no-default workspace passes 1,026 active main-crate tests plus one unrelated ignore and companion matrices of 2 and 32 tests; the complete all-feature library passes 1,023 active tests plus the same unrelated ignore. Workspace all-target/all-feature compilation, strict Clippy, private-item rustdoc with warnings denied, all 51 doctests, rustfmt, and whitespace checks pass. Production GC, unsafe destruction, and split/scatter calls are validated through pure state machines and mock/loopback transports; this package gate deliberately does not repeat the separately completed live-cluster matrix.
