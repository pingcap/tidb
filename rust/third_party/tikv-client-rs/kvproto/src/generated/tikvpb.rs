...
pub mod versioned_kv_server {
    ...
    #[async_trait]
    pub trait VersionedKv: std::marker::Send + std::marker::Sync + 'static {
    ...
    ///  (each `VersionedKeyRange.range` must be a point range) and keep `coprocessor.Request.ranges` empty.
    #[derive(Debug)]
    pub struct VersionedKvServer<T> {
        inner: Arc<T>,
        accept_compression_encodings: EnabledCompressionEncodings,
        send_compression_encodings: EnabledCompressionEncodings,
        max_decoding_message_size: Option<usize>,
    ...
    impl<T> Clone for VersionedKvServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "tikvpb.VersionedKv";
    impl<T> tonic::server::NamedService for VersionedKvServer<T> {
        const NAME: &'static str = SERVICE_NAME;
    }
}
