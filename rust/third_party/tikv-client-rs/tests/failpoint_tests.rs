...
// A failed public commit closes the transaction and schedules source-style
// cleanup. The caller cannot re-enter rollback after that first attempt.
#[tokio::test]
#[serial]
async fn txn_failed_commit_closes_and_cleans_prewrite_locks() -> Result<()> {
    init().await?;
    let scenario = FailScenario::setup();
