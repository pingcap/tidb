...
use super::{
    valid_to_analyze, NOT_PARTITIONED_TABLE, PARTITION_NOT_EXIST, SCHEMA_NOT_EXIST, TABLE_NOT_EXIST,
};
...
pub struct StaticPartitionedJob {
...
impl StaticPartitionedJob {
    pub fn validate_and_prepare(
        ...
        let Some(partition) = table
            .partitions
            .iter()
            .find(|partition| partition.id == self.partition_id)
        else {
            return Ok((false, PARTITION_NOT_EXIST.to_owned()));
        };
        self.schema_name = table.schema_name;
        self.table_name = table.table_name;
        self.partition_name = partition.name.clone();
        ...
        valid_to_analyze(
            ...
            &self.schema_name,
            &self.table_name,
            std::slice::from_ref(&self.partition_name),
        )
    }

    pub fn partition_statement(&self) -> SqlStatement {
        SqlStatement {
            sql: "analyze table %n.%n partition %n".to_owned(),
            params: vec![
                SqlValue::Identifier(self.schema_name.clone()),
                SqlValue::Identifier(self.table_name.clone()),
                SqlValue::Identifier(self.partition_name.clone()),
            ],
        }
    }
    ...
    pub fn index_statement(&self, index: &str) -> SqlStatement {
        SqlStatement {
            sql: "analyze table %n.%n partition %n index %n".to_owned(),
            params: vec![
                SqlValue::Identifier(self.schema_name.clone()),
                SqlValue::Identifier(self.table_name.clone()),
                SqlValue::Identifier(self.partition_name.clone()),
    ...
    pub fn analyze(&self, sql: &mut impl SqlPort) -> RuntimeResult<()> {
        if self.index_ids.is_empty() {
            sql.execute(&self.partition_statement())
