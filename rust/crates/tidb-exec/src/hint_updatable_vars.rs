...
//! SET_VAR hint-updatable system-variable registry from
//! `pkg/sessionctx/variable/setvar_affect.go`.
//!
//! The Go source keeps an exact-name registry used to mark system variables
//! safe for hint updates. This leaf ports the complete registry and lookup
//! predicate only; SysVar mutation, hint parsing/application, validation,
//! planner consumption, and session lifecycle remain external.

/// Source-owned system-variable names verified as SET_VAR-hint updatable.
pub const HINT_UPDATABLE_VARIABLES: [&str; 128] = [
