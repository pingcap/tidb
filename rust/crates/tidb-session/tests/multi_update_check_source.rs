//! Multi-table UPDATE honors CHECK constraints: with
//! `tidb_enable_check_constraint` on, a violation on a LATER joined row
//! fails the statement with Go's 3819 and rolls back the EARLIER rows'
//! changes — the multi-table write is one transaction, exactly like the
//! single-table path.
...
#[test]
fn check_violation_rolls_back_the_whole_multi_table_update() {
    let mut session = Session::new();
    // Go: the variable is GLOBAL-scope only; a stock session defaults OFF.
    session
