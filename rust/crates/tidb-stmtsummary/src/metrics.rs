...
//! Go `pkg/metrics/stmtsummary.go`: statement-summary window gauges.
//!
...
pub fn init_dashboard_series() {
    let _ = WINDOW_EVICTED_COUNT.with_label_values(&["v1"]);
    let _ = WINDOW_RECORD_COUNT.with_label_values(&["v1"]);
}
