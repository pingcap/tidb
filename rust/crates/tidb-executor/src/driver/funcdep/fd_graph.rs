...
impl FdSet {
    ...
    pub(crate) fn closure_of_strict(&self, cols: &ColSet) -> ColSet {
    ...
    #[cfg(test)]
    pub(crate) fn closure_of_lax(&self, cols: &ColSet) -> ColSet {
    ...
    pub(crate) fn add_strict(&mut self, from: ColSet, to: ColSet) {
        self.add_functional_dependency(from, to, true, false);
    }

    /// Go `FDSet.AddLaxFunctionalDependency`.
    pub(crate) fn add_lax(&mut self, from: ColSet, to: ColSet) {
    ...
    pub(crate) fn add_equivalence(&mut self, from: ColSet, to: ColSet) {
        if to.subset_of(&from) {
            return;
        }
    ...
    /// edge.
    pub(crate) fn add_constants(&mut self, constants: ColSet) {
    ...
    #[cfg(test)]
    pub(crate) fn constant_cols(&self) -> ColSet {
        self.edges
            .iter()
    ...
    pub(crate) fn make_cartesian_product(&mut self, rhs: &FdSet) {
