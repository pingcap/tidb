...
pub(crate) struct SearchInput<'a> {
    ...
    /// A merge candidate already constructed from the same predicates and
    /// verified against the children that were built for it.
    ///
    /// The driver retains equality-fixed index prefixes in its possible-order
    /// inventory and trims them in `merge_decision`; the compact logical input
    /// below does not carry functional dependencies, so `exhaust_join` can
    /// conservatively miss that same candidate. This receipt closes that
    /// representation gap without manufacturing a merge join.
    pub(crate) merge_available: bool,
...
fn decide(input: &SearchInput<'_>, record: &mut Record) -> Chosen {
    ...
    let logical = logical_join(input, orders);
    let candidates = exhaust_join(&logical, input.required);
    let mut index = false;
    let mut hash = false;
    let mut merge = input.merge_available;
