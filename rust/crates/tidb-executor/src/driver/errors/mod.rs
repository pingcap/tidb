...
impl MysqlError {
    /// The SQLSTATE the code carries, derived from the code itself.
    ///
    ...
    /// Reconstructs an error whose SQLSTATE was carried in from outside this
    /// module (a stored or wire-level error already holding its state), not
    /// one raised fresh here; fresh raises go through [`MysqlError::new`].
    fn with_state(code: u16, state: [u8; 5], message: impl Into<String>) -> Self {
        Self {
            code,
            state,
    ...
    /// still reads as "coded" where a call site emphasizes the lookup.
...
impl DriverError {
    ...
    pub fn to_mysql_error(self) -> MysqlError {
        ...
        DriverError::ColumnCannotBeNull(name) => {
            MysqlError::new(1048, format!("Column '{name}' cannot be null"))
        }
        ...
        DriverError::DuplicateKeyName(name) => {
            MysqlError::new(1061, format!("Duplicate key name '{name}'"))
        }
        ...
        // Go: ErrUnsupportedShardRowIDBits -> "Unsupported shard_row_id_bits
        // for table with primary key as row id" (8200).
        DriverError::UnsupportedShardRowIdBits => MysqlError::new(
            8200,
            "Unsupported shard_row_id_bits for table with primary key as row id".to_string(),
        ),
        ...
        DriverError::AdminCheckTable(detail) => MysqlError::new(8003, detail),
        // Go `ClassAdmin.NewStd(errno.ErrDataInconsistent)`: 8223, HY000.
        DriverError::DataInconsistent(detail) => MysqlError::new(8223, detail),
        DriverError::DataInconsistentMismatchIndex(detail) => {
            MysqlError::new(8134, detail)
        }
        ...
        DriverError::WrongParamCount => {
            MysqlError::new(8112, "Wrong parameter count".to_owned())
        }
        // Go: "Prepared statement not found".
        DriverError::PreparedStmtNotFound => {
            MysqlError::new(8111, "Prepared statement not found".to_owned())
        }
        ...
        DriverError::Schema(crate::SchemaErrorKind::BadTable(name)) => {
            MysqlError::new(1051, format!("Unknown table '{name}'"))
        }
        // Go: "Table '%-.192s' doesn't exist".
        DriverError::Schema(crate::SchemaErrorKind::UnknownTable(name)) => {
            MysqlError::new(1146, format!("Table '{name}' doesn't exist"))
        }
        ...
        DriverError::Schema(crate::SchemaErrorKind::UnknownSequence(name)) => {
            MysqlError::new(4139, format!("Unknown SEQUENCE: '{name}'"))
        }
        ...
        DriverError::Schema(crate::SchemaErrorKind::WrongObject { name, expected }) => {
            MysqlError::new(1347, format!("'{name}' is not {expected}"))
        }
        ...
        DriverError::MemoryExceedForQuery { conn_id } => {
            ...
            MysqlError::with_state(sql_error.code, state, sql_error.message)
        }
        ...
        DriverError::Schema(crate::SchemaErrorKind::NoDatabaseSelected) => {
            MysqlError::new(ER_NO_DB_ERROR, "No database selected".to_owned())
        }
        ...
        DriverError::Var(crate::VarErrorKind::SqlError(error)) => {
            ...
            MysqlError::with_state(error.code, state, error.message.clone())
        }
        ...
        DriverError::Var(crate::VarErrorKind::ValidationRefused(message)) => {
            MysqlError::new(1105, message.clone())
        }
