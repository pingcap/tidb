...
fn tpcc_check_five_keeps_only_the_cross_leaf_residual() {
    ...
    .unwrap();
    for (table_name, column_offsets) in [("orders", vec![2, 1, 0]), ("new_order", vec![2, 1, 0])] {
...
fn tpcc_check_seven_propagates_the_warehouse_range_to_both_leaves() {
    ...
    scale_analyzed_tpcc_table(
        ...
        "orders",
        300_000,
        &[("o_id", 3_000), ("o_d_id", 10), ("o_w_id", 10)],
        &ctx,
    );
    scale_analyzed_tpcc_table(
        &mut catalog,
    ...
    let (_, analyzed) = crate::explain::explain_select_stmt(
        select,
        &catalog,
        "test",
        &ctx,
    ...
    let analyzed = analyzed
        ...
        .collect::<Vec<_>>();
    assert!(
        analyzed.iter().any(|line| {
            line.contains("IndexHashJoin")
                && line.contains(
                    "outer key:test.orders.o_id, test.orders.o_d_id, test.orders.o_w_id, \
                     inner key:test.order_line.ol_o_id, test.order_line.ol_d_id, \
                     test.order_line.ol_w_id",
                )
                && line.contains(
                    "equal cond:eq(test.orders.o_d_id, test.order_line.ol_d_id), \
                     eq(test.orders.o_id, test.order_line.ol_o_id), \
                     eq(test.orders.o_w_id, test.order_line.ol_w_id)",
                )
        }),
...
fn tpcc_stock_level_bounds_both_join_leaves() {
    ...
    assert_eq!(
        ops.scans, 1,
        "an index join opens only its outer child; complete inner handles are point reads"
    );
    assert_eq!(
        ops.gets, 2,
    ...
    scale_analyzed_tpcc_table(
        ...
        "stock",
        1_000_000,
        &[("s_i_id", 100_000), ("s_w_id", 10), ("s_quantity", 91)],
        &ctx,
    );
    catalog.clear_dirty_content();
    let (_, analyzed_rows) =
        explain_select_stmt(select, &catalog, "test", &ctx, ExplainFormat::Row).unwrap();
    let analyzed_plan = analyzed_rows
        .iter()
        .map(|row| {
            row.iter()
                .map(|datum| match datum {
    ...
    let scan_estimate = |table: &str| {
        analyzed_plan
            ...
            .expect("the table has a dynamic range scan")
    };
    assert_eq!(
        scan_estimate("stock"),
        scan_estimate("order_line"),
        "a complete common-handle probe can read at most one row per outer row: {analyzed_plan:?}"
    );
}
...
/// primary key, so Go chooses an ordered MergeJoin over two PointGet tasks.
#[test]
fn tpcc_customer_warehouse_join_uses_two_point_gets() {
    ...
    crate::run_create_table_on(
        "CREATE TABLE customer (\
            ...
            c_state CHAR(2), c_zip CHAR(9), c_phone CHAR(16), c_since DATETIME, \
    ...
    crate::run_create_table_on(
        "CREATE TABLE warehouse (\
            ...
            w_zip CHAR(9), w_tax DECIMAL(4,4), w_ytd DECIMAL(12,2), \
            PRIMARY KEY (w_id))",
        &mut catalog,
    )
    .unwrap();
    ...
    run_insert_on(
        "INSERT INTO customer \
            ...
            VALUES (629,6,1,'Anne','OE','Smith','GC',0.05)",
        &mut catalog,
        &ctx,
    )
    .unwrap();
    run_insert_on(
        "INSERT INTO warehouse (w_id,w_tax) VALUES (1,0.07)",
        &mut catalog,
        &ctx,
    )
    .unwrap();
    ...
    scale_analyzed_tpcc_table(&mut catalog, "warehouse", 10, &[("w_id", 10)], &ctx);
    let (customer_table_id, customer_index_id) = {
        let TableEntry::Kv(customer) = catalog.get_in("test", "customer").unwrap() else {
            panic!("customer is not a KV table");
        };
        let index_id = customer
            .indexes()
            .iter()
            .find(|index| index.name == "idx_customer")
            .map(|index| index.id)
            .expect("idx_customer");
        (customer.table_id, index_id)
    };
    let mut customer_stats = catalog
        .table_statistics(customer_table_id)
        .expect("customer statistics")
        .as_ref()
        .clone();
    let index_stats = customer_stats
        .indexes
        .get_mut(&customer_index_id)
        .expect("idx_customer statistics");
    let customer_prefix = tidb_codec::encode_key(&[Datum::Int(1), Datum::Int(6)]).unwrap();
    let mut customer_prefix_next = customer_prefix.clone();
    for byte in customer_prefix_next.iter_mut().rev() {
        *byte = byte.wrapping_add(1);
        if *byte != 0 {
            break;
        }
    }
    index_stats.histogram = tidb_stats::Histogram {
        id: customer_index_id,
        ndv: 300_000,
        last_update_version: index_stats.histogram.last_update_version,
        buckets: vec![
            tidb_stats::Bucket {
                count: 3_519,
                repeat: 1,
                ndv: 3_519,
                lower_bound: Datum::Bytes(customer_prefix),
                upper_bound: Datum::Bytes(customer_prefix_next),
            },
            tidb_stats::Bucket {
                count: 300_000,
                repeat: 1,
    ...
    index_stats.topn = None;
    catalog.set_table_statistics(customer_table_id, std::sync::Arc::new(customer_stats));
    ...
    let plan: Vec<String> = rows
        ...
        .collect();
    let operator_name = |line: &str| {
        let operator = line.split("\\t").next().unwrap_or_default();
        let Some((prefix, suffix)) = operator.rsplit_once('_') else {
            return operator.to_owned();
        };
        let digits = suffix.bytes().take_while(u8::is_ascii_digit).count();
        if digits > 0 && (digits == suffix.len() || suffix.as_bytes().get(digits) == Some(&b'(')) {
            format!("{prefix}{}", &suffix[digits..])
        } else {
            operator.to_owned()
        }
    };
    assert_eq!(
        plan.iter()
            .map(|line| operator_name(line))
            .collect::<Vec<_>>(),
        vec![
            "Projection".to_owned(),
            "└─Projection".to_owned(),
            "  └─MergeJoin".to_owned(),
            "    ├─Point_Get(Build)".to_owned(),
            "    └─Point_Get(Probe)".to_owned(),
        ],
        "{plan:#?}",
    );
    assert!(
    ...
    assert_eq!(plan[2].split("\\t").nth(1), Some("1.17"), "{plan:#?}");
    assert!(
        plan[3].contains("table:customer") && plan[3].contains("clustered index:PRIMARY"),
        "{plan:#?}",
    );
    assert!(
        plan[4].contains("table:warehouse") && plan[4].contains("handle:1"),
        "{plan:#?}",
    );
...
fn index_join_probe_rows_use_only_the_access_paths_join_keys() {
    ...
    crate::run_create_table_on(
        "CREATE TABLE t2 (\
            k1 INT NOT NULL, id INT NOT NULL, k2 INT NOT NULL, pad VARCHAR(100), \
            PRIMARY KEY (k1, id) CLUSTERED, KEY idx_k1_k2 (k1, k2))",
        &mut catalog,
    )
    .unwrap();
    let ctx = crate::StmtContext::for_query();
    ...
    run_insert_on(
        "INSERT INTO t2 VALUES \
            (1,1,1,'x'),(1,2,2,'x'),(2,1,1,'x'),(2,2,2,'x')",
        &mut catalog,
        &ctx,
    )
    .unwrap();
    scale_analyzed_tpcc_table(&mut catalog, "t1", 1, &[("k1", 1), ("k2", 1)], &ctx);
    scale_analyzed_tpcc_table(
        &mut catalog,
        "t2",
        2_000,
