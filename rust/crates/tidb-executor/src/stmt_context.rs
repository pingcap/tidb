...
impl StmtContext {
    ...
    /// Go `SessionVars.IndexLookupSize`, the maximum number of handles in one
    /// regular index double-read task.
    #[must_use]
    pub fn index_lookup_size(&self) -> usize {
        self.optimizer_cost_env().session.index_lookup_size.max(1.0) as usize
    }
    ...
    pub fn take_warnings(&self) -> Vec<(WarningLevel, u16, String)> {
        ...
        for (level, code, message) in std::mem::take(
            &mut *self
                .cop_batch_warnings
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner),
    ...
    pub fn enter_cop_eval(&self) -> CopEvalGuard<'_> {
        self.cop_eval_depth.fetch_add(1, Ordering::Relaxed);
