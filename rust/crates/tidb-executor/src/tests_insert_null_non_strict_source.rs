...
use crate::{
    run_create_table_on, run_insert_reporting, run_select_on, run_update_on, Catalog, StmtContext,
};
...
fn all_rows(catalog: &Catalog) -> Vec<Vec<String>> {
    run_select_on(
        "select id, col1 from tn order by id",
        catalog,
        &StmtContext::for_query(),
    )
    .expect("select succeeds")
...
fn non_strict_insert_null_semantics_follow_the_values_path() {
    ...
    .unwrap();
    run_create_table_on(
        "create table ts (id int primary key, col1 varchar(10))",
        &mut catalog,
    )
    .unwrap();
    ...
    run_insert_reporting(
        "insert ignore into tn values(5, null)",
        &mut catalog,
        "test",
        &strict_ignore,
    )
    ...
    assert!(
        warnings
            .iter()
            .any(|(_, code, message)| *code == 1048
                && message.contains("Column 'col1' cannot be null")),
    ...
    let error = run_insert_reporting(
    ...
    let error = run_insert_reporting(
        "insert into tn set id = 1, col1 = null",
        &mut catalog,
        "test",
        &non_strict,
    )
    ...
    run_update_on(
        "update tn set col1 = null where id = 3",
        &mut catalog,
        &non_strict,
    )
    .unwrap();
