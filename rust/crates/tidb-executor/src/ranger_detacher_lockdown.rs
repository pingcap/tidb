...
//! File-lockdown owner for Go `pkg/util/ranger/detacher.go`.
//!
...
//! implementation: it owns the lockdown receipt, compiles both seams, and
//! supplies the two condition-list rules that had no native owner.
//!
...
mod tests {
    ...
    fn condition_list_helpers_match_go_existing_slice_semantics() {
        assert_eq!(
            crate::ranger_detacher_lockdown::remove_conditions(&[1, 2, 1, 3], &[1, 4]),
            vec![2, 3]
        );
        assert_eq!(
            crate::ranger_detacher_lockdown::remove_conditions::<i32>(&[], &[1]),
            Vec::<i32>::new()
        );

        assert_eq!(
            crate::ranger_detacher_lockdown::append_conditions_if_not_exist(&[1, 2], &[2, 3, 3],),
            vec![1, 2, 3, 3]
        );
        assert_eq!(
            crate::ranger_detacher_lockdown::append_conditions_if_not_exist(&[1, 1], &[1]),
            vec![1, 1]
        );
    }
