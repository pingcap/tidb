...
//! Transcreation of `pkg/parser/ast/procedure_test.go`.
...
fn test_procedure_visitor_cover() {
    for sql in [
        "CREATE PROCEDURE p(IN id INT) BEGIN SELECT id; END",
...
fn test_procedure_visitor() {
    for sql in [
        "CREATE PROCEDURE p(IN id BIGINT) BEGIN DECLARE s VARCHAR(100) DEFAULT FROM_UNIXTIME(1447430881); SELECT s; INSERT INTO t VALUES (111); END",
        "SHOW CREATE PROCEDURE p",
        "DROP PROCEDURE p",
    ] {
        assert_full_visitor_traversal(sql);
...
fn test_show_create_procedure() {
    assert_eq!(
        r("show create procedure proc_2"),
        "SHOW CREATE PROCEDURE `proc_2`"
    );
    assert_eq!(r("drop procedure proc_2"), "DROP PROCEDURE `proc_2`");
}

#[test]
fn procedure_parse_source_rows() {
    for sql in [
        ...
        let statements = crate::parse_multi(sql)
            .unwrap_or_else(|error| panic!("parse source procedure {sql}: {error:?}"));
        assert!(
            matches!(
                statements.first(),
                Some(tidb_ast::Stmt::Ddl(ddl))
                    if matches!(ddl.as_ref(), tidb_ast::DdlStmt::CreateProcedure(_))
            ),
            "{sql}"
        );
    }
