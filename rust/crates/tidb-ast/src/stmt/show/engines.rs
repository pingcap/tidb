...
/// The Go shared SHOW filter alternatives for `ENGINES`.
#[derive(Debug, Clone, PartialEq)]
...
impl ShowEnginesStmt {
    pub(crate) fn restore_into(&self, out: &mut String) {
        out.push_str("SHOW ENGINES");
        match &self.filter {
            None => {}
            Some(ShowEnginesFilter::Like(expr)) => {
                out.push_str(" LIKE ");
                expr.restore_into(out);
            }
            Some(ShowEnginesFilter::Where(expr)) => {
                out.push_str(" WHERE ");
                expr.restore_into(out);
            }
        }
