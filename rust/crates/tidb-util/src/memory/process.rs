...
fn current_process_memory_usage() -> io::Result<u64> {
    ...
    // Current resident bytes, matching gopsutil's Darwin MemoryInfo without
    // launching a subprocess on every runtime sample.
...
fn cached_process_memory_usage() -> Option<u64> {
    ...
    let value = current_process_memory_usage().ok()?;
    cache.value = value;
...
mod tests {
    ...
    fn process_rss_sampling_needs_no_external_command() {
        ...
        let output = Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "memory::process::tests::process_rss_sampling_needs_no_external_command",
                "--nocapture",
            ])
            .env(CHILD, "1")
