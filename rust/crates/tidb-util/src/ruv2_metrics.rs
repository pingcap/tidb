...
//! Go `pkg/util/execdetails` `ruv2_metrics.go`: the live
...
//!
//! [`RuV2Weights`] and the live metrics container are owned together here, as
//! they are in Go. Formatting snapshots the live atomics into local values in
//! one pass; there is no separate public snapshot API.
//!
//! Runtime mappings:
//! - `kvrpcpb.RUV2`, `kvrpcpb.ExecutorInputs`, and client-go `util.RUDetails`
//!   use the workspace's generated kvproto and canonical `tikv-client`
//!   implementations.
//! - Go context values use `tikv_client::TraceContext`; the lifecycle and
//!   lookup entry points live in the consuming execdetails module.
...
pub use tikv_client::proto::kvrpcpb::{ExecutorInputs, Ruv2};
pub use tikv_client::RuDetails;

use std::fmt::Write as _;
use tikv_client::util::CommitDetails;

/// Go `RUV2Weights`: per-counter weights used by RU v2 accounting.
...
#[derive(Default)]
struct FormattingSnapshot {
    result_chunk_cells: i64,
    executor_l1: BTreeMap<String, i64>,
    executor_l2: BTreeMap<String, i64>,
    executor_l3: BTreeMap<String, i64>,
    executor_l5_insert_rows: i64,
    plan_cnt: i64,
    plan_derive_stats_paths: i64,
    session_parser_total: i64,
    txn_cnt: i64,
    resource_manager_read_cnt: i64,
    resource_manager_write_cnt: i64,
    write_keys: i64,
    write_size: i64,
    tikv_kv_engine_cache_miss: i64,
    tikv_coprocessor_executor_iterations: i64,
    tikv_coprocessor_response_bytes: i64,
    tikv_raftstore_store_write_trigger_wb: i64,
    tikv_storage_processed_keys_batch_get: i64,
    tikv_storage_processed_keys_get: i64,
    tikv_coprocessor_executor_work_total: BTreeMap<String, i64>,
}
...
struct Ruv2PrometheusMetrics {
    result_chunk_cells: Counter,
    executor_l1: CounterVec,
    executor_l2: CounterVec,
    executor_l3: CounterVec,
    executor_l5_insert_rows: Counter,
    plan_cnt: Counter,
    plan_derive_stats_paths: Counter,
    resource_manager_read_cnt: Counter,
    resource_manager_write_cnt: Counter,
    write_keys: Counter,
    write_size: Counter,
    session_parser_total: Counter,
    txn_cnt: Counter,
    tikv_kv_engine_cache_miss: Counter,
    tikv_coprocessor_executor_iterations: Counter,
    tikv_coprocessor_response_bytes: Counter,
    tikv_raftstore_store_write_trigger_wb: Counter,
    tikv_storage_processed_keys_batch_get: Counter,
    tikv_storage_processed_keys_get: Counter,
    tikv_coprocessor_work_total: CounterVec,
}

fn ruv2_counter(name: &str, help: &str) -> Counter {
    Counter::with_opts(Opts::new(name, help).namespace("tidb").subsystem("ruv2"))
        .unwrap_or_else(|error| panic!("invalid RUv2 counter: {error}"))
}

fn ruv2_counter_vec(name: &str, help: &str) -> CounterVec {
    CounterVec::new(
        Opts::new(name, help).namespace("tidb").subsystem("ruv2"),
        &["type"],
    )
    .unwrap_or_else(|error| panic!("invalid RUv2 counter vector: {error}"))
}

fn register_ruv2_collector<C>(collector: &C)
where
    C: Collector + Clone + 'static,
{
    prometheus::default_registry()
        .register(Box::new(collector.clone()))
        .unwrap_or_else(|error| panic!("failed to register RUv2 metric: {error}"));
}

impl Ruv2PrometheusMetrics {
    fn new() -> Self {
        let metrics = Self {
            result_chunk_cells: ruv2_counter(
                "result_chunk_cells",
                "Counter of result chunk cells for RU v2.",
            ),
            executor_l1: ruv2_counter_vec(
                "executor_l1",
                "Counter of executor L1 input/output for RU v2.",
            ),
            executor_l2: ruv2_counter_vec(
                "executor_l2",
                "Counter of executor L2 input/output for RU v2.",
            ),
            executor_l3: ruv2_counter_vec(
                "executor_l3",
                "Counter of executor L3 input/output for RU v2.",
            ),
            executor_l5_insert_rows: ruv2_counter(
                "executor_l5_insert_rows",
                "Counter of insert rows for RU v2.",
            ),
            plan_cnt: ruv2_counter("plan_cnt", "Counter of plan builder executions for RU v2."),
            plan_derive_stats_paths: ruv2_counter(
                "plan_derive_stats_paths",
                "Counter of derive stats paths for RU v2.",
            ),
            resource_manager_read_cnt: ruv2_counter(
                "resource_manager_read_cnt",
                "Counter of resource manager read requests for RU v2.",
            ),
            resource_manager_write_cnt: ruv2_counter(
                "resource_manager_write_cnt",
                "Counter of resource manager write requests for RU v2.",
            ),
            write_keys: ruv2_counter("write_keys", "Counter of commit write keys for RU v2."),
            write_size: ruv2_counter(
                "write_size",
                "Shadow counter of commit write size for RU v2.",
            ),
            session_parser_total: ruv2_counter(
                "session_parser_total",
                "Counter of session parser executions for RU v2.",
            ),
            txn_cnt: ruv2_counter("txn_cnt", "Counter of transactions for RU v2."),
            tikv_kv_engine_cache_miss: ruv2_counter(
                "tikv_kv_engine_cache_miss",
                "Counter of TiKV KV engine cache miss for RU v2.",
            ),
            tikv_coprocessor_executor_iterations: ruv2_counter(
                "tikv_coprocessor_executor_iterations",
                "Counter of TiKV coprocessor executor iterations for RU v2.",
            ),
            tikv_coprocessor_response_bytes: ruv2_counter(
                "tikv_coprocessor_response_bytes",
                "Counter of TiKV coprocessor response bytes for RU v2.",
            ),
            tikv_raftstore_store_write_trigger_wb: ruv2_counter(
                "tikv_raftstore_store_write_trigger_wb_bytes",
                "Counter of TiKV raftstore write trigger WB bytes for RU v2.",
            ),
            tikv_storage_processed_keys_batch_get: ruv2_counter(
                "tikv_storage_processed_keys_batch_get",
                "Counter of TiKV storage processed keys (batch get) for RU v2.",
            ),
            tikv_storage_processed_keys_get: ruv2_counter(
                "tikv_storage_processed_keys_get",
                "Counter of TiKV storage processed keys (get) for RU v2.",
            ),
            tikv_coprocessor_work_total: ruv2_counter_vec(
                "tikv_coprocessor_executor_work_total",
                "Counter of TiKV coprocessor executor work for RU v2.",
            ),
        };
        register_ruv2_collector(&metrics.result_chunk_cells);
        register_ruv2_collector(&metrics.executor_l1);
        register_ruv2_collector(&metrics.executor_l2);
        register_ruv2_collector(&metrics.executor_l3);
        register_ruv2_collector(&metrics.executor_l5_insert_rows);
        register_ruv2_collector(&metrics.plan_cnt);
        register_ruv2_collector(&metrics.plan_derive_stats_paths);
        register_ruv2_collector(&metrics.resource_manager_read_cnt);
        register_ruv2_collector(&metrics.resource_manager_write_cnt);
        register_ruv2_collector(&metrics.write_keys);
        register_ruv2_collector(&metrics.write_size);
        register_ruv2_collector(&metrics.session_parser_total);
        register_ruv2_collector(&metrics.txn_cnt);
        register_ruv2_collector(&metrics.tikv_kv_engine_cache_miss);
        register_ruv2_collector(&metrics.tikv_coprocessor_executor_iterations);
        register_ruv2_collector(&metrics.tikv_coprocessor_response_bytes);
        register_ruv2_collector(&metrics.tikv_raftstore_store_write_trigger_wb);
        register_ruv2_collector(&metrics.tikv_storage_processed_keys_batch_get);
        register_ruv2_collector(&metrics.tikv_storage_processed_keys_get);
        register_ruv2_collector(&metrics.tikv_coprocessor_work_total);
        metrics
    }

    fn executor_counter(&self, level: i64, label: &str) -> Option<Counter> {
        match level {
            1 => Some(self.executor_l1.with_label_values(&[label])),
            2 => Some(self.executor_l2.with_label_values(&[label])),
            3 => Some(self.executor_l3.with_label_values(&[label])),
            _ => None,
        }
    }
}

static RUV2_PROMETHEUS: LazyLock<Ruv2PrometheusMetrics> = LazyLock::new(Ruv2PrometheusMetrics::new);

fn add_prometheus_i64(counter: &Counter, delta: i64) {
    assert!(delta >= 0, "counter cannot decrease");
...
impl RuV2Metrics {
    ...
    pub fn add_result_chunk_cells(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.result_chunk_cells, delta);
    ...
    pub fn add_executor_metric(&self, level: i64, label: &str, delta: i64) {
        ...
        let Some(counter) = RUV2_PROMETHEUS.executor_counter(level, label) else {
            return;
        };
        add_prometheus_i64(&counter, delta);
        match level {
    ...
    pub fn add_executor_l5_insert_rows(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.executor_l5_insert_rows, delta);
        self.ensure_extra()
            .executor_l5_insert_rows
    ...
    pub fn add_plan_cnt(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.plan_cnt, delta);
    ...
    pub fn add_plan_derive_stats_paths(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.plan_derive_stats_paths, delta);
        self.ensure_extra()
            .plan_derive_stats_paths
            .fetch_add(delta, Ordering::Relaxed);
    }
    ...
    pub fn add_session_parser_total(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.session_parser_total, delta);
    ...
    pub fn add_txn_cnt(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.txn_cnt, delta);
    ...
    pub fn add_resource_manager_read_cnt(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.resource_manager_read_cnt, delta);
    ...
    pub fn add_resource_manager_write_cnt(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.resource_manager_write_cnt, delta);
        self.ensure_extra()
            .resource_manager_write_cnt
            .fetch_add(delta, Ordering::Relaxed);
    }
    ...
    pub fn add_write_keys(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.write_keys, delta);
        self.ensure_extra()
            .write_keys
            .fetch_add(delta, Ordering::Relaxed);
    }
    ...
    pub fn add_write_size(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.write_size, delta);
        self.ensure_extra()
            .write_size
            .fetch_add(delta, Ordering::Relaxed);
    }
    ...
    pub fn add_tikv_kv_engine_cache_miss(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.tikv_kv_engine_cache_miss, delta);
    ...
    pub fn add_tikv_coprocessor_executor_iterations(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.tikv_coprocessor_executor_iterations, delta);
        self.ensure_extra()
            .tikv_coprocessor_executor_iterations
            .fetch_add(delta, Ordering::Relaxed);
    }
    ...
    pub fn add_tikv_coprocessor_response_bytes(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.tikv_coprocessor_response_bytes, delta);
        self.ensure_extra()
            .tikv_coprocessor_response_bytes
            .fetch_add(delta, Ordering::Relaxed);
    }
    ...
    pub fn add_tikv_raftstore_store_write_trigger_wb(&self, delta: i64) {
        ...
        add_prometheus_i64(
            &RUV2_PROMETHEUS.tikv_raftstore_store_write_trigger_wb,
            delta,
        );
        self.ensure_extra()
    ...
    pub fn add_tikv_storage_processed_keys_batch_get(&self, delta: i64) {
        ...
        add_prometheus_i64(
            &RUV2_PROMETHEUS.tikv_storage_processed_keys_batch_get,
            delta,
        );
    ...
    pub fn add_tikv_storage_processed_keys_get(&self, delta: i64) {
        ...
        add_prometheus_i64(&RUV2_PROMETHEUS.tikv_storage_processed_keys_get, delta);
    ...
    fn apply_raw_counters(&self, ru: &Ruv2) {
        if ru.read_rpc_count != 0 {
            RUV2_PROMETHEUS
                .resource_manager_read_cnt
                .inc_by(ru.read_rpc_count as f64);
            self.resource_manager_read_cnt
        ...
        if ru.kv_engine_cache_miss != 0 {
            RUV2_PROMETHEUS
                .tikv_kv_engine_cache_miss
                .inc_by(ru.kv_engine_cache_miss as f64);
            self.tikv_kv_engine_cache_miss
        ...
        if ru.storage_processed_keys_batch_get != 0 {
            RUV2_PROMETHEUS
                .tikv_storage_processed_keys_batch_get
                .inc_by(ru.storage_processed_keys_batch_get as f64);
        ...
        if ru.storage_processed_keys_get != 0 {
            RUV2_PROMETHEUS
                .tikv_storage_processed_keys_get
                .inc_by(ru.storage_processed_keys_get as f64);
            self.tikv_storage_processed_keys_get
        ...
        if ru.write_rpc_count != 0 {
            RUV2_PROMETHEUS
                .resource_manager_write_cnt
                .inc_by(ru.write_rpc_count as f64);
            self.ensure_extra()
                .resource_manager_write_cnt
        ...
        if ru.coprocessor_executor_iterations != 0 {
            RUV2_PROMETHEUS
                .tikv_coprocessor_executor_iterations
                .inc_by(ru.coprocessor_executor_iterations as f64);
            self.ensure_extra()
                .tikv_coprocessor_executor_iterations
                .fetch_add(
        ...
        if ru.coprocessor_response_bytes != 0 {
            RUV2_PROMETHEUS
                .tikv_coprocessor_response_bytes
                .inc_by(ru.coprocessor_response_bytes as f64);
            self.ensure_extra()
                .tikv_coprocessor_response_bytes
        ...
        if ru.raftstore_store_write_trigger_wb_bytes != 0 {
            RUV2_PROMETHEUS
                .tikv_raftstore_store_write_trigger_wb
                .inc_by(ru.raftstore_store_write_trigger_wb_bytes as f64);
            self.ensure_extra()
                .tikv_raftstore_store_write_trigger_wb
                .fetch_add(
    ...
    /// Loads the values read by Go `FormatRUV2Summary` in one pass.
    fn formatting_snapshot(&self) -> FormattingSnapshot {
        let mut snapshot = FormattingSnapshot {
...
pub fn sync_ruv2_metrics_from_ru_details(
    ...
    update_ruv2_metrics_from_ruv2(Some(metrics), ru_details.drain_ru_v2().as_ref());
}
...
pub fn update_ruv2_metrics_from_commit_details(
    ...
    if commit_details.write_keys != 0 {
        metrics.add_write_keys(commit_details.write_keys as i64);
    }
    if commit_details.write_size != 0 {
        metrics.add_write_size(commit_details.write_size as i64);
    }
}

/// Go `ExecutorMetricRecorder`: a pre-resolved recorder for one hot L1
/// executor metric. Callers must check [`Self::available`] before recording.
...
impl ExecutorMetricRecorder {
    ...
    pub fn record(&self, m: &RuV2Metrics, delta: i64) {
        add_prometheus_i64(
            self.counter
                .as_ref()
                .expect("Record called on unavailable ExecutorMetricRecorder"),
            delta,
        );
        m.executor_l1
            .field_by_kind(self.kind)
            .expect("Record called on unavailable ExecutorMetricRecorder")
            .fetch_add(delta, Ordering::Relaxed);
    }
}
...
pub fn resolve_executor_metric(level: i64, label: &str) -> ExecutorMetricRecorder {
    ...
    let kind = exec_l1_kind_for_label(label);
    if kind == ExecL1Kind::None {
        return ExecutorMetricRecorder::default();
    }
    let Some(counter) = RUV2_PROMETHEUS.executor_counter(level, label) else {
        return ExecutorMetricRecorder::default();
...
pub fn format_ruv2_summary(
    ...
    if metrics.is_some_and(RuV2Metrics::bypass) {
        return (String::new(), String::new());
    }
    let snapshot = metrics.map_or_else(FormattingSnapshot::default, |metrics| {
        metrics.formatting_snapshot()
    });
    let tidb_ru = metrics.map_or(0.0, |metrics| {
        metrics.calculate_ru_values_with_weights(weights)
    });
    if snapshot.is_zero() && tikv_ru == 0.0 && tiflash_ru == 0.0 {
        return (String::new(), String::new());
    }
    ...
    let append_float = |parts: &mut Vec<String>, key: &str, value: f64| {
        parts.push(format!("{key}:{value:.2}"));
    };
    let append_map = |parts: &mut Vec<String>, key: &str, values: &BTreeMap<String, i64>| {
        let formatted = format_ruv2_label_map(values);
    ...
    append_float(&mut parts, "total_ru", total_ru);
    append_float(&mut parts, "tidb_ru", tidb_ru);
    append_float(&mut parts, "tikv_ru", tikv_ru);
    append_float(&mut parts, "tiflash_ru", tiflash_ru);
    append_int(
        &mut parts,
        "result_chunk_cells",
...
mod tests {
    ...
    fn default_ruv2_weights_for_test() -> RuV2Weights {
        let config = tidb_config::config_tree::RuV2Config::default();
        RuV2Weights {
            ru_scale: config.ru_scale,
            result_chunk_cells: config.result_chunk_cells,
            executor_l1: config.executor_l1,
            executor_l2: config.executor_l2,
            executor_l3: config.executor_l3,
            executor_l5_insert_rows: config.executor_l5_insert_rows,
            plan_cnt: config.plan_cnt,
            plan_derive_stats_paths: config.plan_derive_stats_paths,
            resource_manager_read_cnt: config.resource_manager_read_cnt,
            resource_manager_write_cnt: config.resource_manager_write_cnt,
            write_keys: config.write_keys,
            session_parser_total: config.session_parser_total,
            txn_cnt: config.txn_cnt,
        }
    }
    ...
    fn sync_ruv2_metrics_from_ru_details_incremental() {
        ...
        ru_details.add_ru_v2(Some(&Ruv2 {
            read_rpc_count: 2,
            write_rpc_count: 3,
            kv_engine_cache_miss: 5,
