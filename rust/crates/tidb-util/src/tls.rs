...
/// Process-wide `require_secure_transport` value.
pub static REQUIRE_SECURE_TRANSPORT: AtomicBool = AtomicBool::new(false);
...
/// Cipher names supported by TiDB.
pub static SUPPORT_CIPHER: LazyLock<HashSet<&'static str>> =
    LazyLock::new(|| TLS_CIPHER_STRING.iter().map(|(_, name)| *name).collect());
