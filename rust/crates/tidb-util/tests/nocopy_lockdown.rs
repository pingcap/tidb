...
//! Mutation receipt gate for the complete Go `pkg/util/nocopy` lockdown.
...
const PLAN: &str = include_str!("../src/nocopy/nocopy.mutation-plan.tsv");
const RESULTS: &str = include_str!("../src/nocopy/nocopy.mutation-results.tsv");
...
#[test]
fn nocopy_mutation_receipt_is_current_and_killed() {
    ...
    assert_eq!(expected_counts.values().sum::<usize>(), 8);
    ...
    for row in results {
        ...
        assert_eq!(row[3], "0474a0242a5396e40bb43b8691c29bf4ce919694");
