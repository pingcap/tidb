...
//! Mutation receipt gate for the complete Go `pkg/util/size` lockdown.
...
const PLAN: &str = include_str!("../src/size/size.mutation-plan.tsv");
const RESULTS: &str = include_str!("../src/size/size.mutation-results.tsv");
...
#[test]
fn size_mutation_receipt_is_current_and_killed() {
    ...
    assert_eq!(expected_counts.values().sum::<usize>(), 24);
    ...
    for row in results {
        ...
        assert_eq!(row[3], "88f3d872a42208925072af4d8936eec0ee67473d");
