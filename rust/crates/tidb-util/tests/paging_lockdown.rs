...
//! Mutation receipt gate for the complete Go `pkg/util/paging` lockdown.
...
const PLAN: &str = include_str!("../src/paging.mutation-plan.tsv");
const RESULTS: &str = include_str!("../src/paging.mutation-results.tsv");
...
#[test]
fn paging_mutation_receipt_is_current_and_killed() {
    ...
    assert_eq!(expected_counts.values().sum::<usize>(), 28);
    ...
    for row in results {
        ...
        assert_eq!(row[3], "4e5f29b58742c9304564a11da0271f415147c39a");
