...
use prost::Message;
use tidb_proto::{KvrpcContext, KvrpcPessimisticLockRequest, KvrpcPessimisticLockResponse};
...
pub(in crate::rpc) fn entry(
    ...
    BatchCommandEntry,
    TransactionBatchPending<KvrpcPessimisticLockResponse>,
) {
    let mut request = request.clone();
    request.context = Some(context.clone());
    TransactionBatchPending::entry(
        BatchCommandTag::PessimisticLock,
