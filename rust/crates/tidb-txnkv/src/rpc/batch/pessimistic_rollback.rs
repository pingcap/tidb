...
use prost::Message;
use tidb_proto::{KvrpcContext, KvrpcPessimisticRollbackRequest, KvrpcPessimisticRollbackResponse};
...
pub(in crate::rpc) fn entry(
    ...
    BatchCommandEntry,
    TransactionBatchPending<KvrpcPessimisticRollbackResponse>,
) {
    let mut request = request.clone();
    request.context = Some(context.clone());
    TransactionBatchPending::entry(
        BatchCommandTag::PessimisticRollback,
