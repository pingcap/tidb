...
use prost::Message;
use tidb_proto::{KvrpcContext, KvrpcTxnHeartBeatRequest, KvrpcTxnHeartBeatResponse};
...
pub(in crate::rpc) fn entry(
    ...
    BatchCommandEntry,
    TransactionBatchPending<KvrpcTxnHeartBeatResponse>,
) {
    let mut request = request.clone();
    request.context = Some(context.clone());
    TransactionBatchPending::entry(
        BatchCommandTag::TxnHeartBeat,
