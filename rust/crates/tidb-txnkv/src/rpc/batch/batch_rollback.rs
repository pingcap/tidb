...
use prost::Message;
use tidb_proto::{KvrpcBatchRollbackRequest, KvrpcBatchRollbackResponse, KvrpcContext};
...
pub(in crate::rpc) fn entry(
    ...
    BatchCommandEntry,
    TransactionBatchPending<KvrpcBatchRollbackResponse>,
) {
    let mut request = request.clone();
    request.context = Some(context.clone());
    TransactionBatchPending::entry(
        BatchCommandTag::BatchRollback,
