...
pub(crate) fn is_print(character: char) -> bool {
    ...
    if value < 1 << 16 {
        ...
        let index = IS_PRINT16.partition_point(|candidate| *candidate < value);
        if index >= IS_PRINT16.len()
            || value < IS_PRINT16[index & !1]
            || IS_PRINT16[index | 1] < value
        {
            return false;
        }
        return IS_NOT_PRINT16.binary_search(&value).is_err();
    }

    let index = IS_PRINT32.partition_point(|candidate| *candidate < value);
    if index >= IS_PRINT32.len() || value < IS_PRINT32[index & !1] || IS_PRINT32[index | 1] < value
    {
        return false;
    }
    if value >= 0x20000 {
