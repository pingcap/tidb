...
//! LogicalUnionAll identity from
//! `pkg/planner/core/operator/logicalop/logical_union_all.go` and its generated
//! Hash64/Equals implementation.
//!
//! The source identity hashes the Union plan tag and embedded
//! LogicalSchemaProducer output schema. This leaf preserves that field order
//! over a normalized column adapter; child union construction, predicate
//! pushdown, plan context, and runtime union execution remain explicit
//! external boundaries.

use crate::hash_equaler::{new_hash_equaler, Hasher, NIL_FLAG, NOT_NIL_FLAG};

/// Normalized output-column identity used by LogicalUnionAll.
#[derive(Clone, Debug, Eq, Hash, PartialEq)]
pub struct UnionColumnIdentity {
    id: i64,
    unique_id: i64,
    index: i64,
...
/// Minimal LogicalUnionAll identity and optional output schema.
#[derive(Clone, Debug, Eq, Hash, PartialEq)]
pub struct LogicalUnionAllIdentity {
    schema: Option<Vec<UnionColumnIdentity>>,
}

impl LogicalUnionAllIdentity {
    /// Creates an identity from an optional LogicalSchemaProducer schema.
    #[must_use]
    pub fn new(schema: Option<Vec<UnionColumnIdentity>>) -> Self {
        Self { schema }
    }

    /// Returns the optional output schema.
    #[must_use]
    pub fn schema(&self) -> Option<&[UnionColumnIdentity]> {
...
fn hash_column(hasher: &mut impl Hasher, column: &UnionColumnIdentity) {
