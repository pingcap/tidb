...
//! LogicalSequence identity from
//! `pkg/planner/core/operator/logicalop/logical_sequence.go` and its generated
//! Hash64/Equals implementation.
//!
//! The source operator has no attributes of its own. Its generated identity
//! hashes the `Sequence` plan tag and embedded BaseLogicalPlan ID, and equality
//! compares that same ID. This leaf preserves that identity over a normalized
//! plan ID; CTE child ordering, schema/predicate/statistics/context, and
//! runtime sequence behavior remain explicit external boundaries.

use crate::hash_equaler::{new_hash_equaler, Hasher};

/// Minimal LogicalSequence identity.
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
pub struct LogicalSequenceIdentity {
    plan_id: i64,
}

impl LogicalSequenceIdentity {
