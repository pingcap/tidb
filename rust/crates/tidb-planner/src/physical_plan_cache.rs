...
pub type DeferredExpressionEvaluator<'a> =
    dyn Fn(&Expression) -> Result<Datum, tidb_expr::EvalError> + Send + Sync + 'a;
...
impl<'a> CachedPlanRebuildContext<'a> {
    ...
    fn evaluate(&self, expression: &Expression) -> Result<Datum, tidb_expr::EvalError> {
        match self.deferred_evaluator {
            Some(evaluate) => evaluate(expression),
            None => tidb_expr::eval_expression_once(expression, self),
        }
    }
}

impl tidb_expr::Columns for CachedPlanRebuildContext<'_> {
    ...
    match expression {
        ...
        Expression::Constant(constant) => {
            ...
            if let Some(deferred) = constant.deferred_expr.as_mut() {
                ...
                let value = evaluator(deferred).map_err(|error| {
                    PlanCacheRebuildError::DeferredEvaluation(format!("{error:?}"))
                })?;
...
/// session expression context. Refresh cached values without dropping marker
/// provenance; range construction and execution still read the current context.
...
fn rebuild_table_scan(
    ...
    if let Some(handle_type) = &rebuild.handle_type {
        let result = crate::ranger::ranger::build_table_range_in(
