...
//! Go `pkg/metrics/topsql.go`: the TopSQL ignore-counter family. The Rust node has no TopSQL subsystem yet; the family registers so the dashboard query resolves, matching Go's registered-but-quiescent state.
//!
