...
- [x] (2026-08-24) Confirmation A/B on the same HEAD (4x150s): Go median 236.02, Rust median 158.35 QPS (ratio 0.6709), rust rounds clustered 153.90..160.05 with zero errors - the locking-read fix's gain is stable across runs. (ratio 0.6586) over 4x150s rounds — Rust +47% this iteration (120.14 -> 157.43), +196% across the loop. The remaining per-statement deltas are within ~0.3 ms of Go on every class in the nocost breakdown. clean A/B on origin tip — Go median 232.09, Rust median 120.14 QPS, ratio 0.5176; consistent with iteration 3 within noise, zero unexpected errors, no regressions from any landed commit.
- [x] (2026-08-24) Final fresh-restart comparison (both TiDB servers freshly started): tpmC Go 6442 vs Rust 4048 (ratio 0.628); per-statement nocost shows Rust FASTER than Go on 5/9 classes (sel_warehouse -7%, ins_orders -17%, ins_new_order -17%, ins_order_line_batch -13%, and txn average -41%). Go's upd_district (2.42ms) and commit (4.75ms) are inflated by post-restart auto-analyze; the stable-state comparison remains Go ~236 / Rust ~158 QPS (ratio ~0.67).
