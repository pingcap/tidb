# b112 gate baseline (clean tree, before edits)

Recorded 2026-08-28 on branch `testport/tikv2-b112` at `19bfb1f0` (working
tree clean), from `rust/`:

```
cargo nextest run --locked -p tidb-executor -E 'not test(/bench/)' --no-fail-fast
```

Three pre-edit runs all reported the same counts:
`1230 tests run: 1185 passed (1 slow), 45 failed, 10 skipped`
(137.164s / 136.647s / 134.750s). Failure set is stable across runs.

The 45 baseline failures (full nextest ids, lib target of `tidb-executor`):
...
- tidb-executor::driver::tests::aggregates::tpcc_condition_eleven_pushes_filters_through_nested_derived_joins
...
Post-edit gate (identical filter): `1230 tests run: 1185 passed (1 slow),
45 failed, 64 skipped` — failure set diffed IDENTICAL to the list above;
skipped went 10 -> 64 (+54 = the batch's new `#[ignore]` tests).
