# testport batch b111 — gate baseline (clean tree, before any edit)

Command: `cargo nextest run --locked -p tidb-executor -E 'not test(/bench/)' --no-fail-fast` from `rust/`, with `PROTOC=/opt/protoc/bin/protoc` (the system protoc 3.14 cannot compile tikvpb.proto's proto3 optional fields; /opt/protoc is 36.0). Run twice on the clean tree before any edit; the 48-name failure set was identical both times. (After the batch's edits the same gate ran 1355 tests: 1307 passed, the SAME 48 failed, 370 skipped — the 17 new running tests all pass and the failure set is unchanged.)
