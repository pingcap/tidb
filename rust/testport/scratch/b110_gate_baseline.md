# testport batch b110 — gate baseline (clean tree, before any edit)

Command: `cargo nextest run --locked -p tidb-executor -E 'not test(/bench/)' --no-fail-fast` from `rust/`, with `PROTOC=/opt/protoc/bin/protoc` (the system protoc 3.14 cannot compile tikvpb.proto's proto3 optional fields; /opt/protoc is 36.0).
