...
failpoints, and handler registration. The current Rust branch contains a
...
## Current Rust owner inventory and restore correction (2026-09-06)

All four tracked crate artifacts were read before the corrective edit; the
owner totals 1,615 lines and 10 inline tests after the two focused regressions.
There are no Rust fixtures, generated sources or inputs, platform variants,
benchmarks, fuzz targets, or additional build artifacts beyond the workspace
...
Ancestor commit `ea46de762b1` had removed the Rust-only return enforcement,
but restore commit `8d42bcc7035` reintroduced every annotation and deleted the
focused tests/receipt updates. On the restored current tree, the 30 Go-shaped
event constructors/getters/type queries plus `DdlNotifier::new` again emitted
31 `unused return value` diagnostics. The two deny-on-discard regressions
reproduced that exact count (`/tmp/tidb-ddl-notifier-restored-prefix.log`) and
pass after removing the annotations. The explicit `Result` contract on
`publish_schema_change_to_store` remains unchanged.

Ready evidence for the corrective Rust-only batch:

- `cargo +nightly-2026-08-22 test --manifest-path rust/Cargo.toml --offline --locked -p tidb-ddl-notifier --lib go_ -- --test-threads=1` (4 passed, including both focused regressions);
- `cargo +nightly-2026-08-22 nextest run --manifest-path rust/Cargo.toml --offline --locked -p tidb-ddl-notifier --lib --test-threads=1` (10 passed);
- `cargo +nightly-2026-08-22 check --manifest-path rust/Cargo.toml --offline --locked -p tidb-ddl-notifier --all-targets --quiet`;
- `rustfmt +nightly-2026-08-22 --check --edition 2021 rust/crates/tidb-ddl-notifier/src/event.rs rust/crates/tidb-ddl-notifier/src/subscribe.rs rust/crates/tidb-ddl-notifier/src/lib.rs`;
- `make lint`;
- `git diff --check`.

No event payload, serialization, publication, transaction, delivery ordering,
retry, rollback, or cleanup behavior changed. No Go/Bazel/Cargo/module/import
graph changed, so `make bazel_prepare` was not required; Go execution remains
outside the requested Rust-only follow-up.
...
contracts, including current-tree fail-before/pass-after evidence after the
restore regression; the rolling package audit continues.
