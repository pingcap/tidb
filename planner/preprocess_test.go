...
func (s *testValidatorSuite) TestValidator(c *C) {
	...
	tests := []struct {
		...
		{`explain format = "xx" select 100;`, false, planner.ErrUnknownExplainFormat.GenWithStackByArgs("xx")},
		...
		{"select * from ( select 1 ) a, (select 2) a;", false, planner.ErrNonUniqTable},
		{"select * from ( select 1 ) a, (select 2) b, (select 3) a;", false, planner.ErrNonUniqTable},
		{"select * from ( select 1 ) a, (select 2) b, (select 3) A;", false, planner.ErrNonUniqTable},
		{"select * from ( select 1 ) a join (select 2) b join (select 3) a;", false, planner.ErrNonUniqTable},
	...
	is := infoschema.MockInfoSchema([]*model.TableInfo{planner.MockTable()})
	for _, tt := range tests {
		stmts, err1 := session.Parse(ctx, tt.sql)
		c.Assert(err1, IsNil)
		c.Assert(stmts, HasLen, 1)
		stmt := stmts[0]
		err = planner.Preprocess(ctx, stmt, is, tt.inPrepare)
