...
func (s *testPlanSuite) SetUpSuite(c *C) {
	s.is = infoschema.MockInfoSchema([]*model.TableInfo{planner.MockTable()})
	s.Parser = parser.New()
}
...
func (s *testPlanSuite) TestDAGPlanBuilderSimpleCase(c *C) {
	...
	for i, tt := range tests {
		...
		c.Assert(err, IsNil)
		c.Assert(planner.ToString(p), Equals, tt.best, comment)
	}
}
...
func (s *testPlanSuite) TestDAGPlanBuilderBasePhysicalPlan(c *C) {
	...
	for _, tt := range tests {
		...
		planner.Preprocess(se, stmt, s.is, false)
		p, err := planner.Optimize(se, stmt, s.is)
		c.Assert(err, IsNil)
		c.Assert(planner.ToString(p), Equals, tt.best, Commentf("for %s", tt.sql))
	}
...
func (s *testPlanSuite) TestRequestTypeSupportedOff(c *C) {
	...
	p, err := planner.Optimize(se, stmt, s.is)
	c.Assert(err, IsNil)
	c.Assert(planner.ToString(p), Equals, expect, Commentf("for %s", sql))
}
