...
func (s *testPlanSuite) TestDAGPlanBuilderSimpleCase(c *C) {
	...
	tests := []struct {
		...
		},
		{
			...
			best: "IndexJoin{TableReader(Table(t)->TopN([Column#2],0,10))->TopN([Column#2],0,10)->TableReader(Table(t))}(Column#1,Column#25)->Limit",
		},
...
func (s *testPlanSuite) TestDAGPlanBuilderJoin(c *C) {
	...
	tests := []struct {
		...
		},
		{
			...
			best: "IndexJoin{TableReader(Table(t)->Limit)->Limit->TableReader(Table(t))}(Column#2,Column#13)->Limit",
		},
		{
			sql:  "select * from t t1 join t t2 on t1.b = t2.a and t1.c = 1 and t1.d = 1 and t1.e = 1 order by t1.a limit 1",
			best: "IndexJoin{IndexLookUp(Index(t.c_d_e)[[1 1 1,1 1 1]], Table(t))->TableReader(Table(t))}(Column#2,Column#13)->TopN([Column#1],0,1)",
		},
		{
		...
		},
		{
			...
			best: "IndexJoin{IndexJoin{TableReader(Table(t)->Sel([eq(Column#2, 1)]))->IndexLookUp(Index(t.c_d_e)[[1,1]], Table(t))}(Column#25,Column#1)->TableReader(Table(t))}(Column#1,Column#13)->Projection",
		},
		{
		...
		// Test Index Join + TableScan.
		{
			...
			best: "IndexJoin{TableReader(Table(t))->TableReader(Table(t))}(Column#1,Column#13)",
		},
		...
		// Test Index Join + SingleRead.
		{
			...
			best: "IndexJoin{TableReader(Table(t))->IndexReader(Index(t.c_d_e)[[NULL,+inf]])}(Column#1,Column#15)->Projection",
		},
		...
		// Test Index Join + TableScan + Rotate.
		{
			...
			best: "IndexJoin{TableReader(Table(t))->TableReader(Table(t))}(Column#15,Column#1)->Projection",
		},
		// Test Index Join + OuterJoin + TableScan.
		{
			sql:  "select /*+ TIDB_INLJ(t1, t2) */ * from t t1 left outer join t t2 on t1.a = t2.a and t2.b < 1",
			best: "IndexJoin{TableReader(Table(t))->TableReader(Table(t)->Sel([lt(Column#14, 1)]))}(Column#1,Column#13)",
		},
		{
		...
		// Test Semi Join hint success.
		{
			...
			best: "IndexJoin{TableReader(Table(t))->TableReader(Table(t))}(Column#1,Column#13)->Projection",
		},
		// Test Semi Join hint fail.
		{
			sql:  "select /*+ TIDB_INLJ(t1) */ * from t t1 where t1.a in (select a from t t2)",
			best: "IndexJoin{TableReader(Table(t))->TableReader(Table(t))}(Column#13,Column#1)->Projection",
		},
		{
		...
		},
		{
			...
			best: "IndexJoin{TableReader(Table(t))->TableReader(Table(t)->Sel([in(Column#13, 1, 2)]))}(Column#1,Column#13)",
		},
		{
...
func (s *testPlanSuite) TestDAGPlanBuilderSubquery(c *C) {
	...
	tests := []struct {
		...
		},
		{
			...
			best: "LeftHashJoin{TableReader(Table(t))->MergeInnerJoin{TableReader(Table(t))->TableReader(Table(t))}(Column#13,Column#25)->StreamAgg}(Column#1,Column#13)->Projection->Projection",
		},
		{
...
func (s *testPlanSuite) TestDAGPlanBuilderUnionScan(c *C) {
	...
	tests := []struct {
		...
		},
		{
			...
			best: "TableReader(Table(t)->Sel([eq(Column#2, 1)]))->UnionScan([eq(Column#2, 1)])",
		},
		{
			sql:  "select * from t where a = 1",
			best: "TableReader(Table(t))->UnionScan([eq(Column#1, 1)])",
		},
		{
		...
		},
		{
			...
			best: "TableReader(Table(t))->UnionScan([eq(Column#1, 1)])->Sort",
		},
		{
			sql:  "select * from t where a = 1 limit 1",
			best: "TableReader(Table(t))->UnionScan([eq(Column#1, 1)])->Limit",
		},
		{
			sql:  "select * from t where c = 1",
			best: "IndexLookUp(Index(t.c_d_e)[[1,1]], Table(t))->UnionScan([eq(Column#3, 1)])",
		},
		{
			sql:  "select c from t where c = 1",
			best: "IndexReader(Index(t.c_d_e)[[1,1]])->UnionScan([eq(Column#3, 1)])->Projection",
		},
	}
	for _, tt := range tests {
		comment := Commentf("for %s", tt.sql)
...
func (s *testPlanSuite) TestDAGPlanBuilderAgg(c *C) {
	...
	tests := []struct {
		...
		},
		{
			...
			best: "MergeLeftOuterJoin{TableReader(Table(t))->TableReader(Table(t))->Projection}(Column#1,Column#13)->Projection->Projection",
		},
		...
		// Test index join + stream agg
		{
			...
			best: "IndexJoin{IndexReader(Index(t.g)[(60,+inf]])->IndexReader(Index(t.g)[[NULL,+inf]]->Sel([gt(Column#22, 60)]))}(Column#10,Column#22)->Projection->StreamAgg->Limit->Projection",
		},
		{
...
func (s *testPlanSuite) TestRefine(c *C) {
	...
	tests := []struct {
		...
		{
			sql:  `select a from t where c_str like 'abc\\\\_'`,
			best: "IndexReader(Index(t.c_d_e_str)[(\"abc\\\",\"abc]\")]->Sel([like(Column#6, abc\\\\_, 92)]))->Projection",
		},
		{
			sql:  `select a from t where c_str like 'abc\\_%'`,
...
func (s *testPlanSuite) TestAggEliminater(c *C) {
	...
	tests := []struct {
		...
		// If inner is not a data source, we can still do transformation.
		{
			...
			best: "IndexJoin{TableReader(Table(t))->TableReader(Table(t))}(Column#1,Column#13)->Limit->StreamAgg",
		},
	}
...
func (s *testPlanSuite) TestIndexJoinUnionScan(c *C) {
	...
	tests := []struct {
		...
		// Test Index Join + UnionScan + IndexScan.
		{
			...
			best: "IndexJoin{TableReader(Table(t))->UnionScan([])->IndexReader(Index(t.c_d_e)[[NULL,+inf]])->UnionScan([])}(Column#1,Column#15)->Projection",
			is:   s.is,
		},
...
func (s *testPlanSuite) TestSemiJoinToInner(c *C) {
	...
	p, _, err := planner.Optimize(context.TODO(), se, stmt, s.is)
	c.Assert(err, IsNil)
	c.Assert(core.ToString(p), Equals, "Apply{TableReader(Table(t))->IndexJoin{IndexReader(Index(t.c_d_e)[[NULL,+inf]]->HashAgg)->HashAgg->IndexReader(Index(t.g)[[NULL,+inf]])}(Column#29,Column#23)}->HashAgg")
}
...
func (s *testPlanSuite) TestJoinHints(c *C) {
	...
	tests := []struct {
	...
	}{
		{
			...
			best:    "RightHashJoin{TableReader(Table(t))->IndexJoin{TableReader(Table(t))->TableReader(Table(t))}(Column#13,Column#1)}(Column#25,Column#13)->Projection",
			warning: "",
		},
		{
