...
func TestConstantPropagation(t *testing.T) {
	...
	// fmt.Println("explain select t.a, t.b, (select sum(c) from s where s.a = t.a) from t where t.a = 2;")
	//rs := tk.MustQuery("explain select t.a, t.b, (select sum(c) from s where s.a = t.a ) from t where t.a = 2;")
	fmt.Println("explain select t.a, t.b, sum(s.c) from t left join s on t.a=s.a where t.a = 2 group by t.a , t.b;")
	rs = tk.MustQuery("explain select t.a, t.b, sum(s.c) from t left join s on t.a=s.a where t.a = 2 group by t.a , t.b;")
	for _, row := range rs.Rows() {
		for j := range row {
			fmt.Printf("`%v    ", row[j])
		}
	...
	fmt.Println("explain select t.a, t.b, (select sum(c) from s where s.a = 2) from t where t.a = 2;")
	rs = tk.MustQuery("explain select t.a, t.b, (select sum(c) from s where s.a = 2) from t where t.a = 2;")
	for _, row := range rs.Rows() {
		for j := range row {
			fmt.Printf("`%v    ", row[j])
		}
