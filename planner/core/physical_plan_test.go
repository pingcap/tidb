...
func (s *testPlanSuite) TestDAGPlanBuilderJoin(c *C) {
	...
	tests := []struct {
		...
		},
		{
			...
			best: "LeftHashJoin{IndexJoin{TableReader(Table(t)->Sel([eq(test.t1.b, 1)]))->TableReader(Table(t))}(test.t1.a,test.t2.a)->IndexLookUp(Index(t.c_d_e)[[1,1]], Table(t))}(test.t1.a,test.t3.a)",
		},
		{
		...
		// Test Semi Join hint fail.
		{
			...
			best: "MergeSemiJoin{TableReader(Table(t))->TableReader(Table(t))}(test.t1.a,test.t2.a)",
		},
		{
...
func (s *testPlanSuite) TestDAGPlanBuilderSubquery(c *C) {
	...
	tests := []struct {
		...
		// Test join key with cast.
		{
			...
			best: "LeftHashJoin{TableReader(Table(t))->Projection->TableReader(Table(t)->StreamAgg)->StreamAgg}(cast(test.t.a),sel_agg_2)->Projection",
		},
		{
			sql:  "select * from t where exists (select s.a from t s having sum(s.a) = t.a ) order by t.a",
			best: "LeftHashJoin{TableReader(Table(t))->Projection->TableReader(Table(t)->StreamAgg)->StreamAgg}(cast(test.t.a),sel_agg_2)->Projection->Sort",
		},
		...
		// Test Nested sub query.
		{
			...
			best: "LeftHashJoin{TableReader(Table(t))->Projection->MergeSemiJoin{IndexReader(Index(t.c_d_e)[[NULL,+inf]])->IndexReader(Index(t.c_d_e)[[NULL,+inf]])}(test.s.c,test.k.c)(test.s.d,test.k.d)->Projection->StreamAgg}(cast(test.t.a),sel_agg_2)->Projection",
		},
...
func (s *testPlanSuite) TestDAGPlanBuilderAgg(c *C) {
	...
	tests := []struct {
		...
		},
		{
			...
			best: "LeftHashJoin{TableReader(Table(t))->TableReader(Table(t)->StreamAgg)->StreamAgg->Sel([ne(k, 0)])}(test.t.a,test.s.a)->Projection->Projection",
		},
