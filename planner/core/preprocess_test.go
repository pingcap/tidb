...
func (s *testValidatorSuite) TestValidator(c *C) {
	...
	tests := []struct {
		...
		{"CREATE TEMPORARY TABLE t (a INT);", false, ddl.ErrUnsupportedTempTable.GenWithStack("'CREATE TEMPORARY TABLE' is currently unsupported")},
		{"DROP TEMPORARY TABLE t;", false, ddl.ErrUnsupportedTempTable.GenWithStack("'DROP TEMPORARY TABLE' is currently unsupported")},
	}
