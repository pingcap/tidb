...
func (p *LogicalJoin) tryToGetMppHashJoin(prop *property.PhysicalProperty, useBCJ bool) []PhysicalPlan {
	...
	if p.JoinType == LeftOuterJoin || p.JoinType == RightOuterJoin {
		if useBCJ || p.ctx.GetSessionVars().MPPOuterJoinWithFixedBuildSide || len(p.OtherConditions) > 0 {
...
func (p *baseLogicalPlan) canPushToCop(storeTp kv.StoreType) bool {
	...
	for _, ch := range p.children {
		...
		case *DataSource:
			...
			ret = ret && validDs
		case *LogicalAggregation, *LogicalProjection, *LogicalSelection, *LogicalJoin, *LogicalUnionAll, *LogicalTableDual:
