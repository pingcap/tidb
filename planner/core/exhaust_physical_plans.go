...
func (p *LogicalUnionScan) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	}.Init(p.SCtx(), p.StatsInfo(), p.BlockOffset(), childProp)
...
func (p *LogicalJoin) GetMergeJoin(prop *property.PhysicalProperty, schema *expression.Schema, statsInfo *property.StatsInfo, leftStatsInfo *property.StatsInfo, rightStatsInfo *property.StatsInfo) []PhysicalPlan {
	...
	for _, lhsChildProperty := range p.leftProperties {
		...
		mergeJoin := PhysicalMergeJoin{basePhysicalJoin: baseJoin}.Init(p.SCtx(), statsInfo.ScaleByExpectCnt(prop.ExpectedCnt), p.BlockOffset())
...
func (p *LogicalJoin) getEnforcedMergeJoin(prop *property.PhysicalProperty, schema *expression.Schema, statsInfo *property.StatsInfo) []PhysicalPlan {
	...
	enforcedPhysicalMergeJoin := PhysicalMergeJoin{basePhysicalJoin: baseJoin, Desc: desc}.Init(p.SCtx(), statsInfo.ScaleByExpectCnt(prop.ExpectedCnt), p.BlockOffset())
...
func (p *LogicalJoin) constructIndexJoin(
	...
	}.Init(p.SCtx(), p.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), p.BlockOffset(), chReqProps...)
	if path != nil {
...
func (p *LogicalJoin) constructInnerTableScanTask(
	...
	if len(ts.filterCondition) > 0 {
		var err error
		selectivity, _, err = ds.tableStats.HistColl.Selectivity(ds.SCtx(), ts.filterCondition, ds.possibleAccessPaths)
...
func (*LogicalJoin) constructInnerSel(sel *LogicalSelection, child PhysicalPlan) PhysicalPlan {
	...
	}.Init(sel.SCtx(), sel.StatsInfo(), sel.BlockOffset(), nil)
...
func (*LogicalJoin) constructInnerProj(proj *LogicalProjection, child PhysicalPlan) PhysicalPlan {
	...
	}.Init(proj.SCtx(), proj.StatsInfo(), proj.BlockOffset(), nil)
...
func (*LogicalJoin) constructInnerUnionScan(us *LogicalUnionScan, reader PhysicalPlan) PhysicalPlan {
	...
	}.Init(us.SCtx(), reader.StatsInfo(), us.BlockOffset(), nil)
...
func (p *LogicalJoin) constructInnerIndexScanTask(
	...
	if len(tblConds) > 0 {
		selectivity, _, err := ds.tableStats.HistColl.Selectivity(ds.SCtx(), tblConds, ds.possibleAccessPaths)
		if err != nil || selectivity <= 0 {
	...
	if len(indexConds) > 0 {
		selectivity, _, err := ds.tableStats.HistColl.Selectivity(ds.SCtx(), indexConds, ds.possibleAccessPaths)
		if err != nil || selectivity <= 0 {
			logutil.BgLogger().Debug("unexpected selectivity, use selection factor", zap.Float64("selectivity", selectivity), zap.String("table", ds.TableAsName.L))
			selectivity = SelectionFactor
		}
...
func (p *LogicalJoin) tryToGetMppHashJoin(prop *property.PhysicalProperty, useBCJ bool) []PhysicalPlan {
	...
	}.Init(p.SCtx(), p.StatsInfo(), p.BlockOffset(), childrenProps...)
	join.SetSchema(p.schema)
	return []PhysicalPlan{join}
}
...
func (p *LogicalExpand) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	if p.SCtx().GetSessionVars().IsMPPAllowed() {
		...
		}.Init(p.SCtx(), p.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), p.BlockOffset(), mppProp)
...
func (p *LogicalProjection) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	for _, newProp := range newProps {
		...
		}.Init(p.SCtx(), p.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), p.BlockOffset(), newProp)
...
func (lt *LogicalTopN) getPhysTopN(prop *property.PhysicalProperty) []PhysicalPlan {
	...
	for _, tp := range allTaskTypes {
		...
		topN := PhysicalTopN{
			...
			Count:       lt.Count,
			Offset:      lt.Offset,
		}.Init(lt.SCtx(), lt.StatsInfo(), lt.BlockOffset(), resultProp)
		ret = append(ret, topN)
	}
	return ret
}
...
func (lw *LogicalWindow) tryToGetMppWindows(prop *property.PhysicalProperty) []PhysicalPlan {
	...
	window := PhysicalWindow{
		...
		Frame:           lw.Frame,
		storeTp:         kv.TiFlash,
	}.Init(lw.SCtx(), lw.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), lw.BlockOffset(), childProperty)
...
func (la *LogicalAggregation) getEnforcedStreamAggs(prop *property.PhysicalProperty) []PhysicalPlan {
	...
	for _, taskTp := range taskTypes {
		...
		*copiedChildProperty = *childProp // It's ok to not deep copy the "cols" field.
		...
		}.initForStream(la.SCtx(), la.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), la.BlockOffset(), copiedChildProperty)
...
func (p *LogicalLimit) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	for _, tp := range allTaskTypes {
		...
		}.Init(p.SCtx(), p.StatsInfo(), p.BlockOffset(), resultProp)
...
func (p *LogicalUnionAll) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	if canUseMpp && prop.TaskTp == property.RootTaskType {
		...
		mppUA := PhysicalUnionAll{mpp: true}.Init(p.SCtx(), p.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), p.BlockOffset(), chReqProps...)
...
func (ls *LogicalSort) getPhysicalSort(prop *property.PhysicalProperty) *PhysicalSort {
	ps := PhysicalSort{ByItems: ls.ByItems}.Init(ls.SCtx(), ls.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), ls.BlockOffset(), &property.PhysicalProperty{TaskTp: prop.TaskTp, ExpectedCnt: math.MaxFloat64, RejectSort: true, CTEProducerStatus: prop.CTEProducerStatus})
	return ps
}
...
func (ls *LogicalSort) getNominalSort(reqProp *property.PhysicalProperty) *NominalSort {
	...
	ps := NominalSort{OnlyColumn: onlyColumn, ByItems: ls.ByItems}.Init(
		ls.SCtx(), ls.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), ls.BlockOffset(), prop)
	return ps
}
	...
	if prop.TaskTp == property.RootTaskType {
		...
		if ls.canPushToCopImpl(kv.TiFlash, true) {
			...
			ps := NominalSort{OnlyColumn: true, ByItems: ls.ByItems}.Init(
				ls.SCtx(), ls.StatsInfo().ScaleByExpectCnt(prop.ExpectedCnt), ls.BlockOffset(), newProp)
...
func (p *LogicalMaxOneRow) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	mor := PhysicalMaxOneRow{}.Init(p.SCtx(), p.StatsInfo(), p.BlockOffset(), &property.PhysicalProperty{ExpectedCnt: 2, CTEProducerStatus: prop.CTEProducerStatus})
...
func (p *LogicalSequence) exhaustPhysicalPlans(prop *property.PhysicalProperty) ([]PhysicalPlan, bool, error) {
	...
	for _, propChoice := range possibleChildrenProps {
		...
		seq := PhysicalSequence{}.Init(p.SCtx(), p.StatsInfo(), p.BlockOffset(), childReqs...)
