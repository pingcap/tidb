...
// TestIssue27167 is just want to test some problem, omit this
func (s *testIntegrationSerialSuite) TestIssue27167(c *C) {
	...
	tk.MustQuery("select collation(c) from (select d_date c from all_types union select d_int c from all_types) t").Check(testkit.Rows("binary", "binary"))
}
