...
func (s *testIntegrationSuite) TestApproxCountDistinctInPartitionTable(c *C) {
	...
	tk.MustExec(`set @@tidb_partition_prune_mode='` + string(variable.StaticOnly) + `'`)
	tk.MustQuery("explain select approx_count_distinct(a), b from t group by b order by b desc").Check(testkit.Rows("Sort_12 16000.00 root  schema:[Column#4 test.t.b], items:test.t.b:desc",
