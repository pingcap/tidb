...
func (s *testIntegrationSerialSuite) TestNoneAccessPathsFoundByIsolationRead(c *C) {
	...
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "[planner:1815]Internal : No access path for table 't' is found with 'tidb_isolation_read_engines' = 'tiflash', valid value can be 'tikv'. Please check tiflash replica or ensure the query is readonly.")
...
func (s *testIntegrationSerialSuite) TestNotReadOnlySQLOnTiFlash(c *C) {
	...
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, `[planner:1815]Internal : No access path for table 't' is found with 'tidb_isolation_read_engines' = 'tiflash', valid value can be 'tiflash, tikv'. Please check tiflash replica or ensure the query is readonly.`)
...
func (s *testIntegrationSuite) TestCreateViewIsolationRead(c *C) {
	...
	tk.MustGetErrMsg("select * from v0;", "[planner:1815]Internal : No access path for table 't' is found with 'tidb_isolation_read_engines' = 'tiflash,tidb', valid value can be 'tikv'.")
