...
func (s *testIntegrationSuite) TestIssue19926(c *C) {
	...
	tk.MustExec("CREATE TABLE `ta`  (  `id` varchar(36) NOT NULL ,  `status` varchar(1) NOT NULL );")
	tk.MustExec("CREATE TABLE `tb`  (  `id` varchar(36) NOT NULL ,  `status` varchar(1) NOT NULL );")
	tk.MustExec("CREATE TABLE `tc`  (  `id` varchar(36) NOT NULL ,  `status` varchar(1) NOT NULL );")
	tk.MustExec("insert into ta values('1','1');")
	tk.MustExec("insert into tb values('1','1');")
	tk.MustExec("insert into tc values('1','1');")
	tk.MustExec("create definer='root'@'localhost' view v as select concat(`ta`.`status`,`tb`.`status`) AS `status`, `ta`.`id` AS `id`  from (`ta` join `tb`) where (`ta`.`id` = `tb`.`id`);")
