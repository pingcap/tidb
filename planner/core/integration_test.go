...
func TestOctIntOrStringPushDownToTiFlash(t *testing.T) {
	store, dom := testkit.CreateMockStoreAndDomain(t)
	tk := testkit.NewTestKit(t, store)
	...
	rows := [][]interface{}{
		{"TableReader_9", "root", "data:ExchangeSender_8"},
		{"└─ExchangeSender_8", "mpp[tiflash]", "ExchangeType: PassThrough"},
		{"  └─Projection_4", "mpp[tiflash]", "oct(test.t.a)->Column#4"},
		{"    └─TableFullScan_7", "mpp[tiflash]", "keep order:false, stats:pseudo"},
	}
	tk.MustQuery("explain select oct(a) from t;").CheckAt([]int{0, 2, 4}, rows)

	rows = [][]interface{}{
		{"TableReader_9", "root", "data:ExchangeSender_8"},
		{"└─ExchangeSender_8", "mpp[tiflash]", "ExchangeType: PassThrough"},
		{"  └─Projection_4", "mpp[tiflash]", "oct(test.t.b)->Column#4"},
		{"    └─TableFullScan_7", "mpp[tiflash]", "keep order:false, stats:pseudo"},
	}
	tk.MustQuery("explain select oct(b) from t;").CheckAt([]int{0, 2, 4}, rows)
}
