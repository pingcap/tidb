...
func (s *testAnalyzeSuite) TestCorrelatedEstimation(c *C) {
	...
	tk.MustQuery("explain select t.c in (select count(*) from t s , t t1 where s.a = t.a and s.a = t1.a) from t;").
		Check(testkit.Rows(
			"Projection_12 10.00 root 9_aux_0",
			"└─Apply_14 10.00 root left outer semi join, inner:Selection_17, equal:[eq(test.t.c, 7_col_0)]",
			"  ├─TableReader_16 10.00 root data:TableScan_15",
			"  │ └─TableScan_15 10.00 cop table:t, range:[-inf,+inf], keep order:false",
			"  └─Selection_17 0.80 root not(isnull(7_col_0))",
			"    └─StreamAgg_22 1.00 root funcs:count(1)",
			"      └─HashLeftJoin_23 1.00 root inner join, inner:TableReader_30, equal:[eq(s.a, t1.a)]",
			"        ├─TableReader_27 1.00 root data:Selection_26",
			"        │ └─Selection_26 1.00 cop eq(s.a, test.t.a), not(isnull(s.a))",
			"        │   └─TableScan_25 10.00 cop table:s, range:[-inf,+inf], keep order:false",
			"        └─TableReader_30 1.00 root data:Selection_29",
			"          └─Selection_29 1.00 cop eq(t1.a, test.t.a), not(isnull(t1.a))",
			"            └─TableScan_28 10.00 cop table:t1, range:[-inf,+inf], keep order:false",
		))
