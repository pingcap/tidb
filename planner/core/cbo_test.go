...
func (s *testAnalyzeSuite) TestInconsistentEstimation(c *C) {
	...
	tk.MustQuery("explain select * from t use index(ab) where a = 5 and c = 5").
		Check(testkit.Rows(
			...
			"├─IndexRangeScan_6(Build) 12.50 cop[tikv] table:t, index:a, b, range:[5,5], keep order:false",
			"└─Selection_8(Probe) 10.00 cop[tikv] eq(test.t.c, 5)",
			"  └─TableRowIDScan_7 12.50 cop[tikv] table:t, keep order:false",
		))
}
