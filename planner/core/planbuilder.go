...
func (b *PlanBuilder) buildShow(ctx context.Context, show *ast.ShowStmt) (Plan, error) {
	...
	mockTablePlan := LogicalTableDual{placeHolder: true}.Init(b.ctx, b.getSelectOffset())
	...
	if np != mockTablePlan {
		...
		proj := LogicalProjection{Exprs: make([]expression.Expression, 0, fieldsLen)}.Init(b.ctx, b.getSelectOffset())
