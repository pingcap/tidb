...
func (b *PlanBuilder) buildSetConfig(ctx context.Context, v *ast.SetConfigStmt) (Plan, error) {
	...
	mockTablePlan := LogicalDualScan{}.Init(b.ctx, b.getSelectOffset())
...
func (b *PlanBuilder) buildDo(ctx context.Context, v *ast.DoStmt) (Plan, error) {
	var p LogicalPlan
	dual := LogicalDualScan{RowCount: 1}.Init(b.ctx, b.getSelectOffset())
	dual.SetSchema(expression.NewSchema())
	p = dual
	proj := LogicalProject{Exprs: make([]expression.Expression, 0, len(v.Exprs))}.Init(b.ctx, b.getSelectOffset())
...
func (b *PlanBuilder) buildPhysicalIndexLookupGather(ctx context.Context, dbName model.CIStr, tbl table.Table, idx *model.IndexInfo) (Plan, error) {
	tblInfo := tbl.Meta()
...
func (b *PlanBuilder) buildPhysicalIndexLookupGathers(ctx context.Context, dbName model.CIStr, tbl table.Table, indices []table.Index) ([]Plan, []*model.IndexInfo, error) {
	tblInfo := tbl.Meta()
	...
	for _, idx := range indices {
		...
		if pi := tbl.Meta().GetPartitionInfo(); pi != nil {
			for _, def := range pi.Definitions {
				...
				reader, err := b.buildPhysicalIndexLookupGather(ctx, dbName, t, idxInfo)
				if err != nil {
					return nil, nil, err
				}
		...
		reader, err := b.buildPhysicalIndexLookupGather(ctx, dbName, tbl, idxInfo)
		if err != nil {
			return nil, nil, err
		}
		indexLookUpReaders = append(indexLookUpReaders, reader)
...
func (b *PlanBuilder) buildAdminCheckTable(ctx context.Context, as *ast.AdminStmt) (*CheckTable, error) {
	...
	if as.Tp == ast.AdminCheckIndex {
		...
		p.CheckIndex = true
		readerPlans, indexInfos, err = b.buildPhysicalIndexLookupGathers(ctx, tblName.Schema, tbl, []table.Index{idx})
	} else {
		readerPlans, indexInfos, err = b.buildPhysicalIndexLookupGathers(ctx, tblName.Schema, tbl, tbl.Indices())
	}
	if err != nil {
		return nil, errors.Trace(err)
	}
	readers := make([]*PhysicalIndexLookupGather, 0, len(readerPlans))
	for _, plan := range readerPlans {
		readers = append(readers, plan.(*PhysicalIndexLookupGather))
	}
...
func (b *PlanBuilder) buildShow(ctx context.Context, show *ast.ShowStmt) (Plan, error) {
	...
	if np != p {
		...
		proj := LogicalProject{Exprs: make([]expression.Expression, 0, fieldsLen)}.Init(b.ctx, 0)
...
func (b *PlanBuilder) buildSetValuesOfInsert(ctx context.Context, insert *ast.InsertStmt, insertPlan *Insert, mockTablePlan *LogicalDualScan, checkRefColumn func(n ast.Node) ast.Node) error {
	...
	for i, assign := range insert.Setlist {
		...
		if _, ok := assign.Expr.(*ast.SubqueryExpr); ok {
			usingPlan = LogicalDualScan{}.Init(b.ctx, b.getSelectOffset())
		}
...
func (b *PlanBuilder) buildValuesListOfInsert(ctx context.Context, insert *ast.InsertStmt, insertPlan *Insert, mockTablePlan *LogicalDualScan, checkRefColumn func(n ast.Node) ast.Node) error {
...
func convertValueListToData(valueList []ast.ExprNode, handleColInfos []*model.ColumnInfo, rowIdx int,
	b *PlanBuilder, mockTablePlan *LogicalDualScan) ([]types.Datum, error) {
