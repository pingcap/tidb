...
func (ds *DataSource) fillRangesInIndexPath(path *util.AccessPath) error {
	sc := ds.ctx.GetSessionVars().StmtCtx
...
func (ds *DataSource) deriveIndexPathStats(path *util.AccessPath) (bool, error) {
	...
	if path.EqOrInCondCount == len(path.AccessConds) {
		accesses, remained := SplitCorColAccessCondFromFilters(path, path.EqOrInCondCount)
	...
	if path.IndexFilters != nil {
		selectivity, _, err := ds.stats.HistColl.Selectivity(ds.ctx, path.IndexFilters, nil)
		if err != nil {
...
func SplitCorColAccessCondFromFilters(path *util.AccessPath, eqOrInCount int) (access, remained []expression.Expression) {
