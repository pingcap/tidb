...
// accessPath tells how we access one index, just access table, or access table with multiple indexes.
type accessPath struct {
	...
	forced bool
	// isIndexMerge indicates whether this path use multiple indexes to access a table.
	isIndexMergePath bool
	...
	// indexMergeType indicates the type of IndexMergePath.
	indexMergeType int
}
...
func (ds *DataSource) deriveIndexPathStats(path *accessPath, considerConditions []expression.Expression) (bool, error) {
	...
	if len(path.idxCols) != 0 {
		res, err := ranger.DetachCondAndBuildRangeForIndex(ds.ctx, considerConditions, path.idxCols, path.idxColLens)
		if err != nil {
			return false, err
		}
		...
		path.tableFilters = considerConditions
	}
