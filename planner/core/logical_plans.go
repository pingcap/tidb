...
func (ds *DataSource) deriveTablePathStats(path *util.AccessPath) (bool, error) {
	var err error
...
func (ds *DataSource) fillRangesForIndexPath(path *util.AccessPath) error {
	sc := ds.ctx.GetSessionVars().StmtCtx
	...
	if len(path.IdxCols) != 0 {
		...
		path.EqOrInCount = res.EqOrInCount
...
func (ds *DataSource) deriveIndexPathStats(path *util.AccessPath) (bool, error) {
	sc := ds.ctx.GetSessionVars().StmtCtx

	if path.EqOrInCount == len(path.AccessConds) {
		accesses, remained := path.SplitCorColAccessCondFromFilters(path.EqOrInCount)
		...
		if len(accesses) > 0 && ds.statisticTable.Pseudo {
			...
			for i := range accesses {
				col := path.IdxCols[path.EqOrInCount+i]
	...
	if path.IndexFilters != nil {
		selectivity, _, err := ds.tableStats.HistColl.Selectivity(ds.ctx, ds.getHandleCol(), path.IndexFilters, nil)
		if err != nil {
