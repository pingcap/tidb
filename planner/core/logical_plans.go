...
func (ds *DataSource) deriveIndexPathStats(path *util.AccessPath) (bool, error) {
	...
	if path.EqCondCount == len(path.AccessConds) {
		accesses, remained := splitCorColAccessCondFromFilters(path)
	...
	if path.IndexFilters != nil {
		selectivity, err := ds.stats.HistColl.Selectivity(ds.ctx, path.IndexFilters, nil)
		if err != nil {
...
func splitCorColAccessCondFromFilters(path *util.AccessPath) (access, remained []expression.Expression) {
