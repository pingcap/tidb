...
func (p *PhysicalWindow) attach2TaskForMPP(mpp *mppTask) task {
	...
	failpoint.Inject("CheckWindowSchemaLength", func() {
		if len(p.Schema().Columns) <= len(mpp.plan().Schema().Columns) {
			panic("mpp physical window's schema should be longer than sub operator's schema.")
		}
	})
