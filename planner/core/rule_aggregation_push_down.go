...
func (a *aggregationPushDownSolver) tryToPushDownAgg(aggFuncs []*aggregation.AggFuncDesc, gbyCols []*expression.Column, join *LogicalJoin, childIdx int, blockOffset int) (_ LogicalPlan, err error) {
	...
	agg, err := a.makeNewAgg(join.ctx, aggFuncs, gbyCols, blockOffset)
	if err != nil {
		return nil, err
	}
...
func (a *aggregationPushDownSolver) makeNewAgg(ctx sessionctx.Context, aggFuncs []*aggregation.AggFuncDesc, gbyCols []*expression.Column, blockOffset int) (*LogicalAggregation, error) {
			...
			if join, ok1 := child.(*LogicalJoin); ok1 && a.checkValidJoin(join) {
				if valid, leftAggFuncs, rightAggFuncs, leftGbyCols, rightGbyCols := a.splitAggFuncsAndGbyCols(agg, join); valid {
					...
					if rightInvalid {
						...
						rChild, err = a.tryToPushDownAgg(rightAggFuncs, rightGbyCols, join, 1, agg.blockOffset)
						if err != nil {
							return nil, err
						}
					}
					if leftInvalid {
						...
						lChild, err = a.tryToPushDownAgg(leftAggFuncs, leftGbyCols, join, 0, agg.blockOffset)
						if err != nil {
							return nil, err
						}
					}
				...
				pushedAgg, err := a.makeNewAgg(agg.ctx, agg.AggFuncs, gbyCols, agg.blockOffset)
				if err != nil {
					return nil, err
				}
