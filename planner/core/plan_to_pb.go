...
func (p *PhysicalHashAgg) ToPB(ctx sessionctx.Context, storeType kv.StoreType) (*tipb.Executor, error) {
	...
	for _, aggFunc := range p.AggFuncs {
		isMppFinal := false
		if aggFunc.Mode == aggregation.FinalMode && p.MppRunMode == Mpp2Phase {
			isMppFinal = true
		}
		aggExec.AggFunc = append(aggExec.AggFunc, aggregation.AggFuncToPBExpr(sc, client, aggFunc, isMppFinal))
	}
	executorID := ""
...
func (p *PhysicalStreamAgg) ToPB(ctx sessionctx.Context, storeType kv.StoreType) (*tipb.Executor, error) {
	...
	for _, aggFunc := range p.AggFuncs {
		aggExec.AggFunc = append(aggExec.AggFunc, aggregation.AggFuncToPBExpr(sc, client, aggFunc, false))
	}
	executorID := ""
	if storeType == kv.TiFlash {
		var err error
