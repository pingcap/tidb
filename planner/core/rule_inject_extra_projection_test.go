...
func (s *testInjectProjSuite) TestWrapCastForAggFuncs(c *C) {
			...
			for _, mode := range modes {
				for _, retType := range retTypes {
					...
					aggFunc, err := aggregation.NewAggFuncDesc(sctx, name,
						[]expression.Expression{&expression.Constant{Value: types.Datum{}, RetType: types.NewFieldTypeBuilder(retType)}},
						hasDistinct)
					c.Assert(err, IsNil)
					aggFunc.Mode = mode
	...
	for i := range aggFuncs {
		if aggFuncs[i].Mode != aggregation.FinalMode && aggFuncs[i].Mode != aggregation.Partial2Mode {
			c.Assert(aggFuncs[i].RetTp.Tp, Equals, aggFuncs[i].Args[0].GetType().GetTp())
		} else {
			c.Assert(aggFuncs[i].Args[0].GetType().Tp, Equals, orgAggFuncs[i].Args[0].GetType().GetTp())
		}
	}
}
