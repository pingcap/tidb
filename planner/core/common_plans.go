...
func (e *Execute) rebuildRange(p Plan) error {
	...
	case *PointGetPlan:
		...
		if x.AccessCondition != nil {
			if x.IndexInfo != nil {
				ranges, err := ranger.DetachCondAndBuildRangeForIndex(x.ctx, []expression.Expression{x.AccessCondition}, x.IdxCols, x.IdxColLens)
				if err != nil {
					return err
				}
				...
				if pkCol != nil {
					ranges, err := ranger.BuildTableRange([]expression.Expression{x.AccessCondition}, x.ctx.GetSessionVars().StmtCtx, pkCol.RetType)
					if err != nil {
						return err
					}
