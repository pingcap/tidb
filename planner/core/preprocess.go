...
func (p *preprocessor) checkBindGrammar(createBindingStmt *ast.CreateBindingStmt) {
	...
	if originSQL != hintedSQL {
		p.err = errors.New(fmt.Sprint("hinted sql and origin sql don't match when hinted sql erase the hint info, after erase hint info, originSQL:", originSQL, ", hintedSQL:", hintedSQL))
	}
}
