...
func TestNewCostInterfaceTiKV(t *testing.T) {
	...
	tk.MustExec(`create table t (a int primary key, b int, c int, d int, k int, key b(b), key cd(c, d), unique index(k))`)
	...
	queries := []string{
		...
		// point get
		"select * from t where a = 1", // generated by FastPlan
		"select * from t where a in (1, 2, 3, 4, 5)",
		"select * from t where k = 1",
		"select * from t where k in (1, 2, 3, 4, 5)",
		"select * from t where a=1 and mod(a, b)=2", // generated by Physical Optimization
