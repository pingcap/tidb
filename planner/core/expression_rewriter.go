...
func (er *expressionRewriter) buildSubquery(ctx context.Context, subq *ast.SubqueryExpr) (LogicalPlan, error) {
	...
	// when in the outer analyzing phase, we may still need to down-forward build entire sub-query tree out. (temporarily shut
	// this off, otherwise rewrite sub-query won't build the plan tree)
...
func (er *expressionRewriter) buildQuantifierPlan(plan4Agg *LogicalAggregation, cond, lexpr, rexpr expression.Expression, all bool) {
	...
	if all {
		...
		// should be rewritten to t.id < min(s.id) and if(sum(s.id is null) != 0, null, true).
...
// handleEQAll handles the case of = all. For example, if the query is t.id = all (select s.id from s), it will be rewritten to
