...
func (s *testPlanSuite) TestExplainNonEvaledSubquery(c *C) {
	...
	for i, ts := range input {
		s.testData.OnRecord(func() {
			...
			if ts.HasErr {
				...
				c.Assert(err, IsNil, Commentf("Failed at case #%d", i))
				...
				output[i].Plan = s.testData.ConvertRowsToStrings(rows)
				output[i].Error = ""
			}
		})
		if ts.HasErr {
			...
			compareStringSlice(c, s.testData.ConvertRowsToStrings(rows), s.testData.ConvertRowsToStrings(testkit.Rows(output[i].Plan...)))
		}
	}
}
