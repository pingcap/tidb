...
func (s *testPlanSuite) TestPredicatePushDown(c *C) {
	...
	for ith, ca := range input {
		...
		p, _, err := BuildTestLogicalPlan(ctx, s.ctx, stmt, s.is)
		c.Assert(err, IsNil)
...
func buildLogicPlan4GroupBy(s *testPlanSuite, c *C, sql string) (Plan, error) {
	...
	p, _, err := BuildTestLogicalPlan(context.Background(), s.ctx, stmt, s.is)
	return p, err
}
...
func (s *testPlanSuite) TestTablePartition(c *C) {
	...
	for i, ca := range input {
		...
		p, _, err := BuildTestLogicalPlan(ctx, s.ctx, stmt, isChoices[ca.IsIdx])
		c.Assert(err, IsNil)
...
func (s *testPlanSuite) TestValidate(c *C) {
	...
	for _, tt := range tests {
		...
		err = Preprocess(s.ctx, stmt, WithPreprocessorReturn(&PreprocessorReturn{InfoSchema: s.is}))
		c.Assert(err, IsNil)
		_, _, err = BuildTestLogicalPlan(ctx, s.ctx, stmt, s.is)
		if tt.err == nil {
			c.Assert(err, IsNil, comment)
		} else {
