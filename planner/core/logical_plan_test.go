...
func (s *testPlanSuite) TestAggPrune(c *C) {
	...
	tests := []struct {
		...
		},
		{
			...
			best: "DataScan(t)->Aggr(max(test.t.c),sum(test.t.d))->Projection",
		},
		{
			sql:  "select max(ct), sum(sum_d) from (select b, c+1 as ct, sum(d) as sum_d from t group by b, c+1) tmp group by b",
			best: "DataScan(t)->Aggr(max(plus(test.t.c, 1)),sum(test.t.d))->Projection",
		},
		{
			sql:  "select max(ct), sum(sum_d) from (select b, min(c+1) as ct, sum(d) as sum_d from t group by b, c+1) tmp group by b;",
			best: "DataScan(t)->Aggr(max(plus(test.t.c, 1)),sum(test.t.d))->Projection",
		},
		{
		...
		},
		{
			...
			best: "DataScan(t)->Aggr(max(test.t.c),firstrow(test.t.b))->Projection",
		},
		{
			sql:  "select bt, max(ct) from (select b as bt, max(c) as ct from t group by c, b order by ct) tt group by bt",
			best: "DataScan(t)->Aggr(max(test.t.c),firstrow(test.t.b))->Projection",
		},
		{
		...
		},
		{
			sql: `select v.a, v.b, max(v.sm), sum(v.sm)
					from (select t1.a, t1.b, sum(t2.f) as sm
						from   t as t1, t as t2
						where  t1.d > 3
						and    t1.e_str = t2.e_str
						and    t2.c > 10
		...
		},
		{
			sql: `select u.a, u.b, min(u.mn), sum(u.sm)
					from (select v.a, v.b, v.c, min(mx) as mn, sum(v.sm) as sm
						from (select t1.a, t1.b, t1.c, t1.d, max(t1.c) as mx, max(t2.d) as mx2, sum(t2.f) as sm
							from   t as t1, t as t2
							where  t1.d > 3
							and    t1.e_str = t2.e_str
							and    t2.c > 10
		...
		{
			// limit between aggregations
			sql:  "select bt, max(ct) from (select b as bt, max(c) as ct from t group by c, b limit 5) tt group by bt",
			best: "DataScan(t)->Aggr(max(test.t.c),firstrow(test.t.b))->Limit->Aggr(max(ct),firstrow(test.t.bt))->Projection",
		},
		{
			// selections between aggregations
			sql:  "select bt, max(ct) from (select b as bt, max(c) as ct from t group by c, b having ct > 0) tt group by bt",
			best: "DataScan(t)->Aggr(max(test.t.c),firstrow(test.t.b))->Sel([gt(ct, 0)])->Aggr(max(ct),firstrow(test.t.bt))->Projection",
		},
		{
			// selections between aggregations
			sql:  "select bt, max(ct) from (select b as bt, max(c) as ct from t group by c, b) tt where ct > 0 group by ct",
			best: "DataScan(t)->Aggr(max(test.t.c),firstrow(test.t.b))->Sel([gt(ct, 0)])->Aggr(max(ct),firstrow(test.t.bt))->Projection",
		},
		{
		...
		{
			// max(sum(_))
			sql:  "select max(sum_c), sum(sum_d) from (select b, sum(c) as sum_c, sum(d) as sum_d from t group by b, c) tmp group by b",
			best: "DataScan(t)->Aggr(sum(test.t.c),sum(test.t.d),firstrow(test.t.b))->Aggr(max(sum_c),sum(sum_d))->Projection",
		},
		{
			// max(max(_)+1)
