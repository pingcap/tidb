...
func (s *testPlanSuite) TestNameResolver(c *C) {
	...
	tests := []struct {
		...
		{"select a from t", ""},
		{"select c3 from t", "[DB:planner:1054] Unknown column 'c3' in 'field list'"},
		{"select c1 from t4", "[DB:schema:1146] Table 'test.t4' doesn't exist"},
		{"select * from t", ""},
		{"select t.* from t", ""},
		{"select t2.* from t", "[DB:planner:1051] Unknown table 't2'"},
		{"select b as a, c as a from t group by a", "[DB:planner:1052] Column 'c' in field list is ambiguous"},
		...
		{"select * from t as t1, t as t2 join t as t3 on t1.c1 = t2.a", "[DB:planner:1054] Unknown column 't1.c1' in 'on clause'"},
		{"select a from t group by a having a = 3", ""},
		{"select a from t group by a having c2 = 3", "[DB:planner:1054] Unknown column 'c2' in 'having clause'"},
		{"select a from t where exists (select b)", ""},
		{"select cnt from (select count(a) as cnt from t group by b) as t2 group by cnt", ""},
		{"select a from t where t11.a < t.a", "[DB:planner:1054] Unknown column 't11.a' in 'where clause'"},
		{"select a from t having t11.c1 < t.a", "[DB:planner:1054] Unknown column 't11.c1' in 'having clause'"},
		{"select a from t where t.a < t.a order by t11.c1", "[DB:planner:1054] Unknown column 't11.c1' in 'order clause'"},
		{"select a from t group by t11.c1", "[DB:planner:1054] Unknown column 't11.c1' in 'group statement'"},
		{"delete a from (select * from t ) as a, t", "[DB:planner:1288] The target table a of the DELETE is not updatable"},
		{"delete b from (select * from t ) as a, t", "[DB:planner:1109] Unknown table 'b' in MULTI DELETE"},
		{"select '' as fakeCol from t group by values(fakeCol)", "[DB:planner:1054] Unknown column '' in 'VALUES() function'"},
		{"update t, (select * from ht) as b set b.a = t.a", "[DB:planner:1288] The target table b of the UPDATE is not updatable"},
		{"select row_number() over () from t group by 1", "[DB:planner:1056] Can't group on 'row_number() over ()'"},
	}
