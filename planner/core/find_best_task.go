...
func (p *LogicalDualScan) findBestTask(prop *property.PhysicalProperty, planCounter *PlanCounterTp) (task, int64, error) {
...
func (p *LogicalMemTableScan) findBestTask(prop *property.PhysicalProperty, planCounter *PlanCounterTp) (t task, cntPlan int64, err error) {
	if !prop.IsEmpty() || planCounter.Empty() {
		return invalidTask, 0, nil
	}
...
func (ds *DataSource) tryToGetDualTask() (task, error) {
	for _, cond := range ds.pushedDownConds {
		if con, ok := cond.(*expression.Constant); ok && con.DeferredExpr == nil && con.ParamMarker == nil {
			...
			if !result {
				dual := PhysicalDualScan{}.Init(ds.ctx, ds.stats, ds.blockOffset)
...
func (ds *DataSource) convertToPartialIndexScan(prop *property.PhysicalProperty, path *util.AccessPath) (
	...
	if indexConds != nil {
		...
		indexPlan := PhysicalFilter{Conditions: indexConds}.Init(is.ctx, stats, ds.blockOffset)
...
func (ds *DataSource) convertToPartialTableScan(prop *property.PhysicalProperty, path *util.AccessPath) (
	...
	if len(ts.filterCondition) > 0 {
		...
		tablePlan = PhysicalFilter{Conditions: ts.filterCondition}.Init(ts.ctx, ts.stats.ScaleByExpectCnt(selectivity*rowCount), ds.blockOffset)
...
func (ds *DataSource) buildIndexMergeTableScan(prop *property.PhysicalProperty, tableFilters []expression.Expression, totalRowCount float64) (PhysicalPlan, float64, error) {
	...
	if len(tableFilters) > 0 {
		...
		sel := PhysicalFilter{Conditions: tableFilters}.Init(ts.ctx, ts.stats.ScaleByExpectCnt(selectivity*totalRowCount), ts.blockOffset)
		sel.SetChildren(ts)
...
func (is *PhysicalIndexScan) addPushedDownSelection(copTask *copTask, p *DataSource, path *util.AccessPath, finalStats *property.StatsInfo) {
	...
	if indexConds != nil {
		...
		indexSel := PhysicalFilter{Conditions: indexConds}.Init(is.ctx, stats, is.blockOffset)
	...
	if len(tableConds) > 0 {
		...
		copTask.cst += copTask.count() * sessVars.CopCPUFactor
		tableSel := PhysicalFilter{Conditions: tableConds}.Init(is.ctx, finalStats, is.blockOffset)
...
func (ts *PhysicalTableScan) addPushedDownSelection(copTask *copTask, stats *property.StatsInfo) {
	...
	if len(ts.filterCondition) > 0 {
		copTask.cst += copTask.count() * sessVars.CopCPUFactor
		sel := PhysicalFilter{Conditions: ts.filterCondition}.Init(ts.ctx, stats, ts.blockOffset)
		sel.SetChildren(ts)
