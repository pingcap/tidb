...
func (p *baseLogicalPlan) enumeratePhysicalPlans4Task(physicalPlans []PhysicalPlan, prop *property.PhysicalProperty, addEnforcer bool, planCounter *PlanCounterTp, opt *physicalOptimizeOp) (task, int64, error) {
	...
	for _, pp := range physicalPlans {
		candidateInfo := opt.appendCandidate(p.self, pp, prop.String())
		...
		for j, child := range p.children {
			...
			if opt != nil && childTask != nil {
				opt.appendChildToCandidate(candidateInfo, childTask.plan(), prop)
			}
		}
...
func (op *physicalOptimizeOp) buildOrGetPhysicalOptimizeTraceInfo(p LogicalPlan, prop string) *tracing.PhysicalOptimizeTraceInfo {
	...
	if _, ok := op.tracer.State[name][prop]; !ok {
		op.tracer.State[name][prop] = &tracing.PhysicalOptimizeTraceInfo{Property: prop}
	}
	return op.tracer.State[name][prop]
}
...
func (op *physicalOptimizeOp) appendCandidate(logicalPlan LogicalPlan, physicalPlan PhysicalPlan, prop string) *tracing.PhysicalPlanTrace {
	traceInfo := op.buildOrGetPhysicalOptimizeTraceInfo(logicalPlan, prop)
	if traceInfo == nil || physicalPlan == nil {
		return nil
	}
	PhysicalPlanTrace := &tracing.PhysicalPlanTrace{TP: physicalPlan.TP(), ID: physicalPlan.ID(), Property: prop}
...
func (op *physicalOptimizeOp) appendChildToCandidate(candidateInfo *tracing.PhysicalPlanTrace, plan PhysicalPlan,
	...
	childPhysicalPlanTrace := &tracing.PhysicalPlanTrace{TP: plan.TP(), ID: plan.ID(), Info: plan.ExplainInfo(),
		Cost: plan.Cost(), Property: prop.String()}
...
func (op *physicalOptimizeOp) setBest(lp LogicalPlan, pp PhysicalPlan, prop string) {
	traceInfo := op.buildOrGetPhysicalOptimizeTraceInfo(lp, prop)
	if traceInfo == nil || pp == nil {
		return
	}
...
func (p *baseLogicalPlan) findBestTask(prop *property.PhysicalProperty, planCounter *PlanCounterTp, opt *physicalOptimizeOp) (bestTask task, cntPlan int64, err error) {
	...
	var cnt int64
	var curTask task
	opt.buildOrGetPhysicalOptimizeTraceInfo(p, newProp.String())
...
func (ds *DataSource) findBestTask(prop *property.PhysicalProperty, planCounter *PlanCounterTp, opt *physicalOptimizeOp) (t task, cntPlan int64, err error) {
	...
	opt.buildOrGetPhysicalOptimizeTraceInfo(ds, prop.String())
	cntPlan = 0
...
func (ds *DataSource) convertToIndexMergeScanTask(prop *property.PhysicalProperty, candidate *candidatePath, opt *physicalOptimizeOp) (task task, err error) {
	defer appendCandidate(ds, task, prop, opt)
...
func appendCandidate(lp LogicalPlan, task task, prop *property.PhysicalProperty, opt *physicalOptimizeOp) {
	...
	candidate := opt.appendCandidate(lp, task.plan(), prop.String())
	if candidate != nil {
