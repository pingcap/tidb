...
func (ds *DataSource) convertToPartialIndexScan(prop *property.PhysicalProperty, path *util.AccessPath) (
	...
	is.rowSizeForNet = is.indexScanRowSize(idx, ds, false)
	...
	if indexConds != nil {
		...
		partialCost += rowCount * is.rowSizeForNet * sessVars.GetNetworkFactor(ds.tableInfo)
...
func (ds *DataSource) buildIndexMergeTableScan(prop *property.PhysicalProperty, tableFilters []expression.Expression,
	...
	ts.rowSizeForScan = ds.TblColHists.GetTableAvgRowSize(ds.ctx, ds.TblCols, ts.StoreType, true)
	partialCost += totalRowCount * ts.rowSizeForScan * sessVars.GetScanFactor(ds.tableInfo)
...
func (ds *DataSource) getOriginalPhysicalTableScan(prop *property.PhysicalProperty, path *util.AccessPath, isMatchProp bool) (*PhysicalTableScan, float64, float64) {
	...
	if ts.StoreType == kv.TiKV {
		...
		ts.rowSizeForScan = ds.TblColHists.GetTableAvgRowSize(ds.ctx, ts.Schema().Columns, ts.StoreType, ds.handleCols != nil)
	}
	sessVars := ds.ctx.GetSessionVars()
	cost := rowCount * ts.rowSizeForScan * sessVars.GetScanFactor(ds.tableInfo)
	if isMatchProp {
		ts.Desc = prop.SortItems[0].Desc
		if prop.SortItems[0].Desc && prop.ExpectedCnt >= smallScanThreshold {
			cost = rowCount * ts.rowSizeForScan * sessVars.GetDescScanFactor(ds.tableInfo)
		}
		ts.KeepOrder = true
	}
...
func (ds *DataSource) getOriginalPhysicalIndexScan(prop *property.PhysicalProperty, path *util.AccessPath, isMatchProp bool, isSingleScan bool) (*PhysicalIndexScan, float64, float64) {
	...
	is.rowSizeForScan = is.indexScanRowSize(idx, ds, true)
	sessVars := ds.ctx.GetSessionVars()
	cost := rowCount * is.rowSizeForScan * sessVars.GetScanFactor(ds.tableInfo)
	if isMatchProp {
		is.Desc = prop.SortItems[0].Desc
		if prop.SortItems[0].Desc && prop.ExpectedCnt >= smallScanThreshold {
			cost = rowCount * is.rowSizeForScan * sessVars.GetDescScanFactor(ds.tableInfo)
		}
		is.KeepOrder = true
	}
	is.cost = cost
