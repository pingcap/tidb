...
var _ = Suite(&testSharIndexResults{})

type testSharIndexResults struct {
	store kv.Storage
	dom   *domain.Domain
}

func (s *testSharIndexResults) SetUpTest(c *C) {
	var err error
	s.store, s.dom, err = newStoreWithBootstrap()
	c.Assert(err, IsNil)
}

func (s *testSharIndexResults) TearDownTest(c *C) {
	s.dom.Close()
	c.Assert(s.store.Close(), IsNil)
}

func (s *testSharIndexResults) TestSharIndexPlan(c *C) {
