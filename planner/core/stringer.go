...
=======
	...
	case *logicalop.LogicalUnionAll:
		strs = append(strs, "{"+x.FDs().String()+"}")
>>>>>>> ed9b7d5771b (planner: fix planner can't error for union-all query when new-only-full-group-check is enabled (#59212)):pkg/planner/core/stringer.go
	default:
	}
	return strs, idxs
}
