...
func (r *pushDownJoin) predicatePushDownJoin(
	...
	var otherCond, leftPushCond, rightPushCond []expression.Expression
...
func (r *PushSelDownJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	leftCond, rightCond, remainCond, dual := r.predicatePushDownJoin(sctx, sel.Conditions, newJoin, leftGroup.Prop.Schema, rightGroup.Prop.Schema)
	if dual != nil {
	...
	leftGroup = buildChildSelectionGroup(sel.SCtx(), sel.SelectBlockOffset(), leftCond, leftGroup)
	rightGroup = buildChildSelectionGroup(sel.SCtx(), sel.SelectBlockOffset(), rightCond, rightGroup)
...
func (r *TransformJoinCondToSel) Match(expr *memo.ExprIter) bool {
	...
	return len(join.EqualConditions) > 0 || len(join.LeftConditions) > 0 || len(join.RightConditions) > 0 || len(join.OtherConditions) > 0
}
...
func (r *TransformJoinCondToSel) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	leftCond, rightCond, _, dual := r.predicatePushDownJoin(sctx, []expression.Expression{}, newJoin, leftGroup.Prop.Schema, rightGroup.Prop.Schema)
	if dual != nil {
		return []*memo.GroupExpr{memo.NewGroupExpr(dual)}, true, true, nil
	}
