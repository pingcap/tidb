...
func (s *testAnalyzeSuite) TestIndexRead(c *C) {
	...
	for _, tt := range tests {
		...
		c.Assert(err, IsNil)
		c.Assert(planner.ToString(p), Equals, tt.best, Commentf("for %s", tt.sql))
	}
}
...
func (s *testAnalyzeSuite) TestPreparedNullParam(c *C) {
	...
	for _, flag := range flags {
		...
		is := domain.GetDomain(ctx).InfoSchema()
		err = planner.Preprocess(ctx, stmt, is, true)
		c.Assert(err, IsNil)
		p, err := planner.Optimize(ctx, stmt, is)
		c.Assert(err, IsNil)

		c.Assert(planner.ToString(p), Equals, best, Commentf("for %s", sql))
	}
