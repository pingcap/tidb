...
func AdjustRowCountForTableScanByLimit(sctx sessionctx.Context, statsInfo *property.StatsInfo,
	tblStats *statistics.Table, path *util.AccessPath, expectedCnt float64, desc bool) (rowCount float64) {
	selectivity := statsInfo.RowCount / path.CountAfterAccess
	uniformEst := math.Min(path.CountAfterAccess, expectedCnt/selectivity)

	corrEst, ok, corr := crossEstimateTableRowCount(sctx, statsInfo, tblStats, path, expectedCnt, desc)
	if ok {
...
// should be adjusted by the limit number 1, because only one row is returned.
func AdjustRowCountForIndexScanByLimit(sctx sessionctx.Context, statsInfo *property.StatsInfo,
	tblStats *statistics.Table, path *util.AccessPath, expectedCnt float64, desc bool) (rowCount float64) {
...
func crossEstimateIndexRowCount(sctx sessionctx.Context, statsInfo *property.StatsInfo, tblStats *statistics.Table, path *util.AccessPath, expectedCnt float64, desc bool) (float64, bool, float64) {
	...
	col, corr := getMostCorrCol4Index(path, tblStats, sessVars.CorrelationThreshold)
...
func crossEstimateRowCount(sctx sessionctx.Context, statsInfo *property.StatsInfo, tblStats *statistics.Table, path *util.AccessPath, conds []expression.Expression, col *expression.Column, corr, expectedCnt float64, desc bool) (float64, bool, float64) {
	...
	rangeCounts, ok := getColumnRangeCounts(sctx, colID, ranges, &tblStats.HistColl, idxID)
	if !ok {
		return 0, false, corr
	}
	...
	if idxExists {
		rangeCount, err = GetRowCountByIndexRanges(sctx, &tblStats.HistColl, idxID, convertedRanges)
	} else {
		rangeCount, err = GetRowCountByColumnRanges(sctx, &tblStats.HistColl, colID, convertedRanges)
	}
	if err != nil {
		return 0, false, corr
	}
