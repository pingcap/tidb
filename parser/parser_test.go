...
func TestBRIE(t *testing.T) {
	table := []testCase{
		...
		{"backup incremental start to 'noop://'", true, "BACKUP INCREMENTAL START TO 'noop://'"},
		{"backup incremental start to 'noop://' start_ts='20220304'", true, "BACKUP INCREMENTAL START TO 'noop://' START_TS = '20220304'"},
		{"backup incremental pause", true, "BACKUP INCREMENTAL PAUSE"},
		{"backup incremental pause gc_ttl=20220304", true, "BACKUP INCREMENTAL PAUSE GC_TTL = 20220304"},
		{"backup incremental resume", true, "BACKUP INCREMENTAL RESUME"},
		{"backup incremental truncate from 'noop://'", true, "BACKUP INCREMENTAL TRUNCATE FROM 'noop://'"},
		{"backup incremental truncate from 'noop://' until_ts='2012122304'", true, "BACKUP INCREMENTAL TRUNCATE FROM 'noop://' UNTIL_TS = '2012122304'"},
		{"backup incremental status", true, "BACKUP INCREMENTAL STATUS"},
		{"backup incremental metadata from 'noop://'", true, "BACKUP INCREMENTAL METADATA FROM 'noop://'"},
		{"show backup metadata from 'noop://'", true, "SHOW BACKUP METADATA FROM 'noop://'"},
