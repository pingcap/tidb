...
func TestIntervalPartition(t *testing.T) {
	table := []testCase{
		...
		{`ALTER TABLE t split MAX PARTITION (1000)`, true, "ALTER TABLE `t` SPLIT MAX PARTITION (1000)"},
		{`ALTER TABLE t merge first PARTITION (1000)`, true, "ALTER TABLE `t` MERGE FIRST PARTITION (1000)"},
	}
