...
func (s *testParserSuite) TestBuiltin(c *C) {
	...
	table := []testCase{
		...
		{`SELECT ANY_VALUE(0);`, true},
		{`SELECT DEFAULT_FUNC(0);`, true},
		...
		{`SELECT IS_USED_LOCK(0);`, true},
		{`SELECT MASTER_POS_WAIT(0, 0), MASTER_POS_WAIT(0, 0, 0), MASTER_POS_WAIT(0, 0, 0, 0);`, true},
