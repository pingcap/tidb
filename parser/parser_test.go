...
func (s *testParserSuite) TestDDL(c *C) {
	...
	table := []testCase{
		...
		{"create table t (c int) PARTITION BY HASH (c) PARTITIONS 32 (PARTITION p1980 VALUES LESS THAN (1980) ENGINE = MyISAM, PARTITION p1990 VALUES LESS THAN (1990) ENGINE = MyISAM, PARTITION pothers VALUES LESS THAN MAXVALUE ENGINE = MyISAM)", false},
