...
func (s *testParserSuite) TestDDL(c *C) {
	...
	table := []testCase{
		...
		// Partition option generated by mydumper
		{"CREATE TABLE `company_modify` (`id` int(11) NOT NULL AUTO_INCREMENT,`entid` int(11) NOT NULL DEFAULT '0',`ALTITEM` varchar(3) DEFAULT NULL COMMENT '变更事项',`ALTBE` text COMMENT '变更前内容',`ALTAF` text COMMENT '变更后内容',`ALTDATE` date DEFAULT NULL COMMENT '变更日期',`ALTTIME` int(11) DEFAULT NULL COMMENT '变更次数',`updated` datetime DEFAULT NULL,PRIMARY KEY (`id`,`entid`),KEY `entid` (`entid`),KEY `ALTITEM` (`ALTITEM`)) ENGINE=MyISAM AUTO_INCREMENT=96413735 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='变 更记录'/*!50100 PARTITION BY HASH (entid)PARTITIONS 32 */;", true},
		// For issue 974
