...
func TestNormalizeForBinding(t *testing.T) {
	tests := []struct {
		input  string
		expect string
	}{
...
func TestNormalizeDigestForBinding(t *testing.T) {
	tests := []struct {
		sql        string
		normalized string
		digest     string
	...
	for _, test := range tests {
		normalized, digest := parser.NormalizeDigestForBinding(test.sql)
		require.Equal(t, test.normalized, normalized)
		require.Equal(t, test.digest, digest.String())

		normalized = parser.NormalizeForBinding(test.sql)
		digest = parser.DigestNormalized(normalized)
		require.Equal(t, test.normalized, normalized)
		require.Equal(t, test.digest, digest.String())
	}
