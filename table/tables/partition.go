...
/*
	if len(pi.AddingDefinitions) > 0 && len(pi.DroppingDefinitions) > 0 {
		reorgListPrune := &ForListPruning{}
		if len(pi.Columns) == 0 {
			err = reorgListPrune.buildListPruner(ctx, tblInfo, pi.AddingDefinitions, exprCols, columns, names)
		} else {
			err = reorgListPrune.buildListColumnsPruner(ctx, tblInfo, columns, names)
		}
		ret.PartitionReorgExpr = &PartitionReorgExpr{ForListPruning: reorgListPrune}
	}
*/
...
// WASHERE: How to inject the reorg partitions in this?
