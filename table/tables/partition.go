...
func (t *partitionedTable) AddRecord(ctx sessionctx.Context, r []types.Datum, skipHandleCheck, isInsert bool) (recordID int64, err error) {
	...
	return tbl.AddRecord(ctx, r, skipHandleCheck, isInsert)
}
...
func (t *partitionedTable) UpdateRecord(ctx sessionctx.Context, h int64, currData, newData []types.Datum, touched []bool) error {
	...
	if from != to {
		_, err = t.GetPartition(to).AddRecord(ctx, newData, false, true)
		if err != nil {
			return errors.Trace(err)
		}
