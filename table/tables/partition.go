...
func (t *partitionedTable) locatePartition(ctx sessionctx.Context, pi *model.PartitionInfo, r []types.Datum) (int64, error) {
	...
	case model.PartitionTypeRange:
		if pi.SubPart != nil {
			parentIdx, err := t.locateRangePartition(ctx, pi, r)
			if err != nil {
				return 0, err
			}
...
func (t *partitionedTable) locateRangePartition(ctx sessionctx.Context, pi *model.PartitionInfo, r []types.Datum) (idx int, err error) {
	if len(pi.Columns) == 0 {
