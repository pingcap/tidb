...
func (ts *testSuite) TestBasic(c *C) {
	...
	rid, err := tb.AddRecord(ctx, types.MakeDatums(1, "abc"), false, true)
	c.Assert(err, IsNil)
	...
	_, err = tb.AddRecord(ctx, types.MakeDatums(1, "aba"), false, true)
	c.Assert(err, NotNil)
	_, err = tb.AddRecord(ctx, types.MakeDatums(2, "abc"), false, true)
	c.Assert(err, NotNil)
	...
	_, err = tb.AddRecord(ctx, types.MakeDatums(1, "abc"), false, true)
	c.Assert(err, IsNil)
	c.Assert(indexCnt(), Greater, 0)
...
func (ts *testSuite) TestUniqueIndexMultipleNullEntries(c *C) {
	...
	_, err = tb.AddRecord(sctx, types.MakeDatums(1, nil), false, true)
	c.Assert(err, IsNil)
	_, err = tb.AddRecord(sctx, types.MakeDatums(2, nil), false, true)
	c.Assert(err, IsNil)
...
func (ts *testSuite) TestUnsignedPK(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(ts.se.NewTxn(context.Background()), IsNil)
	rid, err := tb.AddRecord(ts.se, types.MakeDatums(1, "abc"), false, true)
	c.Assert(err, IsNil)
...
PARTITION BY RANGE ( id ) (
	...
	rid, err := tb.AddRecord(ts.se, types.MakeDatums(1), false, true)
	c.Assert(err, IsNil)
	...
	// Cover more code.
	_, err = tb.AddRecord(ts.se, types.MakeDatums(7), false, true)
	c.Assert(err, IsNil)
	_, err = tb.AddRecord(ts.se, types.MakeDatums(12), false, true)
	c.Assert(err, IsNil)
	_, err = tb.AddRecord(ts.se, types.MakeDatums(16), false, true)
	c.Assert(err, IsNil)
	...
	_, err = tb.AddRecord(ts.se, types.MakeDatums(22), false, true)
...
func (ts *testSuite) TestHashPartitionAddRecord(c *C) {
	...
	p0 := tbInfo.Partition.Definitions[0]
	c.Assert(ts.se.NewTxn(context.Background()), IsNil)
	rid, err := tb.AddRecord(ts.se, types.MakeDatums(8), false, true)
	c.Assert(err, IsNil)
	...
	// Cover more code.
	_, err = tb.AddRecord(ts.se, types.MakeDatums(-1), false, true)
	c.Assert(err, IsNil)
	_, err = tb.AddRecord(ts.se, types.MakeDatums(3), false, true)
	c.Assert(err, IsNil)
	_, err = tb.AddRecord(ts.se, types.MakeDatums(6), false, true)
	c.Assert(err, IsNil)
	...
	for i := 0; i < 11; i++ {
		c.Assert(ts.se.NewTxn(context.Background()), IsNil)
		rid, err = tb.AddRecord(ts.se, types.MakeDatums(-i), false, true)
		c.Assert(err, IsNil)
		txn, err = ts.se.Txn(true)
		c.Assert(err, IsNil)
