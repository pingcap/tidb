...
func (ts *testSuite) TestBasic(c *C) {
	_, err := ts.se.ExecuteInternal(context.Background(), "CREATE TABLE test.t (a int primary key auto_increment, b varchar(255) unique)")
	c.Assert(err, IsNil)
	...
	c.Assert(err, IsNil)
	_, err = ts.se.ExecuteInternal(context.Background(), "drop table test.t")
	c.Assert(err, IsNil)
...
func (ts *testSuite) TestTypes(c *C) {
	...
	_, err := ts.se.ExecuteInternal(context.Background(), "CREATE TABLE test.t (c1 tinyint, c2 smallint, c3 int, c4 bigint, c5 text, c6 blob, c7 varchar(64), c8 time, c9 timestamp null default CURRENT_TIMESTAMP, c10 decimal(10,1))")
	c.Assert(err, IsNil)
	_, err = ts.dom.InfoSchema().TableByName(model.NewCIStr("test"), model.NewCIStr("t"))
	c.Assert(err, IsNil)
	_, err = ts.se.ExecuteInternal(ctx, "insert test.t values (1, 2, 3, 4, '5', '6', '7', '10:10:10', null, 1.4)")
	c.Assert(err, IsNil)
	rs, err := ts.se.ExecuteInternal(ctx, "select * from test.t where c1 = 1")
	c.Assert(err, IsNil)
	req := rs.NewChunk()
	err = rs.Next(ctx, req)
	c.Assert(err, IsNil)
	...
	_, err = ts.se.ExecuteInternal(ctx, "drop table test.t")
	c.Assert(err, IsNil)

	_, err = ts.se.ExecuteInternal(ctx, "CREATE TABLE test.t (c1 tinyint unsigned, c2 smallint unsigned, c3 int unsigned, c4 bigint unsigned, c5 double, c6 bit(8))")
	c.Assert(err, IsNil)
	_, err = ts.se.ExecuteInternal(ctx, "insert test.t values (1, 2, 3, 4, 5, 6)")
	c.Assert(err, IsNil)
	rs, err = ts.se.ExecuteInternal(ctx, "select * from test.t where c1 = 1")
	c.Assert(err, IsNil)
	req = rs.NewChunk()
	...
	_, err = ts.se.ExecuteInternal(ctx, "CREATE TABLE test.t (c1 enum('a', 'b', 'c'))")
	c.Assert(err, IsNil)
	_, err = ts.se.ExecuteInternal(ctx, "insert test.t values ('a'), (2), ('c')")
	c.Assert(err, IsNil)
	rs, err = ts.se.ExecuteInternal(ctx, "select c1 + 1 from test.t where c1 = 1")
	c.Assert(err, IsNil)
	req = rs.NewChunk()
	err = rs.Next(ctx, req)
	c.Assert(err, IsNil)
...
func (ts *testSuite) TestUniqueIndexMultipleNullEntries(c *C) {
	ctx := context.Background()
	_, err := ts.se.ExecuteInternal(ctx, "drop table if exists test.t")
	c.Assert(err, IsNil)
	_, err = ts.se.ExecuteInternal(ctx, "CREATE TABLE test.t (a int primary key auto_increment, b varchar(255) unique)")
	c.Assert(err, IsNil)
	tb, err := ts.dom.InfoSchema().TableByName(model.NewCIStr("test"), model.NewCIStr("t"))
	c.Assert(err, IsNil)
	c.Assert(tb.Meta().ID, Greater, int64(0))
...
func (ts *testSuite) TestUnsignedPK(c *C) {
	ts.se.ExecuteInternal(context.Background(), "DROP TABLE IF EXISTS test.tPK")
	_, err := ts.se.ExecuteInternal(context.Background(), "CREATE TABLE test.tPK (a bigint unsigned primary key, b varchar(255))")
	c.Assert(err, IsNil)
...
func (ts *testSuite) TestIterRecords(c *C) {
	ts.se.ExecuteInternal(context.Background(), "DROP TABLE IF EXISTS test.tIter")
	_, err := ts.se.ExecuteInternal(context.Background(), "CREATE TABLE test.tIter (a int primary key, b int)")
	c.Assert(err, IsNil)
	_, err = ts.se.ExecuteInternal(context.Background(), "INSERT test.tIter VALUES (-1, 2), (2, NULL)")
	c.Assert(err, IsNil)
	c.Assert(ts.se.NewTxn(context.Background()), IsNil)
...
func (ts *testSuite) TestAddRecordWithCtx(c *C) {
	ts.se.ExecuteInternal(context.Background(), "DROP TABLE IF EXISTS test.tRecord")
	_, err := ts.se.ExecuteInternal(context.Background(), "CREATE TABLE test.tRecord (a bigint unsigned primary key, b varchar(255))")
	c.Assert(err, IsNil)
	tb, err := ts.dom.InfoSchema().TableByName(model.NewCIStr("test"), model.NewCIStr("tRecord"))
	c.Assert(err, IsNil)
	defer ts.se.ExecuteInternal(context.Background(), "DROP TABLE test.tRecord")
