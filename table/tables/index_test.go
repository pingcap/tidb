...
func (s *testIndexSuite) TestIndex(c *C) {
	...
	_, err = index.Create(mockCtx, txn, values, 1, txn)
	c.Assert(err, IsNil)
	...
	_, err = index.Create(mockCtx, txn, values, 0, txn)
	c.Assert(err, IsNil)
	...
	_, err = index.Create(mockCtx, txn, values, 2, txn)
	c.Assert(err, NotNil)
	...
	_, err = index.Create(mockCtx, txn, values2, 2, txn)
	c.Assert(err, IsNil)
	it, err = index.SeekFirst(txn)
	c.Assert(err, IsNil)
	getValues, h, err = it.Next()
