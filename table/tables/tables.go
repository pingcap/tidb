...
func (t *Table) AddRecord(ctx context.Context, r []types.Datum, skipDupCheck bool) (recordID int64, err error) {
	...
	h, err := t.addIndices(ctx, recordID, r, bs, skipDupCheck)
	if err != nil {
...
func (t *Table) addIndices(ctx context.Context, recordID int64, r []types.Datum, bs *kv.BufferStore, skipDupCheck bool) (int64, error) {
	txn := ctx.Txn()
	// Clean up lazy check error environment
	defer txn.DelOption(kv.PresumeKeyNotExistsError)
	skipCheck := ctx.GetSessionVars().SkipConstraintCheck
	if t.meta.PKIsHandle && !skipCheck && !skipDupCheck {
		// Check key exists.
