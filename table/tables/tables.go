...
func (t *Table) AddRecord(ctx context.Context, r []types.Datum, recordDupChecked bool) (recordID int64, err error) {
	...
	h, err := t.addIndices(ctx, recordID, r, bs, recordDupChecked)
	if err != nil {
...
func (t *Table) addIndices(ctx context.Context, recordID int64, r []types.Datum, bs *kv.BufferStore, recordDupChecked bool) (int64, error) {
	txn := ctx.Txn()
	// Clean up lazy check error environment
	defer txn.DelOption(kv.PresumeKeyNotExistsError)
	skipCheck := ctx.GetSessionVars().SkipConstraintCheck
	if t.meta.PKIsHandle && !skipCheck && !recordDupChecked {
		// Check key exists.
