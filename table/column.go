...
func CastValue(ctx sessionctx.Context, val types.Datum, col *model.ColumnInfo) (casted types.Datum, err error) {
	...
	if types.ErrTruncated.Equal(err) {
		...
		if err1 != nil {
			logutil.BgLogger().Error("Datum ToString failed", zap.Stringer("Datum", val), zap.Error(err1))
		}
	...
	logutil.BgLogger().Error("111111111111111111111111111111       ", zap.Bool("zeroMode", ctx.GetSessionVars().SQLMode.HasNoZeroDateMode()), zap.Uint64("connid", ctx.GetSessionVars().ConnectionID))
