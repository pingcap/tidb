...
func (s *tiflashTestSuite) TestReadUnsigedPK(c *C) {
	...
	tk.MustExec("create table t(a int unsigned not null primary key, b int not null)")
	tk.MustExec("alter table t set tiflash replica 1")
	tb := testGetTableByName(c, tk.Se, "test", "t")
	err := domain.GetDomain(tk.Se).DDL().UpdateTableReplicaInfo(tk.Se, tb.Meta().ID, true)
	c.Assert(err, IsNil)
	...
	tk.MustExec("create table t1(a int unsigned not null primary key, b int not null)")
