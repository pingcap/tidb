...
func (s *testSuite) TestMergePartialResult4VarianceDecimal(c *C) {
	...
	partialVarianceFunc := aggfuncs.Build(s.ctx, desc, 0)
	...
	var tmp types.MyDecimal
	delta := types.NewDecFromStringForTest("0.0000000000000000001")
	...
	result = types.NewDecFromStringForTest("1.414213562160963014005550")
	types.DecimalSub(resultChk.GetRow(0).GetMyDecimal(0), result, &tmp)
	// variance(0, 1, 2, 3, 4)
	c.Assert(delta.Compare(&tmp) == 1, IsTrue)
	...
	result = types.NewDecFromStringForTest("0.816496580927726031507683")
	types.DecimalSub(resultChk.GetRow(0).GetMyDecimal(0), result, &tmp)
	...
	c.Assert(err, IsNil)
	partialVarianceFunc.AppendFinalResult2Chunk(s.ctx, partialPr2, resultChk)
	result = types.NewDecFromStringForTest("1.316956718750658107153531")
	types.DecimalSub(resultChk.GetRow(0).GetMyDecimal(0), result, &tmp)
	// variance(10, 9)
	c.Assert(delta.Compare(&tmp) == 1, IsTrue)
}
...
func (s *testSuite) TestMergePartialResult4VarianceFloat(c *C) {
	...
	partialVarianceFunc.AppendFinalResult2Chunk(s.ctx, partialPr1, resultChk)
	// variance(0, 1, 2, 3, 4)
	c.Assert(resultChk.GetRow(0).GetFloat64(0) == 1.4142135623730951, IsTrue)
	...
	partialVarianceFunc.AppendFinalResult2Chunk(s.ctx, partialPr2, resultChk)
	// variance(2, 3, 4)
	c.Assert(resultChk.GetRow(0).GetFloat64(0) == 0.816496580927726, IsTrue)
	...
	c.Assert(err, IsNil)
	// variance(10, 9)
	c.Assert(resultChk.GetRow(0).GetFloat64(0) == 1.3169567191065923, IsTrue)
}
