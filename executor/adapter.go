...
func (a *ExecStmt) handlePessimisticLockError(ctx context.Context, err error) (Executor, error) {
	...
	if deadlock, ok := errors.Cause(err).(*tikv.ErrDeadlock); ok {
		...
		// this branch update ForUpdateTs and report error to client, do no retry
		// update ForUpdateTs to prevent async rollback problem
		...
		//        the async rollback operation rollbacks the lock just acquired, the commitKey will report error
