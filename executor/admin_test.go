...
func (s *testSuite5) TestAdminRecoverIndex(c *C) {
	...
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(1), kv.IntHandle(1), nil)
	c.Assert(err, IsNil)
	...
	txn, err = s.store.Begin()
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(10), kv.IntHandle(10), nil)
	c.Assert(err, IsNil)
	err = txn.Commit(context.Background())
	c.Assert(err, IsNil)
	...
	err = indexOpr.Delete(sc, txn, types.MakeDatums(1), kv.IntHandle(1), nil)
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(2), kv.IntHandle(2), nil)
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(3), kv.IntHandle(3), nil)
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(10), kv.IntHandle(10), nil)
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(20), kv.IntHandle(20), nil)
	c.Assert(err, IsNil)
	err = txn.Commit(context.Background())
	c.Assert(err, IsNil)
...
func (s *testSuite5) TestClusteredIndexAdminRecoverIndex(c *C) {
	...
	err = indexOpr.Delete(sc, txn, types.MakeDatums(2), cHandle, nil)
	c.Assert(err, IsNil)
	err = txn.Commit(context.Background())
	c.Assert(err, IsNil)
...
func (s *testSuite5) TestAdminRecoverPartitionTableIndex(c *C) {
	...
	checkFunc := func(tbl table.Table, pid int64, idxValue int) {
		...
		txn, err := s.store.Begin()
		c.Assert(err, IsNil)
		err = indexOpr.Delete(sc, txn, types.MakeDatums(idxValue), kv.IntHandle(idxValue), nil)
		c.Assert(err, IsNil)
		err = txn.Commit(context.Background())
		c.Assert(err, IsNil)
		err = tk.ExecToErr("admin check table admin_test")
...
func (s *testSuite5) TestAdminRecoverIndex1(c *C) {
	...
	err = indexOpr.Delete(sc, txn, types.MakeDatums("3"), kv.IntHandle(3), nil)
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums("10"), kv.IntHandle(4), nil)
	c.Assert(err, IsNil)
	err = txn.Commit(context.Background())
	c.Assert(err, IsNil)
...
func (s *testSuite3) TestAdminCheckPartitionTableFailed(c *C) {
	...
	for i := 0; i <= 5; i++ {
		...
		txn, err := s.store.Begin()
		c.Assert(err, IsNil)
		err = indexOpr.Delete(sc, txn, types.MakeDatums(i), kv.IntHandle(i), nil)
		c.Assert(err, IsNil)
		err = txn.Commit(context.Background())
		c.Assert(err, IsNil)
	...
	for i := 0; i <= 5; i++ {
		...
		txn, err = s.store.Begin()
		c.Assert(err, IsNil)
		err = indexOpr.Delete(sc, txn, types.MakeDatums(i+8), kv.IntHandle(i+8), nil)
		c.Assert(err, IsNil)
		err = txn.Commit(context.Background())
		c.Assert(err, IsNil)
		tk.MustExec("admin check table admin_test_p")
	...
	for i := 0; i <= 5; i++ {
		...
		txn, err = s.store.Begin()
		c.Assert(err, IsNil)
		err = indexOpr.Delete(sc, txn, types.MakeDatums(i+8), kv.IntHandle(i), nil)
		c.Assert(err, IsNil)
		err = txn.Commit(context.Background())
		c.Assert(err, IsNil)
		tk.MustExec("admin check table admin_test_p")
...
func (s *testSuiteJoinSerial) TestAdminCheckTableFailed(c *C) {
	...
	txn, err := s.store.Begin()
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(-10), kv.IntHandle(-1), nil)
	c.Assert(err, IsNil)
	err = txn.Commit(context.Background())
	c.Assert(err, IsNil)
	err = tk.ExecToErr("admin check table admin_test")
	...
	txn, err = s.store.Begin()
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(0), kv.IntHandle(0), nil)
	c.Assert(err, IsNil)
	...
	txn, err = s.store.Begin()
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(13), kv.IntHandle(2), nil)
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(12), kv.IntHandle(2), nil)
	c.Assert(err, IsNil)
	err = txn.Commit(context.Background())
	c.Assert(err, IsNil)
	err = tk.ExecToErr("admin check table admin_test")
	...
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(20), kv.IntHandle(10), nil)
	c.Assert(err, IsNil)
	err = txn.Commit(context.Background())
	c.Assert(err, IsNil)
	err = tk.ExecToErr("admin check table admin_test")
	...
	txn, err = s.store.Begin()
	c.Assert(err, IsNil)
	err = indexOpr.Delete(sc, txn, types.MakeDatums(19), kv.IntHandle(10), nil)
	c.Assert(err, IsNil)
