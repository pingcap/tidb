...
func TestShowPartitionStats(t *testing.T) {
	...
	testkit.WithPruneMode(tk, variable.Static, func() {
		...
		require.Equal(t, "global", result.Rows()[0][2])
		require.Equal(t, "b", result.Rows()[0][3])
		require.Equal(t, "global", result.Rows()[1][2])
		require.Equal(t, "idx", result.Rows()[1][3])
		require.Equal(t, "p0", result.Rows()[2][2])
		require.Equal(t, "a", result.Rows()[2][3])
		require.Equal(t, "p0", result.Rows()[3][2])
		require.Equal(t, "b", result.Rows()[3][3])
		require.Equal(t, "p0", result.Rows()[4][2])
		require.Equal(t, "idx", result.Rows()[4][3])

		result = tk.MustQuery("show stats_buckets").Sort()
		result.Check(testkit.Rows("test t global b 0 0 1 1 1 1 0", "test t global idx 1 0 1 1 1 1 0", "test t p0 a 0 0 1 1 1 1 0", "test t p0 b 0 0 1 1 1 1 0", "test t p0 idx 1 0 1 1 1 1 0"))

		result = tk.MustQuery("show stats_healthy")
		result.Check(testkit.Rows("test t global 100", "test t p0 100"))
	})
}
