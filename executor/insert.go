...
func (e *InsertExec) updateDupRow(ctx context.Context, idxInBatch int, txn kv.Transaction, row toBeCheckedRow, handle int64, onDuplicate []*expression.Assignment) error {
	...
	if len(e.ctx.GetSessionVars().CurrInsertBatch) > 0 {
		extraCols := e.ctx.GetSessionVars().CurrInsertBatch[idxInBatch]
...
func (e *InsertExec) Close() error {
	...
	e.ctx.GetSessionVars().CurrInsertBatch = e.ctx.GetSessionVars().CurrInsertBatch[0:0:0]
	e.setMessage()
	if e.SelectExec != nil {
