...
func (e *UpdateExec) exec(ctx context.Context, schema *expression.Schema, row, newData []types.Datum) error {
	...
	for i, content := range e.tblColPosInfos {
		...
		// Update row
		fkTriggerExecs := e.fkTriggerExecs[content.TblID]
		changed, err1 := updateRecord(ctx, e.ctx, handle, oldData, newTableData, flags, tbl, false, e.memTracker, fkTriggerExecs)
		if err1 == nil {
...
func (e *UpdateExec) GetForeignKeyTriggerExecs() []*ForeignKeyTriggerExec {
