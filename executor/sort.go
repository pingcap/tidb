...
func (e *SortExec) fetchRowChunks(ctx context.Context) error {
	fields := e.retTypes()
	e.rowChunks = chunk.NewList(fields, e.chunkRowsPerFetch())
...
func (e *SortExec) buildKeyChunks() error {
	e.keyChunks = chunk.NewList(e.keyTypes, e.chunkRowsPerFetch())
...
func (e *TopNExec) Next(ctx context.Context, chk *chunk.Chunk) error {
	...
	for chk.NumRows() < e.chunkRowsPerFetch() && e.Idx < len(e.rowPtrs) {
...
func (e *TopNExec) loadChunksUntilTotalLimit(ctx context.Context) error {
	...
	e.rowChunks = chunk.NewList(e.retTypes(), e.chunkRowsPerFetch())
	e.rowChunks.GetMemTracker().AttachTo(e.memTracker)
	e.rowChunks.GetMemTracker().SetLabel("rowChunks")
...
func (e *TopNExec) executeTopN(ctx context.Context) error {
	...
	if e.keyChunks != nil {
		childKeyChk = chunk.NewChunkWithCapacity(e.keyTypes, e.chunkRowsPerFetch())
	}
...
func (e *TopNExec) doCompaction() error {
	newRowChunks := chunk.NewList(e.retTypes(), e.chunkRowsPerFetch())
	...
	if e.keyChunks != nil {
		newKeyChunks := chunk.NewList(e.keyTypes, e.chunkRowsPerFetch())
		for _, rowPtr := range e.rowPtrs {
