	...
	go func() {
		...
		util.WithRecovery(
			func() {
				...
				} else if !e.partitionTableMode && e.pushedLimit != nil && len(e.byItems) != 0 {
		...
		util.WithRecovery(
			func() {
				...
				for parTblIdx, keyRange := range keyRanges {
					...
					_, fetchErr := worker.fetchHandles(ctx1, result, exitCh, fetchCh, e.finished, e.handleCols, parTblIdx, workID)
