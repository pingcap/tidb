...
func TestIssue21447(t *testing.T) {
	...
	alloc1 := chunk.NewAllocator()
	alloc2 := chunk.NewAllocator()
	//First time does not go to the cache, second time alloc requests memory from the system,
	//Third time gets the chunk from alloc
	idAlloc1 := []chunk.Allocator{nil, alloc1, alloc1}
	idAlloc2 := []chunk.Allocator{nil, alloc2, alloc2}
	idresult := []string{"0", "1", "1"}
	for i, alloc := range idAlloc1 {
		tk1.Session().GetSessionVars().SetAlloc(alloc)
		tk2.Session().GetSessionVars().SetAlloc(idAlloc2[i])
		...
		tk1.MustExec("commit")
		result := tk1.MustQuery("select @@last_sql_use_alloc")
		result.Check(testkit.Rows(idresult[i]))
		tk1.Session().GetSessionVars().EndAlloc()
		result = tk2.MustQuery("select @@last_sql_use_alloc")
		result.Check(testkit.Rows(idresult[i]))
		tk2.Session().GetSessionVars().EndAlloc()
		if alloc != nil {
			alloc.Reset()
		}
	}
