...
func (s *testSuite2) TestShowCreateTable(c *C) {
	...
	tk.MustQuery("show create table v1").Check(testutil.RowsWithSep("|", "v1|CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`127.0.0.1` SQL SECURITY DEFINER VIEW `v1` (`a`, `b`) AS select * from t1|utf8mb4|utf8mb4_bin"))
