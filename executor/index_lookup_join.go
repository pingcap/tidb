...
func (e *indexLookUpJoinRuntimeStats) String() string {
	...
	if e.innerWorker.totalTime > 0 {
		...
		buf.WriteString(execdetails.FormatDurationForExplain(time.Duration(e.innerWorker.totalTime)))
		...
		buf.WriteString(execdetails.FormatDurationForExplain(time.Duration(e.innerWorker.construct)))
		buf.WriteString(", fetch:")
		buf.WriteString(execdetails.FormatDurationForExplain(time.Duration(e.innerWorker.fetch)))
		buf.WriteString(", build:")
		buf.WriteString(execdetails.FormatDurationForExplain(time.Duration(e.innerWorker.build)))
		if e.innerWorker.join > 0 {
			buf.WriteString(", join:")
			buf.WriteString(execdetails.FormatDurationForExplain(time.Duration(e.innerWorker.join)))
		}
		buf.WriteString("}")
	}
	if e.probe > 0 {
