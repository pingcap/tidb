...
func (e *IndexLookUpJoin) Next(ctx context.Context, req *execution.ExecRequest) error {
	...
	for {
		...
		if e.innerIter.Current() != e.innerIter.End() {
			matched, err := e.joiner.tryToMatch(outerRow, e.innerIter, req.RetChunk)
			if err != nil {
				return errors.Trace(err)
			}
		...
		if e.innerIter.Current() == e.innerIter.End() {
			if !task.hasMatch {
				e.joiner.onMissMatch(outerRow, req.RetChunk)
			}
			task.cursor++
			task.hasMatch = false
		}
...
func (ow *outerWorker) buildTask(ctx context.Context) (*lookUpJoinTask, error) {
	...
	for task.outerResult.NumRows() < ow.batchSize {
		err := ow.executor.Next(ctx, &execution.ExecRequest{RetChunk: ow.executorChk})
		if err != nil {
...
func (iw *innerWorker) fetchInnerResults(ctx context.Context, task *lookUpJoinTask, dLookUpKeys [][]types.Datum) error {
	...
	for {
		err := innerExec.Next(ctx, &execution.ExecRequest{RetChunk: iw.executorChk})
		if err != nil {
			return errors.Trace(err)
		}
