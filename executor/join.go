...
func (e *HashJoinExec) fetchProbeSideChunks(ctx context.Context) {
	...
	for {
		...
		case probeSideResult, ok = <-e.probeInputEmptyChk:
			if !ok {
				return
			}
		}
		...
		e.probeInputChk <- probeSideResult
	}
}
...
func (e *HashJoinExec) runJoinWorker(workerID uint, probeKeyColIdx []int) {
	...
	for ok := true; ok; {
		...
		case probeSideResult, ok = <-e.probeInputChk:
		}
		if !ok {
			break
		}
		...
		e.probeInputEmptyChk <- probeSideResult
	}
