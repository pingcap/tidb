...
func TestVectorizedMergeJoin(t *testing.T) {
	...
	runTest := func(ts1, ts2 []int) {
		...
		tk.MustQuery(fmt.Sprintf("explain format = 'brief' select /*+ TIDB_SMJ(%s, %s) */ * from %s, %s where %s.a=%s.a and %s.b>5 and %s.b<5",
			...
			fmt.Sprintf("Projection 4150.01 root  test.%s.a, test.%s.b, test.%s.a, test.%s.b", t1, t1, t2, t2),
			fmt.Sprintf(`└─MergeJoin 4150.01 root  inner join, left key:test.%s.a, right key:test.%s.a`, t2, t1),
			fmt.Sprintf(`  ├─Sort(Build) 3330.00 root  test.%s.a`, t1),
			...
			fmt.Sprintf(`  └─Sort(Probe) 3320.01 root  test.%s.a`, t2),
		...
		tk.MustQuery(fmt.Sprintf("explain format = 'brief' select /*+ TIDB_HJ(%s, %s) */ * from %s, %s where %s.a=%s.a and %s.b>5 and %s.b<5",
			...
			fmt.Sprintf("Projection 4150.01 root  test.%s.a, test.%s.b, test.%s.a, test.%s.b", t1, t1, t2, t2),
			fmt.Sprintf(`└─HashJoin 4150.01 root  inner join, equal:[eq(test.%s.a, test.%s.a)]`, t2, t1),
