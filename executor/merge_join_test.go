...
func (s *testSuite2) TestMergeJoin(c *C) {
	...
	tk.MustQuery("explain select /*+ TIDB_SMJ(t1, t2) */ * from t t1 join t t2 order by t1.a, t2.a").Check(testkit.Rows(
		...
		"  ├─TableReader_13(Build) 10000.00 root data:TableScan_12",
		"  │ └─TableFullScan_12 10000.00 cop[tikv] table:t2, range:[-inf,+inf], keep order:false, stats:pseudo",
		"  └─TableReader_11(Probe) 10000.00 root data:TableScan_10",
		"    └─TableFullScan_10 10000.00 cop[tikv] table:t1, range:[-inf,+inf], keep order:false, stats:pseudo",
	))
	...
	tk.MustQuery("explain select /*+ TIDB_SMJ(t, s) */ a in (select a from s where s.b >= t.b) from t").Check(testkit.Rows(
		...
		"  ├─TableReader_12(Build) 10000.00 root data:TableScan_11",
		"  │ └─TableFullScan_11 10000.00 cop[tikv] table:s, range:[-inf,+inf], keep order:false, stats:pseudo",
		"  └─TableReader_10(Probe) 10000.00 root data:TableScan_9",
		"    └─TableFullScan_9 10000.00 cop[tikv] table:t, range:[-inf,+inf], keep order:false, stats:pseudo",
	))
	...
	tk.MustQuery("explain select s1.a1 from (select a as a1 from s order by s.a desc) as s1 join (select a as a2 from s order by s.a desc) as s2 on s1.a1 = s2.a2 order by s1.a1 desc").Check(testkit.Rows(
		...
		"  │     └─TableFullScan_24 10000.00 cop[tikv] table:s, range:[-inf,+inf], keep order:false, stats:pseudo",
		...
		"        └─TableFullScan_19 10000.00 cop[tikv] table:s, range:[-inf,+inf], keep order:false, stats:pseudo",
	))
...
func (s *testSuiteJoin3) TestVectorizedMergeJoin(c *C) {
	...
	runTest := func(t1, t2 []int) {
		...
		tk.MustQuery("explain select /*+ TIDB_SMJ(t1, t2) */ * from t1, t2 where t1.a=t2.a and t1.b>5 and t2.b<5").Check(testkit.Rows(
			...
			`      └─TableFullScan_8 10000.00 cop[tikv] table:t1, range:[-inf,+inf], keep order:false, stats:pseudo`,
		))
		tk.MustQuery("explain select /*+ TIDB_HJ(t1, t2) */ * from t1, t2 where t1.a=t2.a and t1.b>5 and t2.b<5").Check(testkit.Rows(
			...
			`    └─TableFullScan_9 10000.00 cop[tikv] table:t1, range:[-inf,+inf], keep order:false, stats:pseudo`,
		))
