...
func (s *testPointGetSuite) TestClusterIndexPointGet(c *C) {
	...
	tk.MustQuery(`select * from t where (a, b) in (('a', 'a1'), ('c', 'c1'))`).Check(testkit.Rows("a a1 1 11", "c c1 3 33"))
	tk.MustQuery(`select * from t where a = 'b' and b = 'b1'`).Check(testkit.Rows("b b1 2 22"))
	tk.MustQuery(`select * from t where uk = 1`).Check(testkit.Rows("a a1 1 11"))
	tk.MustQuery(`select * from t where uk in (1, 2, 3)`).Check(testkit.Rows("a a1 1 11", "b b1 2 22", "c c1 3 33"))
}
