...
func (e *InsertValues) getRow(cols []*table.Column, list []expression.Expression, ignoreErr bool) ([]types.Datum, error) {
	row := make(types.DatumnRow, len(e.Table.Cols()))
...
func (e *InsertValues) fillGenColData(cols []*table.Column, valLen int, hasValue []bool, row types.DatumnRow, ignoreErr bool) ([]types.Datum, error) {
...
func (e *InsertExec) onDuplicateUpdate(row []types.Datum, h int64, cols []*expression.Assignment) error {
	...
	newData := make(types.DatumnRow, len(data))
	copy(newData, data)
	for _, col := range cols {
...
func (e *UpdateExec) fetchRows() error {
	for {
		...
		newRowData := make(types.DatumnRow, len(row))
		copy(newRowData, row)
