...
func (e *InsertExec) doDupRowUpdate(handle int64, oldRow []types.Datum, newRow []types.Datum,
	...
	_, handleChanged, newHandle, lastInsertID, err := updateRecord(e.ctx,
		handle, oldRow, newData, assignFlag, e.Table, true, e.IgnoreErr)
	if err != nil {
...
func (e *UpdateExec) exec(ctx context.Context, schema *expression.Schema) (types.DatumRow, error) {
	...
	for id, cols := range schema.TblID2Handle {
		...
		for _, col := range cols {
			...
			// Update row
			changed, _, _, _, err1 := updateRecord(e.ctx, handle, oldData,
				newTableData, flags, tbl, false, e.IgnoreErr)
			if err1 == nil {
				if changed {
