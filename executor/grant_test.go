...
func (s *testSuite) TestWithGrantOption(c *C) {
	...
	tk.MustExec("GRANT select ON test.* TO 'testDB'@'localhost' WITH GRANT OPTION;")
	tk.MustQuery("SELECT grant_priv FROM mysql.DB WHERE User=\"testDB\" and host=\"localhost\" and db=\"test\"").Check(testkit.Rows("Y"))
}
