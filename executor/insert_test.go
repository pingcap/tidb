...
func (s *testSuite10) TestFloatOutRange(c *C) {
	tk := testkit.NewTestKit(c, s.store)
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t1,t2")
	tk.MustExec("create table t1 (a float)")

	tk.MustGetErrCode("insert into t1 values(999999999999999999999999999999999999999)", errno.ErrWarnDataOutOfRange)
	_, err := tk.Exec("insert into t1 values(999999999999999999999999999999999999999)")
	c.Assert(err.Error(), Matches, ".*Out of range value for column 'a' at.*")
	_, err = tk.Exec("insert into t1 values(1),(999999999999999999999999999999999999999)")
	c.Assert(err.Error(), Matches, ".*Out of range value for column 'a' at row 2")
	tk.MustGetErrCode("replace into t1 values(999999999999999999999999999999999999999)", errno.ErrWarnDataOutOfRange)
	_, err = tk.Exec("replace into t1 values(999999999999999999999999999999999999999)")
	c.Assert(err.Error(), Matches, ".*Out of range value for column 'a' at.*")
	_, err = tk.Exec("insert ignore into t1 values(999999999999999999999999999999999999999)")
	tk.MustQuery("show warnings").Check(testutil.RowsWithSep("|", "Warning|1264|Out of range value for column 'a' at row 1"))
	tk.MustExec("create table t2 (a double)")
	tk.MustExec("insert into t2 values(999999999999999999999999999999999999999)")
	tk.MustGetErrCode("insert into t1 select * from t2", errno.ErrWarnDataOutOfRange)
	_, err = tk.Exec("insert into t1 select * from t2")
	c.Assert(err.Error(), Matches, ".*Out of range value for column 'a' at.*")
	...
	c.Assert(err.Error(), Matches, ".*Out of range value for column 'a' at.*")
	tk.MustGetErrCode("insert into t1 set a = 999999999999999999999999999999999999999", errno.ErrWarnDataOutOfRange)
	_, err = tk.Exec("insert into t1 set a = 999999999999999999999999999999999999999")
	c.Assert(err.Error(), Matches, ".*Out of range value for column 'a' at.*")
