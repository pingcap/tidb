	...
	for _, c := range cases {
		b.Run(fmt.Sprintf("MapRows %v", c.rowNum), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				if getB(aggMap) != c.expectedB {
					fmt.Println(getB(aggMap))
					panic("B is different from estimated B in aggPartialResultMapper")
				}
				if getGrowing(aggMap) != c.expectedGrowing {
					panic("Growing is different from estimated growing in aggPartialResultMapper")
				}
			}
		})
	}
