...
func (s *testSuite1) TestIndexJoinUnionScan(c *C) {
	...
	tk.MustQuery("explain select /*+ TIDB_INLJ(t1, t2)*/ * from t1 join t2 on t1.a = t2.id").Check(testkit.Rows(
		...
		"│   └─Selection_14 9990.00 cop[tikv] not(isnull(test.t1.a))",
		"│     └─TableScan_13 10000.00 cop[tikv] table:t1, range:[-inf,+inf], keep order:false, stats:pseudo",
		"└─UnionScan_10 1.00 root ",
		"  └─TableReader_9 1.00 root data:TableScan_8",
		"    └─TableScan_8 1.00 cop[tikv] table:t2, range: decided by [test.t1.a], keep order:false, stats:pseudo",
	))
	...
	tk.MustQuery("explain select /*+ TIDB_INLJ(t1, t2)*/ * from t1 join t2 on t1.a = t2.a").Check(testkit.Rows(
		...
		"│   └─Selection_16 9990.00 cop[tikv] not(isnull(test.t1.a))",
		...
		"    ├─Selection_10 9.99 cop[tikv] not(isnull(test.t2.a))",
		"    │ └─IndexScan_8 10.00 cop[tikv] table:t2, index:a, range: decided by [eq(test.t2.a, test.t1.a)], keep order:false, stats:pseudo",
		"    └─TableScan_9 9.99 cop[tikv] table:t2, keep order:false, stats:pseudo",
	))
	...
	tk.MustQuery("explain select /*+ TIDB_INLJ(t1, t2)*/ t1.a, t2.a from t1 join t2 on t1.a = t2.a").Check(testkit.Rows(
		...
		"      └─Selection_9 9.99 cop[tikv] not(isnull(test.t2.a))",
		"        └─IndexScan_8 10.00 cop[tikv] table:t2, index:a, range: decided by [eq(test.t2.a, test.t1.a)], keep order:false, stats:pseudo",
	))
...
func (s *testSuite1) TestBatchIndexJoinUnionScan(c *C) {
	...
	tk.MustQuery("explain select /*+ TIDB_INLJ(t1, t2)*/ count(*) from t1 join t2 on t1.a = t2.a").Check(testkit.Rows(
		...
		"  │   └─Selection_21 9990.00 cop[tikv] not(isnull(test.t1.a))",
		...
		"      └─Selection_24 9.99 cop[tikv] not(isnull(test.t2.a))",
		"        └─IndexScan_23 10.00 cop[tikv] table:t2, index:a, range: decided by [eq(test.t2.a, test.t1.a)], keep order:false, stats:pseudo",
	))
