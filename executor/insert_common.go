...
func completeInsertErr(col *model.ColumnInfo, val *types.Datum, rowIdx int, err error) error {
	...
	if types.ErrDataTooLong.Equal(err) {
		...
		err = errors2.ErrTruncateWrongInsertValue.GenWithStackByArgs(types.TypeStr(colTp), valStr, colName, rowIdx+1)
...
func (e *InsertValues) doBatchInsert(ctx context.Context) error {
	...
	if err != nil {
		return errors2.ErrBatchInsertFail.GenWithStack("BatchInsert failed with error: %v", err)
	}
