...
func (e *InsertRuntimeStat) String() string {
	if e.CheckInsertTime == 0 {
		...
		if e.Prefetch > 0 && e.SnapshotRuntimeStats != nil {
			return fmt.Sprintf("prefetch: %v, rpc:{%v}", execdetails.FormatDurationForExplain(e.Prefetch), e.SnapshotRuntimeStats.String())
		}
		return ""
	}
	...
	if e.Prefetch > 0 {
		...
		buf.WriteString(fmt.Sprintf("insert:%v", execdetails.FormatDurationForExplain(e.CheckInsertTime)))
	}
	return buf.String()
}
