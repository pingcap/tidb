...
type testWindowFunctionsOpts struct {
	disableSection1 bool
	disableRest     bool
}
...
func (s *testSuite7) TestWindowFunctions(c *C) {
	...
	doTestWindowFunctions(tk, testWindowFunctionsOpts{})
}
...
func (s *testSuite7) TestPipelinedWindowParallelFunctions(c *C) {
	...
	doTestWindowFunctions(tk, testWindowFunctionsOpts{
		disableSection1: true,
		disableRest:     true,
	})
}

func doTestWindowFunctions(tk *testkit.TestKit, opt testWindowFunctionsOpts) {
...
type testSlidingWindowFunctionsOpts struct {
	disablePrepare     bool
	disableCountRows   bool
	disableCountRange  bool
	disableSumRows     bool
	disableSumRange    bool
	disableAvgRows     bool
	disableAvgRange    bool
	disableBitXorRows  bool
	disableBitXorRange bool
	disableMinRows     bool
	disableMinRange    bool
	disableMaxRows     bool
	disableMaxRange    bool
}
	...
	for _, idType := range idTypes {
		for _, useHighPrecision := range useHighPrecisions {
			...
			baseTestSlidingWindowFunctions(tk, testSlidingWindowFunctionsOpts{
				...
				disableBitXorRange: true,
				disableMinRows:     true,
				disableMinRange:    true,
				disableMaxRows:     true,
				disableMaxRange:    true,
			})
		}
	}
}

func baseTestSlidingWindowFunctions(tk *testkit.TestKit, opt testSlidingWindowFunctionsOpts) {
	var result *testkit.Result
