...
func TestUsingAndNaturalJoinSchema(t *testing.T) {
	...
	tk.MustExec("create table t3 (b int, c int);")
	tk.MustExec("create table t4 (y int, c int);")
	tk.MustGetErrMsg(`select * from t1 natural join (t3 cross join t4);`, "[planner:1052]Column 'c' in from clause is ambiguous")
	tk.MustGetErrMsg(`select * from (t3 cross join t4) natural join t1;`, "[planner:1052]Column 'c' in from clause is ambiguous")
	tk.MustGetErrMsg(`select * from (t1 join t2 on t1.b=t2.b) natural join (t3 natural join t4);`, "[planner:1052]Column 'b' in from clause is ambiguous")
	tk.MustGetErrMsg(`select * from  (t3 natural join t4) natural join (t1 join t2 on t1.b=t2.b);`, "[planner:1052]Column 'b' in from clause is ambiguous")
}
