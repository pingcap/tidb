...
func (s *testSuite) TestInSubquery(c *C) {
	...
	result.Check(testkit.Rows("1"))
	result = tk.MustQuery("select m1.a from t as m1 where (3, m1.b) not in (select * from t as m2) order by m1.a")
	result.Check(testkit.Rows("1", "2"))
	...
	result = tk.MustQuery("select * from t1 where a in (select * from t2) order by a")
	result.Check(testkit.Rows("1", "2"))
	result = tk.MustQuery("select * from t1 where a in (select * from t2 where false)")
	result.Check(testkit.Rows())
	result = tk.MustQuery("select * from t1 where a not in (select * from t2 where false) order by a")
	result.Check(testkit.Rows("1", "2"))
