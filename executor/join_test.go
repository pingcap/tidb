...
func (s *testSuiteJoin2) TestJoin(c *C) {
	...
	// Test SWAP_JOIN_INPUTS and NO_SWAP_JOIN_INPUTS for hash joins in outer joins
	result = tk.MustQuery("select /*+ HASH_JOIN(t), SWAP_JOIN_INPUTS(t) */ * from t left outer join t1 on t.c1 = t1.c1 where t.c1 = 1 or t1.c2 > 20")
	result.Check(testkit.Rows("1 1 <nil> <nil>"))
	result = tk.MustQuery("select /*+ HASH_JOIN(t), SWAP_JOIN_INPUTS(t) */ * from t1 right outer join t on t.c1 = t1.c1 where t.c1 = 1 or t1.c2 > 20")
	result.Check(testkit.Rows("<nil> <nil> 1 1"))
	result = tk.MustQuery("select /*+ HASH_JOIN(t), SWAP_JOIN_INPUTS(t) */ * from t right outer join t1 on t.c1 = t1.c1 where t.c1 = 1 or t1.c2 > 20")
	result.Check(testkit.Rows())
	result = tk.MustQuery("select /*+ HASH_JOIN(t), SWAP_JOIN_INPUTS(t) */ * from t left outer join t1 on t.c1 = t1.c1 where t1.c1 = 3 or false")
	result.Check(testkit.Rows())
	result = tk.MustQuery("select /*+ HASH_JOIN(t), SWAP_JOIN_INPUTS(t) */ * from t left outer join t1 on t.c1 = t1.c1 and t.c1 != 1 order by t1.c1")
	result.Check(testkit.Rows("1 1 <nil> <nil>", "2 2 2 3"))

	result = tk.MustQuery("select /*+ HASH_JOIN(t1), SWAP_JOIN_INPUTS(t1) */ * from t left outer join t1 on t.c1 = t1.c1 where t.c1 = 1 or t1.c2 > 20")
	result.Check(testkit.Rows("1 1 <nil> <nil>"))
	result = tk.MustQuery("select /*+ HASH_JOIN(t1), SWAP_JOIN_INPUTS(t1) */ * from t1 right outer join t on t.c1 = t1.c1 where t.c1 = 1 or t1.c2 > 20")
	result.Check(testkit.Rows("<nil> <nil> 1 1"))
	result = tk.MustQuery("select /*+ HASH_JOIN(t1), SWAP_JOIN_INPUTS(t1) */ * from t right outer join t1 on t.c1 = t1.c1 where t.c1 = 1 or t1.c2 > 20")
	result.Check(testkit.Rows())
