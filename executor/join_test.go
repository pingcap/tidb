...
func (s *testSuiteJoin1) TestIndexLookupJoin(c *C) {
	...
	tk.MustQuery("desc select /*+ TIDB_INLJ(s) */ count(*) from t join s use index(idx) on s.a = t.a and s.b < t.b").Check(testkit.Rows(
		...
		"└─IndexJoin_24 12475.01 root inner join, inner:IndexReader_23, outer key:test.t.a, inner key:test.s.a, other cond:lt(test.s.b, test.t.b)",
		"  ├─TableReader_19 9990.00 root data:Selection_18",
		"  │ └─Selection_18 9990.00 cop not(isnull(test.t.b))",
		"  │   └─TableScan_17 10000.00 cop table:t, range:[-inf,+inf], keep order:false, stats:pseudo",
		"  └─IndexReader_23 9.98 root index:Selection_22",
		"    └─Selection_22 9.98 cop not(isnull(test.s.a)), not(isnull(test.s.b))",
		"      └─IndexScan_21 10.00 cop table:s, index:a, b, range: decided by [eq(test.s.a, test.t.a) lt(test.s.b, test.t.b)], keep order:false, stats:pseudo"))
