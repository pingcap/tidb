	...
	for e.cursor == len(e.resultRows) {
		...
		for i := 0; i < batchSize; i++ {
			...
			if match {
				...
				e.innerDatums = append(e.innerDatums, joinDatums)
			} else if e.outer {
...
func (e *IndexLookUpJoin) doJoin() error {
	err := e.innerExec.doRequestForDatums(e.innerDatums, e.ctx.GoCtx())
	if err != nil {
