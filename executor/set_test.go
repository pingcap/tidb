...
func (s *testSerialSuite1) TestSetVar(c *C) {
	...
	// test for tidb_keep_pruning_conds
	tk.MustQuery(`select @@tidb_keep_pruning_conds`).Check(testkit.Rows("0"))
	tk.MustExec(`set global tidb_keep_pruning_conds = 1`)
	tk.MustQuery(`select @@global.tidb_keep_pruning_conds`).Check(testkit.Rows("1"))
	tk.MustExec(`set global tidb_keep_pruning_conds = 0`)
	tk.MustQuery(`select @@global.tidb_keep_pruning_conds`).Check(testkit.Rows("0"))
	tk.MustExec(`set tidb_keep_pruning_conds=1`)
	tk.MustQuery(`select @@global.tidb_keep_pruning_conds`).Check(testkit.Rows("0"))
	tk.MustQuery(`select @@tidb_keep_pruning_conds`).Check(testkit.Rows("1"))
