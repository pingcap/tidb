...
func (s *testSuite2) TestSetVar(c *C) {
	...
	tk.MustExec("set @@tidb_show_plan_in_slow_log = 1")
	tk.MustQuery("select @@tidb_show_plan_in_slow_log;").Check(testkit.Rows("1"))
	tk.MustExec("set @@tidb_show_plan_in_slow_log = 0")
	tk.MustQuery("select @@tidb_show_plan_in_slow_log;").Check(testkit.Rows("0"))
}
