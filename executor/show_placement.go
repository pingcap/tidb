...
func (e *ShowExec) fetchAllPlacementPolicies() error {
	...
	slices.SortFunc(policies, func(i, j *model.PolicyInfo) int { return strings.Compare(i.Name.O, j.Name.O) })
...
func (e *ShowExec) fetchAllDBPlacements(ctx context.Context, scheduleState map[int64]infosync.PlacementScheduleState) error {
	...
	dbs := e.is.AllSchemas()
	slices.SortFunc(dbs, func(i, j *model.DBInfo) int { return strings.Compare(i.Name.O, j.Name.O) })
...
func (e *ShowExec) fetchAllTablePlacements(ctx context.Context, scheduleState map[int64]infosync.PlacementScheduleState) error {
	...
	for _, dbInfo := range dbs {
		...
		slices.SortFunc(tableRowSets, func(i, j tableRowSet) int { return strings.Compare(i.name, j.name) })
