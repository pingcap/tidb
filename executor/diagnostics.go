...
func (currentLoadInspection) inspect(_ context.Context, sctx sessionctx.Context, filter inspectionFilter) []inspectionResult {
	...
	var rules = []struct {
	...
	}{
		{
			...
			"select type, instance, value from inspection_schema.cluster_load where device_type='memory' and device_name='virtual' and load_name='used-percent' and load_value > 0.7",
			"<0.7",
			commonResult,
		},
		{
			"swap-memory-usage",
			"select type, instance, value from inspection_schema.cluster_load where device_type='memory' and device_name='swap' and load_name='used-percent' and load_value > 0",
			"0",
			commonResult,
		},
		{
		...
		{
			"cpu-load1",
			"select type, instance, value from inspection_schema.cluster_load where device_type='cpu' and device_name='cpu' and load_name='load1' and load_value>0.7;",
			"<0.7",
			commonResult,
		},
		{
			"cpu-load5",
			"select type, instance, value from inspection_schema.cluster_load where device_type='cpu' and device_name='cpu' and load_name='load5' and load_value>0.7;",
			"<0.7",
			commonResult,
		},
		{
			"cpu-load15",
			"select type, instance, value from inspection_schema.cluster_load where device_type='cpu' and device_name='cpu' and load_name='load15' and load_value>0.7;",
			"<0.7",
			commonResult,
		},
	}
