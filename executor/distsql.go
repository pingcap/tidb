...
func (e *TableReaderExecutor) Open(goCtx goctx.Context) error {
	...
	e.result, err = distsql.SelectDAG(goCtx, e.ctx, e.ctx.GetClient(), kvReq, e.schema.GetTypes())
	if err != nil {
		return errors.Trace(err)
	}
...
func (e *IndexLookUpExecutor) startIndexWorker(goCtx goctx.Context, kvRanges []kv.KeyRange, workCh chan<- *lookupTableTask) error {
	...
	result, err := distsql.SelectDAG(goCtx, e.ctx, e.ctx.GetClient(), kvReq, []*types.FieldType{types.NewFieldType(mysql.TypeLonglong)})
	if err != nil {
		return errors.Trace(err)
	}
	result.Fetch(goCtx)
