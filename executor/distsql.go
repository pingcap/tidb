...
func (e *IndexReaderExecutor) Next(ctx context.Context, req *execution.ExecRequest) error {
	...
	err := e.result.Next(ctx, req.RetChunk)
	if err != nil {
...
func (e *IndexLookUpExecutor) Next(ctx context.Context, req *execution.ExecRequest) error {
	if e.runtimeStats != nil {
		start := time.Now()
		defer func() { e.runtimeStats.Record(time.Now().Sub(start), req.RetChunk.NumRows()) }()
	}
	...
	for {
		...
		for resultTask.cursor < len(resultTask.rows) {
			req.RetChunk.AppendRow(resultTask.rows[resultTask.cursor])
			resultTask.cursor++
			if req.RetChunk.NumRows() >= e.maxChunkSize {
				return nil
			}
		}
	}
...
func (w *tableWorker) executeTask(ctx context.Context, task *lookupTableTask) error {
	...
	for {
		...
		err = tableReader.Next(ctx, &execution.ExecRequest{RetChunk: chk})
		if err != nil {
			log.Error(err)
			return errors.Trace(err)
		}
