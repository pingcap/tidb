...
func appendPartitionInfo(partitionInfo *model.PartitionInfo, buf *bytes.Buffer) {
	...
	if partitionInfo.SubPart == nil {
		...
		for i, def := range partitionInfo.ParentDef {
			lessThans := strings.Join(def.LessThan, ",")
			fmt.Fprintf(buf, "  PARTITION `%s` VALUES LESS THAN (%s) (\n", def.Name, lessThans)
			for j := 0; j < int(partitionInfo.SubPart.Num); j++ {
				fmt.Fprintf(buf, "    SUBPARTITION `%s`", partitionInfo.Definitions[g].Name)
				g++
				if j < int(partitionInfo.SubPart.Num)-1 {
					buf.WriteString(",\n")
				} else {
					buf.WriteString("\n")
				}
