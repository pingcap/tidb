...
func (builder *dataReaderBuilder) buildExecutorForIndexJoin(ctx context.Context, lookUpContents []*indexJoinLookUpContent,
	...
	innerJoinKeys []*expression.Column) (Executor, error) {
...
func (builder *dataReaderBuilder) buildProjectionForIndexJoin(ctx context.Context, v *plannercore.PhysicalProjection,
	...
	for _, ran := range indexRanges {
		for i, key := range innerJoinKeys {
			...
			expr := v.Exprs[idx]
			var err error
			ran.LowVal[i], err = expr.ReverseEval(builder.ctx.GetSessionVars().StmtCtx, ran.LowVal[i], expression.Floor)
			if err != nil {
				return nil, err
			}
			ran.HighVal[i], err = expr.ReverseEval(builder.ctx.GetSessionVars().StmtCtx, ran.HighVal[i], expression.Ceiling)
			if err != nil {
				return nil, err
			}
		}
	...
	childBuilder := &dataReaderBuilder{Plan: v.Children()[0], executorBuilder: builder.executorBuilder}
	childExec, err := childBuilder.buildExecutorForIndexJoin(ctx, lookUpContents, indexRanges, keyOff2IdxOff, cwc, innerJoinKeys)
	if err != nil {
		return nil, err
	}
