...
func (b *executorBuilder) buildCancelDDLJobs(v *plannercore.CancelDDLJobs) Executor {
	e := &CancelDDLJobsExec{
		baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID()),
		jobIDs:       v.JobIDs,
	}
...
func (b *executorBuilder) buildDeallocate(v *plannercore.Deallocate) Executor {
	base := newBaseExecutor(b.ctx, nil, v.ExplainID(), v.ID())
...
func (b *executorBuilder) buildSelectLock(v *plannercore.PhysicalLock) Executor {
	...
	e := &SelectLockExec{
		baseExecutor:     newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), src),
		Lock:             v.Lock,
...
func (b *executorBuilder) buildLimit(v *plannercore.PhysicalLimit) Executor {
	...
	base := newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), childExec)
	base.initCap = n
	e := &LimitExec{
		baseExecutor: base,
		begin:        v.Offset,
...
func (b *executorBuilder) buildPrepare(v *plannercore.Prepare) Executor {
	base := newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID())
	base.initCap = chunk.ZeroCapacity
	return &PrepareExec{
		baseExecutor: base,
		is:           b.is,
...
func (b *executorBuilder) buildInsert(v *plannercore.Insert) Executor {
	...
	if selectExec != nil {
		baseExec = newBaseExecutor(b.ctx, nil, v.ExplainID(), v.ID(), selectExec)
	} else {
		baseExec = newBaseExecutor(b.ctx, nil, v.ExplainID(), v.ID())
	}
...
func (b *executorBuilder) buildLoadData(v *plannercore.LoadData) Executor {
	...
	insertVal := &InsertValues{
		baseExecutor: newBaseExecutor(b.ctx, nil, v.ExplainID(), v.ID()),
		Table:        tbl,
		Columns:      v.Columns,
		GenExprs:     v.GenCols.Exprs,
	}
...
func (b *executorBuilder) buildGrant(grant *ast.GrantStmt) Executor {
	e := &GrantExec{
		baseExecutor: newBaseExecutor(b.ctx, nil, grantStmtLabel, 0),
		Privs:        grant.Privs,
...
func (b *executorBuilder) buildRevoke(revoke *ast.RevokeStmt) Executor {
	e := &RevokeExec{
		baseExecutor: newBaseExecutor(b.ctx, nil, revokeStmtLabel, 0),
		ctx:          b.ctx,
		Privs:        revoke.Privs,
...
func (b *executorBuilder) buildTrace(v *plannercore.Trace) Executor {
	...
	if t.format == plannercore.TraceFormatLog {
		return &SortExec{
			baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), t),
...
func (b *executorBuilder) buildSelectInto(v *plannercore.SelectInto) Executor {
	...
	return &SelectIntoExec{
		baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), child),
		intoOpt:      v.IntoOpt,
	}
}
...
func (b *executorBuilder) buildUnionScanFromReader(reader Executor, v *plannercore.PhysicalUnionScan) Executor {
	us := &UnionScanExec{baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), reader)}
...
func (b *executorBuilder) buildMergeJoin(v *plannercore.PhysicalMergeJoin) Executor {
	...
	e := &MergeJoinExec{
		...
		baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), leftExec, rightExec),
...
func (b *executorBuilder) buildSelection(v *plannercore.PhysicalSelection) Executor {
	...
	e := &SelectionExec{
		baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), childExec),
...
func (b *executorBuilder) buildApply(v *plannercore.PhysicalApply) Executor {
	...
	serialExec := &NestedLoopApplyExec{
		baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), outerExec, innerExec),
		innerExec:    innerExec,
		outerExec:    outerExec,
	...
	if v.Concurrency > 1 {
		...
		return &ParallelNestedLoopApplyExec{
			baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), allExecs...),
...
func (b *executorBuilder) buildUnionAll(v *plannercore.PhysicalUnionAll) Executor {
	...
	e := &UnionExec{
		baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), childExecs...),
	}
	return e
}
...
func (b *executorBuilder) buildUpdate(v *plannercore.Update) Executor {
	...
	base := newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), selExec)
	base.initCap = chunk.ZeroCapacity
...
func (b *executorBuilder) buildIndexLookUpJoin(v *plannercore.PhysicalIndexJoin) Executor {
	...
	e := &IndexLookUpJoin{
		baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID(), outerExec),
		outerCtx: outerCtx{
			rowTypes: outerTypes,
			filter:   outerFilter,
		},
...
func (builder *dataReaderBuilder) buildExecutorForIndexJoinInternal(ctx context.Context, plan plannercore.Plan, lookUpContents []*indexJoinLookUpContent,
	...
	case *plannercore.PhysicalSelection:
		...
		exec := &SelectionExec{
			baseExecutor: newBaseExecutor(builder.ctx, v.Schema(), v.ExplainID(), v.ID(), childExec),
			filters:      v.Conditions,
		}
		err = exec.open(ctx)
		return exec, err
...
func (b *executorBuilder) buildShuffle(v *plannercore.PhysicalShuffle) *ShuffleExec {
	...
	for i := range shuffle.workers {
		w := &shuffleWorker{
			baseExecutor: newBaseExecutor(b.ctx, v.DataSource.Schema(), v.DataSource.ExplainID(), v.DataSource.ID()),
		}
...
func (b *executorBuilder) buildBatchPointGet(plan *plannercore.BatchPointGetPlan) Executor {
	...
	e := &BatchPointGetExec{
		baseExecutor: newBaseExecutor(b.ctx, plan.Schema(), plan.ExplainID(), plan.ID()),
...
func (b *executorBuilder) buildAdminShowTelemetry(v *plannercore.AdminShowTelemetry) Executor {
	return &AdminShowTelemetryExec{baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID())}
}

func (b *executorBuilder) buildAdminResetTelemetryID(v *plannercore.AdminResetTelemetryID) Executor {
	return &AdminResetTelemetryIDExec{baseExecutor: newBaseExecutor(b.ctx, v.Schema(), v.ExplainID(), v.ID())}
}
