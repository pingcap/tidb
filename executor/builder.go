...
func (b *executorBuilder) build(p plannercore.Plan) Executor {
	...
	case *plannercore.PhysicalFilter:
		return b.buildSelection(v)
	case *plannercore.PhysicalHashAgg:
		return b.buildHashAgg(v)
	case *plannercore.PhysicalStreamAgg:
		return b.buildStreamAgg(v)
	case *plannercore.PhysicalProject:
	...
	case *plannercore.PhysicalIndexLookupGather:
	...
	case *plannercore.PhysicalIndexMerge:
...
func buildIndexLookUpChecker(b *executorBuilder, p *plannercore.PhysicalIndexLookupGather,
...
func (b *executorBuilder) buildSelection(v *plannercore.PhysicalFilter) Executor {
	childExec := b.build(v.Children()[0])
	if b.err != nil {
		return nil
	}
...
func (b *executorBuilder) buildProjection(v *plannercore.PhysicalProject) Executor {
	childExec := b.build(v.Children()[0])
	if b.err != nil {
		return nil
	}
...
func (b *executorBuilder) corColInDistPlan(plans []plannercore.PhysicalPlan) bool {
	for _, p := range plans {
		x, ok := p.(*plannercore.PhysicalFilter)
		if !ok {
			continue
		}
...
func buildNoRangeIndexLookUpReader(b *executorBuilder, v *plannercore.PhysicalIndexLookupGather) (*IndexLookUpExecutor, error) {
	is := v.IndexPlans[0].(*plannercore.PhysicalIndexScan)
	var handleLen int
...
func (b *executorBuilder) buildIndexLookUpReader(v *plannercore.PhysicalIndexLookupGather) Executor {
	if b.ctx.GetSessionVars().IsPessimisticReadConsistency() {
		if err := b.refreshForUpdateTSForRC(); err != nil {
			b.err = err
			return nil
...
func buildNoRangeIndexMergeReader(b *executorBuilder, v *plannercore.PhysicalIndexMerge) (*IndexMergeReaderExecutor, error) {
...
func (b *executorBuilder) buildIndexMergeReader(v *plannercore.PhysicalIndexMerge) Executor {
...
func (builder *dataReaderBuilder) buildIndexLookUpReaderForIndexJoin(ctx context.Context, v *plannercore.PhysicalIndexLookupGather,
	lookUpContents []*indexJoinLookUpContent, indexRanges []*ranger.Range, keyOff2IdxOff []int, cwc *plannercore.ColWithCmpFuncManager) (Executor, error) {
...
func (builder *dataReaderBuilder) buildProjectionForIndexJoin(ctx context.Context, v *plannercore.PhysicalProject,
	lookUpContents []*indexJoinLookUpContent, indexRanges []*ranger.Range, keyOff2IdxOff []int, cwc *plannercore.ColWithCmpFuncManager) (Executor, error) {
	physicalIndexLookUp, isDoubleRead := v.Children()[0].(*plannercore.PhysicalIndexLookupGather)
	if !isDoubleRead {
