...
func TestUnionScanCs(t *testing.T) {
	...
	*/
	...
	// case-8: in pessimistic transaction, duplicate primary key in insert, and primary key is not handle.
	tk.MustExec("set @@tidb_enable_clustered_index = 0;")
	tk.MustExec("set @@tidb_constraint_check_in_place_pessimistic=on")
	tk.MustExec("drop table if exists t")
	tk.MustExec("create table t (pk varchar(10) key, val int)")
	tk.MustExec("insert into t values (1,1)")
	tk.MustExec("begin pessimistic")
...
func TestUnionScanIssue24195(t *testing.T) {
	...
	// case-0: query return duplicate primary key
	...
	go func() {
		...
		tk2.MustQuery("select * from t use index(primary) order by c1, c2;").Check(testkit.Rows("a 1", "a 2")) // expect is "a 2"
	...
	// case-1: query return duplicate unique key
...
func TestUnionScanIssue24195Debug(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk1 := testkit.NewTestKit(t, store)
	tk1.MustExec("use test")
	tk1.MustExec("set @@tidb_constraint_check_in_place = false")
