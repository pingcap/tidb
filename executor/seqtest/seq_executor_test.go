...
func HelperTestAdminShowNextID(c *C, s *seqTestSuite, str string) {
	...
	r.Check(testkit.Rows("test t _tidb_rowid 1 _TIDB_ROW_ID"))
	// Row ID is step + 1.
	tk.MustExec("insert into t values(1, 1)")
	r = tk.MustQuery(str + " t next_row_id")
	r.Check(testkit.Rows("test t _tidb_rowid 11 _TIDB_ROW_ID"))
	// Row ID is original + step.
	for i := 0; i < int(step); i++ {
		tk.MustExec("insert into t values(10000, 1)")
	}
	r = tk.MustQuery(str + " t next_row_id")
	r.Check(testkit.Rows("test t _tidb_rowid 21 _TIDB_ROW_ID"))
	tk.MustExec("drop table t")
	...
	r.Check(testkit.Rows("test1 seq1  15 SEQUENCE"))
	r = tk.MustQuery("select nextval(seq1)")
	r.Check(testkit.Rows("15"))
	r = tk.MustQuery(str + " seq1 next_row_id")
	r.Check(testkit.Rows("test1 seq1  72 SEQUENCE"))
	r = tk.MustQuery("select nextval(seq1)")
	...
	r = tk.MustQuery(str + " seq1 next_row_id")
	r.Check(testkit.Rows("test1 seq1  97 SEQUENCE"))
}
