...
func TestPrepareDealloc(t *testing.T) {
	...
	require.Equal(t, 0, tk.Session().GetPlanCache(false).(*plannercore.LRUPlanCache).Size())
	...
	require.Equal(t, 3, tk.Session().GetPlanCache(false).(*plannercore.LRUPlanCache).Size())
	...
	tk.MustExec(`execute stmt2`)
	require.Equal(t, 1, tk.Session().GetPlanCache(false).(*plannercore.LRUPlanCache).Size()) // use the same cached plan since they have the same statement
	...
	tk.MustExec(`execute stmt3`)
	require.Equal(t, 2, tk.Session().GetPlanCache(false).(*plannercore.LRUPlanCache).Size()) // stmt3 has different DB
}
