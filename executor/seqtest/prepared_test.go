...
func TestPrepareWithAggregation(t *testing.T) {
	...
	for _, flag := range flags {
		...
		se, err := session.CreateSession4TestWithOpt(store, &session.Opt{
			PreparedPlanCache: plannercore.NewLRUPlanCache(100, 0.1, math.MaxUint64, plannercore.PickPlanFromBucket, true),
		})
		require.NoError(t, err)
		tk.SetSession(se)
...
func TestPrepareDealloc(t *testing.T) {
	...
	se, err := session.CreateSession4TestWithOpt(store, &session.Opt{
		PreparedPlanCache: plannercore.NewLRUPlanCache(3, 0.1, math.MaxUint64, plannercore.PickPlanFromBucket, true),
	})
	require.NoError(t, err)
	tk.SetSession(se)
