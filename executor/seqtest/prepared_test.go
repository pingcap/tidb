...
func TestPrepared(t *testing.T) {
	...
	for _, flag := range flags {
		...
		require.NoError(t, err)
		rs, err := tk.Session().ExecutePreparedStmt(ctx, stmtID, []expression.Expression{expression.Value2Expression4Test(1)})
		require.NoError(t, err)
		...
		rs, err = tk.Session().ExecutePreparedStmt(ctx, stmtID, []expression.Expression{expression.Value2Expression4Test(3)})
		require.NoError(t, err)
		...
		rs, err = tk.Session().ExecutePreparedStmt(ctx, stmtID, []expression.Expression{expression.Value2Expression4Test(4)})
		require.NoError(t, err)
		tk.ResultSetToResult(rs, fmt.Sprintf("%v", rs)).Check(testkit.Rows())

		execStmt := &ast.ExecuteStmt{ExecID: stmtID, BinaryArgs: []expression.Expression{expression.Value2Expression4Test(1)}}
		...
		_, err = tk.Session().ExecutePreparedStmt(ctx, stmtID, []expression.Expression{expression.Value2Expression4Test(1)})
		...
		require.NoError(t, err)
		_, err = tk.Session().ExecutePreparedStmt(ctx, stmtID, []expression.Expression{expression.Value2Expression4Test(1), expression.Value2Expression4Test(1)})
		require.NoError(t, err)
	}
}
...
func TestPreparedIssue17419(t *testing.T) {
	...
	rs, err := tk1.Session().ExecutePreparedStmt(ctx, stmtID, []expression.Expression{expression.Value2Expression4Test(nil)})
	require.NoError(t, err)
