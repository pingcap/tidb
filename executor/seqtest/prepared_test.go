...
func (s *seqTestSuite) TestPreparedIssue8644(c *C) {
	...
	for _, flag := range flags {
		...
		tk.MustExec("create table t3(data decimal(10,3));")
		tk.MustExec("prepare stmt3 from 'insert t3 (data) values (?)';")
		tk.MustExec("set @a = 1.1;")
		tk.MustExec("execute stmt3 using @a;")
		tk.MustExec("set @b = 11.11;")
		tk.MustExec("execute stmt3 using @b;")
