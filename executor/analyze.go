...
func (e *AnalyzeExec) saveAnalyzeOptsV2() error {
	...
	_, _, err := exec.ExecRestrictedSQL(context.TODO(), sql.String())
	if err != nil {
		return err
	}
	return nil
...
func (e *AnalyzeFastExec) calculateEstimateSampleStep() (err error) {
	exec := e.ctx.(sqlexec.RestrictedSQLExecutor)
	rows, _, err := exec.ExecRestrictedSQL(context.TODO(), "select flag from mysql.stats_histograms where table_id = %?", e.tableID.GetStatisticsID())
	if err != nil {
		return
	}
