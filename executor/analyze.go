	...
	for _, task := range tasks {
		...
		if _, ok := tidMap[tableID]; !ok {
			if isLocked {
				...
				if !ok {
					log.Warn("Unknown table ID in analyze task", zap.Int64("tid", tableID))
				} else {
...
func (e *AnalyzeExec) analyzeWorker(taskCh <-chan *analyzeTask, resultsCh chan<- *statistics.AnalyzeResults) {
	...
	defer func() {
		if r := recover(); r != nil {
			log.Error("analyze worker panicked", zap.Any("recover", r), zap.Stack("stack"))
...
func AddNewAnalyzeJob(ctx sessionctx.Context, job *statistics.AnalyzeJob) {
	...
	if err != nil {
		log.Error("failed to insert analyze job", zap.Error(err))
	}
}
...
func StartAnalyzeJob(sctx sessionctx.Context, job *statistics.AnalyzeJob) {
	...
	if err != nil {
		log.Warn("failed to update analyze job", zap.String("update", fmt.Sprintf("%s->%s", statistics.AnalyzePending, statistics.AnalyzeRunning)), zap.Error(err))
	}
}
...
func UpdateAnalyzeJob(sctx sessionctx.Context, job *statistics.AnalyzeJob, rowCount int64) {
	...
	if err != nil {
		log.Warn("failed to update analyze job", zap.String("update", fmt.Sprintf("process %v rows", delta)), zap.Error(err))
	}
}
...
func FinishAnalyzeMergeJob(sctx sessionctx.Context, job *statistics.AnalyzeJob, analyzeErr error) {
	...
	if err != nil {
		...
		log.Warn("failed to update analyze job", zap.String("update", fmt.Sprintf("%s->%s", statistics.AnalyzeRunning, state)), zap.Error(err))
	}
}
...
func finishJobWithLog(sctx sessionctx.Context, job *statistics.AnalyzeJob, analyzeErr error) {
	...
	if job != nil {
		...
		log.Info(fmt.Sprintf("analyze table `%s`.`%s` has %s", job.DBName, job.TableName, state),
