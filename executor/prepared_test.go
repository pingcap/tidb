...
func (s *testSuite) TestPrepared(c *C) {
	orgEnable := planner.PreparedPlanCacheEnabled()
	orgCapacity := planner.PreparedPlanCacheCapacity
	defer func() {
		planner.SetPreparedPlanCache(orgEnable)
		planner.PreparedPlanCacheCapacity = orgCapacity
	}()
	flags := []bool{false, true}
	ctx := context.Background()
	for _, flag := range flags {
		planner.SetPreparedPlanCache(flag)
		planner.PreparedPlanCacheCapacity = 100
		...
		c.Assert(planner.ErrWrongParamCount.Equal(err), IsTrue)
		...
		c.Assert(planner.ErrStmtNotFound.Equal(err), IsTrue)
		...
		_, err = tk.Se.ExecutePreparedStmt(ctx, stmtId, 1)
		c.Assert(planner.ErrUnknownColumn.Equal(err), IsTrue)

		tk.MustExec("drop table prepare_test")
		_, err = tk.Se.ExecutePreparedStmt(ctx, stmtId, 1)
		c.Assert(planner.ErrSchemaChanged.Equal(err), IsTrue)
...
func (s *testSuite) TestPreparedLimitOffset(c *C) {
	...
	for _, flag := range flags {
		...
		c.Assert(planner.ErrWrongArguments.Equal(err), IsTrue)
