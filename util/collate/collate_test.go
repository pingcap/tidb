...
func TestUTF8CollatorCompare(t *testing.T) {
	...
	collations := []string{"binary", "utf8mb4_bin", "utf8mb4_general_ci", "utf8mb4_unicode_ci", "gbk_bin"}
	tests := []compareTable{
		{"a", "b", []int{-1, -1, -1, -1, -1}},
		{"a", "A", []int{1, 1, 0, 0, 1}},
		{"À", "A", []int{1, 1, 0, 0, -1}},
		{"abc", "abc", []int{0, 0, 0, 0, 0}},
		{"abc", "ab", []int{1, 1, 1, 1, 1}},
		{"😜", "😃", []int{1, 1, 0, 0, 0}},
		{"a", "a ", []int{-1, 0, 0, 0, 0}},
		{"a ", "a  ", []int{-1, 0, 0, 0, 0}},
		{"a\t", "a", []int{1, 1, 1, 1, 1}},
...
func TestUTF8CollatorKey(t *testing.T) {
	...
	collations := []string{"binary", "utf8mb4_bin", "utf8mb4_general_ci", "utf8mb4_unicode_ci", "gbk_bin"}
	tests := []keyTable{
		{"a", [][]byte{{0x61}, {0x61}, {0x0, 0x41}, {0x0E, 0x33}, {0x61}}},
		{"A", [][]byte{{0x41}, {0x41}, {0x0, 0x41}, {0x0E, 0x33}, {0x41}}},
		...
		{"a ", [][]byte{{0x61, 0x20}, {0x61}, {0x0, 0x41}, {0x0E, 0x33}, {0x61}}},
		{"ﷻ", [][]byte{
			{0xEF, 0xB7, 0xBB},
			{0xEF, 0xB7, 0xBB},
			{0xFD, 0xFB},
