...
// ExponentialAverage is an exponential average measurement implementation.
...
func (m *ExponentialAverage) Add(value float64) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *ExponentialAverage) Get() float64 {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.value
}
