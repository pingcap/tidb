...
func (s *testMyTimeSuite) TestCalcTimeDiff(c *C) {
	cases := []struct{
		...
		{
			mysqlTime{2016, 12, 31, 21, 23, 24, 621234},
			mysqlTime{0, 0, 0, 11, 23, 22, 432126},
			1,
			mysqlTime{2017, 1, 1, 8, 46, 47, 053360},
		},
	}

	for _, t := range cases {
		seconds, microseconds, neg := calcTimeDiff(t.T1, t.T2, t.Sign)
...
func (s *testMyTimeSuite) TestCompareTime(c *C) {
	...
	for _, t := range cases {
		c.Assert(compareTime(t.T1, t.T2), Equals, Expect)
