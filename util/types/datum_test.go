...
func (ts *testTypeConvertSuite) TestToFloat32(c *C) {
	...
	err := datum.ConvertTo(ft)
	c.Assert(err, IsNil)
	c.Assert(datum.Kind(), Equals, KindFloat32)
	c.Assert(datum.GetFloat32(), Equals, float32(281.37))
	...
	err = datum.ConvertTo(ft)
	c.Assert(err, IsNil)
	c.Assert(datum.Kind(), Equals, KindFloat64)
	// Convert to float32 and convert back to float64, we will get a different value.
	c.Assert(datum.GetFloat64(), Not(Equals), 281.37)
}
