	...
	for {
		...
		case <-ticker.C:
			...
			for _, info := range processInfo {
				...
				if time.Since(info.LastExceedExpensiveTimeThreshLogTime) > 60*time.Second && costTime >= time.Second*time.Duration(threshold) && log.GetLevel() <= zapcore.WarnLevel {
					logExpensiveQuery(costTime, info, "expensive_query")
					info.LastExceedExpensiveTimeThreshLogTime = time.Now()
				}
