...
func (s *testCodecSuite) TestHashChunkColumns(c *C) {
	...
	nullBitmap := []byte{0}
	vecHash := []io.Writer{fnv.New64(), fnv.New64(), fnv.New64()}
	rowHash := []io.Writer{fnv.New64(), fnv.New64(), fnv.New64()}

	// Test hash value of the first `Null` column
	c.Assert(chk.GetRow(0).IsNull(0), Equals, true)
	err1 := HashChunkColumns(sc, vecHash, chk, tps[0], 0, buf, nullBitmap)
	...
	c.Assert(nullBitmap[0], Equals, byte(0x07))
	c.Assert(vecHash[0].(hash.Hash64).Sum64(), Equals, rowHash[0].(hash.Hash64).Sum64())
	c.Assert(vecHash[1].(hash.Hash64).Sum64(), Equals, rowHash[1].(hash.Hash64).Sum64())
	c.Assert(vecHash[2].(hash.Hash64).Sum64(), Equals, rowHash[2].(hash.Hash64).Sum64())
	...
	for i := 1; i < len(tps); i++ {
		...
		err1 = HashChunkColumns(sc, vecHash, chk, tps[i], i, buf, nullBitmap)
		...
		c.Assert(nullBitmap[0], Equals, byte(0))
		c.Assert(vecHash[0].(hash.Hash64).Sum64(), Equals, rowHash[0].(hash.Hash64).Sum64())
		c.Assert(vecHash[1].(hash.Hash64).Sum64(), Equals, rowHash[1].(hash.Hash64).Sum64())
		c.Assert(vecHash[2].(hash.Hash64).Sum64(), Equals, rowHash[2].(hash.Hash64).Sum64())
	}
