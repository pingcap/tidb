...
func DecodeRange(b []byte, size int, idxColumnType []byte) ([]types.Datum, []byte, error) {
	if len(b) < 1 {
...
func DecodeAsDateTime(b []byte) (remain []byte, d types.Datum, err error) {
	if len(b) < 1 {
		return nil, d, errors.New("invalid encoded key")
	}
	flag := b[0]
