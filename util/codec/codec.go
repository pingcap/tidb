...
func DecodeRange(b []byte, size int, idxColumnType []byte, loc *time.Location) ([]types.Datum, []byte, error) {
	if len(b) < 1 {
	...
	for len(b) > 1 {
		var d types.Datum
		if idxColumnType == nil {
			b, d, err = DecodeOne(b)
		} else {
			if i >= len(idxColumnType) {
				return values, b, errors.New("invalid length of index's columns")
			}
			if idxColumnType[i] == mysql.TypeDatetime || idxColumnType[i] == mysql.TypeTimestamp || idxColumnType[i] == mysql.TypeDate {
				b, d, err = DecodeAsDateTime(b, idxColumnType[i], loc)
			} else {
				b, d, err = DecodeOne(b)
			}
		}
