...
// DatumMapHandleDecoder decodes a handle to datum map.
type DatumMapHandleDecoder struct {
	decoder
}

// NewDatumMapHandleDecoder creates a NewDatumMapHandleDecoder.
func NewDatumMapHandleDecoder(columns []ColInfo, handleColIDs []int64, loc *time.Location) *DatumMapHandleDecoder {
	return &DatumMapHandleDecoder{decoder{
		columns:      columns,
		handleColIDs: handleColIDs,
		loc:          loc,
	}}
...
// DecodeToDatumMap decodes handle to datum map.
func (decoder *DatumMapHandleDecoder) DecodeToDatumMap(handle kv.Handle, row map[int64]types.Datum) (map[int64]types.Datum, error) {
	if handle == nil || len(decoder.handleColIDs) == 0 {
		return row, nil
	}
	if row == nil {
		row = make(map[int64]types.Datum, len(decoder.columns))
		...
		if handle.IsInt() {
			...
			for i, hid := range decoder.handleColIDs {
				if col.ID == hid {
					...
					if d, err = Unflatten(d, col.Ft, decoder.loc); err != nil {
						return row, err
					}
					row[col.ID] = d
					break
