...
func (decoder *DatumMapDecoder) decodeColDatum(col *ColInfo, colData []byte) (types.Datum, error) {
	...
	case mysql.TypeDate, mysql.TypeDatetime, mysql.TypeTimestamp:
		var t types.Time
		t.SetType(col.Ft.GetTp())
		t.SetFsp(int8(col.Ft.GetDecimal()))
		err := t.FromPackedUint(decodeUint(colData))
		if err != nil {
			return d, err
		}
		if col.Ft.GetTp() == mysql.TypeTimestamp && !t.IsZero() {
	...
	case mysql.TypeDuration:
		...
		dur.Fsp = int8(col.Ft.GetDecimal())
	...
	case mysql.TypeJSON:
		...
		d.SetMysqlJSON(j)
	default:
		return d, errors.Errorf("unknown type %d", col.Ft.GetTp())
	}
	return d, nil
}
...
func (decoder *ChunkDecoder) decodeColToChunk(colIdx int, col *ColInfo, colData []byte, chk *chunk.Chunk) error {
	...
	case mysql.TypeNewDecimal:
		...
		if col.Ft.GetDecimal() != types.UnspecifiedLength && frac > col.Ft.GetDecimal() {
			...
			err := dec.Round(to, col.Ft.GetDecimal(), types.ModeHalfEven)
			if err != nil {
	...
	case mysql.TypeDate, mysql.TypeDatetime, mysql.TypeTimestamp:
		...
		if col.Ft.GetTp() == mysql.TypeTimestamp && decoder.loc != nil && !t.IsZero() {
			err = t.ConvertTimeZone(time.UTC, decoder.loc)
			if err != nil {
				return err
			}
	...
	default:
		return errors.Errorf("unknown type %d", col.Ft.GetTp())
	}
	return nil
}
...
func (decoder *BytesDecoder) decodeToBytesInternal(outputOffset map[int64]int, handle kv.Handle, value []byte, cacheBytes []byte) ([][]byte, error) {
	...
	for i := range decoder.columns {
		col := &decoder.columns[i]
		tp := fieldType2Flag(col.Ft.GetTp(), col.Ft.GetFlag()&mysql.UnsignedFlag == 0)
		colID := col.ID
