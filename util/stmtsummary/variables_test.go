...
func (s *testVariablesSuite) TestSetInVariable(c *C) {
	...
	err = sv.setVariable(typeMaxEvictedInfoCount, "10", false)
	c.Assert(err, IsNil)
	st = sv.getVariable(typeMaxEvictedInfoCount)
	c.Assert(st, Equals, int64(10))
	err = sv.setVariable(typeMaxEvictedInfoCount, "100", false)
	c.Assert(err, IsNil)
	st = sv.getVariable(typeMaxEvictedInfoCount)
	c.Assert(st, Equals, int64(100))
	err = sv.setVariable(typeMaxEvictedInfoCount, "10", true)
	c.Assert(err, IsNil)
	st = sv.getVariable(typeMaxEvictedInfoCount)
	c.Assert(st, Equals, int64(10))
	err = sv.setVariable(typeMaxEvictedInfoCount, "100", true)
	c.Assert(err, IsNil)
	st = sv.getVariable(typeMaxEvictedInfoCount)
	c.Assert(st, Equals, int64(100))
	err = sv.setVariable(typeMaxEvictedInfoCount, "10", false)
	...
	st = sv.getVariable(typeMaxEvictedInfoCount)
	c.Assert(st, Equals, int64(100))
	err = sv.setVariable(typeMaxEvictedInfoCount, "", true)
	c.Assert(err, IsNil)
	st = sv.getVariable(typeMaxEvictedInfoCount)
	c.Assert(st, Equals, int64(10))
	err = sv.setVariable(typeMaxEvictedInfoCount, "", false)
	c.Assert(err, IsNil)
	st = sv.getVariable(typeMaxEvictedInfoCount)
	c.Assert(st, Equals, int64(config.GetGlobalConfig().StmtSummary.MaxEvictedCount))
}
...
func (s *testVariablesSuite) TestMinValue(c *C) {
	...
	err = sv.setVariable(typeMaxEvictedInfoCount, "0", false)
	c.Assert(err, IsNil)
	v = sv.getVariable(typeMaxEvictedInfoCount)
	c.Assert(v, Greater, int64(0))
