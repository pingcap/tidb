...
func (fs *FileSorter) uniqueFileName() (string, error) {
...
func (fs *FileSorter) flushMemory() error {
	...
	fileName, err = fs.uniqueFileName()
	if err != nil {
		return errors.Trace(err)
	}
...
func (fs *FileSorter) Output() (val []types.Datum, handle int64, err error) {
	if !fs.fetched {
		if len(fs.buf) > 0 {
			err = fs.flushMemory()
			if err != nil {
		...
		for id, fd := range fds {
			...
			if n != rowSize {
				return nil, 0, errors.New("incorrect row")
			}
	...
	if fs.rowHeap.Len() > 0 {
		...
		return next.value.val, next.value.handle, nil
	} else {
		for _, fd := range fds {
			fd.Close()
		}
