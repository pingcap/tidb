...
func (fs *FileSorter) Output() (key []types.Datum, val []types.Datum, handle int64, err error) {
	...
	if len(fs.files) == 0 { // performing full in-memory sort
		return fs.fromMem()
	}
	return fs.fromFiles()
}

func (fs *FileSorter) fromMem() (key []types.Datum, val []types.Datum, handle int64, err error) {
	if !fs.fetched {
		sort.Sort(fs)
		if fs.err != nil {
...
func (fs *FileSorter) fromFiles() (key []types.Datum, val []types.Datum, handle int64, err error) {
	if !fs.fetched {
		if len(fs.buf) > 0 {
			err = fs.flushMemory()
			if err != nil {
