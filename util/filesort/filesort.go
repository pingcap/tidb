...
// rowHeap maintains a min-heap porperty of comparableRows
type rowHeap struct {
...
// Push push an element into rowHeap
func (rh *rowHeap) Push(x interface{}) {
	rh.ims = append(rh.ims, x.(*item))
}

// Pop pop the last element from rowHeap
...
// internalSort perform full in-memory sort.
...
// externalSort perform external file sort.
...
// fetchNextRow fetchs the next row given the source file index.
...
// flushToFile flushs the buffer to file if it is full.
