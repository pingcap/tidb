...
func TestLoopQueue(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		err := q.insert(&goWorker[struct{}, struct{}, int, any, pooltask.NilContext]{recycleTime: time.Now()})
		if err != nil {
			break
		}
	}
...
func TestRotatedArraySearch(t *testing.T) {
	...
	_ = q.insert(&goWorker[struct{}, struct{}, int, any, pooltask.NilContext]{recycleTime: time.Now()})
	...
	expiry3 := time.Now()
	_ = q.insert(&goWorker[struct{}, struct{}, int, any, pooltask.NilContext]{recycleTime: expiry3})
	...
	expiry4 := time.Now()
	_ = q.insert(&goWorker[struct{}, struct{}, int, any, pooltask.NilContext]{recycleTime: expiry4})
	...
	expiry5 := time.Now()
	_ = q.insert(&goWorker[struct{}, struct{}, int, any, pooltask.NilContext]{recycleTime: expiry5})
	...
	require.EqualValues(t, 9, q.binarySearch(q.items[9].recycleTime), "index should be 9")
