...
func getPotentialCmpColOffset(sctx sessionctx.Context, f *expression.ScalarFunction, cols []*expression.Column) int {
	...
	if column.RetType.EvalType() == types.ETString {
		cs, collation := constant.CharsetAndCollation()
		// Only allow comparison if the collations are compatible
		if !collate.CompatibleCollate(column.RetType.GetCollate(), collation) {
			// or it has same character set and the constant has binary collation
			if cs != column.RetType.GetCharset() || !collate.IsBinCollation(collation) {
				return -1
			}
		}
	}
