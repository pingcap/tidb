...
func TestIfNeedDoRecord(t *testing.T) {
	...
	needRecord, reason = record.needRecord(uint64(memUsed))
	require.True(t, needRecord)
	require.Equal(t, ExceedAlarmRatio, reason)
	...
	needRecord, reason = record.needRecord(uint64(memUsed))
	require.True(t, needRecord)
	require.Equal(t, GrowTooFast, reason)
}
...
func TestUpdateVariables(t *testing.T) {
	...
	record.initMemoryUsageAlarmRecord()
	require.Equal(t, 0.3, record.memoryUsageAlarmRatio)
	require.Equal(t, int64(3), record.memoryUsageAlarmKeepRecordNum)
	require.Equal(t, uint64(1024), record.serverMemoryLimit)
	...
	record.updateVariable()
	require.Equal(t, 0.6, record.memoryUsageAlarmRatio)
	require.Equal(t, int64(6), record.memoryUsageAlarmKeepRecordNum)
	require.Equal(t, uint64(2048), record.serverMemoryLimit)
}
