...
type RestrictedSQLExecutor interface {
	...
	ParseWithParams(ctx context.Context, sql string, ignoreClientCharsetAndCollation bool, args ...interface{}) (ast.StmtNode, error)
	// ParseWithParamsInternal is same as ParseWithParams except set `s.sessionVars.InRestrictedSQL = true`
	ParseWithParamsInternal(ctx context.Context, sql string, args ...interface{}) (ast.StmtNode, error)
