...
func newListInDiskWriteDisk(fieldTypes []*types.FieldType) (*listInDiskWriteDisk, error) {
	...
	disk, err := os.CreateTemp(config.GetGlobalConfig().Instance.TmpDir.Load(), strconv.Itoa(l.diskTracker.Label()))
	if err != nil {
		return nil, err
	}
	...
	disk2, err := os.CreateTemp(config.GetGlobalConfig().Instance.TmpDir.Load(), "offset"+strconv.Itoa(l.diskTracker.Label()))
	if err != nil {
		return nil, err
	}
