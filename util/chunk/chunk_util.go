...
func CopySelectedJoinRowsWithSameOuterRows(src *Chunk, innerColOffset, outerColOffset int, selected []bool, dst *Chunk, purnOuters bool) (bool, error) {
	if src.NumRows() == 0 {
		return false, nil
	}
	if src.sel != nil || dst.sel != nil {
	...
	numSelected := copySelectedInnerRows(innerColOffset, outerColOffset, src, selected, dst, purnOuters)
	copyOuterRows(innerColOffset, outerColOffset, src, numSelected, dst, purnOuters)
	dst.numVirtualRows += numSelected
	return numSelected > 0, nil
}
...
func copySelectedInnerRows(innerColOffset, outerColOffset int, src *Chunk, selected []bool, dst *Chunk, purnOuters bool) int {
	var srcCols []*Column
	if purnOuters {
...
// to the destination Chunk.
func copyOuterRows(innerColOffset, outerColOffset int, src *Chunk, numRows int, dst *Chunk, purnOuters bool) {
	if numRows <= 0 || purnOuters {
		return
	}
	row := src.GetRow(0)
	var srcCols []*Column
