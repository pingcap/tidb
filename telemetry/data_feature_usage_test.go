...
func TestTablePartition(t *testing.T) {
	...
	require.Equal(t, int64(0), usage.TablePartition.TablePartitionRangeMultiColumnsCnt)
	...
	usage, err = telemetry.GetFeatureUsage(tk.Session())
	require.NoError(t, err)
	require.Equal(t, int64(3), usage.TablePartition.TablePartitionCnt)
	require.Equal(t, int64(0), usage.TablePartition.TablePartitionHashCnt)
	require.Equal(t, int64(7), usage.TablePartition.TablePartitionMaxPartitionsCnt)
	require.Equal(t, int64(0), usage.TablePartition.TablePartitionListCnt)
	require.Equal(t, int64(1), usage.TablePartition.TablePartitionRangeCnt)
	require.Equal(t, int64(2), usage.TablePartition.TablePartitionRangeColumnsCnt)
	require.Equal(t, int64(1), usage.TablePartition.TablePartitionRangeMultiColumnsCnt)
	require.Equal(t, int64(0), usage.TablePartition.TablePartitionListColumnsCnt)
