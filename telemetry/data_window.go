...
var (
	...
	// CurrentTiflashRegexpReplacePushDownCount count the number regexp_replace functions that are pushed down to tiflash
	// CurrentTiflashRegexpReplacePushDownCount atomic.Uint64
...
type tiFlashUsageData struct {
	...
	//RegexpReplacePushDown uint64 `json:"regexpReplacePushDown"`
...
func RotateSubWindow() {
	thisSubWindow := windowData{
		...
		TiFlashUsage: tiFlashUsageData{
			...
			//RegexpReplacePushDown: CurrentTiflashRegexpReplacePushDownCount.Swap(0),
...
func getWindowData() []*windowData {
	...
	for i < len(rotatedSubWindows) {
		...
		for i < len(rotatedSubWindows) && aggregatedSubWindows < maxSubWindowLengthInWindow {
			...
			//thisWindow.TiFlashUsage.RegexpReplacePushDown += rotatedSubWindows[i].TiFlashUsage.RegexpReplacePushDown
