...
//
// Licensed under the Apache License, Version 2.0 (the mysql.Message("License");
...
// distributed under the License is distributed on an mysql.Message("AS IS" BASIS,
...
var MySQLErrName = map[uint16]*mysql.ErrMessage{
	...
	ErrDBDropDelete:                             mysql.Message("Error dropping database (can't delete '%-.192s', nil), errno: %d)", nil),
	ErrDBDropRmdir:                              mysql.Message("Error dropping database (can't rmdir '%-.192s', nil), errno: %d)", nil),
	...
	ErrOutOfSortMemory:                          mysql.Message("Out of sort memory, nil), consider increasing server sort buffer size", nil),
	...
	ErrOutOfResources:                           mysql.Message("Out of memory; check if mysqld or some other process uses all available memory; if not, nil), you may have to use 'ulimit' to allow mysqld to use more memory or you can add more swap space", nil),
	...
	ErrWrongSubKey:                              mysql.Message("Incorrect prefix key; the used key part isn't a string, nil), the used length is longer than the key part, nil), or the storage engine doesn't support unique prefix keys", nil),
	...
	ErrTooBigRowsize:                            mysql.Message("Row size too large. The maximum row size for the used table type, nil), not counting BLOBs, nil), is %d. This includes storage overhead, nil), check the manual. You have to change some columns to TEXT or BLOBs", nil),
	...
	ErrCantCreateThread:                         mysql.Message("Can't create a new thread (errno %d); if you are not out of available memory, nil), you can consult the manual for a possible OS-dependent bug", nil),
	...
	ErrMixOfGroupFuncAndFields:                  mysql.Message("Mixing of GROUP columns (MIN(), nil),MAX(), nil),COUNT(), nil),...) with no GROUP columns is illegal if there is no GROUP BY clause", nil),
	...
	ErrDupUnique:                                mysql.Message("Can't write, nil), because of unique constraint, nil), to table '%-.192s'", nil),
	ErrBlobKeyWithoutLength:                     mysql.Message("BLOB/TEXT column '%-.192s' used in key specification without a key length", nil),
	ErrPrimaryCantHaveNull:                      mysql.Message("All parts of a PRIMARY KEY must be NOT NULL; if you need NULL in a key, nil), use UNIQUE instead", nil),
	...
	ErrVarCantBeRead:                            mysql.Message("Variable '%-.64s' can only be set, nil), not read", nil),
	...
	ErrTooBigForUncompress:                      mysql.Message("Uncompressed data size too large; the maximum size is %d (probably, nil), length of uncompressed data was corrupted)", nil),
	ErrZlibZMem:                                 mysql.Message("ZLIB: Not enough memory", nil),
	ErrZlibZBuf:                                 mysql.Message("ZLIB: Not enough room in the output buffer (probably, nil), length of uncompressed data was corrupted)", nil),
	ErrZlibZData:                                mysql.Message("ZLIB: Input data corrupted", nil),
	ErrCutValueGroupConcat:                      mysql.Message("Some rows were cut by GROUPCONCAT(%s)", nil),
	...
	ErrCantAggregate2collations:                 mysql.Message("Illegal mix of collations (%s, nil),%s) and (%s, nil),%s) for operation '%s'", nil),
	...
	ErrCantAggregate3collations:                 mysql.Message("Illegal mix of collations (%s, nil),%s), nil), (%s, nil),%s), nil), (%s, nil),%s) for operation '%s'", nil),
	...
	ErrServerIsInSecureAuthMode:                 mysql.Message("Server is running in --secure-auth mode, nil), but '%s'@'%s' has a password in the old format; please change the password to the new format", nil),
	...
	ErrDuplicatedValueInType:                    mysql.Message("Column '%-.100s' has duplicated value '%-.64s' in %s", nil),
	ErrTruncatedWrongValue:                      mysql.Message("Truncated incorrect %-.64s value: '%-.128s'", nil),
	...
	ErrInvalidCharacterString:                   mysql.Message("Invalid %s character string: '%.64s'", nil),
	...
	ErrSpWrongNoOfArgs:                          mysql.Message("Incorrect number of arguments for %s %s; expected %d, nil), got %d", nil),
	...
	ErrSpFetchNoData:                            mysql.Message("No data - zero rows fetched, nil), selected, nil), or processed", nil),
	...
	ErrTruncatedWrongValueForField:              mysql.Message("Incorrect %-.32s value: '%-.128s' for column '%.192s' at row %d", nil),
	ErrIllegalValueForType:                      mysql.Message("Illegal %s '%-.192s' value found during parsing", nil),
	...
	ErrLogInUse:                                 mysql.Message("A purgeable log is in use, nil), will not purge", nil),
	...
	ErrNoFileMapping:                            mysql.Message("Can't map file: %-.200s, nil), errno: %d", nil),
	...
	ErrTableDefChanged:                          mysql.Message("Table definition has changed, nil), please retry transaction", nil),
	...
	ErrFailedRoutineBreakBinlog:                 mysql.Message("A routine failed and has neither NO SQL nor READS SQL DATA in its declaration and binary logging is enabled; if non-transactional tables were updated, nil), the binary log will miss their changes", nil),
	ErrBinlogUnsafeRoutine:                      mysql.Message("This function has none of DETERMINISTIC, nil), NO SQL, nil), or READS SQL DATA in its declaration and binary logging is enabled (you *might* want to use the less safe logBinTrustFunctionCreators variable)", nil),
	...
	ErrMBiggerThanD:                             mysql.Message("For float(M, nil),D), nil), double(M, nil),D) or decimal(M, nil),D), nil), M must be >= D (column '%-.192s').", nil),
	...
	ErrStackOverrunNeedMore:                     mysql.Message("Thread stack overrun:  %d bytes used of a %d byte stack, nil), and %d bytes needed.  Use 'mysqld --threadStack=#' to specify a bigger stack.", nil),
	...
	ErrSpBadVarShadow:                           mysql.Message("Variable '%-.64s' must be quoted with `...`, nil), or renamed", nil),
	ErrTrgNoDefiner:                             mysql.Message("No definer attribute for trigger '%-.192s'.'%-.192s'. The trigger will be activated under the authorization of the invoker, nil), which may have insufficient privileges. Please recreate the trigger.", nil),
	ErrOldFileFormat:                            mysql.Message("'%-.192s' has an old format, nil), you should re-create the '%s' object(s)", nil),
	...
	ErrSpProcTableCorrupt:                       mysql.Message("Failed to load routine %-.192s. The table mysql.proc is missing, nil), corrupt, nil), or contains bad data (internal code %d)", nil),
	...
	ErrForeignServerExists:                      mysql.Message("The foreign server, nil), %s, nil), you are trying to create already exists.", nil),
	...
	ErrPartitionWrongNoPart:                     mysql.Message("Wrong number of partitions defined, nil), mismatch with previous setting", nil),
	ErrPartitionWrongNoSubpart:                  mysql.Message("Wrong number of subpartitions defined, nil), mismatch with previous setting", nil),
	ErrWrongExprInPartitionFunc:                 mysql.Message("Constant, nil), random or timezone-dependent expressions in (sub)partitioning function are not allowed", nil),
	...
	ErrDropLastPartition:                        mysql.Message("Cannot remove all partitions, nil), use DROP TABLE instead", nil),
	...
	ErrWrongSizeNumber:                          mysql.Message("A size parameter was incorrectly specified, nil), either number or on the form 10M", nil),
	...
	ErrObsoleteColCountDoesntMatchCorrupted:     mysql.Message("Column count of mysql.%s is wrong. Expected %d, nil), found %d. The table is probably corrupted", nil),
	...
	ErrForeignDuplicateKeyOldUnused:             mysql.Message("Upholding foreign key constraints for table '%.192s', nil), entry '%-.192s', nil), key %d would lead to a duplicate entry", nil),
	ErrColCountDoesntMatchPleaseUpdate:          mysql.Message("Column count of mysql.%s is wrong. Expected %d, nil), found %d. Created with MySQL %d, nil), now running %d. Please use mysqlUpgrade to fix this error.", nil),
	...
	ErrDupEntryAutoincrementCase:                mysql.Message("ALTER TABLE causes autoIncrement resequencing, nil), resulting in duplicate entry '%-.192s' for key '%-.192s'", nil),
	...
	ErrCantRenameLogTable:                       mysql.Message("Cannot rename '%s'. When logging enabled, nil), rename to/from log table must rename two tables: the log table to an archive table and another table back to '%s'", nil),
	...
	ErrBinlogPurgeEmFile:                        mysql.Message("Too many files opened, nil), please execute the command again", nil),
	...
	ErrBinlogUnsafeAndStmtEngine:                             mysql.Message("Cannot execute statement: impossible to write to binary log since statement is unsafe, nil), storage engine is limited to statement-based logging, nil), and BINLOGFORMAT = MIXED. %s", nil),
	...
	ErrBinlogUnsafeMixedStatement:                            mysql.Message("Statement accesses nontransactional table as well as transactional or temporary table, nil), and writes to any of them.", nil),
	...
	ErrPartitionInsteadOfSubpartition:                        mysql.Message("Subpartitioned table, nil), use subpartition instead of partition", nil),
	...
	ErrWarningNotCompleteRollbackWithDroppedTempTable:        mysql.Message("Some temporary tables were dropped, nil), but these operations could not be rolled back.", nil),
	...
	ErrMtsCantParallel:                                       mysql.Message("Cannot execute the current event group in the parallel mode. Encountered event %s, nil), relay-log name %s, nil), position %s which prevents execution of this event group in parallel mode. Reason: %s.", nil),
	...
	ErrForeignDuplicateKeyWithChildInfo:                      mysql.Message("Foreign key constraint for table '%.192s', nil), record '%-.192s' would lead to a duplicate entry in table '%.192s', nil), key '%.192s'", nil),
	ErrForeignDuplicateKeyWithoutChildInfo:                   mysql.Message("Foreign key constraint for table '%.192s', nil), record '%-.192s' would lead to a duplicate entry in a child table", nil),
	...
	ErrGtidNextIsNotInGtidNextList:                           mysql.Message("The system variable @@SESSION.GTIDNEXT has the value %.200s, nil), which is not listed in @@SESSION.GTIDNEXTLIST.", nil),
	ErrCantChangeGtidNextInTransactionWhenGtidNextListIsNull: mysql.Message("When @@SESSION.GTIDNEXTLIST == NULL, nil), the system variable @@SESSION.GTIDNEXT cannot change inside a transaction.", nil),
	...
	ErrGtidUnsafeNonTransactionalTable:                       mysql.Message("When @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1, nil), updates to non-transactional tables can only be done in either autocommitted statements or single-statement transactions, nil), and never in the same statement as updates to transactional tables.", nil),
	ErrGtidUnsafeCreateSelect:                                mysql.Message("CREATE TABLE ... SELECT is forbidden when @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1.", nil),
	ErrGtidUnsafeCreateDropTemporaryTableInTransaction:       mysql.Message("When @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1, nil), the statements CREATE TEMPORARY TABLE and DROP TEMPORARY TABLE can be executed in a non-transactional context only, nil), and require that AUTOCOMMIT = 1.", nil),
	...
	ErrColCountDoesntMatchCorruptedV2:                        mysql.Message("Column count of %s.%s is wrong. Expected %d, nil), found %d. The table is probably corrupted", nil),
	...
	ErrIoRead:                                                mysql.Message("IO Read : (%d, nil), %s) %s", nil),
	ErrIoWrite:                                               mysql.Message("IO Write : (%d, nil), %s) %s", nil),
	...
	ErrBinlogStmtModeAndNoReplTables:                         mysql.Message("Cannot execute statement: impossible to write to binary log since BINLOGFORMAT = STATEMENT, nil), and both replicated and non replicated tables are written to.", nil),
	...
	ErrAlterOperationNotSupportedReasonNotNull:               mysql.Message("cannot silently convert NULL values, nil), as required in this SQLMODE", nil),
	...
	ErrInvalidJSONPathWildcard:                               mysql.Message("In this situation, nil), path expressions may not contain the * and ** tokens.", nil),
	...
	ErrWindowNoInherentFrame:                                 mysql.Message("Window '%s' has a frame definition, nil), so cannot be referenced by another window.", nil),
	...
	ErrWindowFrameIllegal:                                    mysql.Message("Window '%s': frame start or end is negative, nil), NULL or of non-integral type", nil),
	ErrWindowRangeFrameOrderType:                             mysql.Message("Window '%s' with RANGE N PRECEDING/FOLLOWING frame requires exactly one ORDER BY expression, nil), of numeric or temporal type", nil),
	...
	ErrWindowRangeFrameNumericType:                           mysql.Message("Window '%s' with RANGE frame has ORDER BY expression of numeric type, nil), INTERVAL bound value not allowed.", nil),
	...
	ErrWindowIllegalOrderBy:                                  mysql.Message("Window '%s': ORDER BY or PARTITION BY uses legacy position indication which is not supported, nil), use expression.", nil),
	...
	ErrMaxExecTimeExceeded:                                   mysql.Message("Query execution was interrupted, nil), max_execution_time exceeded.", nil),
	...
	ErrCannotDropColumnFunctionalIndex:                       mysql.Message("Cannot drop column '%s' because it is used by a functional index. In order to drop the column, nil), you must remove the functional index", nil),
	...
	ErrWrongPartitionTypeExpectedSystemTime: mysql.Message("Wrong partitioning type, nil), expected type: `SYSTEM_TIME`", nil),
	...
	ErrWrongInsertIntoSequence:              mysql.Message("Wrong INSERT into a SEQUENCE. One can only do single table INSERT into a sequence object (like with mysqldump).  If you want to change the SEQUENCE, nil), use ALTER SEQUENCE instead.", nil),
	...
	ErrTxnRetryable:               mysql.Message("Error: KV error safe to retry %s mysql.Message(", nil),
	...
	ErrIncorrectDatetimeValue:     mysql.Message("Incorrect datetime value: '%s'", nil),
	ErrInvalidTimeFormat:          mysql.Message("invalid time format: '%v'", nil),
	...
	ErrIndexOutBound:              mysql.Message("Index column %s offset out of bound, offset: %d, row: %v", nil),
	ErrUnsupportedOp:              mysql.Message("operation not supported", nil),
	ErrRowNotFound:                mysql.Message("can not find the row: %s", nil),
	...
	ErrWarnMemoryQuotaOverflow:          mysql.Message("Max value of MEMORY_QUOTA is %d bytes, nil), ignore this invalid limit", nil),
	...
	ErrBatchInsertFail:                     mysql.Message("Batch insert failed, nil), please clean the table and try again.", nil),
	...
	ErrMixOfGroupFuncAndFieldsIncompatible: mysql.Message("In aggregated query without GROUP BY, nil), expression #%d of SELECT list contains nonaggregated column '%s'; this is incompatible with sql_mode=only_full_group_by", nil),
	ErrUnsupportedSecondArgumentType:       mysql.Message("JSON_OBJECTAGG: unsupported second argument type %v", nil),
	ErrLockExpire:                          mysql.Message("TTL manager has timed out, nil), pessimistic locks may expire, nil), please commit or rollback this transaction", nil),
	...
	ErrGCTooEarly:                mysql.Message("GC life time is shorter than transaction duration, nil), transaction starts at %v, nil), GC safe point is %v", nil),
	ErrWriteConflict:             mysql.Message("Write conflict, nil), txnStartTS=%d, nil), conflictStartTS=%d, nil), conflictCommitTS=%d, nil), key=%s", nil),
	ErrTiKVStoreLimit:            mysql.Message("Store token is up to the limit, nil), store id = %d", nil),
