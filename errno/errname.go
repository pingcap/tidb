...
// ErrMessage holds an error message
...
var MySQLErrName = map[uint16]*ErrMessage{
	...
	ErrNotHintUpdatable:                                      Message("Variable '%s' cannot be set using SET_VAR hint.", nil),
	...
	ErrInvalidPluginID:               Message("Wrong plugin id: %s, valid plugin id is [name]-[version], both name and version should not contain '-'", nil),
	...
	ErrCastAsSignedOverflow:          Message("Cast to signed converted positive out-of-range integer to it's negative complement", nil),
	...
	ErrDataInConsistent:              Message("index:%#v != record:%#v", []int{0, 1}),
	...
	ErrDataInConsistentExtraIndex:    Message("handle %#v, index:%#v != record:%#v", []int{0, 1, 2}),
	ErrDataInConsistentMisMatchIndex: Message("col %s, handle %#v, index:%#v != record:%#v, compare err:%#v", []int{1, 2, 3, 4}),
	...
	ErrPartitionStatsMissing: Message("Build table: %s global-level stats failed due to missing partition-level stats", nil),
	ErrNotSupportedWithSem:   Message("Feature '%s' is not supported when security enhanced mode is enabled", nil),

	ErrInvalidPlacementSpec:            Message("Invalid placement policy '%s': %s", nil),
	...
	ErrInvalidAttributesSpec:           Message("Invalid attributes '%s': %s", nil),
	...
	// TiKV/PD errors.
	ErrPDServerTimeout:           Message("PD server timeout", nil),
	...
	ErrWriteConflict:             Message("Write conflict, txnStartTS=%d, conflictStartTS=%d, conflictCommitTS=%d, key=%s", []int{3}),
