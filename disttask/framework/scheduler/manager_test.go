...
func TestOnRunnableTasks(t *testing.T) {
	...
	// get subtask failed
	mockTaskTable.On("HasSubtasksInStates", id, taskID, []any{proto.TaskStatePending, proto.TaskStateRevertPending}).Return(false, errors.New("get subtask failed")).Once()
	m.onRunnableTasks(context.Background(), []*proto.Task{task})

	// no subtask
	mockTaskTable.On("HasSubtasksInStates", id, taskID, []any{proto.TaskStatePending, proto.TaskStateRevertPending}).Return(false, nil).Once()
	m.onRunnableTasks(context.Background(), []*proto.Task{task})

	// pool error
	mockTaskTable.On("HasSubtasksInStates", id, taskID, []any{proto.TaskStatePending, proto.TaskStateRevertPending}).Return(true, nil).Once()
...
func TestManager(t *testing.T) {
	...
	// task1
	mockTaskTable.On("HasSubtasksInStates", id, taskID1, []any{proto.TaskStatePending, proto.TaskStateRevertPending}).Return(true, nil).Once()
	mockPool.On("Run", mock.Anything).Return(nil).Once()
	mockInternalScheduler.On("Start").Once()
	...
	mockTaskTable.On("HasSubtasksInStates", id, taskID1, []any{proto.TaskStatePending, proto.TaskStateRevertPending}).Return(false, nil)
	mockInternalScheduler.On("Stop").Once()
	// task2
	mockTaskTable.On("HasSubtasksInStates", id, taskID2, []any{proto.TaskStatePending, proto.TaskStateRevertPending}).Return(true, nil).Once()
	mockPool.On("Run", mock.Anything).Return(nil).Once()
	mockInternalScheduler.On("Start").Once()
	mockTaskTable.On("GetGlobalTaskByID", taskID2).Return(task2, nil)
	mockTaskTable.On("HasSubtasksInStates", id, taskID2, []any{proto.TaskStatePending, proto.TaskStateRevertPending}).Return(true, nil).Once()
	mockInternalScheduler.On("Rollback", mock.Anything, task2).Return(nil).Once()
	mockTaskTable.On("HasSubtasksInStates", id, taskID2, []any{proto.TaskStatePending, proto.TaskStateRevertPending}).Return(false, nil)
	mockInternalScheduler.On("Stop").Once()
