...
func (ps *perfSchema) addRecords(tableIdent table.Ident, rows [][]interface{}) error {
	...
	case TableSetupActors:
		store = ps.setupActorsStore
		curNo = atomic.AddUint64(&ps.setupActorsRecordNo, uint64(len(rows)))
	case TableSetupObjects:
		store = ps.setupObjectsStore
		curNo = atomic.AddUint64(&ps.setupObjectsRecordNo, uint64(len(rows)))
	default:
...
func (ps *perfSchema) getTable(tableIdent table.Ident) *model.TableInfo {
	...
	return ps.tables[strings.ToUpper(tableIdent.Name.O)]
}
