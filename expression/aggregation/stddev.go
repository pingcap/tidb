...
func (sf *stddevFunction) updateSingle(sc *stmtctx.StatementContext, evalCtx *AggEvaluateContext, row chunk.Row) error {
	a := sf.Args[0]
...
func (sf *stddevFunction) updateAll(sc *stmtctx.StatementContext, evalCtx *AggEvaluateContext, row chunk.Row) error {
...
func (sf *stddevFunction) Update(evalCtx *AggEvaluateContext, sc *stmtctx.StatementContext, row chunk.Row) (err error) {
	switch sf.Mode {
...
func (sf *stddevFunction) GetResult(evalCtx *AggEvaluateContext) (d types.Datum) {
	...
	case types.KindFloat64:
		...
		avg := sum / count
		d.SetFloat64(math.Sqrt((sumSquare - 2*sum*avg + count*avg*avg) / count))
	case types.KindMysqlDecimal:
		...
		err = types.DecimalDiv(tmpVariance2, count, variance, types.DivFracIncr)
		terror.Log(errors.Trace(err))
...
func (sf *stddevFunction) GetPartialResult(evalCtx *AggEvaluateContext) []types.Datum {
