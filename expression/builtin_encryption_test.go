...
var createDigestCases = []struct {
	digestType interface{}
	str        interface{}
	digest     interface{}
	validCase  bool
}{
	{"SHA224", "pingcap", "11111111111111011111011011001111110110011110111010111111111111111011110111111111111111110110100101001001010100111111111111110110010011111111111111011100101111011100100011100101001011100010011111111111111011000000110110111", true},
	{"SHA256", "pingcap", "", true},
	{"SHA384", "pingcap", "", true},
	{"SHA512", "pingcap", "", true},
	{"SHA123", "pingcap", nil, false},
	{123, "pingcap", nil, false},
	{nil, "pingcap", nil, false},
	{"SHA224", nil, nil, false},
	{nil, nil, nil, false},
}

func (s *testEvaluatorSuite) TestCreateDigest(c *C) {
	defer testleak.AfterTest(c)()
	fc := funcs[ast.CreateDigest]
	for _, test := range createDigestCases {
		digestType := types.NewDatum(test.digestType)
		str := types.NewDatum(test.str)
		f, err := fc.getFunction(datumsToConstants([]types.Datum{digestType, str}), s.ctx)
		digest, err := f.eval(nil)
		c.Assert(err, IsNil)
		if test.validCase {
			res, err := digest.ToString()
			c.Assert(err, IsNil)
			c.Assert(res, Equals, test.digest)
		} else {
