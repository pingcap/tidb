...
func (c *collationInfo) SetSkip() {
	c.skip = true
}

func (c *collationInfo) Skipped() bool {
	return c.skip
}
...
type CollationInfo interface {
	...
	// SetSkip set the skip flag to true.
	SetSkip()

	// Skipped indicate this expression should skip when infer the collation.
	Skipped() bool
}
