...
// GetCtx gets the context value from function.
...
func NewFunction(ctx context.Context, funcName string, retType *types.FieldType, args ...Expression) (Expression, error) {
	...
	copy(newArgs, args)
	fun, err := f.getFunction(newArgs, ctx)
	return &ScalarFunction{
		FuncName: model.NewCIStr(funcName),
		RetType:  retType,
		Function: fun}, errors.Trace(err)
}
...
func (sf *ScalarFunction) Clone() Expression {
	...
	case *builtinValues:
		return NewValuesFunc(v.v, sf.GetCtx())
	}
...
func (sf *ScalarFunction) IsCorrelated() bool {
	for _, arg := range sf.Function.getArgs() {
		if arg.IsCorrelated() {
			return true
		}
	}
