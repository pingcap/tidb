...
func TestDeferredParamNotNull(t *testing.T) {
	...
	require.Equal(t, mysql.TypeVarString, cstJSON.GetType().GetTp())
	require.Equal(t, mysql.TypeNewDecimal, cstDec.GetType().GetTp())
	require.Equal(t, mysql.TypeLonglong, cstInt.GetType().GetTp())
	require.Equal(t, mysql.TypeLonglong, cstUint.GetType().GetTp())
	require.Equal(t, mysql.TypeTimestamp, cstTime.GetType().GetTp())
	require.Equal(t, mysql.TypeDuration, cstDuration.GetType().GetTp())
	require.Equal(t, mysql.TypeBlob, cstBytes.GetType().GetTp())
	require.Equal(t, mysql.TypeVarString, cstBinary.GetType().GetTp())
	require.Equal(t, mysql.TypeVarString, cstBit.GetType().GetTp())
	require.Equal(t, mysql.TypeFloat, cstFloat32.GetType().GetTp())
	require.Equal(t, mysql.TypeDouble, cstFloat64.GetType().GetTp())
	require.Equal(t, mysql.TypeEnum, cstEnum.GetType().GetTp())
