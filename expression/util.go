...
func ExtractColumnSet(exprs ...Expression) fd.FastIntSet {
	set := fd.NewFastIntSet()
	for _, expr := range exprs {
...
func extractColumnSet(expr Expression, set *fd.FastIntSet) {
	switch v := expr.(type) {
	case *Column:
...
>>>>>>> def18e5ac98 (expression: truncate decimal instead of return error for PrepStmt (#62792)):pkg/expression/util.go
