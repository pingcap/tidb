...
// Generally, CanonicalHashCode intends to recognize different SQL statements that are semantically equal;
// while this function intends to recognize different expressions which are generated by optimizer from the same SQL
// The detail differences between this function with CanonicalHashCode are:
// 1. CanonicalHashCode will treat "a < b" the same with "b >= a", and "a + b" the same with "b + a" while this function won't
...
// TODO: check if some SessionVar can be used to check if it is "execute stmt"(expressions may contain parameter marker),
// if not, we can use HashCode instead of HistoryStatsHashCode here
