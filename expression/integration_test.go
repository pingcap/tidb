...
func (s *testIntegrationSuite) TestInPredicate4UnsignedInt(c *C) {
	...
	tk.MustExec("INSERT INTO t VALUES (0), (4), (5), (6), (7), (8);")
	r := tk.MustQuery(`SELECT a FROM t WHERE a NOT IN (-1, -2);`)
	r.Check(testkit.Rows("0", "4", "5", "6", "7", "8"))
	r = tk.MustQuery(`SELECT a FROM t WHERE a NOT IN (-1, -2, 4);`)
	r.Check(testkit.Rows("0", "5", "6", "7", "8"))
	r = tk.MustQuery(`SELECT a FROM t WHERE a NOT IN (-1, -2, 0, 4);`)
	r.Check(testkit.Rows("5", "6", "7", "8"))
