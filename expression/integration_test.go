...
func (s *testIntegrationSuite) TestEncryptionBuiltin(c *C) {
	...
	// for COMPRESS
	tk.MustExec("DROP TABLE IF EXISTS t;")
	tk.MustExec("CREATE TABLE t(a VARCHAR(1000), b VARBINARY(1000));")
	tk.MustExec("INSERT INTO t SET a = '12345', b = COMPRESS(a);")
	tk.MustExec("INSERT INTO t SET a = '23456', b = UNHEX('001122334455')")
	result = tk.MustQuery("SELECT a, HEX(b) FROM t;")
	result.Check(testkit.Rows("12345 05000000789C323432363105040000FFFF02F80100", "23456 001122334455"))
	result = tk.MustQuery("SELECT HEX(COMPRESS(a)), HEX(COMPRESS(b)) FROM t;")
	result.Check(testkit.Rows("05000000789C323432363105040000FFFF02F80100 15000000789C62656060A8986364626466C8CAC2C0F0FF3FD30F460640000000FFFF2D66051B", "05000000789C323236313503040000FFFF03070105 06000000789C62105432760905040000FFFF02590100"))
