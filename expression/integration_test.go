...
func (s *testIntegrationSuite) TestTimestamp(c *C) {
	...
	c.Assert(err, IsNil)
	nowStr1 := fmt.Sprintf("%s", tk.MustQuery("SELECT NOW();").Rows()[0])
	now1, err := time.Parse("[2006-01-02 15:04:05]", nowStr1)
	c.Assert(err, IsNil)
	tk.MustExec("set @@timestamp = 12345;")
	tk.MustQuery("SELECT @@timestamp;").Check(testkit.Rows("12345"))
	tk.MustQuery("SELECT NOW();").Check(testkit.Rows("1970-01-01 11:25:45"))
	tk.MustQuery("SELECT NOW();").Check(testkit.Rows("1970-01-01 11:25:45"))
	...
	c.Assert(err, IsNil)
	nowStr2 := fmt.Sprintf("%s", tk.MustQuery("SELECT NOW();").Rows()[0])
	now2, err := time.Parse("[2006-01-02 15:04:05]", nowStr2)
	c.Assert(err, IsNil)
	...
	c.Assert(err, IsNil)
	nowStr3 := fmt.Sprintf("%s", tk.MustQuery("SELECT NOW();").Rows()[0])
	now3, err := time.Parse("[2006-01-02 15:04:05]", nowStr3)
	c.Assert(err, IsNil)
