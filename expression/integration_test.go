...
func (s *testIntegrationSuite) TestDateBuiltin(c *C) {
	...
	_, err = session.GetRows4Test(ctx, tk.Se, rs)
	c.Assert(err, NotNil)
	c.Assert(terror.ErrorEqual(err, types.ErrWrongValue.GenWithStackByArgs(
		strings.ToUpper(types.DateStr), "0000-00-00")), IsTrue)
	c.Assert(rs.Close(), IsNil)
	...
	c.Assert(err, NotNil)
	c.Assert(terror.ErrorEqual(err, types.ErrWrongValue.GenWithStackByArgs(
		strings.ToUpper(types.DateStr), "2017-10-00")), IsTrue)
	c.Assert(rs.Close(), IsNil)
	...
	c.Assert(err, NotNil)
	c.Assert(terror.ErrorEqual(err, types.ErrWrongValue.GenWithStackByArgs(
		strings.ToUpper(types.DateStr), "0000-00-00 00:00:00")), IsTrue)
	...
	c.Assert(err, NotNil)
	c.Assert(terror.ErrorEqual(err, types.ErrWrongValue.GenWithStackByArgs(
		strings.ToUpper(types.DateStr), "201712-31")), IsTrue, Commentf("err: %v", err))

	_, err = tk.Exec("select date 'abcdefg';")
	c.Assert(err, NotNil)
	c.Assert(terror.ErrorEqual(err, types.ErrWrongValue.GenWithStackByArgs(
		strings.ToUpper(types.DateStr), "abcdefg")), IsTrue, Commentf("err: %v", err))
}
...
func (s *testIntegrationSuite) TestTimeLiteral(c *C) {
	...
	c.Assert(err, NotNil)
	c.Assert(terror.ErrorEqual(err, types.ErrWrongValue.GenWithStackByArgs(
		strings.ToUpper(types.TimeStr), "2017-01-01 00:00:00")), IsTrue)

	_, err = tk.Exec("select time '071231235959.999999';")
	c.Assert(err, NotNil)
	c.Assert(terror.ErrorEqual(err, types.ErrWrongValue.GenWithStackByArgs(strings.ToUpper(types.TimeStr), "071231235959.999999")), IsTrue)

	_, err = tk.Exec("select time '20171231235959.999999';")
	c.Assert(err, NotNil)
	c.Assert(terror.ErrorEqual(err, types.ErrWrongValue.GenWithStackByArgs(strings.ToUpper(types.TimeStr), "20171231235959.999999")), IsTrue)
