...
func (s *testIntegrationSuite) TestTimestampDatumEncode(c *C) {
	...
	tk.MustQuery(`explain select * from t where b = (select max(b) from t)`).Check(testkit.Rows(
		"TableReader_33 10.00 root  data:Selection_32",
		"└─Selection_32 10.00 cop[tikv]  eq(test.t.b, 2019-04-29 11:56:12)",
		"  └─TableFullScan_31 10000.00 cop[tikv] table:t keep order:false, stats:pseudo",
	))
