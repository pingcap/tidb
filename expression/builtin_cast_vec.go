...
func (b *builtinCastDurationAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		duration.Fsp = fsp
		dur, err := duration.RoundFrac(types.DefaultFsp, b.ctx.GetSessionVars().TimeZone)
		if err != nil {
			return err
		}
...
func (b *builtinCastTimeAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, t := range arg0s {
		...
		d, err = d.RoundFrac(int8(b.tp.Decimal), b.ctx.GetSessionVars().TimeZone)
		if err != nil {
			return err
		}
		ds[i] = d.Duration
...
func (b *builtinCastDurationAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, v := range res {
		...
		dur.Duration = v
		rd, err = dur.RoundFrac(int8(b.tp.Decimal), b.ctx.GetSessionVars().TimeZone)
		if err != nil {
			return err
		}
		res[i] = rd.Duration
