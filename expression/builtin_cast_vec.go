...
func (b *builtinCastIntAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, err := types.NumberToDuration(i64s[i], int8(b.tp.GetDecimal()))
		if err != nil {
...
func (b *builtinCastDurationAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	ds := buf.GoDurations()
	fsp := int8(b.args[0].GetType().GetDecimal())
	for i := 0; i < n; i++ {
		if result.IsNull(i) {
			continue
		}
...
func (b *builtinCastIntAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if b.args[0].GetType().GetTp() == mysql.TypeYear {
			...
			tm, err = types.ParseTimeFromNum(stmt, i64s[i], b.tp.GetTp(), fsp)
		}
...
func (b *builtinCastJSONAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		tm, err := types.ParseTime(stmtCtx, s, b.tp.GetTp(), fsp)
		if err != nil {
			if err = handleInvalidTimeError(b.ctx, err); err != nil {
				return err
			}
...
func (b *builtinCastRealAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		tm, err := types.ParseTime(stmt, fv, b.tp.GetTp(), fsp)
		if err != nil {
			if err = handleInvalidTimeError(b.ctx, err); err != nil {
				return err
			}
...
func (b *builtinCastIntAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.Column) error {
	...
	isYearType := tp.GetTp() == mysql.TypeYear
	result.ReserveString(n)
	i64s := buf.Int64s()
	for i := 0; i < n; i++ {
		var str string
...
func (b *builtinCastStringAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, err := types.ParseDuration(b.ctx.GetSessionVars().StmtCtx, buf.GetString(i), int8(b.tp.GetDecimal()))
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = b.ctx.GetSessionVars().StmtCtx.HandleTruncate(err)
			}
...
func (b *builtinCastRealAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, err := types.ParseDuration(b.ctx.GetSessionVars().StmtCtx, strconv.FormatFloat(f64s[i], 'f', -1, 64), int8(b.tp.GetDecimal()))
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = b.ctx.GetSessionVars().StmtCtx.HandleTruncate(err)
				if err != nil {
...
func (b *builtinCastTimeAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, t := range arg0s {
		...
		d, err = d.RoundFrac(int8(b.tp.GetDecimal()), b.ctx.GetSessionVars().Location())
		if err != nil {
			return err
		}
		ds[i] = d.Duration
...
func (b *builtinCastDurationAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i, v := range res {
		...
		dur.Duration = v
		rd, err = dur.RoundFrac(int8(b.tp.GetDecimal()), b.ctx.GetSessionVars().Location())
		if err != nil {
			return err
		}
		res[i] = rd.Duration
...
func (b *builtinCastDecimalAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		tm, err := types.ParseTimeFromFloatString(stmt, string(decimals[i].ToString()), b.tp.GetTp(), fsp)
		if err != nil {
			if err = handleInvalidTimeError(b.ctx, err); err != nil {
				return err
			}
...
func (b *builtinCastStringAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		tm, err := types.ParseTime(stmtCtx, buf.GetString(i), b.tp.GetTp(), fsp)
		if err != nil {
			if err = handleInvalidTimeError(b.ctx, err); err != nil {
				return err
			}
...
func (b *builtinCastDecimalAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, err := types.ParseDuration(b.ctx.GetSessionVars().StmtCtx, string(args[i].ToString()), int8(b.tp.GetDecimal()))
		if err != nil {
			if types.ErrTruncatedWrongVal.Equal(err) {
				err = b.ctx.GetSessionVars().StmtCtx.HandleTruncate(err)
				if err != nil {
...
func (b *builtinCastJSONAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		dur, err = types.ParseDuration(ctx, s, int8(b.tp.GetDecimal()))
		if types.ErrTruncatedWrongVal.Equal(err) {
