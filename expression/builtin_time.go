...
func (b *builtinFromUnixTime1ArgSig) evalTime(row chunk.Row) (res types.Time, isNull bool, err error) {
	...
	return evalFromUnixTime(b.ctx, b.tp.Decimal, unixTimeStamp)
}
...
func (b *builtinFromUnixTime2ArgSig) evalString(row chunk.Row) (res string, isNull bool, err error) {
	...
	t, isNull, err := evalFromUnixTime(b.ctx, b.tp.Decimal, unixTimeStamp)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (b *builtinExtractDatetimeFromStringSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	if dt.IsZero() {
		dt.SetFsp(b.args[1].GetType().Decimal)
		if b.ctx.GetSessionVars().SQLMode.HasNoZeroDateMode() {
...
func (b *builtinConvertTzSig) convertTz(dt types.Time, fromTzStr, toTzStr string) (types.Time, bool, error) {
	...
	if !fromTzMatched && !toTzMatched {
		...
		return types.NewTime(types.FromGoTime(t.In(toTz)), mysql.TypeDatetime, b.tp.Decimal), false, nil
	}
	if fromTzMatched && toTzMatched {
		...
		return types.NewTime(types.FromGoTime(t.Add(timeZone2Duration(toTzStr)-timeZone2Duration(fromTzStr))), mysql.TypeDatetime, b.tp.Decimal), false, nil
	}
	return types.ZeroTime, true, nil
}
...
func (b *builtinSecToTimeSig) evalDuration(row chunk.Row) (types.Duration, bool, error) {
	...
	dur, err = types.ParseDuration(b.ctx.GetSessionVars().StmtCtx, fmt.Sprintf("%s%02d:%02d:%s", negative, hour, minute, strconv.FormatFloat(secondDemical, 'f', -1, 64)), b.tp.Decimal)
	if err != nil {
		return types.Duration{}, err != nil, err
	}
	return dur, false, nil
...
func getExpressionFsp(ctx sessionctx.Context, expression Expression) (int, error) {
	...
	return mathutil.Min(warpExpr.GetType().Decimal, types.MaxFsp), nil
}
