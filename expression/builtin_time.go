...
func (b *builtinDateSig) evalTime(row chunk.Row) (types.Time, bool, error) {
	...
	if expr.IsZero() {
		return types.Time(0), true, handleInvalidTimeError(b.ctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, expr.String()))
	}
...
func (b *builtinDateDiffSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	return int64(types.DateDiff(lhs, rhs)), false, nil
}
...
func (b *builtinStrToDateDateSig) evalTime(row chunk.Row) (types.Time, bool, error) {
	...
	if isNull || err != nil {
		return types.Time(0), isNull, err
	}
	format, isNull, err := b.args[1].EvalString(b.ctx, row)
	if isNull || err != nil {
		return types.Time(0), isNull, err
	...
	if !succ {
		return types.Time(0), true, handleInvalidTimeError(b.ctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String()))
	}
...
func (b *builtinUTCTimestampWithArgSig) evalTime(row chunk.Row) (types.Time, bool, error) {
	...
	if !isNull && num > int64(types.MaxFsp) {
		return types.Time(0), true, errors.Errorf("Too-big precision %v specified for 'utc_timestamp'. Maximum is %v.", num, types.MaxFsp)
	}
	if !isNull && num < int64(types.MinFsp) {
		return types.Time(0), true, errors.Errorf("Invalid negative %d specified, must in [0, 6].", num)
	}
...
func (b *builtinNowWithArgSig) evalTime(row chunk.Row) (types.Time, bool, error) {
	...
	if isNull {
		...
		return types.Time(0), true, errors.Errorf("Too-big precision %v specified for 'now'. Maximum is %v.", fsp, types.MaxFsp)
	} else if fsp < int64(types.MinFsp) {
		return types.Time(0), true, errors.Errorf("Invalid negative %d specified, must in [0, 6].", fsp)
	}
...
func (du *baseDateArithmitical) getDateFromInt(ctx sessionctx.Context, args []Expression, row chunk.Row, unit string) (types.Time, bool, error) {
	...
	if err != nil {
		return types.Time(0), true, handleInvalidTimeError(ctx, err)
	}
...
func (du *baseDateArithmitical) add(ctx sessionctx.Context, date types.Time, interval string, unit string) (types.Time, bool, error) {
	...
	if goTime.Year() < 0 || goTime.Year() > (1<<16-1) {
		return types.Time(0), true, handleInvalidTimeError(ctx, types.ErrDatetimeFunctionOverflow.GenWithStackByArgs("datetime"))
	}
...
func (b *builtinLastDaySig) evalTime(row chunk.Row) (types.Time, bool, error) {
	...
	if arg.InvalidZero() {
		return types.Time(0), true, handleInvalidTimeError(b.ctx, types.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, arg.String()))
	}
