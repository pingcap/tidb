...
func (b *builtinDurationStringTimeDiffSig) evalDuration(row chunk.Row) (d types.Duration, isNull bool, err error) {
	...
	sc := b.ctx.GetSessionVars().StmtCtx
	rhs, _, isDuration, err := convertStringToDuration(sc, rhsStr, int8(b.tp.GetDecimal()))
...
func (b *builtinStringDurationTimeDiffSig) evalDuration(row chunk.Row) (d types.Duration, isNull bool, err error) {
	...
	sc := b.ctx.GetSessionVars().StmtCtx
	lhs, _, isDuration, err := convertStringToDuration(sc, lhsStr, int8(b.tp.GetDecimal()))
	if err != nil || !isDuration {
		return d, true, err
	}
...
func (b *builtinTimeStringTimeDiffSig) evalDuration(row chunk.Row) (d types.Duration, isNull bool, err error) {
	...
	sc := b.ctx.GetSessionVars().StmtCtx
	_, rhs, isDuration, err := convertStringToDuration(sc, rhsStr, int8(b.tp.GetDecimal()))
	if err != nil || isDuration {
		return d, true, err
	}
...
func (b *builtinStringTimeTimeDiffSig) evalDuration(row chunk.Row) (d types.Duration, isNull bool, err error) {
	...
	sc := b.ctx.GetSessionVars().StmtCtx
	_, lhs, isDuration, err := convertStringToDuration(sc, lhsStr, int8(b.tp.GetDecimal()))
	if err != nil || isDuration {
		return d, true, err
	}
...
func (c *dateFormatFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	bf.tp = bf.tp.SetFlen((args[1].GetType().GetFlen() + 1) / 2 * 11)
...
func setFlen(bf baseBuiltinFunc, flen int) {
	builder := bf.tp.ToBuilder()
	builder.Flen = flen
	bf.tp = builder.Build()
}

func setFlenAndDecimal(bf baseBuiltinFunc, flen int, decimal int) {
	builder := bf.tp.ToBuilder()
	builder.Flen, builder.Decimal = flen, decimal
	bf.tp = builder.Build()
}

func setFlenCharsetCollate(bf baseBuiltinFunc, flen int, charset, collate string) {
	builder := bf.tp.ToBuilder()
	builder.Charset, builder.Collate, builder.Flen = charset, collate, flen
	bf.tp = builder.Build()
}
...
func (c *monthNameFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	setFlenCharsetCollate(bf, 10, charset, collate)
...
func (c *fromUnixTimeFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (sig builtinFunc, err error) {
	...
	if !isArg0Str {
		if args[0].GetType().GetDecimal() != types.UnspecifiedLength {
			fsp = mathutil.Min(bf.tp.GetDecimal(), args[0].GetType().GetDecimal())
		}
	}
...
func (b *builtinFromUnixTime1ArgSig) evalTime(row chunk.Row) (res types.Time, isNull bool, err error) {
	...
	return evalFromUnixTime(b.ctx, int8(b.tp.GetDecimal()), unixTimeStamp)
}
...
func (b *builtinFromUnixTime2ArgSig) evalString(row chunk.Row) (res string, isNull bool, err error) {
	...
	t, isNull, err := evalFromUnixTime(b.ctx, int8(b.tp.GetDecimal()), unixTimeStamp)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (c *sysDateFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	bf.tp = bf.tp.SetFlag(bf.tp.GetFlag() | mysql.NotNullFlag)
...
func (c *extractFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (sig builtinFunc, err error) {
	...
	if isDatetimeUnit {
		if args[1].GetType().EvalType() != types.ETString {
			...
			bf.args[1].SetType(bf.args[1].GetType().SetDecimal(int(types.MaxFsp)))
...
func (b *builtinExtractDatetimeFromStringSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	if dt.IsZero() {
		dt.SetFsp(int8(b.args[1].GetType().GetDecimal()))
		if b.ctx.GetSessionVars().SQLMode.HasNoZeroDateMode() {
	...
	if dateEvalTp == types.ETDuration {
		...
		case "SECOND":
			...
			if intervalEvalTp != types.ETString {
				internalFsp = mathutil.Min(args[1].GetType().GetDecimal(), int(types.MaxFsp))
			}
		...
		if dateEvalTp == types.ETDatetime && args[0].GetType().GetTp() == mysql.TypeTimestamp {
			tp := types.NewFieldTypeBuilder(mysql.TypeDatetime)
			tp.Decimal = args[0].GetType().GetDecimal()
			tp.Flen = mysql.MaxDatetimeWidthNoFsp
			if tp.Decimal > 0 {
				tp.Flen = tp.Flen + 1 + tp.Decimal
			}
			types.SetBinChsClnFlag(tp)
			args[0] = BuildCastFunction(ctx, args[0], tp.Build())
		}
...
func (c *unixTimestampFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	setFlenAndDecimal(bf, retFLen, retDecimal)
...
func getBf4TimeAddSub(ctx sessionctx.Context, funcName string, args []Expression) (tp1, tp2 *types.FieldType, bf baseBuiltinFunc, err error) {
	...
	case types.ETString:
		builder := bf.tp.ToBuilder()
		builder.Tp, builder.Flen, builder.Decimal = mysql.TypeString, mysql.MaxDatetimeWidthWithFsp, types.UnspecifiedLength
		bf.tp = builder.Build()
	}
	return
}
...
func (c *addTimeFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (sig builtinFunc, err error) {
	...
	case mysql.TypeDate:
		builder := bf.tp.ToBuilder()
		builder.Charset, builder.Collate = ctx.GetSessionVars().GetCharsetInfo()
		bf.tp = builder.Build()
		switch tp2.GetTp() {
		case mysql.TypeDuration:
...
func (b *builtinConvertTzSig) convertTz(dt types.Time, fromTzStr, toTzStr string) (types.Time, bool, error) {
	...
	return types.NewTime(types.FromGoTime(t.In(toTz)), mysql.TypeDatetime, int8(b.tp.GetDecimal())), false, nil
}
...
func (b *builtinSecToTimeSig) evalDuration(row chunk.Row) (types.Duration, bool, error) {
	...
	dur, err = types.ParseDuration(b.ctx.GetSessionVars().StmtCtx, fmt.Sprintf("%s%02d:%02d:%s", negative, hour, minute, strconv.FormatFloat(secondDemical, 'f', -1, 64)), int8(b.tp.GetDecimal()))
	if err != nil {
		return types.Duration{}, err != nil, err
	}
	return dur, false, nil
...
func (c *timeFormatFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	// worst case: formatMask=%r%r%r...%r, each %r takes 11 characters
	setFlen(bf, (args[1].GetType().GetFlen()+1)/2*11)
...
func getExpressionFsp(ctx sessionctx.Context, expression Expression) (int, error) {
	...
	warpExpr := WrapWithCastAsTime(ctx, expression, mysql.TypeDatetime)
	return mathutil.Min(warpExpr.GetType().GetDecimal(), int(types.MaxFsp)), nil
}
...
func (c *tidbParseTsoFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	builder := bf.tp.ToBuilder()
	builder.Tp, builder.Flen, builder.Decimal = mysql.TypeDate, mysql.MaxDateWidth, int(types.DefaultFsp)
	bf.tp = builder.Build()
