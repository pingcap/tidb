...
func (h *benchHelper) init() {
	...
	cols := make([]*expression.Column, 0, len(h.inputTypes))
	...
	h.exprs = make([]expression.Expression, 0, 10)
	if expr, err := expression.NewFunction(h.ctx, ast.Substr, h.inputTypes[3], []expression.Expression{cols[3], cols[2]}...); err != nil {
		panic("create SUBSTR function failed.")
	} else {
		h.exprs = append(h.exprs, expr)
	}

	if expr, err := expression.NewFunction(h.ctx, ast.Plus, h.inputTypes[0], []expression.Expression{cols[1], cols[2]}...); err != nil {
		panic("create PLUS function failed.")
	} else {
		h.exprs = append(h.exprs, expr)
	}

	if expr, err := expression.NewFunction(h.ctx, ast.GT, h.inputTypes[2], []expression.Expression{cols[11], cols[8]}...); err != nil {
		panic("create GT function failed.")
	} else {
		h.exprs = append(h.exprs, expr)
	}

	if expr, err := expression.NewFunction(h.ctx, ast.GT, h.inputTypes[2], []expression.Expression{cols[19], cols[10]}...); err != nil {
		panic("create GT function failed.")
	} else {
		h.exprs = append(h.exprs, expr)
	}

	if expr, err := expression.NewFunction(h.ctx, ast.GT, h.inputTypes[2], []expression.Expression{cols[17], cols[4]}...); err != nil {
		panic("create GT function failed.")
	} else {
		h.exprs = append(h.exprs, expr)
	}

	if expr, err := expression.NewFunction(h.ctx, ast.GT, h.inputTypes[2], []expression.Expression{cols[18], cols[5]}...); err != nil {
		panic("create GT function failed.")
	} else {
		h.exprs = append(h.exprs, expr)
	}

	if expr, err := expression.NewFunction(h.ctx, ast.LE, h.inputTypes[2], []expression.Expression{cols[19], cols[4]}...); err != nil {
		panic("create LE function failed.")
	} else {
		h.exprs = append(h.exprs, expr)
	}

	if expr, err := expression.NewFunction(h.ctx, ast.EQ, h.inputTypes[2], []expression.Expression{cols[20], cols[3]}...); err != nil {
...
func BenchmarkVectorizedExecute(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		h.outputChunk.Reset()
		if err := expression.VectorizedExecute(h.ctx, h.exprs, inputIter, h.outputChunk); err != nil {
...
func BenchmarkScalarFunctionClone(b *testing.B) {
	...
	con1 := expression.NewOne()
	con2 := expression.NewZero()
	add := expression.NewFunctionInternal(mock.NewContext(), ast.Plus, types.NewFieldType(mysql.TypeLonglong), col, con1)
	sub := expression.NewFunctionInternal(mock.NewContext(), ast.Plus, types.NewFieldType(mysql.TypeLonglong), add, con2)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		sub.Clone()
	}
...
func (g *defaultGener) gen() interface{} {
	...
	case types.ETDatetime, types.ETTimestamp:
		...
		t := types.NewTime(gt, expression.convertETType(g.eType), 0)
...
func (g *locationGener) gen() interface{} {
	...
	case 0:
		return expression.usaLocation
	case 1:
		return expression.jisLocation
	case 2:
		return expression.isoLocation
	case 3:
		return expression.eurLocation
	case 4:
		return expression.internalLocation
	default:
		return nil
	}
}
...
func (g *formatGener) gen() interface{} {
	...
	switch g.randGen.Uint32() % 4 {
	case 0:
		return expression.dateFormat
	case 1:
		return expression.datetimeFormat
	case 2:
		return expression.timestampFormat
	case 3:
		return expression.timeFormat
	default:
		return nil
	}
}
...
func genVecExprBenchCase(ctx sessionctx.Context, funcName string, testCase vecExprBenchCase) (expr expression.Expression, fts []*types.FieldType, input *chunk.Chunk, output *chunk.Chunk) {
	...
	cols := make([]expression.Expression, len(testCase.childrenTypes))
	...
	expr, err := expression.NewFunction(ctx, funcName, eType2FieldType(testCase.retEvalType), cols...)
	if err != nil {
		panic(err)
	}
...
func testVectorizedEvalOneVec(t *testing.T, vecExprCases vecExprBenchCases) {
	...
	for funcName, testCases := range vecExprCases {
		for _, testCase := range testCases {
			...
			require.NoErrorf(t, expression.evalOneVec(ctx, expr, input, output, 0), "func: %v, case: %+v", funcName, testCase)
			it := chunk.NewIterator4Chunk(input)
			require.NoErrorf(t, expression.evalOneColumn(ctx, expr, it, output2, 0), "func: %v, case: %+v", funcName, testCase)
	...
	for funcName, testCases := range vecExprCases {
		for _, testCase := range testCases {
			...
			b.Run(exprName+"-EvalOneVec", func(b *testing.B) {
				b.ResetTimer()
				for i := 0; i < b.N; i++ {
					if err := expression.evalOneVec(ctx, expr, input, output, 0); err != nil {
						b.Fatal(err)
					}
				}
			})
			b.Run(exprName+"-EvalOneCol", func(b *testing.B) {
				...
				for i := 0; i < b.N; i++ {
					it := chunk.NewIterator4Chunk(input)
					if err := expression.evalOneColumn(ctx, expr, it, output, 0); err != nil {
						b.Fatal(err)
					}
				}
			})
...
func genVecBuiltinFuncBenchCase(ctx sessionctx.Context, funcName string, testCase vecExprBenchCase) (baseFunc expression.builtinFunc, fts []*types.FieldType, input *chunk.Chunk, result *chunk.Column) {
	...
	cols := make([]expression.Expression, childrenNumber)
	if testCase.chunkSize <= 0 || testCase.chunkSize > 1024 {
		testCase.chunkSize = 1024
	}
	input = chunk.New(fts, testCase.chunkSize, testCase.chunkSize)
	...
	if funcName == ast.Cast {
		var fc expression.functionClass
		tp := eType2FieldType(testCase.retEvalType)
		switch testCase.retEvalType {
		case types.ETInt:
			fc = &expression.castAsIntFunctionClass{expression.baseFunctionClass{ast.Cast, 1, 1}, tp}
		case types.ETDecimal:
			fc = &expression.castAsDecimalFunctionClass{expression.baseFunctionClass{ast.Cast, 1, 1}, tp}
		case types.ETReal:
			fc = &expression.castAsRealFunctionClass{expression.baseFunctionClass{ast.Cast, 1, 1}, tp}
		case types.ETDatetime, types.ETTimestamp:
			fc = &expression.castAsTimeFunctionClass{expression.baseFunctionClass{ast.Cast, 1, 1}, tp}
		case types.ETDuration:
			fc = &expression.castAsDurationFunctionClass{expression.baseFunctionClass{ast.Cast, 1, 1}, tp}
		case types.ETJson:
			fc = &expression.castAsJSONFunctionClass{expression.baseFunctionClass{ast.Cast, 1, 1}, tp}
		case types.ETString:
			fc = &expression.castAsStringFunctionClass{expression.baseFunctionClass{ast.Cast, 1, 1}, tp}
		}
		...
		switch testCase.retEvalType {
		case types.ETInt:
			fc = &expression.getIntVarFunctionClass{expression.getVarFunctionClass{expression.baseFunctionClass{ast.GetVar, 1, 1}, tp}}
		case types.ETDecimal:
			fc = &expression.getDecimalVarFunctionClass{expression.getVarFunctionClass{expression.baseFunctionClass{ast.GetVar, 1, 1}, tp}}
		case types.ETReal:
			fc = &expression.getRealVarFunctionClass{expression.getVarFunctionClass{expression.baseFunctionClass{ast.GetVar, 1, 1}, tp}}
		default:
			fc = &expression.getStringVarFunctionClass{expression.getVarFunctionClass{expression.baseFunctionClass{ast.GetVar, 1, 1}, tp}}
		}
		baseFunc, err = fc.getFunction(ctx, cols)
	} else {
		baseFunc, err = expression.funcs[funcName].getFunction(ctx, cols)
	}
	if err != nil {
		panic(err)
	}
...
func removeTestOptions(args []string) []string {
	...
	for _, arg := range args {
		if strings.HasPrefix(arg, "builtin") || expression.IsFunctionSupported(arg) {
...
func genVecEvalBool(numCols int, colTypes, eTypes []types.EvalType) (expression.CNFExprs, *chunk.Chunk) {
	...
	exprs := make(expression.CNFExprs, 0, numCols)
	for i := 0; i < numCols; i++ {
		fillColumn(ts[i], input, i, vecExprBenchCase{geners: gs})
		exprs = append(exprs, &expression.Column{Index: i, RetType: fts[i]})
	}
	return exprs, input
}
		...
		combFunc = func(nCols int) {
			if nCols == 0 {
				...
				b.Run("Vec-"+name, func(b *testing.B) {
					b.ResetTimer()
					for i := 0; i < b.N; i++ {
						_, _, err := expression.VecEvalBool(ctx, exprs, input, selected, nulls)
						if err != nil {
							b.Fatal(err)
						}
					}
				...
				b.Run("Row-"+name, func(b *testing.B) {
					...
					for i := 0; i < b.N; i++ {
						it := chunk.NewIterator4Chunk(input)
						for row := it.Begin(); row != it.End(); row = it.Next() {
							_, _, err := expression.EvalBool(ctx, exprs, row)
							if err != nil {
								b.Fatal(err)
							}
						}
...
func BenchmarkRowBasedFilterAndVectorizedFilter(b *testing.B) {
		...
		combFunc = func(nCols int) {
			if nCols == 0 {
				...
				b.Run("Vec-"+name, func(b *testing.B) {
					b.ResetTimer()
					for i := 0; i < b.N; i++ {
						_, _, err := expression.vectorizedFilter(ctx, exprs, it, selected, nulls)
						if err != nil {
							b.Fatal(err)
						}
					}
				...
				b.Run("Row-"+name, func(b *testing.B) {
					b.ResetTimer()
					for i := 0; i < b.N; i++ {
						_, _, err := expression.rowBasedFilter(ctx, exprs, it, selected, nulls)
						if err != nil {
							b.Fatal(err)
						}
					}
	...
	b.Run("Vec-special case", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, _, err := expression.vectorizedFilter(ctx, []expression.Expression{expr}, it, selected, nulls)
			if err != nil {
				panic(err)
			}
		}
	...
	b.Run("Row-special case", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, _, err := expression.rowBasedFilter(ctx, []expression.Expression{expr}, it, selected, nulls)
			if err != nil {
				panic(err)
			}
		}
...
func TestBenchDaily(t *testing.T) {
	benchdaily.Run(
		expression.BenchmarkCastIntAsIntRow,
		expression.BenchmarkCastIntAsIntVec,
