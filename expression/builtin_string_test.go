...
func (s *testEvaluatorSuite) TestLocate(c *C) {
	...
	for i, t := range Dtbl {
		...
		c.Assert(err, IsNil)
		got, err := newMockBuiltinFunc(f).eval(nil)
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
...
func (s *testEvaluatorSuite) TestFormat(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(f2, NotNil)
	r2, err := newMockBuiltinFunc(f2).eval(nil)
	...
	c.Assert(err, IsNil)
	c.Assert(f3, NotNil)
	r3, err := newMockBuiltinFunc(f3).eval(nil)
...
func (s *testEvaluatorSuite) TestExportSet(c *C) {
	...
	for _, t := range estd {
		...
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		exportSetRes, err := newMockBuiltinFunc(f).eval(nil)
