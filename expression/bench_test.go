...
func (g *defaultGener) gen() interface{} {
	...
	case types.ETDatetime, types.ETTimestamp:
		...
		for _, err := t.GoTime(Local); err != nil; {
			gt = getRandomTime(g.randGen.Rand)
			t = types.NewTime(gt, convertETType(g.eType), 0)
			_, err = t.GoTime(Local)
		}
		return t
...
func init() {
	...
	for _, v := range ambiguousTimeStrSlice {
		start, _ := time.ParseInLocation(format, v.start, Local)
		end, _ := time.ParseInLocation(format, v.end, Local)
		amt := ambiguousTime{
...
func NotAmbiguousTime(t types.CoreTime) bool {
	ok := true
	for _, amt := range ambiguousTimeSlice {
		if uint64(t) >= amt.start && uint64(t) <= amt.end {
			ok = false
			break
		}
	}
