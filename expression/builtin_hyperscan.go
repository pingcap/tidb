 ...
 *
 * Function list:
 *   hs_match(source, patterns, [type])
 *   hs_match_json(source, jsonPatterns)
 *   hs_match_all(source, patterns, [type])
 *   hs_match_all_json(source, jsonPatterns)
 *   hs_match_ids(source, patterns, [type])
 *   hs_match_ids_json(source, jsonPatterns)
 ...
 * params:
 *    source: string, required, table column or string for matching
 *    patterns: string, required, patterns format see Patterns Format
 *    type: string, optional, patterns format type see Patterns Format
 *
 * hs_match_json(source, jsonPatterns)
 *    match any pattern in patterns
 * params:
 ...
 *
 * hs_build_db_json(jsonPatterns, [encodeType])
 *    build hyperscan database and marshal into encodeType
 * params:
 *    jsonPatterns: string, required, patterns format in json, see Patterns Format
 *    encodeType: string, optional, marshal data encode type should be `hex` or `base64`, if ignored default is hex
 *
 * Patterns Format
 *   1. `lines`
 ...
 *   In `hs_match_all` function if source type is `hex` or `base64`, function cannot find out how many patterns in database, so `hs_match_all` function only support source type in `lines` and `json`.
 */
