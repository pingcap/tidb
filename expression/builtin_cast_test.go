...
func (s *testEvaluatorSuite) TestCastFuncSig(c *C) {
	...
	for i, t := range castToDecCases {
		...
		b, err := newBaseBuiltinFunc(ctx, &baseCastFunctionClass{}, args, 0)
		c.Assert(err, IsNil)
	...
	for i, t := range castToStringCases {
		...
		tp.Charset = charset.CharsetBin
		args := []Expression{t.before}
		stringFunc, err := newBaseBuiltinFunc(ctx, &baseCastFunctionClass{}, args, 0)
		c.Assert(err, IsNil)
		stringFunc.tp = tp
		switch i {
		case 0:
	...
	for i, t := range castToTimeCases {
		...
		timeFunc, err := newBaseBuiltinFunc(ctx, &baseCastFunctionClass{}, args, 0)
		c.Assert(err, IsNil)
		timeFunc.tp = tp
		switch i {
		case 0:
	...
	for i, t := range castToDurationCases {
		...
		tp.Decimal = int(types.DefaultFsp)
		durationFunc, err := newBaseBuiltinFunc(ctx, &baseCastFunctionClass{}, args, 0)
		c.Assert(err, IsNil)
		durationFunc.tp = tp
		switch i {
		case 0:
	...
	bf, err := newBaseBuiltinFunc(ctx, &baseCastFunctionClass{}, args, 0)
	c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestCastJSONAsDecimalSig(c *C) {
	...
	b, err := newBaseBuiltinFunc(ctx, &baseCastFunctionClass{}, []Expression{col}, 0)
	c.Assert(err, IsNil)
	decFunc := newBaseBuiltinCastFunc(b, false)
	decFunc.tp = types.NewFieldType(mysql.TypeNewDecimal)
	decFunc.tp.Flen = 60
...
func (s *testEvaluatorSuite) TestCastStringAsDecimalSigWithUnsignedFlagInUnion(c *C) {
	...
	b, err := newBaseBuiltinFunc(mock.NewContext(), &baseCastFunctionClass{}, []Expression{col}, 0)
	c.Assert(err, IsNil)
	// set `inUnion` to `true`
