...
func (s *testEvaluatorSuite) TestCastFuncSig(c *C) {
	...
	row := types.DatumnRow{types.NewDatum(nil)}
...
func (s *testEvaluatorSuite) TestWrapWithCastAsTypesClasses(c *C) {
	...
	c.Assert(strExpr.GetType().EvalType().IsStringKind(), IsTrue)
	strRes, isNull, err := strExpr.EvalString(types.DatumnRow{types.NewUintDatum(math.MaxUint64)}, sc)
	c.Assert(err, IsNil)
	c.Assert(strRes, Equals, strconv.FormatUint(math.MaxUint64, 10))
	c.Assert(isNull, Equals, false)

	strRes, isNull, err = strExpr.EvalString(types.DatumnRow{types.NewUintDatum(1234)}, sc)
	c.Assert(err, IsNil)
	c.Assert(isNull, Equals, false)
	...
	c.Assert(decExpr.GetType().EvalType(), Equals, types.ETDecimal)
	decRes, isNull, err := decExpr.EvalDecimal(types.DatumnRow{types.NewUintDatum(uint64(1234))}, sc)
	c.Assert(err, IsNil)
	c.Assert(isNull, Equals, false)
	...
	timeRes, isNull, err := timeExpr.EvalTime(types.DatumnRow{types.NewUintDatum(uint64(curTimeInt))}, sc)
	c.Assert(err, IsNil)
	c.Assert(isNull, Equals, false)
