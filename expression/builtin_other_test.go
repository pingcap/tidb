...
func (s *testEvaluatorSuite) TestBitCount(c *C) {
	...
	for _, test := range bitCountCases {
		...
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		count, err := newMockBuiltinFunc(f).eval(nil)
		c.Assert(err, IsNil)
		if count.IsNull() {
...
func (s *testEvaluatorSuite) TestSetVar(c *C) {
	...
	for _, tc := range testCases {
		...
		c.Assert(err, IsNil)
		d, err := newMockBuiltinFunc(fn).eval(types.MakeDatums(tc.args...))
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestValues(c *C) {
	...
	c.Assert(err, IsNil)
	_, err = newMockBuiltinFunc(sig).eval(nil)
	...
	ret, err := newMockBuiltinFunc(sig).eval(nil)
	c.Assert(err, IsNil)
