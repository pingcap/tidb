...
func TestSetVar(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		if tc.args[1] != nil {
			...
			sessionVar, ok, _, _ := ctx.GetSessionVars().GetUserVar(key)
			require.Equal(t, true, ok)
...
func TestGetVar(t *testing.T) {
	...
	for _, kv := range sessionVars {
		...
		ctx.GetSessionVars().SetUserVar(kv.key, types.NewDatum(kv.val), tp)
	}
...
func TestSetVarFromColumn(t *testing.T) {
	...
	sessionVar, ok, _, _ := sessionVars.GetUserVar("a")
	require.Equal(t, true, ok)
