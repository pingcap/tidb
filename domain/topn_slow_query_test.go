...
func TestPush(t *testing.T) {
	...
	assert.Equal(t, 300*time.Millisecond, slowQuery.user.data[0].Duration)
	checkHeap(&slowQuery.user, t)

	// Update all data in the heap.
	slowQuery.Append(&SlowQueryInfo{Duration: 1300 * time.Millisecond})
	assert.Equal(t, 400*time.Millisecond, slowQuery.user.data[0].Duration)
	slowQuery.Append(&SlowQueryInfo{Duration: 1400 * time.Millisecond})
	assert.Equal(t, 500*time.Millisecond, slowQuery.user.data[0].Duration)
	slowQuery.Append(&SlowQueryInfo{Duration: 1500 * time.Millisecond})
	assert.Equal(t, 600*time.Millisecond, slowQuery.user.data[0].Duration)
	slowQuery.Append(&SlowQueryInfo{Duration: 1500 * time.Millisecond})
	assert.Equal(t, 700*time.Millisecond, slowQuery.user.data[0].Duration)
	slowQuery.Append(&SlowQueryInfo{Duration: 1600 * time.Millisecond})
	assert.Equal(t, 800*time.Millisecond, slowQuery.user.data[0].Duration)
	slowQuery.Append(&SlowQueryInfo{Duration: 1700 * time.Millisecond})
	assert.Equal(t, 900*time.Millisecond, slowQuery.user.data[0].Duration)
	slowQuery.Append(&SlowQueryInfo{Duration: 1800 * time.Millisecond})
	assert.Equal(t, 1000*time.Millisecond, slowQuery.user.data[0].Duration)
	slowQuery.Append(&SlowQueryInfo{Duration: 1900 * time.Millisecond})
	assert.Equal(t, 1100*time.Millisecond, slowQuery.user.data[0].Duration)
	slowQuery.Append(&SlowQueryInfo{Duration: 2000 * time.Millisecond})
	assert.Equal(t, 1200*time.Millisecond, slowQuery.user.data[0].Duration)
	slowQuery.Append(&SlowQueryInfo{Duration: 2100 * time.Millisecond})
	assert.Equal(t, 1300*time.Millisecond, slowQuery.user.data[0].Duration)
	checkHeap(&slowQuery.user, t)
...
func TestRemoveExpired(t *testing.T) {
	...
	assert.Equal(t, 2*time.Nanosecond, slowQuery.user.data[0].Duration)

	slowQuery.RemoveExpired(now.Add(5 * time.Second))
	assert.Len(t, slowQuery.user.data, 2)
	assert.Equal(t, 2*time.Nanosecond, slowQuery.user.data[0].Duration)
	...
	assert.Len(t, slowQuery.user.data, 6)
	assert.Equal(t, 0*time.Nanosecond, slowQuery.user.data[0].Duration)

	slowQuery.RemoveExpired(now.Add(6 * time.Second))
	assert.Len(t, slowQuery.user.data, 4)
	assert.Equal(t, 0*time.Nanosecond, slowQuery.user.data[0].Duration)
}
...
func TestQueue(t *testing.T) {
	...
	assert.Equal(t, "ccc", query[0].SQL)

	query = q.recent.Query(2)
	assert.Equal(t, "ccc", query[0].SQL)
	assert.Equal(t, "bbb", query[1].SQL)

	query = q.recent.Query(6)
	assert.Equal(t, "ccc", query[0].SQL)
	assert.Equal(t, "bbb", query[1].SQL)
	assert.Equal(t, "aaa", query[2].SQL)
	...
	assert.Equal(t, "ggg", query[0].SQL)
	assert.Equal(t, "fff", query[1].SQL)
	assert.Equal(t, "eee", query[2].SQL)

	query = q.recent.Query(6)
	assert.Equal(t, "ggg", query[0].SQL)
	assert.Equal(t, "fff", query[1].SQL)
	assert.Equal(t, "eee", query[2].SQL)
	assert.Equal(t, "ddd", query[3].SQL)
	assert.Equal(t, "ccc", query[4].SQL)
}
...
func checkHeap(q *slowQueryHeap, t *testing.T) {
	for i := 0; i < len(q.data); i++ {
		...
		right := 2*i + 2
		if left < len(q.data) {
			assert.LessOrEqual(t, q.data[i].Duration, q.data[left].Duration)
		}
		if right < len(q.data) {
			assert.LessOrEqual(t, q.data[i].Duration, q.data[right].Duration)
		}
	}
}
