...
func TestAddForceMergeRangesRetriesWithFreshBody(t *testing.T) {
	...
	// Use a known-closed local port to trigger a fast network error on the first address.
	// Some environments may silently drop traffic to privileged ports (e.g. :1), causing
	// the test to block until the request timeout.
	ln, err := net.Listen("tcp", "127.0.0.1:0")
	require.NoError(t, err)
	unreachableAddr := "http://" + ln.Addr().String()
	require.NoError(t, ln.Close())

	setForceMergeInfoSyncerForTest(t, []string{unreachableAddr, server.URL})
