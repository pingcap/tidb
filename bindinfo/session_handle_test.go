...
func TestSessionBinding(t *testing.T) {
	...
	for _, testSQL := range testSQLs {
		...
		pb := &dto.Metric{}
		err = metrics.BindTotalGauge.WithLabelValues(metrics.ScopeSession, bindinfo.Enable).Write(pb)
		require.NoError(t, err)
		require.Equal(t, float64(1), pb.GetGauge().GetValue())
		err = metrics.BindMemoryUsage.WithLabelValues(metrics.ScopeSession, bindinfo.Enable).Write(pb)
		require.NoError(t, err)
...
func TestBaselineDBLowerCase(t *testing.T) {
	...
	tk.MustExec("insert into mysql.bind_info values('select * from `spm` . `t`', 'select * from `spm` . `t`', 'SPM', 'enable', '2000-01-01 09:00:00', '2000-01-01 09:00:00', '', '','" +
		bindinfo.Manual + "')")
...
func TestShowGlobalBindings(t *testing.T) {
	...
	tk.MustExec("insert into mysql.bind_info values('select * from `spm` . `t`', 'select * from `spm` . `t` USE INDEX (`a`)', 'SPM', 'enable', '2000-01-01 09:00:00', '2000-01-01 09:00:00', '', '','" +
		bindinfo.Manual + "')")
	tk.MustExec("insert into mysql.bind_info values('select * from `spm` . `t0`', 'select * from `spm` . `t0` USE INDEX (`a`)', 'SPM', 'enable', '2000-01-02 09:00:00', '2000-01-02 09:00:00', '', '','" +
		bindinfo.Manual + "')")
	tk.MustExec("insert into mysql.bind_info values('select * from `spm` . `t`', 'select /*+ use_index(`t` `a`)*/ * from `spm` . `t`', 'SPM', 'enable', '2000-01-03 09:00:00', '2000-01-03 09:00:00', '', '','" +
		bindinfo.Manual + "')")
	tk.MustExec("insert into mysql.bind_info values('select * from `spm` . `t0`', 'select /*+ use_index(`t0` `a`)*/ * from `spm` . `t0`', 'SPM', 'enable', '2000-01-04 09:00:00', '2000-01-04 09:00:00', '', '','" +
		bindinfo.Manual + "')")
	tk.MustExec("admin reload bindings")
	rows = tk.MustQuery("show global bindings").Rows()
