...
func TestFileCheckpointManager(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			setup: func(t *testing.T, mgr *FileCheckpointManager, ctx context.Context) {
				cp1 := &TableCheckpoint{TableName: "db.t1", JobID: 1, Status: CheckpointStatusRunning}
				require.NoError(t, mgr.Update(ctx, cp1))
			},
			operation: func(t *testing.T, mgr *FileCheckpointManager, ctx context.Context, filePath string) {
		...
		},
		{
			...
			operation: func(t *testing.T, mgr *FileCheckpointManager, ctx context.Context, filePath string) {
				require.NoError(t, mgr.Remove(ctx, common.AllTables))
		...
		},
		{
			...
			setup: func(t *testing.T, mgr *FileCheckpointManager, ctx context.Context) {
				cp1 := &TableCheckpoint{TableName: "db.t1", JobID: 1, Status: CheckpointStatusRunning}
				cp2 := &TableCheckpoint{TableName: "db.t2", JobID: 2, Status: CheckpointStatusFinished}
				require.NoError(t, mgr.Update(ctx, cp1))
			...
			operation: func(t *testing.T, mgr *FileCheckpointManager, ctx context.Context, filePath string) {
				...
				require.Contains(t, m, "db.t1")
				require.Contains(t, m, "db.t2")
			},
		},
	}
...
func TestMySQLCheckpointManager(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "Remove single checkpoint",
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec("DELETE FROM `test_schema`.`import_into_checkpoints` WHERE table_name = \\?").
					WithArgs("db.t1").
					WillReturnResult(sqlmock.NewResult(0, 1))
			},
			operation: func(t *testing.T, mgr *MySQLCheckpointManager, ctx context.Context) {
		...
		},
		{
			...
			operation: func(t *testing.T, mgr *MySQLCheckpointManager, ctx context.Context) {
				require.NoError(t, mgr.IgnoreError(ctx, common.AllTables))
			},
		},
		{
		...
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectBegin()
				mock.ExpectQuery("SELECT table_name, job_id, status, message, group_key FROM `test_schema`.`import_into_checkpoints` WHERE status = \\?").
			...
			operation: func(t *testing.T, mgr *MySQLCheckpointManager, ctx context.Context) {
				destroyed, err := mgr.DestroyError(ctx, common.AllTables)
				require.NoError(t, err)
				require.Len(t, destroyed, 1)
			},
		},
		{
			name: "GetCheckpoints returns all",
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery("SELECT table_name, job_id, status, message, group_key FROM `test_schema`.`import_into_checkpoints`").
					WillReturnRows(sqlmock.NewRows([]string{"table_name", "job_id", "status", "message", "group_key"}).
			...
			operation: func(t *testing.T, mgr *MySQLCheckpointManager, ctx context.Context) {
				...
				require.NoError(t, err)
				require.Len(t, cps, 1)
				require.Equal(t, "db.t1", cps[0].TableName)
			},
		},
		{
			name: "DumpTables writes CSV",
