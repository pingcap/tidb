...
func openParser(
	...
	case mydump.SourceTypeParquet:
		parser, err = mydump.NewParquetParserWithMeta(ctx, store, reader, chunk.FileMeta.Path, chunk.FileMeta.ParquetMeta)
		if err != nil {
			return nil, err
		}
	default:
...
func (cr *chunkProcessor) close() {
	if cr.memLimiter != nil {
		cr.memLimiter.Release(cr.memoryUsage)
	}
	_ = cr.parser.Close()
}
