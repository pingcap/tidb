...
#run_lightning --backend import-into --config "${mydir}/err_config.toml" 2>&1 | tee "${console_output_file}"
...
#run_lightning --backend import-into --config "${mydir}/err_default_config.toml" 2>&1 | tee "${console_output_file}"
#if [[ ${PIPESTATUS[0]} -eq 0 ]]; then
#    echo "The lightning import doesn't fail as expected" >&2
#    exit 1
#fi
