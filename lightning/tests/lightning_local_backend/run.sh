...
! run_lightning --backend import-into --enable-checkpoint=0
...
run_sql 'DROP DATABASE IF EXISTS cpeng;'
run_lightning --backend import-into --enable-checkpoint=1 --log-file "$TEST_DIR/lightning-local.log" --config "$CUR/config.toml" -L debug
...
run_lightning --backend import-into --enable-checkpoint=1 --log-file "$TEST_DIR/lightning-local.log" --config "$CUR/config.toml"
...
run_lightning --backend import-into --enable-checkpoint=1 --config "$CUR/config.toml" --log-file $res_file -L debug
...
for ckpt in mysql file; do
  ...
  run_lightning --backend import-into --enable-checkpoint=1 --log-file "$TEST_DIR/lightning-local.log" --config "$CUR/$ckpt.toml"
  set -e
