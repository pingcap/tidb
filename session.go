...
func (s *session) Execute(sql string) ([]ast.RecordSet, error) {
	...
	sessionExecuteDuration.WithLabelValues(stageParse).Observe(float64(time.Since(startTS).Nanoseconds() / 1e6))
	...
	for i, rst := range rawStmts {
		...
		sessionExecuteDuration.WithLabelValues(stageCompile).Observe(float64(time.Since(startTS).Nanoseconds() / 1e6))
		...
		sessionExecuteDuration.WithLabelValues(stageRun).Observe(float64(time.Since(startTS).Nanoseconds() / 1e6))
		if r != nil {
			rs = append(rs, r)
		}
	}
