...
func validateKeyspaceModeConflict(cfg *config.Config, keyspaceName string) error {
	if kerneltype.IsNextGen() && len(keyspaceName) == 0 && !cfg.Standby.StandByMode {
		return errors.New("keyspace name or standby mode is required for nextgen TiDB")
	}
	if kerneltype.IsClassic() && (len(keyspaceName) > 0 || cfg.Standby.StandByMode) {
		return errors.New("keyspace name or standby mode is not supported for classic TiDB")
	}
	return nil
}
...
func main() {
	...
	if err := validateKeyspaceModeConflict(config.GetGlobalConfig(), config.GetGlobalKeyspaceName()); err != nil {
