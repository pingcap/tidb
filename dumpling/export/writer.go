...
func (w *Writer) handleTask(task Task) error {
	...
	case *TaskTableData:
		err := w.WriteTableData(t.Meta, t.Data, t.ChunkIndex, t.TotalChunks, t.IsLastChunk, t.IsStringChunking)
		if err != nil {
			return err
		}
...
func (w *Writer) WriteTableData(meta TableMeta, ir TableDataIR, currentChunk int, totalChunks int, isLastChunk bool, isStringChunking bool) error {
	...
	return utils.WithRetry(tctx, func() (err error) {
		...
		return w.tryToWriteTableData(tctx, meta, ir, currentChunk, totalChunks, isLastChunk, isStringChunking)
	}, newRebuildConnBackOffer(canRebuildConn(conf.Consistency, conf.TransactionalConsistency)))
}

func (w *Writer) tryToWriteTableData(tctx *tcontext.Context, meta TableMeta, ir TableDataIR, curChkIdx int, totalChunks int, isLastChunk bool, isStringChunking bool) error {
	...
	for {
		...
		n, err := format.WriteInsert(tctx, conf, meta, ir, fileWriter, w.metrics, curChkIdx, totalChunks, isLastChunk, isStringChunking)
