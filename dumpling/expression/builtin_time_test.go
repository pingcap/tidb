...
func (s *testEvaluatorSuite) TestClock(c *C) {
	...
	for _, t := range errTbl {
		...
		fc := funcs[ast.Hour]
		f, err := fc.getFunction(s.ctx, datumsToConstants(td))
		c.Assert(err, IsNil)
		_, err = f.eval(nil)
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestNowAndUTCTimestamp(c *C) {
	...
	for i, x := range []struct {
		...
		f, err := x.fc.getFunction(s.ctx, datumsToConstants(nil))
		c.Assert(err, IsNil)
		if i == 0 {
			c.Assert(f.canBeFolded(), IsTrue)
		} else {
		...
		f, err = x.fc.getFunction(s.ctx, datumsToConstants(types.MakeDatums(6)))
		c.Assert(err, IsNil)
		v, err = f.eval(nil)
		ts = x.now()
		c.Assert(err, IsNil)
		...
		f, err = x.fc.getFunction(s.ctx, datumsToConstants(types.MakeDatums(8)))
		c.Assert(err, IsNil)
		_, err = f.eval(nil)
		c.Assert(err, NotNil)

		f, err = x.fc.getFunction(s.ctx, datumsToConstants(types.MakeDatums(-2)))
		c.Assert(err, IsNil)
		_, err = f.eval(nil)
		c.Assert(err, NotNil)
	}
	...
	fc := funcs[ast.Now]
	f, err := fc.getFunction(s.ctx, datumsToConstants(nil))
	c.Assert(err, IsNil)
	v, err := f.eval(nil)
	c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestSysDate(c *C) {
	...
	last := time.Now()
	f, err := fc.getFunction(s.ctx, datumsToConstants(types.MakeDatums(6)))
	c.Assert(err, IsNil)
	v, err := f.eval(nil)
	c.Assert(err, IsNil)
	n := v.GetMysqlTime()
	...
	f, err = fc.getFunction(s.ctx, datumsToConstants(types.MakeDatums(-2)))
	c.Assert(err, IsNil)
	_, err = f.eval(nil)
	c.Assert(err, NotNil)
}
...
func (s *testEvaluatorSuite) TestFromUnixTime(c *C) {
	...
	for _, t := range tbl {
		...
		if len(t.format) == 0 {
			f, err := fc.getFunction(s.ctx, datumsToConstants([]types.Datum{timestamp}))
			c.Assert(err, IsNil)
			v, err := f.eval(nil)
			c.Assert(err, IsNil)
			ans := v.GetMysqlTime()
			...
			f, err := fc.getFunction(s.ctx, datumsToConstants([]types.Datum{timestamp, format}))
			c.Assert(err, IsNil)
			v, err := f.eval(nil)
			c.Assert(err, IsNil)
	...
	f, err = fc.getFunction(s.ctx, datumsToConstants(types.MakeDatums(math.MaxInt32+1)))
	c.Assert(err, IsNil)
	_, err = f.eval(nil)
	c.Assert(err, IsNil)
	c.Assert(v.Kind(), Equals, types.KindNull)
...
func (s *testEvaluatorSuite) TestUTCTime(c *C) {
	...
	for _, test := range tests {
		f, err := fc.getFunction(s.ctx, datumsToConstants(types.MakeDatums(test.param)))
		c.Assert(err, IsNil)
		c.Assert(f.canBeFolded(), IsTrue)
		v, err := f.eval(nil)
		if test.expect > 0 {
...
func (s *testEvaluatorSuite) TestStrToDate(c *C) {
	...
	for _, test := range tests {
		...
		f, err := fc.getFunction(s.ctx, datumsToConstants([]types.Datum{date, format}))
		c.Assert(err, IsNil)
		c.Assert(f.canBeFolded(), IsTrue)
...
func (s *testEvaluatorSuite) TestFromDays(c *C) {
	...
	for _, test := range tests {
		...
		f, err := fc.getFunction(s.ctx, datumsToConstants([]types.Datum{t1}))
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		c.Assert(f.canBeFolded(), IsTrue)
		result, err := f.eval(nil)
...
func (s *testEvaluatorSuite) TestWeek(c *C) {
	...
	for _, test := range tests {
		...
		f, err := fc.getFunction(s.ctx, datumsToConstants([]types.Datum{arg1, arg2}))
		c.Assert(err, IsNil)
		c.Assert(f.canBeFolded(), IsTrue)
		result, err := f.eval(nil)
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestYearWeek(c *C) {
	...
	f, err := fc.getFunction(s.ctx, datumsToConstants(types.MakeDatums("2016-00-05")))
	c.Assert(err, IsNil)
	result, err := f.eval(nil)
	c.Assert(err, IsNil)
	c.Assert(result.IsNull(), IsTrue)
...
func (s *testEvaluatorSuite) TestTimestampDiff(c *C) {
	...
	f, err := fc.getFunction(s.ctx, datumsToConstants([]types.Datum{types.NewStringDatum("DAY"),
...
func (s *testEvaluatorSuite) TestTimestamp(c *C) {
	...
	for _, test := range tests {
		f, err := fc.getFunction(s.ctx, datumsToConstants(test.t))
		c.Assert(err, IsNil)
		d, err := f.eval(nil)
		c.Assert(err, IsNil)
		result, _ := d.ToString()
...
func (s *testEvaluatorSuite) TestQuarter(c *C) {
	...
	f, err := fc.getFunction(s.ctx, datumsToConstants([]types.Datum{argInvalid}))
	c.Assert(err, IsNil)
	result, err := f.eval(nil)
	c.Assert(err, IsNil)
	c.Assert(result.IsNull(), IsTrue)
...
func (s *testEvaluatorSuite) TestPeriodAdd(c *C) {
	...
	for _, test := range tests {
		...
		f, err := fc.getFunction(s.ctx, datumsToConstants([]types.Datum{period, months}))
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		c.Assert(f.canBeFolded(), IsTrue)
		result, err := f.eval(nil)
...
func (s *testEvaluatorSuite) TestPeriodDiff(c *C) {
	...
	for _, test := range tests {
		...
		f, err := fc.getFunction(s.ctx, datumsToConstants([]types.Datum{period1, period2}))
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		c.Assert(f.canBeFolded(), IsTrue)
		result, err := f.eval(nil)
