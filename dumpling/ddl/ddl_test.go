...
func (ts *testSuite) TestT(c *C) {
	...
	c.Assert(err, IsNil)
	alterStmt := statement("alter table t2 add b varchar(255) first").(*stmts.AlterTableStmt)
	sessionctx.GetDomain(ctx).DDL().AlterTable(ctx, tbIdent2, alterStmt.Specs)
	c.Assert(alterStmt.Specs[0].String(), Not(Equals), "")
	_, err = tb.Row(ctx, 1)
	c.Assert(err, IsNil)
	alterStmt = statement("alter table t2 add c int after b").(*stmts.AlterTableStmt)
	sessionctx.GetDomain(ctx).DDL().AlterTable(ctx, tbIdent2, alterStmt.Specs)
	c.Assert(alterStmt.Specs[0].String(), Not(Equals), "")
	_, err = tb.Row(ctx, 2)
	c.Assert(err, IsNil)
