...
run_dumpling --consistency=lock --tables-list "$DB_NAME.$TABLE_NAME2" -L ${DUMPLING_TEST_DIR}/dumpling.log
set -e

actual=$(grep -w "Error 1146: Table 'non_exist_tables.t2' doesn't exist" ${DUMPLING_TEST_DIR}/dumpling.log|wc -l)
