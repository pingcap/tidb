...
type MVCCStore interface {
	...
	UnsafeCleanupRange(startKey, endKye []byte) error
}
...
// UnsafeCleanupRange cleanup ranges that will never be used again.
func (s *MvccStore) UnsafeCleanupRange(startKey, endKey []byte) error {
	s.Lock()
	defer s.Unlock()
