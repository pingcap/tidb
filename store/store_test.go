...
func (s *testKVSuite) TestIsolationInc(c *C) {
	...
	for i := 0; i < threadCnt; i++ {
		go func() {
			...
			for j := 0; j < 100; j++ {
				var id int64
				err := kv.RunInNewTxn(s.s, oracle.GlobalTxnScope, true, func(txn kv.Transaction) error {
					var err1 error
...
func (s *testKVSuite) TestIsolationMultiInc(c *C) {
	...
	err := kv.RunInNewTxn(s.s, oracle.GlobalTxnScope, false, func(txn kv.Transaction) error {
		for _, key := range keys {
