...
func (h *Helper) FetchHotRegion(rw string) (map[uint64]RegionMetric, error) {
	...
	if err := h.requestPD("get-hot-region", "GET", rw, nil, &regionResp); err != nil {
		return nil, err
	}
	metricCnt := 0
...
func (h *Helper) GetRegionsInfo() (*RegionsInfo, error) {
	...
	err := h.requestPD("get-region", "GET", pdapi.Regions, nil, &regionsInfo)
...
func (h *Helper) GetStoreRegionsInfo(storeID uint64) (*RegionsInfo, error) {
	var regionsInfo RegionsInfo
	err := h.requestPD("get-store-regions", "GET", pdapi.StoreRegions+"/"+strconv.FormatUint(storeID, 10), nil, &regionsInfo)
	return &regionsInfo, err
}
...
func (h *Helper) GetRegionInfoByID(regionID uint64) (*RegionInfo, error) {
	...
	err := h.requestPD("get-region-by-id", "GET", pdapi.RegionByID+"/"+strconv.FormatUint(regionID, 10), nil, &regionInfo)
...
func (h *Helper) GetRegionsInfoByRange(sk, ek []byte) (*RegionsInfo, error) {
	var regionsInfo RegionsInfo
	err := h.requestPD("get-regions-by-range", "GET", fmt.Sprintf("%v?key=%s&end_key=%s", pdapi.ScanRegions,
...
func (h *Helper) GetRegionByKey(k []byte) (*RegionInfo, error) {
	var regionInfo RegionInfo
	err := h.requestPD("get-region-by-key", "GET", fmt.Sprintf("%v/%v", pdapi.RegionKey, url.QueryEscape(string(k))), nil, &regionInfo)
	return &regionInfo, err
}
...
func (h *Helper) GetStoresStat() (*StoresStat, error) {
	...
	err := h.requestPD("get-stores-stat", "GET", pdapi.Stores, nil, &storesStat)
