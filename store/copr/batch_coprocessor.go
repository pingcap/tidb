...
func balanceBatchCopTask(ctx context.Context, kvStore *kvStore, originalTasks []*batchCopTask, isMPP bool, balanceWithContinuity bool, balanceContinuousRegionCount int64) []*batchCopTask {
...
func buildBatchCopTasks(bo *Backoffer, store *kvStore, ranges *KeyRanges, storeType kv.StoreType, isMPP bool, balanceWithContinuity bool, balanceContinuousRegionCount int64) ([]*batchCopTask, error) {
	...
	for {
		...
		balanceStart := time.Now()
		batchTasks = balanceBatchCopTask(bo.GetCtx(), store, batchTasks, isMPP, balanceWithContinuity, balanceContinuousRegionCount)
...
func (c *CopClient) sendBatch(ctx context.Context, req *kv.Request, vars *tikv.Variables) kv.Response {
	...
	tasks, err := buildBatchCopTasks(bo, c.store.kvStore, ranges, req.StoreType, false, false, 0)
	if err != nil {
...
func (b *batchCopIterator) retryBatchCopTask(ctx context.Context, bo *Backoffer, batchTask *batchCopTask) ([]*batchCopTask, error) {
	...
	return buildBatchCopTasks(bo, b.store, NewKeyRanges(ranges), b.req.StoreType, false, false, 0)
}
