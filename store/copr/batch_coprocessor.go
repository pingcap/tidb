...
func balanceBatchCopTask(originalTasks []*batchCopTask, balanceWithContinuity bool, balanceContinuousRegionCount int64) []*batchCopTask {
...
func buildBatchCopTasks(bo *Backoffer, cache *RegionCache, ranges *KeyRanges, storeType kv.StoreType, balanceWithContinuity bool, balanceContinuousRegionCount int64) ([]*batchCopTask, error) {
	start := time.Now()
	...
	for {
		...
		balanceStart := time.Now()
		batchTasks = balanceBatchCopTask(batchTasks, balanceWithContinuity, balanceContinuousRegionCount)
...
func (c *CopClient) sendBatch(ctx context.Context, req *kv.Request, vars *tikv.Variables) kv.Response {
	...
	tasks, err := buildBatchCopTasks(bo, c.store.GetRegionCache(), ranges, req.StoreType, false, 0)
	if err != nil {
...
func (b *batchCopIterator) retryBatchCopTask(ctx context.Context, bo *Backoffer, batchTask *batchCopTask) ([]*batchCopTask, error) {
	...
	return buildBatchCopTasks(bo, b.store.GetRegionCache(), NewKeyRanges(ranges), b.req.StoreType, false, 0)
}
