...
func TestBalanceBatchCopTaskWithEmptyTaskSet(t *testing.T) {
	{
		...
		nilResult := balanceBatchCopTask(nil, nil, nilTaskSet, false, time.Second, false, 0, false)
		require.True(t, nilResult == nil)
	}

	{
		emptyTaskSet := make([]*batchCopTask, 0)
		emptyResult := balanceBatchCopTask(nil, nil, emptyTaskSet, false, time.Second, false, 0, false)
