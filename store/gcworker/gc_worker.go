...
retryScanAndResolve:
	for {
		...
		if regionErr != nil {
			err = bo.BackoffWithCfg(tikv.BoRegionMiss(), errors.New(regionErr.String()))
			if err != nil {
				return stat, errors.Trace(err)
			}
			continue
		...
		for {
			...
			if !ok {
				err = bo.BackoffWithCfg(tikv.BoTxnLock(), errors.Errorf("remain locks: %d", len(locks)))
				if err != nil {
					return stat, errors.Trace(err)
				}
...
func (w *GCWorker) uploadSafePointToPD(ctx context.Context, safePoint uint64) error {
	...
	for {
		...
		if err != nil {
			...
			err = bo.BackoffWithCfg(tikv.BoPDRPC(), errors.Errorf("failed to upload safe point to PD, err: %v", err))
			if err != nil {
				return errors.Trace(err)
			}
			continue
