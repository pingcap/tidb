...
func newDescScanner(snapshot *tikvSnapshot, startKey []byte, endKey []byte, batchSize int) (*DescScanner, error) {
...
// Valid return valid.
func (s *DescScanner) Valid() bool {
	return s.valid
}

// Key return key.
func (s *DescScanner) Key() kv.Key {
	if s.valid {
...
func (s *DescScanner) Next() error {
	...
	for {
		...
		if len(s.startKey) > 0 && kv.Key(current.Key).Cmp(kv.Key(s.startKey)) < 0 {
			s.eof = true
			s.Close()
			return nil
		}
...
func (s *DescScanner) Close() {
	s.valid = false
}

func (s *DescScanner) startTS() uint64 {
	return s.snapshot.version.Ver
}

func (s *DescScanner) resolveCurrentLock(bo *Backoffer, current *pb.KvPair) error {
...
func (s *DescScanner) getData(bo *Backoffer) error {
	log.Debugf("txn getData desc endKey:[%q], txn %d", s.nextEndKey, s.startTS())
	...
	for {
		...
		if len(kvPairs) < s.batchSize {
			...
			if len(loc.StartKey) == 0 || (len(s.startKey) > 0 && kv.Key(s.startKey).Cmp(kv.Key(s.nextEndKey)) >= 0) {
				// Current Region is the last one.
				log.Info("fetch last one, end. ========\n\n")
				s.eof = true
			}
			return nil
		}
