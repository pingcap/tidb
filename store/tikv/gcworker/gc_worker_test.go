...
func (s *testGCWorkerSuite) TestCompatibleParseTime(c *C) {
	// The old format: "20060102-15:04:05 -0700 MST"
	...
	for _, value := range values {
		t, err := CompatibleParseTime(value)
		c.Assert(err, IsNil)
	...
	for _, value := range invalidValues {
		_, err := CompatibleParseTime(value)
		c.Assert(err, NotNil)
	}
}
