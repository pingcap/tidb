...
func (s *testAsyncCommitFailSuite) TestAsyncCommitExceedingMaxCommitTSMultiReq(c *C) {
	...
	// Case 1: All requests return CommitTsTooLarge
	...
	txn, err = s.store.Begin()
	c.Assert(err, IsNil)
	v, err := txn.Get(context.Background(), []byte("a"))
	c.Assert(err, IsNil)
