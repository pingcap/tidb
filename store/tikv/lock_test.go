...
func (s *testLockSuite) lockKey(c *C, key, value, primaryKey, primaryValue []byte, commitPrimary bool) (uint64, uint64) {
	...
	if commitPrimary {
		...
		c.Assert(err, IsNil)
		err = tpc.commitMutations(NewBackofferWithVars(ctx, CommitMaxBackoff, nil), staticMutations{tpc.mutationsOfKeys([][]byte{primaryKey})})
		c.Assert(err, IsNil)
	}
...
func (s *testLockSuite) TestCheckTxnStatusNoWait(c *C) {
	...
	c.Assert(err, IsNil)
	err = committer.prepare(context.Background())
	c.Assert(err, IsNil)
