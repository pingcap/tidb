...
func (s *RegionRequestSender) onSendFail(bo *Backoffer, ctx *RPCContext, err error) error {
	...
	if ctx.Store != nil && ctx.Store.storeType == tikvrpc.TiFlash {
		err = bo.BackoffWithCfg(retry.BoTiFlashRPC, errors.Errorf("send tiflash request error: %v, ctx: %v, try next peer later", err, ctx))
	} else {
		err = bo.BackoffWithCfg(retry.BoTiKVRPC, errors.Errorf("send tikv request error: %v, ctx: %v, try next peer later", err, ctx))
	}
	return errors.Trace(err)
}
...
func (s *RegionRequestSender) onRegionError(bo *Backoffer, ctx *RPCContext, seed *uint32, regionErr *errorpb.Error) (shouldRetry bool, err error) {
	...
	if notLeader := regionErr.GetNotLeader(); notLeader != nil {
		...
		if notLeader.GetLeader() == nil {
			...
			if err = bo.BackoffWithCfg(retry.BoRegionMiss, errors.Errorf("not leader: %v, ctx: %v", notLeader, ctx)); err != nil {
	...
	if regionErr.GetServerIsBusy() != nil {
		...
		if ctx != nil && ctx.Store != nil && ctx.Store.storeType == tikvrpc.TiFlash {
			err = bo.BackoffWithCfg(retry.BoTiFlashServerBusy, errors.Errorf("server is busy, ctx: %v", ctx))
		} else {
			err = bo.BackoffWithCfg(retry.BoTiKVServerBusy, errors.Errorf("server is busy, ctx: %v", ctx))
		}
		if err != nil {
			return false, errors.Trace(err)
		}
	...
	if regionErr.GetStaleCommand() != nil {
		...
		err = bo.BackoffWithCfg(retry.BoStaleCmd, errors.Errorf("stale command, ctx: %v", ctx))
		if err != nil {
			return false, errors.Trace(err)
		}
		return true, nil
	...
	if regionErr.GetMaxTimestampNotSynced() != nil {
		...
		err = bo.BackoffWithCfg(retry.BoMaxTsNotSynced, errors.Errorf("max timestamp not synced, ctx: %v", ctx))
		if err != nil {
			return false, errors.Trace(err)
		}
		return true, nil
