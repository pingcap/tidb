...
func NewRawKVClient(pdAddrs []string, security config.Security, opts ...pd.ClientOption) (*RawKVClient, error) {
	...
	return &RawKVClient{
		...
		regionCache: region.NewRegionCache(pdCli),
		pdClient:    pdCli,
...
func (c *RawKVClient) sendReq(key []byte, req *tikvrpc.Request, reverse bool) (*tikvrpc.Response, *region.KeyLocation, error) {
	bo := retry.NewBackofferWithVars(context.Background(), rawkvMaxBackoff, nil)
	sender := region.NewRegionRequestSender(c.regionCache, c.rpcClient)
	for {
...
func appendKeyBatches(batches []batch, regionID region.VerID, groupKeys [][]byte, limit int) []batch {
	var keys [][]byte
...
func appendBatches(batches []batch, regionID region.VerID, groupKeys [][]byte, keyToValue map[string][]byte, limit int) []batch {
	var start, size int
