...
func (action actionPrewrite) handleSingleBatch(c *twoPhaseCommitter, bo *Backoffer, batch batchMutations) error {
	...
	if c.sessionID > 0 {
		if batch.isPrimary {
			...
			failpoint.Eval(_curpkg_("prewritePrimary")) // for other failures like sleep or pause
		} else {
			if _, _err_ := failpoint.Eval(_curpkg_("prewriteSecondaryFail")); _err_ == nil {
				// Delay to avoid cancelling other normally ongoing prewrite requests.
				time.Sleep(time.Millisecond * 50)
			...
			failpoint.Eval(_curpkg_("prewriteSecondary")) // for other failures like sleep or pause
		}
	}
