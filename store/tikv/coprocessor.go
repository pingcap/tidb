...
func (worker *copIteratorWorker) run(ctx context.Context) {
	defer func() {
		if val, err2 := MockTicase4169.Eval(); err2 == nil {
			if val.(bool) {
...
func (it *copIterator) open(ctx context.Context, enabledRateLimitAction bool) {
	...
	if val, err2 := MockTicase4171.Eval(); err2 == nil {
		if val.(bool) {
	...
	for {
		...
		case resp, ok = <-respCh:
			if it.memTracker != nil && resp != nil {
				...
				if val, err2 := MockTestRateLimitActionMockConsumeAndAssert.Eval(); err2 == nil {
					if val.(bool) {
						if resp != finCopResp {
...
func (it *copIterator) Next(ctx context.Context) (kv.ResultSubset, error) {
	...
	defer func() {
		if resp == nil {
			if val, err2 := MockTicase4170.Eval(); err2 == nil {
				if val.(bool) {
					it.memTracker.Consume(10 * MockResponseSizeForTest)
					it.memTracker.Consume(10 * MockResponseSizeForTest)
				}
	...
	if val, err2 := MockTestRateLimitActionMockWaitMax.Eval(); err2 == nil {
		if val.(bool) {
...
func (worker *copIteratorWorker) handleTaskOnce(bo *Backoffer, task *copTask, ch chan<- *copResponse) ([]*copTask, error) {
	if val, err2 := MockHandleTaskOnceError.Eval(); err2 == nil {
		if val.(bool) {
