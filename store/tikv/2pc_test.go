...
func (s *testCommitterSuite) TestDeleteYourWriteCauseGhostPrimary(c *C) {
	...
	txn1 := s.begin(c)
	txn1.DelOption(option.Pessimistic)
	...
	txn2 := s.begin(c)
	txn2.DelOption(option.Pessimistic)
...
func (s *testCommitterSuite) TestAcquireFalseTimeoutLock(c *C) {
	...
	txn1 := s.begin(c)
	txn1.SetOption(option.Pessimistic, true)
	...
	txn2 := s.begin(c)
	txn2.SetOption(option.Pessimistic, true)
...
func (s *testCommitterSuite) TestPkNotFound(c *C) {
	...
	txn3 := s.begin(c)
	txn3.SetOption(option.Pessimistic, true)
