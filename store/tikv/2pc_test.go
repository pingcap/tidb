...
func (s *testCommitterSuite) TestUnsetPrimaryKey(c *C) {
	...
	err := txn.LockKeys(context.Background(), txn.startTS, config.LockAlwaysWait, key)
	c.Assert(err, NotNil)
...
func (s *testCommitterSuite) TestPessimisticLockedKeysDedup(c *C) {
	txn := s.begin(c)
	txn.SetOption(kv.Pessimistic, true)
	err := txn.LockKeys(context.Background(), 100, config.LockAlwaysWait, kv.Key("abc"), kv.Key("def"))
	c.Assert(err, IsNil)
	err = txn.LockKeys(context.Background(), 100, config.LockAlwaysWait, kv.Key("abc"), kv.Key("def"))
	c.Assert(err, IsNil)
...
func (s *testCommitterSuite) TestPessimisticTTL(c *C) {
	...
	time.Sleep(time.Millisecond * 100)
	key2 := kv.Key("key2")
	err = txn.LockKeys(context.Background(), txn.startTS, config.LockAlwaysWait, key2)
	c.Assert(err, IsNil)
