...
func (s *testCommitterSuite) TestPrewriteRollback(c *C) {
	...
	committer.commitTS, err = s.store.oracle.GetTimestamp(ctx, &oracle.Option{TxnScope: config.DefTxnScope})
	c.Assert(err, IsNil)
...
func (s *testCommitterSuite) TestPessimisticTTL(c *C) {
	...
	msBeforeLockExpired := s.store.GetOracle().UntilExpired(txn.StartTS(), lockInfo.LockTtl, &oracle.Option{TxnScope: config.DefTxnScope})
	...
	for i := 0; i < 50; i++ {
		...
		if lockInfoNew.LockTtl > lockInfo.LockTtl {
			currentTS, err := lr.store.GetOracle().GetTimestamp(bo.ctx, &oracle.Option{TxnScope: config.DefTxnScope})
			c.Assert(err, IsNil)
