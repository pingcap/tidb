...
func (s *testCommitterSuite) TestPrewriteRollback(c *C) {
	...
	c.Assert(err, IsNil)
	committer, err := newTwoPhaseCommitter(txn1, 0, false)
	c.Assert(err, IsNil)
...
func (s *testCommitterSuite) TestCommitBeforePrewrite(c *C) {
	...
	c.Assert(err, IsNil)
	commiter, err := newTwoPhaseCommitter(txn, 0, false)
	c.Assert(err, IsNil)
	ctx := context.Background()
...
func (s *testCommitterSuite) TestPrewritePrimaryKeyFailed(c *C) {
	...
	ctx := context.Background()
	commiter, err := newTwoPhaseCommitter(txn2, 0, false)
	c.Assert(err, IsNil)
	err = commiter.cleanupKeys(NewBackoffer(ctx, cleanupMaxBackoff), commiter.keys)
	c.Assert(err, IsNil)
...
func (s *testCommitterSuite) TestWrittenKeysOnConflict(c *C) {
	...
	for i := 0; i < 10; i++ {
		...
		commiter2, err := newTwoPhaseCommitter(txn2, 2, false)
		c.Assert(err, IsNil)
		err = commiter2.execute(context.Background())
		c.Assert(err, IsNil)
		txn1.Set([]byte("x1"), []byte("1"))
		txn1.Set([]byte("y1"), []byte("2"))
		commiter1, err := newTwoPhaseCommitter(txn1, 2, false)
		c.Assert(err, IsNil)
