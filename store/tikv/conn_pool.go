...
type Conn struct {
	...
	// TODO Use refCount to impl background idle Conn cleanup.
	refCount uint32
}
...
// TODO Implement background cleanup: add a backgroud goroutine to periodically check
...
func (p *ConnPool) Get(addr string) (*grpc.ClientConn, error) {
	...
	// Increment refCount.
	atomic.AddUint32(&conn.refCount, 1)
	return conn.c, nil
}
...
func (p *ConnPool) Put(addr string, c *grpc.ClientConn) {
	...
	// Decrement refCount.
	atomic.AddUint32(&conn.refCount, ^uint32(0))
}
