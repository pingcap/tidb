...
func (tm *ttlManager) keepAlive(c *twoPhaseCommitter) {
	...
	for {
		...
		case <-ticker.C:
			...
			now, err := c.store.GetOracle().GetTimestamp(bo.ctx, &oracle.Option{TxnScope: config.DefTxnScope})
			if err != nil {
...
func (c *twoPhaseCommitter) execute(ctx context.Context) (err error) {
	...
	if commitTSMayBeCalculated && config.GetGlobalConfig().TiKVClient.ExternalConsistency {
		minCommitTS, err := c.store.oracle.GetTimestamp(ctx, &oracle.Option{TxnScope: config.DefTxnScope})
	...
	if c.store.oracle.IsExpired(c.startTS, kv.MaxTxnTimeUse, &oracle.Option{TxnScope: config.DefTxnScope}) {
