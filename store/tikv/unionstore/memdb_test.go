...
func (s *testMemDBSuite) TestBigKV(c *C) {
	...
	err := db.Set([]byte{1}, make([]byte, 80<<20))
	c.Assert(err, IsNil)
	c.Assert(db.vlog.blockSize, Equals, maxBlockSize)
	c.Assert(len(db.vlog.blocks), Equals, 1)
	h := db.Staging()
	err = db.Set([]byte{2}, make([]byte, 127<<20))
	c.Assert(err, IsNil)
	db.Release(h)
	c.Assert(db.vlog.blockSize, Equals, maxBlockSize)
	c.Assert(len(db.vlog.blocks), Equals, 2)
	c.Assert(func() {
		err := db.Set([]byte{3}, make([]byte, maxBlockSize+1))
		c.Assert(err, IsNil)
	}, Panics, "alloc size is larger than max block size")
}
...
func (s *testMemDBSuite) TestKVLargeThanBlock(c *C) {
	db := newMemDB()
	err := db.Set([]byte{1}, make([]byte, 1))
	c.Assert(err, IsNil)
	err = db.Set([]byte{2}, make([]byte, 4096))
	c.Assert(err, IsNil)
	c.Assert(len(db.vlog.blocks), Equals, 2)
	err = db.Set([]byte{3}, make([]byte, 3000))
	c.Assert(err, IsNil)
	c.Assert(len(db.vlog.blocks), Equals, 2)
...
func (s *testMemDBSuite) TestDirty(c *C) {
	...
	db = newMemDB()
	h = db.Staging()
	err = db.SetWithFlags([]byte{1}, []byte{1}, kv.SetKeyLocked)
	c.Assert(err, IsNil)
	db.Cleanup(h)
	c.Assert(db.Dirty(), IsTrue)
	...
	db = newMemDB()
	h = db.Staging()
	err = db.SetWithFlags([]byte{1}, []byte{1}, kv.SetPresumeKeyNotExists)
	c.Assert(err, IsNil)
	db.Cleanup(h)
	c.Assert(db.Dirty(), IsFalse)
}
...
func (s *testMemDBSuite) TestFlags(c *C) {
	...
	for i := uint32(0); i < cnt; i++ {
		...
		if i%2 == 0 {
			err := db.SetWithFlags(buf[:], buf[:], kv.SetPresumeKeyNotExists, kv.SetKeyLocked)
			c.Assert(err, IsNil)
		} else {
			err := db.SetWithFlags(buf[:], buf[:], kv.SetPresumeKeyNotExists)
			c.Assert(err, IsNil)
		}
	}
	db.Cleanup(h)
...
func (s *testKVSuite) TestBufferBatchGetter(c *C) {
	...
	kc := []byte("c")
	kd := []byte("d")
	err := snap.store.Set(ka, ka)
	c.Assert(err, IsNil)
	err = snap.store.Set(kb, kb)
	c.Assert(err, IsNil)
	err = snap.store.Set(kc, kc)
	c.Assert(err, IsNil)
	err = snap.store.Set(kd, kd)
	c.Assert(err, IsNil)

	// middle value is the same as snap
	middle := newMemDB()
	err = middle.Set(ka, []byte("a1"))
	c.Assert(err, IsNil)
	err = middle.Set(kc, []byte("c1"))
	c.Assert(err, IsNil)

	buffer := newMemDB()
	err = buffer.Set(ka, []byte("a2"))
	c.Assert(err, IsNil)
	err = buffer.Delete(kb)
	c.Assert(err, IsNil)
