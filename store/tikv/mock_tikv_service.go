...
func (s *server) BatchCommands(ss tikvpb.Tikv_BatchCommandsServer) error {
	for {
		req, err := ss.Recv()
		if err != nil {
			logutil.Logger(context.Background()).Error("batch commands receive fail", zap.Error(err))
			return err
		}
		...
		if err != nil {
			logutil.Logger(context.Background()).Error("batch commands send fail", zap.Error(err))
			return err
		}
	}
}
...
func startMockTikvService() (*grpc.Server, int) {
	for port := 40000; port < 50000; port++ {
		...
		if err != nil {
			logutil.Logger(context.Background()).Error("can't listen", zap.Error(err))
			continue
		}
	...
	logutil.Logger(context.Background()).Error("can't start mock tikv service because no available ports")
	return nil, -1
}
