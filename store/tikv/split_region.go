...
func (s *KVStore) scatterRegion(bo *Backoffer, regionID uint64, tableID *int64) error {
	...
	for {
		...
		err = bo.BackoffWithCfg(retry.BoPDRPC, errors.New(err.Error()))
		if err != nil {
...
func (s *KVStore) WaitScatterRegionFinish(ctx context.Context, regionID uint64, backOff int) error {
	...
	for {
		...
		if err != nil {
			err = bo.BackoffWithCfg(retry.BoRegionMiss, errors.New(err.Error()))
		} else {
			err = bo.BackoffWithCfg(retry.BoRegionMiss, errors.New("wait scatter region timeout"))
		}
		if err != nil {
			return errors.Trace(err)
		}
