...
func (rs *localRegion) getRowsFromSelectReq(txn kv.Transaction, sel *tipb.SelectRequest) ([]*tipb.Row, error) {
	...
	for _, handle := range handles {
		row, err := rs.getRowFromHandle(txn, tid, handle, sel.TableInfo.Columns)
		if err != nil {
			return nil, errors.Trace(err)
		}
...
func (rs *localRegion) extractRecordKVRanges(tid int64, krans []*tipb.KeyRange) []kv.KeyRange {
...
func (rs *localRegion) getRowFromHandle(txn kv.Transaction, tid, handle int64, columns []*tipb.ColumnInfo) (*tipb.Row, error) {
	row := new(tipb.Row)
	var d types.Datum
	d.SetInt64(handle)
	var err error
...
func (rs *localRegion) getRowsFromIndexReq(txn kv.Transaction, sel *tipb.SelectRequest) ([]*tipb.Row, error) {
	...
	kvRanges := rs.extractIndexKVRanges(tid, idxID, sel.Ranges)
	var rows []*tipb.Row
	for _, ran := range kvRanges {
...
func (rs *localRegion) extractIndexKVRanges(tid, idxID int64, krans []*tipb.KeyRange) []kv.KeyRange {
	var kvRanges []kv.KeyRange
	for _, kran := range krans {
