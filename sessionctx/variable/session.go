...
func (s *SessionVars) SetSystemVar(name string, val string) error {
	...
	case CollationConnection, CollationDatabase, CollationServer:
		...
		// There is an issue where collation could be set without calling validate.
		// In which case, the SetSystemVar will substitute the default character set
		// if the character set is invalid. This requires the "val" to be changed in the
		// SetSession function, which is currently not permitted by the API (intentionally).
		// Values should only be modified in the Validate phase of setting variables.
		// This should be fixed by having the caller not call SetSystemVar() directly, but the
		// validate function first. Then it can discover that the validate failed,
		// set the default collation itself.
