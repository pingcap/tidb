...
const (
	...
	//Currently, CTE can only be inlined through MERGE() hint. But for those queries that have many CTEs,
	//and queries that are generated by BI, it is better to have a system variable to control whether to inline CTE or not.
	//When the variable is turned on, we will expand inline for all CTEs.
