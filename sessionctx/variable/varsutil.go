...
func GetSessionOnlySysVars(s *SessionVars, key string) (string, bool, error) {
	...
	case TiDBShowPlanInSlowLog:
		return strconv.FormatUint(uint64(atomic.LoadUint32(&config.GetGlobalConfig().Log.ShowPlanInSlowLog)), 10), true, nil
...
func ValidateSetSystemVar(vars *SessionVars, name string, value string) (string, error) {
	...
	case TiDBSkipUTF8Check, TiDBOptAggPushDown,
		...
		TiDBScatterRegion, TiDBGeneralLog, TiDBConstraintCheckInPlace, TiDBEnableVectorizedExpression, TiDBShowPlanInSlowLog:
		fallthrough
