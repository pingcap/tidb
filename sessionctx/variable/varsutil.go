...
func GetSessionOnlySysVars(s *SessionVars, key string) (string, bool, error) {
	...
	case TiDBSlowLogPlan:
		return strconv.FormatUint(uint64(atomic.LoadUint32(&config.GetGlobalConfig().Log.SlowLogPlan)), 10), true, nil
...
func ValidateSetSystemVar(vars *SessionVars, name string, value string) (string, error) {
	...
	case TiDBSkipUTF8Check, TiDBOptAggPushDown,
		...
		TiDBScatterRegion, TiDBGeneralLog, TiDBConstraintCheckInPlace, TiDBEnableVectorizedExpression, TiDBSlowLogPlan:
		fallthrough
