...
func GetPumpsClientByLogBin() (*pumpcli.PumpsClient, error) {
	var binlogCli *pumpcli.PumpsClient
	var err error
	if ShouldEnableBinlog() {
		binlogCli, err = GetOrCreatePumpsClient()
	}
	return binlogCli, err
}

// GetOrCreatePumpsClient get or create the pumps client instance
func GetOrCreatePumpsClient() (*pumpcli.PumpsClient, error) {
	pumpsClientLock.RLock()
	client := GetPumpsClient()
	pumpsClientLock.RUnlock()
	if client == nil {
		pumpsClientLock.Lock()
		defer pumpsClientLock.Unlock()
		if pumpsClient != nil {
			client = pumpsClient
		} else {
			var err error
