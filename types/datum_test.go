...
func (ts *testDatumSuite) TestToBytes(c *C) {
	tests := []struct {
		...
		{NewIntDatum(1), 0, []byte("1")},
		{NewDecimalDatum(NewDecFromInt(1)), 0, []byte("1")},
		{NewFloat64Datum(1.23), 2, []byte("1.23")},
		{NewStringDatum("abc"), 0, []byte("abc")},
	}
	sc := new(stmtctx.StatementContext)
	sc.IgnoreTruncate = true
	for _, tt := range tests {
		bin, err := tt.a.ToBytes(tt.fsp)
		c.Assert(err, IsNil)
