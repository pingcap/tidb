...
func TestConvertType(t *testing.T) {
	...
	assert.True(t, ErrDataTooLong.Equal(err))
	...
	assert.Nil(t, err)
	assert.Equal(t, float32(111.11), v)
	...
	assert.Equal(t, float32(999.99), v)
	...
	assert.Nil(t, err)
	assert.Equal(t, float32(999.92), v)
	...
	assert.Nil(t, err)
	assert.Equal(t, float32(999.91), v)
	...
	v, err = Convert(999.9155, ft)
	assert.Nil(t, err)
	assert.Equal(t, float64(999.92), v)
	...
	assert.Nil(t, err)
	assert.Equal(t, "10:11:12.123", v.(Duration).String())
	ft.Decimal = 1
	vv, err := Convert(v, ft)
	assert.Nil(t, err)
	assert.Equal(t, "10:11:12.1", vv.(Duration).String())
	sc := &stmtctx.StatementContext{TimeZone: time.UTC}
	vd, err := ParseTime(sc, "2010-10-10 10:11:11.12345", mysql.TypeDatetime, 2)
	assert.Equal(t, "2010-10-10 10:11:11.12", vd.String())
	assert.Nil(t, err)
	v, err = Convert(vd, ft)
	assert.Nil(t, err)
	assert.Equal(t, "10:11:11.1", v.(Duration).String())

	vt, err := ParseTime(sc, "2010-10-10 10:11:11.12345", mysql.TypeTimestamp, 2)
	assert.Equal(t, "2010-10-10 10:11:11.12", vt.String())
	assert.Nil(t, err)
	v, err = Convert(vt, ft)
	assert.Nil(t, err)
	assert.Equal(t, "10:11:11.1", v.(Duration).String())
	...
	assert.Nil(t, err)
	assert.Equal(t, "2010-10-10 10:11:11.123", v.(Time).String())
	ft.Decimal = 1
	vv, err = Convert(v, ft)
	assert.Nil(t, err)
	assert.Equal(t, "2010-10-10 10:11:11.1", vv.(Time).String())
	...
	v, err = Convert("100", ft)
	assert.Nil(t, err)
	assert.Equal(t, NewBinaryLiteralFromUint(3223600, 3), v)

	v, err = Convert(NewBinaryLiteralFromUint(100, -1), ft)
	assert.Nil(t, err)
	assert.Equal(t, NewBinaryLiteralFromUint(100, 3), v)

	ft.Flen = 1
	v, err = Convert(1, ft)
	assert.Nil(t, err)
	assert.Equal(t, NewBinaryLiteralFromUint(1, 1), v)
	...
	assert.Nil(t, err)
	assert.Equal(t, "3.1416", v.(*MyDecimal).String())
	v, err = Convert("3.1415926", ft)
	assert.Nil(t, err)
	assert.Equal(t, "3.1416", v.(*MyDecimal).String())
	v, err = Convert("99999", ft)
	assert.True(t, terror.ErrorEqual(err, ErrOverflow))
	assert.Equal(t, "9999.9999", v.(*MyDecimal).String())
	v, err = Convert("-10000", ft)
	assert.True(t, terror.ErrorEqual(err, ErrOverflow))
	assert.Equal(t, "-9999.9999", v.(*MyDecimal).String())
	v, err = Convert("1,999.00", ft)
	assert.True(t, terror.ErrorEqual(err, ErrBadNumber))
	assert.Equal(t, "1.0000", v.(*MyDecimal).String())
	v, err = Convert("1,999,999.00", ft)
	assert.True(t, terror.ErrorEqual(err, ErrBadNumber))
	assert.Equal(t, "1.0000", v.(*MyDecimal).String())
	v, err = Convert("199.00 ", ft)
	assert.Nil(t, err)
	assert.Equal(t, "199.0000", v.(*MyDecimal).String())
	...
	assert.Nil(t, err)
	assert.Equal(t, "0", v.(*MyDecimal).String())
	...
	assert.Nil(t, err)
	assert.Equal(t, int64(time.Now().Year()), v)
	...
	v, err = Convert("a", ft)
	assert.Nil(t, err)
	assert.Equal(t, Enum{Name: "a", Value: 1}, v)
	v, err = Convert(2, ft)
	t.Log(errors.ErrorStack(err))
	assert.Nil(t, err)
	assert.Equal(t, Enum{Name: "b", Value: 2}, v)
	_, err = Convert("d", ft)
	assert.NotNil(t, err)
	v, err = Convert(4, ft)
	assert.True(t, terror.ErrorEqual(err, ErrTruncated))
	assert.Equal(t, Enum{}, v)

	ft = NewFieldType(mysql.TypeSet)
	ft.Elems = []string{"a", "b", "c"}
	v, err = Convert("a", ft)
	assert.Nil(t, err)
	assert.Equal(t, Set{Name: "a", Value: 1}, v)
	v, err = Convert(2, ft)
	assert.Nil(t, err)
	assert.Equal(t, Set{Name: "b", Value: 2}, v)
	v, err = Convert(3, ft)
	assert.Nil(t, err)
	assert.Equal(t, Set{Name: "a,b", Value: 3}, v)
	_, err = Convert("d", ft)
	assert.NotNil(t, err)
	_, err = Convert(9, ft)
	assert.NotNil(t, err)
...
func TestConvertToString(t *testing.T) {
	...
	time, err := ParseTime(&stmtctx.StatementContext{TimeZone: time.UTC},
		"2011-11-10 11:11:11.999999", mysql.TypeTimestamp, 6)
	assert.Nil(t, err)
	testToString(t, time, "2011-11-10 11:11:11.999999")
	...
	for _, tt := range tests {
		...
		assert.Equal(t, tt.output, outputDatum.GetString())
	}
}
...
func testStrToInt(t *testing.T, str string, expect int64, truncateAsErr bool, expectErr error) {
	...
	if expectErr != nil {
		assert.True(t, terror.ErrorEqual(err, expectErr))
	} else {
		assert.Nil(t, err)
...
func testSelectUpdateDeleteEmptyStringError(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		val, err := StrToInt(sc, str, false)
		assert.Nil(t, err)
		assert.Equal(t, int64(expect), val)

		val1, err := StrToUint(sc, str, false)
		assert.Nil(t, err)
		assert.Equal(t, uint64(expect), val1)

		val2, err := StrToFloat(sc, str, false)
		assert.Nil(t, err)
		assert.Equal(t, float64(expect), val2)
	}
}
...
func TestRoundIntStr(t *testing.T) {
	...
	for _, cc := range cases {
		assert.Equal(t, cc.c, roundIntStr(cc.b, cc.a))
	}
}
...
func TestGetValidInt(t *testing.T) {
	...
	for _, tt := range tests {
		...
		assert.Nil(t, err)
		assert.Equal(t, tt.valid, prefix)
		if tt.signed {
		...
		if tt.warning {
			assert.Len(t, warnings, warningCount+1)
			assert.True(t, terror.ErrorEqual(warnings[len(warnings)-1].Err, ErrTruncatedWrongVal))
			warningCount += 1
		} else {
			assert.Len(t, warnings, warningCount)
		}
	}
	...
	for _, tt := range tests2 {
		prefix, err := getValidIntPrefix(sc, tt.origin, false)
		if tt.warning {
			assert.True(t, terror.ErrorEqual(err, ErrTruncatedWrongVal))
		} else {
			assert.Nil(t, err)
		}
		assert.Equal(t, tt.valid, prefix)
...
func TestGetValidFloat(t *testing.T) {
	...
	for _, test := range tests2 {
		str, err := floatStrToIntStr(sc, test.origin, test.origin)
		assert.Nil(t, err)
...
func testConvertTimeTimeZone(t *testing.T, sc *stmtctx.StatementContext) {
	...
	for _, test := range tests {
		...
		assert.Nil(t, err)
		time := nd.GetMysqlTime()
		assert.Equal(t, test.expect.Type(), time.Type())
		assert.Equal(t, test.expect.CoreTime(), time.CoreTime())
	}
}
...
func TestConvertJSONToFloat(t *testing.T) {
	...
	for _, tt := range tests {
		...
		assert.Equal(t, tt.ty, j.TypeCode)
...
func TestNumberToDuration(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		assert.Nil(t, err)
		assert.Equal(t, tc.hour, dur.Hour())
		assert.Equal(t, tc.minute, dur.Minute())
		assert.Equal(t, tc.second, dur.Second())
	}
	...
	for _, tc := range testCases1 {
		...
		assert.Nil(t, err)
		assert.Equal(t, tc.dur, dur.Duration)
	}
}
...
func TestStrToDuration(t *testing.T) {
	...
	for _, tt := range tests {
		...
		assert.Nil(t, err)
		assert.Equal(t, tt.isDuration, isDuration)
	}
}
...
func TestConvertDecimalStrToUint(t *testing.T) {
	...
	for _, ca := range cases {
		...
		if !ca.succ {
			...
			assert.Nil(t, err)
			assert.Equal(t, ca.result, result)
		}
	}
}
