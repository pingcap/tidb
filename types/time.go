...
func ParseDateFormat(format string) []string {
	...
	for i := 1; i < len(format); i++ {
		if len(seps) > maxSQLTimeParts { //max len is 7,'Y-M-D H-M-S.ms'
			break
		}
...
func parseWhenLenLess6(format string, lastSepChar uint8, currentSepChar uint8, currentPart string, totalPartCountIn int, sepsIn []string) (totalPartCount int, seps []string, warnMsg string, err error) {
	seps = sepsIn
	totalPartCount = totalPartCountIn
	if lastSepChar == '-' || currentSepChar == '-' {
		seps = append(seps, currentPart)
...
func parseOnePart(format string, lastSepChar uint8, currentSepChar uint8, selfDecideFsp bool, currentPart string, realFspIn int, totalPartCountIn int, sepsIn []string) (realFsp, totalPartCount int, seps []string, warnMsg string, err error) {
	realFsp = realFspIn
	totalPartCount = totalPartCountIn
	seps = sepsIn
	if len(seps) == 0 {
		...
		if lastSepChar == '.' { //'2017.0112 00~00~00.333' -> 2017-01-12 00:00:00.333
			totalPartCount, seps, warnMsg, err = parseWhenLenLess6(format, lastSepChar, currentSepChar, currentPart, totalPartCount, seps)
...
func nextTimePart(format string, start, totalPartCount int, lastSepChar uint8) (currentSepChar uint8, currentPart string, nextStart int, warnMsg string, err error) {
	var i = start + 1
...
func splitDateTime(format string, fsp int, selfDecideFsp bool, isFloat bool) (seps []string, realFsp int, warnMsg string, err error) {
	format = strings.TrimSpace(format)
	realFsp = fsp
	...
	for {
		...
		var currentSepChar, currentPart, nextStart, warnMsgRet, errRet = nextTimePart(format, start, totalPartCount, lastSepChar)
		err = errRet
		warnMsg = warnMsgRet
		if err != nil || warnMsg != "" {
			return
		...
		realFsp, totalPartCount, seps, warnMsg, err = parseOnePart(format, lastSepChar, currentSepChar, selfDecideFsp, currentPart, realFsp, totalPartCount, seps)
		if err != nil {
			return
		}
...
func parseNoDelimiterHavingYear(strNoDelimiter string) ([7]string, int, error) {
	...
	var minute = &seps[4]
	var second = &seps[5]
	var microsecond = &seps[6]

	len := len(strNoDelimiter)
	if len > 15 {
		_, err = fmt.Sscanf(strNoDelimiter, "%4s%2s%2s%2s%2s%2s%2s", year, month, day, hour, minute, second, microsecond)
		validLen = 7
	}
	switch len {
	case 15: // YYYYMMDDHHMMSS+(MS)
		_, err = fmt.Sscanf(strNoDelimiter, "%4s%2s%2s%2s%2s%2s%1s", year, month, day, hour, minute, second, microsecond)
		validLen = 7
	case 14: // No delimiter.
		// YYYYMMDDHHMMSS
		_, err = fmt.Sscanf(strNoDelimiter, "%4s%2s%2s%2s%2s%2s", year, month, day, hour, minute, second)
		validLen = 6
	case 13: //YYMMDDHHMMSS(MS), ex:2011011812233->2020-11-01 18:12:23.300000
		_, err = fmt.Sscanf(strNoDelimiter, "%2s%2s%2s%2s%2s%2s%1s", year, month, day, hour, minute, second, microsecond)
		validLen = 7

	case 12: // YYMMDDHHMMSS
		_, err = fmt.Sscanf(strNoDelimiter, "%2s%2s%2s%2s%2s%2s", year, month, day, hour, minute, second)
		validLen = 6
	case 11: // YYMMDDHHMMS
		_, err = fmt.Sscanf(strNoDelimiter, "%2s%2s%2s%2s%2s%1s", year, month, day, hour, minute, second)
		validLen = 6

	case 10: // YYMMDDHHMM
		_, err = fmt.Sscanf(strNoDelimiter, "%2s%2s%2s%2s%2s", year, month, day, hour, minute)
		validLen = 5

	case 9: // YYMMDDHHM
		_, err = fmt.Sscanf(strNoDelimiter, "%2s%2s%2s%2s%1s", year, month, day, hour, minute)
		validLen = 5

	case 8: // YYYYMMDD
		_, err = fmt.Sscanf(strNoDelimiter, "%4s%2s%2s", year, month, day)
		validLen = 3
	case 7: // YYMMDDH
		_, err = fmt.Sscanf(strNoDelimiter, "%2s%2s%2s%1s", year, month, day, hour)
		validLen = 4

	case 6:
		// YYMMDD
		_, err = fmt.Sscanf(strNoDelimiter, "%2s%2s%2s", year, month, day)
		validLen = 3

	case 5: // YYMMD
		_, err = fmt.Sscanf(strNoDelimiter, "%2s%2s%1s", year, month, day)
		validLen = 3

	case 4: //YYYYYY
		_, err = fmt.Sscanf(strNoDelimiter, "%4s", year)
		validLen = 1
	case 3: //YYD
		_, err = fmt.Sscanf(strNoDelimiter, "%2s%1s", year, month)
		validLen = 2

	case 2: //YY
		_, err = fmt.Sscanf(strNoDelimiter, "%2s", year)
		validLen = 1

	case 1: //Y
		_, err = fmt.Sscanf(strNoDelimiter, "%1s", year)
		validLen = 1

	default:
		err = errors.Trace(ErrInvalidTimeFormat.GenWithStackByArgs(strNoDelimiter))
	}
...
// See https://dev.mysql.com/doc/refman/5.7/en/date-and-time-literals.html.
func parseDatetime(sc *stmtctx.StatementContext, str string, fsp int, isFloat bool, selfDecideFsp bool) (Time, error) {
	var (
		year, month, day, hour, minute, second, microsecond int
		err                                                 error
	)

	seps, realFsp, warnMsg, err := splitDateTime(str, fsp, selfDecideFsp, isFloat)
	if err != nil {
		return ZeroDatetime, errors.Trace(err)
	}
	if warnMsg != "" {
		sc.AppendWarning(ErrTruncatedWrongValue.GenWithStackByArgs(warnMsg, str))
	}

	var overflow = false
	if len(seps) >= 7 {
		err = scanTimeArgs(seps, &year, &month, &day, &hour, &minute, &second, &microsecond)
		microsecond, overflow, err = ParseFrac(seps[6], realFsp)
		if err != nil {
			return ZeroDatetime, errors.Trace(err)
		}
	} else {
...
func parseTimeNoFsp(sc *stmtctx.StatementContext, str string, tp byte, isFloat bool) (Time, error) {
	var fsp = MinFsp
	var selfDecideFsp = true
	return parseTimeDecideFsp(sc, str, tp, fsp, isFloat, selfDecideFsp)
}

func parseTimeDecideFsp(sc *stmtctx.StatementContext, str string, tp byte, fsp int, isFloat bool, selfDecideFsp bool) (Time, error) {
	fsp, err := CheckFsp(fsp)
	if err != nil {
		return Time{Time: ZeroTime, Type: tp}, errors.Trace(err)
	}

	t, err := parseDatetime(sc, str, fsp, isFloat, selfDecideFsp)
	if err != nil {
		return Time{Time: ZeroTime, Type: tp}, errors.Trace(err)
	}
