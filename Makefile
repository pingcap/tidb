...
vet:
	@echo "vet"
	$(GO) vet -all $(PACKAGE_LIST_TIDB_TESTS) 2>&1 | $(FAIL_ON_STDOUT)
...
# - 'ok ' to ignore passed directories
	...
	$(GOTEST) -ldflags '$(TEST_LDFLAGS)' $(EXTRA_TEST_ARGS) -cover $(PACKAGE_LIST_TIDB_TESTS) -check.p true > gotest.log || { $(FAILPOINT_DISABLE); grep -v '^\([[]20\|PASS:\|ok \)' 'gotest.log'; exit 1; }
	@$(FAILPOINT_DISABLE)
...
gotest: failpoint-enable
	@echo "Running in native mode."
	@export log_level=info; export TZ='Asia/Shanghai'; \
	$(GOTEST) -ldflags '$(TEST_LDFLAGS)' $(EXTRA_TEST_ARGS) -timeout 20m -cover $(PACKAGE_LIST_TIDB_TESTS) -coverprofile=coverage.txt -check.p true > gotest.log || { $(FAILPOINT_DISABLE); cat 'gotest.log'; exit 1; }
	@$(FAILPOINT_DISABLE)
...
gotest_in_verify_ci: failpoint-enable tools/bin/gotestsum
	...
	$(PACKAGE_LIST_TIDB_TESTS) -check.p true || { $(FAILPOINT_DISABLE); exit 1; }
	@$(FAILPOINT_DISABLE)
