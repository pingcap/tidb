...
bazel_brietest: failpoint-enable bazel_ci_simple_prepare
	bazel $(BAZEL_GLOBAL_CONFIG) test $(BAZEL_CMD_CONFIG) --test_arg=-with-real-tikv --noremote_accept_cached --define gotags=$(REAL_TIKV_TEST_TAGS) \
...
bazel_pessimistictest: failpoint-enable bazel_ci_simple_prepare
	bazel $(BAZEL_GLOBAL_CONFIG) coverage $(BAZEL_CMD_CONFIG) $(BAZEL_INSTRUMENTATION_FILTER) --test_arg=-with-real-tikv --noremote_accept_cached --@io_bazel_rules_go//go/config:cover_format=go_cover --define gotags=$(REAL_TIKV_TEST_TAGS) \
...
bazel_addindextest: failpoint-enable bazel_ci_simple_prepare
	bazel $(BAZEL_GLOBAL_CONFIG) coverage $(BAZEL_CMD_CONFIG) $(BAZEL_INSTRUMENTATION_FILTER) --test_output=all --test_arg=-with-real-tikv --noremote_accept_cached --@io_bazel_rules_go//go/config:cover_format=go_cover --define gotags=$(REAL_TIKV_TEST_TAGS) \
...
bazel_pipelineddmltest: failpoint-enable bazel_ci_simple_prepare
	bazel $(BAZEL_GLOBAL_CONFIG) test $(BAZEL_CMD_CONFIG) --test_output=all --test_arg=-with-real-tikv --noremote_accept_cached --define gotags=$(REAL_TIKV_TEST_TAGS) \
