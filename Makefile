...
.PHONY: bazel_test
# Extra Bazel flags for Bazel-based unit tests (e.g. set to `--noremote_accept_cached` to reduce remote-cache interference when profiling).
BAZEL_UNIT_TEST_EXTRA_FLAGS ?=
bazel_test: bazel-failpoint-enable bazel_prepare ## Run all tests using Bazel
	bazel $(BAZEL_GLOBAL_CONFIG) test $(BAZEL_CMD_CONFIG) $(BAZEL_UNIT_TEST_EXTRA_FLAGS) --build_tests_only --test_keep_going=false \
...
bazel_coverage_test: bazel-failpoint-enable bazel_ci_simple_prepare
	bazel $(BAZEL_GLOBAL_CONFIG) --nohome_rc coverage $(BAZEL_CMD_CONFIG) $(BAZEL_UNIT_TEST_EXTRA_FLAGS) $(BAZEL_INSTRUMENTATION_FILTER) --jobs=35 --build_tests_only --test_keep_going=false \
