...
bazel_brietest: bazel_integration_test_prepare
	bazel $(BAZEL_GLOBAL_CONFIG) coverage $(BAZEL_CMD_CONFIG) --combined_report=lcov --test_arg=-with-real-tikv --define gotags=deadlock,intest \
...
bazel_importintotest: bazel_integration_test_prepare
	bazel $(BAZEL_GLOBAL_CONFIG) coverage $(BAZEL_CMD_CONFIG) --combined_report=lcov --test_arg=-with-real-tikv --test_output=all --define gotags=deadlock,intest \
