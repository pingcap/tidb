...
bazel_coverage_test: bazel-failpoint-enable bazel_ci_simple_prepare
	bazel $(BAZEL_GLOBAL_CONFIG) --nohome_rc coverage $(BAZEL_CMD_CONFIG) $(BAZEL_INSTRUMENTATION_FILTER) --jobs=HOST_CPUS*0.7 --build_tests_only --test_keep_going=false --remote_accept_cached=false \
...
bazel_coverage_test_ddlargsv1: bazel-failpoint-enable bazel_ci_simple_prepare
	bazel $(BAZEL_GLOBAL_CONFIG) --nohome_rc coverage $(BAZEL_CMD_CONFIG) $(BAZEL_INSTRUMENTATION_FILTER) --jobs=HOST_CPUS*0.7 --build_tests_only --test_keep_going=false \
		--combined_report=lcov \
