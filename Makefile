...
bazel_test: bazel-failpoint-enable bazel_prepare ## Run all tests using Bazel
	bazel $(BAZEL_GLOBAL_CONFIG) test $(BAZEL_CMD_CONFIG) --noremote_accept_cached --build_tests_only --test_keep_going=false \
...
bazel_coverage_test: bazel-failpoint-enable bazel_ci_simple_prepare
	bazel $(BAZEL_GLOBAL_CONFIG) --nohome_rc coverage $(BAZEL_CMD_CONFIG) --noremote_accept_cached $(BAZEL_INSTRUMENTATION_FILTER) --jobs=35 --build_tests_only --test_keep_going=false \
