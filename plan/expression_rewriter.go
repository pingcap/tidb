...
func getRowLen(e expression.Expression) int {
	if f, ok := e.(expression.ScalarFunction); ok && f.GetName().L == ast.RowFunc {
...
func (er *expressionRewriter) Enter(inNode ast.Node) (ast.Node, bool) {
	...
	case *ast.ParenthesesExpr:
	case *ast.ValuesExpr:
		expr := expression.NewValuesFunction(v)
...
func (er *expressionRewriter) Leave(inNode ast.Node) (retNode ast.Node, ok bool) {
	...
	case *ast.FuncCastExpr:
		...
		er.checkArgsOneColumn(expr)
		if er.err != nil {
			break
		}
		expr, er.err = expression.NewCastFunction(v, expr)
		er.ctxStack[len(er.ctxStack)-1] = expr
