...
func (er *expressionRewriter) Enter(inNode ast.Node) (ast.Node, bool) {
	...
	case *ast.ParenthesesExpr:
	case *ast.ValuesExpr:
		er.ctxStack = append(er.ctxStack, expression.NewValuesFunc(v, er.ctx))
		return inNode, true
	default:
		er.asScalar = true
	}
...
func (er *expressionRewriter) rewriteVariable(v *ast.VariableExpr) {
	...
	if !v.IsSystem {
		if v.Value != nil {
			er.ctxStack[stkLen-1], er.err = expression.NewFunction(er.ctx, ast.SetVar,
		...
		if _, ok := sessionVars.Users[name]; ok {
			f, err := expression.NewFunction(er.ctx, ast.GetVar,
