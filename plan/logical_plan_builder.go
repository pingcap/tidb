...
func EliminateProjection(p LogicalPlan) LogicalPlan {
	...
	for _, expr := range exprs {
		if col, ok := expr.(*expression.Column); ok { // only fields in PROJECTION are all Columns might be eliminated.
	...
	if len(newSchema) != len(child.GetSchema()) { // detect expression like "SELECT c AS a, c AS b FROM t" which cannot be eliminated.
		canBeEliminated = false
	}
	if !canBeEliminated {
		return p
	}
	for _, col := range child.GetSchema() { // detect expression like "SELECT c AS a, c AS b FROM t WHERE d = 1" which cannot be eliminated.
