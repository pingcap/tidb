...
func (s *testSuite) testIndex(c *C, tb table.Table, idx table.Index) {
	...
	c.Assert(err, IsNil)
	_, key := tablecodec.EncodeRowKey(nil, tb.Meta().ID, codec.EncodeInt(nil, 4))
	...
	c.Assert(err, IsNil)
	_, key = tablecodec.EncodeRowKey(nil, tb.Meta().ID, codec.EncodeInt(nil, 3))
	...
	_, key = tablecodec.EncodeRowKey(nil, tb.Meta().ID, codec.EncodeInt(nil, 3))
	txn.Delete(key)
	_, key = tablecodec.EncodeRowKey(nil, tb.Meta().ID, codec.EncodeInt(nil, 5))
